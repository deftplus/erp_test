...
Процедура МатериалыИУслугиВариантОбеспеченияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ТекущиеДанные.ТипНоменклатуры = ПредопределенноеЗначение("Перечисление.ТипыНоменклатуры.Работа") Тогда
		...
		МассивДействий.Добавить(ПредопределенноеЗначение("Перечисление.ВариантыОбеспечения.Отгрузить"));
		
		НедопустимыеДействия = Новый ФиксированныйМассив(МассивДействий);
		НовыйПараметр = Новый ПараметрВыбора("НедопустимыеДействия", НедопустимыеДействия);
...
Функция ПараметрыФормыЗапросаКоличестваИСерий(Режим)
	...
	ПараметрыФормы.ДопКолонкиДляУказанияСерий = Новый Структура("ЗаказатьНаСклад,ПроизводитсяВПроцессе,Подразделение");
	...
	Для Каждого Изменение Из Изменения Цикл
		...
		Если ТекСтрока.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Работа
				...
				ТекСтрока.ВариантОбеспечения =
					?(ТекСтрока.Обособленно, Перечисления.ВариантыОбеспечения.Требуется, Перечисления.ВариантыОбеспечения.НеТребуется);
...
Процедура ЗарезервироватьЗапланированноеКПолучению(ТаблицаМатериалов)
	...
	ПорядокВыборкиВариантовОбеспечения.Добавить(
		Новый Структура("ВариантОбеспечения, Обособленно", Перечисления.ВариантыОбеспечения.Требуется, Ложь));
	ПорядокВыборкиВариантовОбеспечения.Добавить(
		Новый Структура("ВариантОбеспечения, Обособленно", Перечисления.ВариантыОбеспечения.Требуется, Истина));
