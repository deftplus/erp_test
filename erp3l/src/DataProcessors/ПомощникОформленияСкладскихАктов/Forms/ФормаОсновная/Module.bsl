	...
	Если ЗначениеЗаполнено(Объект.Склад) Тогда
		Если Не СкладыСервер.ИспользоватьОрдернуюСхемуПриОтраженииИзлишковНедостач(Объект.Склад) Тогда
			...
			ТекстСообщения = НСтр("ru = 'На складе %Склад% не используется ордерная схема при оформлении излишков, недостач и порчи, поэтому невозможно использовать помощник оформления складских актов.';
									|en = 'Advanced warehousing is not used in warehouse ""%Склад%"" while recording surpluses, shortages and damage, so it is impossible to use the warehouse act creation wizard.'");
	...
	Если ТекущаяСтраница = Элементы.СтраницаНастройки Тогда
		...
		Если ДатаОформленияСкладскихАктов < ИнвентаризационныйПериод.ДатаНачала 
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Выберите дату оформления складских актов в рамках инвентаризационного периода.';
																	|en = 'Select warehouse act generation date under the physical inventory count period.'"), , "ДатаОформленияСкладскихАктов");
			ЕстьОшибки = Истина;
		КонецЕсли;
...
Процедура УправлениеДекорациямиПоТоварам()
	...
	Если Не ЗначениеЗаполнено(Объект.Склад) И Не ЗначениеЗаполнено(ИнвентаризационныйПериод) Тогда 
		ЗаголовокДекорации = НСтр("ru = 'Заполните поля ""Склад"" и ""Период инвентаризационной описи"" для определения товаров, по которым необходимо оформить складские акты.';
									|en = 'Fill in fields ""Warehouse"" and ""Physical inventory sheet period"" to determine goods for which it is required to register warehouse acts.'");
	ИначеЕсли ЗначениеЗаполнено(Объект.Склад) И Не ЗначениеЗаполнено(ИнвентаризационныйПериод) Тогда 
		ЗаголовокДекорации = НСтр("ru = 'Заполните поле ""Период инвентаризационной описи"" для определения товаров, по которым необходимо оформить складские акты.';
									|en = 'Fill in field ""Physical inventory sheet period"" to determine goods for which it is required to register warehouse acts.'");
...
Процедура УстановитьОтображениеЗаголовкаИЭлементовУправления()
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьКачествоТоваров") Тогда
		ЗаголовкиСтраниц.Вставить(Элементы.СтраницаПересортицаПорча,		НСтр("ru = 'Зачет по пересортице и порче.';
																				|en = 'Offset by regrading and damage.'"));
	Иначе
