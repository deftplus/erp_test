...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ОткрытиеПоКомандеОбработки Тогда
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ДокументСсылка, "ПорядокРасчетов") Тогда 
			Порядок = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка, "ПорядокРасчетов");
			Если Порядок = Перечисления.ПорядокРасчетов.ПоДоговорамКонтрагентов 
				Или Порядок = Перечисления.ПорядокРасчетов.ПоДоговорамНакладным Тогда
				ОбъектСсылка = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументСсылка, "Договор");
			КонецЕсли;;
		КонецЕсли;
		ОбъектРасчетов = ОбъектыРасчетовСервер.ПолучитьОбъектРасчетовПоСсылке(ОбъектСсылка);
...
Функция ТекстЗапросаПоРасчетамСПоставщиками()
	...
	|		И НЕ РасчетыСПоставщиками.Регистратор ССЫЛКА Документ.ВозвратТоваровПоставщику
...
Функция ВременныеТаблицыПоРасчетамСПоставщиками()
	...
	|	И РасчетыПоСрокам.КорОбъектРасчетов.ТипОбъектаРасчетов = ЗНАЧЕНИЕ(Перечисление.ТипыОбъектовРасчетов.ПлатежВозврат)
	...
	|	И &ОбъектРасчетов = РасчетыПоСрокам.ОбъектРасчетов
	|	И РасчетыПоСрокам.Активность  
	|	И (ЕСТЬNULL(РасчетыПоСрокам.КорОбъектРасчетов.ТипОбъектаРасчетов,0) <> ЗНАЧЕНИЕ(Перечисление.ТипыОбъектовРасчетов.ПлатежВозврат)
...
Функция ТекстЗапросаПоРасчетамСКлиентами()
	...
	|		И НЕ РасчетыСКлиентами.Регистратор ССЫЛКА Документ.ВозвратТоваровОтКлиента
	...
	Для Каждого ДокументСсылка Из МассивОбработанныхДокументов Цикл
		...
		Если ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.СписаниеБезналичныхДенежныхСредств") Тогда
			...
			Если ЗначениеЗаполнено(ЗеркальныйПлатежСсылка) Тогда
				...
				Если Успешно Тогда
					ТекстОшибки = НСтр("ru = 'Помощник зачета оплаты, зеркальное изменение документов оплат.';
										|en = 'Payment offset wizard, inverted change of AR/AP transactions.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
