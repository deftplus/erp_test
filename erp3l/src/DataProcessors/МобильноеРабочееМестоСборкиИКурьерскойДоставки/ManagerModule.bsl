...
Процедура УстановитьСтатусРаспоряженияСобран(Распоряжение, СтруктураЗаказа) Экспорт
	...
	Если СотрудникСеанса <> ТекущийСборщик Тогда
		ТекстОшибки = НСтр("ru = 'Нельзя завершить сборку.';
							|en = 'Assembly cannot be completed.'");
		ТекстОшибки = ТекстОшибки + Символы.ПС + СтрШаблон(НСтр("ru = 'Заказ назначен на: %1.';
	...
	Для Каждого СтрокаСобрано Из СтруктураЗаказа.Собрано Цикл
		...
		Если МассивНайденных.Количество() = 0 Тогда
			...
			СтруктураДействий.Вставить("ПересчитатьКоличествоЕдиниц");
			СтруктураДействий.Вставить("ЗаполнитьСтавкуНДС", Новый Структура("НалогообложениеНДС, Дата, ПоДатеОтгрузки", РаспоряжениеОбъект.НалогообложениеНДС, РаспоряжениеОбъект.Дата, Истина));
