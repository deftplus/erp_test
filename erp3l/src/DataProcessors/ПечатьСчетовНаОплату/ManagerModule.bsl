...
Функция ПечатьПредварительныйПросмотрСчетаНаОплату(Организация, ОбъектыПечати, ИмяМакета)
	...
	СтруктураДанныхПокупатель.Вставить("ПредставлениеПолучателя", 
		НСтр("ru = 'Поле содержит информацию покупателя: полное наименование, ИНН, КПП, юридический адрес, телефоны.';
			|en = 'Field contains customer information: full name, TIN, CRTR, legal address, phones.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	ОбластьМакета.Параметры.Заполнить(СтруктураДанныхПокупатель);
	ТабличныйДокумент.Вывести(ОбластьМакета);
...
Процедура ЗаполнитьТабличныйДокументInvoice(ТабличныйДокумент, ДанныеДляПечати, ОбъектыПечати, ПараметрыПечати, Предварительный = Ложь)
	...
	ШаблоныОшибок.Вставить("ЗаменяющиеТовары", НСтр("ru = 'В документе %1 отсутствуют заменяющие товары. Печать Proforma invoice не требуется';
													|en = 'Substitute goods are missing in the %1 document. Printing of Proforma invoice is not required'"));
	ШаблоныОшибок.Вставить("Этапы", НСтр("ru = 'В документе %1 отсутствуют этапы оплаты. Печать Proforma invoice не требуется';
										|en = 'Payment steps are missing in document %1. Printing of Proforma invoice is not required'"));
...
Процедура ЗаполнитьРеквизитыШапкиInvoice(ДанныеПечати, Макет, ТабличныйДокумент, ТаблицаЭтапыОплаты, ТаблицаТовары, Предварительный)
	...
	Если Предварительный Тогда
		...
		Если ЗначениеЗаполнено(ДатаПлатежа) Тогда
			СтруктураДанныхЗаголовок = Новый Структура;
			НадписьСрокДействия = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Account is valid till %1.';
					|en = 'Account is valid till %1.'", ОбщегоНазначения.КодОсновногоЯзыка()), Формат(ДатаПлатежа, "ДЛФ=D")) + " ";
			СтруктураДанныхЗаголовок.Вставить("СрокДействия", НадписьСрокДействия);
			ОбластьМакета.Параметры.Заполнить(СтруктураДанныхЗаголовок);
		КонецЕсли;
...
Процедура ВывестиЗаголовокInvoice(Макет, ТабличныйДокумент, ДанныеПечати, Предварительный)
	...
	Если Предварительный Тогда
		СтруктураПараметров.Вставить("ТекстЗаголовка", НСтр("ru = 'PROFORMA INVOICE';
															|en = 'PROFORMA INVOICE'", ОбщегоНазначения.КодОсновногоЯзыка()));
	Иначе
		СтруктураПараметров.Вставить("ТекстЗаголовка", НСтр("ru = 'COMMERCIAL INVOICE';
															|en = 'COMMERCIAL INVOICE'", ОбщегоНазначения.КодОсновногоЯзыка()));
	КонецЕсли;
...
Функция ТекстНДСПоСтавкеИнвойс(СтавкаНДС, ЦенаВключаетНДС) Экспорт
	...
	ТекстНДСПоСтавке = ?(ЦенаВключаетНДС, НСтр("ru = 'INCLUDING VAT (%СтавкаНДС%)';
												|en = 'INCLUDING VAT (%СтавкаНДС%)'"), НСтр("ru = 'VAT (%СтавкаНДС%)';
																						|en = 'VAT (%СтавкаНДС%)'"));
