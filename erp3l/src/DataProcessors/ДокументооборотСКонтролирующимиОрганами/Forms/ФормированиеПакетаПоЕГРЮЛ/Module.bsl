...
Процедура ПодписатьДанные(АдресФайла, ВходящийКонтекст)
	...
	Если ТекущийСертификат.ЭтоЭлектроннаяПодписьВМоделиСервиса Тогда
	...
	Иначе
		...
		Если Результат.Выполнено = Неопределено Тогда
			ТекстОшибки = НСтр("ru = 'Вы отказались от установки расширения для работы с криптографией, которое требуется для подписания файлов пакета.';
								|en = 'You have rejected to install a cryptographic extension that is required to sign the package files.'");
		Иначе
			ТекстОшибки = НСтр("ru = 'Не удалось установить расширение для работы с криптографией, которое требуется для подписания файлов пакета электронной подписью.';
								|en = 'Cannot install the cryptography operation extension which is required for digitally signing the package files.'");
		КонецЕсли;
...
Процедура ОтправитьПакетДокументовВФНСЗавершение(РезультатОтправки, ДополнительныеПараметры) Экспорт
	...
	Если РезультатОтправки.Выполнено Тогда
		...
		ТекстОповещения = 
			...
			|
			|Application lead time is up to five workdays.
