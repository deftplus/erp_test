...
Процедура ВывестиПоясняющийТекстИЗаголовокПриЗавершеннойОтправке(ЗавершенУспешно = Ложь, ДополнительноеСообщение = "")
	...
	Если ВидОбъекта.ЭтоЗаявлениеВПФР И ЗавершенУспешно Тогда
		...
		Если ЭтоНаПодключениеИлиНаСертификат Тогда
			ТекстПредупреждения = НСтр("ru = 'Отправка документов по электронным трудовым книжкам в ПФР станет возможна после получения положительного результата рассмотрения заявления.';
										|en = 'Sending documents by electronic employment record books to PF will be possible after receiving a positive result of the application review.'") + Символы.ПС + Символы.ПС;
		Иначе
			ТекстПредупреждения = "";
		КонецЕсли;
