...
&НаКлиенте
Процедура ПереключитьНаЛокальноеХранение()
...
&НаКлиенте
Процедура ПереключитьНаОблачноеХранение()
...
Процедура ИзменитьОформлениеМестаХраненияКлючей()
	...
	ВыбранЛокальныхРежим                = НЕ ЭтоЭПВОблаке(ЭтотОбъект);
...
Процедура ОтправитьЗаявление()
	...
	Если ЭтоЭПВОблаке(ЭтотОбъект) И ЭтоЭлектронноеПодписание Тогда
...
Процедура СохранитьЗаявление(ЗаписатьЗаявление = Истина, НовыйGUID = Ложь) Экспорт
	...
	Иначе
		...
		Иначе
			НовыйДокументЗаявление.ЭлектроннаяПодписьВМоделиСервиса = ЭтоЭПВОблаке(ЭтотОбъект);
		КонецЕсли;
		
		Если ЭтоЭПВОблаке(ЭтотОбъект) Тогда
...
Процедура ПроверитьВключаемыйСертификат(МастерДалее)
	...
	Если ИспользоватьСуществующий(ЭтотОбъект) И ВключаемыйСертификат = Неопределено Тогда
		...
		ТекстОшибки = НСтр("ru = 'Выберите сертификат, либо переключитесь в режим ""Издать новый""';
							|en = 'Select a certificate, or switch to the ""Issue new"" mode'");
...
Процедура ПроверитьПользователей(МастерДалее)
	...
	Если НЕ КонтекстЭДОСервер.ПользователиУказаныКорректно(ЭтотОбъект, Истина) Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Выберите пользователей';
...
Функция ПроверитьТелефонВладельцаЭП(Форма, МастерДалее = Истина, ВыводитьСообщения = Истина)
	...
	Если РезультатПроверки.ТекстОшибки = "" Тогда
		...
		ЭтоЭПВОблаке = ЭтоЭПВОблаке(Форма);
...
Процедура ИзменитьОформлениеВключаемогоСертификата()
	...
	ИзменитьГруппировкуСУЦ();
	ИзменитьОформлениеСУЦ();
	ИзменитьОформлениеКнопкиОчисткиСУЦ();
	ИзменитьОформлениеСпособаПолученияСертификата();
...
Процедура ИзменитьОформлениеПодсказкиСУЦ()
	...
	СсылкаСертификата = Элементы.ВключаемыйСертификат;
	Вопросик = Элементы.ПодсказкаПоВключаемомуСертификату;
	
	Если ЭтоПолучениеЭПС2022Г И ЭтоБюджетополучатель Тогда
		
		СсылкаСертификата.ОтображениеПодсказки = ОтображениеПодсказки.ОтображатьСнизу;
		СсылкаСертификата.РасширеннаяПодсказка.Заголовок = ОбработкаЗаявленийАбонентаКлиентСервер.ПодсказкаПриВыбореСертификатаКазначейства();
		Вопросик.ОтображениеПодсказки = ОтображениеПодсказки.Нет;
		
	ИначеЕсли ЭтоПолучениеЭПС2022Г И ЭтоРуководитель(ЭтотОбъект) И НЕ ВозможноБесшовноеПолучениеСертификатаВДУЦ Тогда
		
		СсылкаСертификата.ОтображениеПодсказки = ОтображениеПодсказки.ОтображатьСнизу;
		СсылкаСертификата.РасширеннаяПодсказка.Заголовок = ОбработкаЗаявленийАбонентаКлиентСервер.ПодсказкаПриВыбореСертификатаФНС(ЭтоЮридическоеЛицо);
		Вопросик.ОтображениеПодсказки = ОтображениеПодсказки.Нет;
		
	Иначе
		
		Текст = НСтр("ru = 'Можно использовать сертификаты, выданные удостоверяющими центрами, которые входят в список доверенных удостоверяющих центров оператора ""Калуга Астрал"", а также сертификаты, изданные для целей 1С-Отчетности в другой программе.';
					|en = 'You can use the certificates issued by certification authorities that are included in the list of trusted certificate authorities of Kaluga Astral and the certificates issued for the purposes of 1C Reporting in another application.'");
		СсылкаСертификата.ОтображениеПодсказки = ОтображениеПодсказки.Нет;
		Вопросик.ОтображениеПодсказки = ОтображениеПодсказки.Кнопка;
		Вопросик.РасширеннаяПодсказка.Заголовок = Текст;
...
&НаСервере
Процедура ИзменитьОформлениеСпособаПолученияСертификата()
	
	Видимость = 
		ДоступнаЭлектроннаяПодписьВМоделиСервиса 
		ИЛИ СертификатыОрганизацииПоИНН.Количество() > 0 
		ИЛИ СертификатДолженБытьПолученОтГосУЦ
		ИЛИ ВозможноБесшовноеПолучениеСертификатаВДУЦ И ЭтоРуководитель(ЭтотОбъект)
		ИЛИ НЕ ВозможноБесшовноеПолучениеСертификатаВДУЦ И НЕ ЭтоРуководитель(ЭтотОбъект);
	
	Элементы.ГруппаПолученияСертификата.Видимость = Видимость;
	
	Если Видимость И ЗапретитьИзменение Тогда
		Элементы.ГруппаПолученияСертификата.ТолькоПросмотр = Истина;
	КонецЕсли;
	
	Элементы.ГруппаПереключатателейСертификата.Видимость = НЕ СертификатДолженБытьПолученОтГосУЦ;
	
	Группа = Элементы.ГруппаСпособПолученияСертификата;
	Если ВозможноБесшовноеПолучениеСертификатаВДУЦ И НЕ ИспользоватьСуществующий(ЭтотОбъект) Тогда
		Группа.РасширеннаяПодсказка.Заголовок = ОбработкаЗаявленийАбонентаКлиентСервер.ПодсказкаПриБесшовномПолученииСертификатаВДУЦ();
...
&НаСервере
Процедура ИзменитьОформлениеСУЦ()
	
	ВыбиратьСуществующий = 
		ИспользоватьСуществующий(ЭтотОбъект)
		ИЛИ СертификатДолженБытьПолученОтГосУЦ;
	
	Если ВыбиратьСуществующий Тогда
...
&НаСервере
Процедура ИзменитьОформлениеКнопкиОчисткиСУЦ()
...
&НаСервере
Процедура ИзменитьГруппировкуСУЦ()
	
	ИспользоватьСуществующий = ИспользоватьСуществующий(ЭтотОбъект);
	
	ВывестиВОднойСтроке = 
		ИспользоватьСуществующий И ВключаемыйСертификат = Неопределено
...
Процедура ИзменитьОформление1СЭДО()
	...
	ВыбраноОблачноеЭП     = ЭтоЭПВОблаке(ЭтотОбъект);
...
Процедура ИнициализацияДляЭПВМоделиСервиса() Экспорт
	...
	Если ДоступнаЭлектроннаяПодписьВМоделиСервиса Тогда
		...
		СпособПодтвержденияКриптоопераций = ?(ЭтоЭПВОблаке(ЭтотОбъект), 
			Перечисления.СпособыПодтвержденияКриптоопераций.СессионныйТокен,
...
&НаКлиентеНаСервереБезКонтекста
Функция ЭтоЭПВОблаке(Форма)

	Возврат Форма.РежимРаботыСКлючами = 1;
...
Процедура Подключаемый_ОбновитьМобильныйТелефонПоТелефонуДляПаролей()
	...
	Если ЭтоЭПВОблаке(ЭтотОбъект) И ПроверкаТелефонДляПаролей.ЗначениеВведено Тогда
...
Процедура Подключаемый_ОпределитьВозможностьБезбумажногоПодписания()
	...
	ПредлагатьУстановкуРасширения =
		...
		ИЛИ НЕ ЭтоЭПВОблаке(ЭтотОбъект))
		ИЛИ НЕ ЭтоВебКлиент;
...
Функция СкрытьПечатьЗаявления() Экспорт
	...
	СкрытьЗаявление = 
		ЭтоРуководитель(ЭтотОбъект) // Получат сертификат в ФНС
		ИЛИ ЭтоБюджетополучатель // Получат сертификат в ФК
		ИЛИ ИспользоватьСуществующий(ЭтотОбъект); // Нет получения сертификата
	
	Возврат СкрытьЗаявление;
...
Процедура ПроверитьХранениеКлючей(МастерДалее)
	...
	Если ИспользоватьСуществующий(ЭтотОбъект) И (ВключаемыйСертификатОблачный ИЛИ НЕ ВключаемыйСертификатОблачный И ЭтоЭПВОблаке(ЭтотОбъект)) Тогда
		Возврат;
	КонецЕсли;
...
Функция ТекстВопросаОбИстекающемСертификате(ДействителенПо)
	...
	ТекстВопроса = НСтр("ru = 'Ваш сертификат истекает через %1.
                         ...
                         |en = 'Your certificate will expire in %1.
                         |You will not be able to send reports once the certificate is expired.'");
...
Процедура ПриПодтвержденииВыбораВыбранногоИстекающегоСертификата(ВходящийКонтекст)
	...
	Если ВключаемыйСертификатОблачный И НЕ ЭтоЭПВОблаке(ЭтотОбъект) Тогда
		ПереключитьНаОблачноеХранение();
	ИначеЕсли НЕ ВключаемыйСертификатОблачный И ЭтоЭПВОблаке(ЭтотОбъект) Тогда
		ПереключитьНаЛокальноеХранение();
	КонецЕсли;
