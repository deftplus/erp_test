...
Процедура ПояснениеПоПодключениюКПФРОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	Если НавигационнаяСсылкаФорматированнойСтроки = "Заявление" Тогда
		...
		КонтекстЭДОКлиент.НапечататьДокумент(ТабДокЗаявления, НСтр("ru = 'Заявление о подключении к электронному документообороту';
																	|en = 'Application for electronic data interchange connection'"), Истина);
	...
	ИначеЕсли НавигационнаяСсылкаФорматированнойСтроки = "Сайт" Тогда
		...
		Иначе
			...
			КонтекстЭДОКлиент.НапечататьДокумент(ТабДокСоглашения, НСтр("ru = 'Соглашение об обмене эл. документами с ПФР';
																		|en = 'Agreement of electronic documents exchange with PF'"), Истина);
		КонецЕсли;
...
Функция ТекстПредупреждения(Форма)
	...
	Если ЗначениеЗаполнено(Форма.НаименованиеУПФР) Тогда
		ТекстПредупреждения = НСтр("ru = 'Обратите внимание, на сайте вашего отделения ПФР (%1) не опубликован бланк Соглашения, в связи с чем сформирован федеральный бланк. Рекомендуем заранее уточнить в вашем отделении, возможно ли использование федерального бланка Соглашения. 
                                    ...
                                    |en = 'Please note that the Agreement form has not been published on the website of your PF branch (%1), and therefore a federal form has been generated. We recommend that you check with your branch office beforehand whether it is possible to use the federal form of the Agreement.
                                    |Branch contact phones: %2.'");
	...
	Иначе
		ТекстПредупреждения = НСтр("ru = 'Обратите внимание, на сайте вашего отделения ПФР не опубликован бланк Соглашения, в связи с чем сформирован федеральный бланк. Рекомендуем заранее уточнить в вашем отделении, возможно ли использование федерального бланка Соглашения.';
									|en = 'Please note that the Agreement form has not been published on the website of your PF branch, and therefore a federal form has been generated. We recommend that you check with your branch office beforehand whether it is possible to use the federal form of the Agreement.'");
	КонецЕсли;
