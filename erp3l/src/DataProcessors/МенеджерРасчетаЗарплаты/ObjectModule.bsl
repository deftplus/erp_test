...
Процедура РассчитатьЗначенияПоказателейКоэффициентИндексацииСреднегоЗаработка(СтрокиНачисленийКРасчету)
	...
	КоэффициентИндексацииСохраняемогоДенежногоДовольствия = ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.КоэффициентИндексацииСохраняемогоДенежногоДовольствия");
	...
	Для Каждого СтрокаНабора Из СтрокиКРасчету Цикл
		...
		Если ТребованияТекущегоНачисления.ТребуетсяПоказательКоэффициентИндексацииСохраняемогоДенежногоДовольствия Тогда
			...
			ЗаполнитьЗначениеПоказателяСтрокиРасчета(СтрокаТаблицыНачисления, КоэффициентИндексацииСохраняемогоДенежногоДовольствия, Значение, Истина);
		КонецЕсли;
	КонецЦикла;
...
Функция СтрокиНачисленийПоТребованиям()
	...
	ПоказателиКоэффициентыИндексации.Вставить(ОбщегоНазначения.ПредопределенныйЭлемент("Справочник.ПоказателиРасчетаЗарплаты.КоэффициентИндексацииСохраняемогоДенежногоДовольствия"), Истина);
...
Процедура РассчитатьПогашениеЗаймов()
	...
	ПогашениеЗаймов = РасчетЗарплатыРасширенный.РезультатРасчетаЗаймов(
			Организация, ДатаПогашения, ИсключаемыйРегистратор, Сотрудники,, НастройкиРасчета.РассчитыватьНДФЛ);
...
Процедура ЗарегистрироватьНалогНаМатериальнуюВыгоду()
	...
	Если ЗарплатаМесяца.ПогашениеЗаймов.Количество() = 0 
		Или Не НастройкиРасчета.РассчитыватьНДФЛ Тогда
