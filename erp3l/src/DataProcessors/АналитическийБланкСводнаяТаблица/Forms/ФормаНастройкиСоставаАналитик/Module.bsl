...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НастройкиСоставаСтрокСводнойТаблицы.Загрузить(СтруктураТаблицИнтерфейса.НастройкиСоставаСтрокСводнойТаблицы);
	НастройкиСоставаКолонокСводнойТаблицы.Загрузить(СтруктураТаблицИнтерфейса.НастройкиСоставаКолонокСводнойТаблицы);
	НастройкиПолейСводнойТаблицы.Загрузить(СтруктураТаблицИнтерфейса.НастройкиПолейСводнойТаблицы);
	...
	Для Каждого Стр Из АналитикиСтрок Цикл
		Если Стр.Поле = Перечисления.ПостоянныеПоляСводнойТаблицы.СтрокиОтчетов
			ИЛИ Стр.Поле = Перечисления.ПостоянныеПоляСводнойТаблицы.КолонкиОтчетов
				ИЛИ  Стр.Поле = Перечисления.ПостоянныеПоляСводнойТаблицы.ПериодОтчета Тогда
			        Продолжить;
		КонецЕсли;			
		нСтр = НастройкиРасположенияПолейСводнойТаблицы.Добавить();
	...
	ИндПредставления = 1;
	Для Каждого Стр Из НастройкиРасположенияПолейСводнойТаблицы Цикл
...
Процедура ДеревоНастроекВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Найти(Поле.Имя,"Представление")>0 Тогда
		...
		СтруткураРасшифровки.Вставить("НазначениеНастройки",НастройкиРасположенияПолейСводнойТаблицы.НайтиСтроки(Новый Структура("ПредставлениеАналитики",Поле.Имя))[0].Поле);			
		...
		Если СтруткураРасшифровки.НазначениеНастройки = ПредопределенноеЗначение("Перечисление.ПостоянныеПоляСводнойТаблицы.Организация") Тогда
			СписокДоступныхЗначений.Добавить(Нстр("ru = 'Синхронизировать по функц. направлениям'"));
		КонецЕсли;	
...
Процедура СоздатьДеревоНастроек()
	...
	ДоступныеАналитики = НастройкиРасположенияПолейСводнойТаблицы.Выгрузить();
...
Процедура ОбойтиДЗ(СтрокиДЗ,СтркиКУдалению,ТЗДанныеЗначенийОтборов)
	...
	тЗначенияОтборов = ЗначенияОборовНастроек.Выгрузить(); 
	...
	Для Каждого СтрокаДерева Из СтрокиДЗ Цикл
		...
		Если СтрокаДерева.Заголовок="<...>" 
			...
			Для Каждого Поле из НастройкиРасположенияПолейСводнойТаблицы Цикл
				
				СохраненныеЗначения = тЗначенияОтборов.НайтиСтроки(Новый Структура("НомерНастройки,СтрокаОтчета,Назначение",СтрокаДерева.НомерНастройки,СтрокаДерева.Ссылка,Поле.Поле));
...
Функция ОпределитьТипТекущейКолонки(ИмяТекЭлемента)
	...
	ТекущаяАналитика =  НастройкиРасположенияПолейСводнойТаблицы.НайтиСтроки(Новый Структура("ПредставлениеАналитики",ИмяТекЭлемента))[0];
...
Процедура ОбновитьСвойстваСтроки(СтрокаНастройки)
	...
	Для Каждого Поле Из НастройкиРасположенияПолейСводнойТаблицы Цикл
		...
		Если ЗначениеЗаполнено(СтрокаНастройки[ЗначениеНастройки]) Тогда
			...
			НастройкиСоставаБланков = РегистрыСВедений.НастройкиСоставаАналитикиСтрокСводнойТаблицы.СоздатьНаборЗаписей();
			НастройкиСоставаБланков.Отбор.СтрокаОтчета.Установить(СтрокаНастройки.Ссылка);
			НастройкиСоставаБланков.Отбор.ВариантСводнойТаблицы.Установить(ВариантСводнойТаблицы);
			НастройкиСоставаБланков.Отбор.НомерНастройки.Установить(СтрокаНастройки.НомерНастройки);
			НастройкиСоставаБланков.Отбор.Назначение.Установить(Поле.Поле);
			НастройкиСоставаБланков.Прочитать();
			ЕстьНастройка = НастройкиСоставаБланков.Количество() > 0;
			НастройкиТаблица  = НастройкиСоставаБланков.Выгрузить();
			НужнаяНастройка = НастройкиТаблица.НайтиСтроки(Новый Структура("НомерНастройки,Назначение",СтрокаНастройки.НомерНастройки,Поле.Поле));
			...
			нНастройкиСоставаБланков.ВариантСводнойТаблицы = ВариантСводнойТаблицы;
		...
		ИначеЕсли СтрокаНастройки[Поле.ПредставлениеАналитики] = "Все элементы" Тогда //Удаляем настройку	
			...
			НастройкиСоставаБланков.Прочитать();
			ЕстьНастройка = НастройкиСоставаБланков.Количество() > 0;
			НастройкиТаблица  = НастройкиСоставаБланков.Выгрузить().СкопироватьКолонки();
			НастройкиСоставаБланков.Загрузить(НастройкиТаблица);			
            НастройкиСоставаБланков.Записать(Истина);
...
Функция СоздатьТекстЗапросаНастроекСоставаАналитик(ДоступныеАналитики) Экспорт
	...
	СтрокиОтчетов = НастройкиСоставаСтрокСводнойТаблицы.Выгрузить().ВыгрузитьКолонку("СтрокаОтчета");
	...
	Запрос.Текст ="ВЫБРАТЬ
	              ...
	              |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НастройкиСоставаАналитикиСтрокСводнойТаблицы КАК НастройкиСоставаАналитикиБланков
	              |		ПО СтрокиОтчетов.Ссылка = НастройкиСоставаАналитикиБланков.СтрокаОтчета
	              |		И НастройкиСоставаАналитикиБланков.ВариантСводнойТаблицы = &ВариантСводнойТаблицы	
				  |ГДЕ
