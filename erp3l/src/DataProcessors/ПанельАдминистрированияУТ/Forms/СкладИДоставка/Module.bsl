...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЗаголовокОтключения = ЗаголовокОтключения + НСтр("ru = '
		...
		|en = '
		|– Usage of in-house production (in Master data and settings – Production)
		|– Usage of subcontracting (in Master data and settings – Sales)'");
	Если ПолучитьФункциональнуюОпцию("ИспользоватьОказаниеАгентскихУслугПриЗакупке") 
		...
		ЗаголовокОтключения = ЗаголовокОтключения + НСтр("ru = '
			...
			|en = '
			|– goods and services assignment option ""By orders"" by agency (section ""Master data and settings"" — ""Sales"")'");
	КонецЕсли;
	//-- НЕ УТКА
	...
	Элементы.КомментарийОбеспечениеРЦ.Заголовок =
		НСтр("ru = 'Этот функционал доступен, если разрешено использование хотя бы одного из следующих видов документов:
			...
			|en = 'This functionality is available, if the use of at least one of the following document kinds is allowed:
			|- Transfer order (section ""Master data and settings"" — ""Warehouse and delivery"")'");
	...
	Элементы.КомментарийОбеспечениеРЦ.Заголовок = Элементы.КомментарийОбеспечениеРЦ.Заголовок
		+ НСтр("ru = '
			...
			|en = '
			|- Orders for materials for production (section ""Master data and settings"" — ""Production"")'");
	//-- НЕ УТ
	...
	Элементы.КомментарийИспользоватьОбособленноеОбеспечениеЗаказовУП.Заголовок = 
		НСтр("ru = 'Этот функционал доступен если разрешено использование хотя бы одного из следующих видов документов:
				...
				|– Inventory consumption orders (""Warehouse and delivery"" section)
				|– Assembly orders (""Warehouse and delivery"" section)
				|– Customer orders, the ""From warehouse and against order"" mode (""Sales"" section)
				|– Return requests, the ""From warehouse and against order"" mode (""Sales"" section)
				|– Transfer orders (""Warehouse and delivery"" section)
				|Also, if:
				|- Processing by the third party is used (""Production and repairs"" section)'");
	//-- НЕ УТ
	...
	Элементы.КомментарийИспользоватьОбособленноеОбеспечениеЗаказовУП.Заголовок =
		Элементы.КомментарийИспользоватьОбособленноеОбеспечениеЗаказовУП.Заголовок
			+ НСтр("ru = '
				...
				|en = '
				|- Production is used (""Production and repairs"" section)
				|- R&M management is used (""Production and repairs"" section)'");
	//-- НЕ УТКА
...
//-- НЕ УТКА
	ЗаголовокЭлемента = НСтр("ru = 'Формирование заказов в соответствии со способом обеспечения (%1) для поддержания остатка (min-max) и обеспечения заказов на отгрузку. Расчет потребностей с учетом плановых сроков поставки и интервала между поставками.';
							|en = 'Order generation in accordance with the procurement strategy (%1) selected for replenishing inventory (min-max) and fulfilling shipment orders. Calculation of demands on the basis of scheduled delivery due dates and interval between deliveries.'");
	...
	Элементы.КомментарийРасширенноеФормированиеЗаказовПоПотребностям.Заголовок = 
		...
		|en = 'This functionality is available if production management is used (in Master data and settings — Production)
