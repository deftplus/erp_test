...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.РазделениеВключено() И Не ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
		ТекстОшибки = НСтр("ru = 'Для удаления помеченных войдите в область данных.';
							|en = 'To delete marked objects, sign in to a data area.'");
		Возврат; // Отказ устанавливается в ПриОткрытии.
	КонецЕсли;
...
Процедура КВыборуОбъектов(Команда)
	...
	Если ТаблицаДействий.Количество() > 0 Тогда
		ПоказатьВопрос(Оповещение, 
			...
			|Do you want to return to the list of objects marked for deletion?'"),
...
Процедура УдалитьВыбранные(Команда)
	...
	Если СведенияОВыбранныхОбъектах.КоличествоВыбранных = СведенияОВыбранныхОбъектах.КоличествоВсего Тогда
	    ТекстВопроса = НСтр("ru = 'Удалить все помеченные на удаление?';
							|en = 'Do you want to delete all objects marked for deletion?'");
	Иначе
		ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Удалить помеченные на удаление (%1)?';
								|en = 'Do you want to delete objects marked for deletion (%1)?'"),
	...
	Иначе
		Если УстанавливатьПометку Тогда
			ТекстВопроса = НСтр("ru = 'Пометить выделенные объекты (%1) на удаление?';
								|en = 'Do you want to mark the selected objects (%1) for deletion?'");
		Иначе
			ТекстВопроса = НСтр("ru = 'Снять с выделенных объектов (%1) пометку на удаление?';
								|en = 'Do you want to clear the deletion mark from the selected objects (%1)?'");
		КонецЕсли;
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Удаление помеченных.Установка / снятие пометки удаления';
				|en = 'Delete marked objects.Set / clear deletion mark'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура НачатьВыполнениеДополнительнойОбработки(Параметр)
	...
	Элементы.ДекорацияПредставлениеДлительнойОперации.Заголовок = НСтр("ru = 'Выполняется дополнительная обработка причин неудаления...';
																		|en = 'Additional processing of reasons for not deleting...'");
	Элементы.КоманднаяПанельФормы.Доступность = Ложь;
	Элементы.СтраницыИнформация.ТолькоПросмотр = Истина;
