...
Функция ЕстьВозможностьВключитьИспользованиеРегламентногоЗадания(ТекстОшибки)
	...
	ИначеЕсли ЭтотУзел.Код <> ТекущаяВерсия Тогда
		...
		Если ЭтотУзел.ВерсияКорреспондента = ТекущаяВерсия
			И ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ЭтотУзел.Код,  ЭтотУзел.ВерсияКорреспондента) < 0 Тогда
	...
	ИначеЕсли ЭтотУзел.Код = ТекущаяВерсия Тогда
		...
		Результат = ОбщегоНазначенияКлиентСервер.СравнитьВерсии(ЭтотУзел.Код, Объект.Код);
	...
	Если НЕ ЕстьПравилаОбменаДанными(ТекущаяВерсия, ЭтотУзел.ВерсияКорреспондента) Тогда
		
		ЭмулироватьЗагрузкуПравилОбменаДанными(ТекущаяВерсия, ЭтотУзел.ВерсияКорреспондента);
