...
Процедура ЗаписатьРезультат()
	...
	Если СтатусВыполненнойЗагрузки Тогда
		...
		ФайлВыгрузки = РезультатВыгрузкиИзФайла.ФайлВыгрузки; // ДвоичныеДанные -
		ФайлВыгрузки.Записать(ПолноеИмяФайлаАрхива);
...
Функция ПроверитьПравилаНаСервере()
	...
	МакетПравилОбмена = Обработки.ПомощникВыгрузкиЗагрузкиМоделиБюджетирования.ПолучитьМакет(ИмяМакета);
