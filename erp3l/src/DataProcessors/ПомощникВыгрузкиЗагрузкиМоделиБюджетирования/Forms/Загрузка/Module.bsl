...
Процедура ПорядковыйНомерПереходаПриИзменении(Знач ЭтоПереходДалее)
	...
	Если ЭтоПереходДалее Тогда
		...
		Если Не ПустаяСтрока(СтрокаПерехода.ИмяОбработчикаПриПереходеДалее) Тогда
			...
			Если СтрокаПерехода.ИмяОбработчикаПриПереходеДалее = "СтраницаПриветствие_ПриПереходеДалее" Тогда
				Подключаемый_СтраницаПриветствие_ПриПереходеДалее(Отказ);
			ИначеЕсли СтрокаПерехода.ИмяОбработчикаПриПереходеДалее = "СтраницаФильтры_ПриПереходеДалее" Тогда
				Подключаемый_СтраницаФильтры_ПриПереходеДалее(Отказ);
			ИначеЕсли СтрокаПерехода.ИмяОбработчикаПриПереходеДалее = "СтраницаЗаменаЗначений_ПриПереходеДалее" Тогда
				Подключаемый_СтраницаЗаменаЗначений_ПриПереходеДалее(Отказ);
			КонецЕсли;
	...
	Иначе
		...
		Если Не ПустаяСтрока(СтрокаПерехода.ИмяОбработчикаПриПереходеНазад) Тогда
			...
			Если СтрокаПерехода.ИмяОбработчикаПриПереходеНазад = "СтраницаПриветствие_ПриПереходеДалее" Тогда
				Подключаемый_СтраницаПриветствие_ПриПереходеДалее(Отказ);
			ИначеЕсли СтрокаПерехода.ИмяОбработчикаПриПереходеНазад = "СтраницаФильтры_ПриПереходеДалее" Тогда
				Подключаемый_СтраницаФильтры_ПриПереходеДалее(Отказ);
			ИначеЕсли СтрокаПерехода.ИмяОбработчикаПриПереходеНазад = "СтраницаЗаменаЗначений_ПриПереходеДалее" Тогда
				Подключаемый_СтраницаЗаменаЗначений_ПриПереходеДалее(Отказ);
			КонецЕсли;
	...
	Если Не ПустаяСтрока(СтрокаПереходаТекущая.ИмяОбработчикаПриОткрытии) Тогда
		...
		Если СтрокаПереходаТекущая.ИмяОбработчикаПриОткрытии = "СтраницаПриветствие_ПриОткрытии" Тогда
			Подключаемый_СтраницаПриветствие_ПриОткрытии(Отказ, ПропуститьСтраницу, ЭтоПереходДалее);
		ИначеЕсли СтрокаПереходаТекущая.ИмяОбработчикаПриОткрытии = "СтраницаФильтры_ПриОткрытии" Тогда
			Подключаемый_СтраницаФильтры_ПриОткрытии(Отказ, ПропуститьСтраницу, ЭтоПереходДалее);
		ИначеЕсли СтрокаПереходаТекущая.ИмяОбработчикаПриОткрытии = "СтраницаЗаменаЗначений_ПриОткрытии" Тогда
			Подключаемый_СтраницаЗаменаЗначений_ПриОткрытии(Отказ, ПропуститьСтраницу, ЭтоПереходДалее);
		ИначеЕсли СтрокаПереходаТекущая.ИмяОбработчикаПриОткрытии = "СтраницаЗавершение_ПриОткрытии" Тогда
			Подключаемый_СтраницаЗавершение_ПриОткрытии(Отказ, ПропуститьСтраницу, ЭтоПереходДалее);
		ИначеЕсли СтрокаПереходаТекущая.ИмяОбработчикаПриОткрытии = "СтраницаОжидания_ПриОткрытии" Тогда
			Подключаемый_СтраницаОжидания_ПриОткрытии(Отказ, ПропуститьСтраницу, ЭтоПереходДалее);
		КонецЕсли;
...
Процедура ВыполнитьОбработчикПослеОткрытия()
	...
	Если Не ПустаяСтрока(СтрокаПереходаТекущая.ИмяОбработчикаПослеОткрытия) Тогда
		...
		Если СтрокаПереходаТекущая.ИмяОбработчикаПослеОткрытия = "СтраницаОжидания_ПослеОткрытия" Тогда
			Подключаемый_СтраницаОжидания_ПослеОткрытия();
		КонецЕсли;
...
Процедура Подключаемый_СтраницаФильтры_ПриОткрытии(Отказ, ПропуститьСтраницу, Знач ЭтоПереходДалее)
	...
	Заголовок = НСтр("ru = 'Выбор загружаемых видов бюджетов и этапов подготовки бюджетов.';
					|en = 'Select imported budget kinds and budgeting steps.'");
...
Функция РазархивироватьФайлНаСервере(АдресХранилищаФайлаНаСервере, ТекстОшибки = "")
	...
	Попытка
		...
		Иначе
			ИмяФайлаМоделиСтрока = СтрШаблон(НСтр("ru = 'Выгрузка модели %1';
													|en = 'Exporting model %1'"), "КА") + ".xml";
			//++ НЕ УТКА
			ИмяФайлаМоделиСтрока = НСтр("ru = 'Выгрузка модели';
