...
Процедура ПоказатьПредставлениеФайлаЭлектронногоДокумента(СтруктураЭД, Отказ)
	...
	Если НЕ ЗначениеЗаполнено(ДокументИБ) И ОписаниеПакета.СписокТипов.Количество() Тогда
		ДокументИБ = ОбменСКонтрагентамиИнтеграция.ОписаниеТипаОбъектаПоСпособуОбработки(ОписаниеПакета.СписокТипов[0].Значение).ПривестиЗначение();
		ИмяОбъектаМетаданных = ДокументИБ.Метаданные().ПолноеИмя();
	КонецЕсли;
