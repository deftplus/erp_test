...
Процедура Подключаемый_ВыполнитьКомандуСоздатьШаблон(Команда)
	...
	НаименованиеШаблона     = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Счет на оплату через ЮKassa (%1)';
			|en = 'Commercial invoice via YooMoney (%1)'"), ПредставлениеДокумента);
...
Процедура ПрочитатьУстановитьДоступность(РеквизитПутьКДанным = "")
	...
	Если ЕстьПодсистемаОбменаСЕИС Тогда
		...
		ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
			"НастройкиОбменаСЕИС", "Доступность", Константа_ИспользоватьЭлектронноеАктированиеВЕИС);
	КонецЕсли;
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
...
Процедура ПриИзмененииНастроекEDI()
	...
	ТекстГиперссылкиНастройки = ?(ЕстьНастройкиОрганизаций, НСтр("ru = 'Настройки 1C:EDI';
																|en = '1C:EDI settings'"), НСтр("ru = 'Настроить 1C:EDI';
																									|en = 'Configure 1C:EDI'"));
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы,
