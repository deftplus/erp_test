		...
		Пока ОбъектЧтениеXML.Прочитать() Цикл
			Если ОбъектЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
				...
				ИначеЕсли ИмяУзла = "СвРегНом"
					И (СтруктураПараметров.ПравилаЗаполнения = "2021Кв3") Тогда
		...
		Для Каждого СтрокаДанных Из Данные.Записи Цикл
			...
			Если СтруктураПараметров.ПравилаЗаполнения = "2017Кв1"
			 ...
			 ИЛИ СтруктураПараметров.ПравилаЗаполнения = "2020Кв4" Тогда
				...
				КонецЦикла;
			ИначеЕсли СтруктураПараметров.ПравилаЗаполнения <> "2015Кв1" Тогда // "2021Кв3"
...
Процедура ПостобработкаДереваВыгрузки(ДеревоВыгрузки)
	...
	ИначеЕсли ПараметрыВыгрузки.ПравилаЗаполнения = "2017Кв1"
		  ...
		  ИЛИ ПараметрыВыгрузки.ПравилаЗаполнения = "2021Кв3" Тогда
...
Функция ПолучитьФормуОтчетаНаКлиенте(ПараметрыФормыОтчета)
	...
	Если СтрВыбраннаяФорма <> "ФормаОтчета2015Кв1"
	   ...
	   И СтрВыбраннаяФорма <> "ФормаОтчета2021Кв3" Тогда
		...
		ПоказатьПредупреждение(, НСтр("ru = 'Консолидация отчетов по НДС реализована только для форм:
			...
			|(in edition of the decree of the FTS of Russia dated August 19, 2020 No. ED-7-3/591@),
			|(in edition of the decree of the FTS of Russia dated March 26, 2021 No ED-7-3/228@)""!'"));
		Возврат Неопределено;
	...
	Если ТипЗнч(ПоказателиОтчета) = Тип("Структура") Тогда
		Для Каждого Раздел Из ПоказателиОтчета Цикл
			...
			Если ИмяРаздела = "Титульный" Тогда
				...
				Если ПараметрыВыгрузки.ПравилаЗаполнения <> "2020Кв4" И ПараметрыВыгрузки.ПравилаЗаполнения <> "2021Кв3" Тогда
