...
Функция ВыполнитьОбъединениеСправочников(Параметры)
	...
	ТаблицаОпераций        = ДанныеДляОбъединения.ТаблицаОпераций;
...
Процедура ПеренестиТаблицуОпераций(Операции, Владелец, Спецификация, СоответствиеОпераций = Неопределено, Коэффициент = 1)
	...
	Для каждого Строка Из Операции Цикл
		ОперацияОбъект = Строка.Операция.Скопировать();
...
Функция СобытиеСозданиеТиповыхТехнологическихПроцессов()
	...
	Возврат НСтр("ru = 'НСИ производства. Создание типовых технологических процессов.';
				|en = 'Production master data. Generating standard technological processes.'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СобытиеПереносОперацийВРесурсныеСпецификации()
	...
	Возврат НСтр("ru = 'НСИ производства. Перенос операций в ресурсные спецификации.';
				|en = 'Production master data. Transferring operations to bills of materials.'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ПолучитьОбъектСпецификацииДляОбработки(Спецификация, СпецификацияОбъект)
	...
	ДвоичныеДанные = ВерсионированиеОбъектов.СериализоватьОбъект(СпецификацияОбъект);
	
	ПараметрыФайла = РаботаСФайлами.ПараметрыДобавленияФайла();
	ПараметрыФайла.ВладелецФайлов     = Спецификация;
	ПараметрыФайла.ИмяБезРасширения   = "Спецификация_"+СпецификацияОбъект.Код;
	ПараметрыФайла.РасширениеБезТочки = "bin";
	ПараметрыФайла.Служебный          = Истина;
	
	РаботаСФайлами.ДобавитьФайл(
		ПараметрыФайла,
		ПоместитьВоВременноеХранилище(ДвоичныеДанные),,
		НСтр("ru = 'Резервная копия. Создана помощником перехода на хранение операций в спецификациях.';
			|en = 'Backup copy. Created by the assistant on transfer to storing operations in bills of materials.'"));
