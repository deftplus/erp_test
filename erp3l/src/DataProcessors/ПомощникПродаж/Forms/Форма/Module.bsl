...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если СтандартныеПодсистемыСервер.ЭтоБазоваяВерсияКонфигурации() Тогда
		ВызватьИсключение НСтр("ru = 'Помощник продаж недоступен в базовой версии программы.';
								|en = 'Sales assistant is not available in the basic version of the program.'");
	КонецЕсли;
...
Процедура ПриЗагрузкеДанныхИзНастроекНаСервере(Настройки)
	...
	Если Не ЗначениеЗаполнено(Объект.ВариантОбеспечения) Тогда
		Объект.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Требуется;
	КонецЕсли;
...
Функция ЗаписатьДоверенностьНаПолучениеДС(МассивНайденныхДокументов, ОснованиеОбъект, Отказ)
	...
	Если ДокументОбъект.ФизЛицо.Пустая() Тогда
		ТекстОшибки = НСтр("ru = 'Не удалось создать доверенность: нет данных физического лица.';
							|en = 'Cannot create authorization letter: no individual data.'");
		Сообщение = Неопределено;
...
// ЗавершениеСозданияИзмененияДокументов
		...
		Если Отказ Тогда
			ВызватьИсключение НСтр("ru = 'Ошибка завершения создания документов';
									|en = 'An error occurred when completing the documents creation'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
		КонецЕсли; 
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Ошибка завершения создания документов';
				|en = 'An error occurred when completing the documents creation'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
	...
	ИначеЕсли ЭтоРеализация(Объект.ВариантОформленияДокументов) Тогда
		...
		Если ТребуетсяОформлениеАкта Тогда
			...
			ДобавитьТекстНадписиВМассив(МассивНадписей, НСтр("ru = 'Акт';
															|en = 'Certificate'"), "%СуммаАкта%", СуммыДокументов.СуммаАкта, СоздаватьСчетНаоплату);
		КонецЕсли;
...
Процедура УстановитьДоступностьКомандОбеспечения()
	...
	Элементы.ТоварыДоступно.Видимость = КомандыОбеспеченияДоступны
		И (Не Объект.УпрощенноеОбеспечение
			Или Не Объект.УпрощенноеОбеспечение
				И Объект.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Требуется);
