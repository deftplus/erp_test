...
Процедура ИнициализироватьКомпоновщикНастроек(Форма, ИмяРегистра) Экспорт
	...
	Если ЗначениеЗаполнено(ИмяРегистра) Тогда
		...
		МенеджерРегистра = ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(ПолноеИмяРегистра); // РегистрНакопленияМенеджер - 
...
// 	ОбъектУчета - ПеречислениеСсылка.ОбъектыБухгалтерскогоУчета - Объект бухгалтреского учета, по которому получаются используемые счета учета.
...
Функция НастройкиЗаполненияСубконто(Отбор, КомпоновщикНастроек, ИспользоватьРучныеНастройки = Истина) Экспорт
	...
	|	И (ПорядокОтражения.СчетУчета = &СчетУчета 
	|		ИЛИ &ВсеСчетаУчета)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	СчетУчета
	...
	ОписаниеОбъектаУчета = Перечисления.ОбъектыБухгалтерскогоУчета.ОписаниеОбъектаБухгалтерскогоУчета(Отбор.ОбъектУчета);
...
Процедура ЗаполнитьСостояниеАвтоматическогоЗаполненияСубконто(СтрокаСостоянияНастроек, ОписаниеОбъектаУчета)
	...
	Для НомерСубконто = 1 По 3 Цикл
		...
		ВозможныеПоля = МеждународныйУчетПоДаннымОстаточныхФинансовыхРегистров.ВозможныеПоляЗаполненияСубконто(ОписаниеТипов, ОписаниеОбъектаУчета);
