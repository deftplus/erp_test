...
Процедура ЗаполнитьСоответсвие(Соответствие, ДатаКурса, СписокКурсовXDTO, МассивВалютДляЗагрузки)
	...
	Для Каждого Курс Из СписокКурсовXDTO Цикл
		...
		Если МассивВалютДляЗагрузки.Найти(ЗагружаемаяВалюта) <> Неопределено Тогда
			КурсыДень.Вставить(ЗагружаемаяВалюта, Число(Курс.rate));
		КонецЕсли;
...
Функция ПолучитьДанныеКурсов(XDTO, НачалоПериодаЗагрузки, ОкончаниеПериодаЗагрузки, МассивВалютДляЗагрузки)
	...
	Если ТипЗнч(XDTO) = Тип("ОбъектXDTO") И XDTO.Свойства().Получить("time") <> Неопределено Тогда
		...
		Если ДатаКурса <= ОкончаниеПериодаЗагрузки И ДатаКурса >= НачалоПериодаЗагрузки Тогда 
			ЗаполнитьСоответсвие(Соответствие, ДатаКурса, XDTO["Cube"], МассивВалютДляЗагрузки);
		Иначе
	...
	ИначеЕсли ТипЗнч(XDTO) = Тип("СписокXDTO") Тогда
		...
		Для Каждого СпискоДат Из XDTO Цикл
			...
			ЗаполнитьСоответсвие(Соответствие, ДатаКурса, СпискоДат.Cube, МассивВалютДляЗагрузки);
	...
	Иначе
		Возврат	ПолучитьДанныеКурсов(XDTO.Cube, НачалоПериодаЗагрузки, ОкончаниеПериодаЗагрузки, МассивВалютДляЗагрузки);
	КонецЕсли;
	
КонецФункции

Функция ПрочитатьФайлКурсов(Файл, НачалоПериодаЗагрузки, ОкончаниеПериодаЗагрузки, МассивВалютДляЗагрузки)
	...
	ДанныеКурсов = ПолучитьДанныеКурсов(Фабрика, НачалоПериодаЗагрузки, ОкончаниеПериодаЗагрузки, МассивВалютДляЗагрузки);
...
Функция ЗагрузитьКурсыВалютПоПараметрам(Знач Валюты, Знач НачалоПериодаЗагрузки, Знач ОкончаниеПериодаЗагрузки, БазоваяВалюта, ПриЗагрузкеВозниклиОшибки = Ложь, ДатаКурсаВалютыВКоллекции = Ложь)
	...
	Если Результат.Статус Тогда
		...
		ДанныеКурсов = ПрочитатьФайлКурсов(Результат, НачалоПериодаЗагрузки, ОкончаниеПериодаЗагрузки, ВалютыЗагружаемыеИзИнтернета);
	Иначе
