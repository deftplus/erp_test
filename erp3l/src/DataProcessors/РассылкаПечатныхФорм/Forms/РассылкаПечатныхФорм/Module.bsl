...
Процедура НачатьОтправкуДокументов()
	...
	Если ДанныеФормСЭЦП.ФайлыНаПодпись.Количество() > 0 Тогда
		...
		КадровыйЭДОКлиент.ПодписатьФайлы(
			ДанныеФормСЭЦП.ФайлыНаПодпись, Оповещение, ЭтотОбъект, ПредопределенноеЗначение("Перечисление.ДействияСПечатнымиФормами.Подписать"));
...
Функция ФайлыНаПодпись(ОбъектыИдентификаторов)
	...
	Для Каждого ОтписаниеТипаОбъектов Из ТипыОбъектов Цикл
		
		Если Не КадровыйЭДО.ЭтоТипОбъектаСПечатнымиФормами(ОтписаниеТипаОбъектов.Ключ) Тогда
			Продолжить;
		КонецЕсли;
		
		СсылкиНаДокументы = ОтписаниеТипаОбъектов.Значение;
