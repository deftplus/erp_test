...
Функция СвободныйОстатокНаСкладе(ТаблицаДляОбеспечения) Экспорт
	...
	Запрос.Текст =
		...
		|	РаспределениеЗапасов.Склад               КАК Склад,
		|	РаспределениеЗапасов.Назначение          КАК Назначение,
		|	СУММА(РаспределениеЗапасов.Распределено) КАК Распределено
		...
		|		Таблица.Назначение     КАК Назначение
		|	ИЗ
		|		ВтТоварыПодразделений КАК Таблица) КАК Товары
		|		
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РаспределениеЗапасов КАК РаспределениеЗапасов
		...
		|		+ ЕСТЬNULL(РаспределениеЗапасов.Распределено, 0)
		|		+ ЕСТЬNULL(ОстатокНаСкладе.Свободно, 0) КАК Количество
		|ИЗ
		|	ВтТовары КАК Таблица
		|		
