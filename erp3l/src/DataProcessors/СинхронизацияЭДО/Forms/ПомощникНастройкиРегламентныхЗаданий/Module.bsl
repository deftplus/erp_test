...
Функция ОписаниеСостояния(ЭлементФормы, ИндексСостояния)
	...
	ИначеЕсли ЭлементФормы = Элементы.НадписьСертификат Тогда
		...
		ОписаниеПодсказок.Вставить(0, НСтр("ru = 'По каждой учетной записи ЭДО должен быть сертификат, установленный на сервере. Сертификат должен быть установлен для пользователя, от имени которого работает сервер 1с. Значение поля ""Пользователь"" сертификата должно быть пустым.';
											|en = 'Each EDI settings account must have a certificate installed on the server. The certificate must be installed for the user under which 1C server is running. The ""User"" field value of the certificate must be blank.'"));
		...
		ОписаниеПодсказок.Вставить(1, НСтр("ru = 'Автоматически получать документы можно только по тем учетным записям ЭДО, по которым есть сертификат. Сертификат должен быть установлен для пользователя, от имени которого работает сервер 1с. Значение поля ""Пользователь"" сертификата должно быть пустым.';
											|en = 'You can automatically receive documents only for those EDI setting accounts that have a certificate. The certificate must be installed for the user under which 1C server is running. The ""User"" field value in the certificate must be blank.'"));
