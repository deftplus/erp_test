...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ДекорацияПоясняющийТекстДлительнойОперации.Заголовок = СтрШаблон(НСтр("ru = 'Загрузка файла %1';
																					|en = 'Import file %1'"), Параметры.ПредставлениеФайла);
...
Процедура ОтправитьОчереднуюПорциюНаСервере(Знач ИмяФайлаИлиАдрес, Знач ПутьФайлаWindows, Знач ПутьФайлаLinux, Знач ДвоичныеДанные, Знач Начало)
	...
	Если РазмерЗаписанногоФрагмента <> Начало Тогда
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Размер файла %1 не соответствует ожидаемому %2';
										|en = 'Size of file %1 does not match the expected size %2'"), РазмерЗаписанногоФрагмента, Начало);
	КонецЕсли;
...
Процедура ОтменитьЗагрузку()
	...
	ИмяСобытияЖР = НСтр("ru = 'Удаление файла.Отмена загрузки';
						|en = 'Deleting file.Export canceled'", ОбщегоНазначения.КодОсновногоЯзыка());
