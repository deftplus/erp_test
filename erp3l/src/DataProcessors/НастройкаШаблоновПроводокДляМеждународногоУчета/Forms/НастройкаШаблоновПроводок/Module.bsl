...
Процедура ШаблоныПроводокПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки, Строки)
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		НастройкаПоШаблону.Вставить(ВыборкаДетальныеЗаписи.Ссылка, ВыборкаДетальныеЗаписи.ДополнительныйОтбор.Получить());
	КонецЦикла;
...
Функция НастройкаФормированияПроводокЗаполнена()
	...
	Если Не НастройкаФормированияПроводокЗаполнена Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не заполнена ""Настройка формирования проводок""';
														|en = 'The ""Entries generation setting"" cannot be blank'"), ,"НастройкаФормированияПроводок");
	КонецЕсли;
...
Процедура ИзменитьОтражениеХозяйственныхОперацийВНастройке(ОтражатьВМеждународномУчете)
	...
	ИзменитьОтражениеХозяйственныхОперацийВНастройкеФормированияПроводокСервер(МассивСтрок, НастройкаФормированияПроводок, ОтражатьВМеждународномУчете);
...
// 	НастройкаФормированияПроводок - СправочникСсылка.НастройкиФормированияПроводокМеждународногоУчета - Описание
// 	ОтражатьВМеждународномУчете - Булево - Описание
&НаСервереБезКонтекста
Процедура ИзменитьОтражениеХозяйственныхОперацийВНастройкеФормированияПроводокСервер(МассивСтрок, НастройкаФормированияПроводок, ОтражатьВМеждународномУчете)
	...
	Если ТаблицаУдаляемыхЗаписей.Количество() > 0 Тогда
		...
		ОтразитьПодчиненныеШаблоныПроводокВНастройкеФормированияПроводок(ТаблицаУдаляемыхЗаписей, НастройкаФормированияПроводок);
...
&НаСервереБезКонтекста
Процедура ОтразитьПодчиненныеШаблоныПроводокВНастройкеФормированияПроводок(ОтражаемыеОперации, НастройкаФормированияПроводок)
