...
Процедура ТоварыПриАктивизацииЯчейки(Элемент)
	...
	Если Элемент.ТекущийЭлемент.Имя = "ТоварыКартинкаПереместитьВправо" И Не ТолькоПросмотр Тогда
		...
		Если КлючевыеПараметры.ПодборТоваров Тогда
			Если Объект.ВариантКомплектацииНоменклатуры.Пустая() Тогда
				КоличествоПодобрать = Объект.Количество - Объект.Товары.Итог("Подобрано");
	...
	Если СтароеКоличествоУпаковок < Объект.КоличествоУпаковок Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Введено количество упаковок меньше, чем уже подобрано. Подобрано %1 %2.';
				|en = 'You have entered fewer packages than already selected. Selected: %1 %2.'"),
...
Процедура УправлениеЭлементамиФормыПриСозданииИИзмененииРеквизитов(Элемент)
	...
	Если Элемент = Неопределено
		...
		ОтборСтрок.Вставить("Номенклатура",   ТекущаяНоменклатура);
		ОтборСтрок.Вставить("Характеристика", ТекущаяХарактеристика);
		ОтборСтрок.Вставить("Обособленно",    Объект.Обособленно);
		
		Если РежимОтображенияТоваров = "СвободныеОстатки" Тогда
			ОтборСтрок.Вставить("ЕстьОстатокНаСкладе", Истина);
		КонецЕсли;
		
		Если РежимОтображенияТоваров = "ОстаткиИГрафик" Тогда
			ОтборСтрок.Вставить("ЕстьОстатокВГрафике", Истина);
		КонецЕсли;
		
		Если ТолькоСкладыВыбраннойГруппы Тогда
			ОтборСтрок.Вставить("ЭтоСкладВыбраннойГруппы", Истина);
		КонецЕсли;
	...
	Если Элемент = Элементы.КУказаниюСерий Тогда
		...
		Если РежимОтображенияСерий = "СвободныеОстатки" Тогда
			ОтборСтрок.Вставить("ПроверятьДоступность", Истина);
		КонецЕсли;
		ОтборСтрок.Вставить("Назначение",     ?(СтрокаТовары.Обособленно И Не ЗначениеЗаполнено(СтрокаТовары.Назначение),
		                                        Неопределено,
		                                        СтрокаТовары.Назначение));
		
		Элементы.Серии.ОтборСтрок = Новый ФиксированнаяСтруктура(ОтборСтрок);
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("РежимОтображенияТоваров");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = "ОстаткиИГрафик";
	...
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.Товары.ПоГрафику");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Меньше;
	ОтборЭлемента.ПравоеЗначение = Новый ПолеКомпоновкиДанных("Объект.Товары.Подобрано");
	...
	Иначе
		...
		Если Объект.ИндексСтрокиДокумента > -1 Тогда
			...
			Если КоличествоИзменение > Окр(СтрокаДокумента.Подобрано * ЕдиницаИзмеренияКоэффициент, 3) Тогда
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Указано количество больше, чем требуется перераспределить на %2 %3.';
												|en = 'Quantity indicated is greater than required to reallocate by %2 %3.'"),
...
Функция ПодобранныеТовары()
	...
	Если Объект.ВариантКомплектацииНоменклатуры.Пустая() Тогда
		...
		Если ОсталосьПодобрать > 0 И Объект.ИндексСтрокиДокумента > -1 Тогда
...
Функция ЗаголовокПодборСклада()
	...
	Если Объект.Обособленно Тогда
		...
		ИначеЕсли Объект.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Требуется Тогда
