...
Процедура ОбновитьЗаголовкиЗакладок(ОбновитьВыгружается = Истина, ОбновитьИсключения = Истина)
	...
	Если ОбновитьВыгружается = Истина Тогда
		...
		ИначеЕсли КоличествоВыгружается > 0 Тогда
			ВыгружаетсяПредставление = СтрШаблон(НСтр("ru = ' (%1)';
														|en = ' (%1)'"), КоличествоВыгружается);
		КонецЕсли;
		Элементы.СтраницаВыгружаются.Заголовок = СтрШаблон(НСтр("ru = 'Будет выгружено%1';
																|en = 'Will be exported%1'"), ВыгружаетсяПредставление);
		Элементы.ЗапуститьВыгрузку.Заголовок   = СтрШаблон(НСтр("ru = 'Выгрузить сейчас%1';
																|en = 'Export now%1'"), ВыгружаетсяПредставление);
	...
	Если ОбновитьИсключения = Истина Тогда
		...
		ИначеЕсли КоличествоИсключения > 0 Тогда
			ИсключенияПредставление = СтрШаблон(НСтр("ru = ' (%1)';
													|en = ' (%1)'"), КоличествоИсключения);
		КонецЕсли;
		Элементы.СтраницаИсключения.Заголовок = СтрШаблон(НСтр("ru = 'Исключения%1';
																|en = 'Ecxeption%1'"), ИсключенияПредставление);
...
Функция ВыгрузитьНоменклатуру(Знач УникальныйИдентификатор, Знач Организация, Знач ДатаОткрытияФормы)
	...
	НаименованиеЗадания = НСтр("ru = 'Работа с номенклатурой. Ввыгрузка номенклатуры';
								|en = 'Work with products. Products export'");
