...
Функция СоздатьЗаказыНаСервере()
	...
	Для каждого СтрокаДанныеЗаказа Из Объект.ФормированиеЗаказов Цикл
		...
		ДокументОбъект.Комментарий = НСтр("ru = 'Сформирован автоматически обработкой ""Формирование заказов на сборку (разборку) по плану"".';
											|en = 'Generated automatically using the ""Convert schedule to assembly (disassembly) orders"" data processor.'");
		...
		Иначе
			Если ЗначениеЗаполнено(СтрокаДанныеЗаказа.Назначение) Тогда
				ДокументОбъект.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Требуется;
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Формирование заказов на сборку (разборку) по плану';
											|en = 'Convert schedule to assembly (disassembly) orders'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
