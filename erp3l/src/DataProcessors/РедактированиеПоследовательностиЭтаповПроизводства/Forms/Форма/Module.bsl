...
Процедура ОбеспечениеМатериаламиИРаботамиПроизводитсяПриИзменении(Элемент)
	...
	ПараметрыМетода.Вставить("НазначениеМатериалы",           ОбъектЭтап.НазначениеМатериалы);
...
Процедура ОбеспечениеМатериаламиИРаботамиСсылкаРедактированиеОчистка(Элемент, СтандартнаяОбработка)
	...
	СтандартнаяОбработка = Ложь;
	СтруктураДействий = ОчиститьЭтапПроизводстваВСтроке("ОбеспечениеМатериаламиИРаботами");
		...
		Если ОбеспечениеВДокументахКлиент.ТребуетсяОбновитьКолонкуДоступно(СтруктураДействий)
				Или ПравоЧтенияАналогов
					...
					ОбеспечениеМатериаламиИРаботамиСсылкаРедактированиеОчисткаСервер(
...
Процедура ОбеспечениеМатериаламиИРаботамиОбособленноПриИзменении(Элемент)
	...
	ПараметрыДействия = ОбеспечениеПроизводстваКлиентСервер.ПараметрыДействияПроверитьЗаполнитьНазначениеОбеспеченияВЭтапеПроизводства(
		ОбъектЭтапПоСсылке(ЭтаФорма, Строка.Ссылка));
...
Процедура ТрудозатратыИсполнительАвтоПодбор(Элемент, Текст, ДанныеВыбора, ПараметрыПолученияДанных, Ожидание, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Текст) Тогда
		...
		СтандартнаяОбработка = Ложь;
		ОбъектЭтап = ОбъектЭтапПоСсылке(ЭтаФорма, ТекущиеДанные.Ссылка);
		ИсполнительПолучениеДанныхВыбора(ДанныеВыбора, Текст, ТекущиеДанные.Подразделение, ОбъектЭтап.Организация);
...
Функция ОбеспечениеУстановитьДействие(КодДействия)
	...
	Если КодДействия = "ФЛАГ_ОБОСОБЛЕННО" Или КодДействия = "ФЛАГ_НЕОБОСОБЛЕННО" Тогда
		...
		Для Каждого Идентификатор Из Идентификаторы Цикл
			...
			Строка = Объект.ОбеспечениеМатериаламиИРаботами.НайтиПоИдентификатору(Идентификатор);
			
			ПараметрыДействия = ОбеспечениеПроизводстваКлиентСервер.ПараметрыДействияПроверитьЗаполнитьНазначениеОбеспеченияВЭтапеПроизводства(
				ДанныеЭтапов[Строка.Ссылка]);
			СтруктураДействий.Вставить("ПроверитьЗаполнитьНазначениеОбеспеченияВЭтапеПроизводства", ПараметрыДействия);
...
&НаКлиенте
Функция ОбработатьИзменениеЭтапаПроизводстваВСтроке(Строка, ИмяТЧ, ОбъектЭтап)
...
&НаКлиенте
Функция ОчиститьЭтапПроизводстваВСтроке(ИмяТЧ)
	...
	Возврат ОбработатьИзменениеЭтапаПроизводстваВСтроке(ТекущиеДанные, ИмяТЧ, ДанныеНовогоЭтапа);
...
&НаСервере
Процедура ОбеспечениеМатериаламиИРаботамиОбработатьВыборЭтапаПроизводстваСервер(ТекущаяСтрока)
	
	Строка = Объект.ОбеспечениеМатериаламиИРаботами.НайтиПоИдентификатору(ТекущаяСтрока);
	ОбновитьКолонкуДоступноСервер();
...
&НаСервере
Процедура ОбеспечениеМатериаламиИРаботамиСсылкаРедактированиеОчисткаСервер(ТекущаяСтрока)
		...
		Для каждого ИдентификаторСтроки Из ВыделенныеСтроки Цикл
			...
			Если Не ЗначениеЗаполнено(СтрокаТаблицы.Ссылка) Тогда
				ТекстПредупреждения = НСтр("ru = 'Необходимо выбрать строки с заполненным этапом производства';
											|en = 'Select lines with the filled production step'");
				Прервать;
...
Процедура ЗаменитьНаАналоги(АдресВХранилище = Неопределено)
	...
	Результат = ?(АдресВХранилище = Неопределено,
		...
		АналогиМатериалов.ВыполнитьЗаменуНаАналоги(Объект[ИмяТаблицы], АдресВХранилище, ДополнительныеПоля + ", Ссылка", Истина));	
...
Процедура ОбработатьВыборЭтапаПроизводства(РезультатЗакрытия, ИмяТЧ) Экспорт
	...
	Для каждого ИдентификаторСтроки Из ВыделенныеСтроки Цикл
		...
		СтруктураДействий = ОбработатьИзменениеЭтапаПроизводстваВСтроке(ТекущиеДанные, ИмяТЧ, ОбъектЭтап);
	КонецЦикла;
	...
	ИначеЕсли ИмяТЧ = "ОбеспечениеМатериаламиИРаботами" Тогда
		...
		Если ОбеспечениеВДокументахКлиент.ТребуетсяОбновитьКолонкуДоступно(СтруктураДействий)
				Или ПравоЧтенияАналогов
					И ПоказатьАналоги Тогда
			ОбеспечениеМатериаламиИРаботамиОбработатьВыборЭтапаПроизводстваСервер(Элементы.ОбеспечениеМатериаламиИРаботами.ТекущаяСтрока);
		КонецЕсли;
