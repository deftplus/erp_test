...
// Возвращает текст правил конвертации объектов 1С в XDTO.
//
Функция ПолучитьПравилаКонвертацииВXDTO() Экспорт
	ОписаниеПравил = РегистрыСведений.ПравилаКонвертацииXSD.ПолучитьПоследнююРедакцию(
		Справочники.ПравилаКонвертацииXSD.Из1СВОбъектыЕИС);
	Если ОписаниеПравил = Неопределено Тогда
		Возврат ПолучитьПравилаКонвертацииИзМакета();
	КонецЕсли;
	Возврат ОписаниеПравил.ТекстПравил;
КонецФункции

Функция ПолучитьПравилаКонвертацииИзМакета() Экспорт
	ТекстСтруктуры = ПолучитьМакет("ПравилаКонвертацииВXDTO");
	Возврат ТекстСтруктуры.ПолучитьТекст();
КонецФункции
	
// Возвращает список команд обмена требующих обязательного заполнения
// рексизита Орагнизация.
//
