...
#Область СтатистикаПубликации
	...
	Если ПараметрыВыполнения.ИспользоватьСтатистикуПубликации Тогда
		...
		Если КоличествоОпубликованных = 0 И КоличествоОшибок = 0 Тогда
			ЧастиЗаголовка.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Отсутствуют предложения в сервисе';
																	|en = 'There are no offers in the service'"), , ЦветаСтиля.ПоясняющийТекст));
		ИначеЕсли КоличествоОшибок = 0 Тогда
			ЧастиЗаголовка.Добавить(Новый ФорматированнаяСтрока(СтрШаблон(НСтр("ru = 'Опубликовано: %1';
																				|en = 'Published: %1'"), КоличествоОпубликованных), , ЦветаСтиля.ПоясняющийТекст));
		ИначеЕсли КоличествоОпубликованных = 0 И КоличествоОшибок = 1000 Тогда
			ЧастиЗаголовка.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Ошибок: более 999';
																	|en = 'errors: over 999'"), , ЦветаСтиля.ПоясняющийОшибкуТекст));
		ИначеЕсли КоличествоОпубликованных = 0 Тогда
			ЧастиЗаголовка.Добавить(Новый ФорматированнаяСтрока(СтрШаблон(НСтр("ru = 'Ошибок: %1';
																				|en = 'errors: %1'"), КоличествоОшибок), , ЦветаСтиля.ПоясняющийОшибкуТекст));
