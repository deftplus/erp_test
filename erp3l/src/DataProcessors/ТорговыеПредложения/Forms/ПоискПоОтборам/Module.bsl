...
Процедура ДобавитьПредложениеВКорзинуПослеОповещения(Параметр)
	...
	СтрокиСписка = Список.НайтиСтроки(Новый Структура("Идентификатор", Параметр.Идентификатор));
	...
	ДобавитьТоварыДляЗаказа(Параметр, СтрокаСписка);
...
&НаКлиенте
Процедура ДобавитьТоварыДляЗаказа(ПараметрыТовара, СтрокаСписка)
	...
	Если РазрешитьИзменениеКонтрагента Тогда
		...
		Если ЗначениеЗаполнено(НаименованиеКонтрагента) Тогда
			...
			Если БыстрыеОтборы <> Неопределено Тогда
				ПредставлениеОтбора = НСтр("ru = 'Поставщик:';
											|en = 'Supplier:'") + " " + НаименованиеКонтрагента;
...
Процедура ДобавитьКонтекстныеБыстрыеОтборы(Форма)
	...
	Если ЗначениеЗаполнено(Форма.ОтборАртикулы) Тогда
		ПредставлениеОтбора = НСтр("ru = 'Артикул:';
									|en = 'Product ID:'") + " " + СтрСоединить(Форма.ОтборАртикулы.ВыгрузитьЗначения(), ", ");
		Форма.БыстрыеОтборы.Добавить("Артикул", ПредставлениеОтбора);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Форма.ОтборШтрихкоды) Тогда
		ПредставлениеОтбора = НСтр("ru = 'Штрихкод:';
									|en = 'Barcode:'") + " " + СтрСоединить(Форма.ОтборШтрихкоды.ВыгрузитьЗначения(), ", ");
