			...
			Пока Выборка.СледующийПоЗначениюПоля("Организация") Цикл
				...
				Пока Выборка.СледующийПоЗначениюПоля("Партнер") Цикл
					...
					Пока Выборка.СледующийПоЗначениюПоля("ОбъектРасчетов") Цикл
						...
						//Выборка по дате платежа, по регистру расчетов по срокам
...
Процедура СформироватьДокументыВводаОстатковТоварыКомиссионные(ОрганизацииУСНФИФО)
	...
	Пока Выборка.СледующийПоЗначениюПоля("Организация") Цикл
		...
		СистемаНалогообложения = НастройкиНалоговУчетныхПолитикЛокализация.СистемаНалогообложенияОрганизации(Выборка.Организация,
			КонецДня(ДатаСверткиИБ));
		ЭтоУСН = (СистемаНалогообложения = Перечисления.СистемыНалогообложения.Упрощенная);
		Если ЭтоУСН Тогда
	...
	Пока ВыборкаЗаказ.Следующий() Цикл
		...
		Для Каждого СтрокаТЧ Из ДокОст.Товары Цикл
			...
			Если ВыборкаСтрокаЗаказа.НайтиСледующий(СтруктураОтбора) Тогда
				...
				Иначе
					//Разделим строку на две, исполненную часть пометим как отмененную
					//Новая строка - будет копией старой строки  
					НоваяСтрокаТЧ = СтрокиКДобавлению.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрокаТЧ, СтрокаТЧ);
					НоваяСтрокаТЧ.КодСтроки = 0;
				...
				Если Выборка.ДокументПроведен Тогда
					...
					Исключение
						ТекстСообщения = НСтр("ru = 'Не удалось отменить проведение документа';
												|en = 'Cannot undo document posting'") + ": " + СокрЛП(ДокОст.Ссылка);
						СообщениеСвертки(ТекстСообщения, Истина, ОписаниеОшибки());
						Продолжить;
					КонецПопытки;
				КонецЕсли;
...
Функция ПреобразоватьСсылкуВПервичныйДокумент(ТолькоПоиск, ДокументСсылка, ДокументДата, ДокументНомер, ТипПервичногоДокумента = Неопределено, Сумма = 0, СуммаРегл = 0)
	...
	Если ТипПервичногоДокумента = Перечисления.ТипыПервичныхДокументов.ПриобретениеУПоставщика
		...
		|ГДЕ СФ.ДокументОснование = &ДокСсылка И СФ.Ссылка.Проведен = ИСТИНА
		|ОБЪЕДИНИТЬ ВСЕ
		|ВЫБРАТЬ РАЗЛИЧНЫЕ 
		|	СФ.Ссылка КАК Ссылка,
		|	ВЫБОР КОГДА НДСПродажи.Регистратор IS NULL ТОГДА
		...
		Пока ВыборкаСФ.Следующий() Цикл
			...
			Для Каждого ДокОсн Из СФОбъект.ДокументыОснования Цикл
				...
				КонецЕсли;
			КонецЦикла;
			Если ТипЗнч(ВыборкаСФ.Ссылка) = Тип("ДокументСсылка.СчетФактураВыданный") Тогда 
...
Процедура СформироватьДокументыВводаОстатковВнеоборотныхАктивов()
	...
	|	ВЫРАЗИТЬ(АрендованныеОС.Договор КАК Справочник.ДоговорыАренды) КАК Договор,
					...
					Пока Выборка.СледующийПоЗначениюПоля("ОтражатьВРеглУчете") Цикл
						Пока Выборка.СледующийПоЗначениюПоля("Местонахождение") Цикл
							Пока Выборка.СледующийПоЗначениюПоля("Контрагент") Цикл
								...
								ДокОст.ОтражатьВРеглУчете = Выборка.ОтражатьВРеглУчете;
								ДокОст.ОтражатьВБУ = Выборка.ОтражатьВРеглУчете; 
								ДокОст.ОтражатьВНУ = Выборка.ОтражатьВРеглУчете; 
		...
		Пока ВыборкаПодразделение.Следующий() Цикл
			...
			Пока ВыборкаУУ.Следующий() Цикл
				...
				Пока ВыборкаБУ.Следующий() Цикл
					...
					ДокОст.ОтражатьВБУ = Истина; 
					ДокОст.ОтражатьВНУ = Истина; 
					ДокОст.ОтражатьВОперативномУчете = Истина;
...
Процедура ОбработатьДокументыСтавкаНалогаНаИмуществоПоОС()
	...
	|ГДЕ СостоянияОС.СостояниеБУ <> ЗНАЧЕНИЕ(Перечисление.СостоянияОС.СнятоСУчета) 
	|	И СостоянияОС.СостояниеБУ <> ЗНАЧЕНИЕ(Перечисление.СостоянияОС.НеПринятоКУчету)
...
Процедура СформироватьДокументыВводаОстатковВзаиморасчетыЛизинг()
	...
	|Сумма(IsNULL(ТабАвансы.Сумма, 0)) КАК Сумма,
	|Сумма(IsNULL(ТабАвансы.СуммаУпр, 0)) КАК СуммаУпр,
	|Сумма(IsNULL(ТабАвансы.СуммаРегл, 0)) КАК СуммаРегл,
	|МАКСИМУМ(ТабАвансы.ТипПлатежа) КАК ТипПлатежа,
	|МАКСИМУМ(ТабАвансы.РасчетныйДокумент) КАК РасчетныйДокумент
	...
	|	ТабРасчеты.Договор
	|УПОРЯДОЧИТЬ ПО
	|	ТабРасчеты.Организация, ТабРасчеты.Партнер, ТабРасчеты.Контрагент, ТабРасчеты.Договор
	|";
	ЗаполнитьПараметрыЗапросаДатаСвертки(Запрос);
	...
	Пока Выборка.СледующийПоЗначениюПоля("Организация") Цикл
		Пока Выборка.СледующийПоЗначениюПоля("Партнер") Цикл
			Пока Выборка.СледующийПоЗначениюПоля("Контрагент") Цикл
				ДокОст = Документы.ВводОстатковВнеоборотныхАктивов2_4.СоздатьДокумент();
				ЗаполнитьШапкуДокументаВводаОстатков(ДокОст, КоличествоСформированныхДокументов, Выборка, Ложь);
				ДокОст.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковВзаиморасчетовПоДоговорамАренды;
				ДокОст.ОтражатьВРеглУчете = Истина;
				ДокОст.ОтражатьВУпрУчете = Истина;
				КодСтроки = 0;
				Пока Выборка.СледующийПоЗначениюПоля("Договор") Цикл
					КодСтроки = КодСтроки + 1;
					НоваяСтрока = ДокОст.РасчетыПоДоговорамАренды.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка);
					Для Каждого КолонкаСумм Из КолонкиСумм Цикл
						Если Выборка[КолонкаСумм] < 0 Тогда
							НоваяСтрока[КолонкаСумм] = (-1) * Выборка[КолонкаСумм];
						КонецЕсли;
					КонецЦикла;
					НоваяСтрока.КодСтроки = КодСтроки;
					// Авансы если есть.
					Пока Выборка.Следующий() Цикл
						Если Выборка.Сумма > 0 Или Выборка.СуммаУпр > 0 Или Выборка.СуммаРегл > 0 Тогда
							НоваяСтрокаАванс = ДокОст.АвансыПоДоговорамАренды.Добавить();
							ЗаполнитьЗначенияСвойств(НоваяСтрокаАванс, Выборка);
							НоваяСтрокаАванс.КодСтроки = КодСтроки;
						КонецЕсли;
					КонецЦикла;
					Если ДокОст.РасчетыПоДоговорамАренды.Количество() >= КоличествоСтрокВДокументеВводаОстатков Тогда
						ДокОст.Записать(РежимЗаписиДокумента.Запись);
						КоличествоСформированныхДокументов = КоличествоСформированныхДокументов + 1;
						ДокОст = Документы.ВводОстатковВнеоборотныхАктивов2_4.СоздатьДокумент();
						ЗаполнитьШапкуДокументаВводаОстатков(ДокОст, КоличествоСформированныхДокументов, Выборка, Ложь);
					...
					КонецЕсли;
				КонецЦикла;
				Если ДокОст.РасчетыПоДоговорамАренды.Количество()>0 Тогда
					ДокОст.Записать(РежимЗаписиДокумента.Запись);
					КоличествоСформированныхДокументов = КоличествоСформированныхДокументов + 1;
				КонецЕсли;
			КонецЦикла;
		КонецЦикла
	КонецЦикла;
	ВывестиСообщениеСформированыДокументы("ВводОстатковВнеоборотныхАктивов2_4", Перечисления.ХозяйственныеОперации.ВводОстатковВзаиморасчетовПоДоговорамАренды);
...
Функция СоздатьПервичныйДокументДляСтрокиРасчетов(ДокОст, ОбъектРасчетов, СтрокаРасчетов)
	...
	Если ЗначениеЗаполнено(ОбъектРасчетов) И 
		(ТипЗнч(ОбъектРасчетов) = Тип("СправочникСсылка.ДоговорыКонтрагентов")
