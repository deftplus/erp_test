...
// Надпись "Резерв превышает остаток на складе" в поле "Состояние".
	...
	Элемент.Представление = НСтр("ru = 'Текст ""Поступило"" если состояние ""Обеспечен на складе"" или ""В резерве""';
								|en = 'The ""Received"" text if the status is ""Supplied to warehouse"" or ""In reserve""'");
...
Процедура УправлениеВидимостью(ИмяЭлемента, Форма, КонтекстИспользования)
	...
	Если ИмяЭлемента = Неопределено Тогда
		...
		Если КонтекстИспользования = "ЗАКАЗ" Тогда
			...
			Если Форма.Параметры.КомандыСпискаВидимость Тогда
	...
	Если КодДействия = "ДЕЙСТВИЕ_ОТГРУЗИТЬ" Тогда
		...
		Если Не ЕстьРабота Тогда
			ПоказатьПредупреждение(,
				...
				|To execute the command, select the order lines in the states:
				|""In reserve"" / ""Supplied to warehouse"" / ""Not supplied""'"));
			Возврат;
		КонецЕсли;
