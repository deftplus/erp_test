	...
	Иначе
		...
		ТекстВопроса = ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для того чтобы не допустить рассогласования данных в программе,
				...
				|en = 'Attribute %1 is locked from editing to prevent data inconsistency.
				|
				...
				Если ЕстьОтборПоСтрокам Тогда 
					Пояснение = Пояснение + " " 
						+ НСтр("ru = 'Изменения будут выполнены в тех строках выбранных элементов, которые удовлетворяют <a href = ""ПерейтиКНастройкеОтбора"">условиям отбора</a>.';
								|en = 'Apply the changes only to the lines of the selected items that match the <a href = ""GoToSelectionSettings"">filter</a>.'")
				Иначе
					Пояснение = Пояснение + " " 
						+ НСтр("ru = 'Изменения будут выполнены <a href = ""ПерейтиКНастройкеОтбора"">во всех строках</a> выбранных элементов.';
								|en = 'Apply the changes <a href = ""GoToSelectionSettings"">to all lines</a> of the selected items.'")
				КонецЕсли;
			КонецЕсли;
		Иначе
