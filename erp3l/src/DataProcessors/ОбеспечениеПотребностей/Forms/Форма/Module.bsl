...
Процедура ЗаказПереработчикуКИсполнению(Команда)
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Невозможно установить статус. Не выбраны заказы переработчику';
																|en = 'Cannot set the status. Purchase orders — Subcontracting are not selected'"));
	КонецЕсли;
	//-- НЕ УТ
	Возврат;
...
Функция ФормированиеДереваЗаказовВФоне()
	...
	ПараметрыПроцедуры.Вставить("Запасы", Объект.Запасы.Выгрузить(,"ИсточникОбеспечения, Склад, СпособОбеспечения, 
								...
								|МинимальнаяСуммаЗаказа, ТипОбеспечения, ВидЦены, ТипЕдиницыИзмерения"));
	...
	Иначе
		...
		ИначеЕсли Не Строка.ФормироватьПлановыеЗаказы И Строка.ДоТочкиЗаказа > 0 Тогда
			...
			ТекстРекомендации = НСтр("ru = 'Не заказывать товар. Ожидать наступления точки заказа.';
									|en = 'Do not order the inventory. Wait till the reorder time.'");
			ЭтоТочкаЗаказа = Ложь;
			Заказать       = Ложь;
...
Процедура ОтразитьСвязанныеПоставкиВКорзине(СоответствиеСтрок)
	...
	Для Каждого КлючЗначение Из СоответствиеСтрок Цикл
		...
		#Область УХ_Внедрение
		Если Строка.ТипОбеспечения = ПредопределенноеЗначение("Перечисление.ТипыОбеспечения.Покупка") 
				И Строка.СогласноПланамПоставокПоДоговорам Тогда
			ОтразитьПоставкуСогласноПланамПоставкиВКорзине(ЭтаФорма, Строка, КоличествоКЗаказу, Не КлючЗначение.Значение);
		Иначе
		#КонецОбласти
...
Функция ПолучитьЦены(СписокТоваров, Соглашение)
	...
	Запрос.Текст = 
		...
		|	(ДанныеЗаполнения.Номенклатура.СтавкаНДС.Ставка + 100) / 100
		|	ИНАЧЕ
		|	1
		|	КОНЕЦ
...
Процедура УстановитьУсловноеОформление()
	...
	Элемент.Представление = НСтр("ru = 'Гиперссылка метод обеспечения';
								|en = 'Hyperlink procurement strategy'");
...
Процедура ОбработатьВыборПоПлануПоставки(АдресВХранилище, ТаблицаЗапасы)
	...
	//Поставка.ИсточникОбеспечения = ОбработатьПустойИсточникОбеспечения(ВыбранноеЗначение, ТекстНеЗаполнено);
	Поставка.ИсточникОбеспечения = НСтр("ru = 'Согласно плану поставок по договорам'");
	КоличествоКЗаказу = Поставка.Округлено * ?(Поставка.Отметка, 1, 0);
...
Процедура УстановитьУсловноеОформлениеУХ(Форма)
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'Согласно плану поставок по договорам'"));
