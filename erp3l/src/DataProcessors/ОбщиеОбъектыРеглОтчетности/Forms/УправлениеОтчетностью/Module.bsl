...
Процедура Выгрузить(Команда)
	...
	Если ТипЗнч(Ссылка) = Тип("ДокументСсылка.РегламентированныйОтчет") Тогда
		...
		Если НаименованиеОтчета = "Реестр для налоговых вычетов по акцизам на виноград" Тогда
			...
			|form approved by the order of the Federal Tax Service of Russia
			|from 04/26/2021 No. ED-7-15/419@ will
			|be available in future application versions'"));
			Возврат;
		КонецЕсли;
		...
		Если НаименованиеОтчета
			...
			|en = 'Online export of the report
			|form approved by the order of the Federal Tax Service of Russia
			|from 04/26/2021 No. ED-7-15/418@ will
			|be available in future application versions'"));
			Возврат;
		КонецЕсли;
