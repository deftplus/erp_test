		...
		Если МетаданныеРоли = Неопределено Тогда
			ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В поставляемом профиле ""%1""
				           ...
				           |en = 'Role ""%2"" provided in built-in profile
				           |""%1"" does not exist.'"),
	   ...
	   И НЕ ПараметрыОбновления.ОбновлятьИзмененныеПрофили Тогда
		...
		ТекстОшибки = ЗаголовокОшибки
			+ НСтр("ru = 'Когда в параметре ПараметрыОбновления свойство
			             ...
			             |en = 'When the UpdateModifiedProfiles property
		...
		Если Не ЗначениеЗаполнено(ОписаниеПрофиля.Идентификатор) Тогда
			ШаблонОшибки = ?(ЭтоГруппа,
				НСтр("ru = 'В описании папки профилей ""%1"" не заполнено свойство Идентификатор.';
					|en = 'ID is not specified for the ""%1"" profile folder.'"),
				НСтр("ru = 'В описании профиля ""%1"" не заполнено свойство Идентификатор.';
					|en = 'ID is not specified for profile ""%1"".'"));
		...
		Если ВсеИдентификаторы.Получить(ВРег(СвойстваПрофиля.Идентификатор)) <> Неопределено Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Идентификатор ""%1"" уже используется в описании другого профиля или папки профилей.';
					|en = 'ID ""%1"" is already used in details of another profile or profile folder.'"),
		...
		Если ЭтоГруппа И Не ЗначениеЗаполнено(СвойстваПрофиля.Имя) Тогда
			ТекстОшибки =  ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В описании папки профилей ""%1"" не заполнено свойство Имя.';
					|en = 'Name is not specified for the ""%1"" profile folder.'"),
			...
			Если СокрЛП(СвойстваПрофиля.Имя) <> СвойстваПрофиля.Имя Тогда
				ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Имя ""%1"" профиля или папки профилей содержит непечатаемые символы.';
						|en = 'Name ""%1"" of a profile or profile folder contains non-printable characters.'"),
					СвойстваПрофиля.Имя);
				ВызватьИсключение ТекстОшибки;
			КонецЕсли;
			Если ВсеИмена.Получить(ВРег(СвойстваПрофиля.Имя)) <> Неопределено Тогда
				ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Имя ""%1"" уже используется в описании другого профиля или папки профилей.';
						|en = 'Name ""%1"" is already used in details of another profile or profile folder.'"),
					СвойстваПрофиля.Имя);
				ВызватьИсключение ТекстОшибки;
			КонецЕсли;
	...
	Для Каждого ОписаниеПрофиля Из ОписанияПрофилей Цикл
		...
		ШаблонОшибки = ?(ЭтоГруппа,
			НСтр("ru = 'В описании папки профилей ""%1"" в свойстве родитель
			           ...
			           |en = 'The details of the ""%1"" profile folder have
			           |a non-existent name of the ""%2"" profile folder specified in the parent property.'"),
			НСтр("ru = 'В описании профиля ""%1"" в свойстве родитель
			           |указано несуществующее имя папки профилей ""%2"".';
			           |en = 'The details of the ""%1"" profile have
			           |a non-existent name of the ""%2"" profile folder specified in the parent property.'"));
		ТекстОшибки =  ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОшибки,
			?(ЗначениеЗаполнено(ОписаниеПрофиля.Имя), ОписаниеПрофиля.Имя, ОписаниеПрофиля.Наименование),
		...
		Пока Истина Цикл
			...
			ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В описании папки профилей ""%1"" в свойстве родитель
				           ...
				           |en = 'In the details of the ""%1"" profile folder, the parent property
				           |contains the ""%2"" profile folder name
				           |that creates a circular dependency: ""%3"".'"),
				?(ЗначениеЗаполнено(ОписаниеПрофиля.Имя), ОписаниеПрофиля.Имя, ОписаниеПрофиля.Наименование),
				ОписаниеПрофиля.Родитель,
			   ...
			   И Не Метаданные.ОпределяемыеТипы.ВнешнийПользователь.Тип.СодержитТип(Тип) Тогда
				ТекстОшибки = ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'В описании профиля ""%1""
					           ...
					           |en = 'Profile ""%1"" has invalid assignment ""%2 (%3)"".
					           |
					           |An assignment with the following properties is expected:
					           |- The type is ""Type"".
					           |- Specified in the User defined type or
					           |- Specified in the ExternalUser defined type (except for the CatalogRef.Users type).'"),
				...
				Если МетаданныеРоли = Неопределено Тогда
					ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'При обновлении поставляемого профиля ""%1""
						           ...
						           |en = 'Non-existent role ""%2"" is found
						           |upon updating built-in profile ""%1"".'"),
						СвойстваПрофиля.Наименование,
						Роль);
				КонецЕсли;
