...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Если ОбъектовОбработано = 0 И ПроблемныхОбъектов <> 0 Тогда
		...
		НСтр("ru = 'Процедуре Справочники.ПрограммыЭлектроннойПодписиИШифрования.ОбработатьДанныеДляПереходаНаНовуюВерсию не удалось обработать некоторые программы электронной подписи (пропущены): %1';
			|en = 'Procedure Catalogs.DigitalSignatureAndEncryptionApplications.ProcessDataForMigrationToNewVersion failed to process (skipped) some digital signature applications: %1'"), 
		ПроблемныхОбъектов);
	...
	Иначе
		...
		НСтр("ru = 'Процедура Справочники.ПрограммыЭлектроннойПодписиИШифрования.ОбработатьДанныеДляПереходаНаНовуюВерсию обработала очередную порцию программ электронной подписи: %1';
			|en = 'The Catalogs.DigitalSignatureAndEncryptionApplications.ProcessDataForMigrationToNewVersion procedure has processed digital signature applications:%1'"),
		ОбъектовОбработано));
	КонецЕсли;
