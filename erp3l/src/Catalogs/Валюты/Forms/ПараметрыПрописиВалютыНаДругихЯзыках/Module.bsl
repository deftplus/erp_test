	...
	Для Каждого ЯзыкКонфигурации Из Языки Цикл
		...
		Добавлять.Добавить(Новый РеквизитФормы("ПодсказкаВвода" + ЯзыкКонфигурации.Имя, ТипСтрока,,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Подсказка ввода для языка %1';
																		|en = 'Input tooltip for the %1 language'"), ЯзыкКонфигурации.Представление)));
	КонецЦикла;
...
Функция ПодсказкаЗаполненияПараметровПрописей(Знач КодЯзыка)
	...
	Если КодЯзыка = "ru" Или КодЯзыка = "be" Тогда
		...
		|en = 'Parameters for writing amounts in words must be separated by commas.
		|Sample filling for Russian and Belarusian (ru_RU, be_BY):
		|
		|рубль, рубля, рублей, м, копейка, копейки, копеек, ж, 2
		|
		|""рубль, рубля, рублей, м"" – the calculation subject:
		|рубль – nominative singular
		|рубля – genitive singular
		|рублей – genitive plural
		|м – masculine (ж – feminine, с – neuter)
		|""копейка, копейки, копеек, ж"" – the fractional part similar to the calculation subject (may be missing)
	...
	ИначеЕсли КодЯзыка = "uk" Тогда
		...
		|""2"" – количество разрядов дробной части (может отсутствовать, по умолчанию равно 2).';
		|en = 'Parameters for writing amounts in words must be separated by commas.
		|Sample filling for Ukrainian (uk_UA):
		|
		|гривна, гривны, гривен, м, копейка, копейки, копеек, ж, 2
		|
		|""гривна, гривны, гривен, м"" – the calculation subject:
	...
	ИначеЕсли КодЯзыка = "pl" Тогда
		...
		|2 - количество разрядов дробной части (может отсутствовать, по умолчанию равно 2).';
		|en = 'Parameters for writing amounts in words must be separated by commas.
		|Sample filling for Polish (pl_PL):
		|
		|złoty, złote, złotych, m, grosz, grosze, groszy, m, 2
		|
	...
	ИначеЕсли КодЯзыка = "en" Или КодЯзыка = "fr" Или КодЯзыка = "fi" Или КодЯзыка = "kk" Тогда
		...
		|""2"" – количество разрядов дробной части (может отсутствовать, по умолчанию равно 2).';
		|en = 'Parameters for writing amounts in words must be separated by commas.
		|Sample filling for English, French, Finnish and Kazakh (en_US, fr_CA,fi_FI, kk_KZ):
		|
		|dollar, dollars, cent, cents, 2
		|
		|""dollar, dollars"" – calculation subject singular and plural
	...
	ИначеЕсли КодЯзыка = "de" Тогда
		...
		|""2"" – количество разрядов дробной части (может отсутствовать, по умолчанию равно 2).';
		|en = 'Parameters for writing amounts in words must be separated by commas.
		|Sample filling for German (de_DE):
		|
		|EURO, EURO, М, Cent, Cent, M, 2
		|
		|""EURO, EURO, М"" – the calculation subject:
		|EURO, EURO - calculation subject singular and plural
		|М – masculine (F – feminine, N - neuter)
		|""Cent, Cent, M"" – the fractional part similar to the calculation subject (may be missing)
		|""2"" – the number of decimal places (may be missing; the default value is 2).'"));
	...
	ИначеЕсли КодЯзыка = "lv" Тогда
		...
		|может отсутствовать, по умолчанию равно ""J"".';
		|en = 'Parameters for writing amounts in words must be separated by commas.
		|Sample filling for Latvian (lv_LV):
		|
		|lats, lati, latu, V, santīms, santīmi, santīmu, V, 2, J, J
		|
		|""lats, lati, latu, v"" – the calculation subject:
		...
		|""santīms, santīmi, santīmu, V"" – the fractional part similar to the calculation subject (may be missing)
		|""2"" – the number of decimal places (may be missing; the default value is 2)
		|""J"" - the number 100 is displayed as ""One hundred"" for the calculation subject (N - the number 100 is displayed as ""Hundred"");
	...
	ИначеЕсли КодЯзыка = "lt" Тогда
		...
		|""2"" - количество разрядов дробной части (может отсутствовать, по умолчанию равно 2).';
		|en = 'Parameters for writing amounts in words must be separated by commas.
		|Sample filling for Lithuanian (lt_LT):
		|
		|litas, litai, litų, М, centas, centai, centų, М, 2
		|
		|""litas, litai, litų, М"" – the calculation subject:
		...
		|""centas, centai, centų, М"" – the fractional part similar to the calculation subject (may be missing)
		|""2"" - the number of decimal places (may be missing; the default value is 2).'"));
	...
	ИначеЕсли КодЯзыка = "et" Тогда
		...
		|2 – количество разрядов дробной части (может отсутствовать, по умолчанию равно 2).';
		|en = 'Parameters for writing amounts in words must be separated by commas.
		|Sample filling for Estonian (et_EE):
		|
		|kroon, krooni, sent, senti, 2
		|
		|""kroon, krooni"" – calculation subject singular and plural
	...
	ИначеЕсли КодЯзыка = "bg" Тогда
		...
		|en = 'Parameters for writing amounts in words must be separated by commas.
		|Sample filling for Bulgarian (bg_BG):
		|
		|лев, лева, м, стотинка, стотинки, ж, 2
		|
		|""лев, лева, м"" – the calculation subject:
	...
	ИначеЕсли КодЯзыка = "ro" Тогда
		...
		|""2"" - количество разрядов дробной части.';
		|en = 'Parameters for writing amounts in words must be separated by commas.
		|Sample filling for Romanian (ro_RO):
		|
		|leu, lei, M, ban, bani, W, 2
		|
		|""leu, lei, M"" – the calculation subject:
	...
	ИначеЕсли КодЯзыка = "ka" Тогда
		...
		|""2"" - количество разрядов дробной части.';
		|en = 'Parameters for writing amounts in words must be separated by commas.
		|Sample filling for Georgian (ka_GE):
		|
		|ლარი, თეთრი, 2
		|
	...
	ИначеЕсли КодЯзыка = "az" Или КодЯзыка = "tk" Тогда
		...
		|en = 'Parameters for writing amounts in words must be separated by commas.
		|Sample filling for Azerbaijani (az) and Turkmen (tk):
		|
		|TL,Kr,2
		|
		|""TL"" - the calculation subject
	...
	ИначеЕсли КодЯзыка = "vi" Тогда
		...
		|2 - количество разрядов дробной части.';
		|en = 'Parameters for writing amounts in words must be separated by commas.
		|Sample filling for Vietnamese (vi_VN):
		|
		|dong, xu, 2
		|
	...
	ИначеЕсли КодЯзыка = "tr" Тогда
		...
		|en = 'Parameters for writing amounts in words must be separated by commas.
		|Sample filling for Turkish (tr_TR):
		|
		|TL,Kr,2,Separate
		|
	...
	ИначеЕсли КодЯзыка = "hu" Тогда
		...
		|""2"" - количество разрядов дробной части.';
		|en = 'Parameters for writing amounts in words must be separated by commas.
		|Sample filling for Hungarian (hu):
		|
		|Forint, fillér, 2
		|
