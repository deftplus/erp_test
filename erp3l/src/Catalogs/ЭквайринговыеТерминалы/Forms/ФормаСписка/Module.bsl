...
Процедура ПеренестиИсточникВПриемник(Источник, Приемник)
	...
	Если ЗначениеЗаполнено(Приемник.Владелец)
		...
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Невозможно переместить элемент %1:
							|	договор эквайринга группы не соответствует перемещаемому элементу';
							|en = 'Cannot move item %1:
							|	 group acquiring agreement does not match the item being moved'"), Строка(Источник.Наименование));
