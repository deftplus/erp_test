...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Генерирует элементы справочника Договоры эквайринга.';
									|en = 'Generates elements of Acquiring agreements catalog.'");
	...
	Пока ВыборкаКонтрагенты.Следующий() Цикл
		...
		Пока ВыборкаБанковскийСчет.Следующий() Цикл
			...
			Пока ВыборкаДоговоры.Следующий() Цикл
				...
				Если Не ЗначениеЗаполнено(ВыборкаБанковскийСчет.ДоговорЭквайринга) Тогда
					...
					Исключение
						...
						Шаблон = НСтр("ru = 'Не удалось сгенерировать договор эквайринга по причине: %1';
										|en = 'Cannot generate acquiring agreement. Reason: %1'");
				...
				//++ НЕ УТ
				//++ Локализация
				Если ЗначениеЗаполнено(ДоговорВладелец) Тогда
					СтруктураИзмерений = НастройкаСчетовУчетаКлиентСервер.ИнициализироватьСтруктуруИзмеренийРегистра();
					СтруктураИзмерений.АналитикаУчета = ДоговорВладелец;
					СтруктураСчетовИсключений = Новый Структура("СчетУчета_ПродажиПоЭквайрингу", ПланыСчетов.Хозрасчетный.ПродажиПоПлатежнымКартам);
					РегистрыСведений.ПорядокОтраженияНаСчетахУчета.СохранитьЗначенияСчетовУчета(СтруктураИзмерений, СтруктураСчетовИсключений, Ложь);
				КонецЕсли;
				//-- Локализация
				//-- НЕ УТ
