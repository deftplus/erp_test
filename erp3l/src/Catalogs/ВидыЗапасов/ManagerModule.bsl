		...
		ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПриемНаХранениеСПравомПродажи
			...
			Если Не ЗначениеЗаполнено(РеквизитыДокумента.ВидЦены) и Не ТипДоговораПоставкаПодПринципала Тогда
				ТекстыИсключения.Добавить(НСтр("ru = 'Не заполнен обязательный реквизит ""Вид Цены"".';
												|en = 'Attribute ""Price type"" is required.'"));
			КонецЕсли;
			...
			ИначеЕсли ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОтгрузкаПринятыхСПравомПродажиТоваровСХранения
				...
				|Even if balance control is disabled, the application does not support goods shipment from custody in the amount exceeding the actual amount of goods in custody.'");
				ВызватьИсключение ТекстИсключения;
			КонецЕсли;
