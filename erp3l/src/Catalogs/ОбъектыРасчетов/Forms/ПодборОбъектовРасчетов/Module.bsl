...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПараметрДополнительныеОтборы = ОбщегоНазначения.СкопироватьРекурсивно(Параметры.ДополнительныеОтборы);
	Если ПараметрДополнительныеОтборы <> Неопределено Тогда
		ПараметрДополнительныеОтборы.Вставить("ПометкаУдаления", Ложь);
	Иначе
		ПараметрДополнительныеОтборы = Новый Структура("ПометкаУдаления", Ложь);
	КонецЕсли;
	ЗаполнитьТаблицуПоРасчетамСПартнерами(ПараметрДополнительныеОтборы);
	УстановитьВидимость();
