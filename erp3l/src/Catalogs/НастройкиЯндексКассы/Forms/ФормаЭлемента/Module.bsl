...
Процедура ПроверкаЗаполненияСтраницаВариантыИВыборОрганизации(Отказ)
	...
	Если Не ЗначениеЗаполнено(Организация) Тогда
		...
		Иначе 
			ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Настройте организацию и повторите попытку настройки';
...
Процедура СоздатьЗаписатьНастройкуЯндексКассы(Отказ)
	...
	Исключение
		...
		ЭлектронноеВзаимодействие.ОбработатьОшибку(НСтр("ru = 'Запись настройки по Яндекс Кассе';
														|en = 'Save Yandex.Checkout settings'"),
			ОписаниеОшибки(), НСтр("ru = 'Возникла ошибка при записи настройки по ЮKassa';
									|en = 'An error occurred while saving YooMoney setting'"), 6);
	КонецПопытки;
