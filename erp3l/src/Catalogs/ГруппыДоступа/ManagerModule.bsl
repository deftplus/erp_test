	...
	Если ОбъектовОбработано = 0 И ПроблемныхОбъектов <> 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Процедуре Справочники.ГруппыДоступа.ОбработатьДанныеДляПереходаНаНовуюВерсию не удалось
			           ...
			           |en = 'Procedure Catalogs.AccessGroups.ProcessDataForMigrationToNewVersion failed to update service data in %1 access groups. The access groups were skipped.'"), 
			ПроблемныхОбъектов);
	...
	Иначе
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
			...
			НСтр("ru = 'Процедура Справочники.ГруппыДоступа.ОбработатьДанныеДляПереходаНаНовуюВерсию
			           ...
			           |processed the next batch of access groups: %1.'"),
		ОбъектовОбработано));
	КонецЕсли;
...
Процедура ОбновитьВспомогательныеДанныеГруппДоступа(Параметры) Экспорт
	...
	ШаблонОшибкиОбработкиГруппыДоступа =
		НСтр("ru = 'Не удалось обработать группу доступа ""%1"" по причине:
		           |%2';
		           |en = 'Cannot process the ""%1"" access group. Reason:
		           |%2'");
	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Недопустимое значение ""%1"" параметра ВидСсылок функции СвойстваВидаСсылок.';
				|en = 'Invalid value ""%1"" of the RefsKind parameter of the RefsKindProperties function.'"),
			ВидСсылок);
	КонецЕсли;
