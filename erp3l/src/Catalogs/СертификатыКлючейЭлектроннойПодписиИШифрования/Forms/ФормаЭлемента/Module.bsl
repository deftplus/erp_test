...
Процедура СертификатОтозван(Команда)
	...
	Если Объект.Отозван Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'После записи отменить отзыв будет невозможно.';
										|en = 'After saving, callback cannot be canceled.'"));
	КонецЕсли;
...
Процедура ОформитьСписокПользователей()
	...
	ИначеЕсли КоличествоПользователей > 1 Тогда
		ПредставлениеПользователей = НСтр("ru = 'Несколько пользователей';
											|en = 'Several users'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка()) 
	...
	Иначе
		ПредставлениеПользователей = НСтр("ru = 'Не указаны';
											|en = 'Not specified'", ОбщегоНазначенияКлиент.КодОсновногоЯзыка());
	КонецЕсли;
