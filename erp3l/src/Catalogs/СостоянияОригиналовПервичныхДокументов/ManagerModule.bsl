...
Процедура ПриНачальномЗаполненииЭлементов(КодыЯзыков, Элементы, ТабличныеЧасти) Экспорт
	...
	Элемент.Наименование = НСтр("ru = 'Форма напечатана';
								|en = 'Form has been printed'", ОбщегоНазначения.КодОсновногоЯзыка());
	Элемент.Описание = НСтр("ru = 'Состояние, означающее, что  печатная форма только печаталась.';
							|en = 'A status indicating that the print form has just been printed.'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Элемент.Наименование = НСтр("ru = 'Оригиналы не все';
								|en = 'Incomplete originals'", ОбщегоНазначения.КодОсновногоЯзыка());
	Элемент.Описание = НСтр("ru = 'Общее состояние для документа, у которого оригиналы печатных форм находятся в разных состояниях.';
							|en = 'General status of a document in which the print form originals have different statuses.'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Элемент.Описание = НСтр("ru = 'Состояние, означающее, что подписанный оригинал печатной формы есть в наличии.';
							|en = 'A status indicating that a signed print form original is available.'", ОбщегоНазначения.КодОсновногоЯзыка());
		...
		Исключение
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось обработать состояние: %1 по причине:
					|%2';
					|en = 'Cannot process status ""%1"" due to:
					|%2'"), 
	...
	Если ОбъектовОбработано = 0 И ПроблемныхОбъектов <> 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Процедуре ЗаполнитьРеквизитРеквизитДопУпорядочивания не удалось обработать некоторые состояния (пропущены): %1';
				|en = 'Procedure ЗаполнитьРеквизитРеквизитДопУпорядочивания failed to process and skipped %1 statuses.'"), 
				ПроблемныхОбъектов);
		...
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Информация,
			Метаданные.Справочники.СостоянияОригиналовПервичныхДокументов,,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Процедура ЗаполнитьРеквизитРеквизитДопУпорядочивания обработала очередную порцию состояний оригиналов: %1';
																			|en = 'Procedure ЗаполнитьРеквизитРеквизитДопУпорядочивания processed another batch of statuses: %1'"),
					ОбъектовОбработано));
	КонецЕсли;
