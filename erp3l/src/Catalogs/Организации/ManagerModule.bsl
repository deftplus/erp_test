...
// Возвращаемое значение:
// 	Булево
Функция ВалютыРегламентированногоУчетаСовпадают(Организация1, Организация2) Экспорт
	
	МассивСсылок = Новый Массив();
	МассивСсылок.Добавить(Организация1);
	МассивСсылок.Добавить(Организация2);
	Результат = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(МассивСсылок, "ВалютаРегламентированногоУчета");
	
	Возврат Результат.Получить(Организация1) = Результат.Получить(Организация2);
...
#Область ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет головную организацию и валюту регламентированного учета';
									|en = 'Fill in a parent company and a local accounting currency'");
