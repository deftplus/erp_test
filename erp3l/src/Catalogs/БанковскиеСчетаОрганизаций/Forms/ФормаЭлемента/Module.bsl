...
&НаКлиенте
Процедура НомерСчетаПриИзмененииКлиент()
	
	Элементы.ГруппаОшибкаВНомереСчета.Видимость = Ложь;
	Объект.НомерСчета = СтрЗаменить(СтрЗаменить(СокрЛП(Объект.НомерСчета), " ", ""), ".", "");
	...
	Если МеждународныеРеквизитыБанковскихСчетов Тогда
		...
		Если Сред(Объект.НомерСчета, 1, 4) = "IBAN" Тогда
			Объект.НомерСчета = Сред(Объект.НомерСчета, 5);
		КонецЕсли;
...
Процедура Подключаемый_НомерСчетаПриИзмененииЛокализация(Элемент)
	...
	Если ДенежныеСредстваКлиентЛокализация.СтрокаСоответствуетФорматуБанковскогоСчетаРФ(Объект.НомерСчета) Тогда
		СтранаБанка = ПредопределенноеЗначение("Справочник.СтраныМира.Россия");
...
Функция ЭтоБанкРФ()
	...
	Если Не Объект.ЭтоIBAN
		И ДенежныеСредстваКлиентЛокализация.СтрокаСоответствуетФорматуБанковскогоСчетаРФ(Объект.НомерСчета) Тогда
		Возврат Истина;
	КонецЕсли;
