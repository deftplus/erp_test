...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		ПерераспределятьСкладскиеЗапасы = Ложь;
...
Процедура НастроитьЭлементыФормыПоПравамДоступа()
	...
	Элементы.ПерераспределятьСкладскиеЗапасы.Видимость = ЧтениеНастройкаКонтроляОбеспечения;
	Элементы.ПерераспределятьСкладскиеЗапасы.Доступность = ИзменениеНастройкаКонтроляОбеспечения;
...
Процедура ПриЧтенииСОграниченнымиПравами(ТекущийОбъект)
	...
	Если ЗначениеЗаполнено(НастройкаКонтроляОбеспечения.ПерераспределятьСкладскиеЗапасы) Тогда
		ПерераспределятьСкладскиеЗапасы = НастройкаКонтроляОбеспечения.ПерераспределятьСкладскиеЗапасы;
	Иначе
		ПерераспределятьСкладскиеЗапасы = ТекущийОбъект.ПерераспределятьСкладскиеЗапасы;
	КонецЕсли;
...
Процедура ПередЗаписьюСОграниченнымиПравами(ТекущийОбъект)
	...
	Если ПравоДоступа("Изменение", Метаданные.РегистрыСведений.НастройкаКонтроляОбеспечения) Тогда
		...
		ТекущийОбъект.ПерераспределятьСкладскиеЗапасы = ПерераспределятьСкладскиеЗапасы;
	КонецЕсли;
...
Процедура УстановитьЗаголовокЭлементовНастройкиСборкиЗаказовИКурьерскойДоставки(Форма)
	...
	ТекстЗаголовка = НСтр("ru = 'Настроить сборку заказов и доставку своими курьерами';
							|en = 'Configure order assembly and delivery by own couriers'");
	Объект = Форма.Объект;
	...
	ИначеЕсли ЗначениеЗаполнено(Форма.ФорматМагазина)
		...
		ТекстЗаголовка = НСтр("ru = 'Посмотреть настройки сборки заказов и доставки своими курьерами формата магазина';
								|en = 'View settings of order assembly and delivery by own couriers of the store format'");
	ИначеЕсли Объект.ВМагазинеПоддерживаетсяСборкаЗаказов
		...
		ТекстЗаголовка = НСтр("ru = 'Изменить настройки сборки заказов и доставки своими курьерами';
								|en = 'Edit settings for order assembly and delivery by own couriers'");
	КонецЕсли;
