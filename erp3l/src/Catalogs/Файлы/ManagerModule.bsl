...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Если ОбъектовОбработано = 0 И ПроблемныхОбъектов <> 0 Тогда
		...
		НСтр("ru = 'Процедуре Справочники.Файлы.ОбработатьДанныеДляПереходаНаНовуюВерсию.ОбработатьДанныеДляПереходаНаНовуюВерсию не удалось обработать программы электронной подписи (пропущены): %1';
			|en = 'Procedure Catalogs.Files.ProcessDataForMigrationToNewVersion.ProcessDataForMigrationToNewVersion failed to process and skipped %1 digital signature applications.'"), 
		ПроблемныхОбъектов);
	...
	Иначе
		...
		НСтр("ru = 'Процедура Справочники.Файлы.ОбработатьДанныеДляПереходаНаНовуюВерсию обработала очередную порцию программ электронной подписи: %1';
			|en = 'The Catalogs.Files.ProcessDataForMigrationToNewVersion procedure processed the next part of digital signature applications: %1.'"),
		ОбъектовОбработано));
	КонецЕсли;
