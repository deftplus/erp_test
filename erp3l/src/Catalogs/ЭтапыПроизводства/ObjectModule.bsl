...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ДополнительныеСвойства.Свойство("НеУчитыватьСтатусВладельца")
		ИЛИ (ЗначениеЗаполнено(Владелец) И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Владелец, "Статус")
														<> Перечисления.СтатусыСпецификаций.ВРазработке) Тогда
	...
	Если ПланироватьРаботуВидовРабочихЦентров Тогда
		...
		РеквизитыДляПроверки.Добавить(
			...
			НСтр("ru = 'Одновременно производимое количество единиц/партий изделий';
				|en = 'Quantity of product units/lots produced simultaneously'"));
		...
		Если СообщитьНаименованиеЭтапа
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru = 'Вид рабочего центра ""%1"" допускается указывать не более одного раза (этап: %2).';
										|en = 'You can specify work center type ""%1"" only once (stage: %2).'"),
									Строка(ВидРЦ),
									КлючДанных.Наименование);
		Иначе
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
