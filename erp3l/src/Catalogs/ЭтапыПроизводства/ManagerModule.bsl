	...
	Для каждого СсылкаНаЭтап Из СписокЭтапов Цикл
		...
		Исключение
			...
			ИмяСобытия = НСтр("ru = 'Ресурсная спецификация';
								|en = 'Bill of materials'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Пересчитывает длительность этапов, содержащих виды рабочих центров с расчетом времени работы по формулам. Конвертирует формулы расчета количества  в новый формат.';
									|en = 'Recalculates the duration of the stages containing the work center types with the calculation of the working time using the formulas. Converts quantity calculation formulas to a new format.'");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Запрос = Новый Запрос(
		...
		|	Справочник.ЭтапыПроизводства КАК ЭтапыПроизводства
		|ГДЕ
		|	НЕ ЭтапыПроизводства.УдалитьПереходНаНовыеФормулыВыполнен");
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Для каждого ЭлементСправочника Из ОбновляемыеДанные Цикл
		...
		Попытка
			...
			Если СправочникОбъект.ПланироватьРаботуВидовРабочихЦентров
				И НЕ СправочникОбъект.ВидыРабочихЦентровСФормуламиОбновлены
				...
				Для каждого Строка Из СправочникОбъект.ВидыРабочихЦентров Цикл
					Если НЕ ПустаяСтрока(Строка.АлгоритмРасчетаКоличества) Тогда
						...
						РегистрыСведений.ЗаданияКРасчетуДлительностиПроизводства.ДобавитьЗадание(СправочникОбъект.Владелец, Истина, Ложь);
			...
			Если НЕ СправочникОбъект.УдалитьПереходНаНовыеФормулыВыполнен Тогда
				РаботаСФормулами.ПреобразоватьВФорматИдентификаторовВыражениеФормулОбъектаПланированияПроизводства(
					СправочникОбъект, ОбъектИзменен);
