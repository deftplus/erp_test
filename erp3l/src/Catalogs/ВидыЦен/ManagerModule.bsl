...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	|
	|2.Заполняет реквизит Назначение с типом %Парам1%.';
	|en = '1.Populates attributes:
	|""Use for retail sales""
	|ИспользоватьПриОптовойПродаже
	|from Boolean types. Sets True if attribute ""Use for wholesale"" is set
	|
	|2.Populates Assignment attribute with type %Парам1%.'");
	
	Обработчик.Комментарий = СтрЗаменить(Обработчик.Комментарий, "%Парам1%", "ПеречислениеСсылка.НазначенияВидовЦен");
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсиюДляНовогоЦенообразования(Параметры) Экспорт
	...
	Для Каждого ЭлементСправочника Из ОбновляемыеДанные Цикл
		...
		Попытка
			...
			Если СправочникОбъект.СхемаКомпоновкиДанных = "ЦеныПоступленияПредопределенный" Тогда
				...
				ПараметрыМакета.ИмяМакета 	= "ЦеныПоступленияПредопределенный";
