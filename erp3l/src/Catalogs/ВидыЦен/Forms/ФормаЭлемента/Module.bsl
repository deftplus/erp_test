...
Процедура УстановитьПараметрыПравилРасчета()
	...
	Если Объект.СпособЗаданияЦены = Перечисления.СпособыЗаданияЦен.ЗаполнятьПоДаннымИБПоПоставщикам
		...
		Элементы.ПараметрыВидаРасчета.СписокВыбора.Добавить(1,"Максимальные цены");
		Элементы.ПараметрыВидаРасчета.СписокВыбора.Добавить(2,"Минимальные цены");
		Элементы.ПараметрыВидаРасчета.СписокВыбора.Добавить(3,"Средние цены");
		Элементы.ПараметрыВидаРасчета.СписокВыбора.Добавить(4,"Последние цены");
	...
	ИначеЕсли Объект.СпособЗаданияЦены = Перечисления.СпособыЗаданияЦен.ЗаполнятьПоДаннымИБПоСебестоимости Тогда
		...
		Элементы.ПараметрыВидаРасчета.СписокВыбора.Очистить();
		Элементы.ПараметрыВидаРасчета.СписокВыбора.Добавить(1,"Без дополнительных расходов");
		Элементы.ПараметрыВидаРасчета.СписокВыбора.Добавить(2,"С дополнительными расходами");
...
Процедура СформироватьСписокВыбораНазначенийВидовЦен()
	...
	СписокВыбора.Добавить(ПредопределенноеЗначение("Перечисление.НазначенияВидовЦен.Общий"), "всех");
		...
		Если Объект.ПравилаОкругленияЦены.Количество() > 1 
			...
			Для Каждого Правило Из Объект.ПравилаОкругленияЦены Цикл
				Если Правило.ТочностьОкругления = 0 Тогда
					МассивПравил.Добавить("" + Правило.ЦеновойДиапазон + " не округлять");				
				иначе					
					МассивПравил.Добавить("" + Правило.ЦеновойДиапазон + " с точностью " + Формат(Правило.ТочностьОкругления, ФорматнаяСтрока));
				КонецЕсли;
			КонецЦикла;
