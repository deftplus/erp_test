...
Процедура ЗаполнениеКомандногоМенюВЗакладках()
	...
	Для каждого Элемент Из СопоставлениеЗакладокСоСписками Цикл
		...
		ЭлементФормы.СписокВыбора.Добавить("Все","Все");
		ЭлементФормы.СписокВыбора.Добавить("","Где действует");
		...
		ЭлементПодменюУстановить.Заголовок = "Изменить";
...
Процедура УстановитьнастройкиПоФО()
	...
	СписокВыбораНазначение.Добавить(ПредопределенноеЗначение("Перечисление.НазначенияВидовЦен.Общий"), "всех");

	Если ИспользоватьНесколькоСкладов Тогда
		СписокВыбораНазначение.Добавить(ПредопределенноеЗначение("Перечисление.НазначенияВидовЦен.ИндивидуальныйДляСклада"), "склада");
	КонецЕсли;
	
	Если ИспользоватьФорматыМагазинов Тогда
		СписокВыбораНазначение.Добавить(ПредопределенноеЗначение("Перечисление.НазначенияВидовЦен.ИндивидуальныйДляФорматаМагазинов"), "формата магазинов");
	КонецЕсли;
	
	Если ИспользоватьПартнеров Тогда
		СписокВыбораНазначение.Добавить(ПредопределенноеЗначение("Перечисление.НазначенияВидовЦен.ИндивидуальныйДляПартнера"), "партнера");
	КонецЕсли;
	
	Если ИспользоватьСоглашенияСКлиентами Тогда
		СписокВыбораНазначение.Добавить(ПредопределенноеЗначение("Перечисление.НазначенияВидовЦен.ИндивидуальныйДляСоглашения"), "соглашения");
	КонецЕсли;
	
	СписокВыбораИспользование = Элементы.ИспользованиеОтбор.СписокВыбора;
	СписокВыбораИспользование.Очистить();	
	СписокВыбораИспользование.Добавить("ИспользоватьПриПродаже", "всех продажах");
	СписокВыбораИспользование.Добавить("ИспользоватьПриОптовойПродаже", "оптовой продаже");
	СписокВыбораИспользование.Добавить("ИспользоватьПриРозничнойПродаже", "розничной продаже");

	Если ИспользоватьПриПередачеМеждуОрганизациями Тогда
		СписокВыбораИспользование.Добавить("ИспользоватьПриПередачеМеждуОрганизациями", "передаче между организациями");
	КонецЕсли;
	
	//++ НЕ УТКА
	Если ИспользоватьПриПередачеПродукцииДавальцу Тогда
		СписокВыбораИспользование.Добавить("ИспользоватьПриПередачеПродукцииДавальцу", "передаче продукции давальцу");
	КонецЕсли;
	//-- НЕ УТКА
	
	Если ИспользоватьПроизводство Тогда
		СписокВыбораИспользование.Добавить("ИспользоватьПриВыпускеПродукции", "выпуске продукции");
	КонецЕсли;
...
Процедура ПрочитатьФО()
	...
	//++ НЕ УТКА
	ИспользоватьПриПередачеПродукцииДавальцу	= ПолучитьФункциональнуюОпцию("ИспользоватьПроизводствоИзДавальческогоСырья");
	//-- НЕ УТКА
...
//
&НаСервере
Процедура СформироватьИнформациюОКоличествеИспользуемыхВидовЦен(ДанныеПоВидуЦен) Экспорт
