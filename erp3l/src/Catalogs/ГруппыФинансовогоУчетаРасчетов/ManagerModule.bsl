...
Процедура ПреобразоватьОтборПараметровВыбора(Отбор) Экспорт
	...
	Если Отбор.Свойство("РасчетыВВалюте") И ТипЗнч(Отбор.РасчетыВВалюте) <> Тип("Булево") Тогда
		Если Отбор.Свойство("Организация") И ЗначениеЗаполнено(Отбор.Организация)Тогда
			...
			Отбор.РасчетыВВалюте = ВзаиморасчетыСервер.ВзаиморасчетыВВалюте(Отбор.РасчетыВВалюте, ВалютаРегламентированногоУчета);
		Иначе
	...
	Если Отбор.Свойство("Договор") И ЗначениеЗаполнено(Отбор.Договор) Тогда
		...
		Если ТипЗнч(УсловияРасчетов) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда
			Отбор.Вставить("ХозяйственнаяОперация", УсловияРасчетов.ХозяйственнаяОперация);
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(УсловияРасчетов) Тогда
		...
		Организация = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(УсловияРасчетов, "Организация");
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполнение признака Расчеты в иностранной валюте.';
									|en = 'Fill in the ""Settlements in foreign currency"" attribute'");
