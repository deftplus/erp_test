...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	|en = 'Fills in the ""VAT rate"" attribute. Fills in the ""Payment in foreign currency"" flag.
	|Fills in the Department in subcontracting contracts attribute.'");
	...
	Пока ЭлементСправочника.Следующий() Цикл
		...
		Попытка
			...
			Если ЭлементСправочника.ТребуетсяРазрешениеНаДочернихПартнеров Тогда
				...
				УстановитьРеквизитРазрешенаРаботаСДочернимиПартнерами(СправочникОбъект.Ссылка);	
				#КонецОбласти
...
#Область УХ_Встраивание
Процедура УстановитьРеквизитРазрешенаРаботаСДочернимиПартнерами(ДоговорСсылка)
	...
	|	И Не ВерсияСоглашенияКоммерческийДоговор.РазрешенаРаботаСДочернимиПартнерами";
	Запрос.УстановитьПараметр("ДоговорКонтрагента", ДоговорСсылка);
	Выборка = Запрос.Выполнить().Выбрать();
	
	Пока Выборка.Следующий() Цикл
		ДокОбъект = Выборка.Ссылка.ПолучитьОбъект();
		ДокОбъект.РазрешенаРаботаСДочернимиПартнерами = Истина;
