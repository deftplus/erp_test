...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет возможные пустые ключи связи в таблице трудозатрат, конвертирует формулы расчета количества и определения значения свойства в новый формат.';
									|en = 'Populates possible blank link keys in the labor costs table, converts the formula for calculating the quantity and determining the value of the property to a new format.'");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|ИЗ
	|	Справочник.МаршрутныеКарты.Трудозатраты КАК Таблица
	|ГДЕ
	...
	|	Справочник.МаршрутныеКарты КАК МаршрутныеКарты
	|ГДЕ
	|	НЕ МаршрутныеКарты.ЭтоГруппа
	|	И НЕ МаршрутныеКарты.УдалитьПереходНаНовыеФормулыВыполнен
	|");
