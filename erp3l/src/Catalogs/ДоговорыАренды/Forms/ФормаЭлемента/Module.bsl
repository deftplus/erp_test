...
Процедура ТипДоговораПриИзменении(Элемент)
	...
	ИначеЕсли Объект.ТипДоговора = ПредопределенноеЗначение("Перечисление.ТипыДоговоровАренды.Аренда") Тогда
		...
		ИзмененныеРеквизиты = ИзмененныеРеквизиты + ",Балансодержатель";
	КонецЕсли;
...
Процедура НастроитьЗависимыеЭлементыФормыНаКлиентеНаСервере(Знач Форма, Знач ИзмененныеРеквизиты = "")
	...
	УказаныУсловияДоговора = НЕ Форма.ЗаключениеДоговораАренды.Пустая();
...
Процедура ЗаполнитьУсловияДоговора()
	...
	Если НЕ ЗначениеЗаполнено(ЗаключениеДоговораАренды) Тогда
		ЗаголовокНадписи = ВнеоборотныеАктивыКлиентСервер.ФорматированнаяСтрока(НСтр("ru = 'Заключение договора: <a href = ""#Создать"">оформить</a>';
																					|en = 'Signing the contract: <a href = ""#Создать"">register</a>'"));
...
Процедура ПрочитатьУсловияДоговора()
	...
	Иначе	
		ЗаголовокНадписи = ВнеоборотныеАктивыКлиентСервер.ФорматированнаяСтрока(
			НСтр("ru = 'Заключение договора: <a href = ""#Создать"">оформить</a>';
				|en = 'Signing the contract: <a href = ""#Создать"">register</a>'"));
	КонецЕсли;			
...
Процедура СостояниеДоговораПриИзменении(Элемент)
	...
	ДоговорыКонтрагентовФормыУХВызовСервера.ЗаписатьСостояниеДоговора(Объект.Ссылка, ЭтотОбъект.СостояниеДоговора);
