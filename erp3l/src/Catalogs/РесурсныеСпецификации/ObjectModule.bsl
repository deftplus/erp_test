...
Процедура УстановкаРеквизитовИсторииПереходаНаНовыеФормулы()
	...
	Для Каждого СтрокаТЧ Из ВыходныеИзделия Цикл
		Если ЗначениеЗаполнено(СтрокаТЧ.АлгоритмРасчетаКоличества)
			...
			СтрокаТЧ.АлгоритмРасчетаКоличестваИсторияПереходаНаНовыеФормулы = СтрокаТЧ.АлгоритмРасчетаКоличества;
		КонецЕсли;
	КонецЦикла;
	Для Каждого СтрокаТЧ Из ВозвратныеОтходы Цикл
		...
		КонецЕсли;
	КонецЦикла;
	Для Каждого СтрокаТЧ Из МатериалыИУслуги Цикл
		Если ЗначениеЗаполнено(СтрокаТЧ.АлгоритмРасчетаКоличества)
			И НЕ ЗначениеЗаполнено(СтрокаТЧ.АлгоритмРасчетаКоличестваИсторияПереходаНаНовыеФормулы) Тогда
			СтрокаТЧ.АлгоритмРасчетаКоличестваИсторияПереходаНаНовыеФормулы = СтрокаТЧ.АлгоритмРасчетаКоличества;
		КонецЕсли;
	КонецЦикла;
	Для Каждого СтрокаТЧ Из Трудозатраты Цикл
		Если ЗначениеЗаполнено(СтрокаТЧ.АлгоритмРасчетаКоличества)
			И НЕ ЗначениеЗаполнено(СтрокаТЧ.АлгоритмРасчетаКоличестваИсторияПереходаНаНовыеФормулы) Тогда
			СтрокаТЧ.АлгоритмРасчетаКоличестваИсторияПереходаНаНовыеФормулы = СтрокаТЧ.АлгоритмРасчетаКоличества;
		КонецЕсли;
	КонецЦикла;
	Для Каждого СтрокаТЧ Из СоответствиеСвойств Цикл
		Если ЗначениеЗаполнено(СтрокаТЧ.АлгоритмРасчетаЗначения)
			...
			СтрокаТЧ.АлгоритмРасчетаЗначенияИсторияПереходаНаНовыеФормулы = СтрокаТЧ.АлгоритмРасчетаЗначения;
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
	...
	Если ПроизводствоСервер.ИспользуетсяПроизводство22()
		...
		Если ТипПроизводственногоПроцесса = Перечисления.ТипыПроизводственныхПроцессов.Разборка
			...
			Шаблон = НСтр("ru = 'Не введено ни одной строки в список ""%1"" по этапу ""%2""';
							|en = 'No lines entered into the ""%1"" list by the ""%2"" step'");
...
#Область ОтборПоСвойствам
	...
	Если ДополнительныеСвойства.Свойство("ПроверитьЭтапы") Тогда
...
Процедура ПроверитьПроизводственныйПроцесс(Отказ)
	...
	Если НЕ ДополнительныеСвойства.Свойство("ПроверитьЭтапы") Тогда
		Возврат;
	КонецЕсли;
	...
	Если Отказ Тогда
		...
		Сообщение.Текст = НСтр("ru = 'Производственный процесс содержит ошибки. Воспользуйтесь проверкой и внесите необходимые исправления в форме спецификации.';
...
//++ НЕ УТКА
	...
	Если ПолучитьФункциональнуюОпцию("ХранитьОперацииВРесурсныхСпецификациях")
			ИЛИ НЕ ДополнительныеСвойства.Свойство("ПроверитьЭтапы") Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПроверитьРеквизитыПорядокОпераций(Отказ)
	...
	Пока ВыборкаЭтапы.Следующий() Цикл
		...
		Пока ВыборкаОперации.Следующий() Цикл
			...
			Если ОперацияОбъект.СодержитТехнологическийПроцесс Тогда
				...
				Если ВыборкаЭтапы.ЭтапПодразделение <> ВыборкаОперации.ТехпроцессПодразделение Тогда
					...
					ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, ВыборкаОперации.ТехнологическийПроцесс,,, Отказ);
		...
		Если ТаблицаОпераций.Количество() > 0
			И НЕ УправлениеДаннымиОбИзделиях.ПоследовательностьСпискаОперацийПравильная(ТаблицаОпераций, СтруктураПроверок, ВыборкаЭтапы.Этап) Тогда
			Отказ = Истина;
		КонецЕсли;
...
Процедура ПроверитьСкорректироватьЭтапыПроизводства(Отказ)
	...
	Индекс = 1;
	МногоэтапныйПроизводственныйПроцесс = ?(Выборка.Количество() > 1, Истина, Ложь);
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			...
			ТекстШаблон = НСтр("ru = 'Не удалось скорректировать этапы ресурсной спецификации %1';
								|en = 'Cannot correct the bill of materials steps %1'");
			...
			ТекстСообщения = СтрШаблон(ТекстШаблон, НСтр("ru = 'по причине:';
														|en = 'Reason:'") + " " + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ЗарегистрироватьРасчетДлительностиПоСпецификации() Экспорт
	...
	|				ИЛИ &ЕстьПолуфабрикатыВПроцессе
	...
	|														ИЛИ РесурсныеСпецификации.ЕстьПолуфабрикатыВПроцессе
	...
	Запрос.УстановитьПараметр("ЕстьПолуфабрикатыВПроцессе",  ЕстьПолуфабрикатыВПроцессе);
...
//++ НЕ УТКА
		...
		ЕстьПолуфабрикатыВПроцессе         = Ложь;
...
//++ НЕ УТКА
		...
		// используется производство полуфабрикатов в процессе
		#Область ЕстьПроизводствоВПроцессе
		
		ЕстьПолуфабрикатыВПроцессе = Ложь;
		Если МатериалыИУслуги.Найти(Перечисления.СпособыПолученияМатериаловВСпецификации.ПроизвестиПоСпецификации, "СпособПолученияМатериала") <> Неопределено Тогда
			ЕстьПолуфабрикатыВПроцессе = Истина;
		КонецЕсли;
