...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ТипЗнч(ИсточникВыбора) = Тип("ФормаКлиентскогоПриложения")
		...
		Если Режим = "ЭтапВыпуска" Тогда
			ЭтапВыпуска = ВыбранноеЗначение.Ссылка;
			ЭтапВыпускаПредставление = ВыбранноеЗначение.Наименование;
			ДобавитьПромежуточныйВыпускПриВыборе = ВыбранноеЗначение.ДобавитьПромежуточныйВыпускПриВыборе
		Иначе
			ПланироватьНеРанее = ВыбранноеЗначение.Ссылка;
			ПланироватьНеРанееПредставление = ВыбранноеЗначение.Наименование;
		КонецЕсли;
...
Процедура ЭтапВыпускаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОткрытьФорму("Справочник.РесурсныеСпецификации.Форма.ФормаВыбораЭлементаПроизводственногоПроцесса", ПараметрыФормы, ЭтаФорма);
...
Процедура НастроитьЭлементыФормыПоПараметрам()
	...
	//-- Устарело_Производство21
		...
		Элементы.Спецификация.ПодсказкаВвода = НСтр("ru = '<спецификация подбирается автоматически>';
													|en = '<specification is automatically selected>'");
	//++ Устарело_Производство21
	КонецЕсли;
	//-- Устарело_Производство21
