...
Процедура ЗаполнитьСведенияОбУчете()
	...
	Если НЕ ВедетсяРегламентированныйУчетВНА Тогда
		...
		Элементы.СведенияСнятСУчета3.Видимость = Ложь;
	...
	Если ВалютаУпр <> ВалютаРегл Тогда
		Если ВедетсяРегламентированныйУчетВНА Тогда
			ЗначениеУчетРегл = СтрШаблон(НСтр("ru = 'БУ (%1):';
												|en = 'AC (%1):'"), Строка(ВалютаРегл));
			ЗначениеУчетУпр = СтрШаблон(НСтр("ru = 'УУ (%1):';
											|en = 'MA (%1):'"), Строка(ВалютаУпр));
		Иначе
			ЗначениеУчетРегл = СтрШаблон(НСтр("ru = 'Регл. (%1):';
												|en = 'Reg. (%1):'"), Строка(ВалютаРегл));
			ЗначениеУчетУпр = СтрШаблон(НСтр("ru = 'Упр. (%1):';
											|en = 'Man. (%1):'"), Строка(ВалютаУпр));
		КонецЕсли; 
	КонецЕсли; 
	...
	Если Объект.ТипОС = ПредопределенноеЗначение("Перечисление.ТипыОС.ОбъектЭксплуатации")
		...
		Для Каждого СтрокаСтоимостиУзлаКомпонента Из СтоимостьИАмортизацияУзловКомпонентов Цикл
			...
			СтрокаУзлаКомпонента.УчетРегл = ЗначениеУчетРегл;
			СтрокаУзлаКомпонента.УчетУпр = ЗначениеУчетУпр;
		КонецЦикла;
...
Процедура НастроитьЗависимыеЭлементыФормы(Форма, Знач ИзмененныеРеквизиты = "")
	...
	Если СтруктураИзмененныхРеквизитов.Свойство("ТипОС")
		...
		Элементы.ГруппаСведенияУзлыКомпонентыАмортизации.Видимость = ЗначениеСвойства;
...
Процедура ПриИзмененииНазначенияВНА()
	...
	Справочники.НазначенияВНА.УстановитьСвойстваЭлементовПоНазначениюВНАНаСервере(
		Элементы, ТекущийВид, Перечисления.КлассыВНА.ОсновноеСредство);
КонецПроцедуры
