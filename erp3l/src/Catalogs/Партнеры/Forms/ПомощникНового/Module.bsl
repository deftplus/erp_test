...
Процедура ЗаполнитьРеквизитыБанкаПоБанку(ТипБанка, Банк = "", ЭтоГоловнойКонтрагент=Ложь)
	...
	Если ТипБанка = "Банк" Тогда
		...
		ИменаРеквизитов = СтрРазделить(РеквизитыБанка(ЭтоГоловнойКонтрагент), ",", Ложь);
		Для каждого ИмяРеквизита Из ИменаРеквизитов Цикл
			ИмяРеквизитаВСправочнике = СтрЗаменить(СтрЗаменить(ИмяРеквизита, "Банка", ""), "ГоловнойКонтрагент", ""); 
			СтруктураРеквизитов.Вставить(ИмяРеквизита, ИмяРеквизитаВСправочнике);
		КонецЦикла;
		СтруктураРеквизитов.Вставить("БИКБанка" + Постфикс, "Код");
		СтруктураРеквизитов.Вставить("СВИФТБанка" + Постфикс, "СВИФТБИК");
		СтруктураРеквизитов.Вставить("НаименованиеБанкаМеждународное" + Постфикс, "МеждународноеНаименование");
		
		РеквизитыБанка = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Банк, СтруктураРеквизитов);
		
		Исключения = "";
		Если Не ЗначениеЗаполнено(РеквизитыБанка["НаименованиеБанкаМеждународное" + Постфикс])
			Или ЗначениеЗаполнено(ЭтотОбъект["НаименованиеБанкаМеждународное" + Постфикс]) Тогда
			Исключения = "НаименованиеБанкаМеждународное" + Постфикс;
		КонецЕсли;
		
		ЗаполнитьЗначенияСвойств(ЭтотОбъект, РеквизитыБанка, , Исключения);
		
		ЭтотОбъект["СчетВБанкеДляРасчетов" + Постфикс] = РеквизитыБанка["КоррСчетБанка" + Постфикс];
		
	ИначеЕсли ТипБанка = "БанкДляРасчетов" Тогда
		
		СтруктураРеквизитов = Новый Структура;
		ИменаРеквизитов = СтрРазделить(РеквизитыБанкаДляРасчетов(ЭтоГоловнойКонтрагент), ",", Ложь);
		Для каждого ИмяРеквизита Из ИменаРеквизитов Цикл
			ИмяРеквизитаВСправочнике = СтрЗаменить(ИмяРеквизита, "БанкаДляРасчетов", "");
			ИмяРеквизитаВСправочнике = СтрЗаменить(ИмяРеквизитаВСправочнике, "ГоловнойКонтрагент", "");
			СтруктураРеквизитов.Вставить(ИмяРеквизита, ИмяРеквизитаВСправочнике);
		КонецЦикла;
		СтруктураРеквизитов.Вставить("БИКБанкаДляРасчетов" + Постфикс, "Код");
		СтруктураРеквизитов.Вставить("СВИФТБанкаДляРасчетов" + Постфикс, "СВИФТБИК");
		СтруктураРеквизитов.Вставить(
			"НаименованиеБанкаДляРасчетовМеждународное" + Постфикс, "МеждународноеНаименование");
		
		РеквизитыБанка = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Банк, СтруктураРеквизитов);
		
		Исключения = "";
		Если Не ЗначениеЗаполнено(РеквизитыБанка["НаименованиеБанкаДляРасчетовМеждународное" + Постфикс])
			Или ЗначениеЗаполнено(ЭтотОбъект["НаименованиеБанкаДляРасчетовМеждународное" + Постфикс]) Тогда
			Исключения = "НаименованиеБанкаДляРасчетовМеждународное" + Постфикс;
		КонецЕсли;
...
Функция РеквизитыБанка(ЭтоГоловнойКонтрагент)
	...
	Если ЭтоГоловнойКонтрагент Тогда
		Возврат "БИКБанкаГоловнойКонтрагент, СВИФТБанкаГоловнойКонтрагент, НаименованиеБанкаГоловнойКонтрагент,
				| НаименованиеБанкаМеждународноеГоловнойКонтрагент, КоррСчетБанкаГоловнойКонтрагент,
				| СтранаБанкаГоловнойКонтрагент, ГородБанкаГоловнойКонтрагент,
				| ГородБанкаМеждународныйГоловнойКонтрагент, АдресБанкаГоловнойКонтрагент,
				| АдресБанкаМеждународныйГоловнойКонтрагент";
	Иначе
		Возврат "БИКБанка, СВИФТБанка, НаименованиеБанка, НаименованиеБанкаМеждународное, КоррСчетБанка, СтранаБанка,
				| ГородБанка, ГородБанкаМеждународный, АдресБанка, АдресБанкаМеждународный";
	КонецЕсли;
...
Функция РеквизитыБанкаДляРасчетов(ЭтоГоловнойКонтрагент)
	...
	Если ЭтоГоловнойКонтрагент Тогда
		Возврат "БИКБанкаДляРасчетовГоловнойКонтрагент, СВИФТБанкаДляРасчетовГоловнойКонтрагент,
				| НаименованиеБанкаДляРасчетовГоловнойКонтрагент, НаименованиеБанкаДляРасчетовМеждународноеГоловнойКонтрагент,
				| КоррСчетБанкаДляРасчетовГоловнойКонтрагент, СтранаБанкаДляРасчетовГоловнойКонтрагент,
				| ГородБанкаДляРасчетовГоловнойКонтрагент, ГородБанкаДляРасчетовМеждународныйГоловнойКонтрагент,
				| АдресБанкаДляРасчетовГоловнойКонтрагент, АдресБанкаДляРасчетовМеждународныйГоловнойКонтрагент";
	Иначе
	...
	Если УказатьБанковскийСчетКонтрагента Тогда
		...
		Если Не ЗначениеЗаполнено(ВалютаДенежныхСредств) Тогда
			ПартнерыИКонтрагентыКлиент.ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать валюту денежных средств';
																		|en = 'You must specify currency'"), ИспользуютсяТолькоПартнеры, "ВалютаДенежныхСредств");
			Отказ = Истина;
		КонецЕсли;
		...
		Если ИспользуетсяБанкДляРасчетов Тогда
			...
			Если ПустаяСтрока(БИКБанкаДляРасчетов)
				...
				ПартнерыИКонтрагентыКлиент.ВывестиСообщениеПользователю(НСтр("ru = 'Необходимо указать БИК банка для расчетов';
																			|en = 'Specify BIC of the settlements bank'"), ИспользуютсяТолькоПартнеры, "БИКБанкаДляРасчетов");
				Отказ = Истина;
			КонецЕсли;
...
Процедура ПриИзмененииНомераСчетаЛокализация(ЭтоГоловнойКонтрагент=Ложь)
	...
	Если ДенежныеСредстваКлиентЛокализация.СтрокаСоответствуетФорматуБанковскогоСчетаРФ(ЭтотОбъект["НомерСчета" + Постфикс]) Тогда
...
Процедура ОпределитьБанкРФ(ЭтоГоловнойКонтрагент)
	...
	Если НЕ ЭтотОбъект["ЭтоIBAN" + Постфикс]
		И ДенежныеСредстваКлиентЛокализация.СтрокаСоответствуетФорматуБанковскогоСчетаРФ(ЭтотОбъект["НомерСчета" + Постфикс]) Тогда
		ЭтотОбъект["ИностранныйБанк" + Постфикс] = Ложь;
	КонецЕсли;
