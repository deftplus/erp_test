	...
	Пока Выборка.СледующийПоЗначениюПоля("Сотрудник") Цикл 
		...
		Пока Выборка.Следующий() Цикл
			...
			Если Выборка.ОснованиеИсчисляемогоСтажа = Справочники.ОснованияИсчисляемогоСтраховогоСтажа.ВИРУС
				И Выборка.ДатаНачалаПериода >= ПерсонифицированныйУчет.ДатаПрекращенияДействияКодаВИРУС() Тогда
			...
			Если ПредыдущаяЗапись <> Неопределено 
				...
				И ПерсонифицированныйУчет.ДанныеСтрокСовпадают(ПредыдущаяЗапись, ДанныеЗаписи, РеквизитыСтроки) Тогда 
...
Процедура ДобавитьСтрокуЗаписейОСтаже(ДанныеОСтаже, Выборка, ДатаПостановления322)
	...
	ПрекращениеДействияКодаВИРУС = ПерсонифицированныйУчет.ДатаПрекращенияДействияКодаВИРУС();
	Если Выборка.ОснованиеИсчисляемогоСтажа = Справочники.ОснованияИсчисляемогоСтраховогоСтажа.ВИРУС
		...
		Иначе
			ПерсонифицированныйУчет.ОбработатьТерриториальныеУсловияДополнительные(Выборка, ДанныеОСтаже);
