...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	|	МАКСИМУМ(ДебиторскаяЗадолженность.ОбъектРасчетов.УникальныйИдентификатор) КАК ИдентификаторСтроки,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ДебиторскаяЗадолженность.ОбъектРасчетов.УникальныйИдентификатор) КАК КоличествоСтрок
	|ПОМЕСТИТЬ втИдентификаторыДебиторскойЗадолженности
	|ИЗ
	...
	|	МАКСИМУМ(КредиторскаяЗадолженность.ОбъектРасчетов.УникальныйИдентификатор) КАК ИдентификаторСтроки,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ КредиторскаяЗадолженность.ОбъектРасчетов.УникальныйИдентификатор) КАК КоличествоСтрок
	|ПОМЕСТИТЬ втИдентификаторыКредиторскойЗадолженности
	|ИЗ
	...
	|	ВЫБОР
	|		КОГДА ЕСТЬNULL(ИдентификаторыДебиторскойЗадолженности.КоличествоСтрок, 0) > 1
	|		  ИЛИ ЕСТЬNULL(ИдентификаторыКредиторскойЗадолженности.КоличествоСтрок, 0) > 1
	|			ТОГДА НЕОПРЕДЕЛЕНО
	|		КОГДА НЕ ИдентификаторыДебиторскойЗадолженности.ИдентификаторСтроки ЕСТЬ NULL
	|			ТОГДА ИдентификаторыДебиторскойЗадолженности.ИдентификаторСтроки
	|		КОГДА НЕ ИдентификаторыКредиторскойЗадолженности.ИдентификаторСтроки ЕСТЬ NULL
	|			ТОГДА ИдентификаторыКредиторскойЗадолженности.ИдентификаторСтроки
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК ИдентификаторФинЗаписи
	|ИЗ
	|	Документ.ВзаимозачетЗадолженности  КАК ДанныеДокумента
	|	ЛЕВОЕ СОЕДИНЕНИЕ втИдентификаторыДебиторскойЗадолженности КАК ИдентификаторыДебиторскойЗадолженности
	|		ПО ИСТИНА
	|	ЛЕВОЕ СОЕДИНЕНИЕ втИдентификаторыКредиторскойЗадолженности КАК ИдентификаторыКредиторскойЗадолженности
	|		ПО ИСТИНА
	|ГДЕ
	...
	Запрос.УстановитьПараметр("ИдентификаторФинЗаписи",           Реквизиты.ИдентификаторФинЗаписи);
...
Функция ТекстЗапросаТаблицаДвиженияПоПрочимАктивамПассивам(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
		"ОбъектыКорРасчетов.УникальныйИдентификатор",
		"ВЫБОР
		|	КОГДА &ИдентификаторФинЗаписи = НЕОПРЕДЕЛЕНО
		|		ТОГДА ОбъектыКорРасчетов.УникальныйИдентификатор
		|	ИНАЧЕ &ИдентификаторФинЗаписи
		|КОНЕЦ");
	
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
		"ОбъектыРасчетов.УникальныйИдентификатор",
		"ВЫБОР
		|	КОГДА &ИдентификаторФинЗаписи = НЕОПРЕДЕЛЕНО
		|		ТОГДА ОбъектыРасчетов.УникальныйИдентификатор
		|	ИНАЧЕ &ИдентификаторФинЗаписи
		|КОНЕЦ");
...
Функция ТекстЗапросаРасчетыСКлиентами()
	...
	|	ВЫБОР
	|		КОГДА &ИдентификаторФинЗаписи = НЕОПРЕДЕЛЕНО
	|			ТОГДА ТабличнаяЧасть.ОбъектРасчетов.УникальныйИдентификатор
	|		ИНАЧЕ &ИдентификаторФинЗаписи
	|	КОНЕЦ                                                                 КАК ИдентификаторФинЗаписи
	|	
	|ИЗ
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет реквизиты ""Объекты расчетов"".';
									|en = 'Fills in ""AP/AR objects"" attributes.'");
	...
	Пока ВыборкаДокумент.Следующий() Цикл
		...
		Попытка
									...
									Если ПроблемныеОбъектыРасчетов.Найти(СтрокаЗадолженность.УдалитьЗаказ) = Неопределено Тогда
											...
											ВызватьИсключение (СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
												НСтр("ru = 'Не удалось заполнить объект расчетов в документе: %1, строка № %2, таличная часть %3';
													|en = 'Cannot fill in the settlement object in document: %1, line %2, tabular section %3'"),
												Ссылка,
									...
									Если ОбъектыРасчетовСервер.ОбъектГотовКЗаполнению(
										...
										ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
											НСтр("ru = 'Не удалось заполнить поле Объект расчетов интеркампани в документе: %1';
												|en = 'Cannot fill in the ""Intercompany AR/AP object"" field in the document: %1'"),
												Ссылка);
							...
							Иначе
								ВызватьИсключение (СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru = 'Не удалось заполнить поле Объект расчетов Дебитор/Кредитор в документе: %1';
										|en = 'Cannot fill in the Settlements object Debtor/Lender field in document: %1'"),
									Ссылка));
							КонецЕсли;
						КонецЕсли;
					КонецЕсли;
			...
			Если ДокументОбъект.Комментарий = КомментарийЗачетОплат
				И ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийВзаимозачетаЗадолженности.Клиента Тогда
			...
			ИначеЕсли ДокументОбъект.Комментарий = КомментарийЗачетОплат
				И ДокументОбъект.ВидОперации = Перечисления.ВидыОперацийВзаимозачетаЗадолженности.Поставщика Тогда
			...
			ИначеЕсли ДокументОбъект.ВидОперации <> Перечисления.ВидыОперацийВзаимозачетаЗадолженности.КлиентаМеждуОрганизациями 
					И ДокументОбъект.ВидОперации <> Перечисления.ВидыОперацийВзаимозачетаЗадолженности.ПоставщикаМеждуОрганизациями	 
					И ДокументОбъект.ВидОперации <> Перечисления.ВидыОперацийВзаимозачетаЗадолженности.Бартер	
