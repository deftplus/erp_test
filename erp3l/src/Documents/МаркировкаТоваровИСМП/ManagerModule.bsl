			...
			Для Каждого СтрокаТЧМаркированныеТовары Из РезультатПоиска.МаркированныеТовары Цикл
				...
				ИначеЕсли НайденныеСтроки.Количество() > 1 Тогда
					...
					Иначе
						ИнтеграцияИСКлиентСервер.ДобавитьТекстОшибки(
							СообщениеJSON,
							СтрШаблон(НСтр("ru = 'В табличной части ""Товары"" дублируется GTIN %1.';
											|en = 'В табличной части ""Товары"" дублируется GTIN %1.'"), ПараметрыОтбора.GTIN));
					КонецЕсли;
				...
				Иначе
					...
					Иначе
						ИнтеграцияИСКлиентСервер.ДобавитьТекстОшибки(
							СообщениеJSON,
							СтрШаблон(НСтр("ru = 'В табличной части ""Товары"" не найден GTIN %1.';
											|en = 'В табличной части ""Товары"" не найден GTIN %1.'"), ПараметрыОтбора.GTIN));
					КонецЕсли;
	...
	ИначеЕсли ИнтеграцияИСКлиентСервер.ВидПродукцииИспользуетОтчетыОНанесенииКодовМаркировки(Шапка.ВидПродукции)
		...
		Если Шапка.ОтчетПроизводственнойЛинии И СоздаватьШтрихкодыУпаковок И ДеревоУпаковок.Строки.Количество() Тогда
			...
			Исключение
				...
				ИмяСобытияЖурналРегистрации = НСтр("ru = 'ГосИС: Запись штрихкодов упаковок при вводе в оборот';
													|en = 'ГосИС: Запись штрихкодов упаковок при вводе в оборот'", ОбщегоНазначения.КодОсновногоЯзыка());
				...
				ИнтеграцияИСКлиентСервер.ДобавитьТекстОшибки(
					...
					СтрШаблон(
						НСтр("ru = 'Не удалось определить адрес производственного объекта для станции %1';
							|en = 'Не удалось определить адрес производственного объекта для станции %1'"),
...
Функция РассчитатьКоличествоВложенныхШтрихкодовСтрокиДерева(СтрокаДерева, СообщениеJSON, Итоги)
	...
	ИначеЕсли  СтрокаДерева.КоличествоПотребительскихУпаковок = 0 И СтрокаДерева.Количество = 0 Тогда
		...
		ИнтеграцияИСКлиентСервер.ДобавитьТекстОшибки(
			СообщениеJSON,
			СтрШаблон(НСтр("ru = 'Не удалось определить плановое количество потребительских упаковок для %1.';
							|en = 'Не удалось определить плановое количество потребительских упаковок для %1.'"), СтрокаДерева.НормализованныйШтрихкод));
		ЕстьОшибки = Истина;
