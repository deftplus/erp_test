...
Функция ОписаниеОперацийУчетаСебестоимости(Документ) Экспорт
	...
	|			ТОГДА 0
	|		КОГДА ЕСТЬNULL(ТаблицаАналитикУчетаПартий.НалогообложениеНДС, ТаблицаВидыЗапасов.ЗакупкаПодДеятельность) В (
	|		  ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС),
	|		  ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаОблагаетсяЕНВД))
	|			ТОГДА ЕСТЬNULL(Суммы.СуммаСНДСРегл, &КоэффициентПересчетаВВалютуРегл * (ТаблицаВидыЗапасов.СуммаПошлины + ТаблицаВидыЗапасов.СуммаНДС))
	|			ИНАЧЕ ЕСТЬNULL(Суммы.СуммаБезНДСРегл, &КоэффициентПересчетаВВалютуРегл * ТаблицаВидыЗапасов.СуммаПошлины)
	...
	Если Данные.Товары<>Неопределено Тогда
		...
		Иначе
			...
			СтруктураДействий.Вставить("ЗаполнитьСтавкуНДС", Новый Структура("НалогообложениеНДС, Дата",
				НастройкиУчетаНДС.ОсновнойВидДеятельностиНДСЗакупки, Объект.Дата));
...
#Область ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	|Fills in Foreign currency transaction flag.
	|Fills in ""AR/AP object"" attribute.'");
...
#Область ОбработатьДанныеДляПереходаНаНовуюВерсиюЗаполнитьИдентификаторыСтрок
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет служебные реквизиты документа ""Таможенная декларация на импорт"".';
									|en = 'Fills in service details of the document ""Import customs declaration"".'");
