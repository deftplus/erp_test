...
//                Если не задан, то для всех.
//	Период - Дата - любая дата месяца за который необходимо провести переоценку.
//  АналитикаРасчета - ПеречислениеСсылка.ТипыСуммГрафикаКредитовИДепозитов, Массив, Неопределено - Типы сумм по которым необходимо провести переоценку.//	
//
...
//                Если не задан, то для всех.
//	Период - Дата - любая дата месяца за который необходимо провести переоценку.
//  АналитикаРасчета - ПеречислениеСсылка.ТипыПлатежейПоАренде, Массив, Неопределено - Типы сумм по которым необходимо провести переоценку.	
//
...
//                Если не задан, то для всех.
//	Период - Дата - любая дата месяца за который необходимо провести переоценку.
//	АналитикаРасчета - СправочникСсылка.КлючиАналитикиУчетаПоПартнерам, Массив, Неопределено - Касса (список касс) по которой необходимо выполнить переоценку.
//	                   Если не задана, то для всех.
//
	...
	Если ТребуетсяПереоценка Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеВыполненСОшибками(
			...
			НСтр("ru = 'Обнаружены курсовые разницы по договорам аренды.';
				|en = 'Exchange differences under lease contracts are found.'", ОбщегоНазначения.КодОсновногоЯзыка()),
			ЗакрытиеМесяцаСервер.ТекстПодробнееПоУмолчанию(),
			ДействиеПодробнее);
	...
	ИначеЕсли НЕ ЕстьВалютныеОстатки Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			...
			НСтр("ru = 'Нет валютных остатков по договорам аренды.';
				|en = 'No currency balance under lease contracts.'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура Выполнить_ПереоценкаАренды(ПараметрыОбработчика) Экспорт
	...
	Пока НачалоРасчета <= ПараметрыРасчета.КонецПериода ИЛИ ТребуетсяПереоценка Цикл
		...
		Попытка
			...
			ПереоценитьАренду(ОрганизацииКПереоценке, НачалоРасчета);
		Исключение
			
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Переоценка остатков по договорам аренды за период %1 завершилась с ошибкой:
					|%2';
					|en = 'An error occurred while revaluating balance under the lease contract for period %1:
					|%2'", ОбщегоНазначения.КодОсновногоЯзыка()),
				РасчетСебестоимостиПротоколРасчета.ПредставлениеПериодаРасчета(ПараметрыРасчета.ПериодРегистрации),
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	Запрос.УстановитьПараметр("ЕстьДоговораАренды",                    Истина);
	Запрос.УстановитьПараметр("ТипДоговораАренды",                     Тип("СправочникСсылка.ДоговорыАренды"));
...
Функция ТекстЗапросаКурсовыеРазницыРасчетыПоФинансовымИнструментам()
	...
	//++ НЕ УТ
	|				И ТИПЗНАЧЕНИЯ(ОстаткиНаДень.Договор) = &ТипДоговораАренды
...
Функция ТекстЗапросаКурсовыеРазницыРезервыПоСомнительнымДолгам()
	...
	|	ОстаткиНаДень.СуммаВВалюте КАК ОстатокВалюты,
	|	0 КАК СуммаРегл,
	|	(ВЫРАЗИТЬ(ОстаткиНаДень.СуммаВВалюте * КурсВалюты.КурсЧислитель / КурсВалюты.КурсЗнаменатель * КурсВалютыУпр.КурсЗнаменатель / КурсВалютыУпр.КурсЧислитель КАК ЧИСЛО(31,2))) - ОстаткиНаДень.СуммаУпр КАК СуммаУпр,
	|	ОстаткиНаДень.ТипСуммы КАК ТипСуммы,
	...
	|		ИЛИ &КонецПериода = КОНЕЦПЕРИОДА(&КонецПериода, МЕСЯЦ)
	|		ИЛИ &ПереоценкаПоДням)
	|	И ВЫРАЗИТЬ(ОстаткиНаДень.СуммаВВалюте * КурсВалюты.КурсЧислитель / КурсВалюты.КурсЗнаменатель * КурсВалютыУпр.КурсЗнаменатель / КурсВалютыУпр.КурсЧислитель КАК ЧИСЛО(31,2)) <> ОстаткиНаДень.СуммаУпр
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
...
Функция ТребуетсяПереоценка(Организация, Период, РегистрыДляРасчета = Неопределено, АналитикаРасчета = Неопределено, ВернутьОрганизацииКПереоценке = Ложь, ДляОтладки = Неопределено) Экспорт
	...
	Если РегистрыДляРасчета.Свойство("РасчетыПоФинансовымИнструментам") Тогда
		...
		|		КОГДА &ЕстьДоговораАренды
		//++ НЕ УТ 
		|				И ТИПЗНАЧЕНИЯ(Остатки.Договор) = &ТипДоговораАренды
