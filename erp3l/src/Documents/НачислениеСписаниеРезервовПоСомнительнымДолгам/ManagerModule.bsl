...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура = "Документы.НачислениеСписаниеРезервовПоСомнительнымДолгам.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	Обработчик.Версия = "2.5.5.32";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("98fccc55-9dcc-42e0-a638-209f129c39a0");
	Обработчик.Многопоточный = Истина;
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "Документы.НачислениеСписаниеРезервовПоСомнительнымДолгам.ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.Комментарий = НСтр("ru = 'Заполняет реквизит %1 с типом %2.';
									|en = 'Fills in %1 attribute with %2 type.'");
	Обработчик.Комментарий = СтрШаблон(Обработчик.Комментарий, "ОбъектыРасчетов", "Справочник.ОбъектыРасчетов");
	...
	Изменяемые.Добавить(Метаданные.Документы.НачислениеСписаниеРезервовПоСомнительнымДолгам.ПолноеИмя());
	Обработчик.ИзменяемыеОбъекты = СтрСоединить(Изменяемые, ",");
	
	Блокируемые = Новый Массив;
	Блокируемые.Добавить(Метаданные.Документы.НачислениеСписаниеРезервовПоСомнительнымДолгам.ПолноеИмя());
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ОбновлениеИнформационнойБазы.УстановитьИсточникДанных(ПараметрыВыборки.ДополнительныеИсточникиДанных, "ДебиторскаяЗадолженность.Контрагент");
	ОбновлениеИнформационнойБазы.УстановитьИсточникДанных(ПараметрыВыборки.ДополнительныеИсточникиДанных, "ДебиторскаяЗадолженность.УдалитьОбъектРасчетов");
	ОбновлениеИнформационнойБазы.УстановитьИсточникДанных(ПараметрыВыборки.ДополнительныеИсточникиДанных, "ПогашеннаяЗадолженность.УдалитьОбъектРасчетов");
	...
	|			ИСТИНА
	|		ИЗ
	|			Документ.НачислениеСписаниеРезервовПоСомнительнымДолгам.ДебиторскаяЗадолженность КАК ТЧДебиторскаяЗадолженность
	|		ГДЕ
	|			ТЧДебиторскаяЗадолженность.Ссылка = НачислениеСписаниеРезервовПоСомнительнымДолгам.Ссылка
	|			И ТЧДебиторскаяЗадолженность.ОбъектРасчетов В (ЗНАЧЕНИЕ(Справочник.ОбъектыРасчетов.ПустаяСсылка), НЕОПРЕДЕЛЕНО)
	|			И ТЧДебиторскаяЗадолженность.УдалитьОбъектРасчетов НЕ В (&ПустыеЗначенияОбъектРасчетов)))
	|	ИЛИ (ИСТИНА В
	|		(ВЫБРАТЬ ПЕРВЫЕ 1
	|			ИСТИНА
	|		ИЗ
	|			Документ.НачислениеСписаниеРезервовПоСомнительнымДолгам.ПогашеннаяЗадолженность КАК ТЧПогашеннаяЗадолженность
	|		ГДЕ
	|			ТЧПогашеннаяЗадолженность.Ссылка = НачислениеСписаниеРезервовПоСомнительнымДолгам.Ссылка
	|			И ТЧПогашеннаяЗадолженность.ОбъектРасчетов В (ЗНАЧЕНИЕ(Справочник.ОбъектыРасчетов.ПустаяСсылка), НЕОПРЕДЕЛЕНО)
	|			И ТЧПогашеннаяЗадолженность.УдалитьОбъектРасчетов НЕ В (&ПустыеЗначенияОбъектРасчетов)))";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ТипыРасчетыСПоставщиками = Метаданные.ОпределяемыеТипы.РасчетныйДокументСПоставщиком.Тип.Типы();
	ТипыРасчетыСКлиентами = Метаданные.ОпределяемыеТипы.РасчетныйДокументСКлиентом.Тип.Типы();
	
	ТекстЗапросаКлючиАналитики = "ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ПогашеннаяЗадолженность.АналитикаУчетаПоПартнерам КАК Аналитика,
	|	Аналитика.Контрагент                              КАК Контрагент,
	|	Аналитика.Партнер                                 КАК Партнер,
	|	Аналитика.Организация                             КАК Организация,
	|	ПогашеннаяЗадолженность.Ссылка.ВерсияДанных       КАК ВерсияДанных,
	|	ПогашеннаяЗадолженность.Ссылка                    КАК Документ
	|ИЗ
	|	Документ.НачислениеСписаниеРезервовПоСомнительнымДолгам.ПогашеннаяЗадолженность КАК ПогашеннаяЗадолженность
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КлючиАналитикиУчетаПоПартнерам КАК Аналитика
	|		ПО ПогашеннаяЗадолженность.АналитикаУчетаПоПартнерам = Аналитика.Ссылка
	|ГДЕ
	|	ПогашеннаяЗадолженность.Ссылка В(&ОбрабатываемыеДанные)";
	ЗапросАналитики = Новый Запрос(ТекстЗапросаКлючиАналитики);
	ЗапросАналитики.УстановитьПараметр("ОбрабатываемыеДанные", ОбновляемыеДанные.ВыгрузитьКолонку("Ссылка"));
	КлючиАналитики = ЗапросАналитики.Выполнить().Выгрузить();
	КлючиАналитики.Индексы.Добавить("Документ, ВерсияДанных, Аналитика");
	...
	Для Каждого Документ Из ОбновляемыеДанные Цикл
		...
		Попытка
			...
			Если ДокументОбъект <> Неопределено Тогда
				...
				Для Каждого СтрокаТЧ Из ДокументОбъект.ПогашеннаяЗадолженность Цикл
					Если Не ЗначениеЗаполнено(СтрокаТЧ.ОбъектРасчетов) Тогда
						Если ТипЗнч(СтрокаТЧ.УдалитьОбъектРасчетов) = Тип("СправочникСсылка.ДоговорыКредитовИДепозитов")
								//++ НЕ УТ
								Или ТипЗнч(СтрокаТЧ.УдалитьОбъектРасчетов) = Тип("СправочникСсылка.ДоговорыАренды")
								//-- НЕ УТ
									Тогда
									СтрокаТЧ.ОбъектРасчетов = СтрокаТЧ.УдалитьОбъектРасчетов;
						Иначе
							ТипРасчетов = ТипРасчетовРасчетоногоДокумента(СтрокаТЧ, ТипыРасчетыСПоставщиками, ТипыРасчетыСКлиентами);
							
							КлючПоиска = Новый Структура("Документ, ВерсияДанных, Аналитика", Ссылка, ДокументОбъект.ВерсияДанных, СтрокаТЧ.АналитикаУчетаПоПартнерам);
							
							НайденныеКлючиАналитики = КлючиАналитики.НайтиСтроки(КлючПоиска);
							Если НайденныеКлючиАналитики.Количество() <> 1 Тогда
								ЗафиксироватьТранзакцию();
							Иначе
								ИсточникОбъектРасчетов = СтрокаТЧ["УдалитьОбъектРасчетов"];
								
								ДопКритерийПоиска = ОбъектыРасчетовСервер.ДополнительныеКритерииПоиска();
								ДопКритерийПоиска.Контрагент = НайденныеКлючиАналитики[0].Контрагент;
								ДопКритерийПоиска.Партнер = НайденныеКлючиАналитики[0].Партнер;
								...
								ОбъектыРасчетов = ОбъектыРасчетовСервер.ПолучитьОбъектРасчетовПоСсылке(ИсточникОбъектРасчетов,
									ДокументОбъект.Организация, ТипРасчетов, ДопКритерийПоиска);
									
								Если ЗначениеЗаполнено(ОбъектыРасчетов) Тогда
									СтрокаТЧ.ОбъектРасчетов = ОбъектыРасчетов;
								ИначеЕсли ЗначениеЗаполнено(ИсточникОбъектРасчетов) Тогда
									Если ОбъектыРасчетовСервер.ОбъектГотовКЗаполнению(
										СтрокаТЧ.ОбъектРасчетов,
										ИсточникОбъектРасчетов,
										ГотовКЗаполнению) Тогда
											
										ДопКритерийПоиска = ОбъектыРасчетовСервер.ДополнительныеКритерииПоиска();
										ДопКритерийПоиска.ОбновлениеИБ = Истина;
										ДопКритерийПоиска.ВернутьПервый = Истина;
										ДопКритерийПоиска.НеГенерироватьИсключение = Истина;
										
										ОбъектРасчетовПоСсылке = ОбъектыРасчетовСервер.ПолучитьОбъектРасчетовПоСсылке(ИсточникОбъектРасчетов,
																	,,ДопКритерийПоиска);
										ЕстьОбъектРасчетов = ЗначениеЗаполнено(ОбъектРасчетовПоСсылке);
										
										Если Не ЕстьОбъектРасчетов
											И ТипЗнч(ИсточникОбъектРасчетов) <> Тип("ДокументСсылка.ПервичныйДокумент") Тогда
											ОбъектОбъектаРасчетов = ИсточникОбъектРасчетов.ПолучитьОбъект();
											ПараметрыВзаиморасчеты = ОбщегоНазначения.МенеджерОбъектаПоСсылке(ИсточникОбъектРасчетов).ПараметрыВзаиморасчеты(ОбъектОбъектаРасчетов);
											ВзаиморасчетыСервер.ДополненныеПараметрыМеханизма(ОбъектОбъектаРасчетов, ПараметрыВзаиморасчеты);
											
											Если Не ТипЗнч(ПараметрыВзаиморасчеты) = Тип("Массив") Тогда
												ПараметрыВзаиморасчеты = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(ПараметрыВзаиморасчеты);
											КонецЕсли;
											
											Для Каждого ПараметрВзаиморасчетов Из ПараметрыВзаиморасчеты Цикл
												Если ОбъектыРасчетовСервер.СсылкаЯвляетсяОбъектомРасчетов(ОбъектОбъектаРасчетов, ПараметрВзаиморасчетов) Тогда
													ЕстьОбъектРасчетов = Истина;
												КонецЕсли;
											КонецЦикла; 
										Иначе
											СтрокаТЧ.ОбъектРасчетов = ОбъектРасчетовПоСсылке;
										КонецЕсли;
										
										Если ЕстьОбъектРасчетов И Не ЗначениеЗаполнено(СтрокаТЧ.ОбъектРасчетов) Тогда
											
											ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
												НСтр("ru = 'Не удалось заполнить объект расчетов в документе: %1, строка № %2, таличная часть %3, источник: %4';
													|en = 'Cannot fill in the AR/AP object in document: %1, line %2, table %3, source: %4'"),
													Ссылка,
				...
				Для Каждого СтрокаТЧ Из ДокументОбъект.ДебиторскаяЗадолженность Цикл
					Если Не ЗначениеЗаполнено(СтрокаТЧ.ОбъектРасчетов) Тогда
						Если ТипЗнч(СтрокаТЧ.УдалитьОбъектРасчетов) = Тип("СправочникСсылка.ДоговорыКредитовИДепозитов")
							//++ НЕ УТ
							Или ТипЗнч(СтрокаТЧ.УдалитьОбъектРасчетов) = Тип("СправочникСсылка.ДоговорыАренды")
						...
						Иначе
							...
							ТипРасчетов = ТипРасчетовРасчетоногоДокумента(СтрокаТЧ, ТипыРасчетыСПоставщиками, ТипыРасчетыСКлиентами);
							ДопКритерийПоиска = ОбъектыРасчетовСервер.ДополнительныеКритерииПоиска();
							ДопКритерийПоиска.Контрагент = СтрокаТЧ.Контрагент;
							ДопКритерийПоиска.Партнер = СтрокаТЧ.Партнер;
							ДопКритерийПоиска.ОбновлениеИБ = Истина;
							ДопКритерийПоиска.ВалютаВзаиморасчетов = СтрокаТЧ.Валюта; 
							ДопКритерийПоиска.ВернутьПервый = Ложь;
							ДопКритерийПоиска.НеГенерироватьИсключение = Истина;
...
Функция ТипРасчетовРасчетоногоДокумента(СтрокаТЧ, ТипыРасчетыСПоставщиками, ТипыРасчетыСКлиентами)
	
	ТипРасчетногоДокумента = ТипЗнч(СтрокаТЧ.РасчетныйДокумент);
	ТипРасчетов = Неопределено;
	РасчетыСпоставщиком = ТипыРасчетыСПоставщиками.Найти(ТипРасчетногоДокумента);
	РасчетыСКлиентом = ТипыРасчетыСКлиентами.Найти(ТипРасчетногоДокумента);
	Если РасчетыСпоставщиком <> Неопределено и РасчетыСКлиентом = Неопределено Тогда
		ТипРасчетов = Перечисления.ТипыРасчетовСПартнерами.РасчетыСПоставщиком;
	ИначеЕсли РасчетыСКлиентом <> Неопределено и РасчетыСпоставщиком = Неопределено Тогда
		ТипРасчетов = Перечисления.ТипыРасчетовСПартнерами.РасчетыСКлиентом;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ТипРасчетов) 
		И ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТЧ, "ВидЗадолженности") Тогда
		
		Если (СтрокаТЧ.ВидЗадолженности = Перечисления.ВидыЗадолженности.ЗадолженностьКонтрагентовВал
			ИЛИ СтрокаТЧ.ВидЗадолженности = Перечисления.ВидыЗадолженности.ЗадолженностьКонтрагентовРегл) Тогда
			ТипРасчетов = Перечисления.ТипыРасчетовСПартнерами.РасчетыСКлиентом;
		ИначеЕсли СтрокаТЧ.ВидЗадолженности = Перечисления.ВидыЗадолженности.ВыданныеАвансы Тогда
			ТипРасчетов = Перечисления.ТипыРасчетовСПартнерами.РасчетыСПоставщиком;
		КонецЕсли;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ТипРасчетов) Тогда
		Если ТипРасчетногоДокумента = Тип("ДокументСсылка.ОтчетКомиссионера") Тогда
			ТипРасчетов = Перечисления.ТипыРасчетовСПартнерами.РасчетыСКлиентом;
		ИначеЕсли ТипРасчетногоДокумента = Тип("ДокументСсылка.ОтчетКомитенту") Тогда
			ТипРасчетов = Перечисления.ТипыРасчетовСПартнерами.РасчетыСПоставщиком;
		ИначеЕсли ТипРасчетногоДокумента = Тип("ДокументСсылка.ОтчетПоКомиссииМеждуОрганизациями") Тогда
			ТипРасчетов = ?(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаТЧ.РасчетныйДокумент, "Организация") = СтрокаТЧ.Контрагент,
