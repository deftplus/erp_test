...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = '1) Создает документы ""Регистратор плана производства"" для планов с расчетом потребностей в материалах (видах РЦ, трудозатратах).
	|2) Заполняет реквизит ""Направление деятельности"" в табличной части ""Продукция"".
	|3) Обновляет признак ""Замещен"" и ""Замещен к заказу"":
	...
	|en = '1) Creates the ""Production plan recorder"" documents for plans with calculation of material demand (Work center types, labor costs).
	|2) Fills in the ""Line of business"" attribute in the ""Products"" table.
	|3) Updates the ""Substituted"" and ""Substituted to order"" check boxes:
	| - in the plans in the Confirmed status, the check box ""Substituted to order"" is selected, if there are plans in the Confirmed status for the plan
	| - in the plans in the Confirmed status, the check box ""Substituted"" is selected, if there are plans in the ""In progress"" status for the plan
	| - in the plans in the Canceled status, the check box ""Substituted"" is unselected.'");
...
Функция ТекстЗапросаДанныеКОбработке() Экспорт
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ТекстЗапроса = ТекстЗапросаДанныеКОбработке();
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	НазначенияИНаправленияДеятельности = ЗапросПакет[5].Выгрузить();
	Выборка = ЗапросПакет[6].Выбрать();
