	...
	Если НастройкиОтправки = Неопределено Тогда
		...
		Иначе 
			ТекстСообщения = НСтр("ru = 'Не создано настроек отправки электронных документов между:
				...
				|en = 'No settings for sending electronic documents between:
				|Company - %1.
				|Counterparty - %2 are created.
				|Go to the administrator as you do not have enough rights to complete the transaction.'");
	...
	Если Не НастройкиОтправки.Формировать Тогда
		...
		Иначе 
			ТекстСообщения = НСтр("ru = 'Запрещена отправка документов с видом %1 между:
				...
				|Контрагент - %3.
				|Обратитесь к администратору, так как у Вас недостаточно прав для выполнения операции.';
				|en = 'Cannot send the documents of the %1 kind between:
				|Company - %2.
				|Counterparty - %3.
				|You do not have permissions for the operation, contact the administrator.'");
			ОбщегоНазначения.СообщитьПользователю(
	...
	Если НастройкиФормированияВнутреннегоЭДО = Неопределено Тогда
		...
		Иначе 
			ТекстСообщения = НСтр("ru = 'Не создано настроек формирования документов для:
				...
				|Вид документа - %2.
				|Обратитесь к администратору, так как у Вас недостаточно прав для выполнения операции.';
				|en = 'No document generation settings created for:
				|Company - %1.
				|Document kind - %2.
				|You do not have permissions for operation, contact the administrator.'");
			ОбщегоНазначения.СообщитьПользователю(
	...
	Если Не НастройкиФормированияВнутреннегоЭДО.Формировать Тогда
		...
		Иначе 
			ТекстСообщения = НСтр("ru = 'Запрещена отправка произвольных документов между:
				...
				|Вид документа - %2.
				|Обратитесь к администратору, так как у Вас недостаточно прав для выполнения операции.';
				|en = 'Sending arbitrary documents between:
				|Company - %1.
				|Document kind - %2 is unavailable.
				|Contact the administrator as you do not have sufficient rights to perform the operation.'");
		КонецЕсли;
		ОбщегоНазначения.СообщитьПользователю(
			СтрШаблон(ТекстСообщения, Объект.Организация, Объект.ВидДокумента))	
	КонецЕсли;
	...
	Иначе
		...
		Если Результат.Переформирован Тогда
			...
			НСтр("ru = 'Сформирован новый документ';
				|en = 'A new document is generated'"), БиблиотекаКартинок.ЭмблемаСервиса1СЭДО48, СтатусОповещенияПользователя.Информация);
