...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипДанныхЗаполнения = Тип("ДокументСсылка.ЗаказКлиента") Тогда
		...
		Если Не ЕстьТоварыПодЗаказ(ДанныеЗаполнения) Тогда
			ТекстИсключения = НСтр("ru = 'Ввод разрешений на замену возможен при наличии товаров с обеспечением ""К обеспечению"" и с установленным флагом ""Обособленно"".';
									|en = 'Entering permissions for replacement is only available if there are goods with the ""For supply"" supply and with the ""Assigned"" checkbox selected.'");
			ВызватьИсключение ТекстИсключения;
		КонецЕсли;
...
Функция ЕстьТоварыПодЗаказ(Ссылка)
	...
	СтруктураОтбора = Новый Структура("ВариантОбеспечения, Обособленно", 
									   Перечисления.ВариантыОбеспечения.Требуется, Истина);
