...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	УстановитьНачальноеОформлениеСтраницы(ИмяСтраницы, ПараметрыОбщие.ПоказатьОстатки);
...
Процедура ДатаПриИзменении(Элемент)
	...
	Если ПорцииПодбора.Количество() > 0 Тогда
		...
		ТекстВопроса = НСтр("ru = 'Необходимо отменить сохраненные подборы для выполнения операции. Отменить подборы?';
							|en = 'You must cancel saved selections to perform the operation. Cancel selections?'");
	...
	Иначе
		...
		Иначе	
			...
			ПараметрыФормыВопроса.Вставить("ТекстСообщения", НСтр("ru = 'Ни один аналог не выбран. Закрыть форму подбора?';
																	|en = 'No analogue has been selected. Close the picking form?'"));
	...
	Иначе
		...
		Если ТекущиеДанные <> Неопределено Тогда
			...
			ЭлементОтбора.ПравоеЗначение = 
				?(ТекущиеДанные.Обособленно И Не ЗначениеЗаполнено(ТекущиеДанные.Назначение), Неопределено, ТекущиеДанные.Назначение);
	...
	Иначе
		...
		Пока ТаблицаМатериалов.Количество() > 0 Цикл
			...
			ОбщееКоличествоКЗамене = СтрокаТаблицы.НеОбеспечивать 
									 + СтрокаТаблицы.ОтложитьОбеспечение 
...
Функция КлючСвязи(Идентификатор, Полный = Истина)
	...
	ЗначениеКлючаСвязи = 
		?(ТипЗнч(Идентификатор) = Тип("Число"), 
			Формат(Идентификатор, СтрШаблон(НСтр("ru = 'ЧГ=; ЧФ=''%1''';
												|en = 'NG=; NF=''%1'''"), ?(Полный, ";Ч;", "Ч;"))), 
...
Функция ПодсказкаПоказатели(СтрокаТаблицы, ПоляПодсказкиПоказатели)
	...
	Для каждого ПолеПодсказки Из ПоляПодсказкиПоказатели Цикл
		...
		Если ЗначениеЗаполнено(ЗначениеПоказателя) Тогда
			...
			ТекстПодсказки = 
				СтрШаблон(ТекстПодсказки, СтрШаблон("%1: %2; ", ПолеПодсказки.Представление, ЗначениеПоказателя), ШаблонТекста);
...
Функция ПоляПодсказкиПоказатели()
	...
	ИмяПоказателя = "ОтложитьОбеспечение";
	ДанныеПоказателей.Добавить(Новый Структура("Имя, Представление", ИмяПоказателя, ПоляПоказатели[ИмяПоказателя]));
...
Функция ПоказателиРаспределениеЗапасов()
	...
	ПоляПоказатели.Вставить("НеОбеспечивать",         НСтр("ru = 'Не обеспечивать';
															|en = 'Do not supply'"));
	ПоляПоказатели.Вставить("ОтложитьОбеспечение",    НСтр("ru = 'Отложено';
															|en = 'Deferred'"));
...
&НаСервере
Процедура УстановитьНачальноеОформлениеСтраницы(ИмяСтраницы, ПризнакПоказатьОстатки)
	...
	Элементы.ИсточникЗамены.Видимость = ?(ИмяСтраницы = "Материалы", ПризнакПоказатьОстатки, Ложь);
	Элементы.ИсточникЗаменыНаСтраницеАналоги.Видимость = ?(ИмяСтраницы = "Аналоги", ПризнакПоказатьОстатки, Ложь);
