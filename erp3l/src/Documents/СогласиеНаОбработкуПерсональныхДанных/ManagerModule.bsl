...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если ПечатнаяФорма <> Неопределено Тогда
		...
		Шаблон = НСтр("ru = 'Согласие на обработку ПДн [Субъект] - [Организация] (№[Номер] получено [ДатаПолучения])';
						|en = 'Consent to personal data processing [Субъект] - [Организация] (#[Номер] received on [ДатаПолучения])'");
...
Процедура ПроверитьВозможностьОформленияСогласия(СубъектПДн) Экспорт
	...
	Если ЗащитаПерсональныхДанных.ЭтоОбъектСоСкрытымиПерсональнымиДанными(СубъектПДн) Тогда
		ВызватьИсключение НСтр("ru = 'Оформление согласия невозможно, ПДн субъекта скрыты.';
								|en = 'Cannot register the consent as personal data of the subject is hidden.'");
	КонецЕсли;
