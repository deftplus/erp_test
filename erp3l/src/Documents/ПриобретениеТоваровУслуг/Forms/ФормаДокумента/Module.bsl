...
Процедура ПартнерПриИзменении(Элемент)
	...
	ПриОжиданииВыполнитьКонтрольЛимитов();
	#КонецОбласти 
...
Процедура УстановитьУсловноеОформление()
	...
	НовыйРеквизит.Заголовок = "Страны производителя";
	...
	Если НЕ ЕстьПравоДобавленияКорректировок() И (ЕстьКорректировки ИЛИ ЕстьСчетаФактуры) И ИспользоватьКорректировкиПриобретений Тогда
		...
		Если ЕстьКорректировки Тогда
			ТекстСообщения = НСтр("ru = 'На основании документа введена корректировка приобретения.
				...
				|Insufficient rights to create the debit note.'");
		Иначе
...
Функция УХ_ПолучитьРезультатЗапросаОстатковДляПриобретенияТоваров(
	...
	Запрос.Текст =
		...
		|			&Момент,
		|			Договор = &Договор) КАК ПланПоставокПоДоговорамОстатки";
...
Функция УХ_ПолучитьСтруктуруДействийДляЗаполнениястрокиТоваров()
	СтруктураПересчетаСуммы =
		ОбработкаТабличнойЧастиКлиентСервер.ПараметрыПересчетаСуммыНДСВСтрокеТЧ(
			Объект);
	СтруктураДействий = Новый Структура;
	СтруктураДействий.Вставить("ПроверитьЗаполнитьСклад", ОбработкаТабличнойЧастиКлиентСервер.ПолучитьСтруктуруЗаполненияСкладаВСтрокеТЧ(Объект, СкладГруппа));
	СтруктураДействий.Вставить("ЗаполнитьПризнакТипНоменклатуры", Новый Структура("Номенклатура", "ТипНоменклатуры"));
...
КонецПроцедуры
 ...
 &НаКлиенте 
Процедура ПриОжиданииВыполнитьКонтрольЛимитов()
