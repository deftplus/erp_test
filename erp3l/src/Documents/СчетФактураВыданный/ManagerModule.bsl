...
Функция ТаблицаТоваров(ПараметрыЗаполнения) Экспорт
	...
	Если ЭтоИсправление Тогда
		...
		КонецЕсли;
	Иначе
		Результат.Товары = ТаблицаТоваровКорректировка(МенеджерВременныхТаблиц);
	КонецЕсли;
...
Функция ТаблицаТоваровКорректировка(МенеджерВременныхТаблиц)
...
#Область ОбработатьДанныеДляПереходаНаНовуюВерсиюТовары
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет служебные реквизиты документа ""Счет-фактура выданный"" (исправленные и корректировочные).';
									|en = 'Fills in service attributes of the document ""Issued tax invoice"" (corrected and adjusting).'");
