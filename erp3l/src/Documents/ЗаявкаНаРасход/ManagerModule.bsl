...
Функция ПроверитьВозможностьУстановкиСостояния(ОбъектСсылка, СостояниеОбъекта, ПротоколОшибок = Неопределено) Экспорт
	...
	Если ТипЗнч(ОбъектСсылка) <> Тип("ДокументСсылка.ЗаявкаНаРасход") Тогда
		ТекстОшибки = СтрШаблон( 
			...
			ОбъектСсылка);
		ВстраиваниеОПККлиентСерверПереопределяемый.ДобавитьОшибкуУстановкиСтатусаОбъекта(ПротоколОшибок, ОбъектСсылка, ТекстОшибки);
		Возврат Ложь;
	КонецЕсли;
...
Функция Источник_ВзаиморасчетыПоКонтрагенту(Объект)
	...
	Контрагент = ЗаявкиНаОперации.РеквизитЗаявки(Объект, РеквизитыЗаявки.Контрагент);
	...
	Результат.Вставить("ВидОперацииУХ", Объект.ВидОперацииУХ);
	Результат.Вставить("Контрагент", Контрагент);
	Результат.Вставить("СуммаПлатежа", Объект.СуммаДокумента);
	Результат.Вставить("Валюта", ЗаявкиНаОперации.РеквизитЗаявки(Объект, РеквизитыЗаявки.ВалютаДокумента));
...
Функция Источник_РасходыПоДоговору(Объект)
	...
	Для Каждого СтрокаОперации Из Объект.ДвиженияОперации Цикл
		...
		Строка.Валюта = Объект.ВалютаДокумента;
		Строка.ЭтоОплата = КонтрольОплаты;
		Строка.Сумма = СтрокаОперации.Сумма;
	КонецЦикла;
...
Функция ПланыДокумента(Источник) Экспорт
	...
	ИначеЕсли ТипЗнч(Источник) = Тип("ДокументОбъект.ЗаявкаНаРасход") Тогда
	...
	ПредопределенныеДанные = Новый Структура(
		...
		ДанныеОбъекта.ДатаНачала, ДанныеОбъекта.Организация, ДанныеОбъекта.Контрагент, ДанныеОбъекта.ДоговорКонтрагента,
...
Функция ДанныеОбъектаПоСсылке(Источник)
	...
	|	ЗаявкаНаРасход.Дата КАК Дата,
	|	ЗаявкаНаРасход.ВалютаДокумента КАК ВалютаДокумента,
	|	ЗаявкаНаРасход.ДатаНачала КАК ДатаНачала,
	|	ЗаявкаНаРасход.Организация КАК Организация,
	|	ЗаявкаНаРасход.Контрагент КАК Контрагент,
	|	ЗаявкаНаРасход.ДоговорКонтрагента КАК ДоговорКонтрагента,
	|	ЗаявкаНаРасход.ЦФО КАК ЦФО,
	|	ЗаявкаНаРасход.Проект КАК Проект,
	|	ЗаявкаНаРасход.ДокументПланирования КАК ДокументПланирования,
	|	ЗаявкаНаРасход.ДвиженияОперации.(
	|		СтатьяДоходовИРасходов КАК СтатьяДоходовИРасходов,
