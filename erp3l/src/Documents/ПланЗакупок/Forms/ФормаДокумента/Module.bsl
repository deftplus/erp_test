...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Объект.КроссТаблица Тогда
		...
		#Область УХ_Внедрение
		Если ЭтоПланированиеПотребностей(Объект.РежимФормированияПотребностейПланомЗакупок) Тогда
			//
...
Процедура Подключаемый_ПриИзмененииСуммы(Элемент)
	...
	#Область УХ_Внедрение
	Если ЭтоПланированиеПотребностей(Объект.РежимФормированияПотребностейПланомЗакупок) И ЗначениеЗаполнено(СтрокаТоваров.ТоварнаяПозиция) Тогда
	...
	Если НЕ Объект.КроссТаблица Тогда
		...
		Если Отказ Тогда
			...
			ТекстВопроса = НСтр("ru = 'В строках списка ""Товары"" указаны даты за границами планирования - строки будут удалены. Удалить строки?';
								|en = 'Dates outside the planning limits are specified in lines of the Item list. Lines will be deleted. Delete the lines?'");
	...
	Если Объект.КроссТаблица Тогда
		...
		ПоказатьВопрос(
			...
			НСтр("ru = 'Подбор аналогов возможно в режиме ""Периоды в строках"". Переключить режим?';
				|en = 'Alternative products can only be selected in the ""Periods in lines"" mode. Enable this mode?'"), 
			РежимДиалогаВопрос.ДаНет, 
			45, 
			КодВозвратаДиалога.Нет, 
			НСтр("ru = 'Подбор аналогов';
				|en = 'Select alternative products'"), 
			КодВозвратаДиалога.Нет);
...
Процедура УстановитьУсловноеОформлениеКроссТаблицы()
	...
	Для каждого Период Из АктивныеПериоды Цикл
		...
		Если Объект.ПланироватьПоСумме Тогда
			...
			КонецЕсли; 
			#Область УХ_Внедрение
			Если ЭтоПланированиеПотребностей(Объект.РежимФормированияПотребностейПланомЗакупок) И НЕ ЭтоОдинПериод Тогда
				ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
...
Процедура ПриИзмененииКоличестваСуммыСтроки(Объект, Форма, СтрокаТоваров)
	...
	Для каждого СтрокаПериода Из Форма.Периоды Цикл
		...
		Если Объект.ПланироватьПоСумме Тогда
			#Область УХ_Внедрение
			Если НЕ (ЭтоПланированиеПотребностей(Объект.РежимФормированияПотребностейПланомЗакупок) И ЗначениеЗаполнено(СтрокаТоваров.ТоварнаяКатегория)) Тогда
			#КонецОбласти 
...
Процедура РассчитатьИтоговыеПоказатели(Форма, РассчитыватьСтроки = Истина)
	...
	Для каждого СтрокаТЧ Из Товары Цикл
		#Область УХ_Внедрение
		ЭтоТК = ЭтоПланированиеПотребностей(Объект.РежимФормированияПотребностейПланомЗакупок) И ЗначениеЗаполнено(СтрокаТЧ.ТоварнаяКатегория);
		#КонецОбласти 
...
Процедура ОбновитьСтруктуруКроссТаблицы(ТаблицаПериоды, ПересчитыватьИтоговыеПоказатели = Истина)
	...
	#Область УХ_Внедрение
	Если ЭтоПланированиеПотребностей(Объект.РежимФормированияПотребностейПланомЗакупок) И НЕ (Объект.ПланироватьПоСумме И ЭтоОдинПериод) Тогда
		Планирование.ДобавитьПолеКроссТаблицыСумма(ПараметрыВывода, Объект.ПланироватьПоСумме И (ЭтоОдинПериод ИЛИ ЭтоПланированиеПотребностей(Объект.РежимФормированияПотребностейПланомЗакупок)), СтруктураДействий);
	Иначе
	#КонецОбласти 
...
Процедура СценарийПриИзмененииСервер()
	...
	#Область УХ_Внедрение
	Реквизиты = Реквизиты + ", РежимФормированияПотребностейПланомЗакупок";
	#КонецОбласти 
...
Процедура УстановитьВидимость()
	...
	#Область УХ_Внедрение
	ВстраиваниеУХПланЗакупок.ПриУстановкеВидимости(ЭтаФорма);
	#КонецОбласти
...
Процедура СкопироватьВКроссТаблицу(КэшированныеЗначения)
	...
	Если Объект.КроссТаблица
		...
		Для каждого СтрокаТЧ Из Объект.Товары Цикл
			...
			#Область УХ_Внедрение
			Если ЭтоПланированиеПотребностей(Объект.РежимФормированияПотребностейПланомЗакупок) И ТипЗнч(СтрокаТЧ.ТоварнаяПозиция) = Тип("СправочникСсылка.ТоварныеКатегории") Тогда
...
Функция СтруктураДействийПриЗагрузкеИзExcel()
	...
	КолонкиПоиска.Добавить(НСтр("ru = 'Уникальный идентификатор (Упаковка)';
								|en = 'Unique ID (Package)'"),"ИдентификаторУпаковка");
...
Функция ПодготовитьПараметрыЗаполненияДокументаДляЗамещения(ПараметрыЗадания, КэшированныеЗначения)
	...
	#Область УХ_Внедрение
	ПараметрыЗадания.Вставить("ЭтоПланированиеПотребностей", ЭтоПланированиеПотребностей(Объект.РежимФормированияПотребностейПланомЗакупок));
...
Процедура УдалитьПустыеСтрокиНаСервере()
	...
	Пока Индекс >= 0 Цикл
		...
		Для каждого Период Из АктивныеПериоды Цикл
			...
			#Область УХ_Внедрение
			Если ЭтоПланированиеПотребностей(Объект.РежимФормированияПотребностейПланомЗакупок) И ЗначениеЗаполнено(СтрокаТЧ.ТоварнаяКатегория)
...
Функция ПараметрыЗамещения(Знач Объект)
	...
	Если ЭтоПланированиеПотребностей(Объект.РежимФормированияПотребностейПланомЗакупок) Тогда
		...
		// 
		//ПараметрыЗамещения.Удалить("Склад");
		//ПараметрыЗамещения.Удалить("Подразделение");
...
Процедура Подключаемый_ЗафиксироватьТипНоменклатуры(Команда)
	...
	Элементы.ТоварыТоварнаяПозиция.ВыбиратьТип = Элементы.ТоварыПоПериодамТоварнаяПозиция.ВыбиратьТип;
	Элементы.ТоварыЗафиксироватьТипНоменклатуры.Заголовок = Элементы.ТоварыПоПериодамЗафиксироватьТипНоменклатуры.Заголовок;
	Элементы.ТоварыЗафиксироватьТипНоменклатуры.Пометка = Элементы.ТоварыПоПериодамЗафиксироватьТипНоменклатуры.Пометка;
