...
Процедура ОбновитьТекстСчетаФактурыПеревыставленные()
	...
	Если Объект.Проведен Тогда
		...
		МассивГиперСсылок.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Перевыставленные счета-фактуры комитенту (принципалу):';
																	|en = 'Reissued proforma invoices to the consignor (principal):'"), , ЦветаСтиля.ГиперссылкаЦвет,, ));
		...
		Если Выборка.Следующий() Тогда
			...
			Если КоличествоВыданы Тогда
				...
				ТекстВыданы = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'выданы (%1)';
																							|en = 'issued (%1)'"), КоличествоВыданы);
			...
			Если КоличествоПеревыставить Тогда
				МассивГиперСсылок.Добавить("  ");
				ТекстТребуютПеревыставления = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'требуют переоформления/не оформлены (%1)';
																											|en = 'need to be reregistered/not registered (%1)'"), КоличествоПеревыставить);
	...
	Иначе
		МассивГиперСсылок.Добавить(Новый ФорматированнаяСтрока(НСтр("ru = 'Для перевыставления счетов-фактуры комитенту (принципалу) необходимо провести документ.';
																	|en = 'Posting the document is required to re-issue tax invoices to the consignor (principal).'"), , ЦветаСтиля.ЦветТекстаФормы,, ));
	КонецЕсли;	
	//-- Локализация
