...
Функция ТекстЗапросаТоварыУслугиКОформлениюОтчетовКомитентуОЗакупкахОстаткиПоНазначениям(ИмяВременнойТаблицы = Неопределено) Экспорт
	...
	ТекстЗапроса = "ВЫБРАТЬ
	               ...
	               |	ТоварыУслугиКОформлениюОтчетовКомитентуОЗакупкахОстатки.Номенклатура.СтавкаНДС КАК СтавкаНДС
	               |,ИмяВременнойТаблицы
	               |ИЗ
...
//
// Возвращаемое значение:
//  Массив - Массив из см. ВзаиморасчетыСервер.ПараметрыМеханизма
//
...
Функция ПараметрыРегистрацииСчетовФактурВыданных(Объект) Экспорт
	...
	ПараметрыРегистрации.РеализацияРаботУслуг	= (НЕ Объект.СпособРасчетаВознаграждения = Перечисления.СпособыРасчетаКомиссионногоВознаграждения.НеРассчитывается);
...
Функция СформироватьГиперссылкуКОформлению(Параметры) Экспорт
	...
	ТекстГиперссылки = НСтр("ru = 'Отчеты комитентам (принципалам) о закупках';
							|en = 'Purchase invoices — Consignment stock sold'");
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	Если НЕ ПраваПользователяПовтИсп.ЭтоПартнер() Тогда
		...
		КомандаПечати.Представление = НСтр("ru = 'Отчет комитенту (принципалу) о закупках';
											|en = 'Purchase invoice — Consignment stock sold'");
		КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
	КонецЕсли;
...
Функция КомплектПечатныхФорм() Экспорт
	...
	РегистрыСведений.НастройкиПечатиОбъектов.ДобавитьПечатнуюФормуВКомплект(КомплектПечатныхФорм, "ОтчетКомитентуОЗакупках", НСтр("ru = 'Отчет комитенту (принципалу) о закупках';
																																	|en = 'Purchase invoice — Consignment stock sold'"), 2);
...
Функция СформироватьПечатнуюФормуАктОбОказанииУслуг(МассивОбъектов, ОбъектыПечати)
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		ТекстОСуммеПрописью = НСтр("ru = 'Сумма комиссионного вознаграждения составила';
									|en = 'Commission fee equals'", ОбщегоНазначения.КодОсновногоЯзыка()) + " "
...
Функция ПолучитьДанныеДляПечатнойФормыОтчетПоЗакупкам(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	Заголовок = НСтр("ru = 'Отчет комитенту о закупках';
					|en = 'Purchase invoice — Consignment stock sold'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	|en = 'Updates the remuneration VAT amount in lines of the ""Purchase invoice — Consignment stock sold"" document lines.
	| Populates ""Document ID"" attribute"".'");
