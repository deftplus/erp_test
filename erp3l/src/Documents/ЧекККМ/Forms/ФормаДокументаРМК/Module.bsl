...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если Источник = "ПодключаемоеОборудование"
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ЭтотОбъект,"КодМаркировкиДляУточнения") Тогда
			Возврат;
		КонецЕсли;
	...
	Если Не глПодключаемоеОборудование.Свойство("ПоследнийСлипЧек", СтрокаСлипЧека)
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(
			НСтр("ru = 'Слип-чек отсутствует.
			         ...
			         |Probably, the POS transaction has not been performed for this session yet.'"));
	...
	Исключение
		...
		Шаблон = НСтр("ru = 'Не удалось записать чек ККМ по причине: %1';
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'РМК';
					|en = 'CWP'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура УстановитьФискальныйСтатусПробит()
	...
	ОбщегоНазначенияКлиент.СообщитьПользователю(
		НСтр("ru = 'Функционал печати чека для документа не реализован. Обратитесь к разработчикам.';
			|en = 'Функционал печати чека для документа не реализован. Обратитесь к разработчикам.'"));
