...
Процедура ЗаполнитьТаблицуПолуфабрикатов(ПараметрыПродукции)
	...
	|	ЕСТЬNULL(ОстатокНаСкладе.ВНаличии / ЕСТЬNULL(&ТекстЗапросаКоэффициентУпаковки, 1), 0) КАК ВНаличии
	|ИЗ
	|	ВТПолуфабрикаты КАК ТПолуфабрикаты
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РаспределениеЗапасов КАК ОстатокНаСкладе
	|		ПО ТПолуфабрикаты.Номенклатура = ОстатокНаСкладе.Номенклатура
	|			И ТПолуфабрикаты.Характеристика = ОстатокНаСкладе.Характеристика
	|			И ТПолуфабрикаты.Склад = ОстатокНаСкладе.Склад
	...
	|ГДЕ
	|	ЕСТЬNULL(ОстатокНаСкладе.ВНаличии, 0) > 0");
