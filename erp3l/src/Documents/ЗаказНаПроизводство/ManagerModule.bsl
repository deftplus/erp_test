...
Функция ПолучитьРезультатЗапросаПоОстаткамПродукцииКОбеспечениюДавальцу(ЗаказДавальца, ТекущийДокумент = Неопределено) Экспорт
	...
	ТекстЗапроса =
		...
		|		Движения.ПоступитКДате + Движения.ПоставкаНаСогласовании КАК Количество
		|	ИЗ
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Процедура = "Документы.ЗаказНаПроизводство.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	Обработчик.Версия = "2.5.7.45";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("744c5d7b-5a78-4903-8c04-e3c2cae2a466");
	Обработчик.Многопоточный = Истина;
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "Документы.ЗаказНаПроизводство.ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.Комментарий = НСтр("ru = 'Заполняет реквизит ""Вариант обеспечения"", признак ""Обособленно"".';
									|en = 'Populates the ""Supply option"" attribute, ""Pegged"" flag.'");
	...
	Изменяемые.Добавить(Метаданные.Документы.ЗаказНаПроизводство.ПолноеИмя());
	Обработчик.ИзменяемыеОбъекты = СтрСоединить(Изменяемые, ",");
	
	Блокируемые = Новый Массив;
	Блокируемые.Добавить(Метаданные.Документы.ЗаказНаПроизводство.ПолноеИмя());
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПараметрыВыборки.ПолныеИменаОбъектов = "Документ.ЗаказНаПроизводство";
	...
	Запрос.Текст =
		...
		|ИЗ
		|	Документ.ЗаказНаПроизводство КАК Реквизиты
		|ГДЕ
		|	ИСТИНА В(
		|		ВЫБРАТЬ ПЕРВЫЕ 1
		|			ИСТИНА
		...
		|				И Товары.ПроизводитсяВПроцессе И НЕ Товары.Обособленно)";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ИмяДокумента = "Документ.ЗаказНаПроизводство"; ИмяТЧ = "МатериалыИУслуги";
	...
	ТекстЗапроса =
		"ВЫБРАТЬ
		|	Реквизиты.Ссылка КАК Ссылка
		|ПОМЕСТИТЬ ДокументыДляОбработкиФлагаОбособленно
		|ИЗ
		|	Документ.ЗаказНаПроизводство КАК Реквизиты
		|ГДЕ
	...
	ТекстЗапроса =
		...
		|	НЕ ДокументыДляОбработкиФлагаОбособленно.Ссылка ЕСТЬ NULL КАК ТребуетсяОбработкаФлагаОбособленно
		|ИЗ
		|	Документ.ЗаказНаПроизводство КАК Документы
		|		
		...
		|		
		|		ЛЕВОЕ СОЕДИНЕНИЕ ДокументыДляОбработкиФлагаОбособленно КАК ДокументыДляОбработкиФлагаОбособленно
		|		ПО ДокументыДляОбработкиФлагаОбособленно.Ссылка = Документы.Ссылка
		|		
		|ГДЕ
	...
	Пока Документ.Следующий() Цикл
		...
		Попытка
			...
			Если ДокументОбъект <> Неопределено Тогда
				...
				Если Документ.ТребуетсяОбработкаФлагаОбособленно Тогда
					Для Каждого Строка Из ДокументОбъект.МатериалыИУслуги Цикл
						
						Если Строка.ПроизводитсяВПроцессе И Не Строка.Обособленно Тогда
