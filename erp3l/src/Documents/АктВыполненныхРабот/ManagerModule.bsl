...
Процедура ЗаполнитьПоОстаткамЗаказов(ДанныеОтбора,
	...
	Если РезультатЗапросаПоОстаткамЗаказов[1].Пустой() 
		...
		ТекстОшибки = НСтр("ru = 'Нет услуг с вариантом оформления ""Акт выполненных работ"", 
			...
			|en = 'No services with registration option
			|""Customer invoice — Services"" are available for shipment. To start 
			|shipping, specify action ""Ship"" for the shipped product items in the list of goods.'");
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		Если СведенияОЗаказчике.Свойство("КПП") И ЗначениеЗаполнено(СведенияОЗаказчике.КПП) Тогда
			ИННКППЗаказчика = НСтр("ru = 'ИНН/КПП:';
									|en = 'TIN/CRTR:'", ОбщегоНазначения.КодОсновногоЯзыка()) + " " + СведенияОЗаказчике.ИНН + "/" + СведенияОЗаказчике.КПП;
		Иначе
		...
		Если СведенияОИсполнителе.Свойство("КПП") И ЗначениеЗаполнено(СведенияОИсполнителе.КПП) Тогда
			ИННКППИсполнителя = НСтр("ru = 'ИНН/КПП:';
									|en = 'TIN/CRTR:'", ОбщегоНазначения.КодОсновногоЯзыка()) + " " + СведенияОИсполнителе.ИНН + "/" + СведенияОИсполнителе.КПП;
		Иначе
...
#Область ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	|en = 'The handler does the following:
	| • Populates the ""VAT rate"" attribute and ""Foreign currency transaction"" flag.
	| • Clears ""Assignment"" field in the ""Services"" tabular section for unassigned items (in Sales Orders).
	| • Populates the ""AR/AP object"" attribute.
	| • Populates the ""Business unit"" attribute in the ""Services"" tabular section for items of the ""Work"" type.'");
