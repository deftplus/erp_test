...
//
Процедура РассчитатьСостояние(Знач ОтчетДавальцу, СостояниеРасчетов) Экспорт
	...
	Если ЗначениеЗаполнено(ОтчетДавальцу) И ПравоДоступа("Чтение", Метаданные.РегистрыНакопления.РасчетыСКлиентами) Тогда
		...
		ДанныеПоСостоянию = ВзаиморасчетыСервер.СостояниеВзаиморасчетов(ОтчетДавальцу, ПараметрыВзаиморасчеты());
...
Функция КомплектПечатныхФорм() Экспорт
	...
	КонецЕсли;
	Если Константы.ПечатьИнвойсов.Получить() Тогда
...
#Область ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет реквизиты ""Ставка НДС"", ""Объект расчетов"", ""Договор не обязателен"", признак оплаты в иностранной валюте.';
									|en = 'Populates the ""VAT rate"", ""AR/AP object"", ""Contract is not required"" attributes, a flag of payment in foreign currency.'");
...
#Область СгенерироватьОбъектыРасчетов
	...
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("60801782-5bef-48cb-801b-13009ec4b8dd");
	Обработчик.Многопоточный = Истина;
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "Документы.ОтчетДавальцу.ЗарегистрироватьДанныеДляГенерацииОбъектовРасчетов";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	ИЛИ НЕ ОтчетДавальцу.ОплатаВВалюте
	|			И (ОтчетДавальцу.УдалитьПорядокОплаты = ЗНАЧЕНИЕ(Перечисление.УдалитьПорядокОплатыПоСоглашениям.РасчетыВВалютеОплатаВВалюте)
	|				ИЛИ ОтчетДавальцу.УдалитьПорядокОплаты = ЗНАЧЕНИЕ(Перечисление.УдалитьПорядокОплатыПоСоглашениям.РасчетыВРубляхОплатаВВалюте))
	|	ИЛИ ИСТИНА В
	|		(ВЫБРАТЬ ПЕРВЫЕ 1
	|			ИСТИНА
	|		ИЗ
	...
	Для Каждого Документ Из ОбновляемыеДанные Цикл
		...
		Попытка
			...
			Если ДокументОбъект <> Неопределено Тогда
				...
				ЗаполнитьЗначенияСвойств(Отбор, Документ);
				...
				МассивСтрок = ТаблицаЗаполненияРеквизитаДоговорНеОбязателен.НайтиСтроки(Отбор);
