...
&НаСервереБезКонтекста
Процедура РасшифровкаПлатежаОснованиеПлатежаОбработкаПолученияДанныхВыбора(ДанныеВыбора, Организация, Контрагент, ХозяйственнаяОперация, Текст, ЭтоОбъектРасчетов = Ложь, ПодборДебиторскойЗадолженности = Ложь)
	...
	Если Не ЭтоОбъектРасчетов Тогда
		...
		ОписаниеТипов = Метаданные.Документы.ПриходныйКассовыйОрдер.ТабличныеЧасти.РасшифровкаПлатежа.Реквизиты.ОснованиеПлатежа.Тип;
		СтруктураПараметров.Вставить("ТипыОснований", ОписаниеТипов.Типы());
	КонецЕсли;
	
	ВзаиморасчетыСервер.ЗаполнитьДанныеВыбораОснованияПлатежа(ДанныеВыбора, СтруктураПараметров,,, ЭтоОбъектРасчетов);
