...
Функция ТекстЗапросаДляРасчетаСостоянийЗаказов() Экспорт
	...
	ТекстЗапроса ="
		...
		|		И (ЕСТЬNULL(РасчетыСКлиентамиОстатки.КОплатеРасход, 0) >= ДокументЗаказКлиента.СуммаПредоплатыДоОтгрузки + ДокументЗаказКлиента.СуммаАвансаДоОбеспечения
		|			ИЛИ НЕ (ДокументЗаказКлиента.ПорядокРасчетов В (ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказам),
		...
		|ВЫБОР
		|	КОГДА
		|		ДокументЗаказКлиента.ПорядокРасчетов = ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказам) И
		...
		|		ДокументЗаказКлиента.СуммаДокумента > 0
		|	ТОГДА
		|		ВЫРАЗИТЬ ((ЕСТЬNULL(ВтДанныеПоРасчетамСКлиентами.ОплатаФакт, 0)) * 100 / ДокументЗаказКлиента.СуммаДокумента КАК ЧИСЛО(15, 10))
		|	ИНАЧЕ
		|		0
...
Функция ПараметрыВзаиморасчеты(ДанныеЗаполнения = Неопределено) Экспорт
	...
	СтруктураПараметров.ДатаОтсчетаГрафика               = "Объект.ДатаОтгрузки"; 
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	Если НЕ ПолучитьФункциональнуюОпцию("ИспользоватьСчетаНаОплатуКлиентам")
		И Константы.ПечатьИнвойсов.Получить() Тогда
...
Функция ЗаполнитьПоФактическойПотребности2_2(Параметры, ТабЧастьМатериалы)
	...
	Запрос.Текст =
		...
		|		Движения.ПоступитКДате  КАК КЗаказу
		|	ИЗ
...
#Область ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет реквизиты ""Ставка НДС"", ""Объект расчетов"", ""Вариант обеспечения"", признак ""Обособленно"", признак оплаты в иностранной валюте.';
									|en = 'Populates attributes ""VAT rate"", ""AR/AP object"", ""Supply option"", the ""Assigned"" flag, payment in foreign currency flag.'");
...
#Область СгенерироватьОбъектыРасчетов
	...
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("88ef2df7-22e5-4e34-9ac5-c1b24d0250ed");
	Обработчик.Многопоточный = Истина;
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "Документы.ЗаказДавальца.ЗарегистрироватьДанныеДляГенерацииОбъектовРасчетов";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
	Обработчик.ЗапускатьТолькоВГлавномУзле = Истина;
	Обработчик.Комментарий = НСтр("ru = 'Генерирует элементы справочнкиа ""Объекты расчетов""';
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	ИЛИ НЕ ЗаказДавальца.ОплатаВВалюте
	|			И (ЗаказДавальца.УдалитьПорядокОплаты = ЗНАЧЕНИЕ(Перечисление.УдалитьПорядокОплатыПоСоглашениям.РасчетыВВалютеОплатаВВалюте)
	|				ИЛИ ЗаказДавальца.УдалитьПорядокОплаты = ЗНАЧЕНИЕ(Перечисление.УдалитьПорядокОплатыПоСоглашениям.РасчетыВРубляхОплатаВВалюте))
	|	ИЛИ (ЗаказДавальца.ОбъектРасчетов = ЗНАЧЕНИЕ(Справочник.ОбъектыРасчетов.ПустаяСсылка)
	|			И ЗаказДавальца.ПорядокРасчетов В (ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказам), 
	|												ЗНАЧЕНИЕ(Перечисление.ПорядокРасчетов.ПоЗаказамНакладным),
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ИмяДокумента = "Документ.ЗаказДавальца"; ИмяТЧ = "Продукция";
