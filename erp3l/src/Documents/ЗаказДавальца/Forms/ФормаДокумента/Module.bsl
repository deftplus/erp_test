...
Процедура ПродукцияПриОкончанииРедактированияЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Нет Тогда
		...
		ПродукцияПриОкончанииРедактированияСервер(ДополнительныеПараметры, КэшированныеЗначения);
	...
	Для каждого СтрокаТовара Из ТаблицаТоваров Цикл
		...
		Если СтрокаТовара.ТипНоменклатуры = Перечисления.ТипыНоменклатуры.МногооборотнаяТара Тогда
			...
			НСтр("ru = 'Спецификация содержит материал с типом ""Многооборотная тара"" (%1). Добавление тары в заказ давальцу не допускается.';
				|en = 'Bill of materials contains material with type ""Reusable packaging"" (%1). Adding package to material provider order is not allowed.'"),
			СтрокаТовара.Номенклатура);
...
&НаСервере
Процедура ПродукцияПриОкончанииРедактированияСервер(ДополнительныеПараметры, КэшированныеЗначения)
