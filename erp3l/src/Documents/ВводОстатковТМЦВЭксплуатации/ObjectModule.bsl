...
Процедура ПроверкаЗаполненияДокумента(НепроверяемыеРеквизиты, Отказ)
	...
	Если НачалоМесяца(Дата) >= НачалоМесяца(РеглУчетКлиентСервер.НачалоПримененияФСБУ5_2019()-1) Тогда
		НепроверяемыеРеквизиты.Добавить("ТМЦВЭксплуатации.ПервоначальнаяСумма");
	КонецЕсли;
