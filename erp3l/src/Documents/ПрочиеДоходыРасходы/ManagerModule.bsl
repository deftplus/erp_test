...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет реквизит ""Настройка счетов учета"".';
									|en = 'Fills in ""GL account settings"" attribute.'");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|				ИСТИНА
	|			ИЗ
	|				Документ.ПрочиеДоходыРасходы.ПрочиеДоходы КАК ТЧПрочиеДоходы
	|			ГДЕ
	|				ТЧПрочиеДоходы.Ссылка = Документ.Ссылка
	|				И ТЧПрочиеДоходы.УдалитьСчетУчета <> &СчетУчетаПустаяСсылка
	|				И ТЧПрочиеДоходы.НастройкаСчетовУчета = ЗНАЧЕНИЕ(Справочник.НастройкиСчетовУчетаПрочихОпераций.ПустаяСсылка))
	|	ИЛИ ИСТИНА В (ВЫБРАТЬ ПЕРВЫЕ 1
	|				ИСТИНА
	|			ИЗ
	|				Документ.ПрочиеДоходыРасходы.ПрочиеРасходы КАК ТЧПрочиеРасходы
	|			ГДЕ
	|				ТЧПрочиеРасходы.Ссылка = Документ.Ссылка
	|				И ТЧПрочиеРасходы.УдалитьСчетУчета <> &СчетУчетаПустаяСсылка
	|				И ТЧПрочиеРасходы.НастройкаСчетовУчета = ЗНАЧЕНИЕ(Справочник.НастройкиСчетовУчетаПрочихОпераций.ПустаяСсылка))
	//-- НЕ УТ
	|";
