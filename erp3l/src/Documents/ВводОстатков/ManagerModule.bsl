		...
		Если СтрокаТЧ.ОбъектРасчетовОтправитель = Справочники.ОбъектыРасчетов.ПустаяСсылка()
			...
			Если ЗначениеЗаполнено(ИсточникОбъектаРасчетов) Тогда
				...
				Если ТипЗнч(ИсточникОбъектаРасчетов) = Тип("ДокументСсылка.ОтчетПоКомиссииМеждуОрганизациями") 
					...
					И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ИсточникОбъектаРасчетов, "ХозяйственнаяОперация") = Перечисления.ХозяйственныеОперации.ПередачаНаКомиссиюВДругуюОрганизацию)) Тогда
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	|en = 'Updates the ""Enter opening balance"" documents:
	|- fills in the ""AR/AP object (sender)"" and ""AR/AP object (recipient)"" attributes.
	|- fills in the ""Business transaction"" and ""VAT rate"" attributes.
	|- fills in the ""Acquiring agreement"" attribute.
	|- fills in the ""Lot"" attribute for STS expenses.
	|- fills in the ""Lot"" attribute for inventory in operation.'");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|ИЗ
	|	Документ.ВводОстатков.РасходыПриУСН КАК РасходыПриУСН
	|ГДЕ
 	...
 	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			Если ДокументОбъект <> Неопределено Тогда
				...
				//++ НЕ УТ
				#Область УСН
				Если ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковРасходовУСНПоМатериалам
					ИЛИ ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковРасходовУСНПоТоварам
					ИЛИ ДокументОбъект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковПрочихРасходовУСН Тогда
					
					Для каждого СтрокаТЧ Из ДокументОбъект.РасходыПриУСН Цикл
						Если ЗначениеЗаполнено(СтрокаТЧ.ДокументВозникновенияРасходов) Тогда
