	...
	Если ПравоДоступа("Добавление", Метаданные.Документы.СписаниеБезналичныхДенежныхСредств) Тогда
		...
		КомандаСоздатьНаОсновании.Представление =
			...
			+ " " + НСтр("ru = 'комитента (вознаграждение комиссионеру)';
						|en = 'consignor (remuneration to commission agent)'");
		КомандаСоздатьНаОсновании.РежимЗаписи = "Проводить";
		КомандаСоздатьНаОсновании.ФункциональныеОпции = "ИспользоватьЗаявкиНаРасходованиеДенежныхСредств";
		Возврат КомандаСоздатьНаОсновании;
	КонецЕсли;
...
Функция ТекстЗапросаТаблицаДенежныеСредстваКВыплате(Запрос, ТекстыЗапроса, Регистры)
	...
	#Область УХ_Встраивание
	// Заполнение регистра не поддерживается
	ТекстДляЗамены = "ДанныеДокумента.Ссылка = &Ссылка";
	ТекстЗамены = "ЛОЖЬ И ";
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	|en = 'Populates ""VAT rate"" attribute. Populates ""AR/AP object"" attribute.
	...
	НоваяСтрока = Обработчик.ПриоритетыВыполнения.Добавить();
	НоваяСтрока.Процедура = "РегистрыСведений.ОтносительныеКурсыВалют.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	НоваяСтрока.Порядок = "После";
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	//++ НЕ УТ
	|	ИЛИ (ЗаявкаНаРасходованиеДенежныхСредств.УдалитьСчетУчета <> &СчетУчетаПустаяСсылка
	|			И ЗаявкаНаРасходованиеДенежныхСредств.НастройкаСчетовУчета = ЗНАЧЕНИЕ(Справочник.НастройкиСчетовУчетаПрочихОпераций.ПустаяСсылка))
	|	ИЛИ ИСТИНА В
	|		(ВЫБРАТЬ ПЕРВЫЕ 1
	|			ИСТИНА
	|		ИЗ
	|			Документ.ЗаявкаНаРасходованиеДенежныхСредств.РасшифровкаПлатежа КАК ТабЧастьРасшифровкаПлатежа
	|		ГДЕ
	|			ТабЧастьРасшифровкаПлатежа.Ссылка = ЗаявкаНаРасходованиеДенежныхСредств.Ссылка
	|			И ТабЧастьРасшифровкаПлатежа.УдалитьСчетУчета <> &СчетУчетаПустаяСсылка
	|			И ТабЧастьРасшифровкаПлатежа.НастройкаСчетовУчета = ЗНАЧЕНИЕ(Справочник.НастройкиСчетовУчетаПрочихОпераций.ПустаяСсылка))
	//-- НЕ УТ
	|	ИЛИ ИСТИНА В
	|			(ВЫБРАТЬ ПЕРВЫЕ 1
	|				ИСТИНА
...
Процедура ЗаполнитьИменаРеквизитовПоХозяйственнойОперацииУХ(Объект, МассивВсехРеквизитов, МассивРеквизитовОперации) Экспорт
	...
	Если Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыдачаДенежныхСредствПодотчетнику Тогда
		...
		Иначе
			// Есть статья в расшифровке платежа
			МассивРеквизитовОперации.Удалить(МассивРеквизитовОперации.Найти("СтатьяДвиженияДенежныхСредств"));
			МассивРеквизитовОперации.Удалить(МассивРеквизитовОперации.Найти("Аналитика1"));
			МассивРеквизитовОперации.Удалить(МассивРеквизитовОперации.Найти("Аналитика2"));
			МассивРеквизитовОперации.Удалить(МассивРеквизитовОперации.Найти("Аналитика3"));
			МассивРеквизитовОперации.Удалить(МассивРеквизитовОперации.Найти("Аналитика4"));
			МассивРеквизитовОперации.Удалить(МассивРеквизитовОперации.Найти("Аналитика5"));
			МассивРеквизитовОперации.Удалить(МассивРеквизитовОперации.Найти("Аналитика6"));
...
Процедура РеквизитыЗаявки(РеквизитыЗаявки) Экспорт
	...
	ЗаявкиНаОперации.РазместитьВПоляхФормы(
		...
		"АналитикаЗачисление%1", "АналитикаЗачисление%1",
...
Функция ПолучитьДвиженияОперацииДокументаДляПлатежнойПозиции(ЗаявкаСсылка) экспорт
	...
	|	ЗаявкаНаРасходованиеДенежныхСредствРасшифровкаПлатежа.Ссылка.ЦеннаяБумага КАК ЦеннаяБумага,
	|	ЗаявкаНаРасходованиеДенежныхСредствРасшифровкаПлатежа.Партнер КАК Партнер,
	|	ЗаявкаНаРасходованиеДенежныхСредствРасшифровкаПлатежа.УдалитьЗаказ КАК Заказ,
	...
	|	ЗаявкаНаРасходованиеДенежныхСредствРасшифровкаПлатежа.УдалитьСчетУчета КАК УдалитьСчетУчета,
	|	ЗаявкаНаРасходованиеДенежныхСредствРасшифровкаПлатежа.УдалитьСубконто1 КАК УдалитьСубконто1,
	|	ЗаявкаНаРасходованиеДенежныхСредствРасшифровкаПлатежа.УдалитьСубконто2 КАК УдалитьСубконто2,
	|	ЗаявкаНаРасходованиеДенежныхСредствРасшифровкаПлатежа.УдалитьСубконто3 КАК УдалитьСубконто3,
	...
	|	ЗаявкаНаРасходованиеДенежныхСредствРасшифровкаПлатежа.УдалитьСчетУчета,
	|	ЗаявкаНаРасходованиеДенежныхСредствРасшифровкаПлатежа.УдалитьСубконто1,
	|	ЗаявкаНаРасходованиеДенежныхСредствРасшифровкаПлатежа.УдалитьСубконто2,
	|	ЗаявкаНаРасходованиеДенежныхСредствРасшифровкаПлатежа.УдалитьСубконто3,
	...
	|	ЗаявкаНаРасходованиеДенежныхСредствРасшифровкаПлатежа.УдалитьЗаказ,
	|	ЗаявкаНаРасходованиеДенежныхСредствРасшифровкаПлатежа.ОбъектРасчетов,
	|	ЗаявкаНаРасходованиеДенежныхСредствРасшифровкаПлатежа.Ссылка.ЦеннаяБумага,
	...
	|	ЗаявкаНаРасходованиеДенежныхСредств.АналитикаЗачисление1,
	|	ЗаявкаНаРасходованиеДенежныхСредств.АналитикаЗачисление2,
	|	ЗаявкаНаРасходованиеДенежныхСредств.АналитикаЗачисление3,
	|	ЗаявкаНаРасходованиеДенежныхСредств.АналитикаЗачисление4,
	|	ЗаявкаНаРасходованиеДенежныхСредств.АналитикаЗачисление5,
	|	ЗаявкаНаРасходованиеДенежныхСредств.АналитикаЗачисление6,
	|	ЗНАЧЕНИЕ(Перечисление.СтавкиНДС.БезНДС),
	|	NULL,
	|	NULL,
	|	NULL,
...
Функция ПланыДокумента(Источник) Экспорт
	...
	ПредопределенныеДанные = Новый Структура(
		...
		ДанныеОбъекта.ЖелательнаяДатаПлатежа, ДанныеОбъекта.Организация, ДанныеОбъекта.Контрагент, ДанныеОбъекта.Договор,
	...
	//
	Если ЗаявкаСРасшифровкойПлатежа(ДанныеОбъекта) Тогда
		Для Каждого Строка Из ДанныеОбъекта.РасшифровкаПлатежа Цикл
...
Функция ДанныеОбъектаПоСсылке(Источник)
	...
	|	ЗаявкаНаОплату.АналитикаЗачисление1 КАК АналитикаЗачисление1,
	|	ЗаявкаНаОплату.АналитикаЗачисление2 КАК АналитикаЗачисление2,
	|	ЗаявкаНаОплату.АналитикаЗачисление3 КАК АналитикаЗачисление3,
	|	ЗаявкаНаОплату.АналитикаЗачисление4 КАК АналитикаЗачисление4,
	|	ЗаявкаНаОплату.АналитикаЗачисление5 КАК АналитикаЗачисление5,
	|	ЗаявкаНаОплату.АналитикаЗачисление6 КАК АналитикаЗачисление6,
...
Функция Источник_ВзаиморасчетыПоДоговору(Объект)
	...
	РасшифровкаПлатежа.Колонки.Добавить("Контрагент", Метаданные.ОпределяемыеТипы.Контрагенты.Тип);
	РасшифровкаПлатежа.Колонки.Добавить("Договор", Метаданные.ОпределяемыеТипы.Договор.Тип);
	...
	ИначеЕсли Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВыплатаЗарплаты Тогда
		...
		РасшифровкаПлатежа.ЗаполнитьЗначения(Справочники.ДоговорыКонтрагентов.ПустаяСсылка(), "Договор");
	...
	ИначеЕсли Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОплатаПоКредитам
		...
		РасшифровкаПлатежа.ЗаполнитьЗначения(Объект.ДоговорКредитаДепозита, "Договор");
	...
	ИначеЕсли Объект.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ОплатаАрендодателю ТОГДА
		...
		РасшифровкаПлатежа.ЗаполнитьЗначения(Объект.ДоговорАренды, "Договор");
