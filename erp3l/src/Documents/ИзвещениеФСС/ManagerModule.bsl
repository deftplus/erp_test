...
Процедура ПослеРасшифровкиИзвещенияПрямыхВыплат(Страхователь, ИдентификаторСообщения, ТекстXML, Результат) Экспорт
	...
	Если ЭлементDOM = Неопределено Тогда
		ТекстОшибки = СтрШаблон(НСтр("ru = 'В xml-содержимом сообщения не удалось найти узел ""notice"". Текст XML: %1';
									|en = 'The ""notice"" node was not found in the xml content of the message. XML text: %1'"), ТекстXML);
	...
	Если РеквизитыВложения.content = Неопределено Тогда
		ТекстОшибки = СтрШаблон(НСтр("ru = 'В xml-содержимом сообщения не заполнен узел ""notice.attachment.content"". Текст XML: %1';
									|en = 'The ""notice.attachment.content"" node is not filled with the xml content of the message. XML text: %1'"), ТекстXML);
		СЭДОФСС.ОшибкаОбработки(Результат, ИдентификаторСообщения, ТекстОшибки);
		Возврат;
	КонецЕсли;
