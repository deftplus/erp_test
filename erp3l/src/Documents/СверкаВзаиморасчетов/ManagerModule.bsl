...
Функция ТекстЗапросаДанныеДокумента(НастройкиПечати)
	...
	ПредставлениеДоговораНаПечать = 
		"ВЫБОР
		|		КОГДА ТИПЗНАЧЕНИЯ(ДокументСверки.Договор) = ТИП(Справочник.ДоговорыАренды)
...
Процедура СвернутьРасчетыПоГруппировкам(ДанныеДокумента, ТаблицаВзаиморасчетов)
	...
	Для Каждого СтрокаРасчетов Из ТаблицаВзаиморасчетов Цикл
		...
		Если СтрокаРасчетов.ТипРасчетов = Перечисления.ТипыРасчетовСПартнерами.РасчетыСКлиентом
			...
			ГруппировкаДетальныхЗаписей = "ТипРасчетов, ОбъектРасчетов, Партнер, РасчетныйДокумент, ВалютаДокумента";
	...
	ДетальныеЗаписи.Свернуть("ТипРасчетов, ОбъектРасчетов, Партнер, РасчетныйДокумент, ОписаниеДокумента, ВалютаДокумента", "СуммаДокумента, СуммаДолг, СуммаДолгРегл, СуммаАванс, СуммаАвансРегл");
...
Функция ТекстыЗапросовДоступныхТаблиц()
	...
	Если ПравоДоступа("Просмотр", Метаданные.Документы.ВыкупВозвратнойТарыКлиентом) Тогда
		Результат.Вставить("#3", ТекстЗапросаВыкупВозвратнойТарыКлиентом()); //ДокументВыкупВозвратнойТарыКлиентом
	КонецЕсли;
	Если ПравоДоступа("Просмотр", Метаданные.Документы.ВыкупВозвратнойТарыУПоставщика) Тогда
		Результат.Вставить("#4", ТекстЗапросаВыкупВозвратнойТарыУПоставщика()); //ДокументВыкупВозвратнойТарыУПоставщика
	КонецЕсли;
	Если ПравоДоступа("Просмотр", Метаданные.Документы.ПервичныйДокумент) Тогда
		Результат.Вставить("#5", ТекстЗапросаПервичныйДокумент()); //ДокументПервичныйДокумент
	КонецЕсли;
	Если ПравоДоступа("Просмотр", Метаданные.Документы.СписаниеЗадолженности) Тогда
		Результат.Вставить("#6", ТекстЗапросаСписаниеЗадолженности()); //ДокументСписаниеЗадолженности
	КонецЕсли;
...
Функция УстаревшиеПредставленияДокументов()
	...
	ВидыДокументовКонтрагента.Вставить("АвансовыйОтчет",                        НСтр("ru = 'Приходный кассовый ордер';
																					|en = 'Cash receipt statement'", ОбщегоНазначения.КодОсновногоЯзыка()));
	ВидыДокументовКонтрагента.Вставить("АктВыполненныхРабот",                   НСтр("ru = 'Акт выполненных работ';
																					|en = 'Acceptance note'", ОбщегоНазначения.КодОсновногоЯзыка()));
	ВидыДокументовКонтрагента.Вставить("ВводОстатков",                          НСтр("ru = 'Корректировка задолженности';
																					|en = 'Debt adjustment'", ОбщегоНазначения.КодОсновногоЯзыка()));
	ВидыДокументовКонтрагента.Вставить("ВзаимозачетЗадолженности",              НСтр("ru = 'Взаимозачет задолженности';
																					|en = 'Debt offset'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ВидыДокументовКонтрагента.Вставить("ВозвратТоваровПоставщику",              НСтр("ru = 'Накладная (возврат)';
																					|en = 'Invoice (return)'", ОбщегоНазначения.КодОсновногоЯзыка()));
	ВидыДокументовКонтрагента.Вставить("ВыкупВозвратнойТарыКлиентом",           НСтр("ru = 'Выкуп возвратной тары';
																					|en = 'Redemption of reusable package'", ОбщегоНазначения.КодОсновногоЯзыка()));
	ВидыДокументовКонтрагента.Вставить("ВыкупВозвратнойТарыУПоставщика",        НСтр("ru = 'Выкуп возвратной тары';
																					|en = 'Redemption of reusable package'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ВидыДокументовКонтрагента.Вставить("ПриходныйКассовыйОрдер",                НСтр("ru = 'Приходный кассовый ордер';
																					|en = 'Cash receipt statement'", ОбщегоНазначения.КодОсновногоЯзыка()));
	ВидыДокументовКонтрагента.Вставить("РасходныйКассовыйОрдер",                НСтр("ru = 'Расходный кассовый ордер';
																					|en = 'Cash voucher'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ВидыДокументовКонтрагента.Вставить("СписаниеБезналичныхДенежныхСредств",    НСтр("ru = 'Платежное поручение';
																					|en = 'Payment order'", ОбщегоНазначения.КодОсновногоЯзыка()));
	ВидыДокументовКонтрагента.Вставить("СписаниеЗадолженности",                 НСтр("ru = 'Списание задолженности';
																					|en = 'Debt relief'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ВидыДокументовКонтрагента.Вставить("ЗаказПереработчику",                    НСтр("ru = 'Заказ';
																					|en = 'Order'", ОбщегоНазначения.КодОсновногоЯзыка()));
	ВидыДокументовКонтрагента.Вставить("ОтчетПереработчика",                    НСтр("ru = 'Отчет переработчика';
																					|en = 'Toller report'", ОбщегоНазначения.КодОсновногоЯзыка()));
	ВидыДокументовКонтрагента.Вставить("ПоступлениеДенежныхДокументов",         НСтр("ru = 'Приходный кассовый ордер';
																					|en = 'Cash receipt statement'", ОбщегоНазначения.КодОсновногоЯзыка()));
	ВидыДокументовКонтрагента.Вставить("ОтчетОператораСистемыПлатон",           НСтр("ru = 'Акт';
																					|en = 'Certificate'", ОбщегоНазначения.КодОсновногоЯзыка()));
