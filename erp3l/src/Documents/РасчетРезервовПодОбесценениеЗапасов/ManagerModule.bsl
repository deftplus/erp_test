		...
		Исключение
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Формирование резервов под обесценение запасов на конец периода %1 завершилось с ошибкой:
					|%2';
					|en = 'An error occurred when generating provisions for impairment of inventory at the end of period %1. Reason:
...
Процедура СоздатьКлючиАналитикиПоПартнерам(Организации, Период)
	...
	Запрос.Текст =
		...
		|		И (АналитикаПартнера.Контрагент = НЕОПРЕДЕЛЕНО)
		|		И (АналитикаПартнера.Договор = НЕОПРЕДЕЛЕНО)
		|		И
		|			(АналитикаПартнера.НаправлениеДеятельности = ЕСТЬNULL(Остатки.АналитикаУчетаНоменклатуры.Назначение.НаправлениеДеятельности,
