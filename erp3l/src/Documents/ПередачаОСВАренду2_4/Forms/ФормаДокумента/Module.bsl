...
Процедура НастроитьЗависимыеЭлементыФормыНаСервере(ИзмененныеРеквизиты = "")
	...
	Если СтруктураИзмененныхРеквизитов.Свойство("Организация")
		...
		Если СлужебныеПараметрыФормы.ИспользуетсяУчетАрендыПоФСБУ25_2018
			И Объект.РасчетыМеждуОрганизациямиАрендатор 
			И ЗначениеЗаполнено(Объект.Арендатор) Тогда
			Объект.Арендатор = Справочники.Контрагенты.ПустаяСсылка();
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьЗалоговуюСтоимостьНаСервере()
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.СтоимостьОС.Остатки(
	|				&Граница, 
	...
	|							ИЗ
	|								втТабличнаяЧасть КАК втТабличнаяЧасть)) КАК СтоимостьОС
	|		ПО втТабличнаяЧасть.ОсновноеСредство = СтоимостьОС.ОсновноеСредство";
