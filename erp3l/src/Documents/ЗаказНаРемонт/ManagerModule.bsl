...
Функция ДоступныеОстаткиПараметрыВстраивания() Экспорт
	...
	ПараметрыВстраивания.ОписаниеПолученияДанныхДокумента =
		...
		|			ЗНАЧЕНИЕ(Перечисление.СтатусыЗаказовНаРемонт.Закрыт)
		|		) КАК ГотовКОтгрузке,
		|	Реквизиты.ДатаНачала КАК ЖелаемаяДатаОтгрузки,
		...
		|	Реквизиты.ДатаНачала КАК ДатаОтгрузкиВсехСтрокОднойДатой
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("5f290a34-0d48-4fd1-a8d2-631516156ff3");
	...
	|en = 'Populates the ""Supply option"" attribute, ""Assigned"" flag.
	|Populates the new ""Priority"" attribute.'");
	...
	Изменяемые.Добавить(Метаданные.Документы.ЗаказНаРемонт.ПолноеИмя());
	Обработчик.ИзменяемыеОбъекты = СтрСоединить(Изменяемые, ",");
	
	Блокируемые = Новый Массив;
	Блокируемые.Добавить(Метаданные.Документы.ЗаказНаРемонт.ПолноеИмя());
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПараметрыВыборки.ПолныеИменаОбъектов = "Документ.ЗаказНаРемонт";
	...
	Запрос.Текст =
		...
		|ИЗ
		|	Документ.ЗаказНаРемонт КАК Реквизиты
		|ГДЕ
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ИмяДокумента = "Документ.ЗаказНаРемонт"; ИмяТЧ = "МатериалыИРаботы";
	...
	ТекстЗапроса =
		...
		|ИЗ
		|	Документ.ЗаказНаРемонт КАК Документы
		|		
		...
		Попытка
			...
			Если ДокументОбъект <> Неопределено Тогда
				...
				ОбеспечениеВДокументахСервер.ОбработатьНеиспользуемыеВариантыОбеспечения(
					ДокументОбъект.МатериалыИРаботы,
					ОбъектИзменен,
