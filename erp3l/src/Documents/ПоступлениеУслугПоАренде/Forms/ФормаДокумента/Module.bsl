...
Процедура ЗаполнитьНачисленияНаСервере()
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ЗачетОбеспечительногоПлатежа <> 0 Тогда
			...
			НоваяСтрока.СуммаСНДС = Выборка.ЗачетОбеспечительногоПлатежа;
			НоваяСтрока.СуммаНДС = Выборка.ЗачетОбеспечительногоПлатежаНДС;
			
			Если Объект.ЦенаВключаетНДС Тогда
				НоваяСтрока.Сумма = Выборка.ЗачетОбеспечительногоПлатежа;
			Иначе
				НоваяСтрока.Сумма = Выборка.ЗачетОбеспечительногоПлатежа - Выборка.ЗачетОбеспечительногоПлатежаНДС;
			КонецЕсли;
