...
Процедура ТекстЗапросаТаблицаПрочиеАктивыПассивы(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ТаблицаНачислений.СуммаСНДС * &КоэффициентПересчетаВВалютуУпр КАК Сумма
	|ИЗ
...
Процедура ТекстЗапросаТаблицаДвиженияПоПрочимАктивамПассивам(Запрос, ТекстыЗапроса, Регистры)
	...
	|
	|	ТаблицаРасходы.СуммаБезНДСРегл  КАК СуммаРегл,
...
Процедура ТекстЗапросаТаблицаВтОплатаОбеспечительногоПлатежа(ТекстыЗапроса)
	...
	|	ПО
	|		Начисления.РасчетныйДокумент = РасшифровкаСписаниеБезналичныхДС.Ссылка
	...
	|		И РасшифровкаСписаниеБезналичныхДС.ТипПлатежаПоАренде = ЗНАЧЕНИЕ(Перечисление.ТипыПлатежейПоАренде.ОбеспечительныйПлатеж)
	|	
	|ГДЕ
	...
	|	ПО
	|		Начисления.РасчетныйДокумент = РасшифровкаРасходныйКассовыйОрдер.Ссылка
	|		И РасшифровкаРасходныйКассовыйОрдер.ДоговорАренды = &Договор
	|		И РасшифровкаРасходныйКассовыйОрдер.ТипПлатежаПоАренде = ЗНАЧЕНИЕ(Перечисление.ТипыПлатежейПоАренде.ОбеспечительныйПлатеж)
	|	
	|ГДЕ
	|	Начисления.Ссылка = &Ссылка
	|	И Начисления.ТипНачисления = ЗНАЧЕНИЕ(Перечисление.ТипыПлатежейПоАренде.ОбеспечительныйПлатеж)
...
Процедура ТекстЗапросаТаблицаВтСуммыДокументовВВалютахУчета(ТекстыЗапроса)
	...
	|		КОГДА Начисления.ТипНачисления = ЗНАЧЕНИЕ(Перечисление.ТипыПлатежейПоАренде.ОбеспечительныйПлатеж)
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Версия = "2.5.4.9";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("bed289c7-3e54-4cae-aa8e-e8a2c93be650");
	...
	Обработчик.Комментарий = НСтр("ru = 'Обновляет документы ""Поступление услуг по аренде""
	|- Заполняет реквизит ""Ставка НДС""
	|- Уменьшает длину номера на один символ';
	|en = 'Updates the ""Receipt of lease services"" documents
	|- Populates the ""VAT rate"" attribute
	|- Decreases the length of the number by one character'");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПараметрыВыборки.ПолныеИменаОбъектов = "Документ.ПоступлениеУслугПоАренде";
	...
	|				ИСТИНА
	|			ИЗ
	|				Документ.ПоступлениеУслугПоАренде.Начисления КАК ПоступлениеУслугПоАрендеНачисления
	|			ГДЕ
	|				ПоступлениеУслугПоАренде.Ссылка = ПоступлениеУслугПоАрендеНачисления.Ссылка
	|				И ПоступлениеУслугПоАрендеНачисления.УдалитьСтавкаНДС <> &ПустаяСтавкаНДС
	|				И ПоступлениеУслугПоАрендеНачисления.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка))
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	Документ.ПоступлениеУслугПоАренде КАК ПоступлениеУслугПоАренде
	|ГДЕ
	|	ПоступлениеУслугПоАренде.Номер ПОДОБНО ""___________[^ ]""";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПолноеИмяОбъекта = "Документ.ПоступлениеУслугПоАренде";
	
	ОбновляемыеДанные = ОбновлениеИнформационнойБазы.ДанныеДляОбновленияВМногопоточномОбработчике(Параметры);
	
	ПрефиксыДокументаПоОрганизации = Новый Соответствие;
	...
	Для Каждого Документ Из ОбновляемыеДанные Цикл
		...
		Попытка
			...
			Если ДокументОбъект <> Неопределено Тогда
				...
				СократитьНомерДокумента(ДокументОбъект, ПрефиксыДокументаПоОрганизации);
...
Процедура СократитьНомерДокумента(ДокументОбъект, ПрефиксыДокументаПоОрганизации)

	НоваяДлинаНомера = 11;
	
	Если СтрДлина(ДокументОбъект.Номер) <= НоваяДлинаНомера Тогда
		Возврат;
	КонецЕсли;
	
	ПрефиксДокумента = ПрефиксыДокументаПоОрганизации.Получить(ДокументОбъект.Организация);
	Если ПрефиксДокумента = Неопределено Тогда
		ПрефиксацияОбъектовСобытия.УстановитьПрефиксИнформационнойБазыИОрганизации(ДокументОбъект, Истина, ПрефиксДокумента);
		ПрефиксыДокументаПоОрганизации.Вставить(ДокументОбъект.Организация, ПрефиксДокумента);
	КонецЕсли;
	
	НовыйНомер = НовыйНомерДокумента(ДокументОбъект, ПрефиксДокумента);
	
	Если ДокументОбъект.Номер = НовыйНомер Тогда
		Возврат;
	КонецЕсли;
		
	Блокировка = Новый БлокировкаДанных;
	
	ЭлементБлокировки = Блокировка.Добавить("Документ.ПоступлениеУслугПоАренде");
	ЭлементБлокировки.УстановитьЗначение("Номер", НовыйНомер);
	ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
	...
	|	Документ.ПоступлениеУслугПоАренде КАК ПоступлениеУслугПоАренде
	|ГДЕ
	|	ПоступлениеУслугПоАренде.Номер = &Номер
	|	И НАЧАЛОПЕРИОДА(ПоступлениеУслугПоАренде.Дата, ГОД) = &НачалоГода
	|	И ПоступлениеУслугПоАренде.Ссылка <> &Ссылка";
...
Функция НовыйНомерДокумента(ДокументОбъект, ПрефиксДокумента)

	СтараяДлинаНомера = 12;
	НоваяДлинаНомера = 11;
	
	Если СтрДлина(СокрЛП(ДокументОбъект.Номер)) <= НоваяДлинаНомера Тогда
		Возврат ДокументОбъект.Номер;
	КонецЕсли;

	ДлинаПрефикса = СтрДлина(ПрефиксДокумента);
	
	НовыйНомер = ПрефиксДокумента + Прав(СокрЛП(ДокументОбъект.Номер), НоваяДлинаНомера - ДлинаПрефикса);
	
	НовыйНомер = СтроковыеФункцииКлиентСервер.ДополнитьСтроку(НовыйНомер, СтараяДлинаНомера, " ", "Справа");
