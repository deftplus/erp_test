...
Функция ДанныеДляПроведения(РеквизитыДляПроведения, СтруктураВидовУчета = Неопределено) Экспорт 
	...
	Если СтруктураВидовУчета.ОстальныеВидыУчета Тогда
		...
		Запрос.Текст =
			...
			|			И (НачисленияОтпуска.ДатаНачала МЕЖДУ ОтпускДополнительныеОтпуска.ДатаНачала И ОтпускДополнительныеОтпуска.ДатаОкончания)
			|ГДЕ
			|	ОтпускДополнительныеОтпуска.Ссылка = &Ссылка
		...
		Пока Выборка.Следующий() Цикл
			...
			Если РеквизитыДляПроведения.ПериодыПриостановкиОтпуска.Количество() > 0 Тогда
				...
				ПериодОтпуска.Вставить("ДатаНачалаПериода", Выборка.ДатаНачалаПериодаОтсутствия);
				ПериодОтпуска.Вставить("ДатаОкончанияПериода", Выборка.ДатаОкончанияПериодаОтсутствия);
				ПериодыОтпускаСУчетомПриостановок = ИсключитьИнтервалыИзПериодов(ПериодОтпуска, РеквизитыДляПроведения.ПериодыПриостановкиОтпуска);
				Если ПериодыОтпускаСУчетомПриостановок.Количество() > 0 Тогда
					ОтпускВРабочихДняхПоДоговору = ОстаткиОтпусков.ОтпускСотрудникаВРабочихДняхПоДоговору(РеквизитыДляПроведения.Сотрудник, РеквизитыДляПроведения.ДатаНачалаСобытия);
					...
					КонецЦикла;
				КонецЕсли;
				КоличествоДнейОтпуска = КоличествоДнейОтпускаСУчетомПриостановок;
			КонецЕсли;

			НоваяСтрока = ДанныеРеестраОтпусков.Добавить();
			
			ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка);
			НоваяСтрока.КоличествоДнейОтпуска = КоличествоДнейОтпуска;
