...
Функция ПредставлениеОбъектаНаОсновномЯзыке() Экспорт
	...
	ПредставлениеОбъекта = НСтр(
		...
		|en = 'Order for materials for production'",
		КодОсновногоЯзыка);
		...
		Пока ВыборкаЗаказы.Следующий() Цикл
			...
			ИначеЕсли ВыборкаЗаказы.ЕстьОшибкиСтатус Тогда
				...
				ТекстОшибки = НСтр("ru = 'Документ %Документ% находится в статусе ""%Статус%"". Ввод на основании разрешен только в статусах ""К выполнению"", ""Закрыт"".';
									|en = 'Document %Документ% is in the ""%Статус%"" status.  Generation is only allowed from base documents in ""For release"" or ""Closed"" statuses.'");
				ТекстОшибки = СтрЗаменить(ТекстОшибки, "%Документ%", ВыборкаЗаказы.Заказ);
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	Если ЗначениеЗаполнено(Реквизиты.Подразделение) Тогда
		ИнформацияПоПодразделению = НСтр("ru = 'Передача в ""%Подразделение%""';
										|en = 'Transfer to ""%Подразделение%""'", ОбщегоНазначения.КодОсновногоЯзыка());
	...
	Если ЗначениеЗаполнено(Реквизиты.Склад) Тогда
		ИнформацияПоСкладу = НСтр("ru = 'Передача с ""%Склад%""';
									|en = 'Transfer from ""%Склад%""'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Запрос.Текст =
		...
		|ИЗ
		|	Документ.ЗаказМатериаловВПроизводство КАК Реквизиты
		|ГДЕ
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ИмяДокумента = "Документ.ЗаказМатериаловВПроизводство"; ИмяТЧ = "Товары";
	...
	ТекстЗапроса =
		...
		|ИЗ
		|	Документ.ЗаказМатериаловВПроизводство КАК Документы
		|		
