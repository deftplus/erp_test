...
Процедура ЗаполнитьОплатыПоставщикам(ОтборРасчетов, ОплатыПоставщику) Экспорт
	...
	СтавкаНДС = ?(ЗначениеЗаполнено(ОтборРасчетов.СтавкаНДС),
		...
		УчетНДСУП.СтавкаНДСПоУмолчанию(НачалоПериода, Истина));
...
Функция ТекстЗапросаТаблицаНДСПредъявленный(Запрос, ТекстыЗапроса, Регистры)
	...
	|	НачислениеНДС.НаправлениеДеятельности  КАК НаправлениеДеятельности,
	|	0                                      КАК СуммаБезНДС,
...
Функция ТекстЗапросаТаблицаВтИсходныеПрочиеРасходы(Запрос, ТекстыЗапроса)
	...
	|ИЗ
	|	ВтПартииПрочихРасходов КАК ТаблицаРасходов
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	ТаблицаРасходов.НаправлениеДеятельности,
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет реквизит ""Ставка НДС"", а так же служебные реквизиты документа ""Счет-фактура налогового агента"".';
									|en = 'Populates the attribute ""VAT rate"" and the internal attributes of the document ""Tax invoice of the tax agent"".'");
