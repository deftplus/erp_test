...
Функция ПараметрыУказанияСерий(Объект) Экспорт
	...
	ХозОперация = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.ДокументОснование, "ХозяйственнаяОперация");
	...
	ХозОперацияТоварыВПутиИНеотфактуровка = ОперацииВПути.Найти(ХозОперация) <> Неопределено
		Или ОперацииНеотфактуровки.Найти(ХозОперация) <> Неопределено;
	ЭтоТоварыВПути = ОперацииВПути.Найти(ХозОперация) <> Неопределено;
...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	|			ТОГДА ВЫБОР
	|				КОГДА ЕСТЬNULL(ДанныеДокумента.ДокументОснование.ХозяйственнаяОперация, НЕОПРЕДЕЛЕНО) В(ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаУПоставщикаТоварыВПути),
...
Функция ТекстЗапросаТаблицаСебестоимостьТоваров(Запрос, ТекстыЗапроса, Регистры) Экспорт
	...
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ПроизводственныеЗатраты)
	|		КОГДА ТаблицаРасхождения.ТипЗапасов = ЗНАЧЕНИЕ(Перечисление.ТипыЗапасов.ТоварНаХраненииСПравомПродажи)
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыНаХраненииСПравомПродажи)
	|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыНаСкладах)
	|	КОНЕЦ КАК РазделУчета,
	|	ТаблицаРасхождения.ВидЗапасов КАК ВидЗапасов,
	...
	|	И НЕ ТаблицаРасхождения.СписатьНаРасходы
	|	И (ТаблицаРасхождения.Назначение = ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка) ИЛИ
	|		ТаблицаРасхождения.Назначение.ТипНазначения <> ЗНАЧЕНИЕ(Перечисление.ТипыНазначений.ПоставкаПодПринципала))
	|УПОРЯДОЧИТЬ ПО
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	//++ НЕ УТ
	|			ИЛИ КорректировкаПриобретенияТовары.УдалитьСчетУчета <> &СчетУчетаПустаяСсылка
	|				И КорректировкаПриобретенияТовары.НастройкаСчетовУчета = ЗНАЧЕНИЕ(Справочник.НастройкиСчетовУчетаПрочихОпераций.ПустаяСсылка)
	//-- НЕ УТ
	|			))
	|	ИЛИ ИСТИНА В
	|		(ВЫБРАТЬ ПЕРВЫЕ 1
	...
	//++ НЕ УТ
	|			ИЛИ КорректировкаПриобретенияРасхождения.УдалитьСчетУчета <> &СчетУчетаПустаяСсылка
	|				И КорректировкаПриобретенияРасхождения.НастройкаСчетовУчета = ЗНАЧЕНИЕ(Справочник.НастройкиСчетовУчетаПрочихОпераций.ПустаяСсылка)
	//-- НЕ УТ
	|			))
	|	ИЛИ ИСТИНА В
	|		(ВЫБРАТЬ ПЕРВЫЕ 1
	...
	//++ НЕ УТ
	|			ИЛИ (КорректировкаПриобретения.УдалитьСчетУчетаАктивов <> &СчетУчетаПустаяСсылка И
	|				КорректировкаПриобретения.НастройкаСчетовУчетаАктивов = ЗНАЧЕНИЕ(Справочник.НастройкиСчетовУчетаПрочихОпераций.ПустаяСсылка))
	|			ИЛИ (КорректировкаПриобретения.УдалитьСчетУчетаПассивов <> &СчетУчетаПустаяСсылка И
	|				КорректировкаПриобретения.НастройкаСчетовУчетаПассивов = ЗНАЧЕНИЕ(Справочник.НастройкиСчетовУчетаПрочихОпераций.ПустаяСсылка))
	//-- НЕ УТ
	|";
	...
	Для Каждого Документ Из ОбновляемыеДанные Цикл
		...
		Попытка
			...
			Если ДокументОбъект <> Неопределено Тогда
				...
				НастройкаСчетовУчетаСервер.ЗаполнитьНастройкиСчетовУчета(ДокументОбъект, "Активов");
				НастройкаСчетовУчетаСервер.ЗаполнитьНастройкиСчетовУчета(ДокументОбъект, "Пассивов");
				//-- НЕ УТ
			КонецЕсли;
