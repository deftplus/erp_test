...
Процедура ЗаполнитьПоОстаткамЗаказов(ДанныеОтбора, Товары, СкладПередачи, МассивЗаказов, ПараметрыЗаполнения) Экспорт
	...
	Пока ВыборкаТовары.Следующий() Цикл
		...
		Если ПересчитатьСуммы Тогда
			...
			СтрокаТаб.СуммаСНДС = СуммаОстаток;
		КонецЕсли;
		...
		Если ЗаказНайден
			...
			Если КурсВалюты.Количество() = 1 Тогда
				...
				СтрокаТаб.СуммаСНДС	= РаботаСКурсамиВалютУТ.ПересчитатьПоКурсу(СтрокаТаб.СуммаСНДС,
																						ПараметрыТекущегоКурса,
																						ПараметрыНовогоКурса);
			КонецЕсли;
		...
		Если ЗаказНайден
			...
			Ценообразование.ПересчитатьСуммыВСтрокеПоСуммеСНДС(СтрокаТаб,
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Представление = НСтр("ru = 'Передача товаров хранителю';
										|en = 'Transfer of goods to bailee'");
	КомандаПечати.ПроверкаПроведенияПередПечатью = Истина;
		...
		Пока ДанныеПечати.Следующий() Цикл
			...
			Пока ВыборкаПоСкладам.Следующий() Цикл
				...
				ПредставлениеРаспоряжения = ОбщегоНазначенияУТКлиентСервер.СформироватьЗаголовокДокумента(
					РеквизитыДокумента,
					НСтр("ru = 'Передача товаров хранителю';
						|en = 'Transfer of goods to bailee'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Функция СформироватьПечатнуюФормуНакладная(МассивОбъектов, ОбъектыПечати)
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		ШапкаТаблицы = ?(ПоказыватьНДС, "ШапкаТаблицыСНДС", "ШапкаТаблицы");
		ПостфиксТовар = ?(ПоказыватьНДС, "ТоварСНДС", "Товар");
		Данные = ?(ПоказыватьНДС, "ДанныеСНДС", "Данные");
		...
		СтрокаТаблицы = ?(ПоказыватьНДС, "СтрокаТаблицыСНДС", "СтрокаТаблицы");
		...
		Если ОбщегоНазначенияУТКлиентСервер.ЕстьРеквизитОбъекта(ВыборкаПоДокументам, "ЭтоНабор") Тогда
			...
			СтрокаТаблицыНабор = ?(ПоказыватьНДС, "СтрокаТаблицыСНДСНабор", "СтрокаТаблицыНабор");
			СтрокаТаблицыКомплектующие = ?(ПоказыватьНДС, "СтрокаТаблицыСНДСКомплектующие",
											"СтрокаТаблицыКомплектующие");
		...
		// Выводим подвал
		ПодвалТаблицы = ?(ПоказыватьНДС, "ПодвалТаблицыСНДС", "ПодвалТаблицы");
...
Функция ПолучитьРезультатЗапросаПоОстаткамЗаказов(ДанныеОтбора, СкладПередачи = Неопределено, МассивЗаказов = Неопределено)
	...
	|	ТоварыПередачи.Цена                 КАК Цена,
	|	ТоварыПередачи.Сумма                КАК Сумма,
	|	ТоварыПередачи.СуммаСНДС            КАК СуммаОстаток,
	|	ТоварыПередачи.СтавкаНДС            КАК СтавкаНДС,
	|	ТоварыПередачи.СуммаНДС             КАК СуммаНДС,
	|	ТоварыПередачи.СуммаСНДС            КАК СуммаСНДС,
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	ДелоРодитель.Представление = НСтр("ru = 'Документы передачи товаров хранителю';
										|en = 'Documents of goods transfer to bailee'");
