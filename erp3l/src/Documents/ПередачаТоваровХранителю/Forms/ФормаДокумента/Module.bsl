...
Процедура СоставНабора(Команда)
	...
	Если Не ЗначениеЗаполнено(ТекущаяСтрока.НоменклатураНабора) Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Номенклатура выбранной строки не входит в состав набора.';
									|en = 'Items in the selected line are not included in set.'"));		
		Возврат;
	КонецЕсли;
