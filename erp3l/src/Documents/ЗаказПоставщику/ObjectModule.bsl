...
Функция ПроверитьЗавершениеРасчетов() Экспорт
	...
	|en = 'There is an outstanding payment due for this order: %СуммаКОплате% %Валюта%.
...
Процедура ЗаполнитьДокументНаОснованииДоговора(Знач СправочникОснование)
	...
	ЗаполнитьЗначенияСвойств(ЭтотОбъект, Выборка);
	#Область УХ_Встраивание
	Соглашение = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Выборка.Договор,"Соглашение");
	#КонецОбласти
...
Процедура ЗаполнитьДокументНаОснованииСделкиПоОбособленныемЗаказам(Знач СправочникОснование)
	...
	|		ВЫБОР
	|			КОГДА НЕ ТоварыЗаказа.Ссылка ЕСТЬ NULL ТОГДА
	|				ТоварыЗаказа.ВариантОбеспечения = ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.Требуется)
	|					И ТоварыЗаказа.Обособленно
	|			ИНАЧЕ
	|				ТоварыЗаявки.ВариантОбеспечения = ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.Требуется)
