...
Функция ТекстЗапросаТаблицаВтПрочиеАктивыПассивы(Запрос, ТекстыЗапроса)
	...
	|	(ВЫБОР
	|		КОГДА ЕСТЬNULL(Суммы.СуммаПриведеннаяУпр, 0) <> 0
	|			ТОГДА Суммы.СуммаПриведеннаяУпр + Суммы.СуммаСНДСУпр
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	//++ НЕ УТ
	|					ИЛИ ПриобретениеУслугПрочихАктивовРасходы.УдалитьСчетУчета <> &СчетУчетаПустаяСсылка
	|						И ПриобретениеУслугПрочихАктивовРасходы.НастройкаСчетовУчета = ЗНАЧЕНИЕ(Справочник.НастройкиСчетовУчетаПрочихОпераций.ПустаяСсылка)
	//-- НЕ УТ					
	|				))
