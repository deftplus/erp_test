	...
	Если ОбязательныеСтроки.Количество() > 0 Тогда
		Если ОбязательныеСтроки.Количество() = 1 Тогда
			НачалоТекстаПредупреждения = СтрШаблон(
				НСтр("ru = 'По требованию %1 отчет ""%2"" представляется обязательно.';
					|en = 'On %1 demand the ""%2"" report submission is mandatory.'"),
				Объект.Банк,
		...
		ИначеЕсли ОбязательныеСтроки.Количество() = 2 Тогда
			НачалоТекстаПредупреждения = СтрШаблон(
				НСтр("ru = 'По требованию %1 отчеты ""%2"" и ""%3"" представляются обязательно.';
					|en = 'On %1 demand submission of reports ""%2"" and ""%3"" is mandatory.'"),
				Объект.Банк,
				ПолноеПредставлениеОтчета(ОбязательныеСтроки[0]),
		...
		Иначе
			...
			НачалоТекстаПредупреждения = СтрШаблон(
				НСтр("ru = 'По требованию %1 отчет ""%2"" и еще %3 представляются обязательно.';
					|en = 'On %1 demand submission of the ""%2"" report and %3 is mandatory.'"),
				Объект.Банк,
				ПолноеПредставлениеОтчета(ОбязательныеСтроки[0]),
				СтрокаПрописью);
		КонецЕсли;
		...
		ТекстПредупреждения = СтрШаблон(
			НСтр("ru = '%1
						...
						|en = '%1
						|
						|Select ""No data"" in the context menu of Fill in/Select hyperlink if the requested reports are not applicable for %2.'"),
...
&НаКлиенте
Процедура ВыполнитьМассовуюОперацию(ИмяОперации, ДополнительныеПараметры = Неопределено) Экспорт
...
Процедура ВыполнитьМассовуюОперацию_ПослеЗаписи(ИмяОперации, ДополнительныеПараметры)
	...
	ИначеЕсли ИмяОперации = "ДанныеОтсутствуют" Тогда
		
		УстановитьДанныеОтсутствуютВГруппе(ДополнительныеПараметры.ИдентификаторыОтчетов);
...
&НаСервере
Процедура ОчиститьДанныеСтрокДерева(ИдентификаторыОтчетов)
	
	Для каждого ИдентификаторОтчета Из ИдентификаторыОтчетов Цикл
		ОчиститьДанныеСтрокиДерева(ИдентификаторОтчета, Истина);
	КонецЦикла;
...
Функция КоличествоОтчетовВГруппе(ЭлементыДереваОтчетов)
	...
	Для каждого ЭлементДерева Из ЭлементыДереваОтчетов Цикл
		Если ЗначениеЗаполнено(ЭлементДерева.КлючСтрокиОтчета) Тогда
			...
			Если ЭлементДерева.ОтчетЗаполнен ИЛИ ЭлементДерева.ДанныеОтсутствуют Тогда
...
Процедура ОбработатьДействиеЗаполнитьГруппу_ПослеВыбораВарианта(ВыбранныйЭлемент, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ВыбранныйЭлемент.Значение = "ДанныеОтсутствуют" Тогда
		...
		ИдентификаторыОтчетов = ИдентификаторыОтчетовВГруппе(СтрокаДереваОтчетов.ПолучитьЭлементы());
		
		ДополнительныеПараметры = Новый Структура();
		ДополнительныеПараметры.Вставить("ИдентификаторыОтчетов", ИдентификаторыОтчетов);
		
		ВыполнитьМассовуюОперацию("ДанныеОтсутствуют", ДополнительныеПараметры);
...
&НаКлиенте
Процедура УстановитьДанныеОтсутствуютВГруппе(ИдентификаторыОтчетов)
	
	ОчиститьДанныеСтрокДерева(ИдентификаторыОтчетов);
...
Процедура ОбработатьДействиеЗаполнитьСтандартныйОтчет_ПослеВыбораВарианта(ВыбранныйЭлемент, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ВыбранныйЭлемент.Значение = "ДанныеОтсутствуют" Тогда
		...
		Если НеПредлагатьУстановкуДанныеОтсутствуютДляАналогичныхОтчетов Тогда
			ОчиститьДанныеСтрокиДерева(ДополнительныеПараметры.ИдентификаторСтроки, Истина);
		Иначе
			ПоказатьВопросУстановитьДанныеОтсутствуют(СтрокаДереваОтчетов);
		КонецЕсли;
...
&НаКлиенте
Процедура ПоказатьВопросУстановитьДанныеОтсутствуют(СтрокаДереваОтчетов)
	
	ИдентификаторыАналогичныхОтчетов = ИдентификаторыНезаполненныхОтчетовСУчетомПериода(
	...
	Если ИдентификаторыАналогичныхОтчетов.Количество() > 1 Тогда
		...
		ТекстВопроса = СтрШаблон(НСтр("ru = 'Установить признак ""Данные отсуствуют"" для всех отчетов
			|за %1?';
			|en = 'Set ""No data"" for all reports
			|for %1?'"), ПредставлениеПериода);
		
		СписокКнопок = Новый СписокЗначений;
		СписокКнопок.Добавить(КодВозвратаДиалога.Да, НСтр("ru = 'Установить для всех';
															|en = 'Set for all'"));
		СписокКнопок.Добавить(КодВозвратаДиалога.Нет, НСтр("ru = 'Только для текущего';
															|en = 'Only for the current one'"));
		СписокКнопок.Добавить(КодВозвратаДиалога.Отмена, НСтр("ru = 'Отмена';
																|en = 'Cancel'"));
		
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, СписокКнопок);
	Иначе
		ОчиститьДанныеСтрокиДерева(СтрокаДереваОтчетов.ПолучитьИдентификатор(), Истина);
	КонецЕсли;
...
Процедура УстановитьДанныеОтсутствуютОбработкаОтвета(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		ВыполнитьМассовуюОперацию_ПослеЗаписи("ДанныеОтсутствуют", ДополнительныеПараметры);
	ИначеЕсли РезультатВопроса = КодВозвратаДиалога.Нет Тогда
		ОчиститьДанныеСтрокиДерева(ДополнительныеПараметры.СтрокаДереваОтчетов.ПолучитьИдентификатор(), Истина);
...
Процедура ОткрытьДиалогВыбораПрисоединенногоФайла(СтрокаДереваОтчетов = Неопределено)
	...
	ПараметрыФормы = Новый Структура();
	ПараметрыФормы.Вставить("Заголовок", СтрШаблон(НСтр("ru = 'Выберите %1';
														|en = 'Select %1'"), ЗаголовокДиалога));
...
&НаКлиенте
Функция ИдентификаторыНезаполненныхОтчетовСУчетомПериода(ЭлементыДереваОтчетов, ДатаНачала, ДатаОкончания)
	...
	Для каждого ЭлементДерева Из ЭлементыДереваОтчетов Цикл
		Если ЗначениеЗаполнено(ЭлементДерева.КлючСтрокиОтчета) Тогда
			// Это отчет
			Если НЕ ЭлементДерева.ОтчетЗаполнен
		...
		Иначе
			// Это группа - надо собрать вложенные отчеты
			ЭлементыГруппы = ЭлементДерева.ПолучитьЭлементы();
			РезультатПоГруппе = ИдентификаторыНезаполненныхОтчетовСУчетомПериода(ЭлементыГруппы, ДатаНачала, ДатаОкончания);
			Для каждого ЭлементРезультата Из РезультатПоГруппе Цикл
				Результат.Добавить(ЭлементРезультата);
			КонецЦикла;
		КонецЕсли;
...
Процедура ПодписатьФинОтчет(ДополнительныеПараметры)
	...
	Если Не ЗначениеЗаполнено(ПараметрыОтбора.Организация) Тогда
		ФинОтчетностьВБанкиКлиент.
			ПоказатьОшибку(НСтр("ru = 'Субъект, подписывающий отчетность, не определен';
	...
	Если РезультатПоиска.Количество() > 0 Тогда
		...
		Если Не РезультатПоискаСертификата.СертификатНайден Тогда
			ПоказатьПредупреждение(, Новый ФорматированнаяСтрока(
				...
				|%1';
				|en = 'Electronic signature is not found
				...
				|en = '
				|
				|Sign with available electronic signature
				|'"),
				Новый ФорматированнаяСтрока(НСтр("ru = 'по инструкции';
												|en = 'according to the instruction'"),,,,
	...
	ПараметрыКриптографии.Вставить("Заголовок",
		СтрШаблон(НСтр("ru = 'Сертификат для %1';
