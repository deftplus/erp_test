...
Процедура РаспределитьСуммуПоУчастникам(Объект) Экспорт
	...
	Результат = РаспределениеВидовРаботПоСотрудникам(Объект, Ложь);
...
Процедура УстановитьПараметрЗапросаТаблицаВидыРаботСотрудников(Запрос)
	...
	Если Запрос.Параметры.ВидНаряда = Перечисления.ВидыБригадныхНарядов.Персональный
		...
		Запрос.УстановитьПараметр("ТаблицаВидыРаботСотрудников", НоваяТаблицаВидовРаботСотрудников());
		Возврат;
	КонецЕсли;
	
	Запрос.УстановитьПараметр("ТаблицаВидыРаботСотрудников", РаспределениеВидовРаботПоСотрудникам(Запрос.Параметры.Ссылка));
...
//
Функция РаспределениеВидовРаботПоСотрудникам(Объект, РаспределятьКоличество = Истина, РаспределятьСумму = Истина)
	...
	|	ТаблицаДокумента.СтатьяКалькуляции     КАК СтатьяКалькуляции,
	|	ТаблицаДокумента.ГруппаПродукции       КАК ГруппаПродукции,
	|	ТаблицаДокумента.КоличествоКОплате     КАК Количество,
	...
	ВидыРабот = Результаты[3].Выбрать();
	Работники = Результаты[4].Выгрузить();
	
	Коэффициенты = Работники.ВыгрузитьКолонку("Коэффициент");
	Результат = НоваяТаблицаВидовРаботСотрудников();
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполнение нового реквизита ""Заявлено"" табличной части ""Виды работ"" документа ""Выработка сотрудников""';
									|en = 'Populating the new attribute ""Declared"" of the table ""Activity kinds"" of the ""Timesheets charge"" document'");
