...
Функция ОписаниеОперацийУчетаСебестоимости(Документ) Экспорт
	...
	#Область ВнутреннееПоступление_Работы
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Дело.Представление  = НСтр("ru = 'Отчеты переработчиков к оформлению';
								|en = 'Subcontractor reports for registration'");
...
#Область ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет реквизиты ""Ставка НДС"", ""Объект расчетов"", ""Идентификатор строки"", ""Настройки счетов учета"", признак оплаты в иностранной валюте.';
									|en = 'Populates the ""VAT rate"", ""AR/AP object"", ""Line ID"", ""Ledger accounts settings"" attributes, a flag of payment in foreign currency.'");
...
#Область СгенерироватьОбъектыРасчетов
	...
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("dd0f1d54-7380-43b1-96bd-23a763a7a928");
	Обработчик.Многопоточный = Истина;
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "Документы.ОтчетПереработчика.ЗарегистрироватьДанныеДляГенерацииОбъектовРасчетов";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	ИЛИ НЕ ОтчетПереработчика.ОплатаВВалюте
	|			И (ОтчетПереработчика.УдалитьПорядокОплаты = ЗНАЧЕНИЕ(Перечисление.УдалитьПорядокОплатыПоСоглашениям.РасчетыВВалютеОплатаВВалюте)
	|				ИЛИ ОтчетПереработчика.УдалитьПорядокОплаты = ЗНАЧЕНИЕ(Перечисление.УдалитьПорядокОплатыПоСоглашениям.РасчетыВРубляхОплатаВВалюте))
	|	ИЛИ (ИСТИНА В
	|			(ВЫБРАТЬ ПЕРВЫЕ 1
	|				ИСТИНА
	|			ИЗ
	...
	|			ИСТИНА
	|		ИЗ
	|			Документ.ОтчетПереработчика.ВидыЗапасов КАК ТабЧасть
	|		ГДЕ
	|			ТабЧасть.Ссылка = ОтчетПереработчика.Ссылка
	|			И ТабЧасть.ИдентификаторСтроки = """")
	|	ИЛИ ИСТИНА В
	|		(ВЫБРАТЬ ПЕРВЫЕ 1
	|			ИСТИНА
	|		ИЗ
	|			Документ.ОтчетПереработчика.Продукция КАК ТабЧастьПродукция
	|		ГДЕ
	|			ТабЧастьПродукция.Ссылка = ОтчетПереработчика.Ссылка
	|			И ТабЧастьПродукция.УдалитьСчетУчета <> &СчетУчетаПустаяСсылка
	|			И ТабЧастьПродукция.НастройкаСчетовУчета = ЗНАЧЕНИЕ(Справочник.НастройкиСчетовУчетаПрочихОпераций.ПустаяСсылка))
	|	ИЛИ ИСТИНА В
	|		(ВЫБРАТЬ ПЕРВЫЕ 1
	|			ИСТИНА
	|		ИЗ
	|			Документ.ОтчетПереработчика.ВозвратныеОтходы КАК ТабЧастьВозвратныеОтходы
	|		ГДЕ
	|			ТабЧастьВозвратныеОтходы.Ссылка = ОтчетПереработчика.Ссылка
	|			И ТабЧастьВозвратныеОтходы.УдалитьСчетУчета <> &СчетУчетаПустаяСсылка
	|			И ТабЧастьВозвратныеОтходы.НастройкаСчетовУчета = ЗНАЧЕНИЕ(Справочник.НастройкиСчетовУчетаПрочихОпераций.ПустаяСсылка))
	|";
