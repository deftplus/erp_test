...
#Область ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	|en = 'Populates CCD number in the returns of goods from customer in the Goods items table. If several CCD numbers from the Inventory owner attributes table match the single string from the Goods items table, it splits out strings in the table Goods items.
	|Populates the VAT rate attribute, Foreign currency transaction and AR/AP object attribute the same way.
	|You cannot edit Returns of goods from customer documents until the handler execution is completed.'");
...
#Область ОбработатьДополнительныеРеквизитыДляПереходаНаНовуюВерсию
	...
	|До окончания выполнения обработчика редактирование документов Возвраты товаров от клиента будет ограничено.';
	|en = 'Restores data from ""Delete additional attributes"" table.
	|Editing Goods return from client documents will be limited until the handler is completed.'");
