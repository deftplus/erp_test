...
Процедура ФормаЗаполненияПровестиЗакрыть(Команда)
	...
	ПоказатьВопрос(ОбработчикОповещенияОЗакрытии,
	                 ...
	                 |Продолжить?';
	                 |en = 'You will not be able to edit your responses after you submit the questionnaire.
...
Функция ПроверитьЗаполнениеАнкеты()
	...
	Пока Выборка.Следующий() Цикл
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Не указан ответ на вопрос';
													|en = 'Reply to the question is not specified.'") + "- "+ СтрЗаменить(Выборка.Ссылка.ПолныйКод(),"/",".") + " " + Выборка.Формулировка);
	КонецЦикла;
