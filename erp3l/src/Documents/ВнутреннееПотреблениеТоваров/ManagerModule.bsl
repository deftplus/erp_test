...
Процедура ЗаполнитьСерииПоОтгрузке(Объект) Экспорт
	...
	|ИЗ
	|	Документ.ВнутреннееПотреблениеТоваров.Товары КАК ПотребленияТоваров
	|ГДЕ
	|	ПотребленияТоваров.Ссылка = &Ссылка
	|	И ПотребленияТоваров.ЗаказНаВнутреннееПотребление <> &РаспоряжениеПоУмолчанию
	|;
	|
	|//////////////////////////////////////////////////////////////////////////////// 2
	|ВЫБРАТЬ
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	Дело.Представление  = НСтр("ru = 'Внутренние потребления товаров к оформлению';
								|en = 'Inventory consumption to register'");
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	|en = 'Updates the ""Inventory consumption — Non-operational activities"" documents:
	|- Fills in the ""Fin. record ID"" attribute of the ""Inventory owner attributes"" table.
	|- Fills in the ""Ledger account settings"" attribute.
	|- Fills in attributes ""Batch specification status (sender)"", ""Batch specification status (recipient)"".
	|- Fills in the ""Status"" attribute'");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	//-- Локализация
	//++ НЕ УТ
	|				ИЛИ ТабЧастьТовары.УдалитьСчетУчета <> &СчетУчетаПустаяСсылка
	|					И ТабЧастьТовары.НастройкаСчетовУчета = ЗНАЧЕНИЕ(Справочник.НастройкиСчетовУчетаПрочихОпераций.ПустаяСсылка)
	//-- НЕ УТ
	|				)
	|		)
	|";
	...
	Выборка = Результат.Выбрать();
 	Пока Выборка.Следующий() Цикл
		...
		Попытка
			...
			НастройкаСчетовУчетаСервер.ЗаполнитьНастройкиСчетовУчетаВидыЗапасов(ДокументОбъект, Новый Структура("ВидыЗапасов", "Товары"));
			//-- НЕ УТ
