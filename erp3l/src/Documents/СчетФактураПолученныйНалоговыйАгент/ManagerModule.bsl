	...
	Если ЗначениеЗаполнено(Реквизиты.ДатаИсправления)
		...
		Если Реквизиты.Корректировочный Тогда
			ИнформацияПоИсправлению = НСтр("ru = 'Исправление %НомерИсправления% от %ДатаИсправления% (корректировочный)';
											|en = 'Correction %НомерИсправления% dated %ДатаИсправления% (corrective)'", ОбщегоНазначения.КодОсновногоЯзыка());
		КонецЕсли;
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет реквизит ""Ставка НДС"", а так же служебные реквизиты документа ""Счет-фактура полученный (налоговый агент)"".';
									|en = 'Populates the attribute ""VAT rate"" and the internal attributes of the document ""Tax invoice received (tax agent)"".'");
