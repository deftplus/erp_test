...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ПараметрыНастройки = Документы.ПоступлениеБезналичныхДенежныхСредств.ПараметрыНастройкиСчетовУчета(Объект.ХозяйственнаяОперация);
	...
	ВстраиваниеУХПоступлениеБезналичныхДенежныхСредств.ОграничениеТипаДокументаПланирования(ЭтаФорма);
	#КонецОбласти
...
Процедура ХозяйственнаяОперацияПриИзмененииСервер()
	...
	Если МассивРеквизитовОперации.Найти("Контрагент") = Неопределено Тогда
		...
		Для каждого ТекСтрокаРасшифровки Из Объект.РасшифровкаПлатежа Цикл
			ТекСтрокаРасшифровки.ОснованиеПлатежа = Документы.ЗаказКлиента.ПустаяСсылка();
...
Процедура КонтрагентПриИзмененииСервер()
	...
	#Область УХ_Встраивание
	НастроитьЗависимыеЭлементыФормыНаСервере("ВалютныйКонтрольУХ");
	#КонецОбласти
...
Процедура ИдентификаторПлатежаАвтоПодбор(Элемент, Текст, ДанныеВыбора, Параметры, Ожидание, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Текст) Тогда
		...
		РасшифровкаПлатежаОснованиеПлатежаОбработкаПолученияДанныхВыбора(
			ДанныеВыбора, Объект.Организация, Объект.Контрагент, Объект.ХозяйственнаяОперация, Текст, Истина);
	КонецЕсли;
	//-- Локализация
...
Процедура РасшифровкаОбъектРасчетовПриИзменении(Элемент)
	...
	Если НЕ ЗначениеЗаполнено(СтрокаТаблицы.ОбъектРасчетов) Тогда
		СтрокаТаблицы.ОснованиеПлатежа = ПредопределенноеЗначение("Документ.ЗаказКлиента.ПустаяСсылка");
	КонецЕсли;
...
Процедура РасшифровкаПлатежаОснованиеПлатежаАвтоПодбор(Элемент, Текст, ДанныеВыбора, Ожидание, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Текст) Тогда
		...
		ПодборДебиторскойЗадолженности = ОбщегоНазначенияУТКлиентСервер.ДанныеПоПути(ЭтаФорма, СтруктураПараметров.ПодборДебиторскойЗадолженности);
		РасшифровкаПлатежаОснованиеПлатежаОбработкаПолученияДанныхВыбора(
			ДанныеВыбора, Объект.Организация, Объект.Контрагент, Объект.ХозяйственнаяОперация, Текст,,, ПодборДебиторскойЗадолженности);
	КонецЕсли;
...
Функция РеквизитыФормы(Форма)
	...
	РеквизитыФормы.Вставить("ВалютныйКонтрольУХ", 
		ВстраиваниеУХПоступлениеБезналичныхДенежныхСредствКлиентСерверПовтИсп.ВалютныйКонтроль(Форма.Объект.Контрагент));
	#КонецОбласти
...
&НаСервереБезКонтекста
Процедура РасшифровкаПлатежаОснованиеПлатежаОбработкаПолученияДанныхВыбора(ДанныеВыбора, Организация, Контрагент, ХозяйственнаяОперация, Текст, ЭтоУИП = Ложь, ЭтоОбъектРасчетов = Ложь, ПодборДебиторскойЗадолженности = Ложь)
	...
	Если Не (ЭтоОбъектРасчетов Или ЭтоУИП) Тогда
		СтруктураПараметров.Вставить("ПодборДебиторскойЗадолженности", ПодборДебиторскойЗадолженности);
		ОписаниеТипов = Метаданные.Документы.ПоступлениеБезналичныхДенежныхСредств.ТабличныеЧасти.РасшифровкаПлатежа.Реквизиты.ОснованиеПлатежа.Тип;
		СтруктураПараметров.Вставить("ТипыОснований", ОписаниеТипов.Типы());
	КонецЕсли;
...
Процедура ЗаполнитьСписокХозяйственныхОперацийЛокализация()
	...
	ДоступныеОперации.Добавить(Операции.ВозвратНеперечисленныхДС, НСтр("ru = 'Возврат неперечисленной зарплаты на лицевые счета';
																		|en = 'Unremitted salary return to personal accounts'"));
	ДоступныеОперации.Добавить(Операции.ВозвратНеперечисленнойЗарплатыПоЗарплатномуПроекту, НСтр("ru = 'Возврат зарплаты по зарплатному проекту';
																								|en = 'Salary return by payroll card program'"));
