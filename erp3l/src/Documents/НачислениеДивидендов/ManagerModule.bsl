...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Версия = "2.5.7.248";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("112de418-5dbb-4389-842a-c313939b987e");
	Обработчик.Многопоточный = Истина;
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "Документы.НачислениеДивидендов.ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию";
	...
	Изменяемые.Добавить(Метаданные.Документы.НачислениеДивидендов.ПолноеИмя());
	Обработчик.ИзменяемыеОбъекты = СтрСоединить(Изменяемые, ",");
	
	Блокируемые = Новый Массив;
	Блокируемые.Добавить(Метаданные.Документы.НачислениеДивидендов.ПолноеИмя());
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПараметрыВыборки.ПолныеИменаОбъектов = "Документ.НачислениеДивидендов";
	...
	Запрос = Новый Запрос(
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	НачисленияДивидендовФизическимЛицам.Ссылка КАК Ссылка
	|ИЗ
	|	Документ.НачислениеДивидендов.ФизическиеЛица КАК НачисленияДивидендовФизическимЛицам
	|ГДЕ
	|	НачисленияДивидендовФизическимЛицам.Ссылка.ДатаВыплаты >= ДАТАВРЕМЯ(2021, 1, 1, 0, 0, 0)
	|	И НачисленияДивидендовФизическимЛицам.ВычетПоНДФЛ <> 0
	|	И НачисленияДивидендовФизическимЛицам.НалогНаПрибыльКЗачету = 0"
	);
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПолноеИмяОбъекта = "Документ.НачислениеДивидендов";
	...
	Для каждого Выборка Из ОбновляемыеДанные Цикл
		...
		Попытка
			...
			Для каждого СтрокаДокумента Из ДокументОбъект.ФизическиеЛица Цикл
			...
			Если ДокументОбъект.Проведен Тогда
				ТаблицаДивидендов = ДокументОбъект.ФизическиеЛица.Выгрузить();
				ТаблицаДивидендов.Колонки.ФизическоеЛицо.Имя = "Акционер";
				...
				УчетНДФЛ.СформироватьДоходыИНДФЛСДивидендов(Выборка.Ссылка, Движения, Отказ, ДокументОбъект.Организация, ДокументОбъект.ДатаВыплаты, ТаблицаДивидендов, ДокументОбъект.ВключатьВДекларациюПоНалогуНаПрибыль);
