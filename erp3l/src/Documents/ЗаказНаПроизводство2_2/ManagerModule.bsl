...
//
// Возвращаемое значение:
// см. УправлениеПроизводством.ДобавитьКомандуСоздатьНаОснованииЗаказ
...
Функция ЕстьКонфликтыВОчереди() Экспорт
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса,
		"&ТекстЗапросаОтбор",
		ТекстЗапросаОтборКонфликтыВОчереди("РаспределениеЗапасов"));
...
//
// Параметры:
//  Распоряжения - Массив из ДокументСсылка.ЗаказНаПроизводство2_2,ДокументСсылка.ЗаказНаПроизводство2_2 - заказ на производство.
//
// Возвращаемое значение:
...
Функция ТекстЗапросаСхемыОбособленияПоРаспоряжению(СоздатьВременнуюТаблицу = Ложь)
	...
	|	
	// настройки для обособленных строк (по шапки документа)
	|	ОБЪЕДИНИТЬ ВСЕ
	|	
	|	ВЫБРАТЬ
	|		Заказы.Ссылка,
...
Функция ТекстЗапросаВТНезавершенныеЗаказы() Экспорт
	...
	|			 * (1 - (&ДопустимоеОтклонениеОтгрузкиПриемкиМерныхТоваров / 100)) > СУММА(Таблица.Произведено)
	|	
	|	) КАК Таблица
	|
	|СГРУППИРОВАТЬ ПО
...
Функция ТекстЗапросаВтСпособыПолученияМатериалов2_2() Экспорт
	...
	Возврат
		...
		|	ИСТИНА                      КАК Обособленно,
		|	НЕОПРЕДЕЛЕНО                КАК ЗаменаНаАналоги
		|ИЗ
		|	Документ.ЗаказНаПроизводство2_2 КАК ЗаказыНаПроизводство
		|ГДЕ
		|	ЗаказыНаПроизводство.Ссылка В(&Распоряжения)
...
//
// Параметры:
//	МассивЗаказов - Массив из ДокументСсылка.ЗаказДавальца - Заказ давальца, для которого необходимо создать/перезаполнить заказ на производство продукции по заказу давальца.
...
Функция ОстаткиПродукцииКОбеспечениюДавальцу(МассивЗаказов, ТекущийДокумент = Неопределено, ТипПроизводственногоПроцесса = Неопределено) Экспорт
	...
	ТекстЗапроса =
		...
		|	И Таблица.ВариантОбеспечения = ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.Требуется)
...
Функция ТекстЗапросаОтборКонфликтыВОчереди(ИмяТаблицыРаспределениеЗапасов) Экспорт
	...
	Возврат СтрЗаменить(Результат, "РаспределениеЗапасов", ИмяТаблицыРаспределениеЗапасов);
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Версия = "2.5.7.116";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("ecbdcf78-7bbd-405a-aabb-2be7e836ac58");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет новый реквизит ""Уникальный идентификатор""';
									|en = 'Populates a new ""UUID"" attribute'");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Запрос = Новый Запрос(
		...
		|	Документ.ЗаказНаПроизводство2_2 КАК ЗаказНаПроизводство2_2
		|ГДЕ
		|	ЗаказНаПроизводство2_2.УникальныйИдентификатор = """"
		|	ИЛИ ЗаказНаПроизводство2_2.Автор = Неопределено");
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Для Каждого Документ Из ОбновляемыеДанные Цикл
		...
		Попытка
			...
			Блокировка = Новый БлокировкаДанных;
			Блокировка.Добавить(ПолноеИмяОбъекта).УстановитьЗначение("Ссылка", Документ.Ссылка);
			Блокировка.Заблокировать();
			...
			Если ДокументОбъект <> Неопределено Тогда
				...
				Если ПустаяСтрока(ДокументОбъект.УникальныйИдентификатор) Тогда
					ДокументОбъект.УникальныйИдентификатор = Строка(Документ.Ссылка.УникальныйИдентификатор());
