...
Процедура РедактированиеЭтаповОбеспечениеМатериаламиИРаботамиПроизводитсяПриИзменении(Элемент)
	...
	СтруктураДействий.Вставить(
		...
		ОбеспечениеПроизводстваКлиентСервер.ПараметрыДействияПроверитьЗаполнитьНазначениеОбеспеченияВЭтапеПроизводства(НайденныеСтроки[0]));
	...
	ПараметрыМетода.Вставить("НазначениеМатериалы",           НайденныеСтроки[0].НазначениеМатериалы);
...
Процедура РедактированиеЭтаповОбеспечениеМатериаламиИРаботамиВариантОбеспеченияПриИзменении(Элемент)
	...
	Если ВариантОбеспеченияВыбранноеЗначение.ОткрытьФормуВыбораСкладаИСерий Тогда
		...
		ПараметрыФормы = ПараметрыФормыЗапросаКоличестваИСерийМатериалыЭтапов(ОбеспечениеВДокументахКлиентСервер.РежимВыборДействия());
...
Процедура РедактированиеЭтаповОбеспечениеМатериаламиИРаботамиОбособленноПриИзменении(Элемент)
	...
	СтруктураПоиска = Новый Структура("Распоряжение", Строка.Распоряжение);
	НайденныеСтроки = РедактированиеЭтапов.Этапы.НайтиСтроки(СтруктураПоиска);
	Если НайденныеСтроки.Количество() > 0 Тогда
...
#КонецОбласти
	...
	УправлениеПроизводством.УстановитьУсловноеОформлениеСтатьиКалькуляцииЭтапаПроизводства(ЭтаФорма, "РедактированиеЭтапов", "ПобочныеИзделия", "РедактированиеЭтаповПобочныеИзделияСтатьяКалькуляции", ПутиКДанным, Истина);
	...
	ДоступныеОстаткиПараметрыВстраивания = Обработки.РедактированиеЭтаповПроизводства.ДоступныеОстаткиПараметрыВстраивания();
	УправлениеПроизводством.УстановитьУсловноеОформлениеОбеспеченияЭтапаПроизводства(ЭтаФорма, "РедактированиеЭтапов", "ОбеспечениеМатериаламиИРаботами", ИменаЭлементов);
...
Процедура ЗаполнитьСлужебныеРеквизитыОбеспеченияЭтапаВСтроке(ТекущиеДанные, Форма, ДанныеЭтапов = Неопределено)
	...
	ОбеспечениеПроизводстваКлиентСервер.ЗаполнитьНазначениеОбеспеченияВСтрокеЭтапаПроизводства(ТекущиеДанные, ПараметрыДействия);
    ...
    Если РезультатВопроса = "ОткрытьСписокЗаказов" Тогда
		...
		Если СписокЗаказовПоНомеру.Количество() > 1 Тогда
			...
			ПараметрыФормы.Вставить("Заголовок", НСтр("ru = 'Заказы переработчикам (%КоличествоДокументов%)';
														|en = 'Purchase orders — Subcontracting (%КоличествоДокументов%)'"));
	...
	Если ЗначениеЗаполнено(Объект.ЖелаемаяДатаПоступления) Тогда
		...
		Если НЕ ОтветПередЗаписью И ЗначениеЗаполнено(МаксДатаАванса) И МаксДатаАванса > Объект.ЖелаемаяДатаПоступления Тогда
			...
			ТекстВопроса = НСтр("ru = 'Желаемая дата поступления меньше даты этапов предоплаты. Дата будет скорректирована до %МаксДатаАванса%';
								|en = 'Required date of receipt is less than the prepayment step date. The date will be adjusted to %МаксДатаАванса%'");
...
Функция ОбеспечениеЭтаповУстановитьДействие(КодДействия)
	...
	Если КодДействия = "ФЛАГ_ОБОСОБЛЕННО" Или КодДействия = "ФЛАГ_НЕОБОСОБЛЕННО" Тогда
		...
		Для Каждого Идентификатор Из Идентификаторы Цикл
			...
			Строка = РедактированиеЭтапов.ОбеспечениеМатериаламиИРаботами.НайтиПоИдентификатору(Идентификатор);
			
			СтруктураПоиска.Распоряжение = Строка.Распоряжение;
			НайденныеСтроки = РедактированиеЭтапов.Этапы.НайтиСтроки(СтруктураПоиска);
			Если НайденныеСтроки.Количество() = 0 Тогда
				Продолжить;
			КонецЕсли;
