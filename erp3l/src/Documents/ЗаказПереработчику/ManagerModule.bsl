	...
	Пока Выборка.Следующий() Цикл
		...
		ИначеЕсли Объект.ВариантПриемкиТоваров <> Выборка.ВариантПриемкиТоваров Тогда
			ВызватьИсключение НСтр("ru = 'Ошибка. Попытка создания заказа переработчику по этапам производства, имеющим разные варианты приемки на ордерный склад';
									|en = 'Error. An attempt to create a ""Purchase order — Subcontracting"" by production stages which have different options of receiving to warehouse'");
		КонецЕсли;
...
Функция ПроверкаПередСменойСтатуса(ВыборкаПроверки, НовыйСтатус, ДополнительныеПараметры) Экспорт
	...
	Если ВыборкаПроверки.ЗапрещеноМенятьСтатус Тогда
		...
		ТекстОшибки = НСтр("ru = 'У документа %Документ% статус ""%Статус%"" не установлен, т.к. заказ переработчику создан не на основании заказа на производство.';
							|en = 'The ""%Статус%"" status is not set for document %Документ% as the ""Purchase order — Subcontracting"" is generated not based on the production order.'");
	...
	Для каждого ИмяТаблицы Из СписокИменТаблиц Цикл
		...
		Иначе
			...
			ТекстИсключения = НСтр("ru = 'В функции ""%1"" не реализовано получение заблокированных распоряжений для таблицы ""%2"".';
									|en = 'Receipt of locked references for table ""%2"" is not implemented in the ""%1"" function"".'");
...
Функция ДоступныеОстаткиПараметрыВстраивания() Экспорт
	...
	ПараметрыВстраивания.ОписаниеПолученияДанныхДокумента =
		...
		// ++ НЕ УТКА
		// ++ Устарело_Производство21
		|			КОГДА НЕ ТаблицаУслуги.Распоряжение.Назначение ЕСТЬ NULL ТОГДА
		|				ТаблицаУслуги.Распоряжение.Назначение
		// -- Устарело_Производство21
		// -- НЕ УТКА
		|			КОГДА ИСТИНА ТОГДА
		|				ЕСТЬNULL(ТаблицаПродукция.Назначение, Реквизиты.Назначение)
		...
		|		 И Реквизиты.ОбосабливатьПоНазначениюПродукции
	...
	Если КодДействия = "ДЕЙСТВИЕ_ОТГРУЗИТЬ" Тогда
		...
		Для Каждого ТекущиеПараметры Из Результаты Цикл
			...
			Если Заказы.Количество() > 0 Тогда
				...
				Если ТипЗнч(Заказы[0]) = Тип("ДокументСсылка.ЗаказПереработчику") Тогда
					Текст =
						"ВЫБРАТЬ ПЕРВЫЕ 1
				...
				//++ НЕ УТКА
				ИначеЕсли ТипЗнч(Заказы[0]) = Тип("ДокументСсылка.ЭтапПроизводства2_2") Тогда
					Текст =
						"ВЫБРАТЬ ПЕРВЫЕ 1
						|	ИСТИНА КАК НуженВопрос
						|ИЗ
...
Процедура ВременныеТаблицыТоваровЗаказа(МенеджерВременныхТаблиц, Заказ, Отбор = Неопределено) Экспорт
	...
	Запрос.Текст =
		...
		|	
		|ГДЕ
		|	Товары.Ссылка.ЗаказПереработчику = &Заказ
		|		И НЕ Товары.Отменено
		|		И &Склад
		|	
		|СГРУППИРОВАТЬ ПО
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	Если ПравоДоступа("Изменение", Метаданные.Документы.ЗаказПереработчику) Тогда
		...
		КомандаПечати.Представление = НСтр("ru = 'Заказ переработчику на услуги по выпуску продукции';
											|en = '""Purchase order — Subcontracting"" for product release services'");
...
Функция СформироватьГиперссылкуСмТакжеВРаботе(Параметры) Экспорт
	...
	ТекстГиперссылки = НСтр("ru = 'Заказы переработчикам';
							|en = 'Purchase orders — Subcontracting'");
...
#Область ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет реквизиты ""Ставка НДС"", ""Объект расчетов"", ""Вариант обеспечения"", признак ""Обособленно"", признак оплаты в иностранной валюте, очищает реквизит ""Назначение"" табличной части ""Сырье и материалы"" для необособленных строк.';
									|en = 'Populates the ""VAT rate"", ""AR/AP object"", ""Supply option"" attributes, ""Assigned"" flag, payment in foreign currency flag, clears the ""Purpose"" attribute of the ""Raw and consumable materials"" table for non-assigned lines.'");
...
#Область СгенерироватьОбъектыРасчетов
	...
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("2b29b596-1b63-4a56-925c-a56f8594ae99");
	Обработчик.Многопоточный = Истина;
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "Документы.ЗаказПереработчику.ЗарегистрироватьДанныеДляГенерацииОбъектовРасчетов";
	Обработчик.ПроцедураПроверки = "ОбновлениеИнформационнойБазы.ДанныеОбновленыНаНовуюВерсиюПрограммы";
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|		ИЛИ НЕ ЗаказПереработчику.ОплатаВВалюте
	|			И (ЗаказПереработчику.УдалитьПорядокОплаты = ЗНАЧЕНИЕ(Перечисление.УдалитьПорядокОплатыПоСоглашениям.РасчетыВВалютеОплатаВВалюте)
	|				ИЛИ ЗаказПереработчику.УдалитьПорядокОплаты = ЗНАЧЕНИЕ(Перечисление.УдалитьПорядокОплатыПоСоглашениям.РасчетыВРубляхОплатаВВалюте))
	...
	|			ГДЕ
	|				ЗаказПереработчикуМатериалы.Ссылка = ЗаказПереработчику.Ссылка
	|				И НЕ (ЗаказПереработчикуМатериалы.ВариантОбеспечения В
	|					(ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.УдалитьОбособленно),
	|					ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.УдалитьОтгрузитьОбособленно)))
	|				И НЕ ЗаказПереработчикуМатериалы.Обособленно
	|				И НЕ ЗаказПереработчикуМатериалы.Назначение = ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)
	|			)
	|		)
	|	) КАК Регистраторы
	|";
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ИмяДокумента = "Документ.ЗаказПереработчику"; ИмяТЧ = "Материалы";
	
	ТекстыЗапроса = Новый Массив();
	Текст = ОбеспечениеВДокументахСервер.ТекстЗапросаДокументыДляОбработкиВариантаОбеспечения(ИмяДокумента, ИмяТЧ);
	ТекстыЗапроса.Добавить(Текст);
	
	ИмяТЧ = "РаспоряжениеСпецификация";
	Текст = ОбеспечениеВДокументахСервер.ТекстЗапросаДокументыДляОбработкиВариантаОбеспечения(ИмяДокумента, ИмяТЧ,
		"ДокументыДляОбработкиВариантаОбеспеченияРаспоряжение");
	ТекстыЗапроса.Добавить(Текст);
	
	Текст =
		"ВЫБРАТЬ
		|	Заказы.Ссылка КАК Ссылка
		|ПОМЕСТИТЬ ОчиститьНазначенияМатериалов
		|ИЗ
		|	Документ.ЗаказПереработчику КАК Заказы
		|ГДЕ
		...
		|				Документ.ЗаказПереработчику.Материалы КАК ЗаказПереработчикуМатериалы
		|			ГДЕ
		|				ЗаказПереработчикуМатериалы.Ссылка = Заказы.Ссылка
		|					И НЕ ЗаказПереработчикуМатериалы.ВариантОбеспечения В
		|						(ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.УдалитьОбособленно),
		|						ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.УдалитьОтгрузитьОбособленно))
		|					И НЕ ЗаказПереработчикуМатериалы.Обособленно
	...
	ТекстЗапроса =
		...
		|	НЕ ДокументыДляОбработкиВариантаОбеспеченияРаспоряжение.Ссылка ЕСТЬ NULL КАК ТребуетсяОбработкаВариантаОбеспеченияРаспоряжение,
		|	НЕ ОчиститьНазначенияМатериалов.Ссылка ЕСТЬ NULL КАК ТребуетсяОчиститьНазначенияМатериалов
		|ИЗ
		|	Документ.ЗаказПереработчику КАК Документы
		|		
		...
		|		
		|		ЛЕВОЕ СОЕДИНЕНИЕ ДокументыДляОбработкиВариантаОбеспеченияРаспоряжение КАК ДокументыДляОбработкиВариантаОбеспеченияРаспоряжение
		|		ПО ДокументыДляОбработкиВариантаОбеспеченияРаспоряжение.Ссылка = Документы.Ссылка
		|		
		|		ЛЕВОЕ СОЕДИНЕНИЕ ОчиститьНазначенияМатериалов КАК ОчиститьНазначенияМатериалов
		|		ПО ОчиститьНазначенияМатериалов.Ссылка = Документы.Ссылка
		|		
		|ГДЕ
		...
		Попытка
			...
			Если ДокументОбъект <> Неопределено Тогда
				...
				ОбщегоНазначенияУТ.ЗаполнитьИдентификаторыДокумента(ДокументОбъект,
					 Метаданные.Документы.ЗаказПереработчику.ТабличныеЧасти.Услуги.Имя);
				...
				ОбеспечениеВДокументахСервер.ОбработатьНеиспользуемыеВариантыОбеспечения(
					ДокументОбъект.Материалы,
					ОбъектИзменен,
					Документ.ТребуетсяОбработкаВариантаОбеспечения);
				
				ОбеспечениеВДокументахСервер.ОбработатьНеиспользуемыеВариантыОбеспечения(
					ДокументОбъект.РаспоряжениеСпецификация,
					ОбъектИзменен,
					Документ.ТребуетсяОбработкаВариантаОбеспеченияРаспоряжение);
				
				Если Документ.ТребуетсяОчиститьНазначенияМатериалов Тогда
					Для Каждого ТекущиеДанные Из ДокументОбъект.Материалы Цикл
						Если Не ТекущиеДанные.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.УдалитьОбособленно
							И Не ТекущиеДанные.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.УдалитьОтгрузитьОбособленно
							И Не ТекущиеДанные.Обособленно
