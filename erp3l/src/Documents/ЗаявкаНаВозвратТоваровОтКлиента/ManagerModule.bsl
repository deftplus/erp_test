...
Процедура ОтразитьРезерв(Запрос, ТекстыЗапроса, Регистры)
	...
	|				И (ДанныеШапки.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаявокНаВозвратТоваровОтКлиентов.НеСогласована)
	|					ИЛИ ДанныеШапки.Статус = ЗНАЧЕНИЕ(Перечисление.СтатусыЗаявокНаВозвратТоваровОтКлиентов.КВозврату)))
...
Процедура ЗапланироватьОтгрузкуТоваров(Запрос, ТекстыЗапроса, Регистры)
	...
	|	ТоварыДокумента.Ссылка В(&Ссылка)
	|	И ДанныеШапки.Статус <> ЗНАЧЕНИЕ(Перечисление.СтатусыЗаявокНаВозвратТоваровОтКлиентов.НеСогласована)
	|	И ДанныеШапки.Статус <> ЗНАЧЕНИЕ(Перечисление.СтатусыЗаявокНаВозвратТоваровОтКлиентов.КВозврату)
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		ВывестиТаблицуТоваровВТабличныйДокументЗаявкиНаВозврат(
			...
			НСтр("ru = 'Возвращаемый товар';
				|en = 'Returned goods'", ОбщегоНазначения.КодОсновногоЯзыка()),
			ДанныеПечати,
			ПараметрыВывода
		);
		...
		Если ЕстьТоварыНаЗамену Тогда
			...
			ВывестиТаблицуТоваровВТабличныйДокументЗаявкиНаВозврат(
				...
				НСтр("ru = 'Заменяющий товар';
					|en = 'Substitute goods'", ОбщегоНазначения.КодОсновногоЯзыка()),
				ДанныеПечати,
				ПараметрыВывода
			);
...
Процедура ВывестиТаблицуТоваровВТабличныйДокументЗаявкиНаВозврат(ТабличныйДокумент,
	...
	Для Каждого Строка Из ТЧ Цикл
		...
		Если  ИмяКолонкиТоваров = НСтр("ru = 'Заменяющий товар';
										|en = 'Substitute goods'", ОбщегоНазначения.КодОсновногоЯзыка()) Тогда
	...
	Если ПараметрыВывода.ТолькоКоличество Тогда
		ИтоговаяСтрока = НСтр("ru = 'Всего наименований %Количество%';
								|en = 'Total items %Количество%'", ОбщегоНазначения.КодОсновногоЯзыка());
...
#Область ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	|en = 'The handler populates the following attributes: • VAT rate • ""Foreign currency transaction"" flag
	| • AR/AP object
	|Supply option • ""Assigned"" flag
	| • Business unit in the ""Replacement items"" tabular section for items of ""Work"" type.'");
