...
Процедура ПодборНМА(Команда)
	...
	ПараметрыПодбора = ВнеоборотныеАктивыКлиентСервер.ПараметрыПодбора(Элементы.НематериальныеАктивыНематериальныйАктив, ЭтотОбъект);
...
Процедура ПодборЕГДС(Команда)
	...
	ПараметрыПодбора = ВнеоборотныеАктивыКлиентСервер.ПараметрыПодбора(Элементы.ЕдиницыГенерирующиеДенежныеСредстваЕГДС, ЭтотОбъект);
...
Функция ЗаполнитьВНА()
	...
	ПараметрыВыполнения.НаименованиеФоновогоЗадания = НСтр("ru = 'заполнение документа обесценения ВНА';
															|en = 'filling in a fixed asset impairment document'");
...
Процедура УправлениеЭлементамиФормыСервер()
	...
	ВозмещаемаяСтоимостьУпр = СтрШаблон(НСтр("ru = 'Возмещаемая стоимость (%1)';
											|en = 'Recoverable amount (%1)'"), ВалютаУправленческогоУчета);
	ВозмещаемаяСтоимостьРегл = СтрШаблон(НСтр("ru = 'Возмещаемая стоимость (%1)';
												|en = 'Recoverable amount (%1)'"), ВалютаРегламентированногоУчета);
