...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("05288a26-96d0-4abe-8fa7-a875d85189b6");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет реквизит ""Идентификатор фин записи"" табличной части ""Виды запасов"".
	|Заполняет реквизит ""Настройка счетов учета"".';
	|en = 'Populates the ""Fin record ID"" attribute in the ""Inventory owner attributes"" table.
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|ИЗ
	|	Документ.РаспределениеПроизводственныхЗатрат КАК Документ
	|ГДЕ
	|	ИСТИНА В
	|		(ВЫБРАТЬ ПЕРВЫЕ 1
	|			ИСТИНА
	|		ИЗ
	|			Документ.РаспределениеПроизводственныхЗатрат.ВидыЗапасов КАК ТабЧасть
	|		ГДЕ
	...
	|			ИСТИНА
	|		ИЗ
	|			Документ.РаспределениеПроизводственныхЗатрат.ПрочиеРасходы КАК ТабЧастьПрочиеРасходы
	|		ГДЕ
	...
	Для Каждого Документ Из ОбновляемыеДанные Цикл
		...
		Попытка
			...
			Если ДокументОбъект <> Неопределено Тогда
				...
				НастройкаСчетовУчетаСервер.ЗаполнитьНастройкиСчетовУчетаВидыЗапасов(ДокументОбъект, Новый Структура("ВидыЗапасов", "ПрочиеРасходы"));
			КонецЕсли;
