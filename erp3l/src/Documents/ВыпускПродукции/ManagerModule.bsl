...
Процедура ОтразитьРаспределениеЗапасовДвижения(Запрос, ТекстыЗапроса, Регистры)
	...
	ТекстЗапросаТабЧасть =
		...
		//++ НЕ УТКА
		|			ИНАЧЕ
		|				ТабЧасть.Ссылка.Распоряжение.Распоряжение
		//-- НЕ УТКА
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("811c9890-6c67-446a-ab7e-346a93997c55");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет реквизит НастройкаСчетовУчета.';
									|en = 'Fills in НастройкаСчетовУчета attribute.'");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|				ИСТИНА
	|			ИЗ
	|				Документ.ВыпускПродукции.Товары КАК ТЧТовары
	|			ГДЕ
	|				ТЧТовары.Ссылка = Документ.Ссылка
	|				И ТЧТовары.УдалитьСчетУчета <> &СчетУчетаПустаяСсылка
	|				И ТЧТовары.НастройкаСчетовУчета = ЗНАЧЕНИЕ(Справочник.НастройкиСчетовУчетаПрочихОпераций.ПустаяСсылка))";
