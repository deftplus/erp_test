	...
	Пока ДанныеОтчета.Следующий() Цикл
		...
		ЗаголовокДокумента = ОбщегоНазначенияУТКлиентСервер.СформироватьЗаголовокДокумента(
			...
			НСтр("ru = 'Задание торговому представителю';
				|en = 'Job for sales representative'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	Пока ДанныеОтчета.Следующий() Цикл
		...
		Если ДанныеОтчета.ТорговыйПредставитель <> ТекущийТорговыйПредставитель Тогда
			...
			ОбластьМакета.Параметры.ТекстЗаголовка = НСтр("ru = 'Сводное задание торговому представителю';
															|en = 'Summary job to the sales representative'", ОбщегоНазначения.КодОсновногоЯзыка());
			ОбластьМакета.Параметры.ТорговыйПредставитель = ДанныеОтчета.ТорговыйПредставитель;
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет реквизит ""Ставка НДС"" ссылкой на справочник ""Ставки НДС""';
									|en = 'Populates the ""VAT rate"" attribute with a reference to the ""VAT rates"" catalog'");
