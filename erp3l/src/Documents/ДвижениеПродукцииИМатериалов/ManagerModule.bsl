		...
		Если РеквизитыШапки.ОтгрузкаИзКладовойВыполняетсяРаспоряжением Тогда
			Если ПолучитьФункциональнуюОпцию("ИспользоватьОбособленноеОбеспечениеЗаказов") Тогда
				...
				ТекстОшибки = НСтр("ru = 'Нет товаров, доступных для отгрузки. Для начала процесса отгрузки необходимо указать действие ""Отгрузить"" или ""Отгрузить обособленно"" для одной или нескольких позиций номенклатуры заказа. При отгрузке из цеховой кладовой оформление накладной не требуется.';
									|en = 'There are no goods available for shipment. To start shipping, specify the ""Ship"" or ""Ship on assigned basis"" action for one or several product items in the order. When shipping from a shop floor stockroom, invoice registration is not required.'");
			...
			Иначе
				...
				ТекстОшибки = НСтр("ru = 'Нет товаров, доступных для отгрузки. Для начала процесса отгрузки необходимо указать действие ""Отгрузить"" для одной или нескольких позиций номенклатуры заказа. При отгрузке из цеховой кладовой оформление накладной не требуется.';
									|en = 'There are no goods available for shipment. To start shipping, specify the ""Ship"" action for one or several product items in the order. When shipping from a shop floor stockroom, invoice registration is not required.'");
...
Процедура УстановитьПараметрыЗапросаИнформацияПоОтправителюИПолучателю(Запрос)
	...
	Если Запрос.Параметры.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаПродукцииИзПроизводства
		...
		ИнформацияПоОтправителю = ?(ТипЗнч( Запрос.Параметры.Отправитель) = Тип("СправочникСсылка.СтруктураПредприятия"),
									НСтр("ru = 'Передача продукции из подразделения ""%1""';
										|en = 'Transfer products from the ""%1"" business unit'", КодЯзыка),
									НСтр("ru = 'Передача продукции из кладовой ""%1""';
										|en = 'Transfer products from the ""%1"" storeroom'", КодЯзыка));
		ИнформацияПоПолучателю  = НСтр("ru = 'Передача продукции на склад ""%1""';
										|en = 'Transfer products to the ""%1"" warehouse'", КодЯзыка);
	...
	ИначеЕсли Запрос.Параметры.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПередачаМатериаловВПроизводство
		...
		ИнформацияПоОтправителю = НСтр("ru = 'Передача материалов со склада ""%1""';
										|en = 'Transfer materials from the ""%1"" warehouse'", КодЯзыка);
		ИнформацияПоПолучателю  = ?(ТипЗнч( Запрос.Параметры.Получатель) = Тип("СправочникСсылка.СтруктураПредприятия"),
									НСтр("ru = 'Передача материалов в подразделение ""%1""';
										|en = 'Transfer materials to the ""%1"" business unit'", КодЯзыка),
									НСтр("ru = 'Передача материалов в кладовую ""%1""';
										|en = 'Transfer materials to the ""%1"" storeroom'", КодЯзыка));
	...
	ИначеЕсли Запрос.Параметры.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВозвратМатериаловИзПроизводства
		...
		ИнформацияПоОтправителю = ?(ТипЗнч( Запрос.Параметры.Отправитель) = Тип("СправочникСсылка.СтруктураПредприятия"),
									НСтр("ru = 'Возврат материалов из подразделения ""%1""';
										|en = 'Return materials from the ""%1"" business unit'", КодЯзыка),
									НСтр("ru = 'Возврат материалов из кладовой ""%1""';
										|en = 'Return materials from the ""%1"" storeroom'", КодЯзыка));
									
		ИнформацияПоПолучателю  = НСтр("ru = 'Возврат материалов на склад ""%1""';
										|en = 'Return materials to the ""%1"" warehouse'", КодЯзыка);
	...
	ИначеЕсли Запрос.Параметры.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ПеремещениеПолуфабрикатов
		...
		ИнформацияПоОтправителю = НСтр("ru = 'Передача из кладовой ""%1""';
										|en = 'Transfer from the ""%1"" storeroom'", КодЯзыка);
		ИнформацияПоПолучателю  = НСтр("ru = 'Передача в кладовую ""%1""';
										|en = 'Transfer to ""%1"" storeroom'", КодЯзыка);
...
Функция ОписаниеОперацийУчетаСебестоимости(Документ) Экспорт
	...
	|				И ТаблицаДокумента.ХозяйственнаяОперация В (ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаМатериаловВПроизводство),
	|															ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаМатериаловВКладовую))
	|			ТОГДА ТаблицаВидыЗапасов.ГруппаПродукции
	|		КОГДА НЕ ТаблицаВидыЗапасов.ГруппаПродукции = ЗНАЧЕНИЕ(Справочник.ГруппыАналитическогоУчетаНоменклатуры.ПустаяСсылка)
	|				И ТаблицаДокумента.ХозяйственнаяОперация В (ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратМатериаловИзПроизводства),
	|															ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратМатериаловИзКладовой))
	|			ТОГДА ТаблицаВидыЗапасов.ГруппаПродукции
	...
	|				И ТаблицаДокумента.ХозяйственнаяОперация В (ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаПродукцииИзПроизводства),
	|															ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПередачаПродукцииИзКладовой),
	|															ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПеремещениеПолуфабрикатов),
	|															ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ПеремещениеПолуфабрикатовМеждуФилиалами))
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "НакладнаяНаВозврат") Тогда
		...
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
			...
			НСтр("ru = 'Оприходование товаров';
				|en = 'Recording goods as received'"),
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("afb85367-2a9a-4d16-9b71-52643e99b999");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет реквизит ""Идентификатор строки"" в табличных частях документа ""Движение продукции и материалов"".';
									|en = 'Populates the ""Line ID"" attribute in the tables of the ""Products and materials register records"" document.'");
	...
	Изменяемые.Добавить(Метаданные.Документы.ДвижениеПродукцииИМатериалов.ПолноеИмя());
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	ПараметрыВыборки.ПолныеИменаОбъектов = "Документ.ДвижениеПродукцииИМатериалов";
	...
	|			ИСТИНА
	|		ИЗ
	|			Документ.ДвижениеПродукцииИМатериалов.ВидыЗапасов КАК ДвижениеПродукцииИМатериаловВидыЗапасов
	|		ГДЕ
	|			ДвижениеПродукцииИМатериаловВидыЗапасов.Ссылка = ДвижениеПродукцииИМатериалов.Ссылка
	|			И ДвижениеПродукцииИМатериаловВидыЗапасов.ИдентификаторСтроки = """")";
	...
	Для Каждого Документ Из ОбновляемыеДанные Цикл
		...
		Попытка
			...
			Если ДокументОбъект <> Неопределено Тогда
				ОбщегоНазначенияУТ.ЗаполнитьИдентификаторыДокумента(ДокументОбъект, "Товары,ВидыЗапасов");
