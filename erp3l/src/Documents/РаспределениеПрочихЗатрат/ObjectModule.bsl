...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
		...
		Если ДанныеЗаполнения.Свойство(КлючВариантаРаспределения)
			...
			Пока Выборка.Следующий() Цикл
				...
				НастройкиНаправленияРаспределения = Новый ХранилищеЗначения(
					Выборка.НастройкиНаправленияРаспределения.Получить());
	...
	ЭтоРаспределениеИзОВЗ = 
		//++ НЕ УТ
		ТипЗнч(АналитикаРасходов) = Тип("СправочникСсылка.ОбъектыВозникновенияЗатрат") ИЛИ 
		//-- НЕ УТ
		ЛОЖЬ;
		...
		ИначеЕсли НалоговыйУчет Тогда
			...
			Если НазначениеНастройкиРаспределения = Перечисления.НазначениеПравилРаспределенияРасходов.РаспределениеРасходовНаФинансовыйРезультат
			 И Не ЗначениеЗаполнено(НаправлениеРаспределения) Тогда
				НаправлениеРаспределения = Перечисления.НаправлениеРаспределенияПоПодразделениям.ПоКоэффициентам;	
				НоваяСтрока = НаправленияДеятельности.Добавить();
