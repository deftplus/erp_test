...
Процедура ЗаполнитьПараметрыИнициализации(Запрос, ДокументСсылка)
	...
	ИнформацияПоОрганизации = НСтр("ru = 'Возврат из ""%Организация%""';
									|en = 'Return from ""%Организация%""'", ОбщегоНазначения.КодОсновногоЯзыка());
...
#Область ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	|en = '1) Fills in ""VAT rate"" attribute
	|2) Fills in the method of determining the cost and the transfer document for ""Credit note  — Intercompany return delivery"" in the Goods table. For the lines that have a filled in transfer document, the method of determining the cost ""From transfer document"" should be set, for the rest - ""From the current document"". 
	|3) Fills in the number of CCD for ""Credit note  — Intercompany return delivery"" in the Goods table. If a single line from the Goods table corresponds to several numbers of the CCD from the Inventory owner attributes table, it should break down the lines in the Goods table.
	|4) Fills in AR/AP objects and Recipient''s AR/AP object attribute.
	|Until the completion of the handler, the documents will not be available for editing.'");
