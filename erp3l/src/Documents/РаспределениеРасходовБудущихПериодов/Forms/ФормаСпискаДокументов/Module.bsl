...
Функция ПолучитьСостояниеРаспределенияРБП(Параметры)
	...
	Если ЗначениеЗаполнено(Параметры.СтатьяРасходов) Тогда
		ТекстЗапроса = "ВЫБРАТЬ
			...
			|	ЕСТЬNULL(ДокументыУУ.Ссылка, ЗНАЧЕНИЕ(Документ.РаспределениеРасходовБудущихПериодов.ПустаяСсылка)) КАК ДокументУпр,
			|
			|	&ИспользоватьРеглУчет И СтатьиРасходов.ВариантРаспределенияРасходовНУ = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаРасходыБудущихПериодов) КАК НалоговыйУчет,
	...
	Иначе
		...
		ТекстЗапроса = "ВЫБРАТЬ
			...
			|	ЕСТЬNULL(ДокументыУУ.Ссылка, ЗНАЧЕНИЕ(Документ.РаспределениеРасходовБудущихПериодов.ПустаяСсылка)) КАК ДокументУпр,
			|
			|	СтатьиРасходов.ВариантРаспределенияРасходовНУ = ЗНАЧЕНИЕ(Перечисление.ВариантыРаспределенияРасходов.НаРасходыБудущихПериодов) КАК НалоговыйУчет,
...
Процедура УстановитьОформлениеДинамическихСписков()
	...
	Если ЗначениеЗаполнено(Организация) Тогда
		...
		Элементы.РасходыКРаспределениюГруппаКРаспределениюРегл.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'К распределению в регл. учете (%1)';
				|en = 'For allocation in local accounting (%1)'"),
			ВалютаРегламентированногоУчета);
	Иначе
		Элементы.РасходыКРаспределениюГруппаКРаспределениюРегл.Заголовок = НСтр("ru = 'К распределению в валюте регл. организации';
																				|en = 'For allocation in the company local accounting currency'");
	КонецЕсли;
