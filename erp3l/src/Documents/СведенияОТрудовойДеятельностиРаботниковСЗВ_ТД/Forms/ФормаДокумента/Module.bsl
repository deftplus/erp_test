		...
		Если ЕстьПоРазномуОтмеченныеСтрокиСотрудников() Тогда
			...
			Если ФлагБлокировкиДокумента Тогда
				...
				ТекстВопроса = НСтр("ru = 'Среди сотрудников есть строки, отмеченные принятыми в ПФР.
					...
					|Продолжить?';
					|en = 'There are rows among the employees that are marked as accepted in PF.
					|Selecting this check box will make all the employee rows accepted by PF.
					|Continue?'");
			...
			Иначе
				...
				ТекстВопроса = НСтр("ru = 'Среди сотрудников есть строки, отмеченные принятыми в ПФР.
					...
					|Продолжить?';
					|en = 'There are rows among the employees that are marked as accepted in PF.
					|Clearing this check box will cause all these strings to no longer be deemed as accepted by PF.
					|Continue?'");
...
Процедура УстановитьЗаголовокГруппыПрежнегоСтрахователя(УправляемаяФорма)
	...
	ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(
		...
		СтрШаблон(НСтр("ru = 'Прежний страхователь (%1)';
						|en = 'Previous insurant (%1)'"), ПредставлениеСтрахователя));
