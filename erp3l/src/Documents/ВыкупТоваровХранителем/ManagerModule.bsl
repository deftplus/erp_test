...
Функция СформироватьПечатнуюФормуНакладная(МассивОбъектов, ОбъектыПечати)
	...
	Пока ДанныеПечати.Следующий() Цикл
		...
		ТекстЗаголовка = ОбщегоНазначенияУТКлиентСервер.СформироватьЗаголовокДокумента(
			ДанныеПечати,
			НСтр("ru = 'Выкуп товаров хранителем';
				|en = 'Redemption of goods by the custodian'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
#Область ОбработатьДанныеДляПереходаНаНовуюВерсию
	...
	|en = 'Fills in CCD number in goods items purchases by the custodian in the Goods Items table. If several CCD numbers from the Inventory owner attributes table match the single string from the Goods items table, it splits out strings in the table Goods items.
	|Also, it populates the VAT rate attribute, Foreign currency transaction flag and the AR/AP object attribute.
	|You cannot edit goods purchases by the custodian until the handler execution is completed.'");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	ИЛИ ВыкупТоваровХранителем.ОбъектРасчетов = ЗНАЧЕНИЕ(Справочник.ОбъектыРасчетов.ПустаяСсылка)";
