...
&НаКлиенте
Процедура ОСОсновноеСредствоВладелецОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)

	ТекущиеДанные = Элементы.ОС.ТекущиеДанные;
	
	Если ЗначениеЗаполнено(ВыбранноеЗначение) И ТекущиеДанные.ОсновноеСредствоВладелецТекущее = ВыбранноеЗначение Тогда
		ВыбранноеЗначение = Неопределено;
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Новое ОС-владелец должно отличаться от текущего!';
														|en = 'Новое ОС-владелец должно отличаться от текущего!'"));
	КонецЕсли;
...
Процедура НастроитьЗависимыеЭлементыФормы(Знач ИзмененныеРеквизиты = "")
	...
	Если СтруктураИзмененныхРеквизитов.Свойство("ЕстьСобственныеОС") 
		...
		Элементы.ДекорацияНадпись.Видимость = ЕстьСобственныеОС
			И Не Объект.ХозяйственнаяОперация = ПредопределенноеЗначение("Перечисление.ХозяйственныеОперации.ПеремещениеУзлов");
	КонецЕсли;
