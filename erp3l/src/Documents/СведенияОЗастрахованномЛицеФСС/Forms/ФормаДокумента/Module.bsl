...
Функция СостояниеОтправки()
	...
	ИначеЕсли Состояние = ВариантыСостояний.ОшибкаПриОтправке Тогда
		...
		Представление     = НСтр("ru = 'При отправке <a href=""1"">возникла ошибка</a>';
								|en = 'При отправке <a href=""1"">возникла ошибка</a>'");
	...
	ИначеЕсли Состояние = ВариантыСостояний.НеПринят Тогда
		...
		Представление = НСтр("ru = 'Не принят, см. <a href=""1"">протокол</a>';
							|en = 'Не принят, см. <a href=""1"">протокол</a>'");
	ИначеЕсли Состояние = ВариантыСостояний.ПринятСЗамечаниями Тогда
		...
		Представление     = НСтр("ru = 'Принят с замечаниями, см. <a href=""1"">несоответствия</a>';
								|en = 'Принят с замечаниями, см. <a href=""1"">несоответствия</a>'");
	Иначе
...
Функция ПодготовитьВопросВПоддержку()
	...
	Если ЗначениеЗаполнено(ТекстЗапросаXML) Тогда
		...
		Вложение.Представление = НСтр("ru = 'СведенияОСотруднике';
										|en = 'СведенияОСотруднике'") + "_" + ИдентификаторДокумента + ".xml";
	...
	Сообщение.Тема = НСтр("ru = 'Вопрос по СЭДО (тип 86: сведения о застрахованном лице ФСС) от %1 (%2)';
							|en = 'EDI question (type 86: details of the insured person of SSF) from %1 (%2)'");
