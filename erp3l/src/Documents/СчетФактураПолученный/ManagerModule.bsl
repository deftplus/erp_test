...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	НоваяСтрока = Обработчик.ПриоритетыВыполнения.Добавить();
	НоваяСтрока.Процедура = "РегистрыСведений.АналитикаУчетаПартий.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	...
	НоваяСтрока = Обработчик.ПриоритетыВыполнения.Добавить();
	НоваяСтрока.Процедура = "РегистрыСведений.РеестрДокументов.ОбработатьДанныеДляПереходаНаНовуюВерсиюОРП";
	НоваяСтрока.Порядок = "До";

	НоваяСтрока = Обработчик.ПриоритетыВыполнения.Добавить();
	НоваяСтрока.Процедура = "Справочники.КлючиАналитикиУчетаПартий.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	НоваяСтрока.Порядок = "После";

	НоваяСтрока = Обработчик.ПриоритетыВыполнения.Добавить();
	НоваяСтрока.Процедура = "УчетНДСРФ.ОбработатьДанныеДляПереходаНаНовуюВерсиюСФиВЭД";
	НоваяСтрока.Порядок = "До";
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|				И ДокументыОснованияСчетаФактуры.СуммаУменьшение = 0
	|				И ДокументыОснованияСчетаФактуры.СуммаНДСУменьшение = 0
	...
	|	ИЛИ ДокументыОснованияСчетаФактуры.УдалитьСтавкаНДС <> &ПустаяСтавкаНДС
	|		И ДокументыОснованияСчетаФактуры.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка)"
	);
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|		КОГДА ДокументыОснования.Ссылка ЕСТЬ NULL
	|			ТОГДА ЛОЖЬ
	|		КОГДА ДокументыОснования.УдалитьСтавкаНДС <> &ПустаяСтавкаНДС
	|				И ДокументыОснования.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.ПустаяСсылка)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	...
	Пока ВыборкаДокументов.Следующий() Цикл
		...
		Попытка
			...
			Если ВыборкаДокументов.ЗаполнятьСтавкуНДС Тогда
				МассивТЧ = Новый Массив();
				МассивТЧ.Добавить("ДокументыОснования");
