	...
	Если Не ОбъектНайденВКэше Тогда
		...
		ВызватьИсключение СтрШаблон(
			НСтр("ru = 'Объект метаданных %1 не может быть обработчиком ВыгрузкаСовместноРазделенныхДанных.ПередВыгрузкойОбъекта(),
                  ...
                  |en = 'The %1 metadata object cannot be the ExportSharedSeparatedData.BeforeExportObject() handler
                  ...
                  |ExportImportSharedSeparatedData.FillSharedSeparatedObjectsCache() method'", Метаданные.ОсновнойЯзык.КодЯзыка),
	...
	Если Не ОбъектНайденВКэше Тогда
		...
		ВызватьИсключение СтрШаблон(
			НСтр("ru = 'Объект метаданных %1 не может быть обработчиком ВыгрузкаСовместноРазделенныхДанных.ПередЗагрузкойОбъекта(),
                  ...
                  |обновление кэша совместно-разделенных объектов с помощью вызова метода
                  |ВыгрузкаЗагрузкаСовместноРазделенныхДанных.ЗаполнитьКэшСовместноРазделенныхОбъектов()';
                  |en = 'The %1 metadata object cannot be the ExportSharedSeparatedData.BeforeImportObject() handler
                  |as it is missing in the cache of together divided objects.
                  |If cache was not updated after metadata structure editing,
                  |update cache of together divided objects by calling the
                  |ExportImportSharedSeparatedData.FillSharedSeparatedObjectsCache() method'", 
