...
Функция ДобавитьБлокировкуИзмененияОбъекта(КоллекцияБлокировок, СсылкаНаОбъект) Экспорт
	...
	УОбъектаЕстьФайлы = УОбъектаЕстьФайлыПечатныхФорм(СсылкаНаОбъект);
	Если УОбъектаЕстьФайлы Тогда
...
Процедура ЗапретитьИзменениеДокументаПоФизическомуЛицу(Источник) Экспорт
	...
	УОбъектаЕстьФайлы = УОбъектаЕстьФайлыПечатныхФорм(Источник.Ссылка);
	Если УОбъектаЕстьФайлы Тогда
...
Процедура ПриЗаполненииСпискаТекущихДел(ТекущиеДела) Экспорт
	...
	КоличествоЗависшихЗаданий = РегистрыСведений.ЗапланированныеДействияСПечатнымиФормами.КоличествоЗависшихЗаданийОбработки(Истина);
	...
	ПрисоединенныеФайлы = РегистрыСведений.ЗапланированныеДействияСПечатнымиФормами.ФайлыНаПодписьПользователя(Истина);
	Если ПрисоединенныеФайлы.Количество() > 0 Тогда
		
		КоличествоПросроченных = ПрисоединенныеФайлы.НайтиСтроки(Новый Структура("Просрочено", Истина)).Количество();
		Дело = ТекущиеДела.Добавить();
		Дело.Идентификатор  = "НаправленыНаПодпись_" + СтрЗаменить(Новый УникальныйИдентификатор, "-", "");
		Дело.Количество     = ПрисоединенныеФайлы.Количество();
		Дело.ЕстьДела       = (Дело.Количество > 0);
		...
		Дело.ПараметрыФормы = КадровыйЭДОВызовСервера.ПараметрыОткрытияФормыПодписанияПечатныхФорм(ПрисоединенныеФайлы);
		Дело.Форма          = "ОбщаяФорма.ПечатьДокументов";
		
		УведомитьОНеобходимостиПодписанияПечтаныхформ(
			ПрисоединенныеФайлы.Количество(), КоличествоПросроченных, Пользователи.ТекущийПользователь());
		
	КонецЕсли;
	
	ЗаданияКИсполнению = РегистрыСведений.ЗапланированныеДействияСПечатнымиФормами.ЗаданияОбработки(Истина);
	Для Каждого ОписаниеЗадания Из ЗаданияКИсполнению Цикл
		
		Если ОписаниеЗадания.Ключ = Перечисления.ДействияСПечатнымиФормами.ОтправитьПоПочте Тогда
			...
			ФайлыКОбработке = РегистрыСведений.ЗапланированныеДействияСПечатнымиФормами.ФайлыКОбработке(
				Истина, Перечисления.ДействияСПечатнымиФормами.ОтправитьПоПочте);
			
			СписокФайлов = ФайлыКОбработке.Получить(Перечисления.ДействияСПечатнымиФормами.ОтправитьПоПочте);
		...
		ИначеЕсли ОписаниеЗадания.Ключ = Перечисления.ДействияСПечатнымиФормами.ЗаписатьНаДиск Тогда
			...
			ФайлыКОбработке = РегистрыСведений.ЗапланированныеДействияСПечатнымиФормами.ФайлыКОбработке(
				Истина, Перечисления.ДействияСПечатнымиФормами.ЗаписатьНаДиск);
			
			СписокФайлов = ФайлыКОбработке.Получить(Перечисления.ДействияСПечатнымиФормами.ЗаписатьНаДиск);
...
Функция ПечатнаяФормаСоШтампами(ФайлПечатнойФормы, Оригинал, ВыводитьОжиданияПодписи = Ложь) Экспорт
	...
	Если ВыводитьОжиданияПодписи Тогда
		УстановитьПривилегированныйРежим(Истина);
		Направления = РегистрыСведений.ЗапланированныеДействияСПечатнымиФормами.НаправленияНаПодпись(ФайлПечатнойФормы);
		УстановитьПривилегированныйРежим(Ложь);
	КонецЕсли;
...
Процедура ВыполнитьОтправкуПечатныхФормПоЭлектроннойПочте() Экспорт
	...
	ФайлыКОбработке = РегистрыСведений.ЗапланированныеДействияСПечатнымиФормами.ФайлыКОбработке(
		Ложь, Перечисления.ДействияСПечатнымиФормами.ОтправитьПоПочте);
	...
	Если СписокФайлов <> Неопределено Тогда
		...
		Если ОтправленныеПолучатели.Количество() > 0 Тогда
			...
			РегистрыСведений.ЗапланированныеДействияСПечатнымиФормами.УдалитьФайлыИзОбработки(
				ОтправленныеФайлы, Перечисления.ДействияСПечатнымиФормами.ОтправитьПоПочте);
...
Функция ДанныеПечатнойФормыПоДаннымФайлаПечатнойФормы(ДанныеФайла) Экспорт
	...
	ДанныеПечатнойФормы.УстановленныеПодписи =       Новый Соответствие;
	ДанныеПечатнойФормы.ПечатнаяФорма =              ДанныеФайла.Оригинал.Получить();
	
	УстановитьПривилегированныйРежим(Истина);
	УстановленныеПодписи = ЭлектроннаяПодпись.УстановленныеПодписи(ДанныеФайла.ФайлОбъекта);
	Если УстановленныеПодписи.Количество() > 0 Тогда
		
		Для Каждого УстановленнаяПодпись Из УстановленныеПодписи Цикл
			ДанныеПечатнойФормы.УстановленныеПодписи.Вставить(
...
Функция КлючСообщенияОНеобходимостиПодписанияПечтаныхформ()
	Возврат "ПодписаниеПечтаныхФорм";
КонецФункции

Процедура УведомитьОНеобходимостиПодписанияПечтаныхформ(КоличествоПечатныхФорм, КоличествоПросроченных, Пользователь) Экспорт
	...
	Если КоличествоПечатныхФорм = 0 Тогда
		ОбсужденияБЗК.УдалитьСообщения(АссистентКЭДО, КлючСообщенияОНеобходимостиПодписанияПечтаныхформ(),
	...
	Иначе
		...
		Если КоличествоПросроченных = 0 Тогда
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Ожидают подписания %1.';
				...
				СтрокаСКоличествомПечатныхФорм(КоличествоПечатныхФорм));
			...
			ТекстСообщения = Новый ФорматированнаяСтрока(
				...
				Новый ФорматированнаяСтрока(
					СтрШаблон(
						...
						СтрокаСКоличествомПечатныхФорм(КоличествоПросроченных)), , ЦветаСтиля.ПоясняющийОшибкуТекст),
				".");
		Иначе
			ТекстСообщения = Новый ФорматированнаяСтрока(
				СтрШаблон(НСтр("ru = 'Ожидают подписания %1 с большим сроком ожидания.';
								|en = 'Pending signing %1 with long timeout.'"),
					СтрокаСКоличествомПечатныхФорм(КоличествоПросроченных)), , ЦветаСтиля.ПоясняющийОшибкуТекст);
		КонецЕсли;
		
		ОписаниеСообщения = ОбсужденияБЗК.ОписаниеСообщения(
			ТекстСообщения, КлючСообщенияОНеобходимостиПодписанияПечтаныхформ());
...
Функция СтрокаСКоличествомПечатныхФорм(КоличествоПечатныхФорм)
...
Функция УОбъектаЕстьФайлыПечатныхФорм(СсылкаНаОбъект)
...
Процедура СохранитьСертификаты(ПараметрыПечати)
	...
	Для Каждого ПодписьБезСертификата Из ПараметрыПечати.ПодписиБезСертификатов Цикл
		...
		Если ЗначениеЗаполнено(ПодписьБезСертификата.Отпечаток) Тогда
			...
			Запись.ПодписанныйОбъект = ПодписьБезСертификата.ПодписанныйОбъект;
			Запись.ПорядковыйНомер = ПодписьБезСертификата.ПорядковыйНомер;
			
			Запись.Прочитать();
			Если Запись.Выбран() Тогда
				Запись.Отпечаток = ПодписьБезСертификата.Отпечаток;
				Запись.Сертификат = Новый ХранилищеЗначения(
					ПолучитьИзВременногоХранилища(ПодписьБезСертификата.Сертификат));
				Запись.Записать();
			КонецЕсли;
	...
	Если УправляемаяФорма.Параметры.Свойство("ПараметрыПечати") Тогда
		Если УправляемаяФорма.Параметры.ПараметрыПечати.Свойство("ДополнительныеПараметры") Тогда
			Если УправляемаяФорма.Параметры.ПараметрыПечати.ДополнительныеПараметры.Свойство("ОписанияФайловПечатныхФорм") Тогда
				...
				Для Каждого ДанныеИдентификторовПечатныхФорм Из УправляемаяФорма.Параметры.ПараметрыПечати.ДополнительныеПараметры.ОписанияФайловПечатныхФорм.ПечатныеФормыОбъектов Цикл
					Для Каждого ДанныеИдентификтораПечатнойФормы Из ДанныеИдентификторовПечатныхФорм.Значение Цикл
						ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ФайлыПечатныхФорм, ДанныеИдентификтораПечатнойФормы.Значение);
					КонецЦикла;
				КонецЦикла;
				Если ЗначениеЗаполнено(ФайлыПечатныхФорм) Тогда
					ФайлыНаПодписьПользователя = РегистрыСведений.ЗапланированныеДействияСПечатнымиФормами.ФайлыНаПодписьПользователя(Истина);
