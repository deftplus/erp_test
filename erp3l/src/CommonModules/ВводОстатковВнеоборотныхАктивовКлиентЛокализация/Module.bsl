...
Процедура ФормаРедактированияСтрокиОС_ПриИзмененииРеквизита(Элемент, Форма) Экспорт
	...
	Иначе
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Отсутствует обработка события ""ПриИзменении"" для элемента ""%1""';
										|en = 'No processing of the event ""ПриИзменении"" for the item ""%1""'"), Элемент.Имя);
	КонецЕсли; 
...
Процедура ФормаРедактированияСтрокиНМА_ПриИзмененииРеквизита(Элемент, Форма) Экспорт
	...
	ИначеЕсли Элемент.Имя = Элементы.ПорядокУчетаБУ.Имя Тогда
		...
		ФормаРедактированияСтрокиНМА_ПорядокУчетаБУПриИзменении(Форма, ТребуетсяВызовСервера, ДополнительныеПараметры);
	...
	ИначеЕсли Элемент = Элементы.СтатьяРасходовБУ
		ИЛИ Элемент = Элементы.СтатьяРасходовНУ
...
Процедура ФормаРедактированияСтрокиНМА_ПорядокУчетаБУПриИзменении(Форма, ТребуетсяВызовСервера, ДополнительныеПараметры)
	
	ИзмененныеРеквизиты = "ПорядокУчетаБУ";
	
	Если Форма.ПорядокУчетаБУ = ПредопределенноеЗначение("Перечисление.ПорядокУчетаСтоимостиВнеоборотныхАктивов.СписыватьПриПринятииКУчету") Тогда
...
Процедура ЗаполнитьПоДаннымОУ(Форма, ТабличнаяЧасть, ИмяКоманды, ТребуетсяВызовСервера, ПродолжитьВыполнениеКоманды)
	...
	Если ТабличнаяЧасть.Количество() <> 0 Тогда
		...
		ТекстВопроса = НСтр("ru = 'При заполнении текущие данные табличной части будут очищены. Продолжить?';
							|en = 'Current tabular section data will be cleared during population. Continue?'");
