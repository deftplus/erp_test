	...
	Если ПриЗапуске Тогда
		КомментарийДляЖурналаРегистрацииБезСтекаВызовов =
			НСтр("ru = 'Недопустимое обращение к неинициализированным параметрам клиента на сервере.
			           ...
			           |The SSL API might have been called prematurely before initialization
			           |was completed in StandardSubsystemsClient.BeforeStart.'",
	...
	Иначе
		КомментарийДляЖурналаРегистрацииБезСтекаВызовов =
			НСтр("ru = 'Недопустимое обращение к неинициализированным параметрам клиента на сервере.
			           ...
			           |en = 'Invalid access to uninitialized client parameters on the server.
			           |The SSL API might have been called after all session parameters
			           |were incorrectly cleared without using Common.ClearSessionParameters.'",
			     ОбщегоНазначения.КодОсновногоЯзыка());
	КонецЕсли;
	...
	Если Не ПриЗапуске Тогда
		ТекстОшибки =
			НСтр("ru = 'Не инициализированы параметры клиента на сервере.
			           ...
			           |en = 'Client parameters on the server are not initialized. To initialize them, retry the action or restart the session.'");
	...
	Если ДинамическиеИзмененияКонфигурации.Исправления <> Неопределено Тогда
		Если ДинамическиеИзмененияКонфигурации.Исправления.Добавлено > 0
			...
			ТекстСообщенияПатчи = НСтр("ru = 'Установлены новые исправления (патчи): %1, удалены: %2.';
										|en = 'New patches installed: %1, deleted: %2.'") + "Ссылка";
		ИначеЕсли ДинамическиеИзмененияКонфигурации.Исправления.Добавлено = 1 Тогда
			ТекстСообщенияПатчи = НСтр("ru = 'Установлено новое исправление (патч).';
										|en = 'A new patch is installed.'") + "Ссылка";
		ИначеЕсли ДинамическиеИзмененияКонфигурации.Исправления.Добавлено > 0 Тогда
			ТекстСообщенияПатчи = НСтр("ru = 'Установлены новые исправления (патчи): %1.';
										|en = 'New patches installed: %1.'") + "Ссылка";
