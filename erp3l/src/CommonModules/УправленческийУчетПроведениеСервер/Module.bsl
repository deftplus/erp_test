			...
			Для Каждого Организация Из Документ.Строки Цикл
				...
				Иначе
					...
					Для Каждого ОбъектРасчетов Из Организация.Строки Цикл
						УникальныеВладельцы.Вставить(ОбъектРасчетов.ВладелецОбъектаРасчета);
					КонецЦикла;
...
//
// Параметры:
//  ПараметрыДвижений - Структура - парамтеры формирования запроса баланса активовов/пассивов.
...
#Область ТекстЗапросаПроцентыДисконтирования
	...
	|	СУММА(Проценты.СуммаДисконтирования) КАК Сумма,
	|	СУММА(Проценты.СуммаДисконтированияУпр) КАК СуммаУпр,
	|	СУММА(Проценты.СуммаДисконтированияРегл) КАК СуммаРегл,
...
//Платежные документы: СписаниеБДС, РКО
		...
		|	ИСТИНА КАК ЭтоГоловнаяОрганизация,
		|	РасчетыСПартнерами.Сторно КАК Сторно
		|ИЗ
		|	РасчетыСПартнерами КАК РасчетыСПартнерами
		|ГДЕ
...
Функция ТекстЗапросаДвиженияПоСрокам(ИмяТаблицы, ИмяВременнойТаблицы = "", Сторно = Ложь)
	...
	|	ВЫБОР
	|		КОГДА РасчетыСПоставщиками.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) И &Ресурс <> 0
	|			ТОГДА ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|		КОГДА РасчетыСПоставщиками.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход) И &Ресурс <> 0
