...
Процедура ЗаполнитьПредставлениеСостоянияПодписей(Подписи) Экспорт
	...
	Для Каждого СтрокаПодписи Из Подписи Цикл
		Если ЗначениеЗаполнено(СтрокаПодписи.ДатаПроверкиПодписи) Тогда
			СтрокаПодписи.ПодписьВернаПредставление = ?(СтрокаПодписи.ПодписьВерна, НСтр("ru = 'Верна';
																						|en = 'Valid'"), НСтр("ru = 'Неверна';
																												|en = 'Invalid'"))
...
Процедура СкорректироватьДатыПодписиНаЧасовойПоясКлиента(СвойстваПодписи)
