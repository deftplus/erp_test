...
//
Функция ПолучитьКоэффициентПересчетаИзВалютыВВалюту(ТекущаяВалюта, НоваяВалюта, Дата, БазоваяВалюта = Неопределено) Экспорт
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Устанавливает новую константу ""Базовая валюта по умолчанию"".';
									|en = 'Sets a new constant ""Default base currency"".'");

КонецПроцедуры

Процедура БазоваяВалютаПоУмолчанию_ОбработатьДанныеДляПереходаНаНовуюВерсию() Экспорт
	
	Константы.БазоваяВалютаПоУмолчанию.Установить(Константы.ВалютаРегламентированногоУчета.Получить());
