...
Процедура ПроверитьНаличиеУстаревшихИсправлений() Экспорт
	...
	Если Результат.ЕстьИзменения Тогда
		ТекстСообщения = НСтр("ru = 'Некорректный вызов функции ""ОбновлениеИнформационнойБазы.ВыполнитьОбновлениеИнформационнойБазы"".
			...
			|en = 'Incorrect call of the InfobaseUpdate.UpdateInfobase function.
			|Obsolete patches were not deleted before the call, it is possible that the update is performed by non-standard means.'");
...
Функция ИсправлениеПрименимо(ЧтениеZip, ВременныйКаталог)
	...
	Исключение
		...
		ВызватьИсключение НСтр("ru = 'Ошибка при чтении файла манифеста исправления';
								|en = 'Error reading patch manifest file'") + ":" + Символы.ПС + ТекстОшибки;
	КонецПопытки;
