			...
			Для Каждого СтрокаШтрихкода Из ДанныеШтрихкодовСписок Цикл
				...
				Если ДанныеРазбора = Неопределено
					...
					"ru = 'Обработка логистических упаковок и групповых упаковок
					      ...
					      |Включите опцию в панели администрирования ""Интеграция с ИС МП (обувь, одежда, табак...)""
					      |в разделе ""Настройки сканирования кодов маркировки"".';
					      |en = 'Обработка логистических упаковок и групповых упаковок
					      |невозможна, так как выключена опция
					      |""Запрашивать данные сервиса ГИС МТ"".
					      |Включите опцию в панели администрирования ""Интеграция с ИС МП (обувь, одежда, табак...)""
					      |в разделе ""Настройки сканирования кодов маркировки"".'");
		...
		Иначе
					...
					СтрокиПредупрежденияКлиента.Добавить(
							СтрШаблон(
								НСтр("ru = 'Нажмите ""%1"", если покупатель согласен приобрести товар.';
									|en = 'Нажмите ""%1"", если покупатель согласен приобрести товар.'"),
			...
			Иначе
				...
				Форма.ШаблонСообщения = НСтр("ru = 'Отрицательный результат проверки кодов маркировки %1 из %2.';
											|en = 'Отрицательный результат проверки кодов маркировки %1 из %2.'");
				...
				Если ОтображениеИгнорирования Тогда
					...
					Если ВозможныеПредставления.Количество() > 1 Тогда
						СтрокиПредупрежденияКлиента.Добавить(НСтр("ru = 'В чеке будуд зафиксированы результаты проверки: %1';
																	|en = 'В чеке будуд зафиксированы результаты проверки: %1'"));
					Иначе
						СтрокиПредупрежденияКлиента.Добавить(НСтр("ru = 'В чеке будет зафиксирован результат проверки: %1';
																	|en = 'В чеке будет зафиксирован результат проверки: %1'"));
					КонецЕсли;
...
Функция СтрокаСодержитОшибкиЧастичногоВыбытия(СтрокаДанных, ПараметрыСканирования, ПравилаПроверкиНаОшибки, КэшМаркируемойПродукции) Экспорт
	...
	Если ПараметрыСканирования.ТребуетсяЧастичноеВыбытие
		...
		ИначеЕсли СтрокаДанных.ЧастичноеВыбытиеВариантУчета = Перечисления.ВариантыУчетаЧастичногоВыбытияИСМП.НастроеннаяНоменклатура
			И Не ЗначениеЗаполнено(СтрокаДанных.ЧастичноеВыбытиеНоменклатура) Тогда
		...
		Для Каждого СтрокаДанных Из ПравилаПроверкиНаОшибки.ОшибкаНастройкиПараметровЧастичногоВыбытия Цикл
			...
			Если СтрокаДанных.ВидПродукции = Перечисления.ВидыПродукцииИС.АльтернативныйТабак Тогда
				ОписаниеПроблемы = НСтр("ru = 'Не указана номенклатура для поштучной продажи сигары (сигарилл, ...).';
										|en = 'Не указана номенклатура для поштучной продажи сигары (сигарилл, ...).'");
			ИначеЕсли СтрокаДанных.ВидПродукции = Перечисления.ВидыПродукцииИС.Духи Тогда
	...
	ИначеЕсли ПараметрыРазбора.ОсновнойШаблонGTIN[ЗначениеИдентификатораGTIN] <> ПолныйХешШаблонаСУчетомGTIN Тогда
		...
		СообщенияОбОшибке.Добавить(
			НСтр("ru = 'Шаблон кода маркировки отличается от основного';
				|en = 'Шаблон кода маркировки отличается от основного'"));
