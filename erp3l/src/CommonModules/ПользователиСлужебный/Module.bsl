	...
	Иначе
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка в процедуре ПользователиСлужебный.ОбработатьИнтерфейсРолей()
			           ...
			           |en = 'UsersInternal.ProcessRolesInterface procedure error.
...
Процедура ПроверитьНазначениеРолей(НазначениеРолей = Неопределено, ПроверитьВсе = Ложь, СписокОшибок = Неопределено) Экспорт
	...
	ЗаголовокОшибки =
		НСтр("ru = 'Ошибка в процедуре ПриОпределенииНазначенияРолей общего модуля ПользователиПереопределяемый.';
			|en = 'UsersOverridable.OnDefineRolesAssignment procedure error.'");
		...
		ИначеЕсли СтрДлина(Имя) > 64 Тогда
			...
			ОбщегоНазначения.СообщитьПользователю(
				НСтр("ru = 'Имя (для входа) превышает 64 символа.';
					|en = 'The username exceeds 64 characters.'"),
				,
				"Имя",
				,
				Отказ);
		...
		ИначеЕсли СтрНайти(Имя, ":") > 0 Тогда
			ОбщегоНазначения.СообщитьПользователю(
				НСтр("ru = 'Имя (для входа) содержит запрещенный символ "":"".';
					|en = 'The username contains an illegal character "":"".'"),
				,
				"Имя",
				,
				Отказ);
		...
		Пока Выборка.Следующий() Цикл
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'При обновлении ролей внешнего пользователя
				          ...
				          |was not found in metadata while updating roles of external user ""%1"".'"),
	...
	Для Каждого ПользователяДляОбновления Из РезультатЗапроса Цикл
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для пользователя %1 не может быть установлена почта %2 для восстановления пароля, т.к. она используется для пользователя %3(%4)';
				|en = 'The %2 email for password recovery cannot be set for the %1 user as it is used for the %3 (%4) user'"),
	...
	Исключение
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось обработать пользователя %1 по причине: %2';
				|en = 'Cannot process the %1 user due to: %2'"),
...
Функция ЗначенияПараметровСеансаТекущийПользователь()
	...
	ЗаголовокОшибки = НСтр("ru = 'Не удалось установить параметр сеанса ТекущийПользователь.';
							|en = 'Cannot set CurrentUser session parameter.'") + Символы.ПС;
	   ...
	   И БлокируемыеРеквизиты.Служебный <> ПользовательОбъект.Служебный Тогда
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка при записи пользователя ""%1"".
			           ...
			           |en = 'Cannot save user ""%1"".
			           |Attribute ""Internal"" cannot be changed in event subscriptions.'"),
			ПользовательОбъект.Ссылка);
	КонецЕсли;
	...
	Если БлокируемыеРеквизиты.Подготовлен <> ПользовательОбъект.Подготовлен Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка при записи пользователя ""%1"".
			           ...
			           |en = 'Cannot save user ""%1"".
			           |Attribute ""Prepared"" cannot be changed in event subscriptions.'"),
			ПользовательОбъект.Ссылка);
	КонецЕсли;
	...
	Если АвтоРеквизиты.ИдентификаторПользователяИБ <> ПользовательОбъект.ИдентификаторПользователяИБ Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка при записи пользователя ""%1"".
			           ...
			           |en = 'Cannot save user ""%1"".
			           |Attribute ""IBUserID"" cannot be changed.
	...
	Если Не ОбщегоНазначения.ДанныеСовпадают(АвтоРеквизиты.СвойстваПользователяИБ,
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Ошибка при записи пользователя ""%1"".
			           ...
			           |Обновление реквизита выполняется автоматически.';
			           |en = 'Cannot save user ""%1"".
			           |Attribute ""IBUserProperties"" cannot be changed.
			           |This attribute is populated automatically.'"),
			ПользовательОбъект.Ссылка);
	КонецЕсли;
