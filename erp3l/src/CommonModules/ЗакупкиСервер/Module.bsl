...
Функция ДополнительныеПараметрыОтбораДоговоров() Экспорт
	...
	Возврат Новый Структура("ВалютаВзаиморасчетов, Налогообложение, НаправлениеДеятельности");
...
Процедура СообщитьОбОшибкахОтменаПроведенияЕстьКорректировки(Знач ДокументЗакупки, Отказ) Экспорт
	...
	ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'На основании документа %1 введена корректировка приобретения. Отмена проведения невозможна.';
			|en = 'Debit note is registered for document %1. Cannot cancel posting.'"),
		ДокументЗакупки);
...
//  ДополнительныеПараметрыОтбораДоговоров - Структура - см ДополнительныеПараметрыОтбораДоговоров.
//
// Возвращаемое значение:
...
Процедура СообщитьОбОшибкахКорректностиВозвращаемыхТоваров(Знач ТаблицаОшибок)
	...
	Для каждого СтрокаОшибки Из ТаблицаОшибок Цикл
		СообщениеОбОшибке = НСтр("ru = 'Возврат по номенклатуре %Номенклатура% \ %Характеристика% превышает количество закупленных товаров по документу поступления %НомерРеализации% на %Количество% %ЕдиницаИзмерения%';
								|en = 'Returned quantity of  item %Номенклатура% \ %Характеристика% exceeds the quantity of product purchased against receipt document %НомерРеализации% for %Количество% %ЕдиницаИзмерения%'");
