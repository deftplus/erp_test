...
Функция ТекстЗапросаВТОтраженияВРеглУчете() Экспорт
	...
	|	ЕСТЬNULL(РезервыПоСомнительнымДолгам.ОбъектРасчетов.Контрагент, ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)) КАК Контрагент,
	|	ВЫБОР
	...
	|	ЕСТЬNULL(РезервыПоСомнительнымДолгам.ОбъектРасчетов.Контрагент, ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка)),
	|	ВЫБОР
	|		КОГДА РезервыПоСомнительнымДолгам.ОбъектРасчетов ССЫЛКА Справочник.ОбъектыРасчетов
	|		ТОГДА РезервыПоСомнительнымДолгам.ОбъектРасчетов.Договор
	|		ИНАЧЕ РезервыПоСомнительнымДолгам.ОбъектРасчетов
...
Функция ИзмененВидРазницВУчетнойПолитике(Период, Организация, ПараметрыУчетнойПолитики, ПараметрыУчетнойПолитикиНУ)Экспорт
	...
	//++ Локализация
	Если (ПараметрыУчетнойПолитики <> Неопределено
			И ПараметрыУчетнойПолитики.ФормироватьРезервыПоСомнительнымДолгамБУ)
		...
		Если ПараметрыУчетнойПолитики <> Неопределено Тогда
			Если ПараметрыУчетнойПолитики.ПериодичностьРезервовПоСомнительнымДолгамБУ = Перечисления.Периодичность.Месяц Тогда
				ПериодичностьБУ = 1;
			ИначеЕсли ПараметрыУчетнойПолитики.ПериодичностьРезервовПоСомнительнымДолгамБУ = Перечисления.Периодичность.Квартал Тогда
				ПериодичностьБУ = 3;
			КонецЕсли;
		КонецЕсли;
		...
		ЧислоМесяцев = Мин(ПериодичностьБУ, ПериодичностьНУ);
		...
		Если ПараметрыУчетнойПолитикиПрошлогоПериода <> Неопределено
			И ПараметрыУчетнойПолитики.ВидРазницПБУ18РезервовПоСомнительнымДолгам
	...
	Если Элемент.СписокВыбора.НайтиПоЗначению(
		...
		Элемент.СписокВыбора.Добавить(
			...
			НСтр("ru = 'дебиторской задолженности за счет резервов';
				|en = 'accounts receivable at the expense of reserves'",ОбщегоНазначения.КодОсновногоЯзыка()));
...
Функция ПериодРасчетаРезервовПоСомнительнымДолгам(Организация, Период) Экспорт
	...
	ФормироватьРезервыПоСомнительнымДолгамБУ = ?(НЕ ПараметрыУчетнойПолитики = Неопределено,
		ПараметрыУчетнойПолитики.ФормироватьРезервыПоСомнительнымДолгамБУ, Ложь);
	...
	Если ФормироватьРезервыПоСомнительнымДолгамБУ
...
Функция ТаблицаОценкиБУ(Организация, Период) Экспорт
	...
	//++ Локализация
	ТаблицаОценкиУчетнойПолитики = РегистрыСведений.УчетнаяПолитикаБухУчета.ПорядокОценкиЗадолженности(Организация, Период);
...
Функция ТекстЗапросаУчетнаяПолитикаБУ() Экспорт
	...
	|	ЗНАЧЕНИЕ(Перечисление.Периодичность.ПустаяСсылка) КАК ПериодичностьБУ
	...
	"ВЫБРАТЬ
	|	ГоловныеОрганизации.Организация КАК Организация,
	|	ТаблицаСрезПоследних.ФормироватьРезервыПоСомнительнымДолгамБУ КАК ФормироватьБУ,
	|	ТаблицаСрезПоследних.ПериодичностьРезервовПоСомнительнымДолгамБУ КАК ПериодичностьБУ
	|ПОМЕСТИТЬ ВтУчетнаяПолитикаБухУчета
	|ИЗ
	|	ВтГоловныеОрганизации КАК ГоловныеОрганизации
...
Функция ТекстЗапросаУчетныеПолитикиБУ() Экспорт
	...
	Результат =
	"ВЫБРАТЬ
	|	ЛОЖЬ КАК ФормироватьРезервыПоСомнительнымДолгамБУ,
	|	ЗНАЧЕНИЕ(Перечисление.Периодичность.ПустаяСсылка) КАК ПериодичностьРезервовПоСомнительнымДолгамБУ,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ЕСТЬNULL(УчетнаяПолитикаБУ.ФормироватьРезервыПоСомнительнымДолгамБУ, ЛОЖЬ) КАК ФормироватьРезервыПоСомнительнымДолгамБУ,
	|	УчетнаяПолитикаБУ.ПериодичностьРезервовПоСомнительнымДолгамБУ КАК ПериодичностьРезервовПоСомнительнымДолгамБУ,
...
Процедура ДополнитьПараметрыИнициализации(Запрос, Реквизиты) Экспорт
	...
	Запрос.УстановитьПараметр("ИзмененВидРазниц", НачислениеСписаниеРезервовПоСомнительнымДолгамЛокализация.ИзмененВидРазницВУчетнойПолитике(
		Реквизиты.Период, Реквизиты.Организация, ПараметрыУчетнойПолитики, ПараметрыУчетнойПолитикиНУ));
