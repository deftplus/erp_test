...
#Если ВебКлиент Тогда
	ВернутьРезультатПослеПоказаПредупреждения(ОбработчикРезультата, НСтр("ru = 'Очистка рабочего каталога не поддерживается в веб-клиенте.';
																		|en = 'Working directory cleanup is not supported in the web client.'"), Неопределено);
	Возврат;
#КонецЕсли
	...
	Исключение
		...
		ВызватьИсключение ТекстИсключения + Символы.ПС 
			+ НСтр("ru = 'В настройках Microsoft Word (меню Файл — Параметры) на закладке ""Общие"" снимите флажок ""Открывать вложения электронной почты в режиме чтения"".';
					|en = 'In Microsoft Word settings (menu File - Options), on the General tab, clear the ""Open e-mail attachments in Reading View"" check box.'");
	КонецПопытки;	
