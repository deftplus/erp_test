	...
	Если СхемаКомпоновкиДанных = Неопределено Тогда
		...
		Иначе
			ШаблонСообщения = НСтр("ru = 'Не определена схема компоновки данных в шаблоне ввода нефинансовых показателей ""%1"" для нефинансового показателя ""%2""';
									|en = 'The data composition schemа is not defined in the non-financial item profile ""%1"" for non-financial item ""%2""'");
			ШаблонСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
Процедура ПересчитатьТаблицуДанныхВВалюту(ТаблицаДанных, Параметры, СохранятьВалюту = Ложь, ПоляРесурсов = Неопределено) Экспорт
	...
	|			И КурсыВалют.БазоваяВалюта = &БазоваяВалютаПоУмолчанию
	...
	|	КурсыВалют.КурсЧислитель / КурсыВалют.КурсЗнаменатель КАК Курс
	|ИЗ
	...
	Запрос.УстановитьПараметр("БазоваяВалютаПоУмолчанию", ЗначениеНастроекПовтИсп.БазоваяВалютаПоУмолчанию());
	
	ПоляВалюта = Новый Соответствие;
	ПоляВалюта.Вставить("ПолеВалюта1", "ТаблицаДанных.Валюта");
	ПоляВалюта.Вставить("ПолеВалюта2", "ТаблицаДанных.Валюта КАК Валюта");
	ПоляВалюта.Вставить("ПолеВалюта3", "Валюты.Валюта");
	
	Если Параметры.ВозвращатьСуммуВВалюте Тогда
		Для каждого КлючИЗначение Из ПоляВалюта Цикл
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "&"+КлючИЗначение.Ключ, КлючИЗначение.Значение);
		КонецЦикла;
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ПолеУсловие1", "ТаблицаДанных.ВалютаХранения <> ТаблицаДанных.Валюта");
	Иначе
		Для каждого КлючИЗначение Из ПоляВалюта Цикл
			Запрос.Текст = СтрЗаменить(Запрос.Текст, "&"+КлючИЗначение.Ключ, "//");
		КонецЦикла;
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&ПолеУсловие1", "ИСТИНА");
	КонецЕсли;
	...
	Если ТаблицаДанных.Колонки.Найти("ТребуетсяПересчетПоКурсу") = Неопределено Тогда
		ВыражениеРасчетаКурсаВалютаОтчета = " * ЕстьNULL(" + "КурсыВалютыХранения" + "_" + "Курс, 1) / ЕстьNULL(КурсыВалютОтчета" + "_" + "Курс, 1)";
		ВыражениеРасчетаКурсаВалюта = " * ЕстьNULL(" + "КурсыВалютыХранения" +"_" + "Курс, 1) / ЕстьNULL(" + "КурсыВалют" + "_" + "Курс, 1)";
	Иначе
		ВыражениеРасчетаКурсаВалютаОтчета = " * ВЫБОР КОГДА ТребуетсяПересчетПоКурсу ТОГДА ЕстьNULL(" + "КурсыВалютыХранения" + "_" + "Курс, 1) / ЕстьNULL(" + "КурсыВалютОтчета" + "_" + "Курс, 1) ИНАЧЕ 1 КОНЕЦ";
		ВыражениеРасчетаКурсаВалюта = " * ВЫБОР КОГДА ТребуетсяПересчетПоКурсу ТОГДА ЕстьNULL(" + "КурсыВалютыХранения" + "_" + "Курс, 1) / ЕстьNULL(" + "КурсыВалют" + "_" + "Курс, 1) ИНАЧЕ 1 КОНЕЦ";
	КонецЕсли;
	...
	Для каждого Поле Из ПоляЗначенияНефинансовогоПоказателя Цикл
		Если НаборТаблицаФакт.Поля.Найти(Поле) = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		Выражение = Поле + " * ЕстьNULL(" + "КурсыВалютыХранения" + "_" + "Курс, 1) / ЕстьNULL(" + "КурсыВалют" + "_" + "Курс, 1)";
		ФинансоваяОтчетностьСервер.НовыйВычисляемыйРесурс(СхемаКомпоновкиДанных, Поле, Выражение, "Сумма");
		КомпоновкаДанныхКлиентСервер.ДобавитьВыбранноеПоле(НастройкиКомпоновкиДанных, Поле);
	КонецЦикла;
...
Функция УстановитьНаиболееПодходящиеПоляЗаполненияСуммы(ИсточникиСуммы, СхемаКомпоновкиДанных, Настройки, Правило, Параметры)
	...
	ИначеЕсли Параметры.ВалютаОтчета = Параметры.ВалютаМеждународ 
		И НЕ ПолучитьФункциональнуюОпцию("ИспользоватьНесколькоПлановСчетовВМеждународномУчете")
	...
	ИначеЕсли Параметры.ВалютаОтчета = Параметры.ВалютаМеждународ 
		...
		ВыражениеВалюты = "МеждународныйУчетСерверПовтИсп.ВалютаФункциональная(ПланСчетовМеждународногоУчета, Организация)";
		ДобавитьПоляСуммы(СхемаКомпоновкиДанных, Настройки, Выражение, ВыражениеВалюты, ВозвращатьСуммуВВалюте);
	...
	Если Не ПолеЗаполненияСуммыУстановлено Тогда
		Для каждого ИсточникСуммы Из ИсточникиСуммы Цикл
			...
			ИначеЕсли ИсточникСуммы.Ключ = "Международ" Тогда
				Если ПолучитьФункциональнуюОпцию("ИспользоватьНесколькоПлановСчетовВМеждународномУчете") Тогда
					Валюта = "МеждународныйУчетСерверПовтИсп.ВалютаФункциональная(ПланСчетовМеждународногоУчета, Организация)";
				Иначе
...
Функция ТаблицаИсточниковДанных(ИсточникиДанных)
	Возврат ОбщегоНазначенияУТ.МассивВТаблицуЗначений(ИсточникиДанных);
КонецФункции
...
Функция РассчитатьФактПоВидуБюджетаАльтернативный(ПараметрыДанных, Знач ИсточникиДанных, ВалютаОтчета, РежимПолученияДанных)
	...
	Для Каждого ЗапрошеннаяАналитика Из ЗапрошенныеВидыАналитик Цикл 
		...
		ПараметрыИдентификацииВыраженийОбороты.ЗначенияПоУмолчаниюДопДанные.Вставить("ИД_" + "ВыражениеИменованнойАналитики" + ИмяИменованнойАналитики);
		...
		ПараметрыИдентификацииВыраженийОстатки.ЗначенияПоУмолчаниюДопДанные.Вставить("ИД_" + "ВыражениеИменованнойАналитики" + ИмяИменованнойАналитики);
	...
	Для Каждого ИсточникДанных Из ИсточникиДанных Цикл
		...
		Иначе
			...
			Если ПолучатьФактическиеДанные Тогда
				...
				КонецЕсли;
				ДополнитьИсходныеВыраженияЗаполненияАналитик(ТаблицаПравилСтатей,
					ЗаполняемыеПараметры.ИсходныеВыраженияЗаполненияАналитик);
		...
		ИначеЕсли СтрокаДопДанных.ЗаполнениеВалюты = "ОпределяетсяПоИсточникуДанных" 
			...
			ИначеЕсли СтрокаДопДанных.РазделИсточникаДанных = Перечисления.РазделыИсточниковДанныхБюджетирования.МеждународныйУчет 
				И Не СтрокаДопДанных.ИсточникВалютный Тогда
					Если ПолучитьФункциональнуюОпцию("ИспользоватьНесколькоПлановСчетовВМеждународномУчете") Тогда
						ВыражениеЗаполненияПоля = "МеждународныйУчетСерверПовтИсп.ВалютаФункциональная(ПланСчетовМеждународногоУчета, Организация)";
					Иначе
						ВыражениеЗаполненияПоля = "&ВалютаМеждународ";
					КонецЕсли;
					ВВалютеИсточника = Ложь;
				//-- НЕ УТКА
			ИначеЕсли ПараметрыПолученияФакта.Свойство("ТребуетсяПересчетПоКурсуУпр")
				И ПараметрыПолученияФакта.ТребуетсяПересчетПоКурсуУпр Тогда
...
Функция ВыражениеЗаполненияПоказателяСумма(Знач ПараметрыПолученияФакта, Знач СтрокаДопДанных)
	...
	ИначеЕсли ПараметрыПолученияФакта.ВалютаОтчета = ПараметрыПолученияФакта.ВалютаМеждународ
		...
		// Валюта устанавливается в методе ВыражениеЗаполненияАналитикиВалюта
		ПолеЗаполненияСуммыУстановлено = Истина;
	...
	Если Не ПолеЗаполненияСуммыУстановлено Тогда
		Для каждого ИсточникСуммы Из СтрокаДопДанных.ВыраженияПоказателейСуммы Цикл
			...
			//++ НЕ УТКА
			НесколькоПлановСчетовМУ = ПолучитьФункциональнуюОпцию("ИспользоватьНесколькоПлановСчетовВМеждународномУчете");
			ТребуетсяПересчетПоКурсуМеждународ = ИсточникМеждународ
				И (НесколькоПлановСчетовМУ
					ИЛИ Не НесколькоПлановСчетовМУ И Не ПараметрыПолученияФакта.ВалютаОтчета = ПараметрыПолученияФакта.ВалютаМеждународ);
			//-- НЕ УТКА
