...
Процедура СоздатьЗаказПоставщикуНаОснованииПоГруппеСкладов(МассивСсылок, ПараметрыВыполнения) Экспорт
	...
	Если ПараметрыСоздания.НесколькоСкладов Тогда
		...
		СписокСкладов = ПараметрыСоздания.СписокСкладов;
		//++ НЕ УТКА
		Если ТипЗнч(ПараметрКоманды) = Тип("ДокументСсылка.ЭтапПроизводства2_2") Тогда
			
			СписокСкладов = Новый СписокЗначений;
			СписокСкладов.ЗагрузитьЗначения(ПараметрыСоздания.СписокСкладов);
			
			ЭлементСписка = СписокСкладов.НайтиПоЗначению(ПредопределенноеЗначение("Справочник.Склады.ПустаяСсылка"));
			Если ЭлементСписка <> Неопределено Тогда
				ЭлементСписка.Представление = НСтр("ru = '<указание склада не требуется>';
													|en = '<warehouse is not required>'");
			КонецЕсли;
		...
		ПараметрыОткрытия.Вставить("ЗначенияДляВыбора", СписокСкладов);
...
Процедура ОткрытьМастерВстречнойКорректировкиНазначения(ПараметрыВыполнения, ОписаниеОповещения = Неопределено) Экспорт
	...
	ПараметрыФормыЗаполнения.Вставить(
		"ВидОперации",
		ПредопределенноеЗначение("Перечисление.ВидыОперацийКорректировкиНазначения.ВстречнаяКорректировка"));
	
	ПараметрыФормыЗаполнения.Вставить("Мастер",      Истина);
	ПараметрыФормыЗаполнения.Вставить("Заказ",       ПараметрыВыполнения.ОбъектыОснований[0]);
	ПараметрыФормыЗаполнения.Вставить("Назначение",  ПараметрыВыполнения.Назначение);
	ПараметрыФормыЗаполнения.Вставить("ТоварыОтбор", ПараметрыВыполнения.ТоварыОтбор);
	ПараметрыФормыЗаполнения.Вставить("Количество",  ПараметрыВыполнения.Количество);
