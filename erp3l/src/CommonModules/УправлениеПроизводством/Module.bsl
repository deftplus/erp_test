	...
	Если РазмерыВременныхТаблиц.РасходыБезПередач = 0 Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			...
			НСтр("ru = 'Все передачи материалов в производство оформлены или оформление не требуется';
				|en = 'All material transfers to production are registered or registration is not required'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ОписаниеПроверок_ОформлениеПередачМатериаловВПроизводство(ТаблицаПроверок) Экспорт
	...
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru = 'Оформление передач материалов в производство.';
			|en = 'Register material transfers to production.'", ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru = 'В объеме израсходованных материалов по этапам производства должны быть оформлены передачи.';
			|en = 'Transfers must be registered for the consumed material volume by production stages.'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ПроверкаНеобходимостиОформлениеПередачМатериаловВПроизводство(ПараметрыПроверки) Экспорт
	...
	ПараметрыРегистрации = ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		...
		НСтр("ru = 'Обнаружены этапы производства, в которых отражен расход, при этом передача материалов не оформлена. Организация ""%1"", период %2';
			|en = 'Production stages with recorded expenses were found, material transfer is not registered. Company %1, period %2'", ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей);
...
Процедура УстановитьУсловноеОформлениеВыходныхИзделийЭтапаПроизводства(Форма, ИмяОбъекта, ИмяТЧ, ИменаЭлементов, ПутиКДанным = Неопределено) Экспорт
	...
	УстановитьУсловноеОформлениеПодразделениеПолучатель(Форма, ИмяОбъекта, ИмяТЧ, , ПутиКДанным);
...
Процедура УстановитьУсловноеОформлениеНазначенияВыходныхИзделийЭтапаПроизводства(Форма, ПутиКДанным = Неопределено) Экспорт
	...
	ОтборЭлемента.ЛевоеЗначение = ПолеКомпоновкиДанныхПоПути("Объект", ПутиКДанным, "ХозяйственнаяОперация");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ОтборЭлемента.ПравоеЗначение = Перечисления.ХозяйственныеОперации.ПроизводствоИзДавальческогоСырья;
...
Функция ИменаЭлементовОбеспеченияЭтапаПроизводства() Экспорт
...
//  ИмяОбъекта       - Строка - имя объекта.
//  ИмяТЧ            - Строка - имя табличной части.
//  ИменаЭлементов   - Структура - названия элементов формы. Содержит поля:
//   * ИмяТЧ - Строка - 
//   * ДатаОтгрузки - Строка -
//   * ТЧНоменклатураЕдиницаИзмерения - Строка -
//   * ТЧХарактеристика - Строка -
//   * ТЧСерия - Строка -
//   * ТЧСпецификация - Строка -
//   * ТЧСклад - Строка -
//   * ТЧДатаОтгрузки - Строка -
//   * ТЧКоличествоУпаковок - Строка -
//   * ТЧОтменено - Строка -
//   * ТЧПричинаОтмены - Строка -
//   * ТЧОбособленно - Строка -
//   * ТЧСтатьяКалькуляции - Строка - 
//  ПутиКДанным      - Структура, Неопределено - пути к данным.
//
Процедура УстановитьУсловноеОформлениеОбеспеченияЭтапаПроизводства(Форма, ИмяОбъекта, ИмяТЧ, ИменаЭлементов, ПутиКДанным = Неопределено) Экспорт
...
Процедура УстановитьУсловноеОформлениеЭкономияМатериаловЭтапаПроизводства(Форма, ИмяОбъекта, ПутиКДанным = Неопределено) Экспорт
	...
	// Подразделение-получатель
	УстановитьУсловноеОформлениеПодразделениеПолучатель(Форма, ИмяОбъекта, "ЭкономияМатериалов", , ПутиКДанным);
...
Процедура УстановитьУсловноеОформлениеРасходМатериаловИРаботЭтапаПроизводства(Форма, ИмяОбъекта, ИмяТЧ, ИменаЭлементов, ПутиКДанным = Неопределено) Экспорт
	...
	УстановитьУсловноеОформлениеСтатьиКалькуляцииЭтапаПроизводства(Форма, ИмяОбъекта, ИмяТЧ, ИмяЭлемента, ПутиКДанным);
...
//
Процедура УстановитьУсловноеОформлениеСтатьиКалькуляцииЭтапаПроизводства(Форма, ИмяОбъекта, ИмяТЧ, ИмяЭлемента, ПутиКДанным = Неопределено, ЕстьОтменаСтрок = Ложь) Экспорт
...
Процедура СоздатьВТОграниченияГрафикаПроизводстваПоМатериалам(МенеджерВременныхТаблиц, Этапы, ВРазрезеНоменклатуры, ВРазрезеОбеспечивающихЭтапов) Экспорт
	...
	ТекстЗапроса =
		...
		|							КОНЕЦ
		|					КОГДА Обеспечение.Состояние = ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.ОтложитьОбеспечение)
		|						ТОГДА Обеспечение.ЖелаемаяДатаОтгрузки
...
Функция РассчитатьОграниченияГрафикаПроизводстваПоМатериалам(Распоряжение) Экспорт
	...
	ТекстЗапроса =
		...
		|	ВЫБРАТЬ
		|		Таблица.ЗаказНаОтгрузку      КАК ЭтапПроизводства,
		|		Таблица.ЖелаемаяДатаОтгрузки КАК ДатаОбеспечения
		|	ИЗ
		|		ВтРаспределение КАК Таблица
		|	ГДЕ
		|		Таблица.Состояние = ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.ОтложитьОбеспечение)
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		|	ВЫБРАТЬ
