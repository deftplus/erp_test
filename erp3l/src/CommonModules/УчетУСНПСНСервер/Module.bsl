...
Функция ПараметрыДляПроведенияДокумента(Документ, Свойства) Экспорт
	...
	Если Не УчетнаяПолитика.ЗаполнятьКУДиР(Документ.Организация, Документ.Дата)
		И Не (ТипЗнч(Документ) = Тип("ДокументОбъект.ВзаимозачетЗадолженности")
			И УчетнаяПолитика.ЗаполнятьКУДиР(Документ.КонтрагентДебитор, Документ.Дата)) Тогда
		Возврат Параметры;
	КонецЕсли;
...
Функция ТекстЗапросаТаблицаДействующихПатентовПодразделениям(Запрос, ТекстыЗапроса, ТолькоУникальные = Истина) Экспорт
	...
	|	Подразделения.СтруктурнаяЕдиница КАК Подразделение
	...
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ПодчиненностьСтруктурныхЕдиниц КАК Подразделения
...
Функция ТекстЗапросаСторноДоходов() Экспорт
	...
	ЗапросыВременныхТаблиц.Добавить("УНИЧТОЖИТЬ ТаблицаПродажИПлатежейОтБанка");
...
Функция ТекстЗапросаВыборкиДокументовДляЗаполненияСторно() Экспорт
	...
	|	ТаблицаРегистраторовДоходов.Патент КАК ПатентПродажи,
	|	ТаблицаРегистраторовДоходов.ДокументВозникновенияДоходов КАК Платеж,
	|	КнигаУчетаДоходовИРасходов.ДокументВозникновенияДоходовРасходов КАК ДокументВозникновенияДоходовРасходов,
	|	КнигаУчетаДоходовИРасходов.Статья КАК Статья,
	|	ВЫБОР КОГДА НАЧАЛОПЕРИОДА(ТаблицаРегистраторовДоходов.Период, Квартал) = НАЧАЛОПЕРИОДА(КнигаУчетаДоходовИРасходов.Период, Квартал)
	|		ТОГДА КнигаУчетаДоходовИРасходов.Содержание ИНАЧЕ """" КОНЕЦ КАК Содержание,
	|	ВЫБОР КОГДА НАЧАЛОПЕРИОДА(ТаблицаРегистраторовДоходов.Период, Квартал) = НАЧАЛОПЕРИОДА(КнигаУчетаДоходовИРасходов.Период, Квартал)
	|		ТОГДА КнигаУчетаДоходовИРасходов.РеквизитыПервичногоДокумента ИНАЧЕ """" КОНЕЦ КАК РеквизитыПервичногоДокумента,
	|	КнигаУчетаДоходовИРасходов.Патент КАК Патент,
	|	(ТаблицаРегистраторовДоходов.СуммаНДС) - ЕСТЬNULL(КнигаУчетаДоходовИРасходовОРП.НДС, 0) КАК СуммаНДС,
	|	(ТаблицаРегистраторовДоходов.СуммаВсего) - ЕСТЬNULL(КнигаУчетаДоходовИРасходовОРП.Графа4, 0) КАК СуммаВсего,
	|	(ТаблицаРегистраторовДоходов.СуммаУСН) - ЕСТЬNULL(КнигаУчетаДоходовИРасходовОРП.Графа5, 0) КАК СуммаУСН,
	|	(ТаблицаРегистраторовДоходов.СуммаЕНВД) - ЕСТЬNULL(КнигаУчетаДоходовИРасходовОРП.ДоходЕНВД, 0) КАК СуммаЕНВД,
	|	(ТаблицаРегистраторовДоходов.СуммаПатент) - ЕСТЬNULL(КнигаУчетаДоходовИРасходовОРП.ДоходПатент, 0) КАК СуммаПатент,
	|	(ТаблицаРегистраторовДоходов.СуммаКомиссии) - ЕСТЬNULL(КнигаУчетаДоходовИРасходовОРП.Графа4 - КнигаУчетаДоходовИРасходовОРП.Графа5 - КнигаУчетаДоходовИРасходовОРП.ДоходЕНВД - КнигаУчетаДоходовИРасходовОРП.ДоходПатент, 0) КАК СуммаНеПризнано,
	|	(ТаблицаРегистраторовДоходов.СуммаОплаты) КАК СуммаОплаты,
	|	(ВЫБОР КОГДА КнигаУчетаДоходовИРасходовОРП.Регистратор ЕСТЬ NULL ТОГДА КнигаУчетаДоходовИРасходов.Графа4 ИНАЧЕ ТаблицаРегистраторовДоходов.СуммаПлатежа КОНЕЦ) КАК СуммаПлатежа,
	|	(КнигаУчетаДоходовИРасходов.НДС) КАК ДоходНДС,
	|	(КнигаУчетаДоходовИРасходов.Графа4) КАК ДоходВсего,
	|	(КнигаУчетаДоходовИРасходов.Графа5) КАК ДоходУСН,
	|	(КнигаУчетаДоходовИРасходов.ДоходЕНВД) КАК ДоходЕНВД,
	|	(КнигаУчетаДоходовИРасходов.ДоходПатент) КАК ДоходПатент,
	|	(КнигаУчетаДоходовИРасходов.Графа4 - КнигаУчетаДоходовИРасходов.Графа5 - КнигаУчетаДоходовИРасходов.ДоходЕНВД - КнигаУчетаДоходовИРасходов.ДоходПатент) КАК ДоходНеПризнано,
	|	ЕСТЬNULL(ДанныеПервичныхДокументов.Номер, """") КАК НомерВходящегоДокумента,
	...
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.КнигаУчетаДоходовИРасходов КАК КнигаУчетаДоходовИРасходовОРП
	...
	|	И ТаблицаРегистраторовДоходов.ДокументВозникновенияДоходов = КнигаУчетаДоходовИРасходовОРП.Регистратор
	...
	|	И НЕ КнигаУчетаДоходовИРасходов.Регистратор ССЫЛКА Документ.ОтчетОРозничныхПродажах
	|	И НЕ КнигаУчетаДоходовИРасходов.Регистратор ССЫЛКА Документ.ОтчетОРозничныхВозвратах
	|	ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ДанныеПервичныхДокументов КАК ДанныеПервичныхДокументов
	...
	|	СРЕДНЕЕ(СуммаЕНВД),
	|	СРЕДНЕЕ(СуммаПатент),
	|	СРЕДНЕЕ(СуммаНеПризнано),
	|	СРЕДНЕЕ(СуммаОплаты),
	|	СРЕДНЕЕ(СуммаПлатежа),
	|	СРЕДНЕЕ(ДоходВсего),
	|	СРЕДНЕЕ(ДоходУСН),
	|	СРЕДНЕЕ(ДоходЕНВД),
	|	СРЕДНЕЕ(ДоходПатент),
	|	СРЕДНЕЕ(ДоходНеПризнано)
	|ПО
	|	Регистратор,
	|	Платеж";
...
Процедура РассчитатьНалогУСНОбъектДоходы(КонтекстРасчета, ТаблицаПроводок, ТаблицаСправкиРасчета, Ошибки)
	...
	СкорректироватьНалоговуюСтавку(КонтекстРасчета, ДанныеПрогрессивнойШкалыУСН, Ошибки);
	...
	НалоговаяБазаЗаПериод = Макс(0, Доходы - ДанныеПрогрессивнойШкалыУСН.НалоговаяБазаДоПревышенияЛимитов);
	НалогЗаПериод = Окр(НалоговаяБазаЗаПериод * КонтекстРасчета.СтавкаНалогаУСН / 100, РазрядностьОкругленияПоказателей);
	НалогСНачалаГодаВсего = НалогЗаПериод + ДанныеПрогрессивнойШкалыУСН.НалогДоПревышенияЛимитов;
	...
	Если КонтекстРасчета.УменьшатьНалогНаТорговыйСбор Тогда
		...
		НалоговаяБазаЗаПериод = Макс(0, ДоходыТорговыйСбор - ДанныеПрогрессивнойШкалыУСН.НалоговаяБазаДоПревышенияЛимитовТорговыйСбор);
		НалогЗаПериод = Окр(НалоговаяБазаЗаПериод * КонтекстРасчета.СтавкаНалогаУСН / 100, РазрядностьОкругленияПоказателей);
		НалогТСВсего = НалогЗаПериод + ДанныеПрогрессивнойШкалыУСН.НалогДоПревышенияЛимитовТорговыйСбор;
...
Процедура РассчитатьНалогУСНОбъектДоходыМинусРасходы(КонтекстРасчета, ТаблицаПроводок, ТаблицаСправкаРасчет, Ошибки)
	...
	НалоговаяБазаЗаПериод = Макс(0, НалоговаяБаза - ДанныеПрогрессивнойШкалыУСН.НалоговаяБазаДоПревышенияЛимитов);
	НалогЗаПериод = Окр(НалоговаяБазаЗаПериод * КонтекстРасчета.СтавкаНалогаУСН / 100, РазрядностьОкругленияПоказателей);
	НалогСНачалаГода = НалогЗаПериод + ДанныеПрогрессивнойШкалыУСН.НалогДоПревышенияЛимитов;
...
Процедура СторнироватьОтраженныеДоходы(ВременныеТаблицы, КоличествоОбработанныхДанных = 0)
	...
	Пока ВыборкаДокументовСторнирования.Следующий() Цикл
		...
		СуммаОплатыПоДокументу = ВыборкаДокументовСторнирования.СуммаОплаты;
		...
		ВыборкаПлатежей = ВыборкаДокументовСторнирования.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		
		Пока ВыборкаПлатежей.Следующий() Цикл
			
			Если ЗначениеЗаполнено(ВыборкаПлатежей.Платеж) Тогда
				ДанныеПлатежа = Платежи.Получить(ВыборкаПлатежей.Платеж);
				УчтеноПоДокументу = Новый Структура("ДоходВсего, ДоходУСН, ДоходПатент, ДоходЕНВД, ДоходНеПризнано, СуммаПлатежа");
				ЗаполнитьЗначенияСвойств(УчтеноПоДокументу, ВыборкаПлатежей);
				ДанныеПлатежа = ?(ДанныеПлатежа = Неопределено, УчтеноПоДокументу, ДанныеПлатежа);
			КонецЕсли;
			СуммаОплаты = Мин(СуммаОплатыПоДокументу, ДанныеПлатежа.СуммаПлатежа, ВыборкаДокументовСторнирования.СуммаВсего);
			СуммаОплатыПоДокументу = СуммаОплатыПоДокументу - СуммаОплаты;
			...
			Пока Выборка.Следующий() И СуммаОплаты > 0 Цикл
				...
				ДоходыПоДокументу = Новый Структура("СуммаВсего, СуммаУСН, СуммаПатент, СуммаЕНВД, СуммаНеПризнано");
				...
				СтруктураКорректировки = Новый Структура("УСН, ЕНВД, Патент", 0, 0, 0);
				...
				Для каждого ЭлементКорректировки Из СтруктураКорректировки Цикл
					...
					Если Знак * ДанныеПродажи["Сумма"+ЭлементКорректировки.Ключ] > 0 И Знак * ДанныеПродажи.СуммаНеПризнано > 0 Тогда
						СуммаУменьшения = Мин(Знак * ДанныеПродажи["Сумма"+ЭлементКорректировки.Ключ], Знак * ДанныеПродажи.СуммаНеПризнано);
						ДанныеПродажи["Сумма"+ЭлементКорректировки.Ключ] = Знак * ДанныеПродажи["Сумма"+ЭлементКорректировки.Ключ] - СуммаУменьшения;
					КонецЕсли;
					СуммаДокумента = Мин(ДанныеПродажи["Сумма"+ЭлементКорректировки.Ключ], СуммаОплаты);
					СуммаПлатежа = Мин(Знак * ДанныеПлатежа["Доход"+ЭлементКорректировки.Ключ], СуммаОплаты);
					
					Если СуммаДокумента > 0 И СуммаПлатежа = 0 Тогда
						ЗначениеКорректировки = ЗначениеКорректировки + СуммаДокумента;
						ДанныеПродажи["Сумма"+ЭлементКорректировки.Ключ] = ДанныеПродажи["Сумма"+ЭлементКорректировки.Ключ] - ЗначениеКорректировки;
					КонецЕсли;
					Если СуммаПлатежа > СуммаДокумента И Знак * ДанныеПродажи.СуммаВсего > 0
						И ДанныеПродажи["Сумма"+ЭлементКорректировки.Ключ] <> ДанныеПродажи.СуммаВсего Тогда
						ЗначениеКорректировки = ЗначениеКорректировки - Мин(Знак * ДанныеПродажи.СуммаВсего,
							СуммаОплаты - СуммаДокумента, СуммаПлатежа - СуммаДокумента);
						УчтеннаяСумма = ЗначениеКорректировки;
					КонецЕсли;
					
					СтруктураКорректировки[ЭлементКорректировки.Ключ] = ЗначениеКорректировки;
					
				КонецЦикла;
				
				Если Знак * ДанныеПродажи.СуммаНеПризнано > 0 Тогда
					ДанныеПродажи.СуммаНеПризнано = Знак * ДанныеПродажи.СуммаНеПризнано - Мин(СуммаОплаты, СуммаПлатежа, Знак * ДанныеПродажи.СуммаВсего);
				КонецЕсли;
				ДанныеПродажи.СуммаВсего = Знак * ДанныеПродажи.СуммаВсего + УчтеннаяСумма;
				СуммаОплаты = СуммаОплаты + УчтеннаяСумма;
				
				Если СтруктураКорректировки.УСН <> 0 ИЛИ СтруктураКорректировки.ЕНВД <> 0 ИЛИ СтруктураКорректировки.Патент <> 0 Тогда
					
					ЗаписьСторноПриход = НаборЗаписей.Добавить();
					ЗаполнитьЗначенияСвойств(ЗаписьСторноПриход, Выборка);
					
					ЗаписьСторноПриход.Патент = Выборка.ПатентПродажи;
					...
					Если ЗначениеЗаполнено(Выборка.ДокументВозникновенияДоходовРасходов) ИЛИ Выборка.Содержание = "" Тогда
						ПараметрыСодержанияДохода = ПараметрыСодержанияДохода();
					...
					КонецЕсли;
					Если ЗначениеЗаполнено(Выборка.ДокументВозникновенияДоходовРасходов) ИЛИ Выборка.РеквизитыПервичногоДокумента = "" Тогда
						ЗаписьСторноПриход.РеквизитыПервичногоДокумента = РеквизитыПервичногоДокумента(Выборка.Регистратор, Выборка);
					КонецЕсли;
					
				КонецЕсли;
				
				ДанныеПлатежа.СуммаПлатежа = ДанныеПлатежа.СуммаПлатежа + УчтеннаяСумма;
				
				Платежи.Вставить(Выборка.Платеж, ДанныеПлатежа);
...
Функция ТекстЗапросаПродаж()
	...
	|	СУММА(ВыручкаИСебестоимостьПродаж.СуммаВыручкиСНДСРегл) КАК СуммаВсего,
	...
	|		ТОГДА ВыручкаИСебестоимостьПродаж.СуммаВыручкиСНДСРегл ИНАЧЕ 0 КОНЕЦ) КАК СуммаПатент,
...
Функция ТекстЗапросаДоходовДляАнализаНеобходимостиСторно()
	...
	|	ВЫБОР
	|		КОГДА РасчетыСКлиентами.Регистратор ССЫЛКА Документ.ВзаимозачетЗадолженности
	...
	|	Аналитика.Организация КАК Организация,
	|	СУММА(ЕСТЬNULL(Взаимозачеты.ДолгРегл, РасчетыСКлиентами.ДолгРегл)) КАК СуммаОплаты
	...
	|		ПО (РасчетыСКлиентами.Регистратор ССЫЛКА Документ.ВзаимозачетЗадолженности)
	|			И (РасчетыСКлиентами.ПредоплатаРегл <> 0)
	|			И (РасчетыСКлиентами.Регистратор = Взаимозачеты.Регистратор)
	|			И (РасчетыСКлиентами.РасчетныйДокумент <> Взаимозачеты.РасчетныйДокумент)
	|ГДЕ
	|	Аналитика.Организация В (&МассивОрганизаций)
	|	И РасчетыСКлиентами.Период МЕЖДУ &НачДата И &КонДата
	|	И НЕ РасчетыСКлиентами.Регистратор ССЫЛКА Документ.РасчетКурсовыхРазниц
	|	И РасчетыСКлиентами.РасчетныйДокумент <> РасчетыСКлиентами.Регистратор
	|	И (НЕ Взаимозачеты.АналитикаУчетаПоПартнерам ЕСТЬ NULL ИЛИ НЕ РасчетыСКлиентами.Регистратор ССЫЛКА Документ.ВзаимозачетЗадолженности)
	|	
	|СГРУППИРОВАТЬ ПО
	...
	Если ПолучитьФункциональнуюОпцию("НоваяАрхитектураВзаиморасчетов") Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "РасчетыСКлиентамиПоДокументам", "РасчетыСКлиентамиПоСрокам");
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, ".Регистратор", ".ДокументРегистратор");
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ЕСТЬNULL(Взаимозачеты.ДолгРегл, РасчетыСКлиентами.ДолгРегл)",
			"ВЫБОР КОГДА РасчетыСКлиентами.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗачетАвансаКлиента)
			|		ТОГДА РасчетыСКлиентами.ПредоплатаРегл ИНАЧЕ РасчетыСКлиентами.ДолгРегл КОНЕЦ + ЕСТЬNULL(Взаимозачеты.ДолгРегл, 0)");
	Иначе
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "РасчетыСКлиентами.РасчетныйДокумент <> РасчетыСКлиентами.ДокументРегистратор", "ИСТИНА");
	КонецЕсли;
...
Функция ТекстЗапросаПродажИПлатежейКлиентам()
	...
	|	СРЕДНЕЕ(Продажи.СуммаВсего - Продажи.СуммаЕНВД - Продажи.СуммаПатент) КАК СуммаУСН,
	...
	|	СУММА(РасчетыСКлиентами.ПредоплатаРегл) КАК СуммаОплаты
	...
	|			И Продажи.Период = РасчетыСКлиентами.Период
	|			И (РасчетыСКлиентами.Регистратор <> РасчетыСКлиентами.РасчетныйДокумент)
	|
	|СГРУППИРОВАТЬ ПО
	|	РасчетыСКлиентами.Период,
	...
	|	СУММА(Продажи.СуммаВыручкиСНДСРегл) КАК СуммаВсего,
	|	СУММА(ВЫБОР
	|			КОГДА ОрганизацииНаЕНВД.Организация ЕСТЬ NULL
	|				ИЛИ Продажи.НалогообложениеНДС = ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПродажаПоПатенту)
	...
	Если ПолучитьФункциональнуюОпцию("НоваяАрхитектураВзаиморасчетов") Тогда
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "РегистрНакопления.РасчетыСКлиентамиПоДокументам", "ДоходыДляПроверкиНаСторно");
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ПредоплатаРегл", "СуммаОплаты");
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Продажи.Период = РасчетыСКлиентами.Период", "ИСТИНА");
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "(РасчетыСКлиентами.Регистратор <> РасчетыСКлиентами.РасчетныйДокумент)", "ИСТИНА");
	КонецЕсли;
...
Функция ТекстЗапросаПродажИПлатежейОтБанка()
	...
	|	РозничныеПродажи.СуммаВсего - РозничныеПродажи.СуммаЕНВД - РозничныеПродажи.СуммаПатент КАК СуммаУСН,
	...
	|			И (ПродажиВСчетПлатежейОтБанка.Период МЕЖДУ &НачДата И &КонДата)
	|			И (ПродажиВСчетПлатежейОтБанка.ВидПереводаДенежныхСредств = ЗНАЧЕНИЕ(Перечисление.ВидыПереводовДенежныхСредств.ПоступлениеОтБанкаПоЭквайрингу))
	|			И (ПродажиВСчетПлатежейОтБанка.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход))
...
Функция ТекстЗапросаРегистраторовДоходов()
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ДенежныеСредстваВПути КАК ПлатежиОтБанка
	...
	|			И ПлатежиОтБанка.Период <= &КонДата
...
Функция РезультатОплатыРасходов(Данные, ТаблицаОстатков, СтруктураПоискаОстатка = Неопределено)
	...
	Для каждого СтрокаОстатка Из СтрокиОстатка Цикл
		...
		Если (ОсталосьОплатитьПоТекущейВыборке > 0 ИЛИ ОплатаВнеЗависимостиОтРасходов) И СуммаОплаты > 0 Тогда
			...
			Если Не Перечисления.ВидыРасходовУСН.ПризнаетсяКакОСНМА(СтрокаОстатка.ВидРасходов) И Данные.Период <= Данные.ДатаОплаты Тогда
...
Функция РезультатВозвратаРасходов(Данные, ТаблицаОстатков, СтруктураПоискаОстатка = Неопределено)
	...
	Если Данные.ВидРасходов = Перечисления.ВидыРасходовУСН.Услуги Тогда
...
Функция РезультатСписанияРасходов(Данные, ТаблицаОстатков, СтруктураПоискаОстатка = Неопределено)
	...
	Для каждого СтрокаОстатка Из СтрокиОстатка Цикл
		...
		Если ОсталосьОтгрузить > 0 И Не РеклассификацияРасходов Тогда
			...
			Если СуммаНеПризнанияЕНВД <> 0 Или СуммаНеПризнания <> 0 Или СуммаПризнания <> 0 Тогда
...
Функция ПризнанныеРасходыПоАмортизации(ПараметрыЗапроса)
	...
	|ИЗ
	|	РегистрСведений.ПервоначальныеСведенияОС.СрезПоследних(&КонГраница, Организация = &Организация) КАК ПервоначальныеСведения
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПорядокУчетаОСБУ.СрезПоследних(&КонГраница, Организация = &Организация) КАК ПорядокУчетаОСБУ
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПараметрыАмортизацииОСБУ.СрезПоследних(&КонГраница, Организация = &Организация) КАК ПараметрыАмортизацииОСБУ
...
Функция ТекстЗапросаРасходовПоСебестоимостиПредварительная()
	...
	|	Партии.ТипЗаписи В (&ТипыЗаписейСебестоимостиДопРасходов) И НЕ Партии.Партия ССЫЛКА Документ.ОтчетПереработчика
	...
	|			ТОГДА ""ВозвратКлиента""
	|		КОГДА Партии.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход) И (Партии.Регистратор = Партии.Партия
	|			И НЕ (Партии.ТипЗаписи В (&ТипыЗаписейСебестоимостиДопРасходов) И НЕ Партии.Регистратор ССЫЛКА Документ.ОтчетПереработчика)
	|			ИЛИ Партии.Партия = Неопределено И Партии.Регистратор = Партии.ДокументПоступления И Партии.ТипЗаписи = ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.Партия)
	|			ИЛИ Партии.Сторно И (Партии.ДокументИсточник = Партии.Партия ИЛИ Партии.Партия = Неопределено
	|				И Партии.ДокументИсточник = Партии.ДокументПоступления И Партии.ТипЗаписи = ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.Партия)))
	|			ТОГДА ""Приход""
	...
	|		И ВЫБОР КОГДА Партии.Регистратор ССЫЛКА Документ.ОтчетПереработчика
	...
	|	И НЕ ТИПЗНАЧЕНИЯ(Партии.ДокументПоступления) В (&ИсключающиесяДокументыПоступленияСебестоимости)
	|	И Партии.ТипЗаписи <> ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.Перемещение)
	|	И (НЕ Партии.Партия ССЫЛКА Документ.ОтчетПереработчика ИЛИ Партии.ДокументИсточник = НЕОПРЕДЕЛЕНО)
	|	И (Партии.ХозяйственнаяОперация <> ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияБезПереходаПраваСобственности)
	|		ИЛИ НЕ ПартииРеализацийБезПереходаПраваСобственности.Партия ЕСТЬ NULL)
	|
	|СГРУППИРОВАТЬ ПО
...
Функция ТекстЗапросаПрочихРасходов()
	...
	|			ИНАЧЕ ""Приход""
	|		КОНЕЦ,
	|		ЕСТЬNULL(ОбъектыРасчетов.Объект, ЕСТЬNULL(ДокСторно.СторнируемыйДокумент,
	|			ЕСТЬNULL(КорректировкаПриобретения.ДокументОснование, ДанныеРегистра.Регистратор))),
...
Функция ТекстЗапросаТаблицыРегистраторовРасходов()
	...
	|		ИЛИ ДокументыОплаты.Регистратор ССЫЛКА Документ.СписаниеЗадолженности) И ДокументыОплаты.Регистратор <> ДокументыОплаты.ДокументВозникновенияРасходов
...
Функция СодержаниеРасхода(Данные, ВидРасходов)
	...
	Если Данные.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.СписаниеКредиторскойЗадолженности
		ИЛИ ТипЗнч(Данные.Регистратор) = Тип("ДокументСсылка.СписаниеЗадолженности") Тогда
...
Функция ИнициализироватьЗапросПоПараметрам(ПараметрыЗапроса, ВременныеТаблицы)
	...
	РасчетСебестоимостиНДС.ИнициализироватьДополнительныеПараметрыЗапросаДетализацииПартийНДС(Запрос);
...
Процедура СкорректироватьНалоговуюСтавку(КонтекстРасчета, ДанныеПрогрессивнойШкалыУСН, Ошибки)
	...
	Если ДанныеПрогрессивнойШкалыУСН.ПрименяетсяПовышеннаяСтавка И КонтекстРасчета.СтавкаНалогаУСН <= СтавкаУСНПоУмолчанию Тогда
		// Должна применять повышенная налоговая ставка, но в учетной политике пока прописана стандартная:
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура = "УчетУСНПСНСервер.ДатаНачалаПризнанияДоходовОтчетомОРозничныхПродажах_ОбработатьДанныеДляПереходаНаНовуюВерсию";
	...
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "УчетУСНПСНСервер.ДатаНачалаПризнанияДоходовОтчетомОРозничныхПродажах_ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию";
	...
	Обработчик.Комментарий = НСтр("ru = 'Установка константы ДатаНачалаПризнанияДоходовОтчетомОРозничныхПродажах. Влияет на движения в регистре накопления КнигаУчетаДоходовИРасходов для документов ОтчетОРозничныхПродажах (начинает делать движения) и ПриходныйКассовыйОрдер с хозяйственной операцией ПоступлениеДенежныхСредствИзКассыККМ (прекращает делать движения)';
									|en = 'Setting of ДатаНачалаПризнанияДоходовОтчетомОРозничныхПродажах. constant affects the records in КнигаУчетаДоходовИРасходов accumulation register for ОтчетОРозничныхПродажах (starts making records) and ПриходныйКассовыйОрдер documents with ПоступлениеДенежныхСредствИзКассыККМ business transaction (stops making records)'");
	...
	Читаемые.Добавить(Метаданные.РегистрыСведений.НастройкиСистемыНалогообложения.ПолноеИмя());
	...
	Блокируемые.Добавить(Метаданные.Константы.ДатаНачалаПризнанияДоходовОтчетомОРозничныхПродажах.ПолноеИмя());
...
Процедура ДобавитьПояснениеПоНалоговомуПериоду(ПараметрыОбработчика, ИмяТаблицыОтбораОрганизаций = "")
	...
	Если Не Результат.Пустой() Тогда
		...
		ЗакрытиеМесяцаСервер.ДобавитьПоясняющуюИнформациюКЭтапу(ПараметрыОбработчика, ТекстСообщения, , , Перечисления.ВариантыВажностиПроблемыСостоянияСистемы.ПолезныйСовет);
