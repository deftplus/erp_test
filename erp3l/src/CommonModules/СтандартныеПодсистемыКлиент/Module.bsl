...
Процедура ПриВыполненииСтандартныхПериодическихПроверок() Экспорт
	...
	ПоказатьОповещениеПользователя(НСтр("ru = 'Установлено обновление программы';
										|en = 'Application update is installed'"), "e1cib/app/ОбщаяФорма.КонтрольДинамическогоОбновления",
	...
	Если ПараметрыЗапуска.Количество() < 2 Тогда
		ВызватьИсключение
			НСтр("ru = 'При указании параметра запуска ВойтиВОбластьДанных,
			           ...
			           |en = 'When specifying the SignInToDataArea startup parameter,
			           |specify the separator value as an additional parameter.'");
	КонецЕсли;
	...
	Исключение
		ВызватьИсключение
			НСтр("ru = 'Значением разделителя в параметре ВойтиВОбластьДанных должно быть число.';
				|en = 'Only a number can be passed as a separator value in the SignInToDataArea startup option.'");
	КонецПопытки;
