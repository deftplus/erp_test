...
Процедура ПриНажатии(Форма, Элемент, СтандартнаяОбработка, Оповещение = Неопределено) Экспорт
	...
	Если НЕ ДанныеНастройкиСчетовУчета.ЗначенияФункциональныхОпций.ИспользоватьМФУ Тогда
		ДоступнаНастройкаМеждународногоУчетаПоСтатье = Ложь;
	ИначеЕсли НЕ ЗначениеЗаполнено(ПараметрыНастройки.СтатьяАктивовПассивов)
		...
		Для Каждого ДанныеМеждународнойНастройки Из ДанныеНастройки.НастройкиМеждународногоУчета Цикл
			Если ЗначениеЗаполнено(ДанныеМеждународнойНастройки.СчетУчета) Тогда
				ДоступнаНастройкаМеждународногоУчетаПоСтатье = Истина;
				Прервать;
			КонецЕсли;
		КонецЦикла;
		//-- НЕ УТКА
	...
	ПараметрыФормыНастройки.Вставить("ДоступнаНастройкаМеждународногоУчетаПоСтатье", ДоступнаНастройкаМеждународногоУчетаПоСтатье);
