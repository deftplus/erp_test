...
Процедура ДобавитьОписаниеПолейАналитикиФинансовогоУчета(МетаданныеРегистра, ОписаниеОперации, ЕстьГруппаПродукции = Истина)
	...
	Если ЕстьГруппаПродукции Тогда	
		ДобавитьОписаниеПоляОперации(
			МетаданныеРегистра,
			ОписаниеОперации.Поля,
			"ГруппаПродукции",
...
Процедура ОтразитьОперациюСебестоимостиВнешнееПоступление(ПараметрыОтражения)
	...
	|	ВЫБОР КОГДА &Константа_ПартионныйУчетВерсии22
	|	  И ДанныеДокумента.Период >= НАЧАЛОПЕРИОДА(&Константа_ДатаПереходаНаПартионныйУчетВерсии22, МЕСЯЦ)
	|	  И НЕ ДанныеДокумента.АналитикаУчетаНоменклатуры.СкладскаяТерритория.ЦеховаяКладовая
	...
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ТипыЗаписейПартий.ДопРасходыСПартией)
	// ТоДо изменения из проекта "Новый учет товаров у переработчика", не удалять!
...
//	|		  ТИП(Документ.ОтчетПереработчика2_5),
	...
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыПереданныеПереработчику)
	// ТоДо изменения из проекта "Новый учет товаров у переработчика", не удалять!
	//|		КОГДА ТИПЗНАЧЕНИЯ(ДанныеДокумента.Ссылка) В (ТИП(Документ.ОтчетПереработчика2_5))
	//|			ТОГДА
	//|				ВЫБОР КОГДА ДанныеДокумента.ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ЗакупкаУПоставщика)
	//|				  ИЛИ ДанныеДокумента.Количество < 0
...
Процедура ОтразитьОперациюСебестоимостиСписаниеНаДругиеПартии(ПараметрыОтражения)
	...
	|		  ИЛИ ДанныеДокумента.АналитикаУчетаНоменклатуры.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Услуга))
	//++ НЕ УТ
	|		  И ТИПЗНАЧЕНИЯ(ДанныеДокумента.Ссылка) <> ТИП(Документ.РаспределениеПроизводственныхЗатрат)
	//-- НЕ УТ
	|			ТОГДА ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ПроизводственныеЗатраты)
	|		КОГДА ДанныеДокумента.ХозяйственнаяОперация В
	|		  (ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.СписаниеТоваровУХранителя),
...
Процедура ДобавитьТекстЗапросаТаблицыДанные(ПараметрыОтражения)
	...
	Для Каждого ОписаниеПоля Из ТаблицаПолей Цикл
		...
		Если ЗначениеЗаполнено(ОписаниеПоля.Дополнительное) Тогда
	...
	Для Каждого ОписаниеСхемы Из СхемыЗапросовДокумента Цикл
		...
		ТекущийЗапросПакета = ОписаниеСхемы.СхемаЗапроса.ПакетЗапросов.Получить(0); //ЗапросВыбораСхемыЗапроса
...
Процедура ДобавитьТекстЗапросаФормированияАналитикиФинансовогоУчета(ПараметрыОтражения)
	...
	|	ДанныеДокумента.Подразделение 	КАК Подразделение,
	|	&ДанныеДокументаГруппаПродукции КАК ГруппаПродукции,
	|	ВЫБОР
	//++ НЕ УТ
	...
	|		ПО &ДанныеДокументаГруппаПродукции = ГруппыАналитическогоУчетаНоменклатуры.Ссылка
	//-- НЕ УТ
	|";
	
	ТекстЗапросаВременнаяТаблица = СтрЗаменить(
		ТекстЗапросаВременнаяТаблица,
		"&ДанныеДокументаГруппаПродукции",
		?(ЕстьПолеТаблицыДанные(ПараметрыОтражения, "ГруппаПродукции"), "ДанныеДокумента.ГруппаПродукции", "НЕОПРЕДЕЛЕНО"));
...
Процедура ПроверитьПоляЗапросаВыборкиДанных(ПараметрыОтражения, ОписаниеОперации, РазборСхемыЗапроса)
	...
	Если ЗначениеЗаполнено(ТекстПоляЗапроса) Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Лишние поля в запросе выборки данных для проведения документа';
													|en = 'Extra fields in the dataset query for document posting'") + ": " + ТекстПоляЗапроса);
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ТекстПоляОперации) Тогда
		ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Не хватает обязательных полей в запросе выборки данных для проведения документа';
													|en = 'Required fields are missing in the dataset query for document posting'") + ": " + ТекстПоляОперации);
	КонецЕсли;
