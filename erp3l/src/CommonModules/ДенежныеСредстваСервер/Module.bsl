...
Процедура УстановитьВидимостьДоступностьЭлементовРасшифровкиБезРазбиения(Форма) Экспорт
	...
	Если Объект.РасшифровкаПлатежа.Количество() Тогда
		...
		СкрытьНалогообложениеНДСВозврата = Не СкрытьНалогообложениеНДС
...
Процедура ЗаполнитьДокументПоЗаявкамНаРасходованиеДенежныхСредств(ДанныеЗаполнения, ТабличнаяЧасть, ФормаОплаты) Экспорт
	...
	|		РасшифровкаДокумента.ЗаявкаНаРасходованиеДенежныхСредств.Проект,	
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Сумма > 0 Тогда
			...
			#Область УХ_Встраивание
			НоваяСтрока.ИдентификаторПозиции = Новый УникальныйИдентификатор(ИдентификаторПозиции);
			#КонецОбласти
...
Процедура ЗаполнитьДокументПоРаспоряжениюНаПеремещениеДенежныхСредств(
	...
	ТекстЗамены = "ВЫРАЗИТЬ(РазмещениеЗаявок.БанковскийСчетКасса КАК Справочник.БанковскиеСчетаОрганизаций)";
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, ТекстДляЗамены, ТекстЗамены);
	#КонецОбласти
...
//
// Параметры:
//  СписокДокументов - Массив - документы, для реквизита Контрагент которых определяется информация о наименовании и КПП
//
// Возвращаемое значение:
//  ТаблицаЗначений:
//   * Документ - ДокументСсылка - ссылка на документ из массива СписокДокументов
//   * ЗначениеРеквизита - Строка - наименование контагента, КПП контрагента
//   * Признак - Число - признак, по которому определяется реквизит значения: 0 - наименование, 1 - КПП.
//
...
Функция ПолучитьПредставленияДокументов()
	...
	ВидыДокументовКонтрагента.Вставить("ЗаявкаНаВозвратТоваровОтКлиента",      НСтр("ru = 'заявке на возврат товаров';
																					|en = 'request for return of goods'"));
...
Функция НазначениеПлатежаСотрудникам(Объект)
	...
	Если ТекстПолучатель <> "" Тогда
		ТекстПолучатель = СтрШаблон(НСтр("ru = 'Для зачисления на счет %1';
										|en = 'For bank receipt %1'"), ТекстПолучатель);
	Иначе
...
Процедура ЗаполнитьНалогообложениеНДС(Объект, НалогообложениеНДСПоУмолчанию, ЭлементНалогообложениеНДС, ЭлементНалогообложениеНДСВозврата=Неопределено) Экспорт
	...
	Для Каждого СтрокаРасшифровкиПлатежа Из Объект.РасшифровкаПлатежа Цикл
		...
		Если ПресчитатьСтавку ИЛИ СтрокаРасшифровкиПлатежа.СтавкаНДС = Справочники.СтавкиНДС.ПустаяСсылка() Тогда
			...
			СтруктураПараметровДействия.Вставить("НалогообложениеНДС", СтрокаРасшифровкиПлатежа.НалогообложениеНДС);
			...
			Иначе
				...
				УчетНДСУП.ЗаполнитьСтавкуНДСДляПлатежей(СтрокаРасшифровкиПлатежа.СтавкаНДС, СтрокаРасшифровкиПлатежа.НалогообложениеНДС);
