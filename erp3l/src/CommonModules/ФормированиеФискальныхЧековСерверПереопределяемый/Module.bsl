...
Функция ОбъектыРасчетовСПризнакамиСпособаРасчетов(ДокументСсылка, ВозможныеПризнакиСпособаРасчетаПоДокументу, ПризнакиСпособаРасчетаАвто, ПризнакСпособаРасчета, ОбъектыРасчетов, СуммаДокумента) Экспорт
	...
	|			ТОГДА ОбъектыРасчетовСПризнакамиСпобораРасчета.ОбъектРасчетов
	...
	|		ИНАЧЕ КорректировкаРеализации.Ссылка
	|	КОНЕЦ,
	|	ОбъектыРасчетовСПризнакамиСпобораРасчета.ПризнакСпособаРасчета";
	Запрос.УстановитьПараметр("ОбъектыРасчетов", ОбъектыРасчетовСПризнакамиСпособаРасчетов);
	
	ОбъектыРасчетовСПризнакамиСпособаРасчетов = Запрос.Выполнить().Выгрузить();
	//-- Локализация
...
Процедура ОбновитьГиперссылкуПробитияФискальногоЧека(ДокументСсылка, Форма, РеквизитГиперссылкиНаФорме) Экспорт
	...
	Если НЕ ПолучитьФункциональнуюОпцию("ИспользоватьРозничныеПродажи")
		ИЛИ НЕ РазрешеноИспользованиеПодключаемогоОборудования()
...
Функция СтавкиНДСПоОбъектуРасчетовПоДокументуОплаты(ДокументОплаты, ОбъектРасчета, СистемаНалогообложения, ИмяКомандыПробитияЧека) Экспорт
	...
	Если СтавкиНДСПоОбъектуРасчетов.Количество() > 0 Тогда
		СтавкиНДСПоОбъектуРасчетов.Свернуть("СтавкаНДС", "Сумма");
	Иначе
...
Функция ОпределитьАвтоПризнакСпособаРасчетовПоОфлайнРасчетам(ПараметрыОперации, ДанныеПоОбъектуРасчетов)
	...
	|					И НЕ ТИПЗНАЧЕНИЯ(РасчетыПоСрокам.Регистратор) В (ТИП(Документ.РасчетКурсовыхРазниц), ТИП(Документ.СписаниеЗадолженности))
...
Функция ДанныеПоОбъектамРасчетовИзРасшифровкиПлатежа(ДокументСсылка)
	...
	Если ТипЗнч(РасшифровкаПлатежа) = Тип("ТаблицаЗначений") Тогда
		...
		Если ДокументОплатыСВозможностьюПробитияЕдиногоЧека(ДокументСсылка) Тогда
			...
			КонецЕсли;
			РасшифровкаПлатежа.Свернуть("ОбъектРасчетов, ОснованиеПлатежа, СтавкаНДС", "Сумма");
		...
		Иначе
			Если РасшифровкаПлатежа.Колонки.Найти("СтавкаНДС") <> Неопределено Тогда
				РасшифровкаПлатежа.Свернуть("ОбъектРасчетов, СтавкаНДС", "Сумма");
			Иначе
