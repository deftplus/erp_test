...
Процедура ДополнитьТекстЗапросаЗаданийКЗакрытиюМесяца(Запрос, ТекстЗапроса, ТекстЗапросаВременныхТаблиц, ИменаВременныхТаблиц, Документ) Экспорт
	...
	Если Не ТипЗнч(Документ) = Тип("Структура") ИЛИ Документ.Свойство("Дата") Тогда
...
Процедура ДобавитьЭтап_ФормированиеДвиженийПоОбесценениюВНА(ТаблицаЭтапов, ТекущийРодитель) Экспорт
	...
	ПредшествующиеОбязательныеЭтапы = Новый Массив;
	ПредшествующиеОбязательныеЭтапы.Добавить(Перечисления.ОперацииЗакрытияМесяца.ФормированиеДвиженийПоВнеоборотнымАктивам);
	
	НоваяСтрока.ПредшествующиеЭтапы = ПредшествующиеОбязательныеЭтапы;
...
//  Элементы - Строка - Список имен элементов формы, разделенный запятыми.
Процедура УправлениеВидимостьюЕГДС(Форма, Элементы) Экспорт
	
	ОбесценениеВНАДоступно = ПолучитьФункциональнуюОпцию("ИспользоватьОбесценениеВНА");
...
Процедура ИнициализироватьВтОстаткиДляРеклассификации(Запрос, ТекстыЗапроса)
	...
	|ИЗ
	|	РегистрНакопления.ОбесценениеВНА.Остатки(&ГраницаДокумента, ВнеоборотныйАктив В
	|		(ВЫБРАТЬ
	|			Т.ВнеоборотныйАктив
	|		ИЗ
	|			ОбъектыВНА КАК Т
...
Процедура ИнициализироватьВтНачисленияВосстановления(Запрос, ТекстыЗапроса)
	...
	|	ОбесценениеВНА.ВозмещаемаяСтоимостьРегл,
	|	ОбесценениеВНА.ВозмещаемаяСтоимостьУпр
	...
	|
	|ВЫБРАТЬ
	|	ОбесценениеВНА.ОсновноеСредство,
	|	ОбесценениеВНА.ВозмещаемаяСтоимостьРегл,
	|	ОбесценениеВНА.ВозмещаемаяСтоимостьУпр
	|ИЗ
	...
	|
	|ВЫБРАТЬ
	|	ОбесценениеВНА.НематериальныйАктив,
	|	ОбесценениеВНА.ВозмещаемаяСтоимостьРегл,
	|	ОбесценениеВНА.ВозмещаемаяСтоимостьУпр
	|ИЗ
	...
	|ГДЕ
	|	// если в документе указан объект ВНА, то обесцениваем его непосредственно, а не в рамках ЕГДС
	|	ОбъектыВНА.ВнеоборотныйАктив ЕСТЬ NULL
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	втВозмещаемаяСтоимость.ВнеоборотныйАктив,
	|	ЗНАЧЕНИЕ(Справочник.ЕдиницыГенерирующиеДенежныеСредства.ПустаяСсылка)
	|ИЗ
	|	втВозмещаемаяСтоимость КАК втВозмещаемаяСтоимость
	|ГДЕ
	...
	|		СтоимостьОСОстатки.ГруппаФинансовогоУчета,
	|		СтоимостьОСОстатки.НаправлениеДеятельности,
	...
	|	ИЗ
	|		РегистрНакопления.СтоимостьОС.Остатки(&ГраницаОстатков, ОсновноеСредство В
	|			(ВЫБРАТЬ
	|				Т.ВнеоборотныйАктив
	|			ИЗ
	...
	|		АмортизацияОСОстатки.АмортизацияРеглОстаток + АмортизацияОСОстатки.АмортизацияЦФОстаток,
	|		АмортизацияОСОстатки.АмортизацияОстаток,
	|		0 КАК ОбесценениеРегл,
	|		0 КАК ОбесценениеУпр
	|	ИЗ
	|		РегистрНакопления.АмортизацияОС.Остатки(&ГраницаОстатков, ОсновноеСредство В
	|			(ВЫБРАТЬ
	|				Т.ВнеоборотныйАктив
	|			ИЗ
	...
	|		СтоимостьНМАОстатки.СтоимостьРеглОстаток + СтоимостьНМАОстатки.СтоимостьЦФОстаток,
	|		СтоимостьНМАОстатки.СтоимостьОстаток,
	|		0 КАК ОбесценениеРегл,
	|		0 КАК ОбесценениеУпр
	|	ИЗ
	|		РегистрНакопления.СтоимостьНМА.Остатки(&ГраницаОстатков, НематериальныйАктив В
	|			(ВЫБРАТЬ
	|				Т.ВнеоборотныйАктив
	|			ИЗ
	...
	|		АмортизацияНМАОстатки.АмортизацияРеглОстаток + АмортизацияНМАОстатки.АмортизацияЦФОстаток,
	|		АмортизацияНМАОстатки.АмортизацияОстаток,
	|		0 КАК ОбесценениеРегл,
	|		0 КАК ОбесценениеУпр
	|	ИЗ
	|		РегистрНакопления.АмортизацияНМА.Остатки(&ГраницаОстатков, НематериальныйАктив В
	|			(ВЫБРАТЬ
	|				Т.ВнеоборотныйАктив
	|			ИЗ
	...
	|		0,
	|		0,
	|		ВЫБОР КОГДА ОбесценениеДвижения.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|			ТОГДА ОбесценениеДвижения.ОбесценениеРегл 
	|		ИНАЧЕ 
	|			-ОбесценениеДвижения.ОбесценениеРегл
	|		КОНЕЦ,
	|		ВЫБОР КОГДА ОбесценениеДвижения.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|			ТОГДА ОбесценениеДвижения.ОбесценениеУпр
	|		ИНАЧЕ 
	|			-ОбесценениеДвижения.ОбесценениеУпр
	|		КОНЕЦ
	|	ИЗ
	...
	|	втВозмещаемаяСтоимость.ВозмещаемаяСтоимостьРегл,
	|	втВозмещаемаяСтоимость.ВозмещаемаяСтоимостьУпр,
	...
	Для Каждого СтрокаОСНМА Из ДанныеПоОСНМА Цикл
		...
		Если СтрокаОСНМА.ВозмещаемаяСтоимостьРегл = 0 
			Или СтрокаОСНМА.ВозмещаемаяСтоимостьРегл > СтрокаОСНМА.СтоимостьРегл - СтрокаОСНМА.ОбесценениеРегл Тогда
		...
		Если СтрокаОСНМА.ВозмещаемаяСтоимостьУпр = 0 
			Или СтрокаОСНМА.ВозмещаемаяСтоимостьУпр > СтрокаОСНМА.СтоимостьУпр - СтрокаОСНМА.ОбесценениеУпр Тогда
	...
	ДанныеПоЕГДС = Результат[8].Выгрузить(); // ТаблицаЗначений - 
	...
	Для Каждого СтрокаЕГДС Из ТаблицаЕГДС Цикл
		...
		НачисленияРегл = ОбщегоНазначенияУТКлиентСервер.РаспределитьПропорционально(
			...
			ТаблицаОбесцененияЕГДС.ВыгрузитьКолонку("СтоимостьРегл"));
		НачисленияУпр = ОбщегоНазначенияУТКлиентСервер.РаспределитьПропорционально(
			СуммаОбесцененияУпр,
			ТаблицаОбесцененияЕГДС.ВыгрузитьКолонку("СтоимостьУпр"));
...
Процедура ТекстЗапросаПрочиеДоходы(Запрос, ТекстыЗапроса)
	...
	|	ПрочиеДоходы.УдалитьСуммаРегл,
	|	ПрочиеДоходы.РасчетСебестоимости,
	|	ПрочиеДоходы.КорНаправлениеДеятельности,
	|	ПрочиеДоходы.Сторно,
	|	ПрочиеДоходы.ДокументРасчета,
	|	ПрочиеДоходы.РежимЗакрытияМесяца
	|ИЗ
	...
	|	втВосстановления.Подразделение,
	|	втВосстановления.НаправлениеДеятельности,
	|	ЕСТЬNULL(ОбесценениеВНА.СтатьяДоходов, ВтОтражениеДоходовРасходов.СтатьяДоходов) КАК СтатьяДоходов,
	|	ЕСТЬNULL(ОбесценениеВНА.АналитикаДоходов, ВтОтражениеДоходовРасходов.АналитикаДоходов) КАК АналитикаДоходов,
