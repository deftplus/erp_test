...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий     = НСтр("ru = 'Обновление публикации сотрудников.';
										|en = 'Updating employee publication.'");
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия			= "3.1.18.43";
	Обработчик.РежимВыполнения	= ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ОсновнойРежимВыполненияОбновления();
	Обработчик.Идентификатор	= Новый УникальныйИдентификатор("f1ca21bf-612b-4691-bbce-8bb45ad6c919");
	Обработчик.Процедура		= "КабинетСотрудникаРасширенный.ДобавитьОбработчикРегистрацияВыполненныхНеопубликованныхЗаявок";
	Обработчик.Комментарий		= НСтр("ru = 'Добавление обработчика РегистрацияВыполненныхНеопубликованныхЗаявок.';
										|en = 'Add the РегистрацияВыполненныхНеопубликованныхЗаявок handler.'");
...
Функция ТипыОбъектовДляРучнойРегистрации() Экспорт
	...
	Для Каждого ТипОбъекта Из Метаданные.ОпределяемыеТипы.ПредметПубликацииКабинетСотрудника.Тип.Типы() Цикл
		...
		Если ТипОбъекта = Тип("СправочникСсылка.ШтатноеРасписание")
			 И ПолучитьФункциональнуюОпцию("НеИспользоватьШтатноеРасписание") Тогда
			Продолжить;
		КонецЕсли;
...
Функция КадровыеДанныеОбновляемыхСотрудников(МенеджерВТ) Экспорт
	...
	|	ЕСТЬNULL(ШтатноеРасписание.Ссылка, ЗНАЧЕНИЕ(Справочник.ШтатноеРасписание.ПустаяСсылка)) КАК ДолжностьПоШтатномуРасписанию,
	|	КадровыеДанные.Организация КАК Организация,
	|	КадровыеДанные.Подразделение КАК Подразделение,
	|	КадровыеДанные.Должность КАК Должность
	|ИЗ
...
Процедура ДобавитьОбработчикРегистрацияВыполненныхНеопубликованныхЗаявок(ПараметрыОбновления = Неопределено) Экспорт
	...
	ИмяОбработчика = "РегистрацияВыполненныхНеопубликованныхЗаявок";
	...
	|ГДЕ
	|	ОбработчикиОбменаКабинетСотрудника.Обработчик = &Обработчик";
	РезультатЗапроса = Запрос.Выполнить();
	Если Не РезультатЗапроса.Пустой() Тогда
		ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.УстановитьПараметрОбновления(ПараметрыОбновления, "ОбработкаЗавершена", Истина);
		Возврат;
