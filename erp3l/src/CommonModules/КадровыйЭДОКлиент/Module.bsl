...
Процедура ПечатьДокументовВыполнитьКоманду(Форма, Команда, ПродолжитьВыполнениеНаСервере, ДополнительныеПараметры) Экспорт
	...
	Если Команда.Имя = "СохранитьВPDFИПодписатьФайл" Тогда
		ПодписатьПечатныеФомыПриПечати(Форма,
			ПредопределенноеЗначение("Перечисление.ДействияСПечатнымиФормами.ЗаписатьНаДиск"));
	ИначеЕсли Команда.Имя = "ОтправитьПодписанныеPDF" Тогда
		ПодписатьПечатныеФомыПриПечати(Форма,
			ПредопределенноеЗначение("Перечисление.ДействияСПечатнымиФормами.ОтправитьПоПочте"));
	ИначеЕсли Команда.Имя = "ПодписатьПечатныеФормы" Тогда
		...
		Иначе
			ПодписатьПечатныеФомыПриПечати(Форма,
				ПредопределенноеЗначение("Перечисление.ДействияСПечатнымиФормами.Подписать"));
		КонецЕсли;
...
//  ЦельПодписания   - ПеречислениеСсылка.ДействияСПечатнымиФормами
//
Процедура ПодписатьПечатныеФомыПриПечати(УправляемаяФорма, ЦельПодписания) Экспорт
	
	// Сохранение отредактированной печатнойформы
...
Процедура ОбработчикПодключаемойКомандыПодписанияПечатныхФорм(ДокументыКадровогоЭДО, ДополнительныеПраметры) Экспорт
	...
	Если ЗначениеЗаполнено(ДокументыКадровогоЭДО) Тогда
		...
		ОткрытьФормуПодписанияПечатныхФорм(ЭлектронныеДокументы);
	КонецЕсли;
	
КонецПроцедуры

Процедура ОзнакомитьсяИПодписатьФайлыПечтаныхФорм() Экспорт
	ФайлыНаПодпись = КадровыйЭДОВызовСервера.ФайлыНаПодписьПользователя();
	Если ФайлыНаПодпись.Количество() = 0 Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Нет печатных форм на подписние.';
										|en = 'No print forms to sign.'"));
	Иначе
		ОткрытьФормуПодписанияПечатныхФорм(
			КадровыйЭДОВызовСервера.ФайлыНаПодписьПользователя(),
			,
			,
			,
			ПредопределенноеЗначение("Перечисление.ДействияСПечатнымиФормами.Подписать"));
		...
		Иначе
			...
			Если ОповещениеЗавершения <> Неопределено Тогда
				ВыполнитьОбработкуОповещения(ОповещениеЗавершения,
					ОповещениеЗавершения.ДополнительныеПараметры.ПечатныеФормыОбъектов);
			КонецЕсли;
...
Процедура ПослеПодписанияСохраненныхФорм(РезультатПодписанияФайлов, ДополнительныеПараметры) Экспорт
	
	Если РезультатПодписанияФайлов <> Неопределено Тогда
		
		ФормаПечатьДокументов = ДополнительныеПараметры.Форма;
		Если ДополнительныеПараметры.ЦельПодписания = ПредопределенноеЗначение("Перечисление.ДействияСПечатнымиФормами.Подписать") Тогда
		...
		Иначе
			...
			Если ОбрабатываемыеФайлы.Количество() > 0 Тогда
				...
				Если ДополнительныеПараметры.ЦельПодписания = ПредопределенноеЗначение("Перечисление.ДействияСПечатнымиФормами.ЗаписатьНаДиск") Тогда
					
					СохранитьПечатныеФормыНаДиск(ОбрабатываемыеФайлы);
					
				ИначеЕсли ДополнительныеПараметры.ЦельПодписания = ПредопределенноеЗначение("Перечисление.ДействияСПечатнымиФормами.ОтправитьПоПочте") Тогда
				...
				ИначеЕсли ДополнительныеПараметры.ЦельПодписания = ПредопределенноеЗначение("Перечисление.ДействияСПечатнымиФормами.ПередатьНаРаботаВРоссии") Тогда
					
					ИнтеграцияСРаботаВРоссииКлиент.ПередатьДокументы(ОбрабатываемыеФайлы);
					
				ИначеЕсли ДополнительныеПараметры.ЦельПодписания = ПредопределенноеЗначение("Перечисление.ДействияСПечатнымиФормами.ПередатьВКабинетСотрудников") Тогда
			...
			Если ДанныеФайловНаПодпись.Количество() > 0 Тогда
				...
				Для Каждого ФайлыНаПодписьОрганизации Из ДанныеФайловНаПодпись Цикл
					ФайловНаПодпись = ФайловНаПодпись + ФайлыНаПодписьОрганизации.Значение.Количество();
				...
				Состояние(СтрокаСЧислом(
					...
					ФайловНаПодпись, ВидЧисловогоЗначения.Количественное));
...
Процедура ПолучитьСведенияОСертификатеПослеСозданияМенеджера(МенеджерКриптографии, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(МенеджерКриптографии) <> Тип("Строка") Тогда
		...
		Для Каждого ПодписьБезСертификата Из ДополнительныеПараметры.ПодписиБезСертификатов Цикл
			...
			Попытка
				Сертификаты = МенеджерКриптографии.ПолучитьСертификатыИзПодписи(
			...
			Если Сертификаты.Количество() > 0 Тогда
				...
				ПодписьБезСертификата.Отпечаток = Base64Строка(Сертификат.Отпечаток);
				ПодписьБезСертификата.Сертификат = ПоместитьВоВременноеХранилище(Сертификат.Выгрузить(), Новый УникальныйИдентификатор);
...
//		ОзнакомитьсяИПодписатьФайлыПечтаныхФорм();
	КонецЕсли;
