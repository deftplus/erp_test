...
Процедура УстановитьСостояниеНовостейДляПользователей(
	...
	ЗаписатьСообщениеВЖурналРегистрации(
		...
		НСтр("ru = 'Новости. Сервис и регламент. УстановитьСостояниеНовостейДляПользователей';
			|en = 'News. Service and regulation. УстановитьСостояниеНовостейДляПользователей'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
		...
		Если ОбщегоНазначения.РазделениеВключено() Тогда
			...
			Если НЕ ОбщегоНазначения.ДоступноИспользованиеРазделенныхДанных() Тогда
				ТекстСообщения = НСтр("ru = 'УстановитьСостояниеНовостейДляПользователей, ошибка выполнения: при работе в модели сервиса разрешено запускать только в разделенном сеансе, в противном случае недоступен справочник Пользователи.
					...
					|en = 'УстановитьСостояниеНовостейДляПользователей, execution error: when using SaaS mode it is only permitted to start in separated session, otherwise the Пользователи.
					| catalog is unavailable Connection string: %СтрокаСоединенияИнформационнойБазы%
					|ТекущийРежимЗапуска: %ТекущийРежимЗапуска%
					|This is separated configuration: %ЭтоРазделеннаяКонфигурация%
					|Separation enabled: %РазделениеВключено%
					|Separated data use is available: %ДоступноИспользованиеРазделенныхДанных%
					...
					|Платформа: %Платформа_Версия%
					|Functional option Using news allowed: %ОбработкаНовостейПовтИсп_РазрешенаРаботаСНовостями%
					|Functional option Using news over internet permitted: %ОбработкаНовостейПовтИсп_РазрешенаРаботаСНовостямиЧерезИнтернет%
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			...
			НСтр("ru = 'Новости. Сервис и регламент. ПриИзмененииЗначенийОтборов. Общие';
				|en = 'News. Service and regulation. ПриИзмененииЗначенийОтборов. Общие'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			...
			НСтр("ru = 'Новости. Сервис и регламент. ПриИзмененииЗначенийОтборов. ДляОбластиДанных';
				|en = 'News. Service and regulation. ПриИзмененииЗначенийОтборов. ДляОбластиДанных'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			...
			НСтр("ru = 'Новости. Сервис и регламент. ПриИзмененииЗначенийОтборов. Пользовательские';
				|en = 'News. Service and regulation. ПриИзмененииЗначенийОтборов. Пользовательские'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			...
			Если ТекущаяДатаДляПроверки >= ДатаПоследнегоОбновления Тогда
				...
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'ОбработкаНовостей.ПриНачалеРаботыСистемы: прошло длительное время после последнего пересчета периодических реквизитов (%1).
						...
						|en = 'ОбработкаНовостей.ПриНачалеРаботыСистемы: a long time has passed since the last recalculation of periodic atributes (%1).
				...
				ЗаписатьСообщениеВЖурналРегистрации(
					...
					НСтр("ru = 'Новости. Сервис и регламент. УстановитьСостояниеПодсистемы';
						|en = 'News. Service and regulation. УстановитьСостояниеПодсистемы'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
					УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
					Метаданные.Справочники.Новости, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			...
			НСтр("ru = 'Новости. Загрузка новостей. Соединение с сервером новостей';
				|en = 'News. Import news. Connection to news server'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			...
			Исключение
				...
				ЗаписатьСообщениеВЖурналРегистрации(
					...
					НСтр("ru = 'Новости. Разное. Ошибка полнотекстового поиска';
						|en = 'News. Miscellaneous. Full-text search error'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
					УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
					Метаданные.Справочники.Новости, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
					...
					ЛогШаговВыполненияВложенный.Добавить(
						СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Разрешена работа через интернет. ТекущаяДатаДляРегистров: %1 / ДатаСледующегоОбновления: %2';
								|en = 'Work via the Internet is allowed. ТекущаяДатаДляРегистров: %1 / ДатаСледующегоОбновления: %2'"),
			...
			ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
				...
				ИначеЕсли (СостояниеПодсистемы = "ТребуетсяПересчетОтборовПослеДлительногоОжидания") Тогда
					ЛогШаговВыполненияВложенный.Добавить(НСтр("ru = 'Необходимо выполнение из-за состояния подсистемы.';
																|en = 'Execution is required due to subsystem status.'"));
					ОбновлениеПериодическихСвойствНовостей(КонтекстВыполненияВложенныйВложенный);
				Иначе
					ЛогШаговВыполненияВложенный.Добавить(НСтр("ru = 'Срок не наступил. Состояние подсистемы удовлетворительное.';
																|en = 'The due date is yet to come. The subsystem status is satisfactory.'"));
				КонецЕсли;
...
#КонецОбласти
	...
	ЗаписатьСообщениеВЖурналРегистрации(
		НСтр("ru = 'БИП:Новости.Все обновления новостей';
			|en = 'ISL: News.All news updates'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
		НСтр("ru = 'Новости. Все обновления новостей';
			|en = 'News. All news updates'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
		?(КодРезультата > 0,
			УровеньЖурналаРегистрации.Ошибка,
			УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			...
			НСтр("ru = 'Новости. Сервис и регламент. УдалитьНовости_Ежедневно';
				|en = 'News. Service and regulation. УдалитьНовости_Ежедневно'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			...
			НСтр("ru = 'Новости. Сервис и регламент. ОбновлениеПериодическихСвойствНовостей';
				|en = 'News. Service and regulation. ОбновлениеПериодическихСвойствНовостей'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			...
			НСтр("ru = 'Новости. Сервис и регламент. ОбновлениеПривязокКМетаданным';
				|en = 'News. Service and regulation. ОбновлениеПривязокКМетаданным'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			...
			НСтр("ru = 'Новости. Все обновления новостей. Получение и обработка новостей по лентам новостей';
				|en = 'News. All news updates. Receive and process news by news feeds'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			...
			НСтр("ru = 'Новости. Все обновления новостей. Обновление классификаторов';
				|en = 'News. All news updates. Update classifiers'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			...
			НСтр("ru = 'Новости. Сервис и регламент. ПересчитатьНеобработанныеКатегорииНовостей';
				|en = 'News. Service and regulation. ПересчитатьНеобработанныеКатегорииНовостей'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			...
			НСтр("ru = 'Новости. Сервис и регламент. ПересчитатьОтборыПоНовостям. РедкоМеняющиеся';
				|en = 'News. Service and regulation. ПересчитатьОтборыПоНовостям. РедкоМеняющиеся'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			...
			НСтр("ru = 'Новости. Сервис и регламент. ПересчитатьОтборыПоНовостям. Общие';
				|en = 'News. Service and regulation. ПересчитатьОтборыПоНовостям. Common'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			...
			НСтр("ru = 'Новости. Сервис и регламент. ПересчитатьОтборыПоНовостям. ДляОбластиДанных';
				|en = 'News. Service and regulation. ПересчитатьОтборыПоНовостям. ДляОбластиДанных'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			...
			НСтр("ru = 'Новости. Сервис и регламент. ПересчитатьОтборыПоНовостям. Пользовательские';
				|en = 'News. Service and regulation. ПересчитатьОтборыПоНовостям. Пользовательские'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
			...
			Исключение
				...
				ЗаписатьСообщениеВЖурналРегистрации(
					...
					НСтр("ru = 'Новости. Сервис и регламент. РассчитатьОтборПоКатегории. Общие';
						|en = 'News. Service and regulation. РассчитатьОтборПоКатегории. Общие'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
					УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
					Метаданные.Справочники.Новости, // ОбъектМетаданных
					, // Данные
					ТекстСообщения, // Комментарий
				...
				Исключение
					...
					ЗаписатьСообщениеВЖурналРегистрации(
						...
						НСтр("ru = 'Новости. Сервис и регламент. РассчитатьОтборПоКатегории. ДляОбластиДанных';
							|en = 'News. Service and regulation. РассчитатьОтборПоКатегории. ДляОбластиДанных'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
						УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
						Метаданные.Справочники.Новости, // ОбъектМетаданных
						, // Данные
						ТекстСообщения, // Комментарий
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			...
			НСтр("ru = 'Новости. Все обновления новостей. Проверка версии платформы';
				|en = 'News. All news updates. Check the platform version.'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			...
			НСтр("ru = 'Новости. Сервис и регламент. ОптимизацияОтборовПоНовостям';
				|en = 'News. Service and regulation. ОптимизацияОтборовПоНовостям'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			...
			НСтр("ru = 'Новости. Сервис и регламент. НайтиИОбработатьНеактивныеЛентыНовостей';
				|en = 'News. Service and regulation. НайтиИОбработатьНеактивныеЛентыНовостей'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
	...
	Если ЕстьОшибка = Истина Тогда
		ЗаписатьСообщениеВЖурналРегистрации(
			...
			НСтр("ru = 'Новости. Сервис и регламент. ИзменениеИспользованияРегламентныхЗаданий';
				|en = 'News. Service and regulation. ИзменениеИспользованияРегламентныхЗаданий'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
	...
	Если ЕстьОшибка = Истина Тогда
		ЗаписатьСообщениеВЖурналРегистрации(
			...
			НСтр("ru = 'Новости. Сервис и регламент. ПолучитьРегламентноеЗаданиеВсеОбновленияНовостей';
				|en = 'News. Service and regulation. ПолучитьРегламентноеЗаданиеВсеОбновленияНовостей'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
			, // Данные
			ТекстСообщения, // Комментарий
...
Процедура СбросУНовостейПризнакаТребуетсяПересчетОтборов(КонтекстВыполнения = Неопределено) Экспорт
	...
	ОбработкаНовостейКлиентСервер.НачатьРегистрациюРезультатаВыполненияОперации(
		...
		НСтр("ru = 'Сброс у новостей признака [ТребуетсяПересчетОтборов]';
			|en = 'Reseting the [ТребуетсяПересчетОтборов] news flag'"));
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			НСтр("ru = 'БИП:Новости.Сервис и регламент';
				|en = 'ISL:News.Service and regulation'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
			НСтр("ru = 'Новости. Сервис и регламент. СбросУНовостейПризнакаТребуетсяПересчетОтборов';
				|en = 'News. Service and regulation. СбросУНовостейПризнакаТребуетсяПересчетОтборов'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			Метаданные.РегламентныеЗадания.ВсеОбновленияНовостей, // ОбъектМетаданных
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			...
			НСтр("ru = 'Новости. Сервис и регламент. УдалитьНовости_Ежедневно. После сбора данных для удаления';
				|en = 'News. Service and regulation. УдалитьНовости_Ежедневно. After data collection for deletion'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			...
			НСтр("ru = 'Новости. Сервис и регламент. Загрузка стандартных значений. Все стандартные значения';
				|en = 'News. Service and regulation. Import standard values. All standard values'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
				...
				Если НЕ ПустаяСтрока(Результат) Тогда
					...
					ЗаписатьСообщениеВЖурналРегистрации(
						НСтр("ru = 'БИП:Новости.Обновление ИБ';
							|en = 'ISL:News.IB update'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
						НСтр("ru = 'Новости. Обновление ИБ. Проверка внедрения';
							|en = 'News. Update IB. Check implementation'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
						УровеньЖурналаРегистрации.Ошибка, // УровеньЖурналаРегистрации.*
						, // ОбъектМетаданных
						, // Данные
						ТекстСообщения, // Комментарий
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			...
			НСтр("ru = 'Новости. Обновление ИБ. ОбщиеДанные. ПервыйЗапуск';
				|en = 'News. Infobase Update. ОбщиеДанные. ПервыйЗапуск'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
		...
		Если Пользователи.ЭтоСеансВнешнегоПользователя() Тогда
			...
			ЗаписатьСообщениеВЖурналРегистрации(
				...
				НСтр("ru = 'Новости. Обновление ИБ. ОбластьДанных. ПервыйЗапуск. Конец';
					|en = 'News. Infobase Update. ОбластьДанных. ПервыйЗапуск. End'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
				УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
				, // ОбъектМетаданных
				, // Данные
				ТекстСообщения, // Комментарий
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			...
			НСтр("ru = 'Новости. Обновление ИБ. ОбластьДанных. ПервыйЗапуск';
				|en = 'News. Infobase Update. ОбластьДанных. ПервыйЗапуск'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
				...
				Иначе
					...
					ЗаписатьСообщениеВЖурналРегистрации(
						...
						НСтр("ru = 'Новости. Обновление ИБ. ОбщиеДанные. ПерейтиНаВерсию';
							|en = 'News. Infobase Update. ОбщиеДанные. ПерейтиНаВерсию'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
						УровеньЖурналаРегистрации.Предупреждение, // УровеньЖурналаРегистрации.*
						, // ОбъектМетаданных
						0, // Данные
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			...
			НСтр("ru = 'Новости. Обновление ИБ. ОбщиеДанные. ПерейтиНаВерсию_ХХХ';
				|en = 'News. Infobase Update. ОбщиеДанные. ПерейтиНаВерсию_ХХХ'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			...
			НСтр("ru = 'Новости. Обновление ИБ. ОбластьДанных. ПерейтиНаВерсию';
				|en = 'News. Infobase Update. ОбластьДанных. ПерейтиНаВерсию'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
	...
	Если ЗаписыватьВЖурналРегистрации = Истина Тогда
		...
		ЗаписатьСообщениеВЖурналРегистрации(
			...
			НСтр("ru = 'Новости. Обновление ИБ. ОбластьДанных. ПерейтиНаВерсию_ХХХ';
				|en = 'News. Infobase Update. ОбластьДанных. ПерейтиНаВерсию_ХХХ'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
			?(КодРезультата > 0,
				УровеньЖурналаРегистрации.Ошибка,
				УровеньЖурналаРегистрации.Информация), // УровеньЖурналаРегистрации.*
			, // ОбъектМетаданных
		...
		Исключение
			...
			ЗаписатьСообщениеВЖурналРегистрации(
				НСтр("ru = 'БИП:БазовыйФункционал.Отладка';
					|en = 'БИП:БазовыйФункционал.Debug'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИмяСобытия.
				НСтр("ru = 'БазовыйФункционал. Отладка. ПредставлениеОбъектаХДТО';
					|en = 'Core. Debug. XDTOObjectPresentation'", ОбщегоНазначения.КодОсновногоЯзыка()), // ИдентификаторШага.
	...
	Если (Результат.ТекущееСостояние <> "Активна")
		...
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'ОбработкаНовостей.УстановитьСостояниеПодсистемы: ожидается состояние подсистемы:
				...
				|Передано значение: [%1].';
				|en = 'ОбработкаНовостей.УстановитьСостояниеПодсистемы: expected subsystem status:
				| - Активна;
				| - ТребуетсяПересчетОтборовПослеДлительногоОжидания;
				| - СейчасЗагружаютсяНовости.
				|Passed value: [%1].'", ОбщегоНазначения.КодОсновногоЯзыка()),
			СостояниеПодсистемы);
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
