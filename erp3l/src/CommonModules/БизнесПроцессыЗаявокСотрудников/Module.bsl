...
Процедура СоздатьИзменитьДокументКЭДОСправкаСотруднику(ЭлектронныйДокумент, СсылкаНаДокумент, РегистрироватьИзмененияКабинетСотрудника = Истина) Экспорт
	...
	Иначе
		...
		ДокументКЭДО.ОснованиеДокумента 	= СсылкаНаДокумент;
		ДокументКЭДО.Организация 			= РеквизитыДокумента.Организация;
		ДокументКЭДО.ФизическоеЛицо 		= РеквизитыДокумента.ФизическоеЛицо;
		ДокументКЭДО.КатегорияДокумента 	= Перечисления.КатегорииДокументовКадровогоЭДО.СправкаСотруднику;
		ДокументКЭДО.Дата					= ТекущаяДатаСеанса();
		ДокументКЭДО.ИдентификаторДокумента = Строка(СсылкаНаДокумент.УникальныйИдентификатор());
	КонецЕсли;
...
Процедура СоздатьИЗаписатьДокументКЭДООбновление(ЗаявкаСотрудника) Экспорт 
	...
	ДокументКЭДО.ФизическоеЛицо 						= ЗаявкаСотрудника.ФизическоеЛицо;
...
Процедура СоздатьДокументКЭДОПоЗаявке(ЗаявкаСотрудника, ЭлектронныйДокумент, Организация, ИдентификаторДокумента)
	...
	ДокументКЭДО.ЭлектронныйДокумент					= ЭлектронныйДокумент;
	ДокументКЭДО.Дата									= ЗаявкаСотрудника.Дата;
	Если ТипЗнч(ЗаявкаСотрудника.Ссылка) = Тип("БизнесПроцессСсылка.ЗаявкаСотрудникаОтпуск") Тогда
		ДокументКЭДО.Сотрудник							= ЗаявкаСотрудника.Сотрудник;
	КонецЕсли;
	ДокументКЭДО.Организация 							= Организация;
	ДокументКЭДО.ФизическоеЛицо 						= ЗаявкаСотрудника.ФизическоеЛицо;
	ДокументКЭДО.ИдентификаторЗаявкиКабинетСотрудника 	= ЗаявкаСотрудника.ИдентификаторЗаявки;
	ДокументКЭДО.ИдентификаторДокумента					= ?(ЗначениеЗаполнено(ИдентификаторДокумента), ИдентификаторДокумента, ЗаявкаСотрудника.ИдентификаторЗаявки);
	ДокументКЭДО.КатегорияДокумента 					= Перечисления.КатегорииДокументовКадровогоЭДО.ЗаявлениеСотрудника;
	ДокументКЭДО.Записать();		
КонецПроцедуры
