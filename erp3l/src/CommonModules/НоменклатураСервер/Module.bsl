			...
			Пока Выборка.Следующий() Цикл
				Если Выборка.СерияНеЗаполнена Тогда
					ТекстСообщения = НСтр("ru = 'Не заполнена колонка ""%ИмяРеквизитаСерия%"" в строке %НомерСтроки% списка ""%ПредставлениеТЧ%""';
											|en = 'Column ""%ИмяРеквизитаСерия%"" in line %НомерСтроки% of the ""%ПредставлениеТЧ%"" list is not filled in'");
				...
				Если Выборка.ОшибкаКоличества Тогда
					Если Выборка.ЭтоТоварноеМесто Тогда
						ТекстСообщения = НСтр("ru = 'Политика учета серий товара ""%НоменклатураПредставление%"" предусматривает, что количество товарных мест будет соответствовать количеству товара равному 1, т.е. %КоличествоТоварныхМест%';
												|en = 'Batch accounting policy of goods%НоменклатураПредставление% requires that number of colli matches goods quantity equal to 1, i.e. %КоличествоТоварныхМест%'");
					...
					Иначе
						ТекстСообщения = НСтр("ru = 'Указанное количество товара ""%НоменклатураПредставление%"" с серией ""%Серия%"" равно %КоличествоСерий%. Политика учета серий данного товара предусматривает, что количество по любой серии этого товара всегда будет равно 1.';
												|en = 'Specified quantity of the %НоменклатураПредставление% goods with the %Серия% batch equals to %КоличествоСерий%. Accounting policy of the goods batch means that quantity for any batch of the goods will always be equal to 1.'");
				...
				Если Выборка.ОшибкаУпаковки Тогда
					ТекстСообщения = НСтр("ru = 'Политика учета серий товара ""%НоменклатураПредставление%"" предусматривает, что серии указываются для каждого экземпляра. Укажите единичную упаковку в строке %НомерСтроки% списка ""%ПредставлениеТЧ%"".';
											|en = 'Accounting policy of the ""%НоменклатураПредставление%"" goods batch provides that batches are specified for each item. Specify a single package in %НомерСтроки% line of the ""%ПредставлениеТЧ%"" list.'");
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НоменклатураПредставление%",	Выборка.НоменклатураПредставление);
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%НомерСтроки%",				Строка(Выборка.НомерСтроки));
					ТекстСообщения = СтрЗаменить(ТекстСообщения, "%ПредставлениеТЧ%",			ПредставлениеТЧ);
			...
			Если Выборка.ОшибкаНеУказанаСерия
				...
				Если (Выборка.СтатусУказанияСерий = 13
					...
					ТекстСообщения = НСтр("ru = 'Не заполнена колонка ""Серия"" в строке %НомерСтроки% списка ""%ПредставлениеТЧ%""';
											|en = 'Column ""Batch"" column in line %НомерСтроки% of the ""%ПредставлениеТЧ%"" list is not filled in'");
				...
				Иначе
					ТекстСообщения = НСтр("ru = 'Для товара ""%Товар%"" указано по сериям %КоличествоСерий% %ЕдиницаИзмерения%. Необходимо указать %КоличествоТоваров% %ЕдиницаИзмерения%. Исправьте серии.';
											|en = 'Specified by batch %КоличествоСерий% %ЕдиницаИзмерения% for the ""%Товар%"" goods. Specify %КоличествоТоваров% %ЕдиницаИзмерения%. Correct batch.'");
	...
	Пока ВыборкаПоСерии.Следующий() Цикл 
		...
		Если ОписанияИзлишков.Количество() <> 0 Или ОписанияНедостач.Количество() <> 0 Тогда
			...
			СообщениеПользователю = НСтр("ru = 'Из следующих товарных мест нельзя собрать целое количество %ЕдИзм% товара ""%Товар%"": %ОписаниеИзлишков%. Необходимо или удалить эти товарные места, или дополнить товарными местами: %ОписаниеНедостач%.';
										|en = 'Cannot assemble the entire quantity %ЕдИзм% of ""%Товар%"" goods from the following colli: %ОписаниеИзлишков%. Either delete these colli, or add the following colli: %ОписаниеНедостач%.'");
	...
	Пока ВыборкаНезаполненные.Следующий() Цикл
		...
		Если ВыборкаНезаполненные.ЭтоТоварноеМесто Тогда
			...
			ТекстСообщения = НСтр("ru = 'В строке %НомерСтроки% для товара ""%ПредставлениеТовара%"" серии не заполнены, т.к. указана упаковка - товарное место. Заполнение серий FEFO по товарным местам не поддерживается.';
									|en = 'Batches are not filled in for the ""%ПредставлениеТовара%"" goods item in line %НомерСтроки%. Collo is specified as package. Cannot fill in FEFO batch according to the colli.'");
...
Процедура ХарактеристикиНоменклатурыОбработкаПолученияДанныхВыбора(ДанныеВыбора, Параметры, СтандартнаяОбработка) Экспорт
	...
	|	СпрНоменклатура.Ссылка = &Номенклатура
	|	И ВЫБОР 
	|		КОГДА &СтрокаПоискаЗаполнена = ИСТИНА ТОГДА
