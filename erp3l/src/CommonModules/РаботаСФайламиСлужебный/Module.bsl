		...
		Исключение
			...
			СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Двоичные данные файла %1 были удалены. Возможно, файл очищен как ненужный или удален антивирусной программой.
					...
					|en = 'Binary data of the %1 file was deleted. The file might have been cleaned up as unused or deleted by the antivirus software.
					|Please contact the administrator.'"), Строка(ОбъектФайла.Ссылка));
...
Функция ПодготовитьФайлКУдалению(ПрисоединенныйФайл)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'РаботаСФайлами.Удаление файла';
										|en = 'FilesOperations.File deletion'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка, 
...
Процедура ОтразитьОшибкиУдаленияВЖурналеРегистрации(РезультатУдаления)
	Если Не РезультатУдаления.Успешно Тогда
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Работа с файлами';
										|en = 'File management'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка, ,,НСтр("ru = 'Были ошибки при удалении файлов.';
													|en = 'Errors occurred when deleting the files.'"));
	КонецЕсли;
КонецПроцедуры
...
Функция СобытиеЖурналаРегистрацииСинхронизация()
	...
	Возврат НСтр("ru = 'Синхронизация файлов с облачным сервисом';
				|en = 'Synchronize files with cloud service'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Процедура ВыполнитьСинхронизациюФайловОблачнымСервисом(ПараметрыСинхронизации)
	...
	Если ТаблицаФайлов = Неопределено Тогда
		ТекстСобытия = НСтр("ru = 'Не удалось получить таблицу файлов из информационной базы, синхронизация не выполнена.';
							|en = 'Cannot get a file table from the infobase. Synchronization is not completed.'");
		ЗаписатьВЖурналСобытийСинхронизацииФайлов(ТекстСобытия, ПараметрыСинхронизации.УчетнаяЗапись, УровеньЖурналаРегистрации.Ошибка);
		Возврат;
	КонецЕсли;
	...
	Если ОбъектовОбработано = 0 И ПроблемныхОбъектов <> 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Процедуре ПеренестиЭлектронныеПодписиИСертификатыШифрованияВРегистрыСведений не удалось обработать некоторые объекты (пропущены): %1';
				|en = 'Procedure MoveDigitalSignaturesAndEncryptionCertificatesToInformationRegisters failed to process and skipped %1 objects.'"),
			ПроблемныхОбъектов);
		...
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(),
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Процедура ПеренестиЭлектронныеПодписиИСертификатыШифрованияВРегистрыСведений обработала очередную порцию объектов: %1';
					|en = 'The MoveDigitalSignaturesAndEncryptionCertificatesToInformationRegisters procedure has processed objects: %1.'"),
				ОбъектовОбработано));
	КонецЕсли;
