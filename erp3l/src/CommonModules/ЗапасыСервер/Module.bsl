...
Процедура ОписаниеПроверок_ОформлениеВыкупаТоваров(ТаблицаПроверок) Экспорт
	...
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru = 'Не оформленные выкупы товаров, принятых на ответственное хранение';
			|en = 'Non-registered redemptions of goods received for custody'", ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru = 'Должны быть оформлены документы ""Выкуп товаров с хранения"".';
			|en = 'The ""Redemption of goods from storage"" documents should be registered.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	Если РазмерыВременныхТаблиц.ВТОстаткиКомитента = 0 И РазмерыВременныхТаблиц.ВТОстаткиКомиссионера = 0 Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			...
			НСтр("ru = 'Нет остатков в регистре ""Товары к оформлению отчетов комитенту"" по схеме Интеркампани';
				|en = 'No balance in the ""Goods to be registered in reports to consignor"" register under Intercompany'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ПроверкаОформленияОтчетовКомитентуМеждуОрганизациями(ПараметрыПроверки) Экспорт
	...
	СписокПолей.Добавить("Комиссионер", 			   НСтр("ru = 'Организация-комиссионер';
															|en = 'Commission agent company'", ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("АналитикаУчетаНоменклатуры", НСтр("ru = 'Аналитика номенклатуры';
															|en = 'Item dimension'", ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("ВидЗапасов", 				   НСтр("ru = 'Вид запасов';
																|en = 'Inventory owner attributes'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ЗакрытиеМесяцаСервер.ДополнитьПараметрыРегистрацииПроблемПроверки(ПараметрыРегистрации,
		...
		НСтр("ru = 'Обнаружены товары, требующие оформления отчетов комитенту от организации ""%1"" на конец периода %2';
			|en = 'Goods were detected that require reports to the consignor from the ""%1"" company as of the end of %2 period'", ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей);
...
Процедура ОписаниеПроверок_ОформлениеОтчетовКомитентамОСписании(ТаблицаПроверок) Экспорт
	...
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru = 'Не оформленные отчеты комитентам о списании';
			|en = 'Non-registered write-off reports to consignors'", ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru = 'Должны быть оформлены документы ""Отчет комитенту о списании"".';
			|en = 'The ""Purchase invoice — Consignment stock lost"" documents should be registered.'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ПроверкаОформленияОтчетовКомитентуОСписании(ПараметрыПроверки) Экспорт
	...
	ПараметрыРегистрации = ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		...
		НСтр("ru = 'Обнаружены товары, требующие оформления отчетов комитенту о списании от организации ""%1"" на конец периода %2';
			|en = 'Goods were detected that require ""Purchase invoices — Consignment stock lost"" from the ""%1"" company as of the end of %2 period'", ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей);
...
Функция ПодбиратьВТЧТоварыПринятыеНаОтветственноеХранение(ПараметрыЗаполненияВидовЗапасов, ВыборкаТоваров)
	...
	ИначеЕсли ВРег(ПараметрыЗаполненияВидовЗапасов.ПодбиратьВТЧТоварыПринятыеНаОтветственноеХранение) = ВРег("ПоХозяйственнойОперации") Тогда
		...
		Если ВРег(ПодбиратьВТЧТоварыПринятыеНаОтветственноеХранение) <> ВРег("Всегда")
			...
			ТекстИсключения = НСтр("ru = 'В параметрах заполнения видов запасов не определено подставлять или нет в табличную часть товары, принятые на ответственное хранение для хозяйственной операции %ХозяйственнаяОперация%.';
									|en = 'In the parameters for filling in the inventory owner attributes, it is not defined whether to insert the goods accepted for custody in the table for the business transaction %ХозяйственнаяОперация%.'");
	...
	Иначе
		ТекстИсключения = НСтр("ru = 'В параметрах заполнения видов запасов не определено подставлять или нет в табличную часть товары, принятые на ответственное хранение.';
								|en = 'In the filling parameters for inventory owner attributes, it is not defined whether to add the goods held for custody to the table.'");
		ВызватьИсключение ТекстИсключения;
	КонецЕсли;
		...
		Для Каждого СтрокаТаблицы Из СтрокиСПустымВидомЗапасовКомитента Цикл
			...
			Если СтрокаТаблицы.Количество <> 0 Тогда
				ТекстСообщения = НСтр("ru = 'По комитенту %Комитент% не хватает %Количество% %ЕдиницаИзмерения% товара %ПредставлениеНоменклатуры%, реализованного комиссионером %Комиссионер%';
										|en = 'By the %Комитент% consignor, not enough %Количество% %ЕдиницаИзмерения% of the %ПредставлениеНоменклатуры% goods sold by the %Комиссионер% commission agent'");
			...
			Если ЭтоОтчетОПродажах
				...
				ТекстСообщения = НСтр("ru = 'Товар: %ПредставлениеНоменклатуры% Сумма продажи превышает сумму реализованного товара комиссионером на %Сумма% %Валюта%';
										|en = 'Goods: %ПредставлениеНоменклатуры% The sale amount exceeds the amount of goods sold by the commission agent by %Сумма% %Валюта%'");
				ТекстСообщения = СтрЗаменить(ТекстСообщения,
											"%ПредставлениеНоменклатуры%",
											НоменклатураКлиентСервер.ПредставлениеНоменклатуры(
												СтрокаТаблицы.АналитикаУчетаНоменклатуры.Номенклатура,
...
Процедура СообщитьОбОшибкахКонтроляТоваровОрганизаций(ДокументОбъект, ТаблицаОшибок, ПараметрыЗаполненияВидовЗапасов, Организация = Неопределено)
	...
	Если ПараметрыЗаполненияВидовЗапасов.СообщатьОбОшибкахЗаполнения
		...
		Для Каждого СтрокаТаблицы Из ТаблицаОшибок Цикл
			...
			ТекстСообщения = НСтр("ru = 'По организации %Организация% не хватает %Количество% %ЕдиницаИзмерения% товара %ПредставлениеНоменклатуры%, %МестоХраненияНаименование% %Склад% с указанными номерами ГТД по налогообложению %НалогообложениеНДС%';
									|en = 'By company %Организация% there is not enough %Количество% %ЕдиницаИзмерения% goods %ПредставлениеНоменклатуры%, %МестоХраненияНаименование% %Склад% with the specified CCD numbers by taxation %НалогообложениеНДС%'");
			
			Если Не СтрокаТаблицы.НеУказанНомерГТД
				И Не КонтролироватьНалогообложениеНДС Тогда
				
				ТекстСообщения = НСтр("ru = 'По организации %Организация% не хватает %Количество% %ЕдиницаИзмерения% товара %ПредставлениеНоменклатуры%, %МестоХраненияНаименование% %Склад%';
										|en = 'By company %Организация% there is not enough %Количество% %ЕдиницаИзмерения% goods %ПредставлениеНоменклатуры%, %МестоХраненияНаименование% %Склад%'");
				
			ИначеЕсли Не СтрокаТаблицы.НеУказанНомерГТД Тогда
				ТекстСообщения = НСтр("ru = 'По организации %Организация% не хватает %Количество% %ЕдиницаИзмерения% товара %ПредставлениеНоменклатуры%, %МестоХраненияНаименование% %Склад% по налогообложению %НалогообложениеНДС%';
										|en = 'By company %Организация% there is not enough %Количество% %ЕдиницаИзмерения% goods %ПредставлениеНоменклатуры%, %МестоХраненияНаименование% %Склад% by taxation %НалогообложениеНДС%'");
			ИначеЕсли Не КонтролироватьНалогообложениеНДС Тогда
				ТекстСообщения = НСтр("ru = 'По организации %Организация% не хватает %Количество% %ЕдиницаИзмерения% товара %ПредставлениеНоменклатуры%, %МестоХраненияНаименование% %Склад% с указанными номерами ГТД';
										|en = 'By company %Организация% there is not enough %Количество% %ЕдиницаИзмерения% goods %ПредставлениеНоменклатуры%, %МестоХраненияНаименование% %Склад% with specified CCD numbers'");
			КонецЕсли;
...
Процедура ТаблицаОстатковДляВозвратаПереданныхПереработчикуТоваров(ДокументОбъект, МенеджерВременныхТаблиц, ПараметрыЗаполненияВидовЗапасов, ДатаОстатков)
	...
	|			ТОГДА 0
	|		ИНАЧЕ ВводОстатковПереданныхПереработчикамТоваров.КоличествоПоРНПТ
	|	КОНЕЦ КАК КоличествоПоРНПТ,
	|	ДАТАВРЕМЯ(1, 1, 1) КАК ДатаРеализации
	|ИЗ
...
Процедура ПередУдалениемРегистратораПроверитьНаличиеЗаписейВРезервахТоваровОрганизаций(Источник, Отказ) Экспорт
	...
	Если Не Запрос.Выполнить().Пустой() Тогда
		...
		ТекстИсключения = НСтр("ru = 'По документу %Документ% есть движения в регистре %Регистр1%. Удаление документа невозможно.';
								|en = 'The %Документ% contains movements in the %Регистр1% register. Cannot remove the document.'");
