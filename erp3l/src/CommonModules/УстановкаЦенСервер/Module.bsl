						...
						Исключение
							...
							Если ЗначениеЗаполнено(СтрокаТаблицыЦен.Характеристика) Тогда
								ТекстСообщения = НСтр("ru = 'Для номенклатуры ""%Номенклатура%"" с характеристикой ""%Характеристика%"" не рассчитана цена по виду цен ""%ВидЦены%"" по причине: %ОписаниеОшибки%';
														|en = 'Price is not calculated for variant ""%Характеристика%"" of item ""%Номенклатура%"" by price type %ВидЦены%. Reason: %ОписаниеОшибки%'");
							Иначе
								ТекстСообщения = НСтр("ru = 'Для номенклатуры ""%Номенклатура%"" не рассчитана цена по виду цен ""%ВидЦены%"" по причине: %ОписаниеОшибки%';
														|en = 'Price is not calculated for  item %Номенклатура% by price type %ВидЦены%. Reason: %ОписаниеОшибки%'");
							КонецЕсли;
	...
	Если ДополнительныеПараметры = Неопределено Тогда
		...
		Если Не ЗначениеЗаполнено(РеквизитыОбъекта.НастройкиКлючаЦенПоХарактеристике) Тогда
			...
			Исключение
				...
				РезультатОбновления.ОписаниеОшибки = НСтр("ru = 'Ошибка при вида номенклатуры';
															|en = 'Item kind error'", ОбщегоНазначения.КодОсновногоЯзыка());
			КонецПопытки;
