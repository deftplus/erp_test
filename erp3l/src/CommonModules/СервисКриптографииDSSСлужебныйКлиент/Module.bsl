			...
			Иначе
				...
				СоздатьТранзакцию(НастройкиПользователя,
						...
						НСтр("ru = 'Подтвердите создание запроса на сертификат.';
							|en = 'Confirm certificate request creation.'", КодЯзыка()),
						, 
						ВходящийКонтекст.ПараметрыОперации);
			КонецЕсли;
...
Процедура СоздатьЗапросНаСертификатОшибка(РезультатВыполнения, ВходящийКонтекст)
	...
	ИначеЕсли СервисКриптографииDSSКлиентСервер.ЭтоОшибкаНеобработанныеЗапросы(РезультатВыполнения.Ошибка) Тогда
		...
		ВывестиВопрос(ОповещениеСледующее, 
			...
			|en = 'Cannot create a new certificate request, because there are pending certificate requests on the server. 
			|Would you like to see the list of pending requests?'"),
			СписокКоманд,
			30,
...
Процедура ПоказатьЗапросНаСертификат(НастройкиПользователя, ОповещениеОЗавершении, ЗапросНаСертификат, ПараметрыОперации = Неопределено) Экспорт
	...
	ПараметрыЦикла.Вставить("ВидОперации", НСтр("ru = 'Показать сведения запроса на сертификат';
												|en = 'Show certificate request information'", КодЯзыка()));
	ПараметрыЦикла.Вставить("ОповещениеОЗавершении",ОповещениеОЗавершении);
	ПараметрыЦикла.Вставить("НастройкиПользователя",НастройкиПользователя);
...
Функция ПолучитьРезультатВыполненияВФоне(ДлительнаяОперация, ВходящийКонтекст = Неопределено) Экспорт
	...
	Если ДлительнаяОперация = Неопределено Тогда
		Ответ.Ошибка = НСтр("ru = 'Работа сервиса криптографии DSS завершилась с ошибкой.';
							|en = 'DSS cryptography service operation failed.'", КодЯзыка());
	...
	Иначе
		Ответ.Ошибка = НСтр("ru = 'Вызов API сервиса криптографии DSS завершился с ошибкой при выполнении.';
							|en = 'Calling DSS cryptography service API failed.'", КодЯзыка()) 
...
Процедура АктивироватьИУстановитьСертификатПослеПроверкиАвторизация(ОтветСервиса, ВходящийКонтекст) Экспорт
	...
	Если НЕ ОтветСервиса.Выполнено Тогда
		...
		ПолучитьОписаниеОшибки(РезультатВыполнения,
							...
							НСтр("ru = 'Ошибка авторизации при установке сертификата в контейнер ключа и хранилище сертификатов сервера DSS';
								|en = 'An authorization error occurred when installing the certificate into the key container and the DSS server certificate storage'", КодЯзыка()));
