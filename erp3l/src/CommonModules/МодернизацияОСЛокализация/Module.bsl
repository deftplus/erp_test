...
Процедура ПриЧтенииСозданииНаСервере(Форма) Экспорт
	...
	ВнеоборотныеАктивыКлиентСервер.УстановитьВидимостьЗначенияСпискаВыбора(
		...
		НСтр("ru = 'Увеличение стоимости без начисления амортизации';
			|en = 'Cost increase without depreciation accrual'"));
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	МАКСИМУМ(ПервоначальныеСведенияОС.Период) КАК Период
	|ПОМЕСТИТЬ ТаблицаОС
	|ИЗ
	|	Документ.МодернизацияОС2_4.ОС КАК ТаблицаОС
	...
	|			И (ПервоначальныеСведенияОС.Организация = ТаблицаОС.Ссылка.Организация)
	|			И (ПервоначальныеСведенияОС.Период < ТаблицаОС.Ссылка.Дата)
	|ГДЕ
	|	ТаблицаОС.Ссылка.Проведен
	|	И ТаблицаОС.Ссылка.ПорядокУчетаНУ = ЗНАЧЕНИЕ(Перечисление.ПорядокВключенияСтоимостиОСВСоставРасходовНУ.ПустаяСсылка)
	|
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаОС.Ссылка,
	|	ТаблицаОС.ОсновноеСредство,
	...
	|		ПО (ПервоначальныеСведенияОС.ОсновноеСредство = ТаблицаОС.ОсновноеСредство)
	|			И (ПервоначальныеСведенияОС.Организация = ТаблицаОС.Ссылка.Организация)
	|			И (ПервоначальныеСведенияОС.Период = ТаблицаОС.Период)
	|ГДЕ
	|	ПервоначальныеСведенияОС.ПорядокУчетаНУ = ЗНАЧЕНИЕ(Перечисление.ПорядокВключенияСтоимостиОСВСоставРасходовНУ.ВключениеВРасходыПриПринятииКУчету)";
...
Процедура ДополнитьСписокЗапросовПриОбработкеДанныхДляПереходаНаНовуюВерсию(СписокЗапросов) Экспорт
...
Функция ВспомогательныеДанныеПриОбработкеДанныхДляПереходаНаНовуюВерсию(РезультатЗапроса) Экспорт
	
	ВспомогательныеДанные = Неопределено;
	
	//++ Локализация
	ВспомогательныеДанные = РезультатЗапроса[РезультатЗапроса.ВГраница()-1].Выгрузить();
	ВспомогательныеДанные.Индексы.Добавить("Ссылка");
	//-- Локализация
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(ДанныеОбъекта, ВспомогательныеДанные) Экспорт

	//++ Локализация
	
	Если НЕ ЗначениеЗаполнено(ДанныеОбъекта.ПорядокУчетаНУ) Тогда
		
		ДанныеСтроки = ВспомогательныеДанные.Найти(ДанныеОбъекта.Ссылка, "Ссылка");
		Если ДанныеСтроки <> Неопределено Тогда
			ДанныеОбъекта.ПорядокУчетаНУ = Перечисления.ПорядокВключенияСтоимостиОСВСоставРасходовНУ.СтоимостьНеВключаетсяВРасходы;
		КонецЕсли;
