...
Процедура ОпределитьПроверкиВеденияУчета(ГруппыПроверок, Проверки) Экспорт
	...
	ГруппаПроверокКонтрольВеденияУчетаПоНДФЛ.Наименование					= НСтр("ru = 'Учет по НДФЛ';
																					|en = 'Personal income tax accounting'");
	...
	Проверка.Причины						= НСтр("ru = 'Потеря связи со справочником кодов дохода';
													|en = 'Connection to the income code catalog is lost'");
	Проверка.Рекомендация					= НСтр("ru = 'Указать правильные коды доходов';
													|en = 'Specify correct income codes'");
...
Процедура СправкиНДФЛПроверитьДанныеНалоговогоАгента(Объект, ДанныеНалоговогоАгента, СпособФормирования, Отказ) Экспорт
		...
		Если СокрЛП(ДанныеНалоговогоАгента.НазваниеОрганизации) = "" Тогда
			ОбщегоНазначения.СообщитьПользователю(
				НСтр("ru = 'Для организации должно быть указано сокращенное наименование организации.';
					|en = 'Abbreviated company name should be specified for the company.'"), 
				Объект.Ссылка, 
	...
	Иначе		
		...
		Если НЕ СтроковыеФункцииКлиентСерверРФ.ТолькоКириллицаВСтроке(
			...
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Для физического лица-работодателя Фамилия, Имя, Отчество должно быть введены русскими буквами.';
														|en = 'Full name for individual employer should be specified with Russian letters.'"), 
		...
		Если ПустаяСтрока(ДанныеНалоговогоАгента.Фамилия) Или ПустаяСтрока(ДанныеНалоговогоАгента.Имя) Тогда
			ОбщегоНазначения.СообщитьПользователю(
				НСтр("ru = 'Для физического лица-работодателя должны быть указаны, как минимум, фамилия и имя.';
					|en = 'At least last name and name should be specified for an individual employer.'"), 
				Объект.Ссылка, 
				"Организация",
				"Объект", 
				Отказ);
	...
	Если СпособФормирования = Перечисления.ПорядокФормированияСправкиОДоходахФизическогоЛица.ВРазрезеКодовОКАТО Тогда
		Если ЗначениеЗаполнено(ДанныеНалоговогоАгента.КодНалоговогоОргана) 
			...
			ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Неверно указан код налогового органа.';
														|en = 'Incorrect tax authority code.'"), Объект.Ссылка, "КодИФНС",, Отказ);
		КонецЕсли;
			...
			Если НЕ ЗначениеЗаполнено(ДанныеНалоговогоАгента.ОКТМО) Тогда
				ОбщегоНазначения.СообщитьПользователю(
					НСтр("ru = 'Не указан код по ОКТМО, по которому формируются сведения.';
						|en = 'RNCMT code to generate information is not specified.'"), 
					КлючДанных, 
					"РегистрацияВНалоговомОргане",
					"Объект", 
					Отказ);		
			ИначеЕсли СтрДлина(СокрЛП(ДанныеНалоговогоАгента.ОКТМО)) <> 11 
				...
				ОбщегоНазначения.СообщитьПользователю(
					НСтр("ru = 'Неверно указан код по ОКТМО, по которому формируются сведения.';
						|en = 'RNCMT code to generate information is specified incorrectly.'"),
					КлючДанных, 
					"РегистрацияВНалоговомОргане",
					"Объект", 
					Отказ);		
			...
			Если НЕ ЗначениеЗаполнено(ДанныеНалоговогоАгента.ОКАТО) Тогда
				ОбщегоНазначения.СообщитьПользователю(
					НСтр("ru = 'Не указан код по ОКАТО, по которому формируются сведения.';
						|en = 'RNCPS code to generate information is not specified.'"), 
					КлючДанных, 
					"РегистрацияВНалоговомОргане",
					"Объект", 
					Отказ);		
			ИначеЕсли СтрДлина(СокрЛП(ДанныеНалоговогоАгента.ОКАТО)) <> 11 Тогда	
				ОбщегоНазначения.СообщитьПользователю(
					НСтр("ru = 'Неверно указан код по ОКАТО, по которому формируются сведения.';
						|en = 'RNCPS code to generate the information is specified incorrectly.'"),
					КлючДанных, 
					"РегистрацияВНалоговомОргане",
					"Объект", 
					Отказ);		
		...
		Если ДанныеНалоговогоАгента.ЭтоЮрЛицо И Не ЗначениеЗаполнено(ДанныеНалоговогоАгента.КПП) Тогда
			ОбщегоНазначения.СообщитьПользователю(
				НСтр("ru = 'Не указан КПП, по которому формируются сведения.';
					|en = 'CRTR code to generate information is not specified.'"), 
				КлючДанных, 
				"РегистрацияВНалоговомОргане",
				"Объект", 
				Отказ);		
	...
	Если НомерКорректировки = 99 Тогда
		...
		Если Доходы.Количество() > 0 Тогда
			Для Каждого СтрокаДоходы Из Доходы Цикл
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 обнаружены сведения о доходах в аннулирующей справке';
																							|en = '%1 information about income is found in the canceling certificate  '"), НачалоСообщения);
		...
		Если Вычеты.Количество() > 0 Тогда
			Для Каждого СтрокаВычеты Из Вычеты Цикл
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 обнаружены сведения о вычетах в аннулирующей справке';
																							|en = '%1 information about deductions is found in the canceling certificate  '"), НачалоСообщения);
		...
		КонецЦикла;
		Если ЕстьИтоговыеДанные Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 обнаружены сведения о налогах в аннулирующей справке';
																						|en = '%1 information about taxes is found in the canceling certificate  '"), НачалоСообщения);
	...
	Иначе
		...
		Для Каждого СтрокаДоходы Из Доходы Цикл
			...
			Если ЗначениеЗаполнено(СтрокаДоходы.СуммаВычета) И Не ЗначениеЗаполнено(СтрокаДоходы.КодВычета) Тогда 
				ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 в строке %2 списка ""Сведения о доходах"" не заполнен код вычета.';
																							|en = '%1 deduction code is not populated in line %2 of the ""Information about income"" list.'"), НачалоСообщения, НомерСтроки);
		...
		Если ДляНалогаНаПрибыль Тогда // среди вычетов могут быть только стандартные
			...
			Для Каждого СтрокаВычеты Из Вычеты Цикл
				...
				Если ДопустимыеВычеты.Найти(СтрокаВычеты.КодВычета) = Неопределено Тогда 
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 в строке %2 списка ""Сведения о вычетах"" указан недопустимый код вычета.';
																								|en = '%1 invalid deduction code is specified in line %2 of the ""Information about deductions"" list.'"), НачалоСообщения, НомерСтроки);
					ОбщегоНазначения.СообщитьПользователю(ТекстОшибки, Ссылка, ПутьКДанным + "КодВычета", ,Отказ);
					Ошибки.Добавить(ТекстОшибки);
				КонецЕсли;
			...
			Если ЕстьИмущественныеВычеты Или ЕстьСоциальныеВычеты Тогда
				...
				Если ЕстьИмущественныеВычеты Тогда
					Если СтрокиУведомленийНаИмущественныйВычет.Количество() = 0 Тогда
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 не указаны данные уведомления о праве на имущественный вычет';
																									|en = '%1 data of notification of property-related deduction entitlement is not specified'"), НачалоСообщения);
					...
					Иначе
						Для Каждого СтрокаУведомлений Из СтрокиУведомленийНаИмущественныйВычет Цикл
							...
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 в данных уведомления о праве на имущественный вычет указаны не все реквизиты';
																										|en = '%1 not all attributes are specified in the data of notification of property-related deduction entitlement'"), НачалоСообщения);
							...
							ИначеЕсли Не СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(СтрокаУведомлений.КодНалоговогоОрганаУведомления) Тогда
								ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 неверно указан код налогового органа, выдавшего уведомление о праве на имущественный вычет';
																											|en = '%1 code of tax authority which issued notification confirming the property-related deduction entitlement is specified incorrectly'"), НачалоСообщения);
								ОбщегоНазначения.СообщитьПользователю(ТекстОшибки, Ссылка, ПутьКДанным + "КодНалоговогоОрганаУведомления",,Отказ);	
								Ошибки.Добавить(ТекстОшибки);
							КонецЕсли;
				...
				Если ЕстьСоциальныеВычеты Тогда
					Если СтрокиУведомленийНаСоциальныйВычет.Количество() = 0 Тогда
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 не указаны данные уведомления о праве на социальный вычет';
																									|en = '%1 data of notification of social tax deduction entitlement is not specified'"), НачалоСообщения);
					...
					Иначе
						Для каждого СтрокаУведомлений Из СтрокиУведомленийНаСоциальныйВычет Цикл
							...
							ПутьКДанным = "УведомленияНОоПравеНаВычеты[" + Формат(СтрокаУведомлений.НомерСтроки - 1, "ЧН=0; ЧГ=") + "].";
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 в данных уведомления о праве на социальный вычет указаны не все реквизиты';
																										|en = '%1 not all attributes are specified in the data of notification of social tax deduction entitlement'"), НачалоСообщения);
							...
							Если Не ЗначениеЗаполнено(СтрокаУведомлений.КодНалоговогоОрганаУведомления) Тогда
								...
								Ошибки.Добавить(ТекстОшибки);
							ИначеЕсли Не СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(СтрокаУведомлений.КодНалоговогоОрганаУведомления) Тогда
								ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 неверно указан код налогового органа, выдавшего уведомление о праве на социальный вычет';
																											|en = '%1 code of tax authority which issued notification confirming the social deduction entitlement is specified incorrectly'"), НачалоСообщения);
								ОбщегоНазначения.СообщитьПользователю(ТекстОшибки, Ссылка, ПутьКДанным + "КодНалоговогоОрганаУведомления",,Отказ);	
								Ошибки.Добавить(ТекстОшибки);
							КонецЕсли;
		...
		Для каждого ЗначениеПеречисления Из Перечисления.НДФЛСтавки Цикл
			...
			Если ДоходыПоСтавке.Количество() > 0 
				...
				Если ОбщаяСуммаДохода <> ДоходыПоСтавке.Итог("СуммаДохода") Тогда
					...
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 общая сумма дохода по ставке %2 не совпадает с итогом таблицы доходов';
																								|en = '%1 total income amount at a rate of %2 does not match the income table total'"), НачалоСообщения, ЗначениеПеречисления);
				...
				Если ОблагаемаяСуммаДохода <> ДоходыПоСтавке.Итог("СуммаДохода") - ДоходыПоСтавке.Итог("СуммаВычета") - ВсегоПрочихВычетов Тогда
					...
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1 сумма налоговой базы по ставке %2 не совпадает с итогами таблиц доходов и вычетов';
																								|en = '%1 tax base amount at a rate of %2 does not match totals of income and deductions tables'"), НачалоСообщения, ЗначениеПеречисления);
					ОбщегоНазначения.СообщитьПользователю(ТекстОшибки, Ссылка, ПутьКДаннымСотрудникаДляОшибок + ИмяПоляИтогов,,Отказ);	
					Ошибки.Добавить(ТекстОшибки);
				КонецЕсли;
			КонецЕсли;
	...
	Для Каждого Ссылка Из МассивОбъектов  Цикл
		...
		Если СтрокиДанныхПоСотрудникам.Количество() = 0 Тогда
			ШаблонПредупреждения = НСтр("ru = 'Документ %1 не содержит сотрудников.';
										|en = 'Document %1 does not contain employees.'");
		...
		Для каждого СтрокаДанных Из СтрокиДанныхПоСотрудникам Цикл 
			...
			Если ДоходыПоВидам.Количество() = 0 И ВычетыПоВидам.Количество() = 0 И Уведомления.Количество() = 0 Тогда
				ШаблонПредупреждения = НСтр("ru = 'В документе %1 по сотруднику %2 отсутствуют доходы.';
											|en = 'Document %1 does not contain income for employee %2.'");
...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Обработка кодов дохода по НДФЛ. До завершения обработки рекомендуется воздержаться от начисления зарплаты, отпускных, б/л и пр.';
									|en = 'Processing PIT income codes. It is not recommended that you accrue salary, leave bonuses, sick leave payments and other until the processing is complete.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполнение документа Возврат НДФЛ.';
									|en = 'Fill in the ""PIT return"" document.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Регистрация межрасчетов как учтенных при расчете НДФЛ документов.';
									|en = 'Registration of payments outside the payroll period as recorded when calculating PIT documents.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Регистрация документов выплаты как учтенных при расчете НДФЛ документов.';
									|en = 'Registration of payment documents as recorded when calculating PIT documents.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Регистрация Операций учета как учтенных при расчете НДФЛ документов.';
									|en = 'Registration of Accounting transactions as recorded when calculating PIT documents.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Исправление зачтенных сумм авансовых платежей, введенных в Операциях учета по НДФЛ.';
									|en = 'Correct set advance payment amounts introduced in income tax accounting Procedures.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Дополнение сведений о доходах, введенных в Операциях учета по НДФЛ, данными об основаниях.';
									|en = 'Add the data about reasons to the information about income introduced in the income tax accounting Procedures.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполнение НДФЛ к перечислению.';
									|en = 'Populating PIT for transfer.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет даты запрета редактирования документов ""Авансовый платеж по НДФЛ"".';
									|en = 'Fills in period-end closing dates for editing ""PIT advance payment"" documents.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет даты запрета редактирования документов ""Заявление на вычеты по НДФЛ"".';
									|en = 'Fills in period-end closing dates for editing ""Application for deductions from PIT"" documents.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет даты запрета редактирования документов ""Отмена стандартных вычетов НДФЛ"".';
									|en = 'Fills in period-end closing dates for editing ""Cancellation of standard PIT deductions"" documents.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет даты запрета редактирования документов ""Уведомление НО о праве на вычеты"".';
									|en = 'Fills in period-end closing dates for editing ""TA notification of deduction entitlement"" documents.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Исправление сроков уплаты 24 июня и 1 июля';
									|en = 'Correction of payment deadlines on June 24 and July 1'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Расчет налога на прибыль, принимаемого в зачет НДФЛ по дивидендам резидентов РФ.';
									|en = 'Calculation of income tax, taken into account of personal income tax on dividends for residents of the Russian Federation.'");
	...
	Обработчик.Комментарий = НСтр("ru = 'Исправление сроков уплаты в мае 2021';
									|en = 'Correction of due dates in May 2021'");
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.18.272";
	...
	Обработчик.Комментарий = НСтр("ru = 'Исправление сроков уплаты в ноябре 2021';
									|en = 'Correction of payment due dates in November 2021'");
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия = "3.1.18.390";
	...
	Обработчик.Комментарий = НСтр("ru = 'Обработка вычетов в справках о доходах для сотрудников.';
									|en = 'Обработка вычетов в справках о доходах для сотрудников.'");
