...
Функция ПечатныеФормыОбъектов(ОписанияПечатныхФорм, ОбъектыПечати, ПараметрыПечати, ИдентификаторХранилища) Экспорт
	...
	РеквизитыОбъектов = РеквизитыОбъектовПечати(ОбъектыПечати.ВыгрузитьЗначения());
		...
		Для Каждого СоответствиеОбъектаПечатнойФорме Из ПечатныеФормыПоОбъектам Цикл
			...
			Иначе
				...
				Для Каждого СоответствиеОбъектаДетальнойПечатнойФорме Из ДетальныеПечатныеФормыПоОбъектам Цикл
					...
					ФИОДобавлено = Ложь;
					ОбъектыДетализации = ОбщегоНазначения.ВыгрузитьКолонку(ОбъектДетализацииПечати, "Значение");
...
Процедура УдалитьФайлыИзОбработкиПользователя(ФайлыКОбработке) Экспорт
	...
	РегистрыСведений.ЗапланированныеДействияСПечатнымиФормами.УдалитьФайлыИзОбработкиПользователя(ФайлыКОбработке);
...
Функция ПараметрыОткрытияФормыПодписанияПрисоединенныхФайлов(ПрисоединенныеФайлы) Экспорт
	...
	Возврат ПараметрыОткрытияФормыПодписанияПечатныхФорм(
		РегистрыСведений.ПодписанныеПечатныеФормы.ДанныеФайловНаПодпись(ПрисоединенныеФайлы));
...
Функция ФайлыНаПодписьПользователя() Экспорт
	Возврат РегистрыСведений.ЗапланированныеДействияСПечатнымиФормами.ФайлыНаПодписьПользователя(Истина).
		ВыгрузитьКолонку("ФайлОбъекта");
КонецФункции

Функция ЭлектронныеДокументыПоСсылкамНаДокументыКадровогоЭДО(ДокументыКадровогоЭДО) Экспорт
	
	ЭлектронныеДокументыОбъектов = ОбщегоНазначения.ЗначениеРеквизитаОбъектов(
		ДокументыКадровогоЭДО, "ЭлектронныйДокумент");
	
	Возврат ОбщегоНазначения.ВыгрузитьКолонку(ЭлектронныеДокументыОбъектов, "Значение");
...
Процедура ОбновитьУведомленияВФоне() Экспорт
	...
	ДлительныеОперации.ВыполнитьПроцедуру(ПараметрыВыполнения, "ОбсужденияБЗК.ОбновитьУведомленияОНеобходимостиПодписанияПечатныхФорм");
...
Функция РеквизитыОбъектовПечати(ОбъектыПечати)
	...
	Для Каждого ОбъектыТипа Из ТипыОбъектов Цикл
		...
		Если ОбщегоНазначения.ЭтоДокумент(МетаданныеТипа) Тогда
			...
			ТекстЗапрос = СтрЗаменить(ТекстЗапрос, "Документ.Увольнение", МетаданныеТипа.ПолноеИмя());
		...
		ИначеЕсли МетаданныеТипа = Метаданные.Справочники.Сотрудники Тогда
		...
		Если ДанныеПечатнойФормы.ПечатнаяФорма <> Неопределено Тогда
			...
			Если ЗначениеЗаполнено(ДанныеПечатнойФормы.ФайлОбъекта) Тогда
				...
				Попытка
					...
					ДокументКадровогоЭДО = Документы.ДокументКадровогоЭДО.Добавить(
...
Функция ИменаВыводимогоФайла(Знач ИмяФайла, Знач Расширение)
	...
	ИменаФайла.Вставить("ИмяФайлаДляПечати", Файл.ИмяБезРасширения + НСтр("ru = '_для_печати';
																			|en = '_to_print'") + Файл.Расширение);
