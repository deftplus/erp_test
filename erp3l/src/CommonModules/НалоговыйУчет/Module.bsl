...
Процедура ЗаполнитьОтчетПрибыльФормаОтчета2015Кв1(Контейнер, ТаблицаРасшифровки, ИДРедакцииОтчета, КонтекстРасчета) Экспорт
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПорядокУчетаОСБУ.СрезПоследних(&ДатаКон, ) КАК ПорядокУчетаОСБУ
...
Функция МаксимальнаяДоляУменьшенияПрибылиЗаСчетУбытковПрошлыхЛет(Период) Экспорт
	...
	// Ст. 283 НК РФ в редакции 305-ФЗ от 02.07.2021
	// В отчетные (налоговые) периоды с 1 января 2017 года по 31 декабря 2024 года
