			...
			Если Строка.Спецификация.Пустая() Тогда
				ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Не удалось подобрать спецификацию для материала в строке %1 (спецификация ""%2"")';
						|en = 'Cannot select the specification for the material in the line %1 (specification ""%2"")'"),
					Строка.НомерСтроки,
...
Процедура ПодобратьОстаткиПолуфабрикатов(СтрокиПотребности, Материалы, Резервы)
	...
	Запрос = Новый Запрос(
		...
		|	ЕСТЬNULL(ОстаткиОбособленно.Свободно, 0) - ЕСТЬNULL(РезервыОбособленно.Количество, 0)     КАК ОстатокОбособленно,
		|	ЕСТЬNULL(ОстаткиНеобособленно.Свободно, 0) - ЕСТЬNULL(РезервыНеобособленно.Количество, 0) КАК ОстатокНеобособленно
		|ИЗ
		|	ВТМатериалы КАК ВТМатериалы
		|
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РаспределениеЗапасов КАК ОстаткиОбособленно
		|		ПО (ОстаткиОбособленно.Состояние = ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.ОстатокНаСкладе))
		|			И ВТМатериалы.Номенклатура = ОстаткиОбособленно.Номенклатура
		...
		|			И ВТМатериалы.Обособленно
		|
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РаспределениеЗапасов КАК ОстаткиНеобособленно
		|		ПО (ОстаткиНеобособленно.Состояние = ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.ОстатокНаСкладе))
		|			И ВТМатериалы.Номенклатура = ОстаткиНеобособленно.Номенклатура
...
Функция РазделитьПотребностиНаПартии(Потребности, ПараметрыРазузлования)
	...
	Потребности.Сортировать("Начало, Окончание, Уровень");
