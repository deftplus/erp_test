	...
	Для Каждого Приглашение Из Приглашения Цикл
		...
		Исключение
			...
			ЭлектронноеВзаимодействие.ОбработатьОшибку(НСтр(
				...
				|en = 'Archive electronic document exchange invitations'"), ПодробноеПредставлениеОшибки(
...
Функция ТаблицаДанныхУчастниковОбмена(КлючиСинхронизации, КонтекстДиагностики, ПроверочныйЗапрос = Ложь,
	...
	КлючОбъекта = "EF_00_00366771";
	КлючНастроек = "ДатыЗапросаПриглашенийОбнулены";
	ДатыЗапросаПриглашенийОбнулены = ОбщегоНазначения.ХранилищеОбщихНастроекЗагрузить(КлючОбъекта, КлючНастроек, Ложь);
	
	ИдентификаторыДляОбнуленияДатыЗапроса = Новый Массив;
	Если Не ДатыЗапросаПриглашенийОбнулены И Не ПроверочныйЗапрос Тогда
		Запрос = Новый Запрос;
		Запрос.Текст = 
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		...
		|ГДЕ
		|	НастройкиОтправкиЭлектронныхДокументовПоВидам.СпособОбменаЭД В(&СпособыОбменаЧерезОператора)
		|	И ПриглашенияКОбменуЭлектроннымиДокументами.Статус ЕСТЬ NULL";
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			ИдентификаторыДляОбнуленияДатыЗапроса.Добавить(ВыборкаДетальныеЗаписи.ИдентификаторОтправителя);
		КонецЦикла;
	КонецЕсли;
	...
	Для Каждого КлючИЗначение Из КлючиСинхронизации Цикл
		...
		Если ИдентификаторыДляОбнуленияДатыЗапроса.Найти(ИдентификаторУчетнойЗаписи) <> Неопределено Тогда
			СтрокаТаблицыПараметрыЗапросаПриглашений.ДатаПолученияПриглашенийИзНастройки
				= СервисЭДО.ДатаЗапросаПоУмолчанию();
		Иначе 
			СтрокаТаблицыПараметрыЗапросаПриглашений.ДатаПолученияПриглашенийИзНастройки
