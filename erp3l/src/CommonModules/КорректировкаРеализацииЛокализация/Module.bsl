...
Функция ПолучитьДанныеДляПечатнойФормыУПД(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	|	МАКСИМУМ(ВЫБОР
	|			КОГДА ТаблицаТоваров.Номенклатура.КодТНВЭД.ПрослеживаемыйТовар
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ЛОЖЬ
...
Функция ТекстОтраженияВРеглУчете() Экспорт
	...
	ТекстыОтражения.Добавить(ТекстВыручкаОтРеализацииСобственногоКомиссионногоТовара());
...
Функция ТекстЗапросаВТОтраженияВРеглУчете() Экспорт
	...
	|	ЕСТЬNULL(ДанныеДокументаОснования.ХозяйственнаяОперация, НЕОПРЕДЕЛЕНО) = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияБезПереходаПраваСобственности) КАК ТоварыВПути,
	|	ЕСТЬNULL(ДанныеДокументаОснования.ХозяйственнаяОперация, НЕОПРЕДЕЛЕНО) В (ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.РеализацияКлиенту),
...
Функция ТекстСебестоимостьРеализованногоТовараПрошлыеПериоды()
	...
	|
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.Производство) КАК ВидСчетаКт,
	|	ЕСТЬNULL(Стоимости.ГруппаПродукции, Строки.ГруппаПродукции) КАК ГруппаФинансовогоУчетаКт,
...
Функция ТекстСебестоимостьРеализованныхРабот()
	...
	|
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.Производство) КАК ВидСчетаКт,
	|	Строки.ГруппаПродукции КАК ГруппаФинансовогоУчетаКт,
	|	Строки.ПодразделениеАналитики КАК МестоУчетаКт,
	|
	|	ЗНАЧЕНИЕ(Справочник.Валюты.ПустаяСсылка) КАК ВалютаКт,
	|	Строки.ПодразделениеАналитики КАК ПодразделениеКт,
...
Функция ТекстНДСПриОтгрузкеБезПереходаПраваСобственности()
	...
	|		Операция.Контрагент
	|	КОНЕЦ КАК СубконтоДт1,
	|	ДанныеДокументаОснования.ДокументОснования КАК СубконтоДт2,
	|	НЕОПРЕДЕЛЕНО КАК СубконтоДт3,
	|
	|	ЕСТЬNULL(Суммы.СуммаНДСРегл, Строки.СуммаНДС) КАК ВалютнаяСуммаДт,
	|	Строки.Количество КАК КоличествоДт,
	...
	|	ПО
	|		Операция.Ссылка = ДанныеДокументаОснования.Ссылка
	|		И ДанныеДокументаОснования.ТоварыВПути
	|		И НАЧАЛОПЕРИОДА(ДанныеДокументаОснования.Дата, ГОД) <> НАЧАЛОПЕРИОДА(ДанныеДокументаОснования.ДатаОтгрузки, ГОД)
	|
	|	ВНУТРЕННЕЕ СОЕДИНЕНИЕ
	|		Документ.КорректировкаРеализации.ВидыЗапасовСписание КАК Строки
	|	ПО
...
Функция ТекстНДСОтгруженногоТовара()
	...
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка) КАК СчетКт,
	|	Операция.Контрагент КАК СубконтоКт1,
	|	ДанныеДокументаОснования.ДокументОснования КАК СубконтоКт2,
	|	НЕОПРЕДЕЛЕНО КАК СубконтоКт3,
	|
	|	ЕСТЬNULL(Суммы.СуммаНДСРегл, Строки.СуммаНДС) КАК ВалютнаяСуммаКт,
	|	0 КАК КоличествоКт,
