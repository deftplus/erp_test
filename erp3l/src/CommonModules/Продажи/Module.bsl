...
Процедура СообщитьОРезультатахКонтроляИзменений(РезультатыКонтроля, Документ, Отказ) Экспорт
	...
	Если ПроведениеДокументов.ЕстьЗаписиВТаблице(Документ, "ДвиженияЗаказыКлиентовИзменение") Тогда
		...
		ШаблонСообщенияСоСкладом = НСтр("ru = 'Номенклатура %1 склад %2
			...
			| Registered more than stated in the instruction %3 by %4 %5'");
	...
	Если ПроведениеДокументов.ЕстьЗаписиВТаблице(Документ,"ДвиженияЗаявкиНаВозвратТоваровОтКлиентовИзменениеСводно")
		...
		ШаблонСообщения = НСтр("ru = 'Номенклатура %1
			...
			|en = 'Items %1
			|Number of the returned ones exceeds the number of ones specified in all registered requests for return of goods and receipt from the custodian under the %2 contract by %3 %4'");
