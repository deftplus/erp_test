	...
	Для каждого СтрокаТЧ Из ТабличнаяЧасть Цикл
		...
		Если Не ЗначениеЗаполнено(СтрокаТЧ.СтавкаНДС) И ЗначениеЗаполнено(НалогообложениеНДС) Тогда
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(СтрокаТЧ, "Номенклатура") Тогда
				СтрокаТЧ.СтавкаНДС = СтавкаНДСПоНоменклатуреИНалогообложению(ДатаАктуальности, СтрокаТЧ.Номенклатура, НалогообложениеНДС);
			Иначе
				СтрокаТЧ.СтавкаНДС = СтавкаНДСПоТипуНалогообложения(НалогообложениеНДС);
			КонецЕсли;
		КонецЕсли;
...
//
Процедура УстановитьПараметрыЗапросаСтавкиНДС(ПараметрыЗапроса) Экспорт
	
	Если Не ПараметрыЗапроса.Свойство("СтавкаНДСБезНДС") Тогда
		СтавкаНДСБезНДС = СтавкаНДСПоТипуНалогообложения(Перечисления.ТипыНалогообложенияНДС.ПродажаНеОблагаетсяНДС);
		ПараметрыЗапроса.Вставить("СтавкаНДСБезНДС", СтавкаНДСБезНДС);
	КонецЕсли;
	
	Если Не ПараметрыЗапроса.Свойство("СтавкаНДСПоУмолчанию") Тогда
		СтавкаНДСПоУмолчанию = СтавкаНДСПоУмолчанию();
...
Функция ТекстЗапросаСтавкаНДС(ВыражениеНалогообложениеНДС, ВыражениеНоменклатура = Неопределено, ВыражениеВозвращатьМногооборотнуюТару = Неопределено) Экспорт
	...
	Если ВыражениеНалогообложениеНДС <> Неопределено Тогда
		...
		|		ТОГДА &ВыражениеНоменклатура.СтавкаНДС
...
Функция СтавкаНДСПоТипуНалогообложения(ТипНалогообложенияНДС) Экспорт
	...
	Возврат УчетНДСУПСлужебныйКлиентСерверПовтИсп.СтавкаНДСПоТипуНалогообложения(ТипНалогообложенияНДС);
...
Процедура СформироватьЗаданияПоДокументам(МассивДокументов) Экспорт
	...
	Для Каждого СтрокаТипа Из ТаблицаТипов Цикл
		...
		Иначе
			УчетНДСЛокализация.СформироватьЗаданияПоДокументам(СтрокаТипа.Тип, МассивДокументов);
		КонецЕсли;
...
Процедура НастроитьВариантыОтчетов(Настройки) Экспорт
	...
	ОписаниеВарианта.Описание =
		НСтр("ru = 'Анализ начислений НДС с полученных авансов в разрезе контрагентов и документов оплаты';
			|en = 'Analysis of VAT accruals from prepayments from customer broken down by counterparties and AR/AP transactions'");
	#КонецОбласти
	...
	ОписаниеВарианта.Описание =
		НСтр("ru = 'Анализ учета НДС по зачетам полученных авансов в разрезе контрагентов и документов оплаты';
			|en = 'Analysis of VAT accounting by prepayments from customer with a breakdown by counterparties and AR/AP transactions'");
	#КонецОбласти
	...
	ОписаниеВарианта.Описание =
		НСтр("ru = 'Анализ учета НДС по зачетам выданных авансов в разрезе контрагентов и документов оплаты';
			|en = 'Analysis of VAT accounting by received prepayment with a breakdown by counterparties and AR/AP transactions'");
	#КонецОбласти
	...
	ОписаниеВарианта.Описание =
		НСтр("ru = 'Обороты по входящему и исходящему НДС за период.';
			|en = 'Incoming and outgoing VAT turnovers for the period.'");
	#КонецОбласти
