	...
	Если ТекущиеДанные <> Неопределено Тогда
		Если ЗначениеЗаполнено(ТекущиеДанные.Счет) Тогда
			...
			Если ИмяЭлемента = "ГруппировкаКор" Тогда 
				...
				Если ДанныеСчета.Свойство("УчетПоНаправлениямДеятельности", ИспользуетсяИзмерение) И ИспользуетсяИзмерение Тогда
					СтрокаПоСубконто = СтрокаПоСубконто + "+" + "!";
					СтрокаПредставление = СтрокаПредставление + НСтр("ru = 'Направление деятельности';
																	|en = 'Line of business'") + ", ";
				КонецЕсли;
				Если ДанныеСчета.Свойство("УчетПоПодразделениям", ИспользуетсяИзмерение) И ИспользуетсяИзмерение Тогда
