...
Функция СформироватьДанныеЗаполненияАктовВыполненныхРабот(МассивСсылок, РеквизитыШапки) Экспорт
	...
	Если ВыборкаРеквизитыШапки.ЕстьОтличияПартнер 
		...
		ИЛИ ВыборкаРеквизитыШапки.ЕстьОтличияГруппаФинансовогоУчета Тогда
			...
			Пока ВыборкаЗаказы.Следующий() Цикл
				...
				ИначеЕсли ВыборкаЗаказы.ЕстьОшибкиСтатус Тогда
					...
					ТекстОшибки = НСтр("ru = 'Документ %Документ% находится в статусе ""%Статус%"". Ввод на основании разрешен только в статусах ""К отгрузке"", ""Закрыт"".';
										|en = 'Document %Документ% is in the ""%Статус%"" status. Generation is only allowed from base documents in ""For shipment"" or ""Closed"" statuses.'");
...
Функция ПолучитьНастройкуКомплектаДокументов() Экспорт
	...
	Возврат ПродажиСервер.ПолучитьНастройкуКомплектаДокументов();
...
Процедура СохранитьНастройкуКомплектаДокументов(Настройки) Экспорт
	...
	ПродажиСервер.СохранитьНастройкуКомплектаДокументов(Настройки);
