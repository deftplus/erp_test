...
Процедура ЗарегистрироватьОбработчикиОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий     = НСтр("ru = 'Обновление публикации объектов сервиса 1С:Кабинет сотрудника.';
										|en = 'Update publication of 1C:Employee Account service objects.'");
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия          = "3.1.18.301";
	Обработчик.РежимВыполнения = ОбновлениеИнформационнойБазыЗарплатаКадрыБазовый.ОсновнойРежимВыполненияОбновления();
...
Функция HTTPОтветСервиса(ПараметрыПодключения, РесурсСервиса, ИмяМетода, СтрокаТела = "", ИмяФайла = "", РазрешенныйКодОтвета = "")
...
Процедура ОбработатьСтрокуДанных(СтрокаДанных, НачисленияТарифнойСтавки, ВидыМестВыплатыРегистраторов)
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(НСтр("ru = 'Данные расчетных листов содержат неизвестное значение группы начисления удержания выплаты.
											...
											|Тип значения: %2
											|Идентификатор: %3';
											|en = 'Payslip data contains an unknown value for the payout accrual withholding group.
											|Unknown value: %1
											|Value type: %2
											|ID: %3'"), СтрокаДанных.Группа, ТипЗнч(СтрокаДанных.Группа), ИдентификаторНеизвестногоЗначения);
...
Функция РезультатВыгрузкиКоллекцииВСервис(ПараметрыПодключения, Знач РесурсСервиса, Знач Данные, ПолеКлюча, РезультатВыгрузки = Неопределено, РазмерПакета = 100) Экспорт
	...
	Если Не ТипЗнч(Данные) = Тип("Массив") Тогда
		ВызватьИсключение НСтр("ru = 'Произошла ошибка при выгрузки коллекции в сервис. Ожидается массив.';
								|en = 'An error occurred while exporting the collection to the service. An array is expected.'");
	КонецЕсли;
...
Функция ВыгрузитьВложенияВСервис(ПараметрыПодключения, ТаблицаДанных, Вложения, ИмяПоляВладельца, НазваниеВложения, Расширение)
	...
	Для Каждого СтрокаТаблицы Из ТаблицаДанных Цикл
		...
		Если Ответ = Неопределено Или Ответ.КодСостояния >= 300 Тогда
			...
			ТекстСообщения = НСтр("ru = 'Ошибка при публикации файла: %1.';
									|en = 'Error when publishing file: %1.'");
...
Функция РезультатВыгрузкиФизическихЛиц(ПараметрыПодключения, Список)
	...
	Если ЗначениеЗаполнено(ДанныеФизическихЛиц.Вложения) Тогда
		...
		БылиОшибки = ВыгрузитьВложенияВСервис(ПараметрыПодключения, ТаблицаДанных, ДанныеФизическихЛиц.Вложения, "ФизическоеЛицо", "Фотография", "jpg");
	КонецЕсли;
...
Процедура ОпубликоватьИзмененияДокументов(ПараметрыПодключения, БылиОшибки)
	...
	|			ТОГДА ЛОЖЬ
	|		КОГДА НЕ ФизическиеЛицаКабинетСотрудника.ФизическоеЛицо ЕСТЬ NULL
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК Публикуется
	|ИЗ
	...
	|			ПО ДокументКадровогоЭДО.ФизическоеЛицо = ФизическиеЛицаКабинетСотрудника.ФизическоеЛицо
	|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОшибкиЗаполненияПубликуемыхОбъектовКабинетСотрудника КАК ОшибкиЗаполнения
	|			ПО ДокументКадровогоЭДО.ФизическоеЛицо = ОшибкиЗаполнения.ОбъектПубликации
	...
	Для каждого ТипДанных Из ТипыДанных Цикл
		...
		Иначе
			...
			Если ТипДанных = Тип("ДокументСсылка.ДокументКадровогоЭДО")
				Или НЕ КадровыйЭДО.ЭтоТипОбъектаСПечатнымиФормами(ТипДанных) Тогда
...
Функция ЭлектронныйДокументРезультатыСогласования(ПараметрыПодключения, ДанныеЭлектронногоДокумента)
	...
	Для каждого Подпись Из Подписи Цикл
		...
		РезультатСогласования.Подписант 			= Подписант;
		РезультатСогласования.ИмяПодписанта 		= ИмяПодписанта;
		РезультатСогласования.ИдентификаторДокумента = Подпись.Владелец;
	...
	Если РезультатыСогласования.Количество() = 0 Тогда
		...
		РезультатСогласования.ИдентификаторДокумента = Оригинал.Владелец;
		РезультатСогласования.РезультатСогласования = "agreed";
		РезультатСогласования.РольПодписанта 		= "employer";
		РезультатСогласования.ВерсияДокумента 		= ВерсияФайла;
...
Функция РезультатВыгрузкиДокументовНаПодпись(ПараметрыПодключения, ТаблицаДокументовНаПодпись)
	...
	Для каждого СтрокаТЗ Из ТаблицаДанных Цикл
		...
		Ответ = ЭлектронныйДокументРезультатыСогласования(ПараметрыПодключения, СтрокаТЗ.ДанныеЭлектронногоДокумента);
		...
		ДокументРезультатыСогласования.Вставить(СтрокаТЗ.ДокументНаПодпись, РезультатыСогласования);
		
		ОписаниеДокументНаПодпись =  ОписаниеОбъекта(ОписаниеОбъектаДокументНаПодпись());
		ОписаниеДокументНаПодпись.ИдентификаторДокумента = СтрокаТЗ.ДокументНаПодпись;
	...
	Для Каждого ДокументНаПодпись Из РезультатВыгрузки.Выгружено Цикл
	...
	Для Каждого Ошибка Из РезультатВыгрузки.НеВыгружено Цикл
		СтрокаДанных = ТаблицаДанных.Найти(Ошибка.Ключ, "ДокументНаПодпись");
	...
	Если РезультатыСогласованияКПубликации.Количество() > 0 Тогда
		...
		Для Каждого Ошибка Из РезультатВыгрузки.НеВыгружено Цикл
			СтрокаДанных = ТаблицаДанных.Найти(Ошибка.Ключ, "ДокументНаПодпись");
			РезультатОбработки.НеВыгружено.Добавить(СтрокаДанных.ПубликуемыйДокумент);
			ВыгруженныеДокументы.Удалить(Ошибка.Ключ);
			ЗарегистрироватьОшибкуПубликацииОбъекта(СтрокаДанных.ПубликуемыйДокумент, РесурсСервиса, Ошибка.Значение);
		КонецЦикла;
		
	КонецЕсли;
	
	Для каждого Ошибка Из ВыгруженныеДокументы Цикл
		СтрокаДанных = ТаблицаДанных.Найти(Ошибка.Ключ, "ДокументНаПодпись");
		РезультатОбработки.Выгружено.Добавить(СтрокаДанных.ПубликуемыйДокумент);
	КонецЦикла;
...
Функция ДанныеДляВыгрузкиДокументовНаПодпись(ТаблицаИзменений)
	...
	ТаблицаДокументыНаПодпись.Колонки.Добавить("ДокументНаПодпись");
	...
	|	ДокументКадровогоЭДО.ФизическоеЛицо КАК ФизическоеЛицо,
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.ФизическоеЛицо) Тогда
			ЗначенияРеквизитов.ФизическиеЛица = ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Выборка.ФизическоеЛицо);
		Иначе
	...
	Для каждого СтрокаТЗ Из ТаблицаИзменений Цикл
		...
		НоваяСтрока.ДокументНаПодпись 	= СтрокаТЗ.Документ;
		...
		НоваяСтрока.НазваниеДокумента 	= ДокументыНаПодписьРеквизитов[СтрокаТЗ.Документ].Представление;
	КонецЦикла;
...
Функция РезультатВыгрузкиСправокСРаботы(ПараметрыПодключения, ТаблицаПоТипуДанных)
	...
	Для каждого СтрокаТЗ Из ТаблицаДанных Цикл
		...
		ОписаниеСправки.Заявка 				= СтрокаТЗ.Заявка;
	...
	РезультатыСогласованияКПубликации = Новый Массив;
	ВыгруженныеЗаявки = Новый Соответствие;
	...
	Если СправкиКПубликации.Количество() > 0  Тогда
		...
		Для Каждого Заявка Из РезультатВыгрузки.Выгружено Цикл
			ВыгруженныеЗаявки.Вставить(Заявка, Истина);
			РезультатыСогласования = ЗаявкаРезультатыСогласования[Заявка];
			Если РезультатыСогласования <> Неопределено Тогда
				Для каждого РезультатСогласования Из РезультатыСогласования Цикл
					РезультатыСогласованияКПубликации.Добавить(РезультатСогласования);
				КонецЦикла;
		...
		Для Каждого Ошибка Из РезультатВыгрузки.НеВыгружено Цикл
			СтрокаДанных = ТаблицаДанных.Найти(Ошибка.Ключ, "Заявка");
			РезультатОбработки.НеВыгружено.Добавить(СтрокаДанных.ПубликуемыйДокумент);
			ЗарегистрироватьОшибкуПубликацииОбъекта(СтрокаДанных.ПубликуемыйДокумент, РесурсСервиса, Ошибка.Значение);
		КонецЦикла;
	...
	Если РезультатыСогласованияКПубликации.Количество() > 0 Тогда
		...
		Для Каждого Ошибка Из РезультатВыгрузки.НеВыгружено Цикл
			СтрокаДанных = ТаблицаДанных.Найти(Ошибка.Ключ, "Заявка");
			РезультатОбработки.НеВыгружено.Добавить(СтрокаДанных.ПубликуемыйДокумент);
			ВыгруженныеЗаявки.Удалить(Ошибка.Ключ);
			ЗарегистрироватьОшибкуПубликацииОбъекта(СтрокаДанных.ПубликуемыйДокумент, РесурсСервиса, Ошибка.Значение);
		КонецЦикла;
		
	КонецЕсли;
	
	Для каждого ЭлементКоллекции Из ВыгруженныеЗаявки Цикл
		СтрокаДанных = ТаблицаДанных.Найти(ЭлементКоллекции.Ключ, "Заявка");
		РезультатОбработки.Выгружено.Добавить(СтрокаДанных.ПубликуемыйДокумент);
	КонецЦикла;
...
Функция РезультатВыгрузки2НДФЛ(ПараметрыПодключения, ТаблицаПоТипуДанных)
	...
	// Коллекция справок с результатами согласования.
	// Ключ - ссылка на справку 2НДФЛ
	// Значение - массив объектов РезультатСогласования.
	СправкаРезультатыСогласования = Новый Соответствие;
	...
	// Коллекция объектов справка 2НДФЛ для получения документа КЭДО.
	// Ключ - ссылка на справку 2НДФЛ
	// Значение - ссылка на справку документ КЭДО.
	СправкиПубликуемыеДокументы = Новый Соответствие;
	...
	Для каждого СтрокаТЗ Из ТаблицаДанных Цикл
		...
		СправкиПубликуемыеДокументы.Вставить(СтрокаТЗ.Справка2НДФЛ, СтрокаТЗ.ПубликуемыйДокумент);
		...
		ОписаниеСправка2НДФЛ.Справка2НДФЛ 		= СтрокаТЗ.Справка2НДФЛ;
		...
		Если ИспользоватьФормат303 Тогда
			...
			Если ТребуетсяЭП И СтрокаТЗ.ДанныеЭлектронногоДокумента <> Неопределено Тогда
				...
				СправкаРезультатыСогласования.Вставить(СтрокаТЗ.Справка2НДФЛ, РезультатыСогласования);
	...
	Если ИспользоватьФормат303 И СправкиКПубликации.Количество() > 0 Тогда
		...
		ВыгруженныеСправки = Новый Соответствие;
		// Добавим в РезультатыСогласованияКПубликации данные по выгруженным справкам.
		РезультатыСогласованияКПубликации = Новый Массив;
		Для Каждого Справка Из РезультатВыгрузки.Выгружено Цикл
			ВыгруженныеСправки.Вставить(Справка, Истина);
			РезультатыСогласования = СправкаРезультатыСогласования[Справка];
			Если РезультатыСогласования <> Неопределено Тогда
				Для каждого РезультатСогласования Из РезультатыСогласования Цикл
					РезультатыСогласованияКПубликации.Добавить(РезультатСогласования);
				КонецЦикла;
		...
		Для Каждого Ошибка Из РезультатВыгрузки.НеВыгружено Цикл
			РезультатОбработки.НеВыгружено.Добавить(СправкиПубликуемыеДокументы[Ошибка.Ключ]);
			ЗарегистрироватьОшибкуПубликацииОбъекта(СправкиПубликуемыеДокументы[Ошибка.Ключ], РесурсСервиса, Ошибка.Значение);
		КонецЦикла;
		...
		Если РезультатыСогласованияКПубликации.Количество() > 0 Тогда
			...
			Для Каждого Ошибка Из РезультатВыгрузки.НеВыгружено Цикл
				РезультатОбработки.НеВыгружено.Добавить(СправкиПубликуемыеДокументы[Ошибка.Ключ]);
				ВыгруженныеСправки.Удалить(Ошибка.Ключ);
				ЗарегистрироватьОшибкуПубликацииОбъекта(СправкиПубликуемыеДокументы[Ошибка.Ключ], РесурсСервиса, Ошибка.Значение);
			КонецЦикла;
			
		КонецЕсли;
		
		Для каждого ЭлементКоллекции Из ВыгруженныеСправки Цикл
			РезультатОбработки.Выгружено.Добавить(СправкиПубликуемыеДокументы[ЭлементКоллекции.Ключ]);
		КонецЦикла;
...
Процедура ОпубликоватьВсеРасчетныеЛисткиЗаПериоды(ПериодыРасчетныхЛистков)
	...
	СтрокаСобытие 		= НСтр("ru = 'Начало процесса публикации расчетных листков после подключения сервиса.';
								|en = 'Start payslip publication after enabling the service.'");
	Комментарий = СтрШаблон("%1 %2 %3", СтрокаСобытие, СтрокаПриложение, СтрокаИС);
	ЗаписьЖурналаРегистрации(ИмяСобытияПрочиеСобытия(), УровеньЖурналаРегистрации.Информация,,,Комментарий);
	...
	СтрокаСобытие = НСтр("ru = 'Окончание процесса публикации расчетных листков после подключения сервиса. Опубликовано: %1.';
						|en = 'Finishing payslip publication after enabling the service. Published: %1.'");
	СтрокаСобытие = СтрШаблон(СтрокаСобытие, КоличествоВыгружено);
	Если БылиОшибки Тогда
		СтрокаСобытие = СтрШаблон("%1 %2", СтрокаСобытие, НСтр("ru = 'Были ошибки.';
																|en = 'There were errors.'"));
...
Функция РезультатВыгрузкиРасчетныхЛистков(ПараметрыПодключения, ПараметрыВыгрузки) 
	...
	ФизическоеЛицоРасчетныйЛисток 	= Новый Соответствие;
	РасчетныеЛисткиКПубликации 		= Новый Массив;
	ДокументыКЭДОФизическиеЛица 	= Новый Соответствие;
	...
	Для Каждого ОбъектИнформацияОЗарплате Из Данные.ИнформацияОЗарплате Цикл
		...
		Попытка
			...
			Если ДокументКЭДОСсылка <> Неопределено Тогда
				...
				ДокументКЭДОФайл = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ДокументКЭДОСсылка, "ЭлектронныйДокумент");
				Если ЗначениеЗаполнено(ДокументКЭДОФайл) Тогда
					...
					ЭлементБлокировки.УстановитьЗначение("Ссылка", ДокументКЭДОФайл);
				КонецЕсли;
				Блокировка.Заблокировать();
			...
			Иначе
				...
				ДокументКЭДООбъект = Документы.ДокументКадровогоЭДО.СоздатьДокумент();
				ДокументКЭДОСсылка = Документы.ДокументКадровогоЭДО.ПолучитьСсылку();
				ДокументКЭДООбъект.ДополнительныеСвойства.Вставить("СсылкаНового", ДокументКЭДОСсылка);
				ДокументКЭДООбъект.Дата = ТекущаяДатаСеанса();
				ДокументКЭДООбъект.Организация = Организация;
				ДокументКЭДООбъект.ФизическоеЛицо = ФизическоеЛицо;
				ДокументКЭДООбъект.ОснованиеДокумента = ДокументКЭДОСсылка;
				ДокументКЭДООбъект.ИдентификаторДокумента = Строка(ДокументКЭДОСсылка.УникальныйИдентификатор());
				ДокументКЭДООбъект.КатегорияДокумента = Перечисления.КатегорииДокументовКадровогоЭДО.РасчетныйЛисток;
			...
			Если ЗначениеЗаполнено(ДокументКЭДОФайл) Тогда
				// удаляем "старый" файл
				ФайлКУдалению = ДокументКЭДОФайл.ПолучитьОбъект();
				ФайлКУдалению.УстановитьПометкуУдаления(Истина);
			КонецЕсли;
			ДокументКЭДООбъект.ЭлектронныйДокумент = ФайлРасчетногоЛистка;
			ДокументКЭДООбъект.ОснованиеДокумента = ДокументКЭДОСсылка;
			ДокументКЭДООбъект.ПодписанСотрудником = Ложь;
			...
			Если ПубликоватьДокументыНаПодпись Тогда
				...
				ОписаниеДокументНаПодпись = ОписаниеОбъекта(ОписаниеОбъектаДокументНаПодпись());
				ОписаниеДокументНаПодпись.ИдентификаторДокумента = ДокументКЭДОСсылка;
				...
				ДокументыНаПодписьКПубликации.Добавить(ОбъектДокументНаПодпись);
				ДокументыКЭДОФизическиеЛица.Вставить(ДокументКЭДОСсылка, ФизическоеЛицо);
			...
			ФизическоеЛицоРасчетныйЛисток.Вставить(ФизическоеЛицо, ОбъектИнформацияОЗарплате);
	...
	Если ДокументыНаПодписьКПубликации.Количество() > 0 Тогда
		...
		Для Каждого ДокументыКЭДО Из РезультатВыгрузки.Выгружено Цикл
			ФизическоеЛицо = ДокументыКЭДОФизическиеЛица[ДокументыКЭДО];
			РасчетныеЛисткиКПубликации.Добавить(ФизическоеЛицоРасчетныйЛисток[ФизическоеЛицо]);
		КонецЦикла;
	...
	Для Каждого ФизическоеЛицо Из РезультатВыгрузки.Выгружено Цикл
		...
		Выгружено.Вставить(ФизическоеЛицо, Истина);
		...
		НоваяСтрока.СостояниеПубликации = Перечисления.СостоянияРасчетныхЛистковКабинетСотрудника.Опубликован;
		НоваяСтрока.ДатаПубликации = ТекущаяДатаСеанса();
		НоваяСтрока.ДокументКадровогоЭДО = ФизическиеЛицаДокументыКЭДО[ФизическоеЛицо];
		НоваяСтрока.Ответственный = ТекущийПользователь;
		
	КонецЦикла;
	
	РезультатОбработки.НеОбработаны = Новый Массив;
	Для Каждого ФизическоеЛицо Из СписокФизическихЛиц Цикл
		Если Выгружено[ФизическоеЛицо] <> Неопределено Тогда
			// Данные по физическому лицу выгружены без ошибок.
			Продолжить;
		КонецЕсли;
		Если ФизическиеЛицаСДаннымиОЗарплате[ФизическоеЛицо] = Неопределено Тогда
			// Не было данных для выгрузки.
			РезультатОбработки.НеОбработаны.Добавить(ФизическоеЛицо);
			Продолжить;
		КонецЕсли;
		...
		НоваяСтрока.Месяц = МесяцРасчетныхЛистов;
		НоваяСтрока.ПерваяПоловинаМесяца = Ложь;
		НоваяСтрока.СостояниеПубликации = Перечисления.СостоянияРасчетныхЛистковКабинетСотрудника.ОшибкаОбработки;
		НоваяСтрока.ДатаПубликации = ТекущаяДатаСеанса();
		НоваяСтрока.ДокументКадровогоЭДО = ФизическиеЛицаДокументыКЭДО[ФизическоеЛицо];
		НоваяСтрока.Ответственный = ТекущийПользователь;
...
Функция ОбработатьИзмененияИзСервиса(ПараметрыПодключения, ПолученныеИзменения)
	...
	Для каждого СтрокаТЗ Из ТаблицаЗаявок Цикл
		...
		Попытка
			...
			Если СтрокаТЗ.ТипЗаявки = Перечисления.ТипыЗаявокКабинетСотрудника.СправкаНДФЛ Тогда
				...
				Если ЗначениеЗаполнено(ОписаниеЗаявки.ЭлектронныйДокумент) Тогда
	...
	Для каждого СтрокаТЗ Из РезультатыСогласования Цикл
		...
		Попытка
			...
			Блокировка = Новый БлокировкаДанных;
			ЭлементБлокировки = Блокировка.Добавить("Документ.ДокументКадровогоЭДО");
			ЭлементБлокировки.УстановитьЗначение("Ссылка", СтрокаТЗДокументКЭДО.Ссылка);
			
			Если РезультатСогласования.РольПодписанта = "Сотрудник" Тогда
			...
			ЭлектронныйДокумент = СтрокаТЗДокументКЭДО.ЭлектронныйДокумент;
			Если Не ЭлектронныйДокументСоответствуетОтвету(ЭлектронныйДокумент, РезультатСогласования) Тогда
			...
			Если РезультатСогласования.РольПодписанта = "Сотрудник" Тогда
				...
				ДокументОбъект = СтрокаТЗДокументКЭДО.Ссылка.ПолучитьОбъект();
				ДокументОбъект.ПодписанСотрудником = Истина;
				ДокументОбъект.Записать();
				
			ИначеЕсли РезультатСогласования.РольПодписанта = "Руководитель" Тогда
			...
			Если ИспользуетсяКадровыйЭДО И ЗначениеЗаполнено(РезультатСогласования.ЭлектроннаяПодпись) Тогда
				СохранитьЭлектроннуюПодписьРезультатаСогласования(ЭлектронныйДокумент, РезультатСогласования)
			КонецЕсли;
...
Функция РезультатОбработкиНеобработанныхРезультатовСогласования()
	...
	Для каждого СтрокаТЗ Из НеобработанныеРезультаты Цикл
		...
		Попытка
			...
			// Обработка записи ЭП
			Если ПолучитьФункциональнуюОпцию("ИспользуетсяКадровыйЭДОКабинетСотрудника") И ЗначениеЗаполнено(РезультатСогласования.ЭлектроннаяПодпись) Тогда
				СохранитьЭлектроннуюПодписьРезультатаСогласования(ЭлектронныйДокумент, РезультатСогласования)
			КонецЕсли;
		...
		Исключение
			...
			ОтменитьТранзакцию();
			// Если ошибка блокировки, не регистрируем ошибку обработки,
			// результат согласования будет сохранен как необработанный.
			Если Не ВыполняетсяБлокировка Тогда
				РезультатОбработки.БылиОшибки = Истина;
				ТипИзменений = НСтр("ru = 'Результат согласования';
									|en = 'Approval result'");
...
Функция ЭлектронныйДокументСоответствуетОтвету(ЭлектронныйДокумент, РезультатСогласования)
	...
	Возврат (ВерсияФайла = ВРег(РезультатСогласования.ВерсияДокумента));
...
Процедура СохранитьНастройкиПубликацииВФоне(Параметры, АдресХранилища) Экспорт
	...
	Исключение
		...
		ВызватьИсключение(НСтр("ru = 'При записи правил публикации произошла ошибка.';
								|en = 'An error occurred while writing the publishing rules.'")
		...
		Иначе
			...
			Для каждого ДанныеСотрудника Из РабочиеМестаСотрудников Цикл
				...
				ИначеЕсли ДанныеСотрудника.ВидСобытия = Перечисления.ВидыКадровыхСобытий.Перемещение Тогда
					...
					Если ПозицияЕстьВПравилах[ДанныеСотрудника.ДолжностьПоШтатномуРасписанию] <> Неопределено Тогда
						ЕстьПубликуемоеРабочееМесто = Истина;
					ИначеЕсли ФизическоеЛицоПубликуется Тогда
...
Процедура ОбработатьОчисткуПравилПубликации()
	...
	Сотрудники = РабочиеМеста.ВыгрузитьКолонку("Сотрудник");
	НастройкиСервиса = РегистрыСведений.НастройкиСервисаКабинетСотрудника.НастройкиСервиса();
...
Процедура ПриЗаписиДокументаКЭДО(ДокументОбъект)
	...
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ФизическоеЛицо", ДокументОбъект.ФизическоеЛицо);
	...
	Если Не РезультатЗапроса[0].Пустой() И Не РезультатЗапроса[1].Пустой() Тогда
		МенеджерЗаписи = РегистрыСведений.ИзмененияДокументовДляСервисаКабинетСотрудника.СоздатьМенеджерЗаписи();
		МенеджерЗаписи.ПубликуемыйДокумент = ДокументОбъект.Ссылка;
		МенеджерЗаписи.ВерсияДанных  = Строка(Новый УникальныйИдентификатор);
		МенеджерЗаписи.Записать();
	КонецЕсли;	
...
Процедура ЗарегистрироватьДокументыНаПодпись() Экспорт
	...
	УстановитьПривилегированныйРежим(Истина);
	ЗаданияКИсполнению = РегистрыСведений.ЗапланированныеДействияСПечатнымиФормами.ФайлыКОбработке(
		Ложь, Перечисления.ДействияСПечатнымиФормами.ПередатьВКабинетСотрудников);
	УстановитьПривилегированныйРежим(Ложь);
	
	СписокФайлов = ЗаданияКИсполнению.Получить(Перечисления.ДействияСПечатнымиФормами.ПередатьВКабинетСотрудников);
...
Процедура ЗарегистрироватьИзмененияДокументовКадровогоЭДО(СписокФайлов)
	...
	|		ПО ДокументКадровогоЭДО.ФизическоеЛицо = ПубликуемыеФизическиеЛица.ФизическоеЛицо
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПубликуемыеОбъектыКабинетСотрудника КАК ПубликуемыеОбъекты
	|		ПО ДокументКадровогоЭДО.Организация = ПубликуемыеОбъекты.ОбъектПубликации
	|ГДЕ
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		Попытка
			...
			РегистрыСведений.ЗапланированныеДействияСПечатнымиФормами.УдалитьФайлыИзОбработки(
							ОбработанныеФайлы,
...
Функция НазваниеГруппыСобытийЖР()
	...
	Возврат НСтр("ru = '1С:Кабинет сотрудника';
...
Функция ИмяСобытияЖРПолучениеИзменений()
	...
	Возврат СтрШаблон("%1.%2", НазваниеГруппыСобытийЖР(), НСтр("ru = 'Получение изменений';
																|en = 'Receiving changes'", ОбщегоНазначения.КодОсновногоЯзыка()));
	
КонецФункции

Функция ИмяСобытияЖРПередачаИзменений()
	
	Возврат СтрШаблон("%1.%2", НазваниеГруппыСобытийЖР(), НСтр("ru = 'Публикация изменений';
																|en = 'Publishing changes'", ОбщегоНазначения.КодОсновногоЯзыка()));
	
КонецФункции

Функция ИмяСобытияЖРУдалениеДанных()
	
	Возврат СтрШаблон("%1.%2", НазваниеГруппыСобытийЖР(), НСтр("ru = 'Удаление данных';
																|en = 'Delete data'", ОбщегоНазначения.КодОсновногоЯзыка()));
	
КонецФункции

Функция ИмяСобытияЖРРегистрацияИзменений()
	
	Возврат СтрШаблон("%1.%2", НазваниеГруппыСобытийЖР(), НСтр("ru = 'Регистрация изменений';
																|en = 'Change registration'", ОбщегоНазначения.КодОсновногоЯзыка()));
	
КонецФункции

Функция ИмяСобытияЖРПубликацияРасчетныхЛистов()
	
	Возврат СтрШаблон("%1.%2", НазваниеГруппыСобытийЖР(), НСтр("ru = 'Публикация расчетных листов';
																|en = 'Publishing payslips'", ОбщегоНазначения.КодОсновногоЯзыка()));
	
КонецФункции

Функция ИмяСобытияЖРСостояниеСервиса()
	
	Возврат СтрШаблон("%1.%2", НазваниеГруппыСобытийЖР(), НСтр("ru = 'Проверка состояния сервиса';
																|en = 'Checking service status'", ОбщегоНазначения.КодОсновногоЯзыка()));
	
КонецФункции

Функция ИмяСобытияЖРПроверкаИспользуемойФункциональности()
	
	Возврат СтрШаблон("%1.%2", НазваниеГруппыСобытийЖР(), НСтр("ru = 'Проверка используемой функциональности';
																|en = 'Checking the functionality used'", ОбщегоНазначения.КодОсновногоЯзыка()));
	
КонецФункции

Функция ИмяСобытияПротоколЗапрос() Экспорт

	Возврат СтрШаблон("%1.%2", НазваниеГруппыСобытийЖР(), НСтр("ru = 'Протокол запрос';
																|en = 'Protocol query'", ОбщегоНазначения.КодОсновногоЯзыка()));

КонецФункции

Функция ИмяСобытияПротоколОтвет() Экспорт

	Возврат СтрШаблон("%1.%2", НазваниеГруппыСобытийЖР(), НСтр("ru = 'Протокол ответ';
																|en = 'Protocol response'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Функция ИмяСобытияОтключениеСервиса() Экспорт
	...
	Возврат СтрШаблон("%1.%2", НазваниеГруппыСобытийЖР(), НСтр("ru = 'Отключение сервиса';
																|en = 'Disconnection from service'", ОбщегоНазначения.КодОсновногоЯзыка()));

КонецФункции

Функция ИмяСобытияЖРАутентификации() Экспорт

	Возврат СтрШаблон("%1.%2", НазваниеГруппыСобытийЖР(), НСтр("ru = 'Аутентификация';
...
Функция ИмяСобытияПодключениеСервиса()
	...
	Возврат СтрШаблон("%1.%2", НазваниеГруппыСобытийЖР(), НСтр("ru = 'Подключение сервиса';
																|en = 'service connection'", ОбщегоНазначения.КодОсновногоЯзыка()));
	
КонецФункции

Функция ИмяСобытияЖРНарушениеПравДоступа() Экспорт
	
	Возврат СтрШаблон("%1.%2", НазваниеГруппыСобытийЖР(), НСтр("ru = 'Нарушение прав доступа';
...
Функция ИмяСобытияОшибкаБлокировки() Экспорт
	...
	Возврат СтрШаблон("%1.%2", НазваниеГруппыСобытийЖР(), НСтр("ru = 'Ошибка блокировки';
																|en = 'Lock error'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ОбменССервисомКабинетСотрудника() Экспорт
	...
	Если БылиОшибки Тогда
		ВызватьИсключение НСтр("ru = 'Обмен данными с сервисом 1С:Кабинет сотрудника завершен с ошибками.';
								|en = 'Data exchange with the 1C:Employee account service completed with errors.'");
	КонецЕсли;
...
Процедура ВключитьНастройкиПрограммыПослеПодключенияСервиса() Экспорт
	...
	Если Не ВерсияПриложенияМеньшеВерсии("3.0.1.1") Тогда
		УстановитьПривилегированныйРежим(Истина);
...
Процедура ЗаполнитьДеревоНастроекПубликации(Параметры, АдресХранилища) Экспорт
	...
	Исключение
		СообщениеОбОшибке = НСтр("ru = 'Настройка публикации. Пользователю недоступны организации и/или позиции штатного расписания.';
								|en = 'Publication setting. Companies or staff list positions are not available to a user.'");
...
Процедура СохранитьЭлектроннуюПодписьРезультатаСогласования(ЭлектронныйДокумент, РезультатСогласования)
