...
Процедура Отразить(ПараметрыФормированияПроводок, ТаблицаПроводок) Экспорт
	...
	Если НЕ Результат.Пустой() Тогда
		ТекстСообщения = НСтр("ru = 'Используется настройка формирования проводок со способом корректировки реверсивными проводками:';
								|en = 'The setting for entry generation with correction by reversing entries is used:'");
		...
		ТекстСообщения = ТекстСообщения + Символы.ПС + НСтр("ru = 'Измените настройку формирования проводок перед отражением в международном учете.';
															|en = 'Change the entry generation setting before recognition in financial accounting.'");
...
Функция ПроверитьПравилаОтражения(МенеджерВременныхТаблиц, ПараметрыПроверки) Экспорт
	...
	|	ХозоперацииКОтражению.ПланСчетов КАК ПланСчетов,
	|	ХозоперацииКОтражению.Организация КАК Организация,
	|	ХозоперацииКОтражению.НастройкаФормированияПроводок КАК НастройкаФормированияПроводок
	...
	|		И ХозоперацииКОтражению.НастройкаФормированияПроводок = ШаблоныПроводок.НастройкаФормированияПроводок
	|	ЛЕВОЕ СОЕДИНЕНИЕ
	...
	|		И ХозоперацииКОтражению.НастройкаФормированияПроводок = НеОтражаемыеХозоперации.НастройкаФормированияПроводок
	|ГДЕ
	...
	Если ЗначениеЗаполнено(ПараметрыПроверки.Документ) Тогда
		МетаданныеДокумента = ПараметрыПроверки.Документ.Метаданные();
	КонецЕсли;
	...
	Для каждого ТекстыЗапроса Из ТекстыЗапросовПоРегистрам Цикл
		...
		Если ТекстыЗапроса.Ключ = МетаданныеРегистрыНакопления.РасчетыСКлиентамиПоСрокам.Имя Тогда
			МетаданныеРегистраДляПроверки = МетаданныеРегистрыНакопления.РасчетыСКлиентами;
		ИначеЕсли ТекстыЗапроса.Ключ = МетаданныеРегистрыНакопления.РасчетыСПоставщикамиПоСрокам.Имя Тогда 
			МетаданныеРегистраДляПроверки = МетаданныеРегистрыНакопления.РасчетыСПоставщиками;
		Иначе
			МетаданныеРегистраДляПроверки = МетаданныеРегистрыНакопления.Найти(ТекстыЗапроса.Ключ);
		КонецЕсли;
		
		Если МетаданныеДокумента <> Неопределено
			И НЕ МетаданныеДокумента.Движения.Содержит(МетаданныеРегистраДляПроверки) Тогда
			Продолжить;
		КонецЕсли;
...
Процедура НастроитьВариантыОтчетов(Настройки) Экспорт
	...
	#Область АнализОперацийФинансовогоУчета
...
Процедура ОтразитьДанныеРегистра(ИмяРегистра, ТекстЗапроса, ПараметрыФормированияПроводок, ТаблицаПроводок);
	...
	ДанныеОперативногоУчета.Свернуть("Регистратор, ХозяйственнаяОперация, ПланСчетов, Организация, Период", "Обработан");
...
Функция НеОтражаемыеХозяйственныеОперации(ДанныеОперативногоУчета, ТаблицаПроводок, СтруктураПроводки)
	...
	Пока ВыборкаНеОтражаемыеХозоперации.Следующий() Цикл
		...
		ТекстОшибки = НСтр("ru = 'Хозяйственная операция ""%ХозяйственнаяОперация%"" игнорируется применяемой настройкой формирования проводок';
							|en = 'The ""%ХозяйственнаяОперация%"" business transaction is ignored by the applied posting generation setting'");
