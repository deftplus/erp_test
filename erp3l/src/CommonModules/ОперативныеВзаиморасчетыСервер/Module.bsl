		...
		Если Индексы.ИндексОплаты <= ЗаписейОплаты - 1 И Индексы.ИндексОтгрузкиПоставки <= ЗаписейОтгрузкиПоставки - 1 Тогда 
			...
			Если СтрокаОтгрузкиПоставки.Сумма < 0 Или СтрокаОплаты.Сумма < 0 Тогда
				ВызватьИсключение(СтрШаблон(НСтр("ru = 'В регистрах взаиморасчетов обранужены отрицательные остатки на дату %1. Проведение невозможно.';
												|en = 'AR/AP accounting registers show negative inventory on date %1. Posting is unavailable.'"),
...
Процедура ВыполнитьПереоценкуНаДату(МассивОрганизаций, Период, ЭтоРасчетыСКлиентами = Неопределено) Экспорт
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	АналитикиУчетаПоПартерам.Ссылка КАК АналиткаУчетаПоПартнерам,
	|	АналитикиУчетаПоПартерам.Организация.ВалютаРегламентированногоУчета КАК ВалютаРегламентированногоУчета
	|ПОМЕСТИТЬ ВтАналитикиУчетаПоПартерам
	|ИЗ
	|	Справочник.КлючиАналитикиУчетаПоПартнерам КАК АналитикиУчетаПоПартерам 
	|ГДЕ
	|	АналитикиУчетаПоПартерам.Организация В (&МассивОрганизаций)
	|ИНДЕКСИРОВАТЬ ПО
	|	АналиткаУчетаПоПартнерам
	|;
	|
	|//////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|				(ВЫБРАТЬ 
	|					АналитикиУчетаПоПартерам.АналиткаУчетаПоПартнерам
	|				ИЗ
	|					ВтАналитикиУчетаПоПартерам КАК АналитикиУчетаПоПартерам)
	|			И (НЕ (АналитикаУчетаПоПартнерам, Валюта) В (
	|				ВЫБРАТЬ 
	|					АналитикиУчетаПоПартерам.АналиткаУчетаПоПартнерам,
	|					АналитикиУчетаПоПартерам.ВалютаРегламентированногоУчета
	|				ИЗ
	|					ВтАналитикиУчетаПоПартерам КАК АналитикиУчетаПоПартерам
	|				)
	|				ИЛИ Валюта <> &ВалютаУправленческогоУчета)
	|		) КАК РасчетыСКлиентамиПоСрокамОстатки
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.РасчетыСКлиентамиПоСрокам.Обороты(
	|			&НачалоПериода,
	|			&КонецПериода,
	|			,
	|			(АналитикаУчетаПоПартнерам) В 
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.РасчетыСПоставщикамиПоСрокам.Обороты(
	|			&НачалоПериода,
	|			&КонецПериода,
	|			,
	|			(АналитикаУчетаПоПартнерам) В 
	...
	Запрос.УстановитьПараметр("КонецПериода", ГраницаКонецДня);
	Запрос.УстановитьПараметр("Период", ГраницаКонецДня);
	Запрос.УстановитьПараметр("ДатаРасчета", Период);
	
	РезультатыЗапроса = Запрос.ВыполнитьПакет();
	
	ОбъектыКПереоценке = РезультатыЗапроса[2].Выгрузить();
	ДетальныеЗаписи    = РезультатыЗапроса[3].Выгрузить();
...
Функция ТекстЗапросаНекорректныхОстаткиВзаиморасчетовПоСрокам(ИмяВТ = "", БезРучныхКорректировок = Истина) Экспорт
	...
	ТекстЗапроса = "
		...
		|ГДЕ
		|	РасчетыСКлиентамиПоСрокам.АналитикаУчетаПоПартнерам.Организация В(&МассивОрганизаций)
		...
		|ГДЕ
		|	РасчетыСПоставщикамиПоСрокам.АналитикаУчетаПоПартнерам.Организация В(&МассивОрганизаций)
...
Функция ТекстЗапросаРазвернутоеСальдо(ИмяВт = "", БезРучныхКорректировок = Истина) Экспорт
	...
	ТекстЗапроса = "
		...
		|		И РасчетыСПоставщикамиПоСрокам.Активность
		|		И РасчетыСПоставщикамиПоСрокам.Регистратор ССЫЛКА Документ.КорректировкаРегистров
		|		И ВЫРАЗИТЬ(РасчетыСПоставщикамиПоСрокам.Регистратор КАК Документ.КорректировкаРегистров).Операция = ЗНАЧЕНИЕ(Перечисление.ОперацииКорректировкиРегистров.РучнаяКорректировка)) КАК ВложенныйЗапрос
		|
		|СГРУППИРОВАТЬ ПО
		|	ВложенныйЗапрос.АналитикаУчетаПоПартнерам,
		|	ВложенныйЗапрос.ОбъектРасчетов";
...
Функция ПолучитьОплаты(Запрос, ГлобальныеПеременные)
	...
	|					И Расчеты.ИдентификаторФинЗаписи = РасчетыПоСрокам.ИдентификаторФинЗаписи
	|	ГДЕ
	|		Расчеты.Активность
...
Функция ПолучитьОтгрузкиПоставки(Запрос, ГлобальныеПеременные)
	...
	//++ Локализация
	//++ НЕ УТ
	|		ИЛИ ТИПЗНАЧЕНИЯ(РасчетыОтгрузкиПоставки.Регистратор) = ТИП(Документ.ПоступлениеДенежныхДокументов)
	//-- НЕ УТ
	//-- Локализация
	...
	//++ Локализация
	//++ НЕ УТ
	|		ИЛИ ТИПЗНАЧЕНИЯ(РасчетыПоСрокамОстатки.ДокументРегистратор) = ТИП(Документ.ПоступлениеДенежныхДокументов)
	//-- НЕ УТ
	//-- Локализация
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Включение онлайн взаиморасчетов с партнерами. Работа потока.';
					|en = 'Enable online settlements with partners. Flow work.'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
// 
// Возвращаемое значение:
// 	Массив - Массив операций, для которых не нужно делать запись по переносу аванса.
...
// 
// Возвращаемое значение:
// 	Массив - Массив непересчитываемых операций.
