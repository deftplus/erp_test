	...
	Иначе
		...
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Функция ПараметрыВыборки() не должна использоваться для объекта %1.';
				|en = 'The SelectionParameters() function cannot be executed for the %1 object.'"),
...
Процедура ВызватьИсключениеОтсутствуетПодсистемаБТС(Знач ИмяПодсистемы) Экспорт
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Невозможно выполнить операцию по причине - в конфигурации не внедрена подсистема ""%1"".
              ...
              |This subsystem is included in cloud technology library which should be implemented separately in the configuration set.
...
Функция ПрочитатьАрхивВнутр(Знач ИдентификаторФайла, Знач Смещение = 0, Знач Размер = Неопределено)
	...
	Если Не ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.ПередачаДанных") 
		...
		ВызватьИсключение НСтр("ru = 'Получение данных из мендежера сервиса невозможно.';
								|en = 'Cannot retrive data from Service Manager'");
	КонецЕсли;
