...
Процедура ПослеПолученияПараметровСинхронизации(Результат, Контекст) Экспорт
	...
	Иначе
		ПараметрыСинхронизации = РезультатВыполненияФоновогоЗадания(НСтр("ru = 'Получение параметров синхронизации';
																		|en = 'Receive synchronization parameters'"),
		...
		Если ПараметрыСинхронизации.ДанныеСертификатов.Количество() = 0
			...
			Ошибка = ОбработкаНеисправностейБЭДКлиент.НоваяОшибка(
				НСтр("ru = 'Получение настроек ЭДО и параметров сертификатов';
					|en = 'Receiving EDI settings and certificate parameters'"),
	...
	Для каждого КлючИЗначение Из ПараметрыСинхронизации.УчетныеЗаписиБезДоступаКСертификатам Цикл
		...
		Если КоличествоСертификатов = 1 Тогда
			ТекстОшибки = НСтр("ru = '<a href = ""Сертификаты"">Сертификат</a> доступен, но в поле ""Пользователь"" сертификата учетной записи указан пользователь, отличный от текущего';
								|en = '<a href = ""Сертификаты"">Certificate</a> is available, but the ""User"" field of the account contains the user, which is different from the current one'");
		Иначе 
			ТекстОшибки = НСтр("ru = '<a href = ""Сертификаты"">Сертификаты(%1)</a> доступны, но в поле ""Пользователь"" сертификата учетной записи указан пользователь, отличный от текущего';
								|en = '<a href = ""Сертификаты"">Certificates (%1)</a> are available, but the ""User"" field of the account contains the user, which is different from the current one'");
		КонецЕсли;
...
Функция НовыйКонтекстОперацииСинхронизации() Экспорт
	...
	КонтекстДиагностики.ЗаголовокОперации = НСтр("ru = 'При отправке и получении документов';
												|en = 'When sending and receiving documents'"); 
