...
Процедура ДобавитьВТекстЗапросаРеквизитыДокумента(ТекстЗапроса, ИмяТаблицы = Неопределено) Экспорт
	...
	ПолеНачислятьИзнос =
	"ДанныеДокумента.ПорядокУчетаБУ = ЗНАЧЕНИЕ(Перечисление.ПорядокПогашенияСтоимостиОС.НачислениеИзносаПоЕНАОФ)";
...
Процедура ОбработкаПроверкиЗаполнения(Объект, Отказ, ПроверяемыеРеквизиты, ПараметрыВыбораОсновныхСредств) Экспорт
	...
	Если Объект.ОбъединениеОднотипныхОС
		...
		И Объект.СрокИспользованияОстаточныйБУ > ПараметрыВыбораОсновныхСредств.ЗначенияПараметров.СрокИспользованияБУ
		...
		ТекстСообщения = НСтр("ru = 'Остаточный срок использования в бух. учете не может превышать нормативный срок использования %1 (мес).';
								|en = 'Residual useful life in bookkeeping can not exceed the standard useful life %1 (months).'");
		ТекстСообщения = СтрШаблон(ТекстСообщения, ПараметрыВыбораОсновныхСредств.ЗначенияПараметров.СрокИспользованияБУ);
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Объект, "СрокИспользованияОстаточныйБУ",, Отказ);
		
	ИначеЕсли НЕ Объект.ОбъединениеОднотипныхОС
		И Объект.СрокИспользованияОстаточныйБУ > Объект.СрокИспользованияБУ
		И Объект.СрокИспользованияБУ <> 0 
		И Объект.ОтражатьВРеглУчете Тогда
		
		ТекстСообщения = НСтр("ru = 'Остаточный срок использования в бух. учете не может превышать нормативный срок использования %1 (мес).';
								|en = 'Residual useful life in bookkeeping can not exceed the standard useful life %1 (months).'");
		ТекстСообщения = СтрШаблон(ТекстСообщения, Объект.СрокИспользованияБУ);
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения, Объект, "СрокИспользованияОстаточныйБУ",, Отказ);
...
Процедура ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры) Экспорт
	...
	Команда = Документы.РегистрацияЗемельныхУчастков.ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании);
	Если Команда <> Неопределено Тогда
		ПодключаемыеКоманды.ДобавитьУсловиеВидимостиКоманды(Команда, "ОтражатьВРеглУчете", Истина);
	КонецЕсли;
	
	Команда = Документы.ОтменаРегистрацииЗемельныхУчастков.ДобавитьКомандуСоздатьНаОсновании(КомандыСозданияНаОсновании);
	Если Команда <> Неопределено Тогда
		ПодключаемыеКоманды.ДобавитьУсловиеВидимостиКоманды(Команда, "ОтражатьВРеглУчете", Истина);
	КонецЕсли;
...
Процедура ДополнитьСлужебныеПараметрыФормы(СлужебныеПараметрыФормы) Экспорт
	...
	СлужебныеПараметрыФормы.РеквизитыКоторыеДолжныСовпадать = 
		СлужебныеПараметрыФормы.РеквизитыКоторыеДолжныСовпадать
		+ ",ГруппаОС,АмортизационнаяГруппа,СрокИспользованияБУ,СрокИспользованияНУ,НалогообложениеНДС";
...
Функция ТекстЗапросаДляПроверкиОС() Экспорт
	...
	|			ТОГДА ЕСТЬNULL(ПорядокУчетаОСБУПервогоОСБУ.АмортизационнаяГруппа, ЗНАЧЕНИЕ(Перечисление.АмортизационныеГруппы.ПустаяСсылка)) 
	...
	|	ЕСТЬNULL(ПорядокУчетаОСБУПервогоОСБУ.НалогообложениеНДС, ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПустаяСсылка)) КАК НалогообложениеНДС,
	...
	|	ВЫБОР
	|		КОГДА &ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОбъединениеОС)
	|			ТОГДА ЕСТЬNULL(ПорядокУчетаОСБУПервогоОСБУ.СостояниеБУ, НЕОПРЕДЕЛЕНО) 
	|		ИНАЧЕ ЕСТЬNULL(ПорядокУчетаОСБУНовыйОС.СостояниеБУ, НЕОПРЕДЕЛЕНО) 
	|	КОНЕЦ КАК СостояниеБУ,
	|
	|	ВЫБОР
	|		КОГДА &ХозяйственнаяОперация = ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ОбъединениеОС)
	|			ТОГДА ЕСТЬNULL(ПорядокУчетаОСБУПервогоОСБУ.СостояниеНУ, НЕОПРЕДЕЛЕНО) 
	...
	|	ПервоеОС.ГруппаОС,
	|	ЕСТЬNULL(ПорядокУчетаОСБУПервогоОСБУ.АмортизационнаяГруппа, ЗНАЧЕНИЕ(Перечисление.АмортизационныеГруппы.ПустаяСсылка)),
	...
	|	ЕСТЬNULL(ПорядокУчетаОСБУПервогоОСБУ.НалогообложениеНДС, ЗНАЧЕНИЕ(Перечисление.ТипыНалогообложенияНДС.ПустаяСсылка)),
	...
	|	ЕСТЬNULL(ПорядокУчетаОСБУПервогоОСБУ.СостояниеБУ, НЕОПРЕДЕЛЕНО) КАК СостояниеБУ,
	|	ЕСТЬNULL(ПорядокУчетаОСБУПервогоОСБУ.СостояниеНУ, НЕОПРЕДЕЛЕНО) КАК СостояниеНУ
	|ИЗ
	...
	|	&ОтражатьВРеглУчете
	|		И ЗначенияПараметров.СостояниеНУ <> ЗНАЧЕНИЕ(Перечисление.СостоянияОС.ПринятоКУчету)
	|		И ЕСТЬNULL(ПорядокУчетаОСБУ.СостояниеНУ, НЕОПРЕДЕЛЕНО) = ЗНАЧЕНИЕ(Перечисление.СостоянияОС.ПринятоКУчету) КАК ПринятоНУ
	|
	|ИЗ
	...
	|			ИЛИ ПорядокУчетаОСУУ.Состояние = ЗНАЧЕНИЕ(Перечисление.СостоянияОС.ПринятоКУчету))
	|
	|	И (НЕ &ОтражатьВРеглУчете 
	|			ИЛИ ПорядокУчетаОСБУ.СостояниеБУ = ЗНАЧЕНИЕ(Перечисление.СостоянияОС.ПринятоКУчету)
	|			ИЛИ ПорядокУчетаОСБУ.СостояниеНУ = ЗНАЧЕНИЕ(Перечисление.СостоянияОС.ПринятоКУчету))
	|
	|	И (ИсходныеОС.ГруппаОС <> ЗначенияПараметров.ГруппаОС
	|			ИЛИ ЕСТЬNULL(ПорядокУчетаОСБУ.АмортизационнаяГруппа, ЗНАЧЕНИЕ(Перечисление.АмортизационныеГруппы.ПустаяСсылка)) <> ЗначенияПараметров.АмортизационнаяГруппа
	|
	|			ИЛИ (&ОбъединениеОднотипныхОС
	|						И ЕСТЬNULL(ПорядокУчетаОС.НаправлениеДеятельности, ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)) <> ЗначенияПараметров.НаправлениеДеятельности
	|					ИЛИ НЕ &ОбъединениеОднотипныхОС
	|						И ЕСТЬNULL(ПорядокУчетаОС.НаправлениеДеятельности, ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)) <> ЗначенияПараметров.НаправлениеДеятельности
	|						И ЕСТЬNULL(ПорядокУчетаОС.НаправлениеДеятельности, ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка)) <> ЗНАЧЕНИЕ(Справочник.НаправленияДеятельности.ПустаяСсылка))
	|
	...
	|							И ЕСТЬNULL(ЗначенияПараметров.ПорядокУчетаБУ, НЕОПРЕДЕЛЕНО) = ЗНАЧЕНИЕ(Перечисление.ПорядокПогашенияСтоимостиОС.НачислениеИзносаПоЕНАОФ))
	|
	|			ИЛИ НЕ ЕСТЬNULL(МестонахождениеОС.Арендатор, НЕОПРЕДЕЛЕНО) В (
	|					ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка),
	|					ЗНАЧЕНИЕ(Справочник.Контрагенты.ПустаяСсылка),
	|					НЕОПРЕДЕЛЕНО)
	|
...
Процедура ДополнитьПараметрыВыбораОсновныхСредств(ВыборкаПараметры, ПараметрыВыбораОсновныхСредств) Экспорт
	...
	ПараметрыВыбораОсновныхСредств.ЗначенияПараметров.Вставить("СрокИспользованияБУ", ВыборкаПараметры.СрокИспользованияБУ);
	...
	ПараметрыВыбораОсновныхСредств.ЗначенияПараметров.Вставить("ПорядокУчетаБУ", ВыборкаПараметры.ПорядокУчетаБУ);
...
Процедура ЗаполнитьПараметрыВыбораОС(ПараметрыВыбораОсновныхСредств, СлужебныеПараметрыФормы) Экспорт
	...
	ПорядокУчетаБУ = ?(ПараметрыВыбораОсновныхСредств.ЗначенияПараметров <> Неопределено, 
...
Процедура СообщитьОРезультатеПроверкиСтроки(Объект, Поле, РезультатПроверки, Отказ) Экспорт
	...
	Если Объект.ОбъединениеОднотипныхОС
		...
		ТекстСообщения = НСтр("ru = 'У основного средства ""%1"" должен быть срок использования в бух. учете %2 мес.';
								|en = 'The fixed asset ""%1"" should have the useful life in bookkeeping %2 months.'");
		ТекстСообщения = СтрШаблон(ТекстСообщения, 
							РезультатПроверки.ОсновноеСредствоПредставление, 
	...
	Если РезультатПроверки.НеПринятоУУ Тогда
		...
		ТекстСообщения = НСтр("ru = 'Для отражения документа в управленческом учете необходимо, чтобы основное средство ""%1"" было принято к учету в управленческом учете.';
								|en = 'The fixed asset ""%1"" must be recognized in management accounting to record the document in management accounting.'");
		ТекстСообщения = СтрШаблон(ТекстСообщения, РезультатПроверки.ОсновноеСредствоПредставление);
	...
	Если РезультатПроверки.НеПринятоБУ Тогда
		...
		ТекстСообщения = НСтр("ru = 'Для отражения документа в регламентированном учете необходимо, чтобы основное средство ""%1"" было принято к учету в бухгалтерском учете.
							  ...
							  |en = 'The fixed asset ""%1"" must be recognized in accounting to record the document in local accounting.
							  |Combining fixed assets where one fixed asset is recognized, and others are not is not allowed.'");
		ТекстСообщения = СтрШаблон(ТекстСообщения, РезультатПроверки.ОсновноеСредствоПредставление);
	...
	Если РезультатПроверки.ПринятоБУ Тогда
		...
		ТекстСообщения = НСтр("ru = 'Для отражения документа в регламентированном учете необходимо, чтобы основное средство ""%1"" не было принято к учету в бухгалтерском учете.
							  |Не допускается объединять основные средства, где одно ОС принято в бухгалтерском учете, а другие не приняты.';
							  |en = 'The fixed asset ""%1"" must not be recognized in accounting to record the document in local accounting.
							  |Combining fixed assets where one fixed asset is recognized, and others are not is not allowed.'");
		ТекстСообщения = СтрШаблон(ТекстСообщения, РезультатПроверки.ОсновноеСредствоПредставление);
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|	Документ.ОбъединениеОС КАК ДанныеДокумента
	|ГДЕ
	|	ДанныеДокумента.АмортизационнаяГруппа = ЗНАЧЕНИЕ(Перечисление.АмортизационныеГруппы.ПустаяСсылка)
	|	И ДанныеДокумента.ОсновноеСредство.УдалитьАмортизационнаяГруппа <> ЗНАЧЕНИЕ(Перечисление.АмортизационныеГруппы.ПустаяСсылка)
...
Процедура ОбработатьОбъектДляПереходаНаНовуюВерсию(ДанныеОбъекта, ДопПараметры) Экспорт
	...
	//++ Локализация
	ЗначенияРеквизитов = ДопПараметры.ЗначенияРеквизитовОС.Найти(ДанныеОбъекта.ОсновноеСредство, "ОсновноеСредство");
...
Функция ДопПараметрыДляПереходаНаНовуюВерсию(МенеджерВременныхТаблиц) Экспорт
	...
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ТабличнаяЧасть.ОсновноеСредство КАК ОсновноеСредство,
	|	ТабличнаяЧасть.ОсновноеСредство.УдалитьАмортизационнаяГруппа КАК АмортизационнаяГруппа,
	...
	|ГДЕ
	|	(ТабличнаяЧасть.АмортизационнаяГруппа = ЗНАЧЕНИЕ(Перечисление.АмортизационныеГруппы.ПустаяСсылка)
	|				И ТабличнаяЧасть.ОсновноеСредство.УдалитьАмортизационнаяГруппа <> ЗНАЧЕНИЕ(Перечисление.АмортизационныеГруппы.ПустаяСсылка)
	|			ИЛИ ТабличнаяЧасть.НедвижимоеИмущество = ЛОЖЬ
	|				И ТабличнаяЧасть.ОсновноеСредство.УдалитьНедвижимоеИмущество <> ЛОЖЬ)
	...
	ДопПараметры.Вставить("ЗначенияРеквизитовОС", Результат.Выгрузить());
