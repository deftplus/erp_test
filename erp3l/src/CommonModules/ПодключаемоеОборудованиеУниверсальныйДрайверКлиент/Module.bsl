...
Функция ПодключитьУстройство(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВыходныеПараметры) Экспорт
	...
	Если ТипОборудованияИспользуемый = "ККТ" И РевизияИнтерфейса < 2002 Тогда  
		СообщениеПользователю = НСтр("ru = 'Данный стандарт подключения драйверов не поддерживется. Необходимо обновить драйвер ККТ.';
									|en = 'This standard of driver connection is not supported. Update the CRE driver.'");
		Результат = Ложь;
...
Функция ОткрытьСмену(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВходныеПараметры, ВыходныеПараметры)
	...
	Если ПараметрыПодключения.ТипОборудования = "ККТ" Тогда
		...
		Если ПараметрыПодключения.СнятСПоддержки Тогда
			ТекстСообщения = НСтр("ru = 'Используется драйвер ККТ снятый с поддержки.';
									|en = 'Using CRE driver, which is no longer supported.'") + Символы.ПС +  
				НСтр("ru = 'Это может проводить к ошибкам учета и замедлению выполнения фискальных операций.';
					|en = 'This may cause accounting errors and slower fiscal transactions.'") + Символы.ПС + 
				НСтр("ru = 'Пожалуйста, обновите драйвер до последней доступной версии.';
					|en = 'Please update the driver to the latest version available.'");
...
Функция ПечатьЧекаКоррекции(ОбъектДрайвера, Параметры, ПараметрыПодключения, ВходныеПараметры, ВыходныеПараметры) Экспорт
	...
	Исключение
		Результат = Ложь;
		ТекстОшибки = НСтр("ru = 'Ошибка формирования коррекции на ККТ.';
