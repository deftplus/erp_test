...
Процедура ПроверитьПараметр(Знач ИмяПроцедурыИлиФункции, Знач ИмяПараметра, Знач ЗначениеПараметра, 
	...
	Проверить(ТипЗнч(ИмяПроцедурыИлиФункции) = Тип("Строка"), 
		НСтр("ru = 'Недопустимое значение параметра ИмяПроцедурыИлиФункции';
			|en = 'Invalid value of ProcedureOrFunctionName parameter.'"), Контекст);
	Проверить(ТипЗнч(ИмяПараметра) = Тип("Строка"), 
		НСтр("ru = 'Недопустимое значение параметра ИмяПараметра';
			|en = 'Invalid value of ParameterName parameter.'"), Контекст);
	...
	Проверить(ЭтоКорректныйТип <> Неопределено, 
		НСтр("ru = 'Недопустимое значение параметра ОжидаемыеТипы';
			|en = 'Invalid value of ExpectedTypes parameter.'"), Контекст);
	...
	Если Версия1.Количество() <> 4 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Неправильный формат параметра СтрокаВерсии1: %1';
				|en = 'Invalid format of VersionString1 parameter: %1.'"), СтрокаВерсии1);
	КонецЕсли;
	Версия2 = СтрРазделить(Строка2, ".");
	Если Версия2.Количество() <> 4 Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
	    	НСтр("ru = 'Неправильный формат параметра СтрокаВерсии2: %1';
				|en = 'Invalid format of VersionString2 parameter: %1.'"), СтрокаВерсии2);
	КонецЕсли;
	...
	Для Каждого Строка Из СтрРазделить(СписокАдресов, ";", Ложь) Цикл
		...
		Если ЗначениеЗаполнено(СтрЗаменить(ЧастьПредставления, ",", "")) Тогда
			...
			ОписаниеОшибки = НСтр("ru = 'Строка адреса не соответствует формату ""Имя получателя <Адрес@Почты>""';
									|en = 'The address string does not match the format ""Recipient name <Email@Address>""'");
			СтруктураАдреса = Новый Структура("Псевдоним, Адрес, ОписаниеОшибки", Псевдоним, Адрес, ОписаниеОшибки);
			Результат.Добавить(СтруктураАдреса);
		КонецЕсли;
	КонецЦикла;
...
Функция РазобратьСтрокуСПочтовымиАдресами(Знач СтрокаПереданныхАдресов, ВызыватьИсключение = Истина) Экспорт
	...
	СообщениеНедопустимыеСимволы = НСтр("ru = 'Недопустимый символ ""%1"" в адресе электронной почты ""%2""';
										|en = 'Prohibited character %1 in the email address %2.'");
...
Функция ПроверитьИПодготовитьПочтовыйАдрес(Знач ПолноеИмяАдресата, Знач ПочтовыйАдрес)
	...
	НедопустимыйСимволВИмениАдресата = НСтр("ru = 'Недопустимый символ в имени адресата.';
											|en = 'The recipient name contains illegal characters.'");
	НедопустимыйСимволВПочтовомАдресе = НСтр("ru = 'Недопустимый символ в почтовом адресе.';
											|en = 'The email address contains illegal characters.'");
	СимволыОбрамления = "<>[]";
