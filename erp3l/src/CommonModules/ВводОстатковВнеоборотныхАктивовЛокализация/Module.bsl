...
//	Строка - Текст запроса
//
Функция ТекстЗапросаТаблицаПрослеживаемыеТоварыВСоставеОС(ТекстыЗапроса, Регистры) Экспорт
...
Процедура ФормаРедактированияСтрокиОС_ПриЧтенииСозданииНаСервере(Форма) Экспорт
	...
	Если Форма.ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.ВводОстатковПереданныхВАрендуОС
		...
		Элементы.МОЛ.Подсказка = НСтр("ru = 'Материально ответственное лицо за основное средство на время аренды.';
										|en = 'Financially liable person responsible for the fixed asset for the duration of lease.'")
	Иначе
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|
	|		ИЛИ ДанныеДокумента.ЕстьИзменениеПараметровАмортизацииБУ
...
Процедура ОбработатьСтрокуОСДляПереходаНаНовуюВерсию(ДанныеСтроки, ДопПараметры) Экспорт
	...
	ЗначенияРеквизитов = ДопПараметры.ЗначенияРеквизитовОС.Найти(ДанныеСтроки.ОсновноеСредство, "ОсновноеСредство");
	Если ЗначенияРеквизитов <> Неопределено Тогда
		ЗаполнитьЗначенияСвойств(ДанныеСтроки, ЗначенияРеквизитов);
	КонецЕсли; 
	
	Если ДанныеСтроки.СрокИспользованияБУ = 0 
		И ДанныеСтроки.СрокИспользованияБУОстаточный <> 0 Тогда
		
		Если ДанныеСтроки.ЕстьИзменениеПараметровАмортизацииБУ 
			И ДанныеСтроки.ДатаПринятияКУчетуБУ <> '000101010000'
			И ДанныеСтроки.ДатаИзмененияПараметровБУ <> '000101010000'
			И ДанныеСтроки.МетодНачисленияАмортизацииБУ <> Перечисления.СпособыНачисленияАмортизацииОС.ПропорциональноОбъемуПродукции
			И ДанныеСтроки.МетодНачисленияАмортизацииБУ <> Перечисления.СпособыНачисленияАмортизацииОС.ПоЕНАОФНа1000кмПробега Тогда
			
			ДанныеСтроки.СрокИспользованияБУ = 
				ДанныеСтроки.СрокИспользованияБУОстаточный 
				+ ОбщегоНазначенияУТ.РазностьДат(
					ДанныеСтроки.ДатаПринятияКУчетуБУ, 
					ДанныеСтроки.ДатаИзмененияПараметровБУ, 
					Перечисления.Периодичность.Месяц);
			
		Иначе
			ДанныеСтроки.СрокИспользованияБУ = ДанныеСтроки.СрокИспользованияБУОстаточный; 
		КонецЕсли;
