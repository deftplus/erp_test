...
//++ НЕ УТ
#Область ОбновлениеДанных_ПланированиеПроизводства

// Преобразует выражения расчета количества и выражения расчета выбора свойства в новый формат для справочников управления производством.
// 
// Параметры:
// 	СправочникОбъект - СправочникОбъект.РесурсныеСпецификации, СправочникОбъект.МаршрутныеКарты, СправочникОбъект.ЭтапыПроизводства - справочник,
// 				в котором выполняется преобразование формул.
// 	ОбъектИзменен - Булево - Флаг изменения объекта при преобразовании.
//
Процедура ПреобразоватьВФорматИдентификаторовВыражениеФормулОбъектаПланированияПроизводства(СправочникОбъект, ОбъектИзменен) Экспорт
	
	КоллекцииБезВидовНоменклатуры = Новый Массив;
	КоллекцииБезВидовНоменклатуры.Добавить("СтруктураПредприятия");
	КоллекцииБезВидовНоменклатуры.Добавить("ЗаказНаПроизводство2_2");
	КоллекцииБезВидовНоменклатуры.Добавить("НаправленияДеятельности");
	
	ВидыНоменклатурыВТЧ = ВидыНоменклатурыВТабличныхЧастяхОбъектаНСИПроизводства(СправочникОбъект);
	ТаблицаДопРеквизитов = ДополнительныеРеквизитыИСведенияПоЗаголовкам();
	
	ПараметрыПреобразования = Новый Структура;
	ПараметрыПреобразования.Вставить("КоллекцииБезВидовНоменклатуры", КоллекцииБезВидовНоменклатуры);
	ПараметрыПреобразования.Вставить("ВидыНоменклатурыВТЧ", ВидыНоменклатурыВТЧ);
	ПараметрыПреобразования.Вставить("ТаблицаДопРеквизитов", ТаблицаДопРеквизитов);
	ПараметрыПреобразования.Вставить("СвойстваХарактеристик", РаботаСФормуламиПовтИсп.СвойстваПоддерживаемыхИсточниковХарактеристик());
	
	Если ТипЗнч(СправочникОбъект) = Тип("СправочникОбъект.ЭтапыПроизводства") Тогда
		
		Для Каждого СтрокаТЧ Из СправочникОбъект.ВидыРабочихЦентров Цикл
			Если ЗначениеЗаполнено(СтрокаТЧ.АлгоритмРасчетаКоличества)
				И НЕ ЗначениеЗаполнено(СтрокаТЧ.АлгоритмРасчетаКоличестваИсторияПереходаНаНовыеФормулы) Тогда
				ПреобразоватьФормулуПроизводстваВФорматФормул(СправочникОбъект,
					СтрокаТЧ,
					"АлгоритмРасчетаКоличества",
					"АлгоритмРасчетаКоличестваИсторияПереходаНаНовыеФормулы",
					"ВидыРабочихЦентров",
					ПараметрыПреобразования);
				ОбъектИзменен = Истина;
			КонецЕсли;
		КонецЦикла;
		Для Каждого СтрокаТЧ Из СправочникОбъект.АльтернативныеВидыРабочихЦентров Цикл
			Если ЗначениеЗаполнено(СтрокаТЧ.АлгоритмРасчетаКоличества)
				И НЕ ЗначениеЗаполнено(СтрокаТЧ.АлгоритмРасчетаКоличестваИсторияПереходаНаНовыеФормулы) Тогда
				ПреобразоватьФормулуПроизводстваВФорматФормул(СправочникОбъект,
					СтрокаТЧ,
	...
	Иначе
		...
		Для Каждого СтрокаТЧ Из СправочникОбъект.ВыходныеИзделия Цикл
			Если ЗначениеЗаполнено(СтрокаТЧ.АлгоритмРасчетаКоличества)
				И НЕ ЗначениеЗаполнено(СтрокаТЧ.АлгоритмРасчетаКоличестваИсторияПереходаНаНовыеФормулы) Тогда
				ПреобразоватьФормулуПроизводстваВФорматФормул(СправочникОбъект,
					СтрокаТЧ,
			...
			КонецЕсли;
		КонецЦикла;
		Для Каждого СтрокаТЧ Из СправочникОбъект.ВозвратныеОтходы Цикл
			Если ЗначениеЗаполнено(СтрокаТЧ.АлгоритмРасчетаКоличества)
				И НЕ ЗначениеЗаполнено(СтрокаТЧ.АлгоритмРасчетаКоличестваИсторияПереходаНаНовыеФормулы) Тогда
				ПреобразоватьФормулуПроизводстваВФорматФормул(СправочникОбъект,
					СтрокаТЧ,
			...
			КонецЕсли;
		КонецЦикла;
		Для Каждого СтрокаТЧ Из СправочникОбъект.МатериалыИУслуги Цикл
			Если ЗначениеЗаполнено(СтрокаТЧ.АлгоритмРасчетаКоличества)
				И НЕ ЗначениеЗаполнено(СтрокаТЧ.АлгоритмРасчетаКоличестваИсторияПереходаНаНовыеФормулы) Тогда
				ПреобразоватьФормулуПроизводстваВФорматФормул(СправочникОбъект,
					СтрокаТЧ,
			...
			КонецЕсли;
		КонецЦикла;
		Для Каждого СтрокаТЧ Из СправочникОбъект.Трудозатраты Цикл
			Если ЗначениеЗаполнено(СтрокаТЧ.АлгоритмРасчетаКоличества)
				И НЕ ЗначениеЗаполнено(СтрокаТЧ.АлгоритмРасчетаКоличестваИсторияПереходаНаНовыеФормулы) Тогда
				ПреобразоватьФормулуПроизводстваВФорматФормул(СправочникОбъект,
					СтрокаТЧ,
			...
			КонецЕсли;
		КонецЦикла;
		Для Каждого СтрокаТЧ Из СправочникОбъект.СоответствиеСвойств Цикл
			Если ЗначениеЗаполнено(СтрокаТЧ.АлгоритмРасчетаЗначения)
				И НЕ ЗначениеЗаполнено(СтрокаТЧ.АлгоритмРасчетаЗначенияИсторияПереходаНаНовыеФормулы) Тогда
				ПреобразоватьФормулуПроизводстваВФорматФормул(СправочникОбъект,
					СтрокаТЧ,
					"АлгоритмРасчетаЗначения",
					"АлгоритмРасчетаЗначенияИсторияПереходаНаНовыеФормулы",
					"СоответствиеСвойств",
					ПараметрыПреобразования);
				ОбъектИзменен = Истина;
			КонецЕсли;
...
// Преобразует формулу расчета в новый формат
// 
// Параметры:
// 	СправочникОбъект - СправочникОбъект.МаршрутныеКарты, СправочникОбъект.РесурсныеСпецификации, СправочникОбъект.ЭтапыПроизводства - Обрабатываемый элемент.
// 	СтрокаТЧ - СтрокаТабличнойЧасти - Строка табличной части.
// 	ИмяРеквизита - Строка - Имя реквизита, содержащего формулу.
// 	ИмяРеквизитаИстория - Строка - Имя реквизита истории.
// 	ИмяТЧВладельца - Строка - Имя табличной части.
// 	ПараметрыПреобразования - Структура - общие параметры преобразования.
Процедура ПреобразоватьФормулуПроизводстваВФорматФормул(СправочникОбъект, СтрокаТЧ, ИмяРеквизита, ИмяРеквизитаИстория, ИмяТЧВладельца, ПараметрыПреобразования)
	
	СвойстваХарактеристик = ПараметрыПреобразования.СвойстваХарактеристик;
	КоллекцииБезВидовНоменклатуры = ПараметрыПреобразования.КоллекцииБезВидовНоменклатуры;
	ВидыНоменклатурыВТЧ = ПараметрыПреобразования.ВидыНоменклатурыВТЧ;
	ТаблицаДопРеквизитов = ПараметрыПреобразования.ТаблицаДопРеквизитов;
	
	ФормулаСоответствияСвойств = (ИмяРеквизита = "АлгоритмРасчетаЗначения");
	НомерТекущейСтрокиВКоллекции = СтрокаТЧ.НомерСтроки;
	
	ИсходноеВыражение = СтрокаТЧ[ИмяРеквизита];
	ВыражениеЗамены = ИсходноеВыражение;
	Если ФормулаУказанаВФорматеИдентификаторов(ИсходноеВыражение, СвойстваХарактеристик) Тогда
		
		СтрокаТЧ[ИмяРеквизитаИстория] = ВыражениеЗамены;
		
	Иначе
		ЕстьОшибкаПреобразования = Ложь;
		ТекстОшибки = "";
			
		Операнды = РаботаСФормуламиКлиентСервер.ОперандыТекстовойФормулы(ИсходноеВыражение, Истина);
		Для Каждого ОперандБезВнешнихСкобок Из Операнды Цикл
			ПараметрыОперанда = ПараметрыОперандаПроизводства(ОперандБезВнешнихСкобок,
				ФормулаСоответствияСвойств,
				НомерТекущейСтрокиВКоллекции);
			
			Если ПараметрыОперанда.ЭтоХарактеристика Тогда
				Если КоллекцииБезВидовНоменклатуры.Найти(ПараметрыОперанда.Коллекция) <> Неопределено Тогда
					ВладелецДопРеквизита = ПараметрыОперанда.Коллекция;
				Иначе
					Поиск = Новый Структура("ИмяКоллекции, НомерСтроки", ПараметрыОперанда.Коллекция, ПараметрыОперанда.НомерСтрокиВКоллекции);
					СтрокиВидовНоменклатуры = ВидыНоменклатурыВТЧ.НайтиСтроки(Поиск);
					Если СтрокиВидовНоменклатуры.Количество() = 0 Тогда
						ЕстьОшибкаПреобразования = Истина;
						ТекстОшибки = НСтр("ru = 'Не найдена строка номер %1 в табличной части ""%2"" в формуле %3.
											|Формула находится в строке номер %4 табличной части ""%5"".';
											|en = 'Cannot find row number %1 in table ""%2"" in formula %3.
											|Formula is located in row number %4 of table ""%5"".'");
						ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки,
							ПараметрыОперанда.НомерСтрокиВКоллекции,
							ПараметрыОперанда.Коллекция,
							ИсходноеВыражение,
							СтрокаТЧ.НомерСтроки,
							ИмяТЧВладельца);
					Иначе
						ВладелецДопРеквизита = СтрокиВидовНоменклатуры[0].ВидНоменклатуры;
					КонецЕсли;
				КонецЕсли;
				Если Не ЕстьОшибкаПреобразования Тогда
					Поиск = Новый Структура("ВладелецДопРеквизита, ЗаголовокВерхнийРегистр", ВладелецДопРеквизита, ВРег(ПараметрыОперанда.ЗаголовокХарактеристики));
					СтрокиХарактеристик = ТаблицаДопРеквизитов.НайтиСтроки(Поиск);
					Если СтрокиХарактеристик.Количество() = 0 Тогда
						ЕстьОшибкаПреобразования = Истина;
						Если ТипЗнч(ВладелецДопРеквизита) = Тип("СправочникСсылка.ВидыНоменклатуры") Тогда
							ТекстОшибки = НСтр("ru = 'Не найдена характеристика с заголовком ""%1"" для вида номенклатуры ""%2"" в формуле %3.
													|Формула находится в строке номер %4 табличной части ""%5"".';
													|en = 'Cannot find item variant named ""%1"" for product type ""%2"" in formula %3.
													|Formula is located in row number %4 of table ""%5"".'");
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки,
								ПараметрыОперанда.ЗаголовокХарактеристики,
								ВладелецДопРеквизита,
								ИсходноеВыражение,
								СтрокаТЧ.НомерСтроки,
								ИмяТЧВладельца);
						Иначе
							Если ПараметрыОперанда.Коллекция = "СтруктураПредприятия" Тогда
								ПредставлениеНабора = НСтр("ru = 'Структура предприятия';
															|en = 'Business units'");
							ИначеЕсли ПараметрыОперанда.Коллекция = "ЗаказНаПроизводство2_2" Тогда
								ПредставлениеНабора = НСтр("ru = 'Заказ на производство';
															|en = 'Production order'");
							Иначе
								ПредставлениеНабора = НСтр("ru = 'Направления деятельности';
															|en = 'Lines of business'");
							КонецЕсли;
							ТекстОшибки = НСтр("ru = 'Не найдена характеристика с заголовком ""%1"" для набора дополнительных реквизитов ""%2"" в формуле %3.
													|Формула находится в строке номер %4 табличной части ""%5"".';
													|en = 'Cannot find item variant with title ""%1"" for the set of additional attributes ""%2"" in formula %3.
													|Formula is located in row number %4 of table ""%5"".'");
							ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки,
								ПараметрыОперанда.ЗаголовокХарактеристики,
								ПредставлениеНабора,
						...
						ВызватьИсключение ТекстОшибки;
					Иначе
						ДлинаЗаголовка = СтрДлина(ПараметрыОперанда.ЗаголовокХарактеристики);
						Если СтрДлина(ОперандБезВнешнихСкобок) = ДлинаЗаголовка Тогда
							ТекстДоЗаголовка = "";
						Иначе
							ТекстДоЗаголовка = Сред(ОперандБезВнешнихСкобок, 1, СтрДлина(ОперандБезВнешнихСкобок) - ДлинаЗаголовка);
						КонецЕсли;
						ЗаменаОперанда = ТекстДоЗаголовка + СтрокиХарактеристик[0].ПолныйИдентификатор;
						ОперандСоСкобками = РаботаСФормуламиКлиентСервер.ПолучитьТекстОперандаДляВставки(ОперандБезВнешнихСкобок);
						ЗаменаОперанда = РаботаСФормуламиКлиентСервер.ПолучитьТекстОперандаДляВставки(ЗаменаОперанда);
						
						ВыражениеЗамены = СтрЗаменить(ВыражениеЗамены, ОперандСоСкобками, ЗаменаОперанда);
					КонецЕсли;
				КонецЕсли;
				Если ЕстьОшибкаПреобразования Тогда
					Прервать;
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
		...
		Если НЕ ЕстьОшибкаПреобразования Тогда
			СтрокаТЧ[ИмяРеквизита] = ВыражениеЗамены;
			СтрокаТЧ[ИмяРеквизитаИстория] = ИсходноеВыражение;
		Иначе
...
Функция ФормулаУказанаВФорматеИдентификаторов(Формула, СвойстваХарактеристик)
	
	// Вся формула указана или в формате идентификаторов или представлений.
	// Но для 1 объекта разные формулы могут быть в разных форматах,
	// если было ручное исправление.
	
	ФорматИдентификаторов = Ложь;
	
	Если СтрНайти(Формула, "}") > 0 Тогда
		Для Каждого ОписаниеСвойств Из СвойстваХарактеристик Цикл
			Если СтрНайти(Формула, "{" + ОписаниеСвойств.ПрефиксТипаВидаХарактеристик) > 0 Тогда
				ФорматИдентификаторов = Истина;
				Прервать;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
	
	Возврат ФорматИдентификаторов;
	
КонецФункции

Функция ПараметрыОперандаПроизводства(Операнд, ФормулаСоответствияСвойств, НомерТекущейСтрокиВКоллекции)
	
	СвойстваОперанда = Новый Структура;
	СвойстваОперанда.Вставить("Коллекция", "");
	СвойстваОперанда.Вставить("НомерСтрокиВКоллекции", 0);
	СвойстваОперанда.Вставить("ЭтоХарактеристика", Ложь);
	СвойстваОперанда.Вставить("ЗаголовокХарактеристики", "");
	
	ОписаниеТипаЧисло = ОбщегоНазначения.ОписаниеТипаЧисло(10);
	
	ТекстОперанда = Операнд;
	Если СтрНачинаетсяС(Операнд, "[") И СтрЗаканчиваетсяНа(Операнд, "]") Тогда
		ТекстОперанда = Сред(ТекстОперанда, 1, СтрДлина(ТекстОперанда) - 1);
		ТекстОперанда = Сред(ТекстОперанда, 2);
	КонецЕсли;
	Если ФормулаСоответствияСвойств Тогда
		// В операндах для формул соответствия свойств (подбор характеристик номенклатуры) участвуют только
		// доп.реквизиты 1ой строки табличной части ВыходныеИзделия.
		СвойстваОперанда.Коллекция = "ВыходныеИзделия";
		СвойстваОперанда.НомерСтрокиВКоллекции = 1;
		СвойстваОперанда.ЭтоХарактеристика = Истина;
		СвойстваОперанда.ЗаголовокХарактеристики = ТекстОперанда;
	Иначе
		Если СтрНачинаетсяС(ТекстОперанда, "ОсновноеИзделие.") Тогда
			СвойстваОперанда.Коллекция = "ВыходныеИзделия";
			СвойстваОперанда.НомерСтрокиВКоллекции = 1;
			ОстатокТекста = Сред(ТекстОперанда, СтрДлина("ОсновноеИзделие.") + 1);
			ЗаполнитьФлагКоличестваИлиЗаголовокХарактеристики(СвойстваОперанда, ОстатокТекста);
		ИначеЕсли СтрНачинаетсяС(ТекстОперанда, "ВыходныеИзделия[") Тогда
			НомерПозиции = СтрНайти(ТекстОперанда, "].");
			КоличествоСимволов = НомерПозиции - СтрДлина("ВыходныеИзделия[") - 1;
			ТекстНомерСтроки = Сред(ТекстОперанда, СтрДлина("ВыходныеИзделия[") + 1, КоличествоСимволов);
			ОстатокТекста = Сред(ТекстОперанда, НомерПозиции + СтрДлина("]."));
			СвойстваОперанда.Коллекция = "ВыходныеИзделия";
			СвойстваОперанда.НомерСтрокиВКоллекции = ОписаниеТипаЧисло.ПривестиЗначение(ТекстНомерСтроки);
			ЗаполнитьФлагКоличестваИлиЗаголовокХарактеристики(СвойстваОперанда, ОстатокТекста);
		ИначеЕсли СтрНачинаетсяС(ТекстОперанда, "ВыходноеИзделие.") Тогда
			СвойстваОперанда.Коллекция = "ВыходныеИзделия";
			СвойстваОперанда.НомерСтрокиВКоллекции = НомерТекущейСтрокиВКоллекции;
			ОстатокТекста = Сред(ТекстОперанда, СтрДлина("ВыходноеИзделие.") + 1);
			ЗаполнитьФлагКоличестваИлиЗаголовокХарактеристики(СвойстваОперанда, ОстатокТекста);
		ИначеЕсли СтрНачинаетсяС(ТекстОперанда, "ВозвратныеОтходы[") Тогда
			НомерПозиции = СтрНайти(ТекстОперанда, "].");
			КоличествоСимволов = НомерПозиции - СтрДлина("ВозвратныеОтходы[") - 1;
			ТекстНомерСтроки = Сред(ТекстОперанда, СтрДлина("ВозвратныеОтходы[") + 1, КоличествоСимволов);
			ОстатокТекста = Сред(ТекстОперанда, НомерПозиции + СтрДлина("]."));
			СвойстваОперанда.Коллекция = "ВозвратныеОтходы";
			СвойстваОперанда.НомерСтрокиВКоллекции = ОписаниеТипаЧисло.ПривестиЗначение(ТекстНомерСтроки);
			ЗаполнитьФлагКоличестваИлиЗаголовокХарактеристики(СвойстваОперанда, ОстатокТекста);
		ИначеЕсли СтрНачинаетсяС(ТекстОперанда, "ВозвратныйОтход.") Тогда
			СвойстваОперанда.Коллекция = "ВозвратныеОтходы";
			СвойстваОперанда.НомерСтрокиВКоллекции = НомерТекущейСтрокиВКоллекции;
			ОстатокТекста = Сред(ТекстОперанда, СтрДлина("ВозвратныйОтход.") + 1);
			ЗаполнитьФлагКоличестваИлиЗаголовокХарактеристики(СвойстваОперанда, ОстатокТекста);
		ИначеЕсли СтрНачинаетсяС(ТекстОперанда, "МатериалыИУслуги[") Тогда
			НомерПозиции = СтрНайти(ТекстОперанда, "].");
			КоличествоСимволов = НомерПозиции - СтрДлина("МатериалыИУслуги[") - 1;
			ТекстНомерСтроки = Сред(ТекстОперанда, СтрДлина("МатериалыИУслуги[") + 1, КоличествоСимволов);
			ОстатокТекста = Сред(ТекстОперанда, НомерПозиции + СтрДлина("]."));
			СвойстваОперанда.Коллекция = "МатериалыИУслуги";
			СвойстваОперанда.НомерСтрокиВКоллекции = ОписаниеТипаЧисло.ПривестиЗначение(ТекстНомерСтроки);
			ЗаполнитьФлагКоличестваИлиЗаголовокХарактеристики(СвойстваОперанда, ОстатокТекста);
		ИначеЕсли СтрНачинаетсяС(ТекстОперанда, "Материал.") Тогда
			СвойстваОперанда.Коллекция = "МатериалыИУслуги";
			СвойстваОперанда.НомерСтрокиВКоллекции = НомерТекущейСтрокиВКоллекции;
			ОстатокТекста = Сред(ТекстОперанда, СтрДлина("Материал.") + 1);
			ЗаполнитьФлагКоличестваИлиЗаголовокХарактеристики(СвойстваОперанда, ОстатокТекста);
		ИначеЕсли СтрНачинаетсяС(ТекстОперанда, "Подразделение.") Тогда
			СвойстваОперанда.Коллекция = "СтруктураПредприятия";
			СвойстваОперанда.НомерСтрокиВКоллекции = 0;
			ОстатокТекста = Сред(ТекстОперанда, СтрДлина("Подразделение.") + 1);
			СвойстваОперанда.ЭтоХарактеристика = Истина;
			СвойстваОперанда.ЗаголовокХарактеристики = ОстатокТекста;
		ИначеЕсли СтрНачинаетсяС(ТекстОперанда, "Распоряжение.") Тогда
			СвойстваОперанда.Коллекция = "ЗаказНаПроизводство2_2";
			СвойстваОперанда.НомерСтрокиВКоллекции = 0;
			ОстатокТекста = Сред(ТекстОперанда, СтрДлина("Распоряжение.") + 1);
			СвойстваОперанда.ЭтоХарактеристика = Истина;
			СвойстваОперанда.ЗаголовокХарактеристики = ОстатокТекста;
		ИначеЕсли СтрНачинаетсяС(ТекстОперанда, "НаправлениеДеятельности.") Тогда
			СвойстваОперанда.Коллекция = "НаправленияДеятельности";
			СвойстваОперанда.НомерСтрокиВКоллекции = 0;
			ОстатокТекста = Сред(ТекстОперанда, СтрДлина("НаправлениеДеятельности.") + 1);
			СвойстваОперанда.ЭтоХарактеристика = Истина;
			СвойстваОперанда.ЗаголовокХарактеристики = ОстатокТекста;
		КонецЕсли;
	КонецЕсли;
	
	Возврат СвойстваОперанда
	
КонецФункции

Процедура ЗаполнитьФлагКоличестваИлиЗаголовокХарактеристики(СвойстваОперанда, ОстатокТекста)
	Если ВРЕГ(ОстатокТекста) = ВРЕГ("Количество") Тогда
		СвойстваОперанда.ЭтоХарактеристика = Ложь;
	Иначе
		СвойстваОперанда.ЭтоХарактеристика = Истина;
		СвойстваОперанда.ЗаголовокХарактеристики = ОстатокТекста;
	КонецЕсли;
...
Функция ДополнительныеРеквизитыИСведенияПоЗаголовкам()
	
	Запрос = Новый Запрос;
	Текст =
	"ВЫБРАТЬ
	|	""СтруктураПредприятия"" КАК ВладелецДопРеквизита,
	|	ДополнительныеРеквизитыИСведения.Ссылка КАК ДопРеквизит,
	|	ДополнительныеРеквизитыИСведения.Заголовок КАК ЗаголовокХарактеристики,
	|	&ВыражениеИдентификатор КАК ПолныйИдентификатор,
	|	ДополнительныеРеквизитыИСведения.ТипЗначения
	|ИЗ
	|	Справочник.НаборыДополнительныхРеквизитовИСведений.ДополнительныеРеквизиты КАК НаборыДопРеквизиты
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПланВидовХарактеристик.ДополнительныеРеквизитыИСведения КАК ДополнительныеРеквизитыИСведения
	|		ПО НаборыДопРеквизиты.Свойство = ДополнительныеРеквизитыИСведения.Ссылка
	|ГДЕ
	|	НаборыДопРеквизиты.Ссылка = &НаборСтруктураПредприятия
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	""НаправленияДеятельности"" КАК ВладелецДопРеквизита,
	|	ДополнительныеРеквизитыИСведения.Ссылка КАК ДопРеквизит,
	|	ДополнительныеРеквизитыИСведения.Заголовок КАК ЗаголовокХарактеристики,
	|	&ВыражениеИдентификатор КАК ПолныйИдентификатор,
	|	ДополнительныеРеквизитыИСведения.ТипЗначения
	...
	|		ПО НаборыДопРеквизиты.Свойство = ДополнительныеРеквизитыИСведения.Ссылка
	|ГДЕ
	|	НаборыДопРеквизиты.Ссылка = &НаборНаправленияДеятельности
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	""ЗаказНаПроизводство2_2"" КАК ВладелецДопРеквизита,
	|	ДополнительныеРеквизитыИСведения.Ссылка КАК ДопРеквизит,
	|	ДополнительныеРеквизитыИСведения.Заголовок КАК ЗаголовокХарактеристики,
	|	&ВыражениеИдентификатор КАК ПолныйИдентификатор,
	|	ДополнительныеРеквизитыИСведения.ТипЗначения
	...
	|		ПО НаборыДопРеквизиты.Свойство = ДополнительныеРеквизитыИСведения.Ссылка
	|ГДЕ
	|	НаборыДопРеквизиты.Ссылка = &НаборЗаказНаПроизводство2_2
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ВидыНоменклатуры.Ссылка КАК ВладелецДопРеквизита,
	|	ДополнительныеРеквизитыИСведения.Ссылка КАК ДопРеквизит,
	|	ДополнительныеРеквизитыИСведения.Заголовок КАК ЗаголовокХарактеристики,
	|	&ВыражениеИдентификатор КАК ПолныйИдентификатор,
	|	ДополнительныеРеквизитыИСведения.ТипЗначения
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.НаборыДополнительныхРеквизитовИСведений.ДополнительныеРеквизиты КАК НаборыДопРеквизиты
	|		ПО НаборыДопРеквизиты.Ссылка = &Набор_Справочник_Номенклатура_Общие
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПланВидовХарактеристик.ДополнительныеРеквизитыИСведения КАК ДополнительныеРеквизитыИСведения
	|		ПО НаборыДопРеквизиты.Свойство = ДополнительныеРеквизитыИСведения.Ссылка
	|
	...
	|	Справочник.ВидыНоменклатуры КАК ВидыНоменклатуры
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.НаборыДополнительныхРеквизитовИСведений.ДополнительныеРеквизиты КАК НаборыДопРеквизиты
	|		ПО НаборыДопРеквизиты.Ссылка = ВидыНоменклатуры.НаборСвойств
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПланВидовХарактеристик.ДополнительныеРеквизитыИСведения КАК ДополнительныеРеквизитыИСведения
	|		ПО НаборыДопРеквизиты.Свойство = ДополнительныеРеквизитыИСведения.Ссылка
	|
	...
	|	Справочник.ВидыНоменклатуры КАК ВидыНоменклатуры
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.НаборыДополнительныхРеквизитовИСведений.ДополнительныеРеквизиты КАК НаборыДопРеквизиты
	|		ПО НаборыДопРеквизиты.Ссылка = &Набор_Справочник_ХарактеристикиНоменклатуры_Общие
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПланВидовХарактеристик.ДополнительныеРеквизитыИСведения КАК ДополнительныеРеквизитыИСведения
	|		ПО НаборыДопРеквизиты.Свойство = ДополнительныеРеквизитыИСведения.Ссылка
	|
	...
	|	Справочник.ВидыНоменклатуры КАК ВидыНоменклатуры
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.НаборыДополнительныхРеквизитовИСведений.ДополнительныеРеквизиты КАК НаборыДопРеквизиты
	|		ПО (ВЫБОР КОГДА ВидыНоменклатуры.ИспользованиеХарактеристик = ЗНАЧЕНИЕ(Перечисление.ВариантыИспользованияХарактеристикНоменклатуры.ОбщиеСДругимВидомНоменклатуры)
	|				ТОГДА ВидыНоменклатуры.ВладелецХарактеристик.НаборСвойствХарактеристик
	|				ИНАЧЕ ВидыНоменклатуры.НаборСвойствХарактеристик
	|			КОНЕЦ) = НаборыДопРеквизиты.Ссылка
	|
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ПланВидовХарактеристик.ДополнительныеРеквизитыИСведения КАК ДополнительныеРеквизитыИСведения
	|		ПО НаборыДопРеквизиты.Свойство = ДополнительныеРеквизитыИСведения.Ссылка";
	
	МетаданныеДопРеквизитов = Метаданные.ПланыВидовХарактеристик.ДополнительныеРеквизитыИСведения;
	ВыражениеИдентификатор = ВыражениеПолногоИдентификатораХарактеристики(МетаданныеДопРеквизитов, Ложь);
	ВыражениеИдентификатор = СтрЗаменить(ВыражениеИдентификатор, "%ИмяТаблицы%", "ДополнительныеРеквизитыИСведения");
	Текст = СтрЗаменить(Текст, "&ВыражениеИдентификатор", ВыражениеИдентификатор);
	Запрос.Текст = Текст;
	
	НаборСтруктураПредприятия = УправлениеСвойствами.НаборСвойствПоИмени("Справочник_СтруктураПредприятия");
	НаборНаправленияДеятельности = УправлениеСвойствами.НаборСвойствПоИмени("Справочник_НаправленияДеятельности");
	НаборЗаказНаПроизводство2_2 = УправлениеСвойствами.НаборСвойствПоИмени("Документ_ЗаказНаПроизводство2_2");
	
	Запрос.УстановитьПараметр("НаборСтруктураПредприятия", НаборСтруктураПредприятия);
	Запрос.УстановитьПараметр("НаборНаправленияДеятельности", НаборНаправленияДеятельности);
	Запрос.УстановитьПараметр("НаборЗаказНаПроизводство2_2", НаборЗаказНаПроизводство2_2);
	...
	ТаблицаДопРеквизитов.Колонки.Добавить("ЗаголовокВерхнийРегистр", ОбщегоНазначения.ОписаниеТипаСтрока(150));
	
	КоличествоСтрок = ТаблицаДопРеквизитов.Количество();
	Для НомерСтроки = 1 По КоличествоСтрок Цикл
		ИндексСтроки = КоличествоСтрок - НомерСтроки;
		СтрокаТЗ = ТаблицаДопРеквизитов[ИндексСтроки];
		
		Типы = СтрокаТЗ.ТипЗначения.Типы();
		Если Типы.Найти(Тип("Число")) = Неопределено
			И Типы.Найти(Тип("Строка")) = Неопределено
			И Типы.Найти(Тип("Булево")) = Неопределено Тогда
			ТаблицаДопРеквизитов.Удалить(СтрокаТЗ);
		Иначе
			СтрокаТЗ.ЗаголовокВерхнийРегистр = ВРег(СтрокаТЗ.ЗаголовокХарактеристики);
		КонецЕсли;
	КонецЦикла;
	ТаблицаДопРеквизитов.Индексы.Добавить("ВладелецДопРеквизита, ЗаголовокВерхнийРегистр");
	
	Возврат ТаблицаДопРеквизитов;
	
КонецФункции

Функция ВидыНоменклатурыВТабличныхЧастяхОбъектаНСИПроизводства(СправочникОбъект)
	
	ЗапросПоВидамНоменклатуры = Новый Запрос;
	ЗапросПоВидамНоменклатуры.МенеджерВременныхТаблиц = Новый МенеджерВременныхТаблиц();
	
	Если ТипЗнч(СправочникОбъект) = Тип("СправочникОбъект.ЭтапыПроизводства") Тогда
		
		ЗапросПоВидамНоменклатуры.Текст =
		"ВЫБРАТЬ
		...
		|ГДЕ
		|	ВыходныеИзделия.Ссылка = &РесурснаяСпецификация
		|;
		|
		|ВЫБРАТЬ
		...
		|ГДЕ
		|	ВозвратныеОтходы.Ссылка = &РесурснаяСпецификация
		|;
		|
		|ВЫБРАТЬ
		...
		|ГДЕ
		|	МатериалыИУслуги.Ссылка = &РесурснаяСпецификация";
		ЗапросПоВидамНоменклатуры.УстановитьПараметр("РесурснаяСпецификация", СправочникОбъект.Владелец);
		ЗапросПоВидамНоменклатуры.Выполнить();
	...
	Иначе
		...
		ЗапросПоВидамНоменклатуры.УстановитьПараметр("ВыходныеИзделия", СправочникОбъект.ВыходныеИзделия.Выгрузить(, "НомерСтроки, Номенклатура"));
		ЗапросПоВидамНоменклатуры.УстановитьПараметр("ВозвратныеОтходы", СправочникОбъект.ВозвратныеОтходы.Выгрузить(, "НомерСтроки, Номенклатура"));
		ЗапросПоВидамНоменклатуры.УстановитьПараметр("МатериалыИУслуги", СправочникОбъект.МатериалыИУслуги.Выгрузить(, "НомерСтроки, Номенклатура"));
		ЗапросПоВидамНоменклатуры.Выполнить();
	...
	ЗапросПоВидамНоменклатуры.Текст =
	"ВЫБРАТЬ
		|	""ВыходныеИзделия"" КАК ИмяКоллекции,
		|	ВыходныеИзделия.НомерСтроки КАК НомерСтроки,
		|	ВЫРАЗИТЬ(ВыходныеИзделия.Номенклатура КАК Справочник.Номенклатура).ВидНоменклатуры КАК ВидНоменклатуры
		|ИЗ
		...
		|
		|ВЫБРАТЬ
		|	""ВозвратныеОтходы"" КАК ИмяКоллекции,
		|	ВозвратныеОтходы.НомерСтроки КАК НомерСтроки,
		|	ВЫРАЗИТЬ(ВозвратныеОтходы.Номенклатура КАК Справочник.Номенклатура).ВидНоменклатуры КАК ВидНоменклатуры
		|ИЗ
		|	ВТВозвратныеОтходы КАК ВозвратныеОтходы
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	""МатериалыИУслуги"" КАК ИмяКоллекции,
		|	МатериалыИУслуги.НомерСтроки КАК НомерСтроки,
		|	ВЫРАЗИТЬ(МатериалыИУслуги.Номенклатура КАК Справочник.Номенклатура).ВидНоменклатуры КАК ВидНоменклатуры
		|ИЗ
		|	ВТМатериалыИУслуги КАК МатериалыИУслуги";
		
	ВидыНоменклатурыДокумента = ЗапросПоВидамНоменклатуры.Выполнить().Выгрузить();
	ВидыНоменклатурыДокумента.Индексы.Добавить("ИмяКоллекции, НомерСтроки");
	
	Возврат ВидыНоменклатурыДокумента;
	
КонецФункции

#КонецОбласти // ОбновлениеДанных_ПланированиеПроизводства
//-- НЕ УТ
