			...
			МассивСтрокОписания.Добавить(
				СтрШаблон(СтрокаШаблон,
					НСтр("ru = 'Вариант учета НДС при изменении вида деятельности';
						|en = 'VAT accounting option on activity category change'"),
...
Процедура ЗаполнитьСтавкуНДСПоНоменклатуреИНалогообложению(СтавкаНДС, Знач Дата, Знач Номенклатура, Знач НалогообложениеНДС, 
	...
	Если НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ПроизводствоСДЦ Тогда
		...
		Иначе
			СтавкаНДС = УчетНДСУП.СтавкаНДСПоУмолчанию(Дата, Ложь);
		КонецЕсли;
		
	ИначеЕсли НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяЕНВД Тогда
		
		Если РеквизитыНоменклатуры <> Неопределено И РеквизитыНоменклатуры.ПодакцизныйТовар Тогда
			СтавкаНДС = РеквизитыНоменклатуры.СтавкаНДС;
		Иначе
	...
	ИначеЕсли НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ОблагаетсяНДСУПокупателя Тогда
		...
		СтавкаНДС = УчетНДСУП.СтавкаНДСПоУмолчанию(Дата, Истина, НалогообложениеНДС);
	...
	ИначеЕсли НалогообложениеНДС = Перечисления.ТипыНалогообложенияНДС.ПродажаОблагаетсяНДС 
...
Процедура СкорректироватьСтавкуНДС(НоваяСтавкаНДС, СтавкаНДС, Знач Дата) Экспорт
	...
	//++ Локализация
	Если СтавкаНДС = СтавкаНДСПоПеречислению(Перечисления.СтавкиНДС.НДС18) И Дата >= '20190101' Тогда
		НоваяСтавкаНДС = СтавкаНДСПоПеречислению(Перечисления.СтавкиНДС.НДС20);
	ИначеЕсли СтавкаНДС = СтавкаНДСПоПеречислению(Перечисления.СтавкиНДС.НДС18_118) И Дата >= '20190101' Тогда
		НоваяСтавкаНДС = СтавкаНДСПоПеречислению(Перечисления.СтавкиНДС.НДС20_120);
	ИначеЕсли СтавкаНДС = СтавкаНДСПоПеречислению(Перечисления.СтавкиНДС.НДС20) И Дата < '20190101' Тогда
		НоваяСтавкаНДС = СтавкаНДСПоПеречислению(Перечисления.СтавкиНДС.НДС18);
	ИначеЕсли СтавкаНДС = СтавкаНДСПоПеречислению(Перечисления.СтавкиНДС.НДС20_120) И Дата < '20190101' Тогда
		НоваяСтавкаНДС = СтавкаНДСПоПеречислению(Перечисления.СтавкиНДС.НДС18_118);
	КонецЕсли;
	//-- Локализация
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.Признак = "ПодакцизныйТовар" Тогда
			...
			ТекстОшибки = НСтр("ru = 'При режиме налогообложения ""Продажа облагается ЕНВД"" запрещено выбирать подакцизный товар в строке %НомерСтроки% списка ""Товары""';
								|en = 'You cannot select excisable goods in line %НомерСтроки% of the Item list for the ""Sale is subject to UTII"" taxation treatment'");
		...
		ИначеЕсли Выборка.Признак = "ОблагаетсяНДСУПокупателя" Тогда
			ТекстОшибки = НСтр("ru = 'Товар в строке %НомерСтроки% списка ""Товары"" не соответствует режиму налогообложения ""Облагается НДС у покупателя""';
								|en = 'Goods in the %НомерСтроки% line of the Item list does not match the Subject to VAT at customer taxation treatment'");
			ТекстОшибки = СтрЗаменить(ТекстОшибки, "%НомерСтроки%",  Выборка.НомерСтроки);
...
Процедура СообщитьОРезультатахКонтроляИзменений(РезультатыКонтроля, Документ, Отказ) Экспорт
	...
	Если ПроведениеДокументов.ЕстьЗаписиВТаблице(Документ, "ТоварыКОформлениюЗаявленийОВвозеИзменение") Тогда
		...
		ШаблонСообщенияБезПоступления = НСтр("ru = 'Номенклатура %1
			...
			|en = 'Products %1
			|Quantity of goods to be registered is not equal to quantity in the receipt document by %2 %3'");
		ШаблонСообщенияСПоступлением = ШаблонСообщенияБезПоступления + " " + НСтр("ru = '(см. документ %4)';
																					|en = '(see the document %4)'");
