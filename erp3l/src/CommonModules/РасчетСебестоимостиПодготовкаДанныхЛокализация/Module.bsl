...
Процедура ИсправитьПроблемыВДвиженияхИДокументах(ПараметрыРасчета) Экспорт
	...
	РасшифровкаКодовОшибок.Вставить(1, НСтр("ru = 'Для движения не найдено соответствующее ему правило заполнения типа записи';
											|en = 'A corresponding record type filling rule was not found for the register record'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(2, НСтр("ru = 'Тип записи приходного движения не соответствует правилу';
											|en = 'Receipt movement record type does not correspond to the rule'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(3, НСтр("ru = 'Тип записи расходного движения не соответствует правилу';
											|en = 'Expense movement record type does not correspond to the rule'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	РасшифровкаКодовОшибок.Вставить(6, НСтр("ru = 'Для организаций с методом оценки ""ФИФО (скользящая)"" некорректно заполнена кор. партия в расходном движении';
											|en = 'Cor. lot in expense movement is filled in incorrectly for companies with the ""FIFO (rolling valuation)"" estimation method'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(18, НСтр("ru = 'Заполнение документа-источника в приходном движении не соответствует правилу.';
											|en = 'Source document population in the receipt movement does not match the rule.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	РасшифровкаКодовОшибок.Вставить(19, НСтр("ru = 'Заполнение документа-источника в расходном движении не соответствует правилу.';
											|en = 'Source document population in expense movement does not correspond to the rule.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	|		ИЛИ (Т.НДС <> 0 И Т.НДСУпр = 0 И ВЫРАЗИТЬ(Т.НДС * Курсы.КоэффициентВалютыУпр КАК ЧИСЛО(31,2)) <> 0))
	...
	РасшифровкаКодовОшибок.Вставить(1, НСтр("ru = 'Отражение статей с вариантом распределения НаПроизводственныеЗатраты в регистре ПартииПрочихРасходов не соответствует учетной политике организации';
											|en = 'Recording of items with the НаПроизводственныеЗатраты allocation option in the ПартииПрочихРасходов register does not comply with the company accounting policy'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.ПерепровестиДокументыПоОтдельнымРегистрамНакопления(
		...
		НСтр("ru = 'некорректные движения по регистру ""Партии прочих расходов""';
			|en = 'incorrect records in the ""Batches of other expenses"" register'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура КорректировкаНачальныхОстатковРегистровДетализацииПартийНДС(ПараметрыРасчета)
	...
	ПараметрыКорректировкиНДСФИФО = РасчетСебестоимостиПрикладныеАлгоритмы.ИнициализироватьКорректировкуОстатковРегистра(
		ПараметрыРасчета,
		Метаданные.РегистрыСведений.ДетализацияСебестоимостиТоваров.ПолноеИмя());
	
	ПараметрыКорректировкиНДСФИФОСтарый = РасчетСебестоимостиПрикладныеАлгоритмы.ИнициализироватьКорректировкуОстатковРегистра(
		ПараметрыРасчета,
	...
	Если НЕ ПараметрыРасчета.ФормироватьНачальныеОстаткиПартий22 Тогда
		...
		КорректировкаПереходНаНовыйРегистрДетализацияСебестоимостиТоваров(ПараметрыРасчета, ПараметрыКорректировкиНДСФИФО);
	...
	Если РасчетСебестоимостиПовтИсп.СебестоимостьТоваровПоНазначениям(ПараметрыРасчета.РасчетныйПериод.НачалоПериода) Тогда
		Если РасчетСебестоимостиПовтИсп.ДатаВключенияОбособленногоУчетаСебестоимостиПоНазначениям() = ПараметрыРасчета.РасчетныйПериод.НачалоПериода Тогда
			КорректировкаВключитьУчетПоНазначениямДетализацияСебестоимостиТоваров(ПараметрыРасчета, ПараметрыКорректировкиНДСФИФО);
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.ЗавершитьКорректировкуОстатковРегистра(ПараметрыРасчета, ПараметрыКорректировкиНДСФИФОСтарый); // для очистки прошлых корректировок
...
Процедура СформироватьНачальныеОстаткиПартийНДСиУСН(ПараметрыРасчета, ПараметрыКорректировкиНДССредняя, ПараметрыКорректировкиНДСФИФО)
	...
	|	ВЫРАЗИТЬ(Т.СтоимостьБезНДС / ОстаткиПоКоличеству.Количество КАК ЧИСЛО (31,10)) КАК СтоимостьБезНДС,
	|	ВЫРАЗИТЬ(Т.НДС / ОстаткиПоКоличеству.Количество КАК ЧИСЛО (31,10)) КАК НДС,
	|	ВЫРАЗИТЬ(Т.СтоимостьБезНДСУпр / ОстаткиПоКоличеству.Количество КАК ЧИСЛО (31,10)) КАК СтоимостьБезНДСУпр,
	|	ВЫРАЗИТЬ(Т.НДСУпр / ОстаткиПоКоличеству.Количество КАК ЧИСЛО (31,10)) КАК НДСУпр
...
Процедура КорректировкаПереходНаНовыйРегистрДетализацияСебестоимостиТоваров(ПараметрыРасчета, ПараметрыКорректировки)
	...
	|	ВЫРАЗИТЬ(Т.СтоимостьБезНДС / Т.КоличествоСебестоимость КАК ЧИСЛО(31, 10)) КАК СтоимостьБезНДС,
	|	ВЫРАЗИТЬ(Т.НДС / Т.КоличествоСебестоимость КАК ЧИСЛО(31, 10)) КАК НДС,
	|	ВЫРАЗИТЬ(Т.СтоимостьБезНДСУпр / Т.КоличествоСебестоимость КАК ЧИСЛО(31, 10)) КАК СтоимостьБезНДСУпр,
	|	ВЫРАЗИТЬ(Т.НДСУпр / Т.КоличествоСебестоимость КАК ЧИСЛО(31, 10)) КАК НДСУпр
	|ПОМЕСТИТЬ ВТОстаткиДетализацииПартий
	|ИЗ
...
// Выполняет включение учета по назначениям - заполняет назначения в аналитике номенклатуры.
//
Процедура КорректировкаВключитьУчетПоНазначениямДетализацияСебестоимостиТоваров(ПараметрыРасчета, ПараметрыКорректировкиНДСФИФО)
	...
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "%ИзмеренияРегистра",
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			РасчетСебестоимостиУниверсальныеАлгоритмы.УдалитьЭлементыИзСтрокиШаблона(
				ПараметрыКорректировкиНДСФИФО.ОписаниеРегистра.ИзмеренияРегистра,
				"%1ТипЗаписи"),
			"Т."));
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "%РесурсыРегистра",
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ПараметрыКорректировкиНДСФИФО.ОписаниеРегистра.РесурсыРегистра, "Т."));
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "%ПоляАналитикиИменованные",
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ОписаниеАналитики.ПоляИменованные, "Т."));
	...
	РаспределяемыеРесурсы = РасчетСебестоимостиУниверсальныеАлгоритмы.УдалитьЭлементыИзСтрокиШаблона(
		ПараметрыКорректировкиНДСФИФО.ОписаниеРегистра.РесурсыРегистра,
		"%1КоличествоПартии");
	...
	РасчетСебестоимостиПрикладныеАлгоритмы.РаспределитьМетодомУменьшаемогоОстаткаСЗамеромВремени(
		...
		ПараметрыРаспределения,
		"РаспределитьМетодомУменьшаемогоОстатка",
		"КорректировкаВключитьУчетПоНазначениямДетализацияСебестоимостиТоваров");
...
Функция ТекстЗапросаСтатьиНаПроизводственныеЗатратыВРегистреПартииПрочихРасходов() Экспорт
	Возврат
		...
		|		Т.Период МЕЖДУ &НачалоПериода И &КонецПериода
		|		И Т.Организация В(&МассивОрганизаций)
		|		И (Статья.ВариантРаспределенияРасходовУпр В (
