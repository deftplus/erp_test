...
Процедура ОбновитьСостояниеЗаявленияНаРегистрациюЮЛ(
	...
	Если ТокенАвторизации = Неопределено И ЗначениеЗаполнено(СвойстваОтправки.ОтпечатокСертификата) Тогда
	...
	ИначеЕсли ТокенАвторизации = Неопределено Тогда
...
Процедура СоздатьМенеджерКриптографии(Оповещение, Криптопровайдер = Неопределено, ИнтерактивныйРежим = Ложь) Экспорт
	...
	ПараметрыЦикла.Вставить("ЭтоLinux", ОбщегоНазначенияКлиент.ЭтоLinuxКлиент());
...
Процедура АвторизацияПользователяЛокальныеСертификаты(СертификатыХранилища, ПараметрыЦикла) Экспорт
	...
	Если СертификатыХранилища <> Неопределено Тогда
		...
		ПараметрыЦикла.МассивВыбора = АвторизацияПользователяПодготовитьМассивСертификатов(СертификатыХранилища, ОтпечатокСертификата);
		...
		ОповещениеСледующее = Новый ОписаниеОповещения("АвторизацияПользователяВыбратьСертификат", ЭтотОбъект, ПараметрыЦикла);
		ОбработкаОшибкиОповещения(ОповещениеСледующее, Новый ОписаниеОповещения("АвторизацияПользователяОшибка", ЭтотОбъект));
		Если ЭлектроннаяПодписьВМоделиСервисаКлиентСервер.ИспользованиеВозможно() Тогда
			ХранилищеСертификатовКлиент.Получить(ОповещениеСледующее, "ПерсональныеСертификаты");
		Иначе
			РезультатВызова = Новый Структура;
			РезультатВызова.Вставить("Выполнено", Истина);
			РезультатВызова.Вставить("Сертификаты", Новый Массив);
			ВыполнитьОбработкуОповещения(ОповещениеСледующее, РезультатВызова);
		КонецЕсли;	
...
// Продолжение АвторизацияПользователя
Процедура АвторизацияПользователяВыбратьСертификат(РезультатВызова, ПараметрыЦикла) Экспорт
	...
	Если РезультатВызова.Выполнено Тогда
		...
		МассивВыбора = АвторизацияПользователяПодготовитьМассивСертификатов(РезультатВызова.Сертификаты, ОтпечатокСертификата, ПараметрыЦикла.МассивВыбора);
		Для Каждого СтрокаМассива Из РезультатВызова.Сертификаты Цикл
			ПараметрыЦикла.МассивСертификатов.Вставить(ПолучитьHexСтрокуИзДвоичныхДанных(СтрокаМассива.Отпечаток), СтрокаМассива);
		КонецЦикла;	
		
		Если МассивВыбора.Количество() = 1 
			И МассивВыбора[0].Отпечаток = ОтпечатокСертификата
...
Процедура АвторизацияПользователяПослеВыбораСертификата(РезультатВызова, ПараметрыЦикла) Экспорт
	...
	Иначе
		...
		ИначеЕсли ОбщегоНазначенияКлиентСервер.СвойствоСтруктуры(НашлиСертификат, "ЭтоЭлектроннаяПодписьВМоделиСервиса", Ложь) Тогда
			Сертификат.Сертификат = НашлиСертификат.ДвДанные;
			ОповещениеСледующее = Новый ОписаниеОповещения("АвторизацияПользователяПослеСозданияСертификата", ЭтотОбъект, ПараметрыЦикла);
			ОбработкаОшибкиОповещения(ОповещениеСледующее, Новый ОписаниеОповещения("АвторизацияПользователяОшибка", ЭтотОбъект));
			НовыйСертификат = Новый СертификатКриптографии;
...
Процедура АвторизацияПользователяПослеПолученияВременногоТокена(РезультатВыполнения, ПараметрыЦикла) Экспорт
	...
	Если РезультатВыполнения.Выполнено Тогда
		...
		Если Сертификат.ЭтоЭлектроннаяПодписьВМоделиСервиса Тогда
		...
		Иначе
			...
			Если Криптопровайдер = Неопределено Тогда
				АвторизацияПользователяРезультат(
					НСтр("ru = 'Не удалось определить криптопровайдер в сертификате';
						|en = 'Не удалось определить криптопровайдер в сертификате'"), 
					ПараметрыЦикла);
			Иначе
				ОповещениеСледующее = Новый ОписаниеОповещения("АвторизацияПользователяПолучениеМенеджера", ЭтотОбъект, ПараметрыЦикла);
				ОбработкаОшибкиОповещения(ОповещениеСледующее, Новый ОписаниеОповещения("АвторизацияПользователяОшибка", ЭтотОбъект));
				СоздатьМенеджерКриптографии(ОповещениеСледующее, Криптопровайдер, ИнтерактивныйРежим);
			КонецЕсли;
		КонецЕсли;
...
// Продолжение АвторизацияПользователя
Процедура АвторизацияПользователяПолучениеМенеджера(РезультатВызова, ПараметрыЦикла) Экспорт
	...
	Если РезультатВызова.Выполнено Тогда
		...
		АвторизацияПользователяНачатьПодписание(ПараметрыЦикла);
	Иначе
		АвторизацияПользователяРезультат(НСтр("ru = 'Ошибка создания менеджера криптографии.';
												|en = 'Ошибка создания менеджера криптографии.'"), ПараметрыЦикла);
	КонецЕсли;
...
// Служебное для АвторизацияПользователя
Функция АвторизацияПользователяПодготовитьМассивСертификатов(СписокСертификатов, ОтпечатокСертификата = Неопределено, ДополнитьМассив = Неопределено)
	...
	Для Каждого СтрокаМассива Из СписокСертификатов Цикл
		Просрочен = 
			НачалоДня(ТекДата) < НачалоДня(СтрокаМассива.ДатаНачала)
			ИЛИ НачалоДня(ТекДата) > НачалоДня(СтрокаМассива.ДатаОкончания)
		...
		НоваяСтрока.ДействителенС = СтрокаМассива.ДатаНачала;
		НоваяСтрока.ДействителенПо = СтрокаМассива.ДатаОкончания;
		НоваяСтрока.Отпечаток = ПолучитьHexСтрокуИзДвоичныхДанных(СтрокаМассива.Отпечаток);
		
		Если ТипЗнч(СтрокаМассива) <> Тип("СертификатКриптографии") Тогда
			НоваяСтрока.ЭтоЭлектроннаяПодписьВМоделиСервиса = Истина;
		КонецЕсли;
		...
		Если ВРег(ОтпечатокСертификата) = НоваяСтрока.Отпечаток Тогда
	...
	Для Каждого СтрокаМассива Из МассивСертификатов Цикл
		Если ВРег(ОтпечатокСертификата) = СтрокаМассива.Отпечаток Тогда
