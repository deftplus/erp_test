...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполнение пустых значений мультиязычных реквизитов значением основного языка.';
									|en = 'Filling in empty values of multi-language attributes with the main language value.'");
	...
	Если ОбъектовОбработано = 0 И ПроблемныхОбъектов <> 0 Тогда
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Процедуре МультиязычностьСервер.ОбработатьДанныеДляПереходаНаНовуюВерсию не удалось обработать некоторые объекты (пропущены): %1';
				|en = 'Procedure МультиязычностьСервер.ОбработатьДанныеДляПереходаНаНовуюВерсию failed to process (skipped) some objects: %1'"), 
			ПроблемныхОбъектов);
	...
	Иначе
		ЗаписьЖурналаРегистрации(ОбновлениеИнформационнойБазы.СобытиеЖурналаРегистрации(), УровеньЖурналаРегистрации.Информация,
			...
			НСтр("ru = 'Процедура МультиязычностьСервер.ОбработатьДанныеДляПереходаНаНовуюВерсию обработала очередную порцию объектов: %1';
				|en = 'The NationalLanguageSupportServer.ProcessDataForMigrationToNewVersion procedure has processed objects: %1'"),
			ОбъектовОбработано));
	КонецЕсли;
