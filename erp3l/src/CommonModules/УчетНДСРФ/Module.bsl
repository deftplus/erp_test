...
#Область Установить_ИспользоватьВвозТоваровИзТаможенногоСоюза
	...
	Обработчик.Комментарий = НСтр("ru = 'Установка константу ИспользоватьВвозТоваровИзТаможенногоСоюза';
									|en = 'Setting a constant ИспользоватьВвозТоваровИзТаможенногоСоюза'");
...
#Область Установить_ИспользоватьПродажиНаЭкспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Установка константу ИспользоватьПродажиНаЭкспорт';
									|en = 'Setting a constant ИспользоватьПродажиНаЭкспорт'");
...
#Область Установить_ИспользоватьРазделыТаможеннойДекларацииИмпорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Установка константу ИспользоватьРазделыТаможеннойДекларацииИмпорт';
									|en = 'Setting a constant ИспользоватьРазделыТаможеннойДекларацииИмпорт'");
...
#Область ОбработатьДанныеДляПереходаНаНовуюВерсиюСФиВЭД
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Процедура = "УчетНДСРФ.ОбработатьДанныеДляПереходаНаНовуюВерсиюСФиВЭД";
	Обработчик.Версия = "2.5.7.112";
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("8d5319ed-22b4-4905-8e0f-dea3c5fa5b7f");
	Обработчик.Многопоточный = Истина;
	Обработчик.ПроцедураЗаполненияДанныхОбновления = "УчетНДСРФ.ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсиюСФиВЭД";
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет регистр ""Реестр документов"" по данным относящихся к нему документов (только счета-фактуры).';
									|en = 'Fills in the ""Document registry"" register according to the related document data (the tax invoice only).'");
	...
	Иначе
		...
		ТекстПояснения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Требуется оформление документов по учету НДС (%1 шт.)';
				|en = 'Register VAT accounting documents (%1 pcs)'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсиюСФиВЭД(Параметры) Экспорт
	...
	РегистрыСведений.РеестрДокументов.ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры);
		...
		Исключение
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Ручная корректировка журнала СФ';
					|en = 'Manual tax invoice register adjustment'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ДополнитьИнформациейОРНПТ(Знач ДополнительнаяИнформация, Знач СтруктураПараметров, Запрос, ПоискРНПТПоОснованию = Ложь) Экспорт
	...
	ДополнитьЗапросИнформациейОРозничныхПродажах(Запрос);
	...
	Для каждого Организация Из СтруктураПараметров.СписокОрганизаций Цикл
		СтрокаТаблицы = ТаблицаБазовыхВалют.Добавить();
		СтрокаТаблицы.Организация = Организация.Значение;
		СтрокаТаблицы.БазоваяВалюта = ЗначениеНастроекПовтИсп.ВалютаРегламентированногоУчетаОрганизации(Организация.Значение);
	КонецЦикла;
	
	Запрос.УстановитьПараметр("ТаблицаБазовыхВалют",  ТаблицаБазовыхВалют);
...
Функция ТекстЗапросаДанныеОРНПТДляОтчетности(ПоискРНПТПоОснованию)
	...
	|	ТоварыОрганизаций.НомерГТД.Код КАК РегНомПросл,
	...
	|		ПО Операции.Регистратор = СписокСчетовФактур.СчетФактура
	...
	|	И НЕ ТИПЗНАЧЕНИЯ(СписокСчетовФактур.СчетФактура) = ТИП(Документ.КорректировкаПриобретения)
	|	И НЕ ТИПЗНАЧЕНИЯ(СписокСчетовФактур.СчетФактура) = ТИП(Документ.КорректировкаРеализации)
	...
	|
	|ВЫБРАТЬ
	|	ВТ_РозничныеПродажи.СчетФактура,
	|	Операции.Организация,
	|	ТоварыОрганизаций.НомерГТД.Код,
	|	ТоварыОрганизаций.АналитикаУчетаНоменклатуры.Номенклатура.КодТНВЭД.ЕдиницаИзмерения.Код,
	|	ТоварыОрганизаций.АналитикаУчетаНоменклатуры,
	|	ТоварыОрганизаций.ВидЗапасов,
	|	1,
	|	СУММА(ТоварыОрганизаций.КоличествоПоРНПТ)
	|ИЗ
	|	РегистрСведений.ОперацииСПрослеживаемымиТоварами КАК Операции
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_РозничныеПродажи КАК ВТ_РозничныеПродажи
	|		ПО Операции.Регистратор = ВТ_РозничныеПродажи.СчетФактура
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрНакопления.ТоварыОрганизаций КАК ТоварыОрганизаций
	|		ПО Операции.Регистратор = ТоварыОрганизаций.Регистратор
	|		И Операции.Организация = ТоварыОрганизаций.Организация
	|		И Операции.ТипЗапасов = ТоварыОрганизаций.ВидЗапасов.ТипЗапасов
	...
	|	И (СебестоимостьТоваров.Регистратор, СебестоимостьТоваров.Организация,
	|		СебестоимостьТоваров.АналитикаУчетаНоменклатуры, СебестоимостьТоваров.ВидЗапасов) В
	|		(ВЫБРАТЬ РАЗЛИЧНЫЕ
	|			ВТ_ПрослеживаемыеОперации.СчетФактура,
	...
	|	И (ВыручкаИСебестоимостьПродаж.Регистратор, ВыручкаИСебестоимостьПродаж.АналитикаУчетаНоменклатуры,
	|		ВыручкаИСебестоимостьПродаж.ВидЗапасов) В
	|		(ВЫБРАТЬ РАЗЛИЧНЫЕ
	|			ВТ_ПрослеживаемыеОперации.СчетФактура,
	|			ВТ_ПрослеживаемыеОперации.АналитикаУчетаНоменклатуры,
	|			ВТ_ПрослеживаемыеОперации.ВидЗапасов
	...
	|	И (ТоварыКОформлениюДокументовИмпорта.Регистратор, ТоварыКОформлениюДокументовИмпорта.АналитикаУчетаНоменклатуры,
	|		ТоварыКОформлениюДокументовИмпорта.ВидЗапасов) В
	|		(ВЫБРАТЬ РАЗЛИЧНЫЕ
	|			ВТ_ПрослеживаемыеОперации.СчетФактура,
	|			ВТ_ПрослеживаемыеОперации.АналитикаУчетаНоменклатуры,
	|			ВТ_ПрослеживаемыеОперации.ВидЗапасов
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ПрослеживаемыеОперации.СчетФактура КАК СчетФактура,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ПрослеживаемыеОперации.СчетФактура КАК СчетФактура,
	|	ПрослеживаемыеОперации.РегНомПросл КАК РегНомПросл,
	|	ПрослеживаемыеОперации.ОКЕИ КАК ОКЕИ,
	|	ПрослеживаемыеОперации.КоличествоЭлементов КАК КоличествоЭлементов,
	|	ВЫБОР
	...
	|				ВТ_КоличествоТоваров.Количество * ПрослеживаемыеОперации.Количество
	...
	|		ПО ПрослеживаемыеОперации.СчетФактура = СебестоимостьТоваров.Регистратор
	...
	|		ПО ПрослеживаемыеОперации.СчетФактура = ВыручкаИСебестоимостьПродаж.Регистратор
	...
	|		ПО ПрослеживаемыеОперации.СчетФактура = ВТ_КоличествоТоваров.СчетФактура
	...
	|ГДЕ
	|	НЕ ТИПЗНАЧЕНИЯ(ПрослеживаемыеОперации.СчетФактура) = ТИП(Документ.ТаможеннаяДекларацияИмпорт)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	|	ПрослеживаемыеОперации.СчетФактура,
	|	ПрослеживаемыеОперации.РегНомПросл,
	|	ПрослеживаемыеОперации.ОКЕИ,
	|	ПрослеживаемыеОперации.КоличествоЭлементов,
	|	ВЫБОР
	|		КОГДА ПрослеживаемыеОперации.Количество >= 0
	|			ТОГДА ПрослеживаемыеОперации.Количество
	|		ИНАЧЕ -ПрослеживаемыеОперации.Количество
	...
	|			ТОГДА ЕСТЬNULL(ВТ_ТоварыКОформлениюДокументовИмпорта.Сумма, 0) / ВТ_КоличествоТоваров.Количество *
	|				ПрослеживаемыеОперации.Количество
	|		ИНАЧЕ -ВТ_ТоварыКОформлениюДокументовИмпорта.Сумма / ВТ_КоличествоТоваров.Количество *
	|			ПрослеживаемыеОперации.Количество
	|	КОНЕЦ
	|ИЗ
	|	ВТ_ПрослеживаемыеОперации КАК ПрослеживаемыеОперации
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_ТоварыКОформлениюДокументовИмпорта КАК ВТ_ТоварыКОформлениюДокументовИмпорта
	|		ПО ПрослеживаемыеОперации.СчетФактура = ВТ_ТоварыКОформлениюДокументовИмпорта.Регистратор
	|		И
	|			ПрослеживаемыеОперации.АналитикаУчетаНоменклатуры = ВТ_ТоварыКОформлениюДокументовИмпорта.АналитикаУчетаНоменклатуры
	...
	|		И ПрослеживаемыеОперации.ВидЗапасов = ВТ_КоличествоТоваров.ВидЗапасов
	|ГДЕ
	|	ТИПЗНАЧЕНИЯ(ПрослеживаемыеОперации.СчетФактура) = ТИП(Документ.ТаможеннаяДекларацияИмпорт)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|		ПО СписокСчетовФактур.СчетФактура = КорректировкаПриобретения.Ссылка
	...
	|ГДЕ
	|	ТИПЗНАЧЕНИЯ(СписокСчетовФактур.СчетФактура) = ТИП(Документ.КорректировкаПриобретения)
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_КорректировкиПриобретения_Предварительная.СчетФактура КАК СчетФактура,
	...
	|	КорректировкаПриобретенияТовары.Цена * ЕСТЬNULL(КурсыВалют.КурсЧислитель, 0) * ЕСТЬNULL(КурсыВалют.КурсЗнаменатель,
	|		0) * КорректировкаПриобретенияТовары.КоличествоПоРНПТ КАК СтоимТовПросл
	...
	|		ПО ВТ_КорректировкиПриобретения_Предварительная.СчетФактура = КорректировкаПриобретенияТовары.Ссылка
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ОтносительныеКурсыВалют КАК КурсыВалют
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	КорректировкиРеализации.СчетФактура КАК СчетФактура,
	|	КорректировкиРеализации.РегНомПросл КАК РегНомПросл,
	|	КорректировкиРеализации.ОКЕИ КАК ОКЕИ,
	|	СУММА(КорректировкиРеализации.КолТовПросл) КАК КолТовПросл,
	|	СУММА(КорректировкиРеализации.СтоимТовПросл) КАК СтоимТовПросл,
	|	1 КАК КоличествоЭлементов
	|ПОМЕСТИТЬ ВТ_КорректировкиРеализации
	|ИЗ
	|	(ВЫБРАТЬ
	|		СписокСчетовФактур.СчетФактура КАК СчетФактура,
	|		КорректировкаРеализацииВидыЗапасовКорректировкаВыручки.НомерГТД КАК РегНомПросл,
	|		КорректировкаРеализацииВидыЗапасовКорректировкаВыручки.КодТНВЭД.ЕдиницаИзмерения.Код КАК ОКЕИ,
	|		0 КАК КолТовПросл,
	|		-СуммыДокументовВВалютахУчета.СуммаБезНДС КАК СтоимТовПросл
	|	ИЗ
	|		РегистрСведений.ОперацииСПрослеживаемымиТоварами КАК Операции
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ СписокСчетовФактур КАК СписокСчетовФактур
	|			ПО Операции.Регистратор = СписокСчетовФактур.СчетФактура
	|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.КорректировкаРеализации.ВидыЗапасовКорректировкаВыручки КАК
	...
	|				ПО (СуммыДокументовВВалютахУчета.Регистратор = КорректировкаРеализацииВидыЗапасовКорректировкаВыручки.Ссылка)
	|				И
	|					(СуммыДокументовВВалютахУчета.ИдентификаторСтроки = КорректировкаРеализацииВидыЗапасовКорректировкаВыручки.ИдентификаторСтроки)
	|			ПО СписокСчетовФактур.СчетФактура = КорректировкаРеализацииВидыЗапасовКорректировкаВыручки.Ссылка
	|	ГДЕ
	|		Операции.ОтражениеВОтчетности = &ОтражениеВОтчетности
	|		И Операции.Организация В (&Организация)
	|		И Операции.Активность
	|		И ТИПЗНАЧЕНИЯ(СписокСчетовФактур.СчетФактура) = ТИП(Документ.КорректировкаРеализации)
	|
	|	ОБЪЕДИНИТЬ ВСЕ
	|
	|	ВЫБРАТЬ
	...
	|		КорректировкаРеализацииВидыЗапасовОприходование.КодТНВЭД.ЕдиницаИзмерения.Код,
	|		КорректировкаРеализацииВидыЗапасовОприходование.КоличествоПоРНПТ,
	|		-СуммыДокументовВВалютахУчета.СуммаБезНДС
	|	ИЗ
	|		РегистрСведений.ОперацииСПрослеживаемымиТоварами КАК Операции
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ СписокСчетовФактур КАК СписокСчетовФактур
	|			ПО Операции.Регистратор = СписокСчетовФактур.СчетФактура
	|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.КорректировкаРеализации.ВидыЗапасовОприходование КАК
	|				КорректировкаРеализацииВидыЗапасовОприходование
	|				ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СуммыДокументовВВалютахУчета КАК СуммыДокументовВВалютахУчета
	|				ПО (СуммыДокументовВВалютахУчета.Регистратор = КорректировкаРеализацииВидыЗапасовОприходование.Ссылка)
	|				И
	|					(СуммыДокументовВВалютахУчета.ИдентификаторСтроки = КорректировкаРеализацииВидыЗапасовОприходование.ИдентификаторСтроки)
	|			ПО СписокСчетовФактур.СчетФактура = КорректировкаРеализацииВидыЗапасовОприходование.Ссылка
	|	ГДЕ
	|		Операции.ОтражениеВОтчетности = &ОтражениеВОтчетности
	|		И Операции.Организация В (&Организация)
	|		И Операции.Активность
	...
	|		КорректировкаРеализацииВидыЗапасовСписание.КодТНВЭД.ЕдиницаИзмерения.Код,
	|		КорректировкаРеализацииВидыЗапасовСписание.КоличествоПоРНПТ,
	|		-СуммыДокументовВВалютахУчета.СуммаБезНДС
	|	ИЗ
	|		РегистрСведений.ОперацииСПрослеживаемымиТоварами КАК Операции
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ СписокСчетовФактур КАК СписокСчетовФактур
	|			ПО Операции.Регистратор = СписокСчетовФактур.СчетФактура
	|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.КорректировкаРеализации.ВидыЗапасовСписание КАК
	|				КорректировкаРеализацииВидыЗапасовСписание
	|				ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СуммыДокументовВВалютахУчета КАК СуммыДокументовВВалютахУчета
	|				ПО (СуммыДокументовВВалютахУчета.Регистратор = КорректировкаРеализацииВидыЗапасовСписание.Ссылка)
	|				И
	|					(СуммыДокументовВВалютахУчета.ИдентификаторСтроки = КорректировкаРеализацииВидыЗапасовСписание.ИдентификаторСтроки)
	|			ПО СписокСчетовФактур.СчетФактура = КорректировкаРеализацииВидыЗапасовСписание.Ссылка
	|	ГДЕ
	|		Операции.ОтражениеВОтчетности = &ОтражениеВОтчетности
	|		И Операции.Организация В (&Организация)
	|		И Операции.Активность
	|		И ТИПЗНАЧЕНИЯ(СписокСчетовФактур.СчетФактура) = ТИП(Документ.КорректировкаРеализации)) КАК КорректировкиРеализации
	|СГРУППИРОВАТЬ ПО
	|	КорректировкиРеализации.СчетФактура,
	|	КорректировкиРеализации.РегНомПросл,
	|	КорректировкиРеализации.ОКЕИ
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|	РегистрСведений.ОперацииСПрослеживаемымиТоварами КАК Операции
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ СписокСчетовФактур КАК СписокСчетовФактур
	|		ПО (Операции.Регистратор = ВЫРАЗИТЬ(СписокСчетовФактур.СчетФактура КАК
	|			Документ.СчетФактураВыданный).ДокументОснование)
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.СчетФактураВыданный.Товары КАК СчетФактураВыданныйТовары
	|		ПО (СписокСчетовФактур.СчетФактура = СчетФактураВыданныйТовары.Ссылка)
	|ГДЕ
	|	Операции.ОтражениеВОтчетности = &ОтражениеВОтчетности
	|	И Операции.Организация В (&Организация)
	|	И Операции.Активность
	...
	|	ПрослеживаемыеТоварыВСоставеОС.РНПТ.Код КАК РегНомПросл,
	...
	|	ПрослеживаемыеТоварыВСоставеОС.РНПТ.Код,
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Документ.РеализацияУслугПрочихАктивов.ПрослеживаемыеТовары КАК
	|			РеализацияУслугПрочихАктивовПрослеживаемыеТовары
	|		ПО ВТ_РНПТвОС.СчетФактура = РеализацияУслугПрочихАктивовПрослеживаемыеТовары.Ссылка
	|		И ВТ_РНПТвОС.РНПТ = РеализацияУслугПрочихАктивовПрослеживаемыеТовары.НомерГТД
	...
	|		И
	|			(РеализацияУслугПрочихАктивовПрослеживаемыеТовары.ОсновноеСредство = РеализацияУслугПрочихАктивовДоходы.АналитикаДоходов)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	СчетФактура
	|;
	...
	|	ВТ_РУиПА_Предварительная.КоличествоЭлементов КАК КоличествоЭлементов,
	...
	|		ПО ВТ_РНПТвОС.СчетФактура = ПриобретениеУслугПрочихАктивовРасходы.Ссылка
	|		И ВТ_РНПТвОС.РНПТ = ПриобретениеУслугПрочихАктивовРасходы.НомерГТД
	...
	|	ВыкупАрендованныхОСОС.Сумма КАК Сумма
	...
	|		ПО ВТ_РНПТвОС.СчетФактура = ВыкупАрендованныхОСПрослеживаемыеТовары.Ссылка
	|		И ВТ_РНПТвОС.РНПТ = ВыкупАрендованныхОСПрослеживаемыеТовары.НомерГТД
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ВыкупАрендованныхОС_Предварительная.СчетФактура КАК СчетФактура,
	...
	|СГРУППИРОВАТЬ ПО
	|	ВТ_ВыкупАрендованныхОС_Предварительная.СчетФактура,
	...
	|	ВТ_ВыкупАрендованныхОС_Предварительная.КоличествоЭлементов КАК КоличествоЭлементов,
	...
	|			ТОГДА 0
	|		ИНАЧЕ ВТ_ВыкупАрендованныхОС_Предварительная.Сумма / ВТ_ВыкупАрендованныхОС_БазаРаспределения.Количество *
	|			ВТ_ВыкупАрендованныхОС_Предварительная.Количество
	|	КОНЕЦ КАК СтоимТовПросл
	...
	|		ПО ВТ_ВыкупАрендованныхОС_Предварительная.СчетФактура = ВТ_ВыкупАрендованныхОС_БазаРаспределения.СчетФактура
	|		И
	|			ВТ_ВыкупАрендованныхОС_Предварительная.ОсновноеСредство = ВТ_ВыкупАрендованныхОС_БазаРаспределения.ОсновноеСредство
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	вт_ПокупкиПродажи.СчетФактура КАК СчетФактура,
	|	вт_ПокупкиПродажи.РегНомПросл КАК РегНомПросл,
	|	вт_ПокупкиПродажи.ОКЕИ КАК ОКЕИ,
	|	вт_ПокупкиПродажи.КоличествоЭлементов КАК КоличествоЭлементов,
	|	вт_ПокупкиПродажи.КолТовПросл КАК КолТовПросл,
	|	ВЫРАЗИТЬ(вт_ПокупкиПродажи.СтоимТовПросл КАК ЧИСЛО(15, 0)) КАК СтоимТовПросл
	|ИЗ
	...
	|
	|ВЫБРАТЬ
	|	ВТ_КорректировкиПриобретения.СчетФактура,
	|	ВТ_КорректировкиПриобретения.РегНомПросл,
	|	ВТ_КорректировкиПриобретения.ОКЕИ,
	|	ВТ_КорректировкиПриобретения.КоличествоЭлементов,
	...
	|
	|ВЫБРАТЬ
	|	ВТ_КорректировкиРеализации.СчетФактура,
	|	ВТ_КорректировкиРеализации.РегНомПросл,
	|	ВТ_КорректировкиРеализации.ОКЕИ,
	|	ВТ_КорректировкиРеализации.КоличествоЭлементов,
	...
	|
	|ВЫБРАТЬ
	|	ВТ_РУиПА.СчетФактура,
	|	ВТ_РУиПА.РегНомПросл,
	|	ВТ_РУиПА.ОКЕИ,
	|	ВТ_РУиПА.КоличествоЭлементов,
	...
	|
	|ВЫБРАТЬ
	|	ВТ_ПУиПА.СчетФактура,
	|	ВТ_ПУиПА.РегНомПросл,
	|	ВТ_ПУиПА.ОКЕИ,
	|	ВТ_ПУиПА.КоличествоЭлементов,
	...
	|
	|ВЫБРАТЬ
	|	ВТ_ВыкупАрендованныхОС.СчетФактура,
	|	ВТ_ВыкупАрендованныхОС.РегНомПросл,
	|	ВТ_ВыкупАрендованныхОС.ОКЕИ,
	|	ВТ_ВыкупАрендованныхОС.КоличествоЭлементов,
...
Процедура ДополнитьЗапросИнформациейОРозничныхПродажах(Запрос)
