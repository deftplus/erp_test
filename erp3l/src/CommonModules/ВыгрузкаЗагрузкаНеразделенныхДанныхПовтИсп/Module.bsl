					...
					Если ТипПоля = Тип("ХранилищеЗначения") Тогда
						...
						ВызватьИсключение СтрШаблон(
							НСтр("ru = 'Поле %1 не может использоваться в качестве поля естественного ключа объекта %2: использование
                                  ...
                                  |en = 'Cannot use the %1 field as a natural key field of %2 object:
                                  |using ValueStorage values as natural keys is not supported'", Метаданные.ОсновнойЯзык.КодЯзыка),
							ПолеЕстественногоКлюча,
							ТипОбщегоКлассификатора.ПолноеИмя());
					...
					Иначе
						...
						ВызватьИсключение СтрШаблон(
							НСтр("ru = 'Поле %1 не может использоваться в качестве поля естественного ключа объекта %2:
                                  ...
                                  |en = 'Cannot use the %1 field as a natural key field of %2 object:
                                  |the field type might be based on object %3, which is not included
                                  |in the common data set through overridable procedure
                                  |ExportImportDataOverridable.OnFillCommonDataTypesSupportingReferenceComparisonOnImport().'", Метаданные.ОсновнойЯзык.КодЯзыка),
							ПолеЕстественногоКлюча,
							ТипОбщегоКлассификатора.ПолноеИмя(),
	...
	Если Не ПриВыгрузке Тогда
		...
		ДополнениеОшибки = ДополнениеОшибки + Символы.ПС + СтрШаблон(
			НСтр("ru = 'Если корректное сопоставление ссылок на неразделенные данные в ИБ, из которой выгружены
                  ...
                  |en = 'If correct mapping of references to shared data in the infobase from which data and infobase are exported,
                  |and into which they are imported, is guaranteed by other tools, 
                  |specify metadata object %1 in procedure
                  | ExportImportDataOverridable.OnFillCommonDataTypesThatDoNotRequireMappingRefsOnImport().'", Метаданные.ОсновнойЯзык.КодЯзыка),
			МетаданныеСсылки.ПолноеИмя());
