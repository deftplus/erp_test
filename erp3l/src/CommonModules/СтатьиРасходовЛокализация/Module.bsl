...
Процедура ОбработкаПроверкиЗаполнения(Объект, Отказ, ПроверяемыеРеквизиты) Экспорт
	...
	Если Объект.ВариантРаспределенияРасходовРегл <> Перечисления.ВариантыРаспределенияРасходов.НаРасходыБудущихПериодов 
		ИЛИ НЕ Объект.ПринятиеКНалоговомуУчету Тогда 
...
Процедура ЗаполнитьРеквизитыПредопределеннойСтатьиРасходов(СтатьяРасходов) Экспорт
	...
	Если СтатьяРасходов.Ссылка = ПланыВидовХарактеристик.СтатьиРасходов.ТорговыйСбор Тогда
		СтатьяРасходов.ПринятиеКНалоговомуУчету = Ложь;
	Иначе
		СтатьяРасходов.ПринятиеКНалоговомуУчету = Истина;
	КонецЕсли;

	// ВидРасходов
	Если ВидРасходовИспользуется(СтатьяРасходов) Тогда
		Если СтатьяРасходов.Ссылка = ПланыВидовХарактеристик.СтатьиРасходов.ВыбытияТоваровВПрошлыхПериодах
		 ИЛИ СтатьяРасходов.Ссылка = ПланыВидовХарактеристик.СтатьиРасходов.СторноПроизводственныхЗатрат Тогда
			СтатьяРасходов.ВидРасходов = Перечисления.ВидыРасходовНУ.МатериальныеРасходы;
		
		ИначеЕсли СтатьяРасходов.Ссылка = ПланыВидовХарактеристик.СтатьиРасходов.ТорговыйСбор Тогда
			СтатьяРасходов.ВидРасходов = Перечисления.ВидыРасходовНУ.НеУчитываемыеВЦеляхНалогообложения;
		Иначе
			СтатьяРасходов.ВидРасходов = Перечисления.ВидыРасходовНУ.ПрочиеРасходы;
		КонецЕсли;
	Иначе
		СтатьяРасходов.ВидРасходов = Неопределено;
	КонецЕсли;
	
	// ВидПрочихДоходовИРасходов
	Если ВидПрочихРасходовИспользуется(СтатьяРасходов) Тогда
		
		Если СтатьяРасходов.Ссылка = ПланыВидовХарактеристик.СтатьиРасходов.КурсовыеРазницы Тогда
			СтатьяРасходов.ВидПрочихДоходовИРасходов = Перечисления.ВидыПрочихДоходовИРасходов.КурсовыеРазницы;
		
		ИначеЕсли СтатьяРасходов.Ссылка = ПланыВидовХарактеристик.СтатьиРасходов.РасходыПриКонвертацииВалюты Тогда
			СтатьяРасходов.ВидПрочихДоходовИРасходов = Перечисления.ВидыПрочихДоходовИРасходов.ПрочиеВнереализационныеДоходыРасходы;
		
		ИначеЕсли СтатьяРасходов.Ссылка = ПланыВидовХарактеристик.СтатьиРасходов.ВыбытияТоваровВПрошлыхПериодах Тогда
			СтатьяРасходов.ВидПрочихДоходовИРасходов = Перечисления.ВидыПрочихДоходовИРасходов.ПрочиеВнереализационныеДоходыРасходы;
		
		ИначеЕсли СтатьяРасходов.Ссылка = ПланыВидовХарактеристик.СтатьиРасходов.ПрибыльУбытокПрошлыхЛет Тогда
			СтатьяРасходов.ВидПрочихДоходовИРасходов = Перечисления.ВидыПрочихДоходовИРасходов.ПрибыльУбытокПрошлыхЛет;
			
		ИначеЕсли СтатьяРасходов.Ссылка = ПланыВидовХарактеристик.СтатьиРасходов.ФормированиеРезервовПоСомнительнымДолгам Тогда
			СтатьяРасходов.ВидПрочихДоходовИРасходов = Перечисления.ВидыПрочихДоходовИРасходов.ОтчисленияВОценочныеРезервы;
	...
	СтатьяРасходов.ВидДеятельностиДляНалоговогоУчетаЗатрат = Перечисления.ВидыДеятельностиДляНалоговогоУчетаЗатрат.ОсновнаяСистемаНалогообложения;
	//-- НЕ УТ
	//-- Локализация
...
Процедура УправлениеЭлементамиФормы(Форма) Экспорт
	...
	Элементы.ВидПрочихРасходов.Видимость = Объект.ПринятиеКНалоговомуУчету И ИспользуетсяВидПрочихРасходов;
	...
	Элементы.ГруппаРБП.Видимость      = НаРасходыБудущихПериодовРегл;
...
Процедура ВидимостьДоступностьНастроекРаспределения(Форма) Экспорт
	...
	Элементы.НадписьПравилоРаспределенияНУКакРегл.Видимость = 
		НЕ Объект.ВариантРаспределенияРасходовНУ.Пустая()
		И (НЕ Перечисления.ВариантыРаспределенияРасходов.ВариантИспользуетПравилоРаспределенияНУ(Объект.ВариантРаспределенияРасходовНУ)
			ИЛИ Объект.ВариантРаспределенияРасходовНУ = Перечисления.ВариантыРаспределенияРасходов.НаСебестоимостьТоваров)
		...
		И (НЕ ДоступнаНастройкаВариантаРаспределенияНУ
			ИЛИ Объект.ВариантРаспределенияРасходовНУ = Перечисления.ВариантыРаспределенияРасходов.НаСебестоимостьТоваров);
...
Функция ВидРасходовИспользуется(Объект)
	...
	Возврат (НаНаправленияДеятельности ИЛИ НеРаспределять ИЛИ НаПроизводственныеЗатраты ИЛИ НаВнеоборотныеАктивы ИЛИ НаСебестоимостьПроизводства)
