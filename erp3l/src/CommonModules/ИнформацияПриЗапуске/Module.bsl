...
Функция ПакетыСтраниц(НосительМакетов) Экспорт
	...
	Для НомерСтроки = 3 По ТабличныйДокумент.ВысотаТаблицы Цикл
		...
		Если НРег(СтрокаТаблицы.Раздел) = НРег(НСтр("ru = 'Реклама';
													|en = 'Advertising'")) Тогда // АПК:1391 Раздел локализуется.
	...
	Исключение
		ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Не удалось извлечь файл из макета %1 (ЛокализованноеИмяМакета: %2) обработки ИнформацияПриЗапуске по причине:';
				|en = 'Cannot retrieve the file from the %1 template (LocalizedTemplateName: %2) of the NotificationAtStartup data processor due to:'"),
