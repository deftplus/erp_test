...
Процедура ДобавитьИдентификаторыВоВременнуюТаблицу(ИмяТаблицы, МенеджерВременныхТаблиц, ИмяКолонки = "ИдентификаторСтроки") Экспорт
	...
	КоличествоДанныхТаблицы = РазмерВременнойТаблицы(МенеджерВременныхТаблиц, ИмяТаблицы);
	Для Счетчик = 1 По КоличествоДанныхТаблицы Цикл
		...
		ИначеЕсли КоличествоДокументов <= СтруктураПараметров.МаксимальноеКоличествоКомандВПодменю
			...
			Для Каждого СтрокаДокумент Из Дерево.Строки Цикл
				...
				Если СтрокаДокумент.Строки.Количество() = 1 Тогда
					...
					ДобавитьКомандуСКнопкой(Форма, ИмяГруппыСоздать, МестоРазмещенияКнопки, СтрокаДокумент.ДокументПредставление, СтрокаДокумент.Строки[0], Префикс);
				Иначе
					ГруппаКнопок = ДобавитьГруппуКнопок(Форма, ИмяГруппыСоздать, СтрокаДокумент.ИмяДокумента, СтрокаДокумент.ДокументПредставление, Префикс);
