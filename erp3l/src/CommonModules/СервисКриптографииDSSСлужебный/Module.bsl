...
Процедура ПриЗаполненииТаблицыПараметровИБ(Знач ТаблицаПараметров) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ТехнологияСервиса.БазоваяФункциональность") Тогда
		...
		СтрокаПараметра.Описание = НСтр("ru = 'Сервер электронной подписи DSS';
										|en = 'Server of digital DSS signature'", КодЯзыка());
...
Процедура ОбработкаДанныхОтменена(Знач Дескриптор) Экспорт
	...
	ПротоколОшибкиПриВызове(НСтр("ru = 'Обработка данных отменена.';
								|en = 'Data processing is canceled.'"), 
						"ПоставляемыеДанные", 
...
Функция СоздатьУчетнуюЗаписьОблачнойПодписи(НастройкиУчетнойЗаписи) Экспорт
	...
	Если ЕстьИзменения Тогда
		...
		Иначе
			ПротоколОшибкиПриВызове(
				НСтр("ru = 'Не выполнена проверка перед записью учетной записи сервера DSS';
					|en = 'A check is not completed before recording the DSS server account'", КодЯзыка), 
				"ЗаписьДанныхНастроекУчетнойЗаписи",
				НастройкиУчетнойЗаписи);
		КонецЕсли;
...
Функция ОписаниеОшибки_invalid_rev_request()
	...
	Результат = ОбщееОписаниеОшибки("300015", 
			НСтр("ru = 'Неверно указан идентификатор запроса.';
				|en = 'Request ID is specified incorrectly.'"));
...
Функция ОписаниеОшибки_request_not_found()
	...
	Результат = ОбщееОписаниеОшибки("300016", 
			НСтр("ru = 'Не найден соответствующий сертификату запрос.';
				|en = 'The request that corresponds to the certificate is not found.'"));
...
Функция ОписаниеОшибки_invalid_request_status()
	...
	Результат = ОбщееОписаниеОшибки("300019", 
			НСтр("ru = 'Соответствующий запрос не находится в состоянии ""Обрабатывается"".';
				|en = 'The matching request is not in the ""Processing"" state.'"));
...
Функция ОписаниеОшибки_invalid_subject_name()
	...
	Результат = ОбщееОписаниеОшибки("300040", 
			НСтр("ru = 'Неправильно указано имя запроса на сертификат.';
				|en = 'Certificate request name is specified incorrectly.'"));
...
Функция ОписаниеОшибки_СертификатБезУчетнойЗаписи()
	...
	Результат = ОбщееОписаниеОшибки("300048", 
			НСтр("ru = 'Выбранный сертификат хранится на сервере DSS, но в параметрах не указана учетная запись DSS';
				|en = 'Selected certificate is stored on the DSS server but the DSS account is not specified in the parameters'"));
...
Процедура ОбработатьПоставляемыеСервера(ПутьКФайлу)
	...
	Если ВсеДанные.Количество() <> 1 Тогда
		ПротоколОшибкиПриВызове(
			НСтр("ru = 'Поставляемые данные не содержат нужной информации.';
				|en = 'Default data does not contain the required information.'", КодЯзыка), 
			"ПоставляемыеДанные",
			ПутьКФайлу,
			ИнформацияОбОшибке());
		Удачно = Ложь;
	...
	Если ЭтоМассивСерверов Тогда
		...
		Если КоличествоСерверов = 0 
			...
			ПротоколОшибкиПриВызове(
				НСтр("ru = 'Запись поставляемых данных завершилась с ошибками.';
					|en = 'Error saving the default data.'", КодЯзыка), 
				"ПоставляемыеДанные",
				ПутьКФайлу,
				ИнформацияОбОшибке());
			Удачно = Ложь;
