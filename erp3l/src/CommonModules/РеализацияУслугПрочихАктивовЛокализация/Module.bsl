...
Функция ТекстОтраженияВРеглУчете() Экспорт
	...
	// Реализация ОС с отложенным переходом прав версии 2.4
	ТекстыОтражения.Добавить(ТекстРеализацияОССОтложеннымПереходомПрав24());
...
Функция ТекстЗапросаТаблицаВтТаблицаВНА(ТекстыЗапроса) Экспорт
	...
	ВнеоборотныеАктивыЛокализация.ТекстЗапросаТаблицаВтПорядокУчетаОСБУ(ТекстыЗапроса, "Документ.РеализацияУслугПрочихАктивов");
...
Функция РазрешеноПробитиеФискальныхЧековПоДокументу(Форма) Экспорт
	...
	Если Форма.Объект.ХозяйственнаяОперация <> Перечисления.ХозяйственныеОперации.РеализацияКлиенту
		И Форма.Объект.ХозяйственнаяОперация <> Перечисления.ХозяйственныеОперации.РеализацияВнеоборотныхАктивов Тогда
...
Функция ПозицииНоменклатурыПоДокументу(ДокументСсылка) Экспорт
	...
	|	""""                      							  КАК Характеристика,
	|	""""                      							  КАК Упаковка,
	|	РеализацияУслугПрочихАктивовТаблицаТоваров.КоличествоУпаковок КАК Количество,
	|	(ВЫРАЗИТЬ((РеализацияУслугПрочихАктивовТаблицаТоваров.СуммаБезНДС + РеализацияУслугПрочихАктивовТаблицаТоваров.СуммаНДС) /
	|		РеализацияУслугПрочихАктивовТаблицаТоваров.КоличествоУпаковок КАК ЧИСЛО(31,2))) КАК Цена,
...
Функция ПредметыРасчетовПоДокументу(ДокументСсылка) Экспорт
	...
	|ИЗ
	|	РеализацияУслугПрочихАктивовТаблицаТоваров КАК РеализацияУслугПрочихАктивовТаблицаТоваров
	|	ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаДанныхДокументов ПО ИСТИНА";
...
Функция ТекстЗадолженностьПокупателяЗаУслугиДоПереходаПрава()
	...
	|		И втПорядокУчетаОСБУ.ОсновноеСредство = ТаблицаРасходы.ВнеоборотныйАктив
	|ГДЕ
...
Функция ТекстЗадолженностьПокупателяЗаУслугиПослеПереходаПрава()
	...
	|	ЕСТЬNULL(Суммы.СуммаБезНДСРегл, Строки.СуммаСНДС - Строки.СуммаНДС) КАК СуммаНУКт,
	|	0 КАК СуммаПРКт,
	|	ЕСТЬNULL(Суммы.СуммаНДСРегл, Строки.СуммаНДС) КАК СуммаВРКт,
	|	""Задолженность покупателя после перехода права собственности"" КАК Содержание
	|	
	|ИЗ
	|	ДокументыКОтражению КАК ДокументыКОтражению
...
Функция ТекстРеализацияОССОтложеннымПереходомПрав24()

	ТекстыОтражения = Новый Массив;	
	
	#Область Недвижимость
	
	#Область ПередачаАктиваДоПереходаПраваСобственности_Дт_45_04___Кт_01_09
	ТекстЗапроса =
	"ВЫБРАТЬ //// Передача актива до перехода права собственности (Дт <45.04> :: Кт <01.09>)
	|	
	|	Строки.Регистратор КАК Ссылка,
	...
	|		И втПорядокУчетаОСБУ.ОсновноеСредство = Строки.ОсновноеСредство
	|	
	|ГДЕ
	...
	|	И Строки.КорСтатьяРасходов <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиРасходов.ПустаяСсылка)
	|	И втПорядокУчетаОСБУ.НедвижимоеИмущество";
	ТекстыОтражения.Добавить(ТекстЗапроса);
	#КонецОбласти
	
	#Область РасходыОтСписанияАктиваСОтложеннымПереходомПрав_Дт_91_02___Кт_45_04
	ТекстЗапроса =
	...
	#Область ДвижимоеИмущество
	
	#Область ПередачаАктиваДоПереходаПраваСобственности_Дт_45_03___Кт_01_09
	ТекстЗапроса =
	...
	|	И Строки.ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|	И Строки.КорСтатьяРасходов <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиРасходов.ПустаяСсылка)
	|	И НЕ втПорядокУчетаОСБУ.НедвижимоеИмущество";
	ТекстыОтражения.Добавить(ТекстЗапроса);
	#КонецОбласти
	
	#Область РасходыОтСписанияАктиваСОтложеннымПереходомПрав_Дт_91_02__Кт_45_03
	ТекстЗапроса =
...
Функция ТекстВосстановитьАмортизационнуюПремию()
	...
	|	Строки.СтатьяДоходов КАК СубконтоКт1,
	|	Строки.АналитикаДоходов КАК СубконтоКт2,
	|	Строки.ВнеоборотныйАктив КАК СубконтоКт3,
	|	
	|	0 КАК ВалютнаяСуммаКт,
	|	0 КАК КоличествоКт,
	|	Строки.СуммаАмортизационнойПремии КАК СуммаНУКт,
	...
	#Область ПередачаАктиваДоПереходаПраваСобственности_Дт_45_04__45_03___Кт_01_09
	ТекстЗапроса =
	"ВЫБРАТЬ //// Передача актива до перехода права собственности (Дт <45.04>, <45.03> :: Кт <01.09>)
	|	
	|	Строки.Регистратор КАК Ссылка,
	|	Строки.Период КАК Период,
	|	Строки.Организация КАК Организация,
	...
	|	
	|	ВЫБОР 
	|		КОГДА ЕСТЬNULL(втПорядокУчетаОСБУ.НедвижимоеИмущество, ЛОЖЬ)
	|			ТОГДА ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПереданныеОбъектыНедвижимости)
	|		ИНАЧЕ ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПрочиеТоварыОтгруженные)
	|	КОНЕЦ КАК СчетДт,
	|	
	|	Строки.КорКонтрагент КАК СубконтоДт1,
	|	Строки.ОсновноеСредство КАК СубконтоДт2,
	...
	#Область РасходыОтСписанияАктиваСОтложеннымПереходомПрав_Дт_91_02__Кт_45_04
	ТекстЗапроса =
	"ВЫБРАТЬ //// Расходы от списания актива с отложенным переходом прав (Дт <91.02> :: Кт <45.04>)
	|	
	|	Строки.Регистратор КАК Ссылка,
