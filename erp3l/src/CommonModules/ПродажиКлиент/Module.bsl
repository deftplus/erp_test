	...
	Иначе
		ПоказатьОповещениеПользователя(
			...
			НСтр("ru = 'Ни в одной строке цены по соглашению с клиентом не заполнены';
				|en = 'No lines have customer agreement prices filled in'"),
			БиблиотекаКартинок.Информация32);
	КонецЕсли;
...
Процедура СформироватьКомплектДокументовВызов(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Не ДополнительныеПараметры.ПараметрыФормирования.СохраненыНастройкиОформления Тогда
		...
		ОткрытьФормуНастройкиПараметров(Оповещение, ДополнительныеПараметры.Форма);
		Возврат;
	КонецЕсли;
...
Процедура СформироватьКомплектДокументовЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ПродажиВызовСервера.СохранитьНастройкуКомплектаДокументов(Результат);
...
Процедура ОформитьПродажуНаКлиентеЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ПараметрыСозданныхДокументов.Свойство("ИмяФормы") 
			И (НЕ ПараметрыФормирования.НеОткрыватьФормуСозданногоДокумента 
				ИЛИ (ПараметрыФормирования.НеОткрыватьФормуСозданногоДокумента И ЕстьОшибкиСозданияДокументов)) Тогда
...
Функция ПараметрыФормированияДокументов(ДополнительныеПараметры)
	...
	ПараметрыФормирования = ПродажиВызовСервера.ПолучитьНастройкуКомплектаДокументов(); 
...
Процедура ОткрытьФормуНастройкиПараметров(Оповещение, Форма)
	
	ПараметрыФормы = ПродажиВызовСервера.ПолучитьНастройкуКомплектаДокументов();
	Если ПараметрыФормы = Неопределено Тогда
		ПараметрыФормы = ПродажиВызовСервера.НовыеПараметрыОформленияКомплектаДокументов();
	КонецЕсли; 
