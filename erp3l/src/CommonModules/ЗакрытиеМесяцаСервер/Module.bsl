...
Процедура ЗаполнитьОписаниеГруппЭтаповЗакрытияМесяца(ТаблицаГруппЭтапов) Экспорт
	...
	НоваяСтрока.Наименование = НСтр("ru = 'Контроль финансовых регистров';
									|en = 'Control of financial registers'");
	...
	НоваяСтрока.Наименование = НСтр("ru = 'Распределение производственных затрат';
									|en = 'Production expense allocation'");
...
Процедура ЗаполнитьОписаниеЭтаповЗакрытияМесяца(ТаблицаЭтапов) Экспорт
	...
	//++ НЕ УТКА
	ДобавитьЭтап_ИсправлениеОшибокКорреспонденцииФинансовыхРегистров(ТаблицаЭтапов, ТекущийРодитель);
	//-- НЕ УТКА
...
Процедура ОписаниеПроверок_ИсправлениеОшибокВИсходныхДанных(ТаблицаПроверок)
	...
	ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru = 'Корректность указания партий производства в документах распределения расходов.';
			|en = 'Correctness of specification of production lots in the cost allocation documents.'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
#Область ИсправлениеОшибокКорреспонденцииФинансовыхРегистров

Процедура ДобавитьЭтап_ИсправлениеОшибокКорреспонденцииФинансовыхРегистров(ТаблицаЭтапов,ТекущийРодитель) Экспорт
	
	НоваяСтрока = ЗакрытиеМесяцаСервер.ДобавитьЭтапВТаблицу(ТаблицаЭтапов, ТекущийРодитель,
		Перечисления.ОперацииЗакрытияМесяца.ИсправлениеОшибокКорреспонденцииФинансовыхРегистров);
		
	НоваяСтрока.ТекстВыполнить = НСтр("ru = 'Исправить';
										|en = 'Correct'", ОбщегоНазначения.КодОсновногоЯзыка());
	НоваяСтрока.ДействиеИспользование = ЗакрытиеМесяцаСервер.ОписаниеДействия_СервернаяПроцедура(
		"ЗакрытиеМесяцаСервер.Использование_ИсправлениеОшибокКорреспонденцииФинансовыхРегистров");
	НоваяСтрока.ДействиеВыполнить  = ЗакрытиеМесяцаСервер.ОписаниеДействия_ВыполнитьРасчет(
		"ЗакрытиеМесяцаСервер.Выполнить_ИсправлениеОшибокКорреспонденцииФинансовыхРегистров");
	НоваяСтрока.ДействиеПодробнее = ЗакрытиеМесяцаСервер.ОписаниеДействия_КлиентскаяПроцедура(
		"МеждународныйУчетПоДаннымОстаточныхФинансовыхРегистровКлиент.АнализОшибокКорреспонденций");
	НоваяСтрока.ВыполняетсяПриПредварительномЗакрытииМесяца = Истина;
	
КонецПроцедуры

Процедура Использование_ИсправлениеОшибокКорреспонденцииФинансовыхРегистров(ПараметрыОбработчика) Экспорт
	...
	Если НЕ ПолучитьФункциональнуюОпцию("НастройкаПроводокПоОбъектамУчета") Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			...
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Функциональная опция ""%1"" отключена.';
					|en = 'The ""%1"" functional option is disabled.'", ОбщегоНазначения.КодОсновногоЯзыка()),
				Метаданные.ФункциональныеОпции.НастройкаПроводокПоОбъектамУчета.Синоним));
	...
	НекорректныеДвижения = МеждународныйУчетПоДаннымОстаточныхФинансовыхРегистров.ДокументыСНарушениямиДвойнойЗаписиВРегистрах(
		Запрос.Параметры.НачалоПериода,
		Запрос.Параметры.МассивОрганизаций);
		
	Если НЕ ЗначениеЗаполнено(НекорректныеДвижения) Тогда
		
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			ПараметрыОбработчика,
			НСтр("ru = 'Нет ошибок в движениях документов.';
				|en = 'No errors in document register records.'", ОбщегоНазначения.КодОсновногоЯзыка()));
			
		Возврат;
		
	КонецЕсли;
	
	УстановитьТекстЗапросаЗагрузкиТаблицыНекорректныхДвижений(Запрос, НекорректныеДвижения);
	...
	|ВЫБРАТЬ
	|	Т.Организация КАК Организация,
	|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ Т.Ссылка) КАК КоличествоДокументов
	|ИЗ
	|(ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	Т.Организация,
	|	Т.Ссылка
	...
	|	Т.Ссылка
	|ИЗ
	|	ВТРегистраторыДляПолногоПроведения КАК Т
	|) КАК Т
	|СГРУППИРОВАТЬ ПО
	|	Т.Организация
	|УПОРЯДОЧИТЬ ПО
	...
	Пока Выборка.Следующий() Цикл
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеВыполнен(
			ПараметрыОбработчика,
			СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'По организации %1 требуется исправление движений документов (%2 шт.)';
					|en = 'The %1 company requires correcting document register records (%2 pcs)'", ОбщегоНазначения.КодОсновногоЯзыка()),
				Выборка.Организация,
				РасчетСебестоимостиПротоколРасчета.ПредставлениеЗначения(Выборка.КоличествоДокументов)));
				
	КонецЦикла;
	
	ТекстИнформация = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Некоторые ошибки могут быть исправлены после выполнения этапа ""%1""';
			|en = 'Some errors can be fixed after the ""%1"" step is completed'", ОбщегоНазначения.КодОсновногоЯзыка()),
		СокрЛП(Перечисления.ОперацииЗакрытияМесяца.РасчетПартийИСебестоимости));
	
	ДобавитьПоясняющуюИнформациюКЭтапу(ПараметрыОбработчика, ТекстИнформация);
	
КонецПроцедуры

Процедура Выполнить_ИсправлениеОшибокКорреспонденцииФинансовыхРегистров(ПараметрыОбработчика) Экспорт
	
	ПериодРасчета    = ПараметрыОбработчика.ПараметрыРасчета.ПериодРегистрации;
	ТекущаяОперация  = ПараметрыОбработчика.ДанныеЭтапа.Код;
	...
	РасшифровкаКодовОшибок.Вставить(1, НСтр("ru = 'Нарушена двойная запись в финансовых регистрах';
											|en = 'Double entry of financial registers is violated'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ПараметрыПроведения = ПроведениеДокументов.ИнициализироватьПараметрыВосстановленияДвижений(
		...
		"ВТРегистраторыДляПолногоПроведения");
	
	ПараметрыПроведения.Вставить("ПричинаПерепроведения",				НСтр("ru = 'Заполнение идентификатора финансовой записи';
																				|en = 'Fill in financial record ID'"));
	ПараметрыПроведения.Вставить("ОперацииЗакрытияМесяца",				ТекущаяОперация);
	ПараметрыПроведения.Вставить("ПериодРасчета",						ПериодРасчета);
	ПараметрыПроведения.Вставить("ИмяСобытияЖурналаРегистрации",		ЗакрытиеМесяцаСервер.ИмяСобытияЖурналаРегистрации(ОбщегоНазначения.ИмяЗначенияПеречисления(ТекущаяОперация)));
	ПараметрыПроведения.Вставить("ТолькоПерепроведение",				Истина);
	ПараметрыПроведения.Вставить("ТолькоРегистрироватьОшибки",			Ложь);
	
	ПроведениеДокументов.ВосстановитьДвиженияДокументов(ПараметрыПроведения);
	
	// Зарегистрируем ошибки, не исправленные в результате проведения.
	НекорректныеДвижения = МеждународныйУчетПоДаннымОстаточныхФинансовыхРегистров.ДокументыСНарушениямиДвойнойЗаписиВРегистрах(
		Запрос.Параметры.НачалоПериода,
		Запрос.Параметры.МассивОрганизаций);
	...
	ПараметрыПроведения.Вставить("ТолькоПерепроведение",				Ложь);
	ПараметрыПроведения.Вставить("ТолькоРегистрироватьОшибки",			Истина);
	
	ПроведениеДокументов.ВосстановитьДвиженияДокументов(ПараметрыПроведения);
	
КонецПроцедуры

Процедура УстановитьТекстЗапросаЗагрузкиТаблицыНекорректныхДвижений(Запрос, НекорректныеДвижения)
	
	Запрос.Текст =
	"ВЫБРАТЬ
	|	Т.Организация 							КАК Организация,
	|	Т.Регистратор 							КАК Ссылка,
	|	ВЫРАЗИТЬ(Т.ИмяРегистра КАК СТРОКА(255)) КАК ИмяРегистра,
	|	1 										КАК КодОшибки,
	|	ВЫБОР КОГДА Т.ИмяРегистра = """"
	|		ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ 									КАК ПолноеПроведение
	|ПОМЕСТИТЬ ВТРегистраторыСНекорректнымиДвижениямиПредварительная
	|ИЗ
	|	&НекорректныеДвижения КАК Т
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка
	|;
	|
	...
	|ПОМЕСТИТЬ ВТРегистраторыДляПолногоПроведения
	|ИЗ
	|	ВТРегистраторыСНекорректнымиДвижениямиПредварительная КАК Т
	|ГДЕ
	|	Т.ПолноеПроведение
	|ИНДЕКСИРОВАТЬ ПО
	|	Ссылка
	...
	|ИЗ
	|	ВТРегистраторыСНекорректнымиДвижениямиПредварительная КАК Т
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВТРегистраторыДляПолногоПроведения КАК Отбор
	|		ПО Т.Ссылка = Отбор.Ссылка
	|ГДЕ
	|	Отбор.Ссылка ЕСТЬ NULL
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|УНИЧТОЖИТЬ ВТРегистраторыСНекорректнымиДвижениямиПредварительная
	|";
	
	Для Каждого ТекСтр Из НекорректныеДвижения Цикл
		ТекСтр.ИмяРегистра = СокрЛП(ТекСтр.ИмяРегистра);
	КонецЦикла;
	
	Запрос.УстановитьПараметр("НекорректныеДвижения", НекорректныеДвижения);
...
Процедура ОтразитьЗаданияКЗакрытиюМесяца(Документ, МенеджерВременныхТаблиц) Экспорт
	...
	Если ЗначениеЗаполнено(ТекстВложенногоЗапроса) Тогда // есть хотя бы один контрольный регистр.
		...
		// ++ Локализация
		ПроверитьКонтрольныеРегистрыВИсточникахДанныхДляЗаданийЗакрытияМесяца(Запрос);
		// -- Локализация
...
Функция ДатаЗаданияНаСледующийПериод(Операция, Организация, ДокументЗадания, Знач СледующийМесяц) Экспорт
	...
	Если Операция = Перечисления.ОперацииЗакрытияМесяца.ФормированиеРезервовПоСомнительнымДолгам Тогда
		...
		СледующийМесяц = Документы.НачислениеСписаниеРезервовПоСомнительнымДолгам.ПериодРасчетаРезервовПоСомнительнымДолгам(
			Организация, СледующийМесяц);
				
	ИначеЕсли Операция = Перечисления.ОперацииЗакрытияМесяца.НачислениеПроцентовПоАренде Тогда
		
		СледующийМесяц = Документы.НачислениеПроцентовПоАренде.ПериодНачисленияПроцентовПоАренде(Организация, СледующийМесяц);
				
	ИначеЕсли Операция = Перечисления.ОперацииЗакрытияМесяца.ЗакрытиеРасходовОтВыбытияОС Тогда
		
		СледующийМесяц = ЗакрытиеРасходовОтВыбытияОС.СледующийПериодОперации(Организация, СледующийМесяц);
...
Процедура ПроверитьКонтрольныеРегистрыВИсточникахДанныхДляЗаданийЗакрытияМесяца(Запрос)
