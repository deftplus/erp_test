		...
		Иначе
			...
			Если ДанныеПриглашения = Неопределено Тогда
				...
				ПредставлениеНенастроеннойСвязи = СтрШаблон(НСтр(
					...
					|en = 'Company ID %1(%2) - counterparty TIN/CRTR %3'"),
...
Функция ВидОшибкиНеУказанОбратныйАдресКонтрагента() Экспорт
	...
	ВидОшибки.ЗаголовокПроблемы = НСтр("ru = 'Не удалось сформировать транспортный контейнер ЭДО';
										|en = 'Cannot generate the EDI transport container'");
	ВидОшибки.ОписаниеПроблемы = НСтр("ru = 'Не заполнен обратный адрес контрагента';
										|en = 'Counterparty return address is blank'");
	ВидОшибки.ОписаниеРешения = НСтр("ru = 'Рекомендуется обновить конфигурацию информационной базы корреспондента
			...
			|en = 'It is recommended to update the configuration of the correspondent infobase
			|or fill in the value for the ""return address"" in the incoming document card'");
...
Процедура ЗаполнитьКонтрагентаПриРаспаковкеКонтейнера(ПараметрыКонтейнера,
	...
	Если ЗначениеЗаполнено(Карточка.КППКонтрагентаУточненный) Тогда
		...
		КППКонтрагента = Карточка.КППКонтрагентаУточненный;
		НаименованиеКонтрагента = Карточка.НаименованиеКонтрагентаУточненное;
		
	КонецЕсли;
	Если Не ЗначениеЗаполнено(Контрагент) И ЗначениеЗаполнено(Карточка.ИННКонтрагента) Тогда
		
		Контрагент = ИнтеграцияЭДО.СсылкаНаОбъектПоИННКПП("Контрагенты", Карточка.ИННКонтрагента,
			Карточка.КППКонтрагента, Карточка.Дата);
		
		Если Не ЗначениеЗаполнено(КППКонтрагента) Тогда
			КППКонтрагента = Карточка.КППКонтрагента;
		КонецЕсли;
		Если Не ЗначениеЗаполнено(НаименованиеКонтрагента) Тогда
			НаименованиеКонтрагента = Карточка.НаименованиеКонтрагента;
		КонецЕсли;
	...
	Если СоздаватьКонтрагентовАвтоматически
		И ЗначениеЗаполнено(Карточка.ИННКонтрагента)
		И ЗначениеЗаполнено(НаименованиеКонтрагента) Тогда
		
		РеквизитыКонтрагента = Новый Структура;
		РеквизитыКонтрагента.Вставить("ИНН", Карточка.ИННКонтрагента);
		РеквизитыКонтрагента.Вставить("КПП", КППКонтрагента);
