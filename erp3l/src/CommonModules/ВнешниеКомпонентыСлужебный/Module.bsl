	...
	Если ПустаяСтрока(Идентификатор) Тогда
		...
		Если КомпонентаСодержитЕдинственныйКлассОбъектов Тогда
			ВызватьИсключение НСтр("ru = 'Недопустимо одновременно не указывать Идентификатор и ИдентификаторыСозданияОбъектов в ВнешниеКомпонентыСлужебный.ПроверитьПодключениеКомпоненты';
									|en = 'Either the ID or ObjectsCreationIDs must be specified in AddInsInternal.CheckAddInAttachment'");
		КонецЕсли;
...
Процедура НоваяКомпонентаСПортала(ПараметрыПроцедуры, АдресРезультата) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ИнтернетПоддержкаПользователей.ПолучениеВнешнихКомпонент") Тогда
		...
		Если ЗначениеЗаполнено(КодОшибки) Тогда
			...
			ИначеЕсли КодОшибки = "ФайлНеЗагружен" Тогда
				ИнформацияОбОшибке = НСтр("ru = 'При загрузке внешней компоненты %1 произошла непредвиденная ошибка (код ФайлНеЗагружен).';
											|en = 'An unexpected error occurred when importing the %1 add-in (the FileNotImported code).'");
			ИначеЕсли КодОшибки = "АктуальнаяВерсия" Тогда
				ИнформацияОбОшибке = НСтр("ru = 'При загрузке внешней компоненты %1 произошла непредвиденная ошибка (код АктуальнаяВерсия).';
											|en = 'An unexpected error occurred when importing the %1 add-in (the LatestVersion code).'");
			КонецЕсли;
			...
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Обновление внешних компонент';
											|en = 'Updating add-ins'", ОбщегоНазначения.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, , , ТекстОшибки);
		...
		Если Не Информация.Разобрано Тогда
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Обновление внешних компонент';
											|en = 'Updating add-ins'", ОбщегоНазначения.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка, , , Информация.ОписаниеОшибки);
			ВызватьИсключение Информация.ОписаниеОшибки;
		КонецЕсли;
		...
		Попытка
			...
			Объект.ОписаниеОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Загружена с Портала 1С:ИТС. %1.';
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Подсистема ""ИнтернетПоддержкаПользователей.ПолучениеВнешнихКомпонент"" не внедрена в конфигурацию.';
								|en = 'The OnlineUserSupport.GettingAddIns subsystem is not built in the configuration.'");
	КонецЕсли;
