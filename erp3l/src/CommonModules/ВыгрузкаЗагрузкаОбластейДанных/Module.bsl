...
Функция ВыгрузитьТекущуюОбластьВАрхив(Знач АдресДанных = Неопределено,
	...
	Если ЗначениеЗаполнено(ОшибкиВыгружаемыхДанных) Тогда
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Выгрузка данных. Обнаружены ошибки в выгружаемых данных';
										|en = 'Data export. Errors are found in data to export'", ОбщегоНазначения.КодОсновногоЯзыка()),
		...
		ТекстОшибки = СтрШаблон(
			...
			|
			|%1';
			|en = 'Before fixing, we recommend that you back up the infobase.
			|
			|%1'"), ТекстОшибки);
		...
		Для Каждого ВыгружаемыйТип Из ВыгружаемыеТипы Цикл
			...
			Если Выборка.Следующий() И Выборка.Регистратор = Неопределено Тогда
				...
				ТекстОшибки = СтрШаблон(
					...
					|en = 'Обнаружены отсутствующие регистраторы в таблице %1.
					|Рекомендуется восстановить отсутствующие регистраторы или удалить проблемные записи.'"),
					ПолноеИмя) + " ";
				ОшибкиВыгружаемыхДанных.Добавить(ТекстОшибки);
