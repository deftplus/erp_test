...
Функция ПараметрыКомпонентыCMSSign(ПараметрыCMS, ОписаниеДанных) Экспорт
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Некорректный параметр ТипПодписи метода CMSSign компоненты ExtraCryptoAPI.';
								|en = 'Incorrect SignatureType parameter of the CMSSign method of the ExtraCryptoAPI add-in.'");
	КонецЕсли;
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Некорректный параметр Данные метода CMSSign компоненты ExtraCryptoAPI.';
								|en = 'Incorrect Data parameter of the CMSSign method of the ExtraCryptoAPI add-in.'");
	КонецЕсли;
	...
	Иначе
		ВызватьИсключение НСтр("ru = 'Некорректный параметр Открепленная метода CMSSign компоненты ExtraCryptoAPI.';
								|en = 'Incorrect Detached parameter of the CMSSign method of the ExtraCryptoAP add-in.'");
	КонецЕсли;
...
Функция ОбластьXML(СтрокаXML, ИмяЭлемента) Экспорт
	...
	Если Позиция > 0 Тогда
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Найдено более одного элемента ""%1"" в документе XML.';
				|en = 'More than one %1 element is found in the XML document.'"), ИмяЭлемента);
		Результат.ТекстОшибки = ТекстОшибки;
	КонецЕсли;
			...
			ОбщегоНазначенияКлиентСервер.Проверить(Ложь,
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Недопустимый адрес временного хранилища в параметре Данные:
					           |%1';
					           |en = 'Invalid address of temporary storage in the Data parameter:
					           |%1'") + Символы.ПС, Данные),
				"ЭлектроннаяПодписьСлужебныйКлиентСервер.АлгоритмПодписи");
		КонецЕсли;
