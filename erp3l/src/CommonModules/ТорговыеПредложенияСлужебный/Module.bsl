...
Процедура ПроверитьДанныеПрайсЛистаПередПубликацией(ПараметрыЗапроса, ЭтоСозданиеПрайсЛиста, Отказ)
	...
	Если ЭтоСозданиеПрайсЛиста И Не ТелефонКорректен(ПараметрыЗапроса.Телефон) Тогда
		...
		ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(НСтр("ru = 'Некорректно заполнен телефон организации';
															|en = 'Company phone number is filled in incorrectly'"), 
			ОбщегоНазначенияБЭДКлиентСервер.ПодсистемыБЭД().БизнесСеть, УровеньЖурналаРегистрации.Предупреждение, ПараметрыЗаписи);
...
Процедура ЗаполнитьПредставлениеКатегории(ИдентификаторКатегории, ИдентификаторТорговогоПредложения, РодительскиеКатегории, ПараметрыВывода, Отказ)
	...
	Иначе
		...
		Если Отказ Тогда
			...
			ПодробнаяИнформация = ТекстСообщения + Символы.ПС + СтрШаблон(НСтр("ru = 'Идентификатор %1.';
																				|en = 'Identifier %1.'"), ИдентификаторТорговогоПредложения)
...
Процедура ДополнитьЗапросСлужебнымиТаблицами(Запрос)
	...
	|		ИНАЧЕ ИСТИНА
	|	КОНЕЦ КАК ОрганизацияЗарегистрирована,
	|	СостоянияСинхронизацииТорговыеПредложения.ПубликоватьСкидкиЗаРазовыйОбъемПродаж КАК ПубликоватьСкидкиЗаРазовыйОбъемПродаж,
	...
	|				И ПрайсЛистыСлужебный.ВариантПубликацииОстатков В (ЗНАЧЕНИЕ(Перечисление.ВариантыПубликацииОстатковТорговыеПредложения.Наличие), ЗНАЧЕНИЕ(Перечисление.ВариантыПубликацииОстатковТорговыеПредложения.КоличествоИНаличие), ЗНАЧЕНИЕ(Перечисление.ВариантыПубликацииОстатковТорговыеПредложения.ПустаяСсылка))
	|			ТОГДА Товары.Наличие
	|		ИНАЧЕ НЕОПРЕДЕЛЕНО
	|	КОНЕЦ КАК Наличие,
	|	ВЫБОР
	|		КОГДА ПрайсЛистыСлужебный.ПубликоватьОстатки
	|				И ПрайсЛистыСлужебный.ВариантПубликацииОстатков В (ЗНАЧЕНИЕ(Перечисление.ВариантыПубликацииОстатковТорговыеПредложения.Количество), ЗНАЧЕНИЕ(Перечисление.ВариантыПубликацииОстатковТорговыеПредложения.КоличествоИНаличие))
...
Функция ОбработатьОшибкуСервиса(Соединение, ЗапросСервиса, КодСостояния, Данные, Ошибка, Отказ)
	...
	ПодробныйТекстОшибки = ТекстОшибки
		...
		+ Символы.ПС + СтрШаблон(НСтр("ru = 'Код состояния: %1';
	...
	Если ЕстьСвойство(Данные, "detail", ЗначениеСвойства) Тогда
		ОшибкаСервиса = ?(ПустаяСтрока(ОшибкаСервиса), "", ОшибкаСервиса + Символы.ПС) 
			+ НСтр("ru = 'Описание ошибки';
					|en = 'Error details'") + ": " + ЗначениеСвойства;
	КонецЕсли;		
	
	Если ЕстьСвойство(Данные, "requestTraceId", ЗначениеСвойства) Тогда
		ОшибкаСервиса = ?(ПустаяСтрока(ОшибкаСервиса), "", ОшибкаСервиса + Символы.ПС)
			+ НСтр("ru = 'Идентификатор запроса';
					|en = 'Query ID'") + ": " + ЗначениеСвойства;
	КонецЕсли;
		...
		Если ЗначениеЗаполнено(ОшибкиВложенные) Тогда
			ОшибкаСервиса = ?(ПустаяСтрока(ОшибкаСервиса), "", ОшибкаСервиса  + Символы.ПС) 
				+ НСтр("ru = 'Дополнительное описание';
						|en = 'Additional details'") + ": " + ОшибкиВложенные;
		КонецЕсли;
...
Процедура ОбработатьВыгрузкуПрайсЛиста(Знач ПрайсЛист, ТекстОшибки, ЧастичнаяПубликация, КоличествоОпубликованных, ПараметрыСозданияПрайсЛиста, Отказ)
	...
	Если Отказ Тогда
		ТекстОшибки = НСтр("ru = 'Ошибка удаления устаревших изображений номенклатуры поставщика.';
							|en = 'An error occurred while deleting supplier''s products obsolete images.'");
		Возврат;
	КонецЕсли;
...
Процедура ОбработатьТорговыеПредложения(ПрайсЛист, ЧастичнаяПубликация, КоличествоОпубликованных, Отказ)
	...
	Для Каждого СтрокаТовары Из ПрайсЛист.Товары Цикл
		...
		ОбновлятьОстаткиИЦены = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПрайсЛист.Ссылка, "ОбновлятьОстаткиИЦены");
		Если ОбновлятьОстаткиИЦены Тогда
...
Процедура ПроверитьРеквизитыКонтрагентов(ТаблицаКонтрагентов, ПрайсЛист, Отказ)
	...
	Для каждого Строка Из ТаблицаКонтрагентов Цикл
		...
		ИначеЕсли Не СтрДлина(Строка.КПП) = 9 Тогда
			ТекстОшибки = СтрШаблон(НСтр("ru = 'Для контрагента %1 некорректно задан КПП (%2) - КПП должен состоять из 9 цифр';
										|en = 'CRTR (%2) is incorrect for counterparty %1 - CRTR must contain 9 numbers'"), 
		...
		ИначеЕсли Не СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(Строка.КПП) Тогда
			ТекстОшибки = СтрШаблон(НСтр("ru = 'Для контрагента %1 некорректно задан КПП (%2) - КПП должен состоять только из цифр';
										|en = 'CRTR (%2) is incorrect for counterparty %1 - CRTR must contain only numbers'"), 
				Строка.Ссылка, Строка.КПП);
			Ошибки.Добавить(ТекстОшибки);
		КонецЕсли;
	...
	ТекстЗаписи = СтрШаблон(НСтр("ru = 'Некорректно заполнены сведения о контрагентах:
		|%1.';
		|en = 'Information about counterparties is incorrect:
...
Процедура ВыполнитьОбновлениеТорговыхПредложенийТорговойПлощадки(Знач ПараметрыПроцедуры, АдресРезультата, КоличествоЗаписей = 0)
	...
	Исключение
		...
		ТекстОшибки = НСтр("ru = 'Обновление торговых предложений торговой площадки.';
							|en = 'Update selling propositions of the trading platform.'");
		
		ПодробныйТекстОшибки = ТекстОшибки + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			
		ЗаписьЖурналаРегистрации(ОбщегоНазначенияБЭД.ЗаписатьВЖурналРегистрации(
