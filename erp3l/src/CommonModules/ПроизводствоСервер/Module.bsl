	...
	Если РазмерыВременныхТаблиц.ВТНеоформленноеПроизводство = 0 И РазмерыВременныхТаблиц.ВТДокументыПроизводства = 0 Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			...
			НСтр("ru = 'Нет материалов и работ для списания на производство без заказов';
				|en = 'No materials and works to write off for backflush production'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ОписаниеПроверок_ОформлениеПроизводстваБезЗаказов(ТаблицаПроверок) Экспорт
	...
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru = 'Не списаны материалы (работы) и трудозатраты на производство';
			|en = 'Materials (works) and labor costs are not written off to production'", ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru = 'Должно быть оформлено списание всех материалов (работ) и трудозатрат на производство без заказов.';
			|en = 'Write-off of all materials (works) and labor costs for backflush production should be registered.'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ПроверкаНеобходимостиОформленияПроизводстваБезЗаказов(ПараметрыПроверки) Экспорт
	...
	ПараметрыРегистрации = ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		...
		НСтр("ru = 'Обнаружены не списанные на производство без заказа материалы (работы) организации ""%1"" на конец периода %2';
			|en = 'Materials (works) not written off for backflush production are detected for the ""%1"" company as of the end of %2 period'", ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей);
...
Функция ПараметрыПодстановкиАлгоритмаРасчетаДолейСтоимости(ИмяТаблицы = "ТабличнаяЧасть", 
															...
															ИспользуетсяЦенообразование25 = Ложь) Экспорт
...
Процедура ВыполнитьПодстановкуАлгоритмаРасчетаДолейСтоимости(ТекстЗапроса, Подстановка, ИспользуетсяЦенообразование25 = Ложь) Экспорт
	...
	Если ИспользуетсяЦенообразование25 Тогда
		Шаблон = Шаблон + "ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры25.СрезПоследних(,
		|		ВидЦены = &ВидЦены
		...
		|";
	Иначе
		Шаблон = Шаблон + "ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры.СрезПоследних(,
		|		ВидЦены = &ВидЦены
		|		И &ТекстЗапросаОтборПоНоменклатуре
		|		И &ТекстЗапросаОтборСпособРаспределенияЗатрат
		|) КАК ПлановыеЦены
...
Функция СобытиеРасчетНормативнойДлительности() Экспорт
	...
	Событие  = СтрШаблон("%1.%2",
						 НСтр("ru = 'НСИ производства';
								|en = 'Production master data'", КодЯзыка),
...
Функция СобытиеРасчетОчередиПроизводственныхОпераций() Экспорт
	...
	Событие  = СтрШаблон("%1.%2",
						 ...
						 НСтр("ru = 'Формирование очереди производственных операций';
								|en = 'Generating production operation queue'", КодЯзыка));
...
Функция СобытиеЗаполнениеЭтаповПоПроизводственнымОперациям() Экспорт
	...
	Событие  = СтрШаблон("%1.%2",
						 ...
						 НСтр("ru = 'Заполнение этапов по производственным операциям';
								|en = 'Filling in stages according to production operations'", КодЯзыка));
