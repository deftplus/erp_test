...
Функция ВыполнитьДействиеДляНесколькихТиповЗаказов(Параметры) Экспорт
	...
	ПараметрыЗаполнения.Вставить("СнятьФлагОтгружатьОднойДатой", Параметры.СнятьФлагОтгружатьОднойДатой);
	ПараметрыЗаполнения.Вставить("ЗаполнитьДатуОтгрузкиОднойДатой", Параметры.ЗаполнитьДатуОтгрузкиОднойДатой);
...
Функция ПараметрыВыбораОбеспечения(Форма, ТекущаяСтрока, Режим) Экспорт
	...
	ИначеЕсли Режим = ОбеспечениеВДокументахКлиентСервер.РежимПодборСкладов() Тогда
		ПараметрыФормы.ЗаполняемыеПоля = "КоличествоУпаковок,Склад";
	КонецЕсли;
...
Процедура ДоступныеОстаткиДобавитьОформлениеКолонкиДоступно(Форма) Экспорт
	...
	// Скрытие колонки, если действие не "К обеспечению" и не "Не обеспечивать".
	...
	// Скрытие колонки, если действие "Не обеспечивать" и и номенклатура не тара .
	Элемент = УсловноеОформление.Элементы.Добавить();
...
// 
// Параметры:
//  ПоляСтрокой - Строка - "СкладОбязателен" и/или "ДатаОтгрузкиОбязательна" через запятую.
...
//
Процедура АктуализироватьДатуОтгрузки(ПараметрыЗаполнения, НеОтгружатьЧастями, ДатаОтгрузки, Товары, ИмяПоля) Экспорт
...
//
Процедура ПроверитьЗаполнитьДатуОтгрузкиДляСтрокОтгрузить(Изменения, ИмяПоля, ОтгружатьОднойДатой, ДатаОтгрузки = Неопределено) Экспорт
...
Функция ВыполнитьДействиеДляЗаказов(Заказы, КодДействия, УникальныйИдентификатор, ПараметрыВстраиванияВходящие) Экспорт
	...
	Если КодДействия = "ДЕЙСТВИЕ_ОТГРУЗИТЬ" И Заказы.Количество() > 0 Тогда
		...
		Результат = Новый Структура();
		Результат.Вставить("НуженВопрос", Не Запрос.Выполнить().Пустой());
	...
	Для Каждого Ссылка Из Заказы Цикл
		...
		Попытка
			...
			Если ДокументОбъект.Проведен Тогда
				...
				ОстаткиСклада = ОстаткиДляЗаписанногоДокумента(Запрос.МенеджерВременныхТаблиц);
				Запрос.Текст =
					"ВЫБРАТЬ
					|	Таблица.Номенклатура                 КАК Номенклатура,
					|	Таблица.Номенклатура.ТипНоменклатуры КАК ТипНоменклатуры,
				...
				Иначе
					...
					ИдентификаторыСтрок = ДокументОбъект[ИмяТабличнойЧасти].Выгрузить(, "НомерСтроки").ВыгрузитьКолонку("НомерСтроки");
				КонецЕсли;
				...
				Если КодДействия = "ДЕЙСТВИЕ_ОТГРУЗИТЬ" И ТаблицаИзменений.Количество() > 0 Тогда
					...
					Если ДанныеСтрок.Количество() > 0 Тогда
						Если ДанныеСтрок[0].ДатаОтгрузкиВсехСтрокОднойДатой <> Неопределено
							И ДанныеСтрок[0].ДатаОтгрузкиВсехСтрокОднойДатой < НачалоДня(ТекущаяДатаСеанса())
							И ПараметрыВстраивания.ВыражениеДляВопросаПриВыполненииДействияВСпискеЗаказов <> "НЕОПРЕДЕЛЕНО" Тогда
							...
							Если ДанныеСтрок.Количество() = ТаблицаИзменений.Количество() Тогда
								ПараметрыЗаполненияТекущие.ЗаполнитьДатуОтгрузкиОднойДатой = Истина;
							КонецЕсли;
						КонецЕсли;
					КонецЕсли;
...
Функция ТекстОшибкиНетТоваровДоступныхДляОтгрузки() Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьОбособленноеОбеспечениеЗаказов") Тогда
		...
		ТекстОшибки = НСтр("ru = 'Нет товаров, доступных для отгрузки. Для начала процесса отгрузки необходимо указать действие
		                       ...
		                       |en = 'There are no goods available for shipment. To start shipping, specify 
		                       |the ""Ship"" or ""Ship on assigned basis"" action for one or several product items in the order.'");
	...
	Иначе
		...
		ТекстОшибки = НСтр("ru = 'Нет товаров, доступных для отгрузки. Для начала процесса отгрузки необходимо
		                       ...
		                       |en = 'There are no goods available for shipment. To start shipping, 
		                       |specify the ""Ship"" action for one or several product items in the order.'");
	...
	Иначе
		...
		ИначеЕсли Таблица[0].РезервПревышаетОстатки Тогда
			...
			СвойстваГиперссылки.Заголовок      = НСтр("ru = 'Резерв превышает остаток на складе';
														|en = 'The reserve exceeds the warehouse balance'");
...
//                     "ОтложитьОбеспечение" (в документе указано действие "К обеспечению"),
...
Функция РаспределениеЗапасовДляРедактируемогоДокумента(Форма, ОтборСостояний = Неопределено) Экспорт
	...
	Если ОтборСостояний = Неопределено Тогда
		...
		ОтборСостояний.Добавить(Перечисления.РаспределениеЗапасовСостояния.ОтложитьОбеспечение);
...
Функция ЗаполнитьОбеспечениеИзФормыСостоянияОбеспечения(КодДействия, АдресАналитик, Форма) Экспорт
	...
	Запрос.Текст =
		...
		|ИНДЕКСИРОВАТЬ ПО
		|	ЗаказНаОтгрузку,ЖелаемаяДатаОтгрузки,Номенклатура,Характеристика,Склад,Назначение,Действие,Обособленно
		|;
		|
		|/////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|		И ТаблицаДанныхДокумента.ВариантОбеспечения  = ТаблицаАналитик.Действие
...
//
// Параметры:
//  ПустаяСсылка - ЛюбаяСсылка - пустая ссылка на документ определенного типа.
//
// Возвращаемое значение:
//  Массив из ЛюбаяСсылка - заказы заданий из регистра заданий распределения запасов
//
Функция ЗаданияПоЗаказамПоТипу(ПустаяСсылка) Экспорт
	
	Запрос = Новый Запрос();
	Типы = ТипЗнч(ПустаяСсылка);
	Если Типы = Тип("Массив") Тогда
		Типы = Новый Массив();
		Для Каждого Элемент Из ПустаяСсылка Цикл
...
Функция ПодстановкиПлановойДатыПоставки(Номенклатура, Характеристика, Склад, НачалоЭтогоДня, УсловиеПолучения) Экспорт
	...
	ТекстЗапроса =
		...
		|				
		|				ДОБАВИТЬКДАТЕ(ВедущаяТаблица.НачалоЭтогоДня, ДЕНЬ, ОсновнойСпособОбеспечения.ГарантированныйСрокОтгрузки)
		|				
		|			ИНАЧЕ
	...
	Подстановки = ПодстановкиДатыПоКалендарю(
		...
		"ОсновнойСпособОбеспечения.ГарантированныйСрокОтгрузки",
		"ВедущаяТаблица.УсловиеПолучения",
		"Срок");
...
Функция ТекстЗапросаРегистрацииДокументовКОбработке(ИмяДокумента, ИмяТЧ, ИмяПоляВариантОбеспечения = "ВариантОбеспечения") Экспорт
...
Процедура ОбработатьНеиспользуемыеВариантыОбеспечения(ТабличнаяЧасть, ОбъектИзменен, ТребуетсяОбработкаВариантаОбеспечения, ЕстьИсходный = Ложь) Экспорт
	
	Если Не ТребуетсяОбработкаВариантаОбеспечения Тогда
		Возврат;
	КонецЕсли;
	...
	Если ТипЗнч(ТабличнаяЧасть) = Тип("ДокументОбъект.ЗаказНаСборку") Тогда
		...
		Если Строка.ВариантОбеспечения = ВариантИзЗаказов Тогда
		...
		Если Строка.ВариантОбеспечения = ВариантОтгрузитьОбособленно Тогда
			...
			ОбъектИзменен = Истина;
		КонецЕсли;
		Если Строка.ВариантОбеспечения = ВариантОбособленно Тогда
			Строка.ВариантОбеспеченияДоИзмененияОбновлениемИБ = Строка.ВариантОбеспечения;
			Строка.ВариантОбеспечения = ВариантТребуется;
			Строка.Обособленно = Истина;
			ОбъектИзменен = Истина;
	...
	Иначе
		...
		Для Каждого Строка Из ТабличнаяЧасть Цикл
			...
			Если ЕстьИсходный Тогда
				...
				Если Строка.ВариантОбеспеченияИсходный = ВариантИзЗаказов Тогда
				...
				Если Строка.ВариантОбеспеченияИсходный = ВариантОтгрузитьОбособленно Тогда
					...
					ОбъектИзменен = Истина;
				КонецЕсли;
				Если Строка.ВариантОбеспеченияИсходный = ВариантОбособленно Тогда
					Строка.ВариантОбеспеченияИсходныйДоИзмененияОбновлениемИБ = Строка.ВариантОбеспеченияИсходный;
					Строка.ВариантОбеспеченияИсходный = ВариантТребуется;
					Строка.ОбособленноИсходный = Истина;
					ОбъектИзменен = Истина;
...
Функция ТекстИПараметрыЗапросаОбеспеченияЗаказовНаОтгрузку(ИмяВременнойТаблицы)
	...
	ТекстЗапроса =
		...
		|										ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.ОтложитьОбеспечение)) ТОГДА
		|									
		...
		|							
		|					КОГДА СостояниеЗаказа.Состояние = ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.ОтложитьОбеспечение) ТОГДА
		|							
		|						ВЫБОР КОГДА СостояниеЗаказа.ЖелаемаяДатаОтгрузки < &НачалоТекущегоДня ТОГДА
		|									&НачалоТекущегоДня
		|								ИНАЧЕ СостояниеЗаказа.ЖелаемаяДатаОтгрузки
		|							КОНЕЦ
		|							
...
Функция ОстаткиДляЗаписанногоДокумента(МенеджерВременныхТаблиц)
	...
	Запрос.Текст =
		...
		|		СостояниеЗаказа.Распределено               КАК Доступно
		|	ИЗ
		|		ВыделенныеЗаказы КАК Заказы
		|			
...
Процедура ДобавитьВременныеТаблицыДляПолученияОстатков(Запрос, ОбособленныеИНеобособленныеОстатки, ТаблицаОформлено, КорректировкаДвижений, СторнироватьОтгрузитьВСтроке)
	...
	Текст =
		...
		|	СУММА(Набор.Распределено) КАК ВНаличии
		|ИЗ(
		|	ВЫБРАТЬ
		...
		|		РаспределениеЗапасов.Распределено КАК Распределено
		|	ИЗ
		|		РегистрСведений.РаспределениеЗапасов КАК РаспределениеЗапасов
		|	ГДЕ
...
Функция ОстаткиДляПодбораСкладов(Запрос, СкладыГруппы, КоэффициентУпаковки)
	...
	Запрос.Текст =
		...
		|	СостояниеЗаказа.Склад                      КАК Склад,
		|	СостояниеЗаказа.Назначение                 КАК Назначение,
		|	СУММА(СостояниеЗаказа.Распределено)        КАК Количество
...
//   * ВариантОбеспечения - ПеречисленияСсылка.ВариантыОбеспечения -
//   * Склад - СправочникСсылка.Склады -
//   * Идентификатор - Число -
//   * Строка - ДанныеФормыЭлементКоллекции, ТабличнаяЧастьСтрока -
//
Функция ТаблицаДляУстановкиДействий(КодДействия, ЗаполнятьЦелымиУпаковками, Коллекция, ВыделенныеСтроки, ДанныеСтрок, КоэффициентыУпаковок, ДанныеУчета)
	...
	Для Каждого ИдентификаторСтроки Из ВыделенныеСтроки Цикл
		...
		ВариантОбеспечения = ?(КодДействия = "ДЕЙСТВИЕ_КОБЕСПЕЧЕНИЮ",  Перечисления.ВариантыОбеспечения.Требуется,
		...
		Иначе
			...
			Если НайденныеСтрокиОстатка.Количество() > 0 Тогда
				...
				Если СтрокаОстатка.Количество >= Строка.Количество Тогда // количество может быть округлено для мерных упаковок.
					
					КоличествоУпаковок = Строка.КоличествоУпаковок;
					СтрокаОстатка.Количество = СтрокаОстатка.Количество - Строка.Количество;
					
					
				Иначе
					
					ОстатокУпаковок = Окр(СтрокаОстатка.Количество / Коэффициент, 3, РежимОкругления.Окр15как10);
					КоличествоУпаковок = Мин(ОстатокУпаковок, Строка.КоличествоУпаковок);
...
Функция ОстаткиДляТаблицыДокумента(МенеджерВременныхТаблиц, КэшОстатков, ТаблицаИзменений)
	...
	Текст =
		...
		|	СУММА(Набор.Распределено) КАК РаспределеноНаСкладеСводно
	...
	Если КэшОстатков = Неопределено Тогда
		...
		Текст =
			...
			|		ИЛИ ЕСТЬNULL(НастройкаКонтроляОбеспечения.ПерераспределятьСкладскиеЗапасы, ЛОЖЬ) КАК ПерераспределятьСкладскиеЗапасы,
			|	ЕСТЬNULL(НастройкаКонтроляОбеспеченияХарактеристика.КонтролироватьСвободныеОстатки,
			|			ЕСТЬNULL(НастройкаКонтроляОбеспеченияНоменклатура.КонтролироватьСвободныеОстатки,
	...
	Иначе
		...
		Текст =
			...
			|							ЕСТЬNULL(НастройкаКонтроляОбеспеченияНоменклатура.КонтролироватьСвободныеОстатки,
			|								ЕСТЬNULL(НастройкаКонтроляОбеспечения.КонтролироватьСвободныеОстатки, ЛОЖЬ)))
			|						ИЛИ ЕСТЬNULL(НастройкаКонтроляОбеспечения.ПерераспределятьСкладскиеЗапасы,
			|									Таблица.Номенклатура.ТипНоменклатуры В(
			|											ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Товар),
			|											ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.МногооборотнаяТара))
			|										И Таблица.Склад = ЗНАЧЕНИЕ(Справочник.Склады.ПустаяСсылка)
...
Функция ТаблицаОбязательностиЗаполненияПоВременнойТаблицеДанныхДокумента(Запрос)
	...
	Запрос.Текст =
		...
		|						ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.Отгрузить),
		|						ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.СоСклада))
		|					ИЛИ ТаблицаДокумента.ВариантОбеспечения = ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.Требуется)
