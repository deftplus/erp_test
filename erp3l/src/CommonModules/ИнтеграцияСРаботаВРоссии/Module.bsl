...
Процедура ВыполнитьПередачуПечатныхФорм() Экспорт
	...
	ЗаданияКИсполнению = РегистрыСведений.ЗапланированныеДействияСПечатнымиФормами.ФайлыКОбработке(
		Ложь, Перечисления.ДействияСПечатнымиФормами.ПередатьНаРаботаВРоссии);
	
	СписокЗаданий = ЗаданияКИсполнению.Получить(Перечисления.ДействияСПечатнымиФормами.ПередатьНаРаботаВРоссии);
			...
			Если Не КлючиРезультата.Свойство("documentId", ИдентификаторДокумента)
				...
				ВызватьИсключение
					НСтр("ru = 'Ошибка передачи документа на портал ""Работа в России"": В ответе портала отсутствует идентификатор документа.';
						|en = 'An error occurred while transferring the document to ""Work in Russia"": The document ID is missing in the portal response.'");
		...
		Иначе
			ВызватьИсключение СтрШаблон(
				НСтр("ru = 'Ошибка передачи документа на портал ""Работа в России"" (Код: %1)';
					|en = 'An error occurred while transferring the document to ""Work in Russia"" (Code: %1)'"),
	...
	Исключение
		...
		ВызватьИсключение СтрШаблон(
			НСтр("ru = 'Ошибка передачи документа на портал ""Работа в России"": %1';
				|en = 'An error occurred while transferring the document to ""Work in Russia"": %1'"),
...
Процедура УдалитьФайлИзОбработки(ПрисоединенныйФайл)
	...
	РегистрыСведений.ЗапланированныеДействияСПечатнымиФормами.УдалитьФайлыИзОбработки(
		...
		Перечисления.ДействияСПечатнымиФормами.ПередатьНаРаботаВРоссии);
			...
			Если ОтветСервиса.КодСостояния <> 200 Тогда
				ВызватьИсключение СтрШаблон(
					НСтр("ru = 'Ошибка передачи ЭЦП документа на портал ""Работа в России"" (Код: %1)';
						|en = 'An error occurred while transferring an electronic signature of the document to ""Work in Russia"" (Code: %1)'"),
					ОтветСервиса.КодСостояния);
			КонецЕсли;
	...
	Исключение
		...
		ВызватьИсключение СтрШаблон(
			НСтр("ru = 'Ошибка передачи ЭЦП документа на портал ""Работа в России"": %1';
				|en = 'An error occurred while transferring an electronic signature of the document to ""Work in Russia"": %1'"),
			ПодробноеПредставлениеОшибки(Ошибка));
...
Функция НастройкиАвторизацииЗаданы(Настройки = Неопределено, ВызыватьИсключение = Ложь)
	...
	Если Не НастройкиЗаданы Тогда
		...
		ТекстЗаписи = НСтр("ru = 'Интеграция с Работа в России.Не заданы параметры авторизации';
							|en = 'Integration with Work in Russia. Authorization parameters are not specified'", ОбщегоНазначения.КодОсновногоЯзыка());
