...
Функция НайденЗапретИзмененияДанных(Знач ДанныеДляПроверки, ПараметрыСообщенияОЗапрете,
	...
	ЗаголовокОшибки =
		НСтр("ru = 'Ошибка в функции НайденЗапретИзмененияДанных общего модуля ДатыЗапретаИзменения.';
			|en = 'PeriodClosingDates.PeriodEndClosingFound function error.'")
		+ Символы.ПС
		+ Символы.ПС;
		...
		Если СвойстваРаздела = Неопределено Тогда
			ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'В параметре ДанныеДляПроверки указан несуществующий раздел ""%1"".';
					|en = 'DataToCheck parameter contains non-existent section: ""%1"".'"),
				Строка.Раздел);
		КонецЕсли;
	 ...
	 Или ИсточникиДанных.Количество() = 0 Тогда
		...
		ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Для таблицы ""%1""
			           ...
			           |en = 'For the ""%1""
			           |table in the FillDataSourcesForPeriodClosingCheck procedure
			           | of the PeriodClosingDatesOverridable
			           |common module, data sources required to check closing dates are not specified.'"),
			Таблица);
...
Функция ТекущиеИсточникиДанныхДляПроверкиЗапретаИзменения(СвойстваРазделов)
	...
	ЗаголовокОшибки =
		НСтр("ru = 'Ошибка в процедуре ЗаполнитьИсточникиДанныхДляПроверкиЗапретаИзменения
		           |общего модуля ДатыЗапретаИзмененияПереопределяемый.';
		           |en = 'PeriodClosingDatesOverridable.FillDataSourcesForPeriodClosingCheck
		           |procedure error.'")
		+ Символы.ПС
		+ Символы.ПС;
			...
			Если СвойстваРаздела = Неопределено Тогда
				ВызватьИсключение ЗаголовокОшибки + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Для проверки запрета изменения в источнике данных указан несуществующий
					           ...
					           |en = 'The period-end closing date section ""%1""
					           |that is specified for table ""%2"" in the data source does not exist (see PeriodClosingDatesOverridable.FillDataSourcesForPeriodClosingCheck).'"),
