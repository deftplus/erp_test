...
Функция ПолучитьДанныеДляЗаполненияДокумента(ПодключаемоеОборудование, ТаблицаТоваров, ТаблицаШтрихкодовАкцизов, ЗагрузкаАкцизныхМарок)
	...
	|
	|ИТОГИ ПО
	|	Товары.НомерСмены, Товары.УникальныйИдентификатор";
...
Функция ЗаполнитьОтчетОРозничныхПродажах(ПараметрыПодключаемогоОборудования, ПараметрыКассовойСмены, ВыборкаПоСменам, ТаблицаАкцизныеМарки, ТаблицаОплат, ЗагрузкаАкцизныхМарок)
	...
	СтавкаНДСПоНалогообложению = УчетНДСУП.СтавкаНДСПоТипуНалогообложения(ДокументОбъект.НалогообложениеНДС);
			...
			Пока ВыборкаПоТоварам.Следующий() Цикл
				...
				Если ВыборкаПоТоварам.ТипНоменклатуры <> Перечисления.ТипыНоменклатуры.Набор Тогда
					...
					Если ЗначениеЗаполнено(НоваяСтрока.Номенклатура) Тогда
						...
						РеквизитыНоменклатуры.Вставить("СтавкаНДС", ВыборкаПоТоварам.СтавкаНДС);
						РеквизитыНоменклатуры.Вставить("ПодакцизныйТовар", ВыборкаПоТоварам.ПодакцизныйТовар);
						РеквизитыНоменклатуры.Вставить("ТипНоменклатуры", ВыборкаПоТоварам.ТипНоменклатуры);
				...
				Для Каждого ВыборкаПоТоварам Из ТаблицаНаборов Цикл
					...
					Для Каждого СтрокаТЧ Из Данные Цикл
						...
						Если ЗначениеЗаполнено(НоваяСтрока.Номенклатура) Тогда
							...
							РеквизитыНоменклатуры = Новый Структура();
							РеквизитыНоменклатуры.Вставить("СтавкаНДС", ВыборкаПоТоварам.СтавкаНДС);
							РеквизитыНоменклатуры.Вставить("ПодакцизныйТовар", СтрокаТЧ.ПодакцизныйТовар);
							РеквизитыНоменклатуры.Вставить("ТипНоменклатуры", Перечисления.ТипыНоменклатуры.Набор);
