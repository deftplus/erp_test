...
Функция ТекстЗапросаСпецификацийНоменклатуры(ПараметрыЗапроса, ПараметрыВыбораСпецификаций) Экспорт
	...
	Иначе
		...
		|
		|	И &ТекстУсловиеИсключитьПроизводствоНаСтороне
		|
		|;
		|////////////////////////////////////////////////////////////////////////////////
		|";
	...
	ТекстУсловиеОтбораПоУпаковке = ТекстУсловияОтбораПоУпаковкеПриВыбореСпецификаций(
		ТекстПоляНоменклатура, ТекстПоляНоменклатураСпецификации, ТекстПоляУпаковкаСпецификации);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "И &ТекстУсловиеОтбораПоУпаковке", ТекстУсловиеОтбораПоУпаковке);
	
	Если ПараметрыВыбораСпецификаций.Свойство("СтрокаПоиска") Тогда
		ТекстУсловиеОтбораПоНаименованию = "
		|	И (&ТекстПоляСпецификация.Наименование ПОДОБНО &СтрокаПоиска
		|		ИЛИ &СтрокаПоиска = """")";
	Иначе
		ТекстУсловиеОтбораПоНаименованию = "";
	КонецЕсли;
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "И &ТекстУсловиеОтбораПоНаименованию", ТекстУсловиеОтбораПоНаименованию);
	
	Если ПараметрыВыбораСпецификаций.Свойство("ИсключитьПроизводствоНаСтороне")
		И ПараметрыВыбораСпецификаций.ИсключитьПроизводствоНаСтороне Тогда
		ТекстУсловиеИсключитьПроизводствоНаСтороне = "
		|	И НЕ ИСТИНА В (
		|		ВЫБРАТЬ ПЕРВЫЕ 1
		|			ИСТИНА
		|		ИЗ
	...
	Иначе
		ТекстУсловиеИсключитьПроизводствоНаСтороне = "";
	КонецЕсли;
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "И &ТекстУсловиеИсключитьПроизводствоНаСтороне", ТекстУсловиеИсключитьПроизводствоНаСтороне);
...
Функция ПостроитьДеревоОператоров() Экспорт
	...
	ПредставлениеГруппы = НСтр("ru = 'Прочие функции';
								|en = 'Other functions'");
	ГруппаОператоров = РаботаСФормулами.ДобавитьГруппуОператоров(Дерево, "ПрочиеФункции", ПредставлениеГруппы);
				...
				Если ПараметрыАвтовыбора.ПолучитьСведенияАвтовыбора Тогда
					СтрокаНабораДанных.СведенияАвтовыбора =
						СтрШаблон(НСтр("ru = 'Характеристику не удалось подобрать по свойствам: %1';
										|en = 'Failed to select a variant by properties: %1'"),
...
Процедура ОбработатьВыборХарактеристикиПоАлгоритму(НаборДанных, ПараметрыАвтовыбора)
	...
	ШаблонОшибкиОписания   = НСтр("ru = 'Не найден алгоритм автовыбора характеристики: %1';
									|en = 'The algorithm for autoselection of the variant is not found: %1'");
	ШаблонОшибкиВыполнения = НСтр("ru = 'Не удалось подобрать характеристику по алгоритму: %1';
									|en = 'Failed to select the variant by the algorithm: %1'");
...
Процедура ДополнитьОписаниеИсточниковСоставомОперандов(ОписаниеИсточников, Объект, ИмяТЧ, ДанныеСтроки = Неопределено) Экспорт
	...
	Для каждого КлючИЗначение Из ОписаниеИсточников Цикл
		...
		Если ОписаниеИсточника.Коллекция Тогда
			...
			Для каждого Строка Из Таблица Цикл
				...
				Для каждого Реквизит Из ОписаниеИсточника.Реквизиты Цикл
					...
					СвойстваОперанда.Вставить("НомерСтроки", Строка.НомерСтроки);
					СвойстваОперанда.Вставить("Идентификатор", ИдентификаторСтроки + Реквизит.Идентификатор);
					СвойстваОперанда.Вставить("Представление", ПредставлениеСтроки + Реквизит.Представление);
					СвойстваОперанда.Вставить("ТипЗначения", Реквизит.ТипЗначения);
					СвойстваОперанда.Вставить("ТипОперанда", ТипыОперандов.Реквизит);
					...
					Если ОписаниеИсточника.СодержитСвойства Тогда
						...
						Для каждого Свойство Из НайденныеСвойства Цикл
							Если Свойство.ТипЗначения.СодержитТип(Тип("Число")) Тогда
								СвойстваОперанда = Новый Структура;
								СвойстваОперанда.Вставить("НомерСтроки", Строка.НомерСтроки);
								СвойстваОперанда.Вставить("Идентификатор", ИдентификаторСтроки + Свойство.Идентификатор);
								СвойстваОперанда.Вставить("Представление", ПредставлениеСтроки + Свойство.Представление);
								СвойстваОперанда.Вставить("ТипЗначения", Свойство.ТипЗначения);
								СвойстваОперанда.Вставить("ТипОперанда", ТипыОперандов.ДополнительныйРеквизит);
								ОперандыИсточника.Добавить(СвойстваОперанда);
							КонецЕсли;
						КонецЦикла;
		...
		Иначе
			...
			Для каждого Реквизит Из ОписаниеИсточника.Реквизиты Цикл
				СвойстваОперанда = Новый Структура;
				СвойстваОперанда.Вставить("Идентификатор", ОписаниеИсточника.Идентификатор+"."+Реквизит.Идентификатор);
				СвойстваОперанда.Вставить("Представление", Реквизит.Представление);
				СвойстваОперанда.Вставить("ТипЗначения", Реквизит.ТипЗначения);
				СвойстваОперанда.Вставить("ТипОперанда", ТипыОперандов.Реквизит);
				ОперандыИсточника.Добавить(СвойстваОперанда);
			КонецЦикла;
			...
			Если ИспользоватьДополнительныеРеквизитыИСведения Тогда
				...
				Если ОписаниеИсточника.СодержитСвойства Тогда
					...
					Для каждого Свойство Из НайденныеСвойства Цикл
						Если Свойство.ТипЗначения.СодержитТип(Тип("Число")) Тогда
							СвойстваОперанда = Новый Структура;
							СвойстваОперанда.Вставить("Идентификатор", ОписаниеИсточника.Идентификатор+"."+Свойство.Идентификатор);
							СвойстваОперанда.Вставить("Представление", Свойство.Представление);
							СвойстваОперанда.Вставить("ТипЗначения", Свойство.ТипЗначения);
							СвойстваОперанда.Вставить("ТипОперанда", ТипыОперандов.ДополнительныйРеквизит);
							ОперандыИсточника.Добавить(СвойстваОперанда);
						КонецЕсли;
		...
		Если ОперандыИсточника.Количество() Тогда
			...
			ТаблицаОперандов.Колонки.Добавить("ТипОперанда", Новый ОписаниеТипов("Строка"));
...
Процедура ЗаполнитьДанныеУпаковокВНабореДанных(НаборДанных, ТаблицаДанныхУпаковок) Экспорт
	...
	СтруктураУпаковкиПоУмолчанию = СтруктураДанныхУпаковки();
	
	Для каждого Строка Из НаборДанных Цикл
		ЗаполнитьЗначенияСвойств(СтруктураПоиска, Строка);
		ЗаполнитьДанныеУпаковокВСтрокеНабораДанных(Строка, ТаблицаДанныхУпаковок, СтруктураПоиска, СтруктураУпаковкиПоУмолчанию);
	КонецЦикла;
	
КонецПроцедуры

Процедура ЗаполнитьДанныеУпаковокВСтрокеНабораДанных(
	Строка, ТаблицаДанныхУпаковок, СтруктураПоиска = Неопределено, СтруктураУпаковкиПоУмолчанию = Неопределено) Экспорт
	...
	Если НайденныеСтроки.Количество() > 0 Тогда
		...
		СтрокаТаблицы.ДанныеУпаковки = ПолучитьСтруктуруДанныхУпаковки(НайденныеСтроки[0], СтруктураУпаковкиПоУмолчанию);
	...
	Если ЗначениеЗаполнено(СтрокаТаблицы) И ЗначениеЗаполнено(СтрокаТаблицы.ДанныеУпаковки) Тогда
		Строка.ДанныеУпаковки = СтрокаТаблицы.ДанныеУпаковки;
	Иначе
		Строка.ДанныеУпаковки = СтруктураДанныхУпаковки(,,СтруктураУпаковкиПоУмолчанию);
	КонецЕсли;
...
Функция ПолучитьСтруктуруДанныхУпаковки(РеквизитыУпаковки, СтруктураУпаковкиПоУмолчанию = Неопределено)
	...
	Если Числитель = 0 ИЛИ Знаменатель = 0 Тогда
		Возврат СтруктураДанныхУпаковки(,,СтруктураУпаковкиПоУмолчанию);
	Иначе
		Возврат СтруктураДанныхУпаковки(Числитель, Знаменатель, СтруктураУпаковкиПоУмолчанию);
	КонецЕсли;
	
КонецФункции

Функция СтруктураДанныхУпаковки(Числитель = 1, Знаменатель = 1, СтруктураУпаковкиПоУмолчанию = Неопределено) Экспорт
	
	Если СтруктураУпаковкиПоУмолчанию <> Неопределено
		И СтруктураУпаковкиПоУмолчанию.Числитель = Числитель
		И СтруктураУпаковкиПоУмолчанию.Знаменатель = Знаменатель Тогда
			Возврат СтруктураУпаковкиПоУмолчанию;
	Иначе
