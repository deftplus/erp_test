...
Функция ПоместитьВоВременноеХранилищеРаспределениеЗапасов(
	...
	Если Отбор.Количество() = 0 Тогда
		...
		Отбор.Добавить(Перечисления.РаспределениеЗапасовСостояния.ОтложитьОбеспечение);
	...
	ТаблицаРаспределенияЗапасов	= ОбеспечениеВДокументахСервер.РаспределениеЗапасовДляРедактируемогоДокумента(Форма, Отбор);
...
Функция ПолучитьДанныеТоваров(
	...
	ИначеЕсли Режим = 2 Тогда// заменить аналогами
		...
		ОтборПоНоменклатуре.ВариантОбеспечения = Перечисления.ВариантыОбеспечения.Требуется;
...
Функция ТекстЗапросаТаблицаОстатков(ПараметрыОтборов) Экспорт
	...
	|				ТаблицаАналогов КАК ТаблицаАналогов
	|			ГДЕ
	|				ТаблицаАналогов.Номенклатура = Таблица.Номенклатура
	|				И ТаблицаАналогов.Характеристика = Таблица.Характеристика
	|				И ТаблицаАналогов.Склад = Таблица.Склад)
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	Номенклатура,
	|	Характеристика,
...
Процедура ДополнитьТаблицуРаспределенияЗапасов(АдресВХранилище, ТаблицаЗапасов)
	...
	|	И РаспределениеЗапасов.Свободно ЕСТЬ НЕ NULL 
	|
	|СГРУППИРОВАТЬ ПО
	|	ТаблицаТоваров.Номенклатура,
	|	ТаблицаТоваров.Характеристика,
