...
Функция НастройкиКомплектаОтчетности(Банк, ИдентификаторКомплекта) Экспорт
	...
	Для каждого ОтчетXDTO Из НайденныйКомплектXDTO.Отчет Цикл
		...
		// Проверим конфигурации, для которых применяются условия.
		Если ЗначениеЗаполнено(ОтчетXDTO.СемействоКонфигураций)
		   И Не УниверсальныйОбменСБанкамиКлиентСервер.ПринадлежитСемействуКонфигураций(ТекущееСемействоКонфигураций, ОтчетXDTO.СемействоКонфигураций) Тогда
			// Отчет предназначен для другой конфигурации, для текущей конфигурации его пропускаем.
			Продолжить;
		КонецЕсли;
...
//
// Параметры:
//  ПараметрыПоискаРеглОтчетов - ТаблицаЗначений - см. ЗаполнениеФинОтчетностиВБанки.НовыеПараметрыПоискаРеглОтчетов()
//
...
Функция ТребованияБанкаXDTO(Банк)
	...
	Исключение
		...
		ИмяСобытия = НСтр("ru = 'Прочитать объект XDTO';
