...
Функция ДанныеФайла(ПрисоединенныйФайл, Знач ВызыватьИсключение = Истина) Экспорт
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Файлы.Получение файла из тома';
										|en = 'Files.Get file from volume'", ОбщегоНазначения.КодОсновногоЯзыка()),
		...
		ТекстОшибки = НСтр("ru = 'Не удалось открыть файл:
			...
			|%2.
			|Reason: binary file data was deleted. The file might have been cleaned up as unused or deleted by the antivirus software.
			...
			Если СуществующийФайл.Количество() = 0 Тогда
				...
				НесуществующийФайл.СтатусПроверки = ?(Выборка.ПометкаУдаления,
					...
					НСтр("ru = 'Отсутствуют данные в томе на диске';
						|en = 'No data in volume on the hard drive'"));
