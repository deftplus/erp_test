...
#Область ВозвратАвансаПогашениеДолга
	...
	|	0 КАК СуммаВРДт,
	|	
	|	ВЫБОР КОГДА РасчетыПоДокументам.ДокументОбъектаРасчетов ССЫЛКА Документ.ВозвратТоваровПоставщику
	|			ИЛИ РасчетыПоДокументам.ДокументОбъектаРасчетов ССЫЛКА Документ.ВыбытиеДенежныхДокументов ТОГДА
	...
	|	РасчетыПоДокументам.ГруппаФинансовогоУчета КАК АналитикаУчетаКт,
	|	НЕОПРЕДЕЛЕНО КАК МестоУчетаКт,
	|
	|	РасчетыПоДокументам.Валюта КАК ВалютаКт,
	|	РасчетыПоДокументам.Подразделение КАК ПодразделениеКт,
	|	РасчетыПоДокументам.НаправлениеДеятельности КАК НаправлениеДеятельностиКт,
	|
	|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка) КАК СчетКт,
	|	РасчетыПоДокументам.Контрагент КАК СубконтоКт1,
	|	РасчетыПоДокументам.Договор КАК СубконтоКт2,
	|	НЕОПРЕДЕЛЕНО КАК СубконтоКт3,
	|
...
#Область ВозвратДС
	...
	|
	|ГДЕ 
	|	Операция.ХозяйственнаяОперация В (ЗНАЧЕНИЕ(Перечисление.ХозяйственныеОперации.ВозвратДенежныхСредствОтПоставщика),
...
#КонецОбласти
	...
	ТекстыОтражения.Добавить(ТекстПоступлениеДолгаАвансаОтКлиентаДругойОрганизацииМежФилиальнаяДт());
	ТекстыОтражения.Добавить(ТекстПоступлениеДолгаАвансаОтКлиентаДругойОрганизацииМежФилиальнаяКт());
...
Функция ТекстПоступлениеДолгаАвансаОтКлиентаДругойОрганизацииМежФилиальнаяКт()
...
Функция ТекстПоступлениеДолгаАвансаОтКлиентаДругойОрганизацииМежФилиальнаяДт()
	
	Текст = "
	|ВЫБРАТЬ ПЕРВЫЕ 1 //// Поступление аванса от клиента/другой организации
	|
	|	Операция.Ссылка КАК Ссылка,
	|	Операция.Дата КАК Период,
	|	Операция.Организация КАК Организация,
	...
	|	НЕОПРЕДЕЛЕНО КАК СубконтоДт3,
	|
	|	ДенежныеСредстваКонтрагент.СуммаПостоплатыВВалютеПлатежа + ДенежныеСредстваКонтрагент.СуммаПредоплатыВВалютеПлатежа КАК ВалютнаяСуммаДт,
	|	0 КАК КоличествоДт,
	|	0 КАК СуммаНУДт,
	|	0 КАК СуммаПРДт,
	|	0 КАК СуммаВРДт,
