...
Процедура ПодготовитьСтруктуруОбъектовКомандЭДО(СоставКомандЭДО) Экспорт
	...
	//++ НЕ УТ		
	СоставКомандЭДО.Внутренние.Добавить("Документ.ИнвентаризацияОС");
	СоставКомандЭДО.Внутренние.Добавить("Документ.ИнвентаризацияНМА");
	СоставКомандЭДО.Внутренние.Добавить("Документ.ПеремещениеОС2_4");
	СоставКомандЭДО.Внутренние.Добавить("Документ.СписаниеОС2_4");
	СоставКомандЭДО.Внутренние.Добавить("Документ.МодернизацияОС2_4");
	СоставКомандЭДО.Внутренние.Добавить("Документ.ПринятиеКУчетуОС2_4");
	СоставКомандЭДО.Внутренние.Добавить("Документ.ПринятиеКУчетуУзловКомпонентовАмортизации");
	СоставКомандЭДО.Внутренние.Добавить("Документ.ВозвратОСИзАренды2_4");
	СоставКомандЭДО.Внутренние.Добавить("Документ.ПередачаОСВАренду2_4");
	СоставКомандЭДО.Внутренние.Добавить("Документ.ПодготовкаКПередачеОС2_4");
	СоставКомандЭДО.Внутренние.Добавить("Документ.ПеремещениеВЭксплуатации");
	СоставКомандЭДО.Внутренние.Добавить("Документ.СписаниеИзЭксплуатации");
	СоставКомандЭДО.Внутренние.Добавить("Документ.ПроизводствоБезЗаказа");
	//-- НЕ УТ		
...
Процедура СоответствиеИсходящихТиповЭлектронныхДокументовДокументамИБ(СоответствиеТиповЭлектронныхДокументовДокументамИБ) Экспорт 
	...
	СоответствиеТиповЭлектронныхДокументовДокументамИБ.Вставить(ТипыДокументов.АктВыполненныхРабот,        				НСтр("ru = 'Акт выполненных работ для клиента';
																																|en = 'Customer invoice — Services for customer'"));
	...
	СоответствиеТиповЭлектронныхДокументовДокументамИБ.Вставить(ТипыДокументов.ОтчетОСписанииКомиссионногоТовара, 		НСтр("ru = 'Отчет комитенту о списании';
																																|en = 'Purchase invoice — Consignment stock lost'"));
	...
	СоответствиеТиповЭлектронныхДокументовДокументамИБ.Вставить(ТипыДокументов.УКД,            							НСтр("ru = 'Акты, накладные, счет-фактуры (Корректировка)';
																																|en = 'Deeds, consignments, tax invoices (Adjustment)'"));
	...
	СоответствиеТиповЭлектронныхДокументовДокументамИБ.Вставить(ТипыДокументов.КаталогТоваров,         					НСтр("ru = 'Каталог товаров (формируется из Настройки отправки)';
																																|en = 'Goods directory (generated from the Sending settings)'"));
...
Процедура ЗаполнитьПараметрыЭДПоИсточнику(Источник, ПараметрыЭлектронногоДокумента) Экспорт
	...
	ИначеЕсли ТипИсточника = Тип("ДокументСсылка.КорректировкаПриобретения")
		...
		Если ХозяйственнаяОперация = Перечисления.ХозяйственныеОперации.КорректировкаПоСогласованиюСторон Тогда
			Тип = ТипыДокументов.СоглашениеОбИзмененииСтоимости;
		ИначеЕсли ТипЗнч(Источник.ДокументОснование) = Тип("ДокументСсылка.ПриобретениеУслугПрочихАктивов") Тогда
			Тип = ТипыДокументов.АктВыполненныхРабот;
	...
	ИначеЕсли ТипИсточника = Тип("ДокументСсылка.АвансовыйОтчет")
		...
		//++ НЕ УТ
		ИЛИ ТипИсточника = Тип("ДокументСсылка.ИнвентаризацияОС")
		ИЛИ ТипИсточника = Тип("ДокументСсылка.ИнвентаризацияНМА")
		ИЛИ ТипИсточника = Тип("ДокументСсылка.ПеремещениеОС2_4")
		ИЛИ ТипИсточника = Тип("ДокументСсылка.СписаниеОС2_4")
		ИЛИ ТипИсточника = Тип("ДокументСсылка.МодернизацияОС2_4")
		ИЛИ ТипИсточника = Тип("ДокументСсылка.ПринятиеКУчетуОС2_4")
		ИЛИ ТипИсточника = Тип("ДокументСсылка.ПринятиеКУчетуУзловКомпонентовАмортизации")
		ИЛИ ТипИсточника = Тип("ДокументСсылка.ВозвратОСИзАренды2_4")
		ИЛИ ТипИсточника = Тип("ДокументСсылка.ПередачаОСВАренду2_4")
		ИЛИ ТипИсточника = Тип("ДокументСсылка.ПодготовкаКПередачеОС2_4")
		ИЛИ ТипИсточника = Тип("ДокументСсылка.ПеремещениеВЭксплуатации")
		ИЛИ ТипИсточника = Тип("ДокументСсылка.СписаниеИзЭксплуатации")
		ИЛИ ТипИсточника = Тип("ДокументСсылка.ПроизводствоБезЗаказа")
		//-- НЕ УТ	
		...
		//++ НЕ УТ		
		ИЛИ ТипИсточника = Тип("ДокументОбъект.ИнвентаризацияОС")
		ИЛИ ТипИсточника = Тип("ДокументОбъект.ИнвентаризацияНМА")
		ИЛИ ТипИсточника = Тип("ДокументОбъект.ПеремещениеОС2_4")
		...
		ИЛИ ТипИсточника = Тип("ДокументОбъект.ВозвратОСИзАренды2_4")
		ИЛИ ТипИсточника = Тип("ДокументОбъект.ПередачаОСВАренду2_4")
		ИЛИ ТипИсточника = Тип("ДокументОбъект.ПодготовкаКПередачеОС2_4")
		ИЛИ ТипИсточника = Тип("ДокументОбъект.ПеремещениеВЭксплуатации")
		ИЛИ ТипИсточника = Тип("ДокументОбъект.СписаниеИзЭксплуатации")
		ИЛИ ТипИсточника = Тип("ДокументОбъект.ПроизводствоБезЗаказа")
		//-- НЕ УТ		 
		Тогда
...
Процедура ПослеЗавершенияОбменаЭлектроннымДокументом(ЭлектронныйДокумент) Экспорт
	...
	Для Каждого ЭлементМассива Из ОснованияЭлектронногоДокумента Цикл
		Если ЗначениеЗаполнено(ЭлементМассива) Тогда
			Основания.Добавить(ЭлементМассива);
		КонецЕсли;
	КонецЦикла;
...
Функция НайтиСоздатьВыкупАрендованныхОС(СтрокаДляЗагрузки, ДеревоРазбора, СсылкаНаВладельца = Неопределено, Записывать = Истина) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);

	//ДанныеДляЗагрузки = ПодготовитьСтруктуруДляВыкупаАрендованныхОС1(СтрокаДляЗагрузки, ДеревоРазбора);		
	
	Документ = СсылкаНаВладельца;
	//ЗаполнитьДокументВыкупАрендованныхОС(Документ, ДанныеДляЗагрузки, Записывать);		
	...
	Если ЗначениеЗаполнено(СсылкаНаВладельца) Тогда
		...
		Если ДокументОбъект.Проведен Тогда
			...
			Текст = НСтр("ru = 'Операция возможна только для непроведенных документов';
						|en = 'The operation is only available for unposted documents'");
		КонецЕсли;
	Иначе
		УстановитьПривилегированныйРежим(истина);
	...
	Исключение
		...
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Заполнение документа на основе ЭД.%1';
						|en = 'Populate the document based on ED. %1'", ОбщегоНазначения.КодОсновногоЯзыка()),
					Текст);
...
Функция НайтиСоздатьЗаявкуНаРасходованиеДенежныхСредств(СтрокаДляЗагрузки, ДеревоРазбора, СсылкаНаВладельца = Неопределено, Записывать = Истина) Экспорт
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьЗаявкиНаРасходованиеДенежныхСредств") Тогда
		Текст = НСтр("ru = 'Для создания документа ""Заявка на расходование денежных средств"" необходимо включить функциональную опцию ""Использовать заявки на расходование денежных средств""';
					|en = 'You must enable the ""Use expenditure requests"" functional option to create an ""Expenditure request"" document'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Текст);		
		Возврат Неопределено		
	КонецЕсли;
	...
	Если НайденСледующий Тогда
		...
		Если ЗначениеЗаполнено(РеквизитыШапки.Организация) Тогда
			НаименованиеСоставителяДокумента = СведенияОПоставщике.ПолноеНаименование + ?(ЗначениеЗаполнено(
				СведенияОПоставщике.КПП), СтрШаблон(НСтр("ru = ', ИНН/КПП %1/%2';
														|en = ', TIN/CRTR %1/%2'"), СведенияОПоставщике.ИНН,
				СведенияОПоставщике.КПП), СтрШаблон(НСтр("ru = ', ИНН %1';
		...
		Если (РеквизитыШапки.Статус <> Перечисления.СтатусыРеализацийТоваровУслуг.Отгружено) Тогда 
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Документ %1 для формирования ЭД должен быть в статусе ""Реализовано"".';
					|en = 'Document %1 used to generate ED must have ""Implemented"" status.'"), 
...
Процедура НайтиСоздатьДокументПередачаТоваров(ДеревоДанных, СсылкаНаВладельца, СпособОбработки, ОписаниеОшибки) Экспорт
	...
	Если СпособОбработки <> "ВыкупАрендованныхОС" Тогда
		ДанныеДляЗагрузки = ПодготовитьСтруктуруДляПриобретенияТоваровУслугПоПередачеТоваров(ДеревоДанных);
	Иначе
		//ДанныеДляЗагрузки = ПодготовитьСтруктуруДляВыкупаАрендованныхОС(ДеревоДанных);
	КонецЕсли;
	...
	ИначеЕсли СпособОбработки = "ВыкупАрендованныхОС" Тогда
		//ЗаполнитьДокументВыкупАрендованныхОС(СсылкаНаВладельца, ДанныеДляЗагрузки, Истина);		
	Иначе
...
Процедура ЗаполнитьДанныеУКД_ИнформацияПродавца(Основание, Настройки, Данные, Описание, Отказ) Экспорт
	...
	Иначе
		...
		Если МассивСчетовФактур.Количество() = 0 Тогда
			...
			НоваяСчетФактура.КодВидаОперации 	= НоваяСчетФактура.КодВидаОперации(Основание);
...
Функция СоздатьКонтрагентаВБД(ДанныеКонтрагента) Экспорт
	...
	Исключение
		...
		ЭлектронноеВзаимодействие.ОбработатьОшибку(НСтр("ru = 'Создание нового контрагента';
...
Функция СоздатьПерезаполнитьНоменклатуруПоставщика(СтрокаОбъекта, ДеревоРазбора) Экспорт
	...
	Исключение
		Текст = НСтр("ru = 'Создание элемента справочника ""Номенклатура поставщиков"".';
					|en = 'Create an item of the ""Supplier products"" catalog.'", ОбщегоНазначения.КодОсновногоЯзыка());
		ЗаписьЖурналаРегистрации(Текст, УровеньЖурналаРегистрации.Ошибка,,,ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ВызватьИсключение;
	КонецПопытки;
	...
	Исключение
		Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Создание элемента справочника %1.';
						|en = 'Creating catalog item %1.'", ОбщегоНазначения.КодОсновногоЯзыка()),
					ВидРС); 
		ЗаписьЖурналаРегистрации(Текст, УровеньЖурналаРегистрации.Ошибка,,,ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));		
		ВызватьИсключение;
	КонецПопытки;
...
Процедура СведенияФизЛицаПользователя(Пользователь, Организация, ДанныеФизЛица) Экспорт
	...
	Если ТипЗнч(Пользователь) = Тип("СправочникСсылка.Пользователи") Тогда
		...
		ПараметрыПолученияДанных.НачалоПериода = ТекущаяДатаСеанса();
		ПараметрыПолученияДанных.ОкончаниеПериода = ТекущаяДатаСеанса();
		...
		КадровыеДанные = КадровыйУчет.КадровыеДанныеСотрудников(
			Истина, Сотрудники.ВыгрузитьКолонку("Сотрудник"), ПолучаемыеДанные, ТекущаяДатаСеанса());
			
		Если КадровыеДанные.Количество() > 0 Тогда
			ДанныеФизЛица.Должность = Строка(КадровыеДанные[0].Должность);
		КонецЕсли;
...
Функция ВидыЭДПоИмениОбъекта(ПолноеИмяОбъектаМетаданных) Экспорт
	...
	ТекстСообщенияОбОшибке = НСтр("ru = 'Не удалось определить вид документа';
									|en = 'Cannot determine a document kind'");
...
Процедура ЗаполнитьДанныеУПД2019_ИнформацияПродавца(Основание, Настройки, Данные, Описание, Отказ) Экспорт
	...
	Если СтруктураЭД.Функция <> "ДОП" Тогда
		...
		ЗаполнитьДанныеПервичногоДокументаУПД_2019(СтруктураДанных, СтруктураЭД, Данные, Отказ);
	Иначе
	...
	Если ЕстьДокументыУчета Тогда 
		...
		Иначе
			...
			СодержаниеОперации = НСтр("ru = 'Товары (работы, услуги, права) приняты с расхождениями (претензией)';
										|en = 'Goods (works, services, rights) are received with discrepancies (complaint)'");
...
Процедура ЗаполнитьДанныеУКД2020_ИнформацияПродавца(Основание, Настройки, Данные, Описание, Отказ) Экспорт
	...
	Если ВидДокумента = ВсеВидыДокументов.УКД Тогда		
		СтруктураЭД.Вставить("Функция", "КСЧФДИС");		
		ЗаполнитьДанныеСчетаФактурыУКД(СтруктураДанных, СтруктураЭД, Данные);		
		ЗаполнитьДанныеПервичногоДокументаУКД_2020(СтруктураДанных, СтруктураЭД, Данные);		
	ИначеЕсли ВидДокумента = ВсеВидыДокументов.КорректировочныйСчетФактура Тогда		
		СтруктураЭД.Вставить("Функция", "КСЧФ");
		ЗаполнитьДанныеСчетаФактурыУКД(СтруктураДанных, СтруктураЭД, Данные);
		ЗаполнитьДанныеПервичногоДокументаУКД_2020(СтруктураДанных, СтруктураЭД, Данные);		
		...
		Если ЗначениеЗаполнено(СтрокаТовары.КоличествоПоРНПТ) Тогда
			...
			ЭлектронноеВзаимодействие.ДобавитьВТаблицуОбработкуОшибкиЧерезСообщениеПользователю(НовСтрока.СведенияОПрослеживаемости, "ЕдиницаИзмеренияКод",,,
				НСтр("ru = 'Не заполнена единица измерения ТН ВЭД';
					|en = 'FEACN unit of measure is not populated'"));			
			...
			ЭлектронноеВзаимодействие.ДобавитьВТаблицуОбработкуОшибкиЧерезСообщениеПользователю(НовСтрока.СведенияОПрослеживаемости, "Количество",,,
				НСтр("ru = 'Не заполнено количество в единицах прослеживаемости';
					|en = 'Quantity in traceability units cannot be empty'"));				
...
Процедура ЗаполнитьДанныеАктОРасхождениях_ИнформацияПокупателя(Знач Основание, Знач Настройки, Данные, Описание, Отказ) Экспорт
	...
	НаименованиеДокумента = НСтр("ru = 'Акт об установленном расхождении по количеству и качеству при приемке товарно-материальных ценностей';
								|en = 'Certificate of quantity and quality discrepancy upon inventory'");
	...
	Если ЗначениеЗаполнено(ВыборкаШапка.Организация) Тогда
		...
		СоставительДокументаНаименование = Покупатель.ПолноеНаименование
			...
			СтрШаблон(НСтр("ru = ', ИНН/КПП %1/%2';
							|en = ', TIN/CRTR %1/%2'"), Покупатель.ИНН, Покупатель.КПП),
			СтрШаблон(НСтр("ru = ', ИНН %1';
							|en = ', TIN %1'"), Покупатель.ИНН));
	...
	ОписаниеСобытия = НСтр("ru = 'При приемке указанных в документе ценностей (результатов работ) установлены расхождения с сопроводительными документами.';
							|en = 'The discrepancies with accompanying documents were found upon acceptance of the assets (results of work) indicated in the document.'");
	ЭлектронноеВзаимодействие.ЗаполнитьЗначениеРеквизитаВДереве(
...
Процедура СохранитьДанныеОбъектаВБД(СтрокаДляЗагрузки, ДеревоРазбора, ПараметрыОбработки, ДополнительныеДанные, НайденныйОбъект) Экспорт
	...
	ИначеЕсли СтрокаДляЗагрузки.ВидЭД = ТипыДокументов.ТоварнаяНакладная
		И СпособОбработки = "ВыкупАрендованныхОС" Тогда
		НайденныйОбъект = НайтиСоздатьВыкупАрендованныхОС(СтрокаДляЗагрузки, ДеревоРазбора, СсылкаНаВладельца, Записывать);
...
Процедура ПодготовитьДанныеПоПередачеТоваровМеждуОрганизациями(СсылкаНаОбъект, ДеревоЭД, Отказ) Экспорт
	...
	ЭлектронноеВзаимодействие.ДобавитьВТаблицуОбработкуОшибкиЧерезСообщениеПользователю(
		ТаблицаТоваров, "Наименование",,, НСтр("ru = 'Не указано наименование товара в табличной части';
												|en = 'Goods name is not specified in the tabular section'"));
...
Процедура СпособыОтраженияВУчетеТипаЭлектронногоДокумента(ТипДокумента, СписокСпособовОбработки) Экспорт 
	...
	Если ТипДокумента = ТипыДокументов.АктВыполненныхРабот Тогда
		СписокСпособовОбработки.Добавить("ПриобретениеУслугПрочихАктивов", НСтр("ru = 'Приобретение услуг и прочих активов';
																				|en = 'Vendor invoice — Services and Assets'"), Истина);
	...
	ИначеЕсли ТипДокумента = ТипыДокументов.ПередачаТоваровМеждуОрганизациями Тогда	
		СписокСпособовОбработки.Добавить("ПередачаТоваровМеждуОрганизациями", НСтр("ru = 'Передача товаров между организациями';
																					|en = 'Customer invoice  — Intercompany'"), Истина);
	ИначеЕсли ТипДокумента = ТипыДокументов.ЗаказТовара Тогда
	...
	ИначеЕсли ТипДокумента = ТипыДокументов.СчетНаОплату Тогда
		СписокСпособовОбработки.Добавить("ЗаявкаНаРасходованиеДенежныхСредств", НСтр("ru = 'Заявка на расходование денежных средств';
																					|en = 'Payment request'"), Истина);
	ИначеЕсли ТипДокумента = ТипыДокументов.ОтчетОПродажахКомиссионногоТовара Тогда
		СписокСпособовОбработки.Добавить("ОтчетКомиссионера", НСтр("ru = 'Отчет комиссионера (агента) о продажах';
																	|en = 'Consignment report from Customer'"), Истина);
	ИначеЕсли ТипДокумента = ТипыДокументов.ОтчетОСписанииКомиссионногоТовара Тогда
		СписокСпособовОбработки.Добавить("ОтчетКомиссионераОСписании", НСтр("ru = 'Отчет комиссионера о списании';
																			|en = 'Write-off report from commission agent'"), Истина);
	ИначеЕсли ТипДокумента = ТипыДокументов.АктОРасхождениях Тогда
		СписокСпособовОбработки.Добавить("АктОРасхожденияхПослеОтгрузки", НСтр("ru = 'Акт о расхождениях после реализации';
																				|en = 'Discrepancy certificate after sale'"), Истина);
	ИначеЕсли ТипДокумента = ТипыДокументов.КаталогТоваров Тогда
		СписокСпособовОбработки.Добавить("НоменклатураПоставщиков", НСтр("ru = 'Номенклатура поставщиков';
																		|en = 'Vendor part numbers'"), Истина);
	КонецЕсли;
...
Процедура РазобратьФайлРеквизитовКонтрагента(СсылкаНаФайл, СтруктураВозврата, РезультатРазбора, ОшибкаРазбора = "") Экспорт
	...
	Исключение
		...
		ТекстСообщения = КраткоеПредставлениеОшибки(ИнформацияОбОшибке()) + " (" + НСтр("ru = 'подробности см. в Журнале регистрации';
																						|en = 'for more information, see the Event log'") + ").";
		ТекстОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ЭлектронноеВзаимодействие.ОбработатьОшибку(НСтр("ru = 'Разбор файла с реквизитами контрагента';
	...
	ИначеЕсли Наименование = "СпособРасчета" Тогда
		...
		Если ПредставлениеПеречисления = "От суммы" Тогда
			ПредставлениеПеречисления = НСтр("ru = 'Процент от суммы продажи';
											|en = 'Percent rate of the revenue'");
		ИначеЕсли ПредставлениеПеречисления = "От разницы" Тогда
			ПредставлениеПеречисления = НСтр("ru = 'Процент от разности суммы продажи и суммы комитента';
											|en = 'Percent rate of the sales markup'");
		КонецЕсли;	
	Иначе
	...
	Иначе
		...
		Исключение
			Текст = НСтр("ru = 'Создание элемента справочника ""Виды цен поставщиков"".';
						|en = 'Create an item of the ""Supplier price types"" catalog.'", ОбщегоНазначения.КодОсновногоЯзыка());
			ЗаписьЖурналаРегистрации(Текст, УровеньЖурналаРегистрации.Ошибка,,,ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ВызватьИсключение;
		КонецПопытки;
	КонецЕсли;
...
Процедура ЗаполнитьДанныеПервичногоДокументаУКД_2020(СтруктураДанных, СтруктураЭД, ДеревоДанных)
	...
	Иначе
		...
		ИсходныйДокументСтрока = ИсходныеДокументы.Добавить();
		ИсходныйДокументСтрока.НомерИсходногоДокумента = Основание.Номер;
		...
		Если ЗначениеЗаполнено(РезультатПоИсходнымДанным.НомерСчетаФактуры)
			...
			Если ЗначениеЗаполнено(РезультатПоИсходнымДанным.НомерИсправленияСчетаФактуры)
				...
				ОписаниеОснования = ОписаниеОснования + ", " + СтрШаблон(НСтр("ru = 'с учетом исправления № %1 от %2';
																				|en = 'considering correction No. %1 dated %2'"),
...
Функция СоздатьНовогоКонтрагента(СтрокаДляЗагрузки, ДеревоРазбора, СсылкаНаВладельца)
	...
	Исключение
		...
		ТекстОшибки = НСтр("ru = 'Не удалось загрузить контрагента';
							|en = 'Cannot import the counterparty'");
		ЗаписьЖурналаРегистрации(
				ТекстОшибки,
				УровеньЖурналаРегистрации.Ошибка,,,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
...
//
Процедура ПолучитьАдресСтруктурой(СтруктураАдреса, СтруктураПараметров, ВидКонтрагента = "Контрагент", ВидАдреса = "Юр", ТекстОшибки = "")
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Устанавливает константу ""Использовать обмен электронными документами"".';
									|en = 'Sets up the constant ""Use electronic document exchange""'");
