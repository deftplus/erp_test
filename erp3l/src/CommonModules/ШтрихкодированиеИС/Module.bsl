				...
				ОбщегоНазначения.СообщитьПользователю(
					СтрШаблон(
						НСтр("ru = 'GTIN %1 найден в составе кода маркировки потребительской упакови.
						           ...
						           |en = 'GTIN %1 найден в составе кода маркировки потребительской упакови.
						           |В регистре Описание GTIN ИС %1 соответствует виду упаковки: %2.
						           |GTIN логистических, групповых и потребительских упаковок должны быть различными.'"),
			...
			Если Не СтрокаДанных.НоменклатураСопоставленаПоУПД Тогда
				...
				Если Не (РезультатПоискаУникален = Неопределено И ВариантовВыбора = 1) Тогда
					...
					ИначеЕсли СтрокаДанных.ЕстьВПулеКодовМаркировки
							И РезультатПоискаУникален = Неопределено
							И ЗначениеЗаполнено(СтрокаДанных.Номенклатура)
							И (СтрокаИнформации.Номенклатура <> СтрокаДанных.Номенклатура
...
Функция ОбработатьУточнениеДанныхДляМаркированногоТовара(Форма, РезультатВыбора, РезультатОбработки, КэшированныеЗначения)
	...
	Для Каждого КлючЗначение Из ДанныеВыбора Цикл
		...
		Если ДанныеВыбора.ЧастичноеВыбытие
			И ИмяСвойства = "Номенклатура" Тогда
			ДанныеШтрихкода.Характеристика = ДанныеВыбора.Характеристика;
			ДанныеШтрихкода.Упаковка       = ИнтеграцияИС.ПустоеЗначениеОпределяемогоТипа("Упаковка");
		КонецЕсли;
