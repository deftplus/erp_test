	...
	Для Каждого СтруктураПараметров Из МассивСтруктур Цикл
		...
		Если СсылкаЯвляетсяОбъектомРасчетов(Объект, СтруктураПараметров) Тогда
			Если СтруктураПараметров.ОрганизацияВСтроках Тогда
				...
				ИмяРеквизита = Имена[Имена.ВГраница()];
				ТЧОрганизаций = ОбщегоНазначенияУТКлиентСервер.ДанныеПоПути(Объект, СтруктураПараметров.ПутьКДаннымТЧОрганизаций);
				Для Каждого стр Из ТЧОрганизаций Цикл
					НовСтр = ТаблицаСторон.Добавить();
					НовСтр.Организация = стр[ИмяРеквизита];
...
Функция ПроверитьЗаполнитьОбъектРасчетовПоСтруктуре(Объект, СтруктураПараметров, ВернутьВсеОбъекты = Ложь, РежимЗаписи = Неопределено) Экспорт
	...
	Если ОрганизацияВСтроках ИЛИ КонтрагентВСтроках ИЛИ ПартнерВСтроках Тогда
		...
		Если ОрганизацияВСтроках Тогда
			ТЧ = ОбщегоНазначенияУТКлиентСервер.ДанныеПоПути(Объект, СтруктураПараметров.ПутьКДаннымТЧОрганизаций); // ТабличнаяЧасть
		...
		Для Каждого Стр Из ТЧ Цикл
			...
			Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Стр, "ОбъектРасчетов")
				И ЗначениеЗаполнено(Стр.ОбъектРасчетов)
...
// Возвращаемое значение:
// 	Массив - Массив объектов расчетов.
...
Функция ВсеОбъектыРасчетовСгенерированы(Очередь) Экспорт
		...
		ОбработчикиГенерации.Добавить("Документ.ОтчетОператораСистемыПлатон");
		//-- Локализация
		ОбработчикиГенерации.Добавить("Документ.ВыкупПринятыхНаХранениеТоваров");
		ОбработчикиГенерации.Добавить("Документ.ВыкупТоваровХранителем");
		ОбработчикиГенерации.Добавить("Документ.ЗаказПереработчику");
		ОбработчикиГенерации.Добавить("Документ.СписаниеПринятыхНаХранениеТоваров");
				...
				Иначе
					ВызватьИсключение (СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
									НСтр("ru = 'Не удалось сгенерировать объект расчетов для элемента: %1.';
										|en = 'Cannot generate AR/AP object for element: %1.'"),
									СсылкаНаЭлемент));
				КонецЕсли;
			КонецЕсли;
				...
				Если СтрокаОбъектаРасчетов.Количество > 1 Тогда
					ВызватьИсключение(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'По ключу ""%1, %2, %3, %4"", найдено несколько объектов расчетов.';
							|en = 'Multiple AR/AP objects were found by key ""%1, %2, %3, %4"".'"),
...
Процедура ЗаполнитьОбъектРасчетов(Объект, МассивСтруктур, ОбновлениеДанных = Ложь, РежимЗаписи = Неопределено, ТабЧастиВидыЗапасов = Неопределено)
	...
	Для Каждого СтруктураПараметров Из МассивСтруктур Цикл
		...
		Если (РеквизитОбъектРасчетов = Неопределено ИЛИ НЕ СтруктураПараметров.ИзменяетРасчеты) И НЕ ЭтоДоговор Тогда
			Продолжить;
		КонецЕсли;
						...
						Если Не ЗначениеЗаполнено(ОбъектРасчетов) И Не (ТипЗнч(Объект) = Тип("ДокументОбъект.ВозвратТоваровОтКлиента") И ЗначениеЗаполнено(Объект.ЗаявкаНаВозвратТоваровОтКлиента)
							...
							ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
								НСтр("ru = 'Не удалось заполнить объект расчетов в документе: %1';
									|en = 'Cannot fill in the AR/AP object in the document: %1'"),
									Ссылка);
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Исправляет некорректные и задвоенные движения в расчетных регистрах.';
									|en = 'Corrects incorrect and duplicate register records in the calculation registers.'");
	...
	НоваяСтрока = Обработчик.ПриоритетыВыполнения.Добавить();
	НоваяСтрока.Процедура = "Документы.РегистраторРасчетов.ОбработатьДанныеДляПереходаНаНовуюВерсию";
	НоваяСтрока.Порядок = "После";
...
Функция ЭкземплярыТиповОбъектовРасчетов()
	...
	ЭкземплярыОбъектов.Добавить(Документы.ОтчетОператораСистемыПлатон.ПустаяСсылка());
...
Функция УсловиеГенерацииОбъектаРасчетов(ЭкземплярОбъекта)
	...
	ИначеЕсли ПараметрыВзаиморасчетов.ЭтоДоговор Тогда
				...
				Если ТребуетсяГенерацияОР Тогда
					...
					Если Не ЗначениеЗаполнено(НовыйОбъектРасчетов) Тогда
						Шаблон = НСтр("ru = 'Не удалось создать объект расчетов для документа: ""%1""';
										|en = 'Cannot create AR/AP object for the document: ""%1""'");
							ТекстСообщения = 
							СтрШаблон(Шаблон,
								Объект.Ссылка);
		...
		Если Не ЗначениеЗаполнено(СтрокаРасшифровки.ОбъектРасчетов) 
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось заполнить объект расчетов в документе: %1, в табличной части %2, по заказу %3';
					|en = 'Cannot fill in the AR/AP object in the document: %1, in the table %2, by the order %3'"),
					Ссылка,
			...
			Если Не ЗначениеЗаполнено(ОбъектРасчетов) 
				...
				ИЛИ Не ЗначениеЗаполнено(ДатаНачалаПересчета) Тогда
					ТекстИсключения = НСтр("ru = 'Некорректные параметры для перезаполнение расчетных регистров.';
											|en = 'Invalid parameters for calculation registers re-filling.'");
