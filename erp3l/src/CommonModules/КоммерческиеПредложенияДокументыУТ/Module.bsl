...
#Область КоммерческиеПредложенияКлиентуОбработатьДокументыДляПереводаНаВерсиюБЭД
	...
	Обработчик.Комментарий = НСтр("ru = 'Обновление документа ""Коммерческое предложение клиенту""';
									|en = 'Update the ""Sales quotation for customer"" document'");
...
Процедура КоммерческиеПредложенияКлиентуОбработатьДокументыДляПереводаНаВерсиюБЭД(Параметры) Экспорт
	...
	Если ОбработанныхОбъектов = 0 И ПроблемныхОбъектов <> 0 Тогда
		ШаблонСообщения = НСтр("ru = 'Не удалось обработать некоторые коммерческие предложения контрагенту (пропущены): %1';
								|en = 'Cannot process some sales quotations for the counterparty (skipped): %1'");
		ТекстСообщения = СтрШаблон(ШаблонСообщения, ПроблемныхОбъектов);
		ВызватьИсключение ТекстСообщения;
	Иначе
		ШаблонСообщения = НСтр("ru = 'Обработана очередная порция коммерческих предложений клиенту: %1';
								|en = 'The next batch of sales quotations for customer is processed: %1'");
...
Процедура ЗаполнитьДокументКоммерческоеПредложениеКлиенту(ДокументОбъект, Контрагенты)
	...
	Если ДокументОбъект.УдалитьСпособДоставки = Перечисления.СпособыДоставки.Самовывоз Тогда
		УсловияДоставкиТекст = НСтр("ru = 'Самовывоз (с нашего склада)';
									|en = 'Self-pickup (from our warehouse)'");
	ИначеЕсли ДокументОбъект.УдалитьСпособДоставки = Перечисления.СпособыДоставки.ДоКлиента Тогда
		УсловияДоставкиТекст = НСтр("ru = 'Наша транспортная служба до клиента';
									|en = 'Our transportation service to customer'");
	ИначеЕсли ДокументОбъект.УдалитьСпособДоставки = Перечисления.СпособыДоставки.СиламиПеревозчикаПоАдресу Тогда
		УсловияДоставкиТекст = НСтр("ru = 'Наша транспортная служба до перевозчика';
									|en = 'Our transportation service to carrier'");
	...
	ИначеЕсли ДокументОбъект.УдалитьСпособДоставки = Перечисления.СпособыДоставки.КПолучателюОпределяетСлужбаДоставки Тогда
		УсловияДоставкиТекст = НСтр("ru = 'На усмотрение транспортной службы';
									|en = 'At the discretion of transportation service'");
	Иначе
