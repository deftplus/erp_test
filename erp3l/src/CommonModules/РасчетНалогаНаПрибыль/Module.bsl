...
Функция ПодготовитьПараметры(РасчетНалога, ЗаписьДолиБазыНалогаНаПрибыль, ТаблицаРеквизиты) Экспорт
	...
	+ "СреднесписочнаяЧисленность,"        // <Число, 15, 2>
...
// 		см. Документы.РегламентнаяОперация.ПодготовитьДанныеРасчетаНалогаНаПрибыль().ТаблицаРеквизитыРасчетНалога
// 	ПараметрРаспределениеНалогаНаПрибыль - ТаблицаЗначений -
// 		см. НалоговыйУчетОбособленныхПодразделений.ПодготовитьТаблицуРасчетДолейБазыНалогаНаПрибыль()
//
// Возвращаемое значение:
// 	Структура - содержит:
// 		* Проводки - Число - общая сумма (до)начисленного налога.
// 		* Всего    - Число - сумма налога
//
Функция НачислитьНалогНаПрибыль(Движения, КонтекстРасчета, Знач Реквизиты, Знач ПараметрРаспределениеНалогаНаПрибыль) Экспорт
	...
	РасчетРаспределенияНалоговойБазы = Параметры.ЗаписьДолиБазыНалогаНаПрибыль;
	...
	БазаНалогаЗаГод  = НалоговаяБаза(
		КонтекстРасчета.НачалоНалоговогоПериода,
		КонецМесяца(КонтекстРасчета.КонецПериода),
		КонтекстРасчета.Организация);
	...
	РаспределениеНалогаНаПрибыль =
		ПолучитьРаспределениеНалогаНаПрибыль(РасчетРаспределенияНалоговойБазы, КонтекстРасчета);
...
//
Функция НачислениеНалогаНаПрибыль(КонтекстРасчета, НалоговаяБаза, СтавкаФедеральныйБюджет, СтавкаРегиональныйБюджет, РаспределениеНалогаНаПрибыль)
	...
	ИсключитьРанееНачисленныеАвансы(НачислениеНалога, КонтекстРасчета, РаспределениеНалогаНаПрибыль);
	
	Возврат НачислениеНалога;
	
КонецФункции	

Процедура ИсключитьРанееНачисленныеАвансы(НачислениеНалога, КонтекстРасчета, РаспределениеНалогаНаПрибыль = Неопределено)
...
Функция ПолучитьРаспределениеНалогаНаПрибыль(РасчетРаспределенияНалоговойБазы, КонтекстРасчета)
	...
	Если ЗначениеЗаполнено(РаспределениеНалогаНаПрибыль) Тогда
		...
		Для Каждого СтрокаРаспределениеНалогаНаПрибыль Из РаспределениеНалогаНаПрибыль Цикл
			...
			СтрокаРаспределениеНалогаНаПрибыль.СтавкаРегиональныйБюджет =
				СтавкиРегиональныйБюджет[СтрокаРаспределениеНалогаНаПрибыль.РегистрацияВНалоговомОргане];
		КонецЦикла;
	КонецЕсли;
...
Процедура ОтразитьОсвобождениеОтНалога(Проводки, НачислениеНалога, КонтекстРасчета, РаспределениеНалогаНаПрибыль)
	...
	ИсключитьАвансовыеПлатежиДоОсвобожденияОтНалога(
		...
		КонтекстРасчета.Организация,
		РаспределениеНалогаНаПрибыль,
		КонтекстРасчета.РеквизитыРеглОперации);
...
Процедура ИсключитьАвансовыеПлатежиДоОсвобожденияОтНалога(ОсвобождениеОтНалога, НачалоПериодаОсвобождения, Организация, РаспределениеНалогаНаПрибыль, РеквизитыРеглОперации = Неопределено)
	
	КонтекстРасчета = НовыйКонтекстРасчета(Организация, НачалоПериодаОсвобождения - 1, РеквизитыРеглОперации);
	...
	КонтекстРасчета.РеквизитыРеглОперации.Свойство("ТаблицаРеквизитыРасчетНалога", ТаблицаРеквизиты);
	
	Если ЗначениеЗаполнено(ТаблицаРеквизиты) Тогда
		
		СтрокаТаблицыРеквизитов            = ТаблицаРеквизиты[0];
		СтрокаТаблицыРеквизитов.Период     = КонтекстРасчета.КонецПериода;
		СтрокаТаблицыРеквизитов.НачалоГода = НачалоГода(КонтекстРасчета.НачалоМесяца);
		СтрокаТаблицыРеквизитов.НачДата    = КонтекстРасчета.НачалоМесяца;
		СтрокаТаблицыРеквизитов.КонДата    = КонтекстРасчета.КонецПериода;
		...
		РаспределениеНалогаНаПрибыль =
			ПолучитьРаспределениеНалогаНаПрибыль(ТаблицаДолейБазыНалогаНаПрибыль, КонтекстРасчета);
			
	КонецЕсли;
	
	ИсключитьРанееНачисленныеАвансы(ОсвобождениеОтНалога, КонтекстРасчета, РаспределениеНалогаНаПрибыль)
