...
Процедура ПриОпределенииЗапросаДляИСМП(
	...
	ИначеЕсли МетаданныеДокументаОснования = Метаданные.Документы.ПроизводствоБезЗаказа Тогда
		...
		Если МетаданныеДокументаИСМП = Метаданные.Документы.МаркировкаТоваровИСМП Тогда
			...
			// ВыходныеИзделия
			ВыходныеИзделияПриход = ШаблонОписанияПриходнойТабличнойЧастиДокумента("ВыходныеИзделия", "ВыходныеИзделияСерии");
			ВыходныеИзделияПриход.ДопОтборы_Общие = СтрШаблон("И %1", ДокументПроведен());
			
			// ПобочныеИзделия
			ПобочныеИзделияПриход = ШаблонОписанияПриходнойТабличнойЧастиДокумента("ПобочныеИзделия", "ПобочныеИзделияСерии");
			ПобочныеИзделияПриход.ДопОтборы_Общие = СтрШаблон("И %1", ДокументПроведен());
	...
	ИначеЕсли МетаданныеДокументаОснования = Метаданные.Документы.РеализацияТоваровУслуг Тогда
		...
		ИначеЕсли МетаданныеДокументаИСМП = Метаданные.Документы.ОтгрузкаТоваровИСМП Тогда
			...
			ТоварыПриход.ДопОтборы_Общие = СтрШаблон("И %1 И Не %2 И %3 И %4",
				"ТаблицаДокумента.Контрагент.ЮрФизЛицо <> ЗНАЧЕНИЕ(Перечисление.ЮрФизЛицо.ЮрЛицоНеРезидент)",
				КонтрагентФизЛицо(),
				"НЕ(1 В (ВЫБРАТЬ 1 Из РегистрСведений.СостоянияПоОбъектамУчетаЭДО КАК СостоянияЭД 
				|	ГДЕ СостоянияЭД.СсылкаНаОбъект = ТаблицаДокумента.Ссылка 
				|	И СостоянияЭД.УдалитьЭлектронныйДокумент.СодержитМаркируемыеТовары))",
				ДокументПроведен());
	...
	ИначеЕсли МетаданныеДокументаОснования = Метаданные.Документы.КорректировкаРеализации Тогда
		...
		Если МетаданныеДокументаИСМП = Метаданные.Документы.ОтгрузкаТоваровИСМП Тогда
			...
			ТоварыПриход.ДопОтборы_Общие = СтрШаблон("И %1", ДокументПроведен()) + "
				|И НЕ(1 В (ВЫБРАТЬ 1 Из РегистрСведений.СостоянияПоОбъектамУчетаЭДО КАК СостоянияЭД 
				|	ГДЕ СостоянияЭД.СсылкаНаОбъект = ТаблицаДокумента.Ссылка 
				|	И СостоянияЭД.УдалитьЭлектронныйДокумент.СодержитМаркируемыеТовары))";
