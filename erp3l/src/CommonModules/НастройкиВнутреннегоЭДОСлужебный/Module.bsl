...
Функция ШаблонПоложенияОбИспользованииПЭП(Параметры) Экспорт
	...
	Исключение
		ТекстОшибки = НСтр("ru = 'Не удалось заполнить значения параметров шаблона положения об использовании простой ЭП причине:';
							|en = 'Cannot fill in values of the parameters of the simple electronic signature practice statement template due to:'") + Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
		ВызватьИсключение ТекстОшибки;
	КонецПопытки;
