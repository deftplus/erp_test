...
Процедура ПриОпределенииПроверок(ГруппыПроверок, Проверки) Экспорт
	...
	Рекомендация                          = НСтр("ru = 'В зависимости от ситуации следует выбрать один из вариантов:
		...
		|Затем выполнить синхронизацию с подчиненными узлами.';
		|en = 'Depending on the situation, choose one of the options:
		|•	Check if the address is entered correctly by checking the address on the website of the Federal Tax Service of the Russian Federation: https://fias.nalog.ru/ExtendedSearch
		|It is recommended that you inform local authorities or the technical support service of the FTS of the Russian Federation about incorrect or incomplete address information on the website of the FTS of the Russian Federation.
		|•	If the address is outdated due to renaming, reassignment, or the abolition of address objects (streets or localities), then enter the current address.
		|
		|If the application has the address classifier, do the following: 
		|•	First, update it to the last address information version (the Administration section)
		|•	To identify and automatically correct obsolete addresses, import the history of changing object addresses
		|The %1 link
		|
		|If you work in a distributed infobase (DIB), then corrections must be made only in the master node.
