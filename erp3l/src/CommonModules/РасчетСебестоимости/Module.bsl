	...
	Иначе
		...
		Попытка
			...
			Если ПараметрыЗапуска.Свойство("ТолькоОднаОрганизация") Тогда
				ТолькоОднаОрганизация = ПараметрыЗапуска.ТолькоОднаОрганизация;
			Иначе
				ТолькоОднаОрганизация = Ложь;
			КонецЕсли;
			...
			СхемаРасчета = РасчетСебестоимостиПрикладныеАлгоритмы.СхемаРасчетаПартий(
				...
				Неопределено, // РазделительВПредставлении
				0, // НомерЗадания
...
//		// - ВТСтоимостьПартийТоваров
		...
		РасчетСебестоимостиПостатейныеЗатраты.СформироватьДвиженияРасходыНаПродажу(ПараметрыРасчета);
