...
Функция ДанныеПоСреднейСтоимостиДвижимогоИмущества(Организации, ДатаНачала, ДатаОкончания) Экспорт 
	...
	// Период, за который получаются данные.
	// При получении данных по месяцам используется для определения заключительного периода.
	ПараметрыРасчетаСтоимостиДвижимогоИмущества.Вставить("ПериодРасчета", ДатаОкончания);
	
	РасчетИмущественныхНалоговПереопределяемый.ДополнитьПараметрыРасчетаСтоимостиДвижимогоИмущества(
		ПараметрыРасчетаСтоимостиДвижимогоИмущества, 
		Организации[0], 
		ДатаОкончания);
	...
	Пока Период <= НачалоМесяца(ДатаОкончания) Цикл
		...
		РасчетИмущественныхНалоговПереопределяемый.ДобавитьТекстЗапросаУчетОсновныхСредств(Период, ПараметрыРасчетаСтоимостиДвижимогоИмущества, СписокЗапросов);
...
//
Функция ПараметрыРасчетаНалогаНаИмущество(Организация, ПериодРасчета, ДополнительныеПараметры) Экспорт 
...
Функция ПолучитьДанныеДляРасчетаНалогаНаИмуществоПоСреднейСтоимости(ПараметрыРасчета)
	...
	Пока Период <= НачалоМесяца(ПараметрыРасчета.мДатаКонцаПериодаОтчета) Цикл
		...
		Если ПараметрыРасчета.ПлательщикЕНВД_НаКонецКвартала Тогда
			...
			|									ТОГДА АмортизацияОС.СуммаОстатокДт 
			|								ИНАЧЕ АмортизацияОС.СуммаОстатокКт 
			|							КОНЕЦ, 0) - ЕСТЬNULL(ОбесценениеОС.СуммаОстатокКт, 0)) * КоэффициентыЕНВД.НеЕНВД
			...
			|									ТОГДА АмортизацияОС.СуммаОстатокДт 
			|								ИНАЧЕ АмортизацияОС.СуммаОстатокКт 
			|							КОНЕЦ, 0) - ЕСТЬNULL(ОбесценениеОС.СуммаОстатокКт, 0)) * КоэффициентыЕНВД.Распределение * &КоэффициентРаспределенияЕНВД КАК ЧИСЛО(15, 2)) КАК ОстаточнаяСтоимость
			|
			|ПОМЕСТИТЬ ИмяВременнойТаблицы
			|ИЗ
			|	СписокОС
