	...
	Если ЗначениеЗаполнено(ДвоичныеДанные) Тогда
		...
		Иначе
			ПодробноеПредставлениеОшибки = НСтр("ru = 'Не удалось получить данные сертификата криптографии %1.';
												|en = 'Cannot retrieve the cryptography certificate data %1.'");
...
Функция ОтправитьКонтейнер(КлючСинхронизации, ОписаниеКонтейнера, КонтекстДиагностики, ИстекСрокДействияМаркера = Неопределено) Экспорт
	...
	ПараметрыВыполненияОперации.ПредставлениеОперации = НСтр("ru = 'Отправка электронного документа';
															|en = 'Electronic document transmission'");
...
Функция ПроверитьПодключениеАбонентовКСервису1СЭДО(Знач ДанныеДляПроверки, КонтекстДиагностики) Экспорт
	...
	Для НомерПорции = 1 По КоличествоЗапросов Цикл 
		...
		ПараметрыВыполненияОперации.ПредставлениеОперации = НСтр("ru = 'Проверка подключения абонентов к сервису 1С-ЭДО';
																|en = 'Check subscriber connection to 1C:EDI service'");
...
Функция ДанныеРегистрационнойИнформации(ПараметрыРегистрации, ДанныеОператора)
	...
	Данные.Вставить("ДвоичныеДанные",
		РегистрационныеДанныеДляОператораЭДО(ДеревоРегистрационнойИнформации));
...
Функция РегистрационныеДанныеДляОператораЭДО(ДеревоРегистрационнойИнформации)
