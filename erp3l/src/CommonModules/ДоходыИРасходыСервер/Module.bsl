		...
		Если ВыборкаПоИндексу.База = 0 Тогда
			...
			Текст = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Отсутствует база для распределения расходов по правилу %1 для аналитики %2. Укажите другое правило или удалите строку из документа.';
					|en = 'No base for cost allocation according to rule %1 for dimension %2. Specify another rule or remove the line from the document.'"),
...
Процедура ОписаниеПроверок_РаспределениеРасходовБудущихПериодов(ТаблицаПроверок) Экспорт
	...
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		...
		НСтр("ru = 'Обнаружены нераспределенные расходы по статьям с вариантом распределения ""На расходы будущих периодов"".';
			|en = 'Обнаружены нераспределенные расходы по статьям с вариантом распределения ""На расходы будущих периодов"".'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ПроверкаНераспределенныеРасходыБудущихПериодов(ПараметрыПроверки) Экспорт
	...
	СписокПолей.Добавить("СуммаРегл",		 		НСтр("ru = 'Сумма (БУ)';
															|en = 'Сумма (БУ)'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//++ НЕ УТ
	СписокПолей.Добавить("СуммаНУ",		 			НСтр("ru = 'Сумма (НУ)';
															|en = 'Amount (FA)'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	СписокПолей.Добавить("БезСуммУпр",		 		НСтр("ru = 'Автосумма (УУ)';
															|en = 'Автосумма (УУ)'", ОбщегоНазначения.КодОсновногоЯзыка()));
	СписокПолей.Добавить("БезСуммРегл",		 		НСтр("ru = 'Автосумма (БУ)';
															|en = 'Автосумма (БУ)'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//++ НЕ УТ
	СписокПолей.Добавить("БезСуммНУ",		 		НСтр("ru = 'Автосумма (НУ)';
															|en = 'Автосумма (НУ)'", ОбщегоНазначения.КодОсновногоЯзыка()));
	//-- НЕ УТ
	...
	ПараметрыРегистрации = ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		...
		НСтр("ru = 'Обнаружены нераспределенные расходы будущих периодов по организации ""%1"" за период %2';
			|en = 'Unallocated deferred expenses are detected for the ""%1"" company for %2 period'", ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей);
