	...
	Если РазмерыВременныхТаблиц.вт_ОрганизацииСРезервами = 0 Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			...
			НСтр("ru = 'Нет организаций, для которых в учетной политике используется формирование резервов в БУ или НУ';
				|en = 'No companies for which generation of reserves in AC or TA is used in accounting policy'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ИначеЕсли РазмерыВременныхТаблиц.вт_ОрганизацииСОстатками = 0 Тогда
		...
		ЗакрытиеМесяцаСервер.УстановитьСостояниеНеТребуется(
			...
			НСтр("ru = 'Нет остатков по регистрам расчетов с клиентами, поставщиками, по регистрам финансовых инструментов и по резервов';
				|en = 'No balance in registers of payments to customers, vendors, in registers of financial instruments and in reserves'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ОписаниеПроверок_ФормированиеРезервовПоСомнительнымДолгам(ТаблицаПроверок) Экспорт
	...
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		...
		НСтр("ru = 'В каждом месяце должен быть создан документ ""Начисление и списание резервов по сомнительным долгам"".';
			|en = 'Create the ""Accrual and write-off of loss allowance"" document for each month.'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ЗакрытиеМесяцаСервер.ЗаполнитьПредставлениеНовойПроверки(ОписаниеПроверки,
		НСтр("ru = 'Изменилась дебиторская задолженность резервов по сомнительным долгам';
			|en = 'Receivables of loss allowance were changed'", ОбщегоНазначения.КодОсновногоЯзыка()),
		НСтр("ru = 'Дебиторская задолженность по оперативным регистрам учета должна совпадать с документом ""Начисление и списание резервов по сомнительным долгам"".';
			|en = 'Receivables for operational accounting registers must match the ""Accrual and write-off of loss allowance"" document.'", ОбщегоНазначения.КодОсновногоЯзыка()));
...
Процедура ПроверкаНеобходимостиФормированияРезервовПоСомнительнымДолгам(ПараметрыПроверки) Экспорт
	...
	ПараметрыРегистрации = ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		...
		НСтр("ru = 'Не создан документ ""Начисление и списание резервов по сомнительным долгам"" по организации ""%1"" за период %2';
			|en = 'The ""Accrual and write-off of loss allowance"" document is not created for the ""%1"" company for period %2'", ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей);
...
Процедура ПроверкаИзмененияДебиторскойЗадолженностиРезервовПоСомнительнымДолгам(ПараметрыПроверки) Экспорт
	...
	СписокПолей.Добавить("Документ", 	НСтр("ru = 'Документ ""Начисление и списание резервов по сомнительным долгам""';
												|en = 'The ""Accrual and write-off of loss allowance"" document'", ОбщегоНазначения.КодОсновногоЯзыка()));
	...
	ПараметрыРегистрации = ЗакрытиеМесяцаСервер.ИнициализироватьПараметрыРегистрацииПроблемПроверки(
		...
		|After correction clear the ""Need check"" check box in these lines.'", ОбщегоНазначения.КодОсновногоЯзыка()),
		СписокПолей,
		"Документ");
...
Функция ТекстЗапросаЗаданийКЗакрытиюМесяцаПриИзмененииРасчетов(ИмяТаблицыДействующиеУчетныеПолитики) Экспорт
	...
	|ГДЕ
	|	(вт_ДействующиеУчетныеПолитики.ФормироватьРезервыПоСомнительнымДолгамБУ
	|			ИЛИ вт_ДействующиеУчетныеПолитики.ФормироватьРезервыПоСомнительнымДолгамУУ
...
Функция ТекстЗапросаДействующихУчетныхПолитик(ИмяТаблицыИзменений, ИмяПодготовленнойТаблицы) Экспорт
	...
	ТекстыЗапросаПакет.Добавить(ТекстЗапросаУчетныеПолитикиУУ());
	ТекстыЗапросаПакет.Добавить(НачислениеСписаниеРезервовПоСомнительнымДолгамЛокализация.ТекстЗапросаУчетныеПолитикиБУ());
	ТекстыЗапросаПакет.Добавить(НачислениеСписаниеРезервовПоСомнительнымДолгамЛокализация.ТекстЗапросаУчетныеПолитикиНУ());
	...
	|	вт_ТаблицаИзменений.Организация КАК Организация,
	|	вт_ТаблицаИзменений.Документ КАК Документ,
	|	вт_УчетныеПолитикиБУ.ФормироватьРезервыПоСомнительнымДолгамБУ КАК ФормироватьРезервыПоСомнительнымДолгамБУ,
	|	вт_УчетныеПолитикиБУ.ПериодичностьРезервовПоСомнительнымДолгамБУ КАК ПериодичностьРезервовПоСомнительнымДолгамБУ,
	...
	|	вт_УчетныеПолитикиУУ.ФормироватьРезервыПоСомнительнымДолгамУУ КАК ФормироватьРезервыПоСомнительнымДолгамУУ,
	|	вт_УчетныеПолитикиУУ.ПериодичностьРезервовПоСомнительнымДолгамУУ КАК ПериодичностьРезервовПоСомнительнымДолгамУУ
...
Процедура СформироватьРезервыПриЗакрытииМесяца(Организации, Период)
	...
	ТекстыЗапросаПакет.Добавить(НачислениеСписаниеРезервовПоСомнительнымДолгамЛокализация.ТекстЗапросаУчетнаяПолитикаБУ());
	ТекстыЗапросаПакет.Добавить(НачислениеСписаниеРезервовПоСомнительнымДолгамЛокализация.ТекстЗапросаУчетнаяПолитикаНУ());
	
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	ГоловныеОрганизации.Организация КАК Организация,
	|	ЕСТЬNULL(ВтУчетнаяПолитикаБухУчета.ФормироватьБУ, ЛОЖЬ) КАК ФормироватьБУ,
	|	ЕСТЬNULL(ВтУчетнаяПолитикаБухУчета.ПериодичностьБУ, ЛОЖЬ) КАК ПериодичностьБУ,
	...
	|	ВТУчетныеПолитикиОрганизаций.ФормироватьБУ КАК ФормироватьБУ,
	|	ВТУчетныеПолитикиОрганизаций.ФормироватьНУ КАК ФормироватьНУ,
	|	ВТУчетныеПолитикиОрганизаций.ФормироватьУУ КАК ФормироватьУУ,
	|	ВТУчетныеПолитикиОрганизаций.ПериодичностьБУ КАК ПериодичностьБУ,
	...
	Пока Выборка.Следующий() Цикл
		Если Выборка.ДокументСуществует Тогда
			...
			Если Выборка.ДокументПроведен И Не Выборка.ФормироватьБУ И Не Выборка.ФормироватьНУ И Не Выборка.ФормироватьУУ Тогда
				ДокументОбъект.Записать(РежимЗаписиДокумента.ОтменаПроведения);
				Продолжить;
			КонецЕсли;
		ИначеЕсли Не Выборка.ФормироватьБУ И Не Выборка.ФормироватьНУ И Не Выборка.ФормироватьУУ Тогда
			 Продолжить;
		Иначе
			...
			ДокументОбъект.ОтражатьВУправленческомУчете = Выборка.ФормироватьУУ И
				ТребуетсяФормированиеРезервовВУказанномПериоде(Выборка.ПериодичностьУУ, Период);
...
Функция ТекстЗапросаУчетныеПолитикиУУ()
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ЕСТЬNULL(УчетнаяПолитикаУУ.ФормироватьРезервыПоСомнительнымДолгам, ЛОЖЬ) КАК ФормироватьРезервыПоСомнительнымДолгамУУ,
	|	УчетнаяПолитикаУУ.ПериодичностьРезервовПоСомнительнымДолгам КАК ПериодичностьРезервовПоСомнительнымДолгамУУ,
...
Процедура ДополнитьТекстЗапросаЗаданийКЗакрытиюМесяца(Запрос, ТекстЗапроса, ТекстЗапросаВременныхТаблиц, ИменаВременныхТаблиц, ОтменаПроведения) Экспорт
...
//
Функция ТекстЗапросаИзмененийПоАналитикеУчетаПоПартнерам(ИмяТаблицыИзменений, ИмяПодготовленнойТаблицыИзменений, ИмяРеквизитаРегистратора = "Регистратор")
...
Функция СоответствиеЗапросовКонтрольнымРегистрам(Запрос, ОтменаПроведения)
	...
	СоответствиеТекстовЗапросов = Новый Соответствие();
	//++ НЕ УТ
	СоответствиеТекстовЗапросов.Вставить("ТаблицаИзмененийДвиженияКонтрагентДоходыРасходы", ТекстЗапросаДвиженияКонтрагентДоходыРасходы(Запрос, ОтменаПроведения));
	...
	СоответствиеТекстовЗапросов.Вставить("ТаблицаИзмененийРасчетыСКлиентамиПоСрокам", ТекстЗапросаРасчетыСКлиентамиПоСрокам(Запрос));
	...
	Если Не ПолучитьФункциональнуюОпцию("НоваяАрхитектураВзаиморасчетов") Тогда
		СоответствиеТекстовЗапросов.Вставить("РасчетыСКлиентамиПоДокументамИзменения", ТекстЗапросаРасчетыСКлиентамиПоДокументам(Запрос));
		СоответствиеТекстовЗапросов.Вставить("РасчетыСПоставщикамиПоДокументамИзменения", ТекстЗапросаРасчетыСПоставщикамиПоДокументам(Запрос));
	КонецЕсли;
	СоответствиеТекстовЗапросов.Вставить("ТаблицаИзмененийРезервыПоСомнительнымДолгам", ТекстЗапросаРезервыПоСомнительнымДолгам(Запрос, ОтменаПроведения));
	//-- НЕ УТ
	Возврат СоответствиеТекстовЗапросов;
...
Функция ТекстЗапросаДвиженияКонтрагентДоходыРасходы(Запрос, ОтменаПроведения)
	...
	|;";
	ТекстЗапросаВременныхТаблиц = СтрЗаменить(ТекстЗапросаВременныхТаблиц, "ИмяПодготовленнойТаблицыИзменений", ИмяПодготовленнойТаблицыИзменений);
	
	ТекстЗапросаВременныхТаблиц =  ТекстЗапросаВременныхТаблиц
		+ ТекстЗапросаДействующихУчетныхПолитик(ИмяПодготовленнойТаблицыИзменений, ИмяТаблицыДействующиеУчетныеПолитики)
		+ ";";
	...
	СтруктураТекстовЗапросов = ЗакрытиеМесяцаСервер.ИнициализироватьСтруктуруТекстовЗапросов(ТекстЗапроса,ТекстЗапросаВременныхТаблиц,ИмяТаблицыДействующиеУчетныеПолитики);
...
Функция ТекстЗапросаВыручкаИСебестоимостьПродаж(Запрос)
	...
	МассивЗапросовВременныхТаблиц.Добавить(ТекстЗапросаДействующихУчетныхПолитик("втИзменениеСуммыВыручкиРегл", ИмяТаблицыДействующиеУчетныеПолитики));
	МассивИменаВременныхТаблицы.Добавить(ИмяТаблицыДействующиеУчетныеПолитики);
	
	ТекстЗапросаВременныхТаблиц = СтрСоединить(МассивЗапросовВременныхТаблиц, ОбщегоНазначения.РазделительПакетаЗапросов());
	...
	ТекстЗапроса = ТекстЗапросаЗаданийКЗакрытиюМесяцаПриИзмененииРасчетов(ИмяТаблицыДействующиеУчетныеПолитики);
	СтруктураТекстовЗапросов = ЗакрытиеМесяцаСервер.ИнициализироватьСтруктуруТекстовЗапросов(ТекстЗапроса,ТекстЗапросаВременныхТаблиц,ИменаВременныхТаблицы);
...
Функция ТекстЗапросаРасчетыПоФинансовымИнструментам(Запрос)
	...
	ТекстЗапросаВременныхТаблиц = ТекстЗапросаИзмененийПоАналитикеУчетаПоПартнерам(ИмяТаблицыИзменений, ИмяПодготовленнойТаблицыИзменений)
		+ ТекстЗапросаДействующихУчетныхПолитик(ИмяПодготовленнойТаблицыИзменений, ИмяТаблицыДействующиеУчетныеПолитики)
		+ ";";
	ИменаВременныхТаблиц = ИмяПодготовленнойТаблицыИзменений + ", " + ИмяТаблицыДействующиеУчетныеПолитики;
	...
	ТекстЗапросаУчетныеПолитики = ТекстЗапросаЗаданийКЗакрытиюМесяцаПриИзмененииРасчетов(ИмяТаблицыДействующиеУчетныеПолитики);
	...
	СтруктураТекстовЗапросов = ЗакрытиеМесяцаСервер.ИнициализироватьСтруктуруТекстовЗапросов(ТекстЗапроса, ТекстЗапросаВременныхТаблиц, ИменаВременныхТаблиц);
	Возврат СтруктураТекстовЗапросов;
...
Функция ТекстЗапросаРасчетыСКлиентамиПоСрокам(Запрос)
	...
	ТекстЗапросаВременныхТаблиц = ТекстЗапросаИзмененийПоАналитикеУчетаПоПартнерам(ИмяТаблицыИзменений, ИмяПодготовленнойТаблицыИзменений, "ДокументРегистратор")
		+ ТекстЗапросаДействующихУчетныхПолитик(ИмяПодготовленнойТаблицыИзменений, ИмяТаблицыДействующиеУчетныеПолитики)
		+ ";";
...
Функция ТекстЗапросаСостояниеФормированияРезервовПоСомнительнымДолгам()
	...
	ТекстыЗапроса.Добавить(НачислениеСписаниеРезервовПоСомнительнымДолгамЛокализация.ТекстЗапросаУчетнаяПолитикаБУ());
	ТекстыЗапроса.Добавить(НачислениеСписаниеРезервовПоСомнительнымДолгамЛокализация.ТекстЗапросаУчетнаяПолитикаНУ());
	
	ТекстЗапроса =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ГоловныеОрганизации.Организация КАК Ссылка,
	|	ЕСТЬNULL(ВтУчетнаяПолитикаБухУчета.ФормироватьБУ, ЛОЖЬ) КАК ФормироватьБУ,
	|	ЕСТЬNULL(ВтУчетнаяПолитикаБухУчета.ПериодичностьБУ, ЛОЖЬ) КАК ПериодичностьБУ,
	|	ЕСТЬNULL(ВтУчетнаяПолитикаНУ.ФормироватьНУ, ЛОЖЬ) КАК ФормироватьНУ,
	|	ЕСТЬNULL(ВтУчетнаяПолитикаНУ.ПериодичностьНУ, ЛОЖЬ) КАК ПериодичностьНУ,
	...
	|		ПО ГоловныеОрганизации.Организация = ВтУчетнаяПолитикаФинансовогоУчета.Организация
	|ГДЕ
	|	ЕСТЬNULL(ВтУчетнаяПолитикаБухУчета.ФормироватьБУ, ЛОЖЬ) = ИСТИНА
	|		ИЛИ ЕСТЬNULL(ВтУчетнаяПолитикаНУ.ФормироватьНУ, ЛОЖЬ) = ИСТИНА
