	...
	ИначеЕсли ДополнительныеПараметры.УсиленнаяЗащитаЗакрытогоКлюча Тогда
		...
		Элементы.ПояснениеУсиленногоПароля.Подсказка =
			НСтр("ru = 'Пароль запрашивает программа электронной подписи, а не программа 1С:Предприятие,
			           ...
			           |as strong private key protection is specified for the selected certificate.'");
			...
			Если Контекст.Операция <> "Подписание" Тогда
				Ошибка = Новый Структура("ОписаниеОшибки",
					НСтр("ru = 'Внешняя компонента ExtraCryptoAPI может использоваться только для подписания.';
						|en = 'The ExtraCryptoAPI add-in can be used only for signing.'"));
				ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст);
				Возврат;
			КонецЕсли;
	...
	Если Результат.Свойство("ОписаниеОшибки") Тогда
		...
		Ошибка = Новый Структура("ОписаниеОшибки",
			НСтр("ru = 'При записи зашифрованных данных возникла ошибка:';
				|en = 'An error occurred when saving encrypted data:'")
			+ Символы.ПС + Результат.ОписаниеОшибки);
		ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст);
		Возврат;
	КонецЕсли;
	...
	Если Результат.Свойство("ОписаниеОшибки") Тогда
		...
		Ошибка = Новый Структура("ОписаниеОшибки",
			НСтр("ru = 'При записи расшифрованных данных возникла ошибка:';
				|en = 'An error occurred when saving decrypted data:'")
			+ Символы.ПС + Результат.ОписаниеОшибки);
		ВыполнитьНаСторонеПослеЦикла(Ошибка, Контекст);
		Возврат;
	КонецЕсли;
	...
	ПодключитьКомпоненту(Контекст,
		?(Контекст.ТипПодписи = "CMS",
			НСтр("ru = 'Для проверки подписи CMS необходима установка компоненты ExtraCryptoAPI';
				|en = 'To check CMS signature, install the ExtraCryptoAPI add-in'"),
			НСтр("ru = 'Для проверки подписи XML необходима установка компоненты ExtraCryptoAPI';
				|en = 'To check XML signature, install the ExtraCryptoAPI add-in'")));
		...
		Если ПустаяСтрока(Результат.ОписаниеОшибки) Тогда 
			...
			ЗавершитьОперациюСОшибкой(
				...
				НСтр("ru = 'Операция невозможна. Требуется установка компоненты ExtraCryptoAPI.';
					|en = 'The operation is not allowed. Installation of the ExtraCryptoAPI add-in is required.'"));
...
Процедура ПослеПодключенияКомпонентыПослеОшибкиУстановкиСоответствиеOID(ИнформацияОбОшибке, СтандартнаяОбработка, Контекст) Экспорт
	...
	ЗавершитьОперациюСОшибкой(Контекст,
		НСтр("ru = 'При установке свойства СоответствиеOID компоненты ExtraCryptoAPI произошла ошибка:';
			|en = 'An error occurred while setting the OIDMap property of the ExtraCryptoAPI add-in:'")
		...
		Иначе
			ЗавершитьОперациюСОшибкой(
				...
				НСтр("ru = 'Не установлен режим работы компоненты ExtraCryptoAPI.';
					|en = 'The ExtraCryptoAPI add-in mode is not set.'"));
		КонецЕсли;
	...
	Иначе
		ЗавершитьОперациюСОшибкой(
			...
			НСтр("ru = 'Не установлен тип подписи компоненты ExtraCryptoAPI.';
				|en = 'The signature type of the ExtraCryptoAPI add-in is not set.'"));
	КонецЕсли;
...
Процедура НачатьПодписание_ПослеВыгрузкиСертификатаКриптографии(ДвоичныеДанныеСертификата, Контекст) Экспорт
	...
	ПодключитьКомпоненту(Контекст,
		НСтр("ru = 'Для подписания XML необходима установка компоненты ExtraCryptoAPI';
			|en = 'To sign XML, install the ExtraCryptoAPI add-in'"));
...
Процедура НачатьПодписаниеCMS_ПослеВыгрузкиСертификатаКриптографии(ДвоичныеДанныеСертификата, Контекст) Экспорт
	...
	ПодключитьКомпоненту(Контекст,
		НСтр("ru = 'Для подписания CMS необходима установка компоненты ExtraCryptoAPI';
			|en = 'To sign CMS, install the ExtraCryptoAPI add-in'"));
	...
	Исключение
		ЗавершитьОперациюСОшибкой(Контекст,
			НСтр("ru = 'Ошибка вызова метода CMSSign компоненты ExtraCryptoAPI.';
				|en = 'An error occurred upon calling the CMSSign method of the ExtraCryptoAPI add-in.'")
			+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Процедура Подписание_ПослеВыполненияCMSSign(АтрибутSignatureValue, Параметры, Контекст) Экспорт
	...
	Если Не ЗначениеЗаполнено(АтрибутSignatureValue) Тогда
		НачатьПолучениеТекстаОшибки(НСтр("ru = 'При выполнении метода CMSSign произошла ошибка:';
										|en = 'An error occurred while executing the CMSSign method:'"), Контекст);
		Возврат;
	КонецЕсли;
	...
	Исключение
		ЗавершитьОперациюСОшибкой(Контекст,
			НСтр("ru = 'Ошибка вызова метода CMSVerifySign компоненты ExtraCryptoAPI.';
				|en = 'An error occurred upon calling the CMSVerifySign method of the ExtraCryptoAPI add-in.'")
			+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Процедура Проверка_ПослеВыполненияCMSVerifySign(ПодписьВерна, Параметры, Контекст) Экспорт
	...
	Если ПодписьВерна <> Истина Тогда
		НачатьПолучениеТекстаОшибки(НСтр("ru = 'При выполнении метода CMSVerifySign произошла ошибка:';
										|en = 'An error occurred while executing the CMSVerifySign method:'"), Контекст);
		Возврат;
	КонецЕсли;
	...
	Исключение
		ЗавершитьОперациюСОшибкой(Контекст,
			НСтр("ru = 'Ошибка вызова метода Hash компоненты ExtraCryptoAPI.';
				|en = 'An error occurred upon calling the Hash method of the ExtraCryptoAPI add-in.'")
			+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
	...
	Исключение
		ЗавершитьОперациюСОшибкой(Контекст,
			НСтр("ru = 'Ошибка вызова метода Sign компоненты ExtraCryptoAPI.';
				|en = 'An error occurred upon calling the Sign method of the ExtraCryptoAPI add-in.'")
			+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
	...
	Исключение
		ЗавершитьОперациюСОшибкой(Контекст,
			НСтр("ru = 'Ошибка вызова метода VerifySign компоненты ExtraCryptoAPI.';
				|en = 'An error occurred upon calling the VerifySign method of the ExtraCryptoAPI add-in.'")
			+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
	...
	Если Не Контекст.ПодписьВерна Или Не ХешСовпадает Тогда
		Если Контекст.ПодписьВерна Тогда
			ТекстОшибки = НСтр("ru = 'Подпись неверна (SignatureValue корректно, DigestValue некорректно).';
								|en = 'Invalid signature. SignatureValue is valid. DigestValue is invalid.'")
		ИначеЕсли ХешСовпадает Тогда
			ТекстОшибки = НСтр("ru = 'Подпись неверна (SignatureValue некорректно, DigestValue корректно).';
								|en = 'Invalid signature. SignatureValue is invalid. DigestValue is valid.'");
		Иначе
			ТекстОшибки = НСтр("ru = 'Подпись неверна (SignatureValue некорректно, DigestValue некорректно).';
								|en = 'Invalid signature. SignatureValue is invalid. DigestValue is invalid.'");
		КонецЕсли;
		ЗавершитьОперациюСОшибкой(Контекст, ТекстОшибки);
		Возврат;
	КонецЕсли;
	...
	Исключение
		ЗавершитьОперациюСОшибкой(Контекст,
			НСтр("ru = 'Ошибка вызова метода C14N компоненты ExtraCryptoAPI.';
				|en = 'An error occurred upon calling the C14N method of the ExtraCryptoAPI add-in.'")
			+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
			...
			Исключение
				ЗавершитьОперациюСОшибкой(Контекст,
					НСтр("ru = 'Ошибка вызова метода C14N_body компоненты ExtraCryptoAPI.';
						|en = 'An error occurred upon calling the C14N_body method of the ExtraCryptoAPI add-in.'")
					+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			КонецПопытки;
		Иначе
			Оповещение = Новый ОписаниеОповещения(
			...
			Исключение
				ЗавершитьОперациюСОшибкой(Контекст,
					НСтр("ru = 'Ошибка вызова метода TransformSMEV компоненты ExtraCryptoAPI.';
						|en = 'An error occurred upon calling the TransformSMEV method of the ExtraCryptoAPI add-in.'")
					+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			КонецПопытки;
		КонецЕсли;
	Иначе
...
Процедура C14N_bodyПослеВызова(КанонизированныйТекстXML, Параметры, Контекст) Экспорт
	...
	Если КанонизированныйТекстXML = Неопределено Тогда
		НачатьПолучениеТекстаОшибки(НСтр("ru = 'При выполнении метода C14N_body произошла ошибка:';
										|en = 'An error occurred while executing C14N_body method:'"), Контекст);
		Возврат;
	КонецЕсли;
...
Процедура TransformSMEVПослеВызова(КанонизированныйТекстXML, Параметры, Контекст) Экспорт
	...
	Если КанонизированныйТекстXML = Неопределено Тогда
		НачатьПолучениеТекстаОшибки(НСтр("ru = 'При выполнении метода TransformSMEV произошла ошибка:';
										|en = 'An error occurred while executing TransformSMEV method:'"), Контекст);
		Возврат;
	КонецЕсли;
	...
	Исключение
		ЗавершитьОперациюСОшибкой(Контекст,
			НСтр("ru = 'Ошибка вызова метода GetLastError компоненты ExtraCryptoAPI.';
				|en = 'An error occurred upon calling the GetLastError method of the ExtraCryptoAPI add-in.'")
			+ Символы.ПС + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
	КонецПопытки;
...
Функция ТекстОшибкиОблачнаяПодпись(Операция, РезультатВызова = Неопределено, ЭтоОшибкаВыполнения = Ложь)
	...
	Возврат НСтр("ru = 'Сервер DSS';
				|en = 'DSS server'") + ": " + ТекстОшибки;
...
Процедура ПослеСбораТехническойИнформацииОКомпоненте(ИнформацияОКомпоненте, Контекст) Экспорт
	...
	Контекст.СопроводительныйТекст = Контекст.СопроводительныйТекст + Символы.ПС
	 + НСтр("ru = 'Версия компоненты ExtraCryptoAPI на клиенте';
			|en = 'The ExtraCryptoAPI add-in version on the client'") + " - " + ИнформацияОКомпоненте + Символы.ПС;
...
Процедура ПолучитьВерсиюКомпоненты(Оповещение)
	...
	ПараметрыПодключения.ТекстПояснения = НСтр("ru = 'Для получения версии требуется установка компоненты ExtraCryptoAPI';
												|en = 'To get the version, installation the ExtraCryptoAPI add-in'");
	...
	Иначе
		Если ПустаяСтрока(Результат.ОписаниеОшибки) Тогда 
			...
			ВерсияКомпоненты = НСтр("ru = 'Компонента ExtraCryptoAPI не установлена.';
									|en = 'The ExtraCryptoAPI add-in is not installed.'");
		Иначе 
			// Установка не удалась, описание ошибки в Результат.ОписаниеОшибки.
			ВерсияКомпоненты = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Компонента ExtraCryptoAPI не установлена (%1).';
						|en = 'The ExtraCryptoAPI add-in is not installed (%1).'"), Результат.ОписаниеОшибки);
		КонецЕсли;
	КонецЕсли;
