	...
	Если ПрименениеПриказа107н Тогда
		...
		Если ДействуетПриказ199н_202101(Период) Тогда
			...
			Статусы.Вставить("19", НСтр("ru = '19 - организации и их филиалы (далее - организации), составившие распоряжение о 
								...
								|on the basis of the enforcement document sent to the company in accordance with the established procedure 
...
Функция КодыВидовДохода(ВыплатаЗарплаты = Ложь) Экспорт
	...
	Коды.Добавить("1", НСтр("ru = '1 - Разрешено удержание по исполнительному листу с ограничением по сумме';
							|en = '1 - Deduction as per enforcement order is allowed with an amount limitation'"));
	Коды.Добавить("2", НСтр("ru = '2 - Запрещено удержание по исполнительному листу';
							|en = '2 - Deduction as per enforcement order is prohibited'"));
	Коды.Добавить("3", НСтр("ru = '3 -  Запрещено удержание по исполнительному листу с оговорками для алиментов';
							|en = '3 - Deduction as per enforcement order is prohibited with reservations for alimony'"));
...
Функция ПроверитьУИН(ИдентификаторПлатежа, ПлатежВФедеральныйОрган = Ложь) Экспорт
	...
	Если ДлинаИдентификатора <> МинимальнаяДлинаУИН() Тогда
		...
		Если ДлинаИдентификатора <> МаксимальнаяДлинаУИН()
			И ДлинаИдентификатора <> ДлинаКодаНПА() Тогда
			ШаблонТекста = НСтр("ru = 'Уникальный идентификатор начисления должен состоять из %1, %2 или %3 символов, а не из %4.';
								|en = 'Уникальный идентификатор начисления должен состоять из %1, %2 или %3 символов, а не из %4.'");
			Возврат СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				ШаблонТекста, МинимальнаяДлинаУИН(), МаксимальнаяДлинаУИН(), ДлинаКодаНПА(), ДлинаИдентификатора);
		КонецЕсли;
