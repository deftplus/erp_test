...
#Область ТекстВключениеИсключениеНДСВСтоимость             // (Дт 20 :: Кт 19.03)
	...
	|	КОНЕЦ КАК СуммаУУ,
	|
	|	ВЫБОР КОГДА УчетнаяПолитикаОрганизаций.СборкаТоваровЧерезСчет20
...
#Область ТекстСборкаРазборкаТоваровНаСкладах // (Дт 41.01 :: Кт 41.01)
	...
	|	НЕОПРЕДЕЛЕНО КАК ИдентификаторСтроки,
	|	СУММА(ЕСТЬNULL(Стоимости.Сумма + Стоимости.СуммаИсключенияНДС, СтрокиПриход.Сумма)) КАК Сумма,
	|	СУММА(ЕСТЬNULL(Стоимости.СуммаУУ + Стоимости.СуммаИсключенияНДСУпр, СтрокиПриход.СуммаУУ)) КАК СуммаУУ,
	|	ЗНАЧЕНИЕ(Перечисление.ВидыСчетовРеглУчета.НаСкладе) КАК ВидСчетаДт,
	...
	|	СУММА(ЕСТЬNULL(Стоимости.СуммаНУ, СтрокиПриход.СуммаНУ)) КАК СуммаНУДт,
	|	СУММА(ЕСТЬNULL(Стоимости.СуммаПР, СтрокиПриход.СуммаПР)) КАК СуммаПРДт,
	|	СУММА(ЕСТЬNULL(Стоимости.СуммаВР, СтрокиПриход.СуммаВР)) КАК СуммаВРДт,
	|	ВЫБОР
	...
	|	СУММА(ЕСТЬNULL(Стоимости.СуммаНУ, СтрокиПриход.СуммаНУ)) КАК СуммаНУКт,
	|	СУММА(ЕСТЬNULL(Стоимости.СуммаПР, СтрокиПриход.СуммаПР)) КАК СуммаПРКт,
	|	СУММА(ЕСТЬNULL(Стоимости.СуммаВР, СтрокиПриход.СуммаВР)) КАК СуммаВРКт,
	|	ВЫБОР
	...
	|	ЛЕВОЕ СОЕДИНЕНИЕ ВтСтоимостиСборка КАК Стоимости ПО
	|		СтрокиПриход.Ссылка = Стоимости.Ссылка
	|		И СтрокиПриход.ВидДвижения = Стоимости.ВидДвижения
	|		И СтрокиПриход.Номенклатура = Стоимости.Номенклатура
	|		И СтрокиПриход.ИдентификаторФинЗаписи = Стоимости.ИдентификаторФинЗаписи
	|ГДЕ
