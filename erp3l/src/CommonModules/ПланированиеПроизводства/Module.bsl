...
Функция РассчитатьОграниченияГрафикаПроизводстваПоМатериалам(Ссылка) Экспорт
	...
	Запрос = Новый Запрос(
		...
		|				И ВариантОбеспечения = ЗНАЧЕНИЕ(Перечисление.ВариантыОбеспечения.Требуется)) КАК Материалы
		|ГДЕ
	...
	ТекстЗапроса =
		...
		|	Таблица.Распределено   КАК Количество
		|ИЗ
		|	ВтРаспределение КАК Таблица
		|ГДЕ
		|	Таблица.Состояние = ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.ОбеспеченНаСкладе)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
...
Функция ОграниченияГрафикаПроизводстваПоМатериалам(Ссылка) Экспорт
	...
	ТекстЗапроса = 
		...
		|	1                                   КАК Состояние,
		|	ДАТАВРЕМЯ(1, 1, 1)                  КАК Дата,
		|	РаспределениеЗапасов.Распределено   КАК Количество
		|ИЗ
		...
		|	РаспределениеЗапасов.Распределено
		|ИЗ
		|	РегистрСведений.РаспределениеЗапасов КАК РаспределениеЗапасов
		|ГДЕ
		|	РаспределениеЗапасов.ЗаказНаОтгрузку = &Ссылка
		...
		|ГДЕ
		|	РаспределениеЗапасов.ЗаказНаОтгрузку = &Ссылка
		|	И РаспределениеЗапасов.Состояние = ЗНАЧЕНИЕ(Перечисление.РаспределениеЗапасовСостояния.ОтложитьОбеспечение)
		|
		|УПОРЯДОЧИТЬ ПО
		|	Номенклатура,
		|	Характеристика,
