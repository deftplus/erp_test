	...
	Для каждого ПараметрыНастройки Из ДанныеНастройкиСчетовУчета.МассивПараметровНастройки Цикл
		...
		Для каждого ИмяЭлемента Из ПараметрыНастройки.ЭлементыФормы Цикл
			...
			Если ЗначениеЗаполнено(ПараметрыНастройки.УсловияДоступностиСчетаУчетаВСтроках) Тогда
				...
				КолонкиКоллекции = КонтекстНастройки.Выгрузить(Новый Массив()).Колонки;
				...
				Для каждого ПолеИЗначение Из ПараметрыНастройки.УсловияДоступностиСчетаУчетаВСтроках Цикл
					...
					Если КолонкиКоллекции.Найти(ПолеИЗначение.Ключ) <> Неопределено Тогда
...
Процедура ПриИзмененииПараметровНастройки(Форма, ПараметрыНастройкиСчетов) Экспорт
	...
	ОчиститьНеактуальныеНастройкиОтраженияВДанныхФормы(Форма, НовыеДанныеНастройкиСчетовУчета.МассивПараметровНастройки);
...
Процедура ОбработкаПроверкиЗаполнения(Форма, Отказ, ПроверяемыеРеквизиты) Экспорт
	...
	Если РазделУчета = "НоменклатураСобственная" Или РазделУчета = "НоменклатураПереданная" Тогда
		
		ИмяРеквизитаСчета = НастройкаСчетовУчетаКлиентСервер.ИмяРеквизитаНаФорме(ДанныеНастройкиСчетовУчета, "Номенклатура", "ВыручкаОтПродаж");
		Если Не БухгалтерскийУчетВызовСервераПовтИсп.ЭтоСчетПрочиеДоходы(Форма[ИмяРеквизитаСчета]) Тогда
			МассивНепроверяемыхРеквизитов.Добавить("Субконто_СтатьяДоходовОСНО");
		КонецЕсли;
		ИмяРеквизитаСчета = НастройкаСчетовУчетаКлиентСервер.ИмяРеквизитаНаФорме(ДанныеНастройкиСчетовУчета, "Номенклатура", "ВыручкаОтПродажЕНВД");
		Если Не БухгалтерскийУчетВызовСервераПовтИсп.ЭтоСчетПрочиеДоходы(Форма[ИмяРеквизитаСчета]) Тогда
			МассивНепроверяемыхРеквизитов.Добавить("Субконто_СтатьяДоходовЕНВД");
		КонецЕсли;
...
Функция ИнициализироватьСтруктуруНастроекРаздела(РазделУчета, ТолькоИспользуемые = Ложь) Экспорт
	...
	Если ПолучитьФункциональнуюОпцию("ВыделятьДолгосрочныеАктивыОбязательства") Тогда
		Для каждого ИмяВидаСчета Из ОбщегоНазначенияКлиентСервер.ОтмеченныеЭлементы(СписокСчетов) Цикл
...
Функция РеквизитыАналитикиВлияющиеНаНастройкуСчетовУчета(РазделУчета) Экспорт
	...
	ИначеЕсли РазделУчета = "НоменклатураСобственная" Или РазделУчета = "Номенклатура" Тогда
		...
		Реквизиты.Вставить("Субконто_СтатьяДоходовОСНО", "ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиДоходов.ПустаяСсылка)");
		Если ПолучитьФункциональнуюОпцию("ИспользуетсяЕНВД") Тогда
			Реквизиты.Вставить("СчетУчета_ВыручкаОтПродажЕНВД", "ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка)");
			Реквизиты.Вставить("Субконто_СтатьяДоходовЕНВД", "ЗНАЧЕНИЕ(ПланВидовХарактеристик.СтатьиДоходов.ПустаяСсылка)");
		КонецЕсли;
	...
	ИначеЕсли Справочники.ГруппыФинансовогоУчетаРасчетов.ВидыРасчетов().Найти(РазделУчета) <> Неопределено Или РазделУчета = "РасчетыСПартнерами"  Тогда
		...
		Реквизиты.Вставить("РасчетыСАрендодателями", "ИСТИНА");
...
Функция НастройкиАналитикиУчетаПоРазделуУчета(РазделУчета) Экспорт
	...
	Если РазделУчета = "РасчетыСКлиентами" Или РазделУчета = "РасчетыСПоставщиками" Или РазделУчета = "РасчетыСКомиссионерами"
		...
		Или РазделУчета = "РасчетыСДебиторами" Или РазделУчета = "РасчетыСАрендодателями" Или РазделУчета = "РасчетыСПартнерами" Тогда
		ЗначениеВозврата.Заголовок = НСтр("ru = 'Группа настроек фин. учета';
											|en = 'Financial accounting settings group'");
		ЗначениеВозврата.ПредставлениеПустого = НСтр("ru = 'не указана';
													|en = 'not specified'");
...
Процедура УстановитьНастройкиСчетовУчетаВФорме(Форма) Экспорт
	...
	Для каждого ЭлементРазделаУчета Из РегистрыСведений.ПорядокОтраженияНаСчетахУчета.СписокРазделовСчетовУчета() Цикл
		...
		Для каждого СчетУчета Из НастройкаРаздела.СчетаУчета Цикл
			...
			ДобавитьГруппыНаФормуДляТекущегоЭлемента(СчетУчета.Значение, Параметры);
			ГруппыБылиДобавлены = Параметры.ГруппаДляДобавления <> НачальнаяГруппаДляДобавления;
			
			Параметры.ЭлементВыводится = СчетУчета.Пометка;
			Параметры.Представление = СчетУчета.Представление;
			ЭлементСчетаУчетаНаФорме(СчетУчета.Значение, Параметры);
			
			Если НастройкаРаздела.ДолгосрочныеСчета.Найти(СчетУчета.Значение) <> Неопределено Тогда
				// Добавим так же настройку долгосрочного счета:
				
				Параметры.ГруппаДляДобавления = НачальнаяГруппаДляДобавления;
				ДобавитьГруппыНаФормуДляТекущегоЭлемента(СчетУчета.Значение, Параметры, "СчетУчетаДолгосрочный");
				ГруппыБылиДобавлены = Параметры.ГруппаДляДобавления <> НачальнаяГруппаДляДобавления;
				
				Параметры.ЭлементВыводится = СчетУчета.Пометка;
				Параметры.Представление = СчетУчета.Представление;
				ЭлементСчетаУчетаНаФорме(СчетУчета.Значение, Параметры, "СчетУчетаДолгосрочный");
				
			КонецЕсли;
			
			Если НастройкаСчетовУчетаКлиентСервер.ИспользуютсяМалоценныеТМЦ(СчетУчета.Значение) Тогда
				// Добавим так же настройку отнесения к малоценным ТМЦ:
				
				Параметры.ГруппаДляДобавления = НачальнаяГруппаДляДобавления;
				ДобавитьГруппыНаФормуДляТекущегоЭлемента("ОтноситсяКМалоценнымТМЦ", Параметры, "СчетУчета");
				ГруппыБылиДобавлены = Параметры.ГруппаДляДобавления <> НачальнаяГруппаДляДобавления;
				
				Параметры.ЭлементВыводится = СчетУчета.Пометка;
				Параметры.Представление = НСтр("ru = 'Относится к малоценным ТМЦ';
												|en = 'Related to low-cost inventory'");
				ЭлементСчетаУчетаНаФорме("ОтноситсяКМалоценнымТМЦ", Параметры, "ДопПоле");
		...
		Для каждого Субконто Из НастройкаРаздела.Субконто Цикл
			...
			Параметры.ГруппаДляДобавления = НачальнаяГруппаДляДобавления;
			ДобавитьГруппыНаФормуДляТекущегоЭлемента(Субконто.Значение, Параметры, "Субконто");
			ГруппыБылиДобавлены = Параметры.ГруппаДляДобавления <> НачальнаяГруппаДляДобавления;
			
			Параметры.ЭлементВыводится = Субконто.Пометка;
			Параметры.Представление = Субконто.Представление;
			ЭлементСчетаУчетаНаФорме(Субконто.Значение, Параметры, "Субконто");
...
Процедура УстановитьУсловноеОформление(Форма, РазделУчета, ИмяЭлементаНастройки, ТипЭлемента = "СчетУчета") Экспорт
	...
	Если Настройка.ИспользуютсяДолгосрочныеСчетаУчета И НастройкаРазделаУчета.ДолгосрочныеСчета.Найти(ИмяЭлементаНастройки) = Неопределено Тогда
		Элемент = Форма.УсловноеОформление.Элементы.Добавить();
	...
	Если ИмяЭлементаНастройки = "СтатьяДоходовОСНО" Тогда
		...
		СписокПрочихДоходов = Новый СписокЗначений;
		СписокПрочихДоходов.Добавить(ПредопределенноеЗначение("ПланСчетов.Хозрасчетный.ПрочиеДоходы"));
		Элемент = Форма.УсловноеОформление.Элементы.Добавить();
		КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(Элемент.Поля, Форма.Элементы[ИмяЭлементаФормы].Имя);
		КомпоновкаДанныхКлиентСервер.ДобавитьОтбор(Элемент.Отбор,
			НастройкаСчетовУчетаКлиентСервер.ИмяРеквизитаНаФорме(Настройка, РазделУчета, "ВыручкаОтПродаж"),
			Неопределено,
		...
		КомпоновкаДанныхКлиентСервер.ДобавитьОтбор(Элемент.Отбор,
			НастройкаСчетовУчетаКлиентСервер.ИмяРеквизитаНаФорме(Настройка, РазделУчета, ИмяЭлементаНастройки, ТипЭлемента),
			Неопределено,
	...
	Если ИмяЭлементаНастройки = "СтатьяДоходовЕНВД" Тогда
		...
		КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(Элемент.Поля, Форма.Элементы[ИмяЭлементаФормы].Имя);
		КомпоновкаДанныхКлиентСервер.ДобавитьОтбор(Элемент.Отбор,
			НастройкаСчетовУчетаКлиентСервер.ИмяРеквизитаНаФорме(Настройка, РазделУчета, "ВыручкаОтПродажЕНВД"),
			Неопределено,
			ВидСравненияКомпоновкиДанных.Заполнено);
		КомпоновкаДанныхКлиентСервер.ДобавитьОтбор(Элемент.Отбор,
			НастройкаСчетовУчетаКлиентСервер.ИмяРеквизитаНаФорме(Настройка, РазделУчета, "ВыручкаОтПродажЕНВД"),
...
Процедура ЗаполнениеРеквизитовФормы(Форма) Экспорт
	...
	ТипОтноситсяКМалоценнымТМЦ = Новый ОписаниеТипов("Булево");
	...
	Для каждого НастройкаРаздела Из Настройка.НастройкиРазделов Цикл
		Для каждого СчетУчета Из НастройкаРаздела.Значение.СчетаУчета Цикл
			Если СчетУчета.Пометка Тогда
				...
				КонецЕсли;
				Если НастройкаСчетовУчетаКлиентСервер.ИспользуютсяМалоценныеТМЦ(СчетУчета.Значение) Тогда
					ИмяРеквизита = "ОтноситсяКМалоценнымТМЦ";
					ИмяРеквизита = НастройкаСчетовУчетаКлиентСервер.ИмяРеквизитаНаФорме(Настройка, НастройкаРаздела.Ключ, ИмяРеквизита, "ДопПоле");
					Если НЕ ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, ИмяРеквизита)
						И Настройка.СоздаватьВСлучаеОтсутствия И ДобавляемыеЭлементы.Найти(ИмяРеквизита) = Неопределено Тогда
						ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы(ИмяРеквизита, ТипОтноситсяКМалоценнымТМЦ,,, Истина));
						ДобавляемыеЭлементы.Добавить(ИмяРеквизита);
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
			...
			Если Субконто.Пометка Тогда
				...
				Если НЕ ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма, ИмяРеквизита)
					...
					Если ТипСубконто <> Неопределено Тогда
						ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы(ИмяРеквизита, ТипСубконто,,, Истина));
						ДобавляемыеЭлементы.Добавить(ИмяРеквизита);
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
...
Функция СчетаУчетаРасчетовРегл() Экспорт
	...
	ВидыСчетов.Вставить(Перечисления.ВидыСчетовРеглУчета.РасчетыПоПретензиям,       "СчетУчетаРасчетовПоПретензиям");
...
// Заполняет реквизит НастройкаСчетовУчета в табличных частах ВидыЗапасов из табличных частей-источников 
// по соответсвию в идентификаторах строки.
// 
// Параметры:
// 	Объект - ДокументОбъект - Документ, в котором требуется заполнить реквизит НастройкаСчетовУчета
// 	СоответствияТЧ - Структура - Структура, в ключах которой содержится имя ТЧ ВидыЗапасов, в значениях имена табличных 
// 	                             частей источников через запятую. Табличные части ВидыЗапасов и источники для ее
// 	                             заполнения должны содержать реквизиты ИдентификаторСтроки и НастройкаСчетовУчета.
Процедура ЗаполнитьНастройкиСчетовУчетаВидыЗапасов(Объект, СоответствияТЧ) Экспорт
	
	Для Каждого Соответствие Из СоответствияТЧ Цикл
		ДанныеДляЗаполнения = Новый Соответствие();
		
		Источники = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(Соответствие.Значение);
		Для Каждого Источник Из Источники Цикл
			Для Каждого СтрокаИсточника Из Объект[Источник] Цикл
				Если ЗначениеЗаполнено(СтрокаИсточника["НастройкаСчетовУчета"]) Тогда
					ДанныеДляЗаполнения.Вставить(СтрокаИсточника["ИдентификаторСтроки"], СтрокаИсточника["НастройкаСчетовУчета"]);
				КонецЕсли;
			КонецЦикла;
		КонецЦикла;
		
		Для Каждого СтрокаВидовЗапасов Из Объект[Соответствие.Ключ] Цикл
			НастройкаСчетовДляЗаполнения = ДанныеДляЗаполнения[СтрокаВидовЗапасов["ИдентификаторСтроки"]];
			Если ЗначениеЗаполнено(НастройкаСчетовДляЗаполнения)
			И НЕ ЗначениеЗаполнено(СтрокаВидовЗапасов["НастройкаСчетовУчета"]) Тогда
				СтрокаВидовЗапасов["НастройкаСчетовУчета"] = НастройкаСчетовДляЗаполнения;
			КонецЕсли;
		КонецЦикла;
	КонецЦикла;
...
Функция ДанныеНастройкиОтраженияОперации(Форма, ПараметрыНастройкиСчетов)
	...
	//++ Локализация
	ДанныеНастройкиСчетовУчета.ЗначенияФункциональныхОпций.ИспользоватьРеглУчет = ПолучитьФункциональнуюОпцию("ИспользоватьРеглУчет");
	//-- Локализация
	
	//++ НЕ УТКА
	ДанныеНастройкиСчетовУчета.ЗначенияФункциональныхОпций.ИспользоватьМФУ = ПолучитьФункциональнуюОпцию("ИспользоватьМеждународныйФинансовыйУчет");
	ДанныеНастройкиСчетовУчета.СтатьиСНастройкамиПоОбъектам = Новый ФиксированныйМассив(Перечисления.ОбъектыБухгалтерскогоУчета.СтатьиАктивовПассивовСНастройками());
	//-- НЕ УТКА
...
Процедура ПроверитьПараметрыНастройкиСчетовУчета(Форма, МассивПараметровНастройки)
		...
		Иначе
			ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(
						...
						СтрШаблон(НСтр("ru = 'Указан неверный путь к данным %1 в параметрах настройки счетов учета.';
										|en = 'Invalid path is specified for data %1 in GL account settings.'"), 
				...
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(
						...
						СтрШаблон(НСтр("ru = 'Отсутствует реквизит %1 в %2 формы объекта.';
									...
									?(ПоискПоТЧ, СтрШаблон(НСтр("ru = 'табличной части %1';
																|en = 'tabular section %1'"), ИмяТЧ), НСтр("ru = 'списке реквизитов';
				...
				ИначеЕсли (ЗначениеЗаполнено(ИмяТЧРеквизита) И ПоискПоТЧ И НЕ ИмяТЧРеквизита = ИмяТЧ) Тогда
					 ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(
							...
							СтрШаблон(НСтр("ru = 'Путь к реквизиту %1 ссылается на неправильную табличную часть %2, ожидается %3.';
											|en = 'Path to attribute %1 refers to an invalid tabular section %2, expected %3.'"), 
										ИмяРеквизита,
										ИмяТЧРеквизита,
										ИмяТЧ),
							"");
				...
				ИначеЕсли (ЗначениеЗаполнено(ИмяТЧРеквизита) 
						...
						ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(
							...
							СтрШаблон(НСтр("ru = 'Реквизит %1 отсутствует в табличной части %2.';
											|en = 'Attribute %1 is missing in the tabular section %2.'"), 
										ИмяРеквизита,
										ИмяТЧ),
							"");
		...
		Для каждого ИмяЭлемента Из ПараметрНастройки.ЭлементыФормы Цикл
			Если НЕ ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(Форма.Элементы, ИмяЭлемента) Тогда
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(
						...
						"",
						СтрШаблон(НСтр("ru = 'Отсутствует реквизит %1 в объекте формы.';
										|en = 'Attribute %1 is missing in the form object.'"), 
									ИмяЭлемента),
						"");
	...
	Если НЕ Ошибки = Неопределено Тогда
		ТекстИсключения = НСтр("ru = 'Найдены ошибки при проверке параметров настройки счетов учета';
								|en = 'Errors found while validating GL account settings'");
...
Функция НовыйЭлементНастройкиСчетовУчета(ДанныеНастройки, ПометкаУдаления, Владелец, СсылкаНового = Неопределено)
...
Функция ЭлементСчетаУчетаНаФорме(ИмяЭлементаНастройки, Параметры, ТипЭлемента = "СчетУчета")
	...
	Если ЭлементФормы <> Неопределено Тогда
		...
		Если ЭлементФормы.Видимость Тогда
			...
			КонецЕсли;
			НастройкаСчетовУчетаКлиентСервер.УстановитьСвойстваСчетовНаФорме(Параметры.Форма, ИмяЭлементаНастройки, Параметры);
...
Процедура ДобавитьГруппыНаФормуДляТекущегоЭлемента(ИмяЭлемента, Параметры, ТипЭлемента = "СчетУчета")
	...
	Настройка = Параметры.Форма.КэшДанныхМеханизмов.НастройкаСчетовУчета;
	ТипЭлементаГруппы = ?(ТипЭлемента = "СчетУчета", "Группа", "ГруппаДолгосрочные");
	...
	Если Настройка.ИспользуютсяДолгосрочныеСчетаУчета И Настройка.СворачиваемыеГруппы.Количество() = 0 Тогда
		...
		ИмяГруппы = ?(ТипЭлемента = "СчетУчета", "Краткосрочные", "Долгосрочные");
	...
	НастройкиГруппы = НастройкаСчетовУчетаКлиентСервер.ГруппаСчетаУчета(ИмяЭлемента, Параметры.РазделУчета, Настройка.РедактированиеВСписке);
