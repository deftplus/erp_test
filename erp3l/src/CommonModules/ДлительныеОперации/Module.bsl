...
Функция ВыполнитьВФоне(Знач ИмяПроцедуры, Знач ПараметрыПроцедуры, Знач ПараметрыВыполнения) Экспорт
	...
	Если ПараметрыВыполнения.ЗапуститьНеВФоне И ПараметрыВыполнения.ЗапуститьВФоне Тогда
		ВызватьИсключение НСтр("ru = 'Параметры ""ВсегдаНеВФоне"" и ""ВсегдаВФоне""
			...
			|cannot both be True in TimeConsumingOperations.ExecuteInBackground.'");
	КонецЕсли;
	Если ПараметрыВыполнения.БезРасширений И ПараметрыВыполнения.СРасширениямиБазыДанных Тогда
		ВызватьИсключение НСтр("ru = 'Параметры ""БезРасширений"" и ""СРасширениямиБазыДанных""
			|не могут одновременно принимать значение Истина в ДлительныеОперации.ВыполнитьВФоне.';
			|en = 'Parameters ""NoExtensions"" and ""WithDatabaseExtensions""
			|cannot both be True in TimeConsumingOperations.ExecuteInBackground.'");
	КонецЕсли;
#Если ВнешнееСоединение Тогда
	...
	ИначеЕсли ПараметрыВыполнения.СРасширениямиБазыДанных И ИнформационнаяБазаФайловая Тогда
		ВызватьИсключение НСтр("ru = 'Фоновое задание не может быть запущено с параметром ""СРасширениямиБазыДанных""
			|в файловой информационной базе в ДлительныеОперации.ВыполнитьВФоне.';
			|en = 'Cannot start a background job with ""WithDatabaseExtensions"" parameter
			|in a file infobase in TimeConsumingOperations.ExecuteInBackground.'");
	КонецЕсли;
#КонецЕсли
	...
	Если ПараметрыВыполнения.Свойство("АдресРезультата") Тогда
			...
			Если Не ЗначениеЗаполнено(ПараметрыВыполнения.ИдентификаторФормы) И ОбщегоНазначения.РежимОтладки() Тогда
				Попытка
					ВызватьИсключение НСтр("ru = 'Не указан ни уникальный идентификатор формы в параметре ПараметрыВыполнения.ИдентификаторФормы,
						...
						|en = 'Form UUID is not specified in the ExecutionParameters.FormID parameter and temporary storage address is not specified
						|in the ExecutionParameters.ResultAddress parameter in TimeConsumingOperations.ExecuteInBackground.
						|Make sure that the temporary storage is cleared explicitly with the DeleteFromTempStorage method on result processing.'");
				Исключение
		...
		ИначеЕсли Не ЭтоАдресВременногоХранилища(ПараметрыВыполнения.АдресРезультата) Тогда
			ВызватьИсключение НСтр("ru = 'Не указан адрес временного хранилища в параметре ПараметрыВыполнения.АдресРезультата
				...
				|en = 'Temporary storage address is not specified in the ExecutionParameters.ResultAddress parameter
				|in TimeConsumingOperations.ExecuteInBackground.'");
		КонецЕсли;	
	...
	ИначеЕсли Задание.Состояние = СостояниеФоновогоЗадания.Отменено Тогда
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Длительные операции.Фоновое задание отменено администратором';
				|en = 'Long-running operations.Background job canceled by administrator'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка, , , НСтр("ru = 'Задание завершилось с неизвестной ошибкой.';
														|en = 'The job completed with an unknown error.'"));
	Иначе
		Возврат Истина;
	КонецЕсли;
...
Функция ОперацияВыполнена(Знач ИдентификаторЗадания, Знач ИсключениеПриОшибке = Ложь, Знач ВыводитьПрогрессВыполнения = Ложь, 
	...
	Если Задание = Неопределено Тогда
		Пояснение = НСтр("ru = 'Операция не выполнена из-за аварийного завершения фонового задания.
			|Фоновое задание не существует';
			|en = 'Cannot perform the operation due to abnormal termination of a background job.
			|The background job does not exist'") + ": " + Строка(ИдентификаторЗадания);
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Длительные операции.Ошибка выполнения';
										|en = 'Long-running operations.Runtime error'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ВызватьПроцедуру(ИмяПроцедуры, ПараметрыВызова)
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Неверный формат параметра ИмяПроцедуры (переданное значение: %1)';
			|en = 'Invalid format of the ProcedureName parameter (passed value: %1).'"), ИмяПроцедуры);
...
Процедура ВызватьФункцию(ИмяФункции, ПараметрыПроцедуры, АдресРезультата)
	...
	ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Неверный формат параметра ИмяФункции (переданное значение: %1)';
			|en = 'Invalid format of the FunctionName parameter (passed value: %1).'"), ИмяФункции);
