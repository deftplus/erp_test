...
Процедура РасчетПоказателей_РСВ_2017(НачалоПериода, ОкончаниеПериода, Организация, ОписаниеТаблиц, ПодробныеДанные, ДатаАктуальности)
	...
	Если ОписаниеТаблиц.Свойство("ПособияПоНетрудоспособности2017") Или ОписаниеТаблиц.Свойство("ПособияПоНетрудоспособностиСверхНорм2017") Или ОписаниеТаблиц.Свойство("ПолучателейПособийСверхНорм2017") Тогда
		...
		|	Пособия.СтраховыеСлучаиОборот КАК СтраховыеСлучаиОборот,
		|	Пособия.ОплаченныеДниОборот КАК ОплаченныеДниОборот,
		|	Пособия.СуммаВсегоОборот КАК СуммаВсегоОборот,
		|	Пособия.СуммаСверхНормОборот КАК СуммаСверхНормОборот,
		|	Пособия.ФизическоеЛицо КАК ФизЛицоДляСвязей
		|ПОМЕСТИТЬ ВТДанныеУчета
		|ИЗ
		|	РегистрНакопления.ПособияПоСоциальномуСтрахованию.Обороты(&НачалоОтчетногоПериода, &КонецОтчетногоПериода, Месяц, Организация В (&Организация)) КАК Пособия";
		Запрос.Выполнить();
	КонецЕсли;
	...
	Если ОписаниеТаблиц.Свойство("ПособияПоУходу2017") Или ОписаниеТаблиц.Свойство("ПолучателейПособияПоУходу2017") Тогда
		...
		|	ПособияСоциальномуСтрахованиюОбороты.СтраховыеСлучаиПоУходуЗаПервымРебенкомОборот КАК СтраховыеСлучаиПоУходуЗаПервымРебенкомОборот,
		|	ПособияСоциальномуСтрахованиюОбороты.СуммаВсегоПоУходуЗаПервымРебенкомОборот КАК СуммаВсегоПоУходуЗаПервымРебенкомОборот,
		|	ПособияСоциальномуСтрахованиюОбороты.СуммаСверхНормПоУходуЗаПервымРебенкомОборот КАК СуммаСверхНормПоУходуЗаПервымРебенкомОборот,
		|	ПособияСоциальномуСтрахованиюОбороты.СтраховыеСлучаиПоУходуЗаВторымРебенкомОборот КАК СтраховыеСлучаиПоУходуЗаВторымРебенкомОборот,
		|	ПособияСоциальномуСтрахованиюОбороты.СуммаВсегоПоУходуЗаВторымРебенкомОборот КАК СуммаВсегоПоУходуЗаВторымРебенкомОборот,
		|	ПособияСоциальномуСтрахованиюОбороты.СуммаСверхНормПоУходуЗаВторымРебенкомОборот КАК СуммаСверхНормПоУходуЗаВторымРебенкомОборот,
		|	ПособияСоциальномуСтрахованиюОбороты.Период КАК Период
		|ПОМЕСТИТЬ ВТПособияПоУходу
		|ИЗ
		|	РегистрНакопления.ПособияПоУходуЗаРебенком.Обороты(&НачалоОтчетногоПериода, &КонецОтчетногоПериода, Квартал, Организация В (&Организация)) КАК ПособияСоциальномуСтрахованиюОбороты
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ФизическоеЛицо";
		Запрос.Выполнить();
	...
	Если ОписаниеТаблиц.Свойство("ПособияПоУходуСверхНорм2017") Или ОписаниеТаблиц.Свойство("ПолучателейПособияПоУходуЗаПервымРебенкомСверхНорм2017")
		...
		|	ПособияСоциальномуСтрахованию.ФизическоеЛицо КАК ФизическоеЛицо,
		|	ПособияСоциальномуСтрахованию.ФинансированиеФедеральнымБюджетом КАК ФинансированиеФедеральнымБюджетом,
		|	ПособияСоциальномуСтрахованию.СтраховыеСлучаиПоУходуЗаПервымРебенкомОборот КАК СтраховыеСлучаиПоУходуЗаПервымРебенкомОборот,
		|	ПособияСоциальномуСтрахованию.СуммаСверхНормПоУходуЗаПервымРебенкомОборот КАК СуммаСверхНормПоУходуЗаПервымРебенкомОборот,
		|	ПособияСоциальномуСтрахованию.СтраховыеСлучаиПоУходуЗаВторымРебенкомОборот КАК СтраховыеСлучаиПоУходуЗаВторымРебенкомОборот,
		|	ПособияСоциальномуСтрахованию.СуммаСверхНормПоУходуЗаВторымРебенкомОборот КАК СуммаСверхНормПоУходуЗаВторымРебенкомОборот
...
Функция НаименованиеПодсистемы(КодЯзыка = Неопределено)
	Возврат НСтр("ru = 'Пособия социального страхования';
				|en = 'Social insurance allowances'", ?(КодЯзыка = Неопределено, ОбщегоНазначения.КодОсновногоЯзыка(), КодЯзыка));
КонецФункции
