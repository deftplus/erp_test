...
Процедура ПроверитьПараметрПадеж(Падеж, ИмяПроцедурыИлиФункции)
	...
	ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Недопустимое значение параметра Падеж в СклонениеПредставлениеОбъектов.Просклонять.
              ...
              |en = 'Invalid value of the Case parameter in DeclensionObjectsPresentation.Decline.
              |The parameter must be a number indicating the serial number of the case, from 1 to 6.
              |The passed value %1 (type %2).'"),
		Падеж,
		ТипЗнч(Падеж));
