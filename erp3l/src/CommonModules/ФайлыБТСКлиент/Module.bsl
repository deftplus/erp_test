...
Процедура ПоместитьФайлИнтерактивно(ПараметрыПомещенияФайла) Экспорт
	...
	|This extension improves file management in web client.'"), ФайлыБТСКлиентСервер.ПредставлениеРазмераФайла(
	...
	Если СсылкаНаФайл.Размер > МаксимальныйРазмер Тогда
		Если МаксимальныйРазмер <= 0 Тогда
			ТекстОшибки = СтрШаблон(НСтр("ru = 'Размер файла должен быть равен %1';
										|en = 'File size must be %1'"), ФайлыБТСКлиентСервер.ПредставлениеРазмераФайла(0));
		Иначе
			ТекстОшибки = СтрШаблон(НСтр("ru = 'Файл %1 слишком большой. Выберите файл менее %2';
										|en = 'File %1 is too large. Select a file with size less than %2'"),
