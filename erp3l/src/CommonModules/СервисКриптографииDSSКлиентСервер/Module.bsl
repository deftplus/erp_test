...
Функция ОписаниеОшибки_ОшибкаВыполнения()
	...
	Результат = ОбщееОписаниеОшибки("100003", 
			...
			|en = 'Service error.
			|1. The server might be currently unavailable, check the network settings and server operation.
			|2. Incorrect query parameters.
			|3. Check the settings and make sure the 1C server is authorized to connect to the DSS server.
...
Функция ОписаниеОшибки_УстановкаРасширенияСФайлами()
	...
	Результат = ОбщееОписаниеОшибки("300046", 
			НСтр("ru = 'Не удалось установить расширения работы с файлами в Веб-клиенте.';
				|en = 'Cannot set file system extensions in the web client.'"));
...
Функция ОписаниеОшибки_ПолучениеФайла()
	...
	Результат = ОбщееОписаниеОшибки("300047", 
			НСтр("ru = 'Возникла ошибка при получении данных файла.';
				|en = 'An error occurred upon getting file data.'"));
...
Функция ПолучитьПредставлениеСпособовАвторизации(ЗначениеВыбора) Экспорт
	...
	Варианты.Вставить(ПредопределенноеЗначение("Перечисление.СпособыАвторизацииDSS.Первичный_ДвухФакторная"),
			НСтр("ru = 'Логин и пароль с дополнительным подтверждением';
				|en = 'Username and password with additional confirmation'"));
	...
	Варианты.Вставить(ПредопределенноеЗначение("Перечисление.СпособыАвторизацииDSS.Вторичный_ОфлайнПодтверждение"),
			НСтр("ru = 'Мобильное приложение недоступно';
				|en = 'Mobile application is not available'"));
