...
Функция ОписаниеЗапросаДляВыбора(Параметры, УстановитьВсеОтборы, МассивОбъектов) Экспорт
	...
	ТекстСоединенияМестонахождениеОС = "";
	
	Если ЭтоВыборУзлаКомпонента Тогда
		ТекстСоединенияМестонахождениеОС = ТекстСоединенияМестонахождениеОС + "
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.УзлыКомпонентыАмортизации.СрезПоследних(&ПАРАМЕТРЫ_СРЕЗАПОСЛЕДНИХ_ДАТА,&ПАРАМЕТРЫ_СРЕЗАПОСЛЕДНИХ) КАК УзлыКомпоненты
	...
	|ИЗ
	|	РегистрСведений.МестонахождениеОС.СрезПоследних(&ПАРАМЕТРЫ_СРЕЗАПОСЛЕДНИХ_ДАТА, &ПАРАМЕТРЫ_СРЕЗАПОСЛЕДНИХ) КАК МестонахождениеОС
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ОсновноеСредство,
	|	Организация
	...
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "//СОЕДИНЕНИЕ_МЕСТОНАХОЖДЕНИЕ_ОС//", ТекстСоединенияМестонахождениеОС);
	...
	Если Параметры.Свойство("ЕстьРегистрацияТранспортныхСредств")
		ИЛИ Параметры.Свойство("ЕстьРегистрацияЗемельныхУчастков") Тогда
...
Функция ПервоначальныеСведения(ОсновноеСредствоИлиСписок, Период) Экспорт
	...
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПараметрыАмортизацииОСБУ.СрезПоследних(&Период, ОсновноеСредство В (&СписокОС)) КАК ПараметрыАмортизацииОСБУ
...
Функция СведенияОбУчете(ОсновноеСредство, ОтборОрганизация) Экспорт
	...
	|	ЕСТЬNULL(ПервоначальныеСведенияОС.ДокументСнятияСУчетаБУ.Дата, ДАТАВРЕМЯ(1, 1, 1)) КАК ДатаСнятияСУчетаБУ,
	|	ЕСТЬNULL(ПервоначальныеСведенияОС.ДокументСнятияСУчетаНУ.Дата, ДАТАВРЕМЯ(1, 1, 1)) КАК ДатаСнятияСУчетаНУ,
	...
	|		ПО (ПервоначальныеСведенияОС.ОсновноеСредство = МестонахождениеОС.ОсновноеСредство)
	|			И (ПервоначальныеСведенияОС.Организация = МестонахождениеОС.Организация)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПорядокУчетаОС.СрезПоследних(, ОсновноеСредство = &Ссылка) КАК ПорядокУчетаОС
...
Функция МожноПринятьКУчету(СведенияОбУчете) Экспорт
	...
	МожноПринятьКУчету = 
		СведенияОбУчете.СостояниеБУ <> Перечисления.СостоянияОС.СнятоСУчета
		И СведенияОбУчете.СостояниеНУ <> Перечисления.СостоянияОС.СнятоСУчета;
	
	//++ Локализация
	МожноПринятьКУчету =
		СведенияОбУчете.ДокументВводаВЭксплуатациюБУ <> Тип("ДокументСсылка.ПоступлениеАрендованныхОС")
...
Функция ДанныеПечатнойФормыОС6_2003(МассивОбъектов, ДатаСведений)
	...
	|ИЗ
	|	РегистрСведений.ПервоначальныеСведенияОС.СрезПоследних(&ДатаСведений, ОсновноеСредство В (&МассивОбъектов)) КАК ПервоначальныеСведенияОС
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ОС
	|;
...
Процедура ПрочитатьПериодическиеРеквизиты(Форма) Экспорт
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.ПорядокУчетаОСБУ.СрезПоследних(, ОсновноеСредство = &ОсновноеСредство) КАК ПорядокУчетаОСБУ
...
Процедура ПриСозданииНаСервере_ФормаВыбора2_4(Форма) Экспорт
	...
	ИначеЕсли Параметры.Свойство("ЕстьРегистрацияЗемельныхУчастков") Тогда
		...
		Форма.Заголовок = НСтр("ru = 'Зарегистрированные земельные участки';
								|en = 'Registered land lots'");
		
	ИначеЕсли ОтборСписка.Свойство("ВидНалога") Тогда
		
		Форма.Заголовок = ЗаголовокФормыВыбораОСПоВидуНалога(ОтборСписка.ВидНалога, Параметры.ДатаСведений);
...
#Область ФормаОтборОстаточногоСрокаИспользования

// Вызывается при создании формы ОтборОстаточногоСрокаИспользования
//
// Параметры:
//  Форма  - ФормаКлиентскогоПриложения - открываемая форма
//
Процедура ПриСозданииНаСервере_ОтборОстаточногоСрокаИспользования(Форма) Экспорт

	//++ Локализация
	
	СписокВидовУчета = Форма.Элементы.ОтборОстаточныйСрокИспользованияВидУчета.СписокВыбора; //СписокЗначений
...
Функция АмортизационныеГруппыОС(СписокОС, Дата) Экспорт
	...
	|ПОМЕСТИТЬ МестонахождениеОС
	|ИЗ
	|	РегистрСведений.МестонахождениеОС.СрезПоследних(&Дата, ОсновноеСредство В (&СписокОС)) КАК МестонахождениеОС
	|
	|ИНДЕКСИРОВАТЬ ПО
	|	ОсновноеСредство,
	|	Организация
