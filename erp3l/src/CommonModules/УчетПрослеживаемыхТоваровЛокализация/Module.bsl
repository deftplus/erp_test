	...
	Если ПроведениеДокументов.ЕстьЗаписиВТаблице(Документ, "ДвиженияПрослеживаемыеТоварыВСоставеОСИзменение") Тогда
		...
		Если Документ.ДополнительныеСвойства.ПроведениеДокументов.Свойство("КонтролироватьСписаниеВНоль")  
			 И Документ.ДополнительныеСвойства.ПроведениеДокументов.КонтролироватьСписаниеВНоль Тогда
					...
					Если СтрокаОшибки.КоличествоПоРНПТОстаток > Макс(СтрокаОшибки.КоличествоСписываемое, -СтрокаОшибки.КоличествоСписываемое) Тогда			
...
Функция ШаблонСообщенияОтличияКодовТНВЭДСборки(ТипНоменклатуры)
	...
	Если ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Набор Тогда
		ТекстШаблона = НСтр("ru = 'Отличаются коды ТН ВЭД прослеживаемого по РНПТ набора ""%1"" и основной комплектующей ""%2"".
							...
							|en = 'FEACN codes of the ""%1"" set traceable by a goods batch registration number and the ""%2"" main component differ.
							|To correctly account for sets traceable by goods batch registration numbers, specify the main component with the FEACN code equal to the set code.'");
	Иначе
		ТекстШаблона = НСтр("ru = 'Отличаются коды ТН ВЭД прослеживаемого по РНПТ комплекта ""%1"" и основной комплектующей ""%2"".
							...
							|en = 'FEACN codes of the ""%1"" kit traceable by goods batch registration numbers and the ""%2"" main component differ.
							|To correctly account for kits traceable by goods batch registration numbers, specify the main component with the FEACN code equal to the kit code.'");
	КонецЕсли;
...
Функция ШаблонСообщенияЗаполненияОсновнойКомплектующей(ТипНоменклатуры)
	...
	Если ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Набор Тогда
		ТекстШаблона = НСтр("ru = 'Не указана основная комплектующая для прослеживаемого по РНПТ набора ""%1"".
							|Для ведения корректного учета прослеживаемых по РНПТ наборов необходимо указать основную комплектующую с кодом ТН ВЭД аналогичным коду набора.';
							|en = 'The main component for the ""%1"" set traceable by goods batch registration numbers is not specified.
							|To correctly account for sets traceable by goods batch registration numbers, specify the main component with the FEACN code equal to the set code.'");
	Иначе
		ТекстШаблона = НСтр("ru = 'Не указана основная комплектующая для прослеживаемого по РНПТ комплекта ""%1"".
							|Для ведения корректного учета прослеживаемых по РНПТ комплектов необходимо указать основную комплектующую с кодом ТН ВЭД аналогичным коду комплекта.';
							|en = 'The main component for the ""%1"" kit traceable by goods batch registration numbers is not specified.
							|To correctly account for kits traceable by goods batch registration numbers, specify the main component with the FEACN code equal to the kit code.'");
	КонецЕсли;
...
Функция ШаблонСообщенияОтличияКодовТНВЭДРазборки(ТипНоменклатуры)
	...
	Иначе
		ТекстШаблона = НСтр("ru = 'Отличаются коды ТН ВЭД прослеживаемого по РНПТ комплекта ""%1"" и комплектующей ""%2"".
							...
							|en = 'FEACN codes of the ""%1"" kit traceable by goods batch registration numbers and the ""%2"" component differ.
							|To correctly account for kits traceable by goods batch registration numbers, specify the component with the FEACN code equal to the kit code.'");
	КонецЕсли;
...
Функция ШаблонСообщенияРазличныеПрослеживаемыеКомплектующие(ТипНоменклатуры, СборкаКомплекта, РедактированиеНабора)
	...
	Если ТипНоменклатуры = Перечисления.ТипыНоменклатуры.Набор Тогда
		ТекстШаблона = НСтр("ru = 'Прослеживаемый по РНПТ набор ""%1"" не должен содержать различные прослеживаемые комплектующие:
							|%2.';
							|en = 'The ""%1"" set traceable by a goods batch registration number must not contain traceable components:
							|%2.'");
		...
		Если Не РедактированиеНабора Тогда
			ТекстШаблона = ТекстШаблона + ПереносТекста + НСтр("ru = 'Оставьте в составе набора одну прослеживаемую по РНПТ комплектующую и укажите ее в качестве основной.';
																|en = 'Keep one component traceable by a goods batch registration number in the set and set it as the main one.'");
		КонецЕсли;
	Иначе
		ТекстШаблона = НСтр("ru = 'Прослеживаемый по РНПТ комплект ""%1"" не должен содержать различные прослеживаемые комплектующие:
							|%2.';
							|en = 'The ""%1"" kit traceable by a goods batch registration number must not contain different traceable components:
							|%2.'");
		...
		Иначе
			ТекстШаблона = ТекстШаблона + ПереносТекста + НСтр("ru = 'Оставьте в составе комплекта одну прослеживаемую по РНПТ комплектующую.';
																|en = 'Keep one component traceable by a goods batch registration number in the kit.'");
		КонецЕсли;
	КонецЕсли;
