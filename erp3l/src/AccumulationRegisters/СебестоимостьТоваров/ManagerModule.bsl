...
Функция ПараметрыОтраженияДвиженийВФинансовомУчете() Экспорт
	...
	ПараметрыОтражения.ПутьКДаннымПодразделение = "ВЫБОР
		|		КОГДА АналитикаУчетаНоменклатуры.МестоХранения ССЫЛКА Справочник.СтруктураПредприятия
		|			ТОГДА АналитикаУчетаНоменклатуры.МестоХранения
		|		КОГДА АналитикаУчетаНоменклатуры.МестоХранения ССЫЛКА Справочник.Склады
		|			ТОГДА АналитикаУчетаНоменклатуры.МестоХранения.Подразделение
		|		ИНАЧЕ АналитикаУчетаНоменклатуры.Подразделение
		|	КОНЕЦ";
	...
	ПараметрыОтражения.ДополнительныйОтбор = "НЕ РазделУчета В (ЗНАЧЕНИЕ(Перечисление.РазделыУчетаСебестоимостиТоваров.ТоварыПринятыеНаКомиссию),
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Очищает измерение ""Вид запасов"" в движениях регистра ""Себестоимость товаров"" для готовой продукции, отраженной отчетами переработчиков.';
									|en = 'Cleans the dimension ""Inventory owner attributes"" in movements of the register ""Inventory cost"" for the ready-made products displayed by reports of recyclers.'");
