...
Процедура ПриЗаписи(Отказ, Замещение)
	...
	Если Не ДополнительныеСвойства.Свойство("ОтключитьРасчетИтогов") Тогда
		...
		ЗаписатьСведенияПоТоварам(Запрос); // вызов обязательно до ЗаписатьСведенияПоЗаказамНаОтгрузку, так как читаются данные до изменений.
...
Процедура СформироватьТаблицуИзмененийДвижений()
	...
	Запрос.Текст =
		...
		|	МИНИМУМ(ТаблицаИзменений.ИгнорироватьРезервыПриКонтролеОстатков)                КАК ИгнорироватьРезервыПриКонтролеОстатков
		|	
...
Процедура УстановитьБлокировки(Запрос)
	...
	// см. ЗаписатьСведенияПоТоварам и ЗаписатьРаспределениеЗапасовНаПотребности
...
Процедура ЗаписатьСведенияПоЗаказамНаОтгрузку(Запрос)
	...
	Текст =
		...
		|			ИЛИ СУММА(Движения.КОбеспечению) <> 0
		...
		|		ГДЕ
		|			Движения.ЗаказНаОтгрузку = ЗаказыНаОтгрузку.ЗаказНаОтгрузку)
		|		И НЕ ИСТИНА В(
		|			ВЫБРАТЬ ПЕРВЫЕ 1
		|				ИСТИНА КАК ЕстьЗаписи
		|			ИЗ
		...
		|			ГДЕ
		|				Сведения.ЗаказНаОтгрузку = ЗаказыНаОтгрузку.ЗаказНаОтгрузку
		...
		|	Сведения.КОбеспечению                  КАК КОбеспечению,
		|	Сведения.ОтложитьОбеспечение           КАК ОтложитьОбеспечение,
	...
	// Пустые наборы для заказов в результате проведения которых записи в регистре сведений нужно удалить.
					...
					Пока Выборка.СледующийПоЗначениюПоля("Назначение") Цикл
						...
						Пока Выборка.Следующий() Цикл
							...
							Если Выборка.КОбеспечению > 0 Тогда
								Итог = Итог + Выборка.КОбеспечению;
								НоваяСтрока.КОбеспечениюИтогНаДату = Итог;
							КонецЕсли;
...
Процедура ЗаписатьСведенияПоЗаказамНаПоступление(Запрос)
	...
	ВсеЗаказы = Пакет[4].Выгрузить().ВыгрузитьКолонку("ЗаказНаПоступление");
	...
	Пока Выборка.СледующийПоЗначениюПоля("ЗаказНаПоступление") Цикл
		...
		Индекс = ВсеЗаказы.Найти(Выборка.ЗаказНаПоступление);
		Если Индекс <> Неопределено Тогда
			ВсеЗаказы.Удалить(Индекс);
		КонецЕсли;
	...
	Набор.Отбор.ЗаказНаОтгрузку.Значение = Неопределено;
	Набор.Отбор.ЗаказНаОтгрузку.Использование = Истина;
	Для Каждого ЗаказНаПоступление Из ВсеЗаказы Цикл
		Набор.Отбор.ЗаказНаПоступление.Установить(ЗаказНаПоступление);
		Набор.Записать();
	КонецЦикла;
...
Процедура ЗаписатьСведенияПоТоварам(Запрос)
	...
	Запрос.Текст =
		...
		// задание еще не отработало. В этом случае были бы получены завышенные данные о доступном для отгрузке товаре для заказа.
		...
		|	ВЫБОР КОГДА СведенияДоИзменения.КОбеспечению < СведенияДоИзменения.Распределено ТОГДА
		|			СведенияДоИзменения.КОбеспечению
		|		ИНАЧЕ
		|			СведенияДоИзменения.Распределено
		|		КОНЕЦ КАК Распределено
		|ПОМЕСТИТЬ РаспределеноНаСкладеПоЗаказамТоваровДляКонтроляОстатков
		|ИЗ(
		|	ВЫБРАТЬ
		|		СведенияДоИзменения.Номенклатура        КАК Номенклатура,
		|		СведенияДоИзменения.Характеристика      КАК Характеристика,
		...
		|		СведенияДоИзменения.Назначение          КАК Назначение,
		|		СведенияДоИзменения.ЗаказНаОтгрузку     КАК ЗаказНаОтгрузку,
		|		СУММА(СведенияДоИзменения.Распределено) КАК Распределено,
		|		СУММА(СведенияДоИзменения.КОбеспечению) КАК КОбеспечению
		|	ИЗ
		...
		|			ИНАЧЕ
		|				ВЫБОР КОГДА ИзмененияДвижений.Свободно + ЕСТЬNULL(Сведения.Распределено, 0) > 0 ТОГДА
		|					0
		|						ИНАЧЕ
		|							ИзмененияДвижений.Свободно + ЕСТЬNULL(Сведения.Распределено, 0)
		|					КОНЕЦ
		|		КОНЕЦ
		...
		|		
		|		ЛЕВОЕ СОЕДИНЕНИЕ РаспределеноНаСкладеПоЗаказамТоваровДляКонтроляОстатков КАК Сведения
		...
		|УНИЧТОЖИТЬ РаспределеноНаСкладеПоЗаказамТоваровДляКонтроляОстатков;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ВНаличии <> 0
				Или Выборка.Запас <> 0
				Или Выборка.Резерв <> 0
				Или Выборка.Свободно <> 0 Тогда
...
Процедура ЗаписатьРаспределениеЗапасовНаПотребности(Запрос)
	...
	РаспределениеЗапасовДвижения.ЗаписатьРаспределениеЗапасовНаПотребностиПоТаблицеРазличныеТовары(Запрос);
...
Процедура ЗаписатьЗадания(Запрос)
	...
	Если ДополнительныеСвойства.Свойство("ИзменилосьМестоВОчередиРаспределенияЗапасов") Тогда
		...
		Текст =
			...
			|		ПО НастройкаСклад.Склад          = Товары.Склад
			...
			|ГДЕ
			|	НастройкаСклад.ПерераспределятьСкладскиеЗапасы";
