...
Функция ПараметрыОтраженияДвиженийВФинансовомУчете() Экспорт
	...
	ПараметрыОтражения.УсловиеДебет = "ВЫБОР
	|		КОГДА ДолгУпр <> 0 ИЛИ ДолгРегл <> 0
	|			ТОГДА ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
	|		КОГДА ПредоплатаУпр <> 0 ИЛИ ПредоплатаРегл <> 0
	|			ТОГДА ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ";
	
	ПараметрыОтражения.УсловиеКредит = "ВЫБОР
	|		КОГДА ДолгУпр <> 0 ИЛИ ДолгРегл <> 0
	|			ТОГДА ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Расход)
	|		КОГДА ПредоплатаУпр <> 0 ИЛИ ПредоплатаРегл <> 0
	|			ТОГДА ВидДвижения = ЗНАЧЕНИЕ(ВидДвиженияНакопления.Приход)
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Комментарий = НСтр("ru = 'Заполняет измерения ""Объект расчетов"". Исправляет движения в регистре взаиморасчетов.';
									|en = 'Fills in ""AR/AP object"" dimensions. Corrects records in the mutual settlement register.'");
