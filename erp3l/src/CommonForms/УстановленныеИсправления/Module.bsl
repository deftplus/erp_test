	...
	Если СтандартныеПодсистемыСервер.ЭтоБазоваяВерсияКонфигурации() Тогда
		...
		Если Не ДоступнаУстановкаПатчей Тогда
			ТекстСообщения = НСтр("ru = 'Для установки исправлений (патчей) рекомендуется обновить платформу 1С:Предприятие на версию:
				...
				|en = 'To install patches, it is recommended that you update 1C:Enterprise platform to version:
				| • 8.3.15.1958 or later
				| • 8.3.16.1476 or later
				| • 8.3.17.1480 or later.'");
...
Процедура СохранитьКак(Команда)
	...
	ПараметрыСохранения.Диалог.Фильтр    = НСтр("ru = 'Файлы исправлений конфигурации (*.cfe)|*.cfe|Все файлы (*.*)|*.*';
												|en = '1C:Enterprise patch files (*.cfe)|*.cfe|All files (*.*)|*.*'");
...
Процедура УдалитьРасширения(ВыделенныеСтроки)
	...
	Если ИдентификаторыРасширений.Количество() > 1 Тогда
		ТекстВопроса = НСтр("ru = 'Удалить выделенные исправления?';
							|en = 'Do you want to delete the selected patches?'", "ru");
	Иначе
		ТекстВопроса = НСтр("ru = 'Удалить исправление?';
							|en = 'Do you want to delete the patch?'", "ru");
	КонецЕсли;
