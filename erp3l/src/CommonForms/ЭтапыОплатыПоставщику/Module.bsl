...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЕстьПредоплата = ЭтоЗаказ И НЕ ПорядокРасчетов = Перечисления.ПорядокРасчетов.ПоНакладным
					ИЛИ ПорядокРасчетов = Перечисления.ПорядокРасчетов.ПоЗаказамНакладным И НакладнаяПоЗаказам И ЗначениеЗаполнено(Параметры.АдресВоВременномХранилище);
	...
	Если ЭтоЗаказ Тогда
		ВариантОплатыПоУмолчанию = ПредопределенноеЗначение("Перечисление.ВариантыОплатыПоставщику.КредитСдвиг");
	Иначе
		ВариантОплатыПоУмолчанию = ПредопределенноеЗначение("Перечисление.ВариантыОплатыПоставщику.КредитПослеПоступления");
	КонецЕсли;
	
	УпрощенныйРежим = ИспользоватьУпрощеннуюСхемуОплатыВЗакупках И НЕ (НакладнаяПоЗаказам И ПорядокРасчетов = Перечисления.ПорядокРасчетов.ПоЗаказам)
		ИЛИ ЗначениеЗаполнено(ГрафикИсполненияДоговора) И ПорядокРасчетов = Перечисления.ПорядокРасчетов.ПоДоговорамКонтрагентов
		ИЛИ НЕ ЗначениеЗаполнено(Параметры.АдресВоВременномХранилище);
...
Процедура ЭтапыГрафикаОплатыПередУдалением(Элемент, Отказ)
	ТекущиеДанные = Элементы.ЭтапыГрафикаОплаты.ТекущиеДанные;
	Если НЕ ЭтоЗаказ И НакладнаяПоЗаказам И ТекущиеДанные <> Неопределено 
		...
		Если ЭтотОбъект.ДолжныФормироватьсяПозиции Тогда
			Для каждого Строка Из ЭтапыГрафикаОплаты Цикл
				Если НЕ Строка.НеУдалять Тогда
					Строка.ПроцентПлатежа = Строка.ПроцентПлатежа - ИтогПроцентов;
					Прервать;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
...
Процедура РаспределитьСуммуПоЭтапамИЗаказамГрафикаОплаты()
	...
	ПараметрыЗаполнения.ТипРасчетов = Перечисления.ТипыРасчетовСПартнерами.РасчетыСПоставщиком; 
	ПараметрыЗаполнения.УпрощеннаяСхема = ИспользоватьУпрощеннуюСхемуОплатыВЗакупках;
...
Процедура ЗаполнитьЭтапыГрафикаОплаты()
	...
	ПараметрыЗаполнения = Новый Структура;
	ПараметрыЗаполнения.Вставить("ЗаполнитьДату", НЕ ЕстьПредоплата И УпрощенныйРежим);
...
Процедура ПроверитьКорректностьЖелаемойДатыПоступления(ПараметрыЗаполнения)
	...
	Если ЗначениеЗаполнено(ЖелаемаяДатаПоступления) И ЖелаемаяДатаПоступления < НачалоДня(Дата) Тогда
		...
		ТекстВопроса = НСтр("ru = 'Желаемая дата поступления меньше даты документа. Необходимо ввести корректную желаемую дату.';
							|en = 'Required date of receipt is less than the document date. Enter a correct required date.'");
	...
	Если Результат <> Неопределено И Результат = КодВозвратаДиалога.ОК Тогда
		...
		ОбщегоНазначенияУТКлиент.ВвестиДатуСКонтролемПустогоЗначения(
			...
			НСтр("ru = 'Введите желаемую дату поступления';
				|en = 'Enter required date of receipt'"),
			ЧастиДаты.Дата,
			Оповещение);
	...
	Если ВыбраннаяДата <> Неопределено И ЗначениеЗаполнено(ВыбраннаяДата) Тогда
		...
		Если ВыбраннаяДата < НачалоДня(Дата) Тогда
			ТекстПредупреждения = НСтр("ru = 'Желаемая дата поступления меньше даты документа. Этапы оплаты не могут быть заполнены.';
										|en = 'Required date of receipt is less than the document date. Payment steps cannot be filled in.'");
...
Процедура НастроитьЭлементыФормыПоПараметрам()
	...
	ОбщегоНазначенияУТКлиентСервер.УстановитьСвойствоЭлементовФормы(
		Элементы, МассивЭлементов, "Видимость", ЕстьПредоплата И УпрощенныйРежим);
	
	Элементы.ЭтапыГрафикаОплатыВариантОплаты.Видимость = ЕстьПредоплата;
	...
	Если ЭтоЗаказ Тогда
		Элементы.ЭтапыГрафикаОплатыВариантОплаты.СписокВыбора.Добавить(Перечисления.ВариантыОплатыПоставщику.АвансДоПодтверждения);
		Элементы.ЭтапыГрафикаОплатыВариантОплаты.СписокВыбора.Добавить(Перечисления.ВариантыОплатыПоставщику.ПредоплатаДоПоступления);
	КонецЕсли;
	Элементы.ЭтапыГрафикаОплатыВариантОплаты.СписокВыбора.Добавить(Перечисления.ВариантыОплатыПоставщику.КредитПослеПоступления);
	Элементы.ЭтапыГрафикаОплатыВариантОплаты.СписокВыбора.Добавить(Перечисления.ВариантыОплатыПоставщику.КредитСдвиг);
...
Функция ПреобразоватьДанныеУпрощенногоРежимаВТаблицуЭтапов()
	...
	Если ЭтапСодержитДанные("Предоплата") И (ЭтоЗаказ ИЛИ НЕ ЕстьПредоплата) Тогда
...
Процедура ПреобразоватьТаблицуЭтаповВДанныеУпрощенногоРежима(ЭтапыОплаты)
	...
	Для Каждого ТекСтрока Из ЭтапыОплаты Цикл
		...
		Если ТекСтрока.ВариантОплаты = Перечисления.ВариантыОплатыПоставщику.КредитПослеПоступления 
			ИЛИ ТекСтрока.ВариантОплаты = Перечисления.ВариантыОплатыПоставщику.КредитСдвиг Тогда
