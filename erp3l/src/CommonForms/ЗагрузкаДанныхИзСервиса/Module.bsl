...
Функция ФайлДанныхСодержитПоставляемыеРасширения(СвойстваВременногоФайла)
	...
	СтрокаСообщения = НСтр("ru = 'Выбранный файл содержит данные добавленные расширениями.';
							|en = 'The selected file contains data added by extensions.'") 
		+ Символы.ПС + НСтр("ru = 'Для восстановления из данного архива установите следующие расширения:';
							|en = 'To restore from this archive, install the following extensions:'");
	...
	ОбщегоНазначения.СообщитьПользователю(НСтр("ru = 'Если все необходимые расширения уже установлены, тогда нажмите ''Продолжить''';
												|en = 'If all necessary extensions are already installed, click Continue'"));
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Загрузка данных';
				|en = 'Importing data.'", ОбщегоНазначения.КодОсновногоЯзыка()),
		...
		ШаблонИсключения = НСтр("ru = 'При загрузке данных произошла ошибка: %1.
                                 ...
                                 |
                                 |Detailed information for the technical support is written to the event log. If the error cause is unknown, it is recommended to contact the technical support providing the event log export file and file which you attempted to import data from for investigation.'");
...
Процедура УдалитьФайлыВПопытке(ИмяПапкиИлиФайла)
	...
	ИмяСобытияЖР = НСтр("ru = 'Удаление файла.Загрузка файла выгрузки';
						|en = 'Deleting file.Importing export file'", ОбщегоНазначения.КодОсновногоЯзыка());
