...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	|- заполняет код быстрого доступа, если он не заполнен
	|- добавляет новый счет учета для резервов под обесценение для номенклатуры переданной';
	|en = 'Updates ""Self-financing"" chart of accounts:
	|- fills in quick access code if needed
	|- adds a GL account for provisions for impairment of the transferred item'");
...
Процедура ЗарегистрироватьДанныеКОбработкеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	|
	|ВЫБРАТЬ ПЕРВЫЕ 1
	|		ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.РезервыПодСнижениеСтоимостиМЦ)
	|	ИЗ
	|		ПланСчетов.Хозрасчетный КАК СчетаУчета
	...
	|		И НЕ ПланСчетов.ЗапретитьИспользоватьВПроводках) КАК Счета
	|	ПО Счета.Ссылка ЕСТЬ NULL
	|");
...
Процедура ОбработатьДанныеДляПереходаНаНовуюВерсию(Параметры) Экспорт
	...
	Пока ОбъектыДляОбработки.Следующий() Цикл
		...
		Если ОбъектыДляОбработки.Ссылка = ПланыСчетов.Хозрасчетный.РезервыПодСнижениеСтоимостиМЦ Тогда
			
			СоздатьИЗаполнитьСчетУчетаРезервовПодОбесценениеНоменклатурыПереданной();
		...
		Попытка
			...
			Если Объект.ИсключитьСуммуБУИзПереоценкиПоПлануСчетов
				И НЕ Объект.ИсключитьСуммуУУИзПереоценкиПоПлануСчетов Тогда
...
Процедура СоздатьИЗаполнитьСчетУчетаРезервовПодОбесценениеНоменклатурыПереданной()

	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1
	|	МАКСИМУМ(СчетаУчета.Код) КАК Код
	|ИЗ
	|	ПланСчетов.Хозрасчетный КАК СчетаУчета
	|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	|			ПланСчетов.Ссылка КАК Ссылка,
	...
	|		ПО Счета.Ссылка ЕСТЬ NULL
	|ГДЕ
	|	СчетаУчета.Ссылка В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.РезервыПодСнижениеСтоимостиМЦ))";
	Результат = Запрос.Выполнить();
	Выборка = Результат.Выбрать();
	
	Если Выборка.Следующий() Тогда
		
		НовыйКодЧислом = Число(Лев(СтрЗаменить(Выборка.Код, "14.", ""), 2)) + 1;
		НовыйКод = "14." + ?(НовыйКодЧислом < 10, "0", "") + Строка(НовыйКодЧислом);
		
		НовыйСчет = ПланыСчетов.Хозрасчетный.СоздатьСчет();
		
		НовыйСчет.Код = НовыйКод;
		НовыйСчет.КодБыстрогоВыбора = СтрЗаменить(НовыйКод, ".", "");
		НовыйСчет.Наименование = НСтр("ru = 'Резервы под снижение стоимости номенклатуры переданной';
										|en = 'Transferred item cost reserve'", ОбщегоНазначения.КодОсновногоЯзыка());
		НовыйСчет.Родитель = ПланыСчетов.Хозрасчетный.РезервыПодСнижениеСтоимостиМЦ;
		
		СтрокаСубконто = НовыйСчет.ВидыСубконто.Добавить();
		СтрокаСубконто.ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Номенклатура;
		СтрокаСубконто.Суммовой = Истина;
		СтрокаСубконто = НовыйСчет.ВидыСубконто.Добавить();
		СтрокаСубконто.ВидСубконто = ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Контрагенты;
		СтрокаСубконто.Суммовой = Истина;
		
		НовыйСчет.Вид = ВидСчета.Пассивный;
		НовыйСчет.УчетПоНаправлениямДеятельности = Истина;
		НовыйСчет.УчетПоПодразделениям = Истина;
