...
Функция СформироватьГиперссылкиУправленияОтборами(ПолучатьКоличество, ПараметрыОтборов = Неопределено) Экспорт
	...
	ТекстГиперссылки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Отправить (%1)';
			|en = 'Send (%1)'"), ОбщаяСтатистика.ОжидаютОтправки);
	...
	ТекстГиперссылки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Ожидать получения РНПТ (%1)';
			|en = 'Await receiving a goods batch registration number (%1)'"), ОбщаяСтатистика.ОжидаютПолучения);
