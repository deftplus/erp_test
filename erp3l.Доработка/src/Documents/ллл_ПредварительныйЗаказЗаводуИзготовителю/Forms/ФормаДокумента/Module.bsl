...
#Область ллл_ОбработкаСобытийФормы_Измененные

&НаСервере
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)

	
	Если не рольДоступна("КазначействоЕХ") и не РольДоступна("ллл_ДоступностьИзмененияРеквизитовКазначейства") 	и не РольДоступна("ПолныеПрава") 
		или  Объект.ВерсияСозданаПослеУтвержденияКазначейством
		
		тогда
		ЭтаФОрма.Элементы.ГруппаРеквизитыКазначейства.ТолькоПросмотр=истина;
		
	иначе
		ЭтаФОрма.Элементы.ГруппаРеквизитыКазначейства.ТолькоПросмотр=ложь;
	КонецЕсли;
	...
	Если ЗначениеЗаполнено(Объект.ллл_КодРС) тогда
		
		НачатьТранзакцию(РежимУправленияБлокировкойданных.Управляемый);
		Блокировка=Новый БлокировкаДанных();
		ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.ллл_РеквизитыКазначействаДляПредварительныхЗаказов");
		ЭлементБлокировки.УстановитьЗначение("КодРС", Объект.ллл_КодРС);
		ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
		Блокировка.Заблокировать();
	
        менеджерЗаписиРегистраКазначейскихРеквизитов=РегистрыСведений.ллл_РеквизитыКазначействаДляПредварительныхЗаказов.СоздатьМенеджерЗаписи();
		менеджерЗаписиРегистраКазначейскихРеквизитов.КодРС=Объект.ллл_КодРС;
		менеджерЗаписиРегистраКазначейскихРеквизитов.Прочитать();
		если менеджерЗаписиРегистраКазначейскихРеквизитов.Выбран() тогда
			ЗаполнитьЗначенияСвойств(ЭтаФОрма,менеджерЗаписиРегистраКазначейскихРеквизитов);
			
			Если менеджерЗаписиРегистраКазначейскихРеквизитов.ОбработаноРуководителемКазначейства тогда
				ЭтаФОрма.Элементы.ГруппаРеквизитыКазначейства.ТолькоПросмотр=истина;	
			КонецЕсли;
			
			Если ЭтаФорма.СуммаАгентуПоДоставке+ЭтаФорма.СуммаАгентуПоОплате>0 тогда
				//ЭтаФОрма.ИспользоватьСложныйПроцент=ложь; 
				ЭтаФОрма.ТекстовоеПолеВыбораРежимаЗаполнения="Распределить суммы, перечисляемые агентам:";
			иначе	
				ЭтаФОрма.ИспользоватьСложныйПроцент=истина;  
				ЭтаФОрма.ТекстовоеПолеВыбораРежимаЗаполнения="Задавать процент наценки для агентов:";
			КонецЕсли;	
		иначе
			ЭтаФОрма.ИспользоватьПроцентСумму=истина;
			ЭтаФОрма.ИспользоватьСложныйПроцент=истина; 
			ЭтаФОрма.ТекстовоеПолеВыбораРежимаЗаполнения="Задавать процент наценки для агентов:";
		КонецЕсли; 
		
		Если ЭтаФорма.СуммаАгентуПоОплате>0 или ЭтаФОрма.ПроцентНаценкиАгентаПоОплате>0 тогда
			ЭтаФОрма.Элементы.КроссДоговор.Видимость=истина;
			
		КонецЕсли;	
		
		
		_Объект=РеквизитФормыВЗначение("Объект");	
		_Объект.РассчитатьСуммыНаСервере(ЭтаФОрма.ИспользоватьСложныйПроцент,ЭтаФОрма.ПроцентНаценкиАгентаПоДоставке,ЭтаФОрма.ПроцентНаценкиАгентаПоОплате,
		ЭтаФОрма.ИСпользоватьПроцентСумму,ЭтаФОрма.СуммаАгентуПоДоставке,ЭтаФОрма.СуммаАгентуПоОплате);
		...
		НачатьТранзакцию(РежимУправленияБлокировкойданных.Управляемый);
		Блокировка=Новый БлокировкаДанных();
		ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.ллл_РеквизитыОтделаЗакупокДляПредварительныхЗаказов");
		ЭлементБлокировки.УстановитьЗначение("КодРС", Объект.ллл_КодРС);
		ЭлементБлокировки.Режим = РежимБлокировкиДанных.Исключительный;
		Блокировка.Заблокировать();
		...
		менеджерЗаписиРегистраРеквизитовЗакупок=РегистрыСведений.ллл_РеквизитыОтделаЗакупокДляПредварительныхЗаказов.СоздатьМенеджерЗаписи();
		менеджерЗаписиРегистраРеквизитовЗакупок.КодРС=Объект.ллл_КодРС;
		менеджерЗаписиРегистраРеквизитовЗакупок.Прочитать();
		если менеджерЗаписиРегистраРеквизитовЗакупок.Выбран() тогда
			ЗаполнитьЗначенияСвойств(ЭтаФОрма,менеджерЗаписиРегистраРеквизитовЗакупок);	
		КонецЕсли;
	...
	Если ЗначениеЗаполнено(ЭтаФорма.АгентПоОплате) тогда
		Элементы.КроссДоговор.Видимость=истина;	 
	 иначе
	 	Элементы.КроссДоговор.Видимость=ложь; 
	 КонецЕсли;	 	
	
	
		
	ИспользоватьРучныеСкидкиВЗакупках           = ложь;
	ИспользоватьПричиныОтменыЗаказовПоставщикам = ложь;
	ИспользоватьСоглашенияСПоставщиками         = ложь;
	
	УстановитьВидимостьОпераций();
	
	
	ИспользоватьСтатусы = Истина;//ложь;
	...
	Элементы.ллл_ОбработаноРуководителемКазначейства.Доступность=РольДоступна("лллДоступностьУтвержденияКазначейскихРеквизитов") или РольДоступна("ПолныеПрава");
...
Процедура ПриОткрытии(Отказ) 
	ВидимостьРеквизитовЗависящихОтВыбораПроцентИлиСумма();
	ПодключаемыеКомандыКлиент.НачатьОбновлениеКоманд(ЭтотОбъект);
	Элементы.Товары.Обновить();
	РассчитатьИтоговыеСуммы();
КонецПроцедуры

//+++ERP1C-189 Стрепков 18.07.2022
&НаСервере
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	нз=РегистрыСведений.ллл_РеквизитыОтделаЗакупокДляПредварительныхЗаказов.СоздатьНаборЗаписей();
	нз.отбор.КодРС.Установить(Объект.ллл_КодРС);
	нз.Прочитать();
	
	Стараязапись=неопределено;
	РеквизитыНеИзменились=ложь;
	ОтправлятьОповещениеЛогистам=ложь;
	
	Если нз.Количество()>0 тогда
		СтараяЗапись=нз[0];
		РеквизитыНеИзменились=(СтараяЗапись.ЗаводИзготовительПолучилДенежныеСредства=ЭтаФорма.ЗаводИзготовительПолучилДенежныеСредства) и 
		(СтараяЗапись.ДатаПолученияДенежныхСредствЗаводомИзготовителем=ЭтаФорма.ДатаПолученияДенежныхСредствЗаводомИзготовителем) и
		(СтараяЗапись.СуммаПолученнаяЗаводомИзготовителем=ЭтаФорма.СуммаПолученнаяЗаводомИзготовителем) и 
		(СтараяЗапись.ВалютаОплатыЗаводуИзготовителю=ЭтаФорма.ВалютаОплатыЗаводуИзготовителю) и
		(СтараяЗапись.ГотовоКОтгрузкеНаЗаводеИзготовителе=ЭтаФорма.ГотовоКОтгрузкеНаЗаводеИзготовителе) и
		(СтараяЗапись.ПредполагаемаяДатаГотовностиКОтгрузкеНаЗаводеИзготовителе=ЭтаФорма.ПредполагаемаяДатаГотовностиКОтгрузкеНаЗаводеИзготовителе);
		
		ОтправлятьОповещениеЛогистам= (не (СтараяЗапись.ЗаводИзготовительПолучилДенежныеСредства=ЭтаФорма.ЗаводИзготовительПолучилДенежныеСредства)) или 
		(не (СтараяЗапись.ГотовоКОтгрузкеНаЗаводеИзготовителе=ЭтаФорма.ГотовоКОтгрузкеНаЗаводеИзготовителе));
		
	иначе
		Если ЭтаФорма.ГотовоКОтгрузкеНаЗаводеИзготовителе или ЭтаФорма.ЗаводИзготовительПолучилДенежныеСредства тогда
			ОтправлятьОповещениеЛогистам=истина;	
			
		КонецЕсли;
		
	КонецЕсли;
	
	Если СтараяЗапись=неопределено или РеквизитыНеИзменились=ложь тогда
		нз.Очистить();  
		
		Если ЗначениеЗаполнено(ЭтаФорма.ЗаводИзготовительПолучилДенежныеСредства) или 
			значениеЗаполнено(ЭтаФорма.ДатаПолученияДенежныхСредствЗаводомИзготовителем) или
			значениеЗаполнено(ЭтаФорма.СуммаПолученнаяЗаводомИзготовителем) или
			значениеЗаполнено(ЭтаФорма.ВалютаОплатыЗаводуИзготовителю) или 
			значениеЗаполнено(ЭтаФорма.ГотовоКОтгрузкеНаЗаводеИзготовителе) или
			значениеЗаполнено(ЭтаФОрма.ПредполагаемаяДатаГотовностиКОтгрузкеНаЗаводеИзготовителе) тогда
			
			з=нз.Добавить();
			з.КодРС=Объект.ллл_КодРС;
			з.Пользователь=ПараметрыСеанса.ТекущийПользователь;
			з.ДатаИзменения=ТекущаяДата();
			з.ОбрабатывалсяНомерВерсии=Объект.ллл_НомерВерсии;
			з.ЗаводИзготовительПолучилДенежныеСредства=ЭтаФорма.ЗаводИзготовительПолучилДенежныеСредства;
			з.ДатаПолученияДенежныхСредствЗаводомИзготовителем=ЭтаФорма.ДатаПолученияДенежныхСредствЗаводомИзготовителем;
			з.СуммаПолученнаяЗаводомИзготовителем=ЭтаФорма.СуммаПолученнаяЗаводомИзготовителем;
			з.ВалютаОплатыЗаводуИзготовителю=ЭтаФорма.ВалютаОплатыЗаводуИзготовителю; 
			з.ГотовоКОтгрузкеНаЗаводеИзготовителе=ЭтаФорма.ГотовоКОтгрузкеНаЗаводеИзготовителе;
			з.ПредполагаемаяДатаГотовностиКОтгрузкеНаЗаводеИзготовителе=ЭтаФОрма.ПредполагаемаяДатаГотовностиКОтгрузкеНаЗаводеИзготовителе;
		КонецЕсли;	
	...
	Если ЭтаФорма.Элементы.ГруппаРеквизитыКазначейства.ТолькоПросмотр=истина тогда возврат; конецЕсли;
		...
		з=неопределено;
		нз=неопределено;
		нз=РегистрыСведений.ллл_РеквизитыКазначействаДляПредварительныхЗаказов.СоздатьНаборЗаписей();
		нз.Отбор.КодРС.Установить(Объект.ллл_КодРС);
		нз.Прочитать();  
		СоздаватьДвиженияПоКазначейству=истина;
		
		Если нз.Количество()>0 тогда
			Если нз[0].ОбработаноРуководителемКазначейства тогда
					СоздаватьДвиженияПоКазначейству=ложь;
			КонецЕсли;
		КонецЕсли;		
		
		
		Если СоздаватьДвиженияПоКазначейству тогда
		нз.Очистить();
		нз.Записывать=истина;
		з=нз.Добавить();
		з.КодРС=Объект.ллл_КодРС;
		з.АгентПоДоставке=ЭтаФорма.АгентПоДоставке;
		з.АгентПоОплате=ЭтаФорма.АгентПоОплате;
		з.ДоговорАгентаПоОплате=ЭтаФорма.ДоговорАгентаПоОплате;
		з.ДоговорАгентаПоДоставке=ЭтаФОрма.ДоговорАгентаПоДоставке;
		з.ОбработаноРуководителемКазначейства=ЭтаФОрма.ОбработаноРуководителемКазначейства;
		з.ИспользоватьСложныйПроцент=ЭтаФОрма.ИспользоватьСложныйПроцент;
		з.ПроцентНаценкиАгентаПоДоставке=ЭтаФОрма.ПроцентНаценкиАгентаПоДоставке;
		з.ПроцентНаценкиАгентаПоОплате=ЭтаФорма.ПроцентНаценкиАгентаПоОплате;  
		з.СуммаАгентуПоДоставке=ЭтаФорма.СуммаАгентуПоДоставке;
		з.СуммаАгентуПоОплате=ЭтаФОрма.СуммаАгентуПоОплате;
		з.КроссДоговор=ЭтаФорма.КроссДоговор;
		з.Пользователь=ПараметрыСеанса.ТекущийПользователь;
		з.ДатаИзменения=ТекущаяДата();
		з.ОбрабатывалсяНомерВерсии=Объект.ллл_НомерВерсии;
		нз.Записать();    
		...
		Если ЭтаФОрма.ОбработаноРуководителемКазначейства и не Объект.ВерсияСозданаПослеУтвержденияКазначейством и 
			ЭтаФорма.Элементы.ГруппаРеквизитыКазначейства.ТолькоПросмотр=ложь тогда
			
			ЭтаФорма.Элементы.ГруппаРеквизитыКазначейства.ТолькоПросмотр=истина;
			
			//ллл_МодульКазначействоОбщегоНазначения.ллл_СоздатьКроссЗаявкуАгентуПоДоставкеОтАгентаПоОплате(Объект.Ссылка);
			
			//ллл_МодульКазначействоОбщегоНазначения.ЗаполнитьНаОснованииПредЗаказа(Объект.Ссылка);
			
			Если Документы.ллл_ПредварительныйЗаказЗаводуИзготовителю.ЕстьЗаказПоставщикуПоСовокупностиПредзаказов(Объект.Ссылка)=ложь тогда
				МассивПараметров=Новый Массив();
				Массивпараметров.Добавить(Объект.Ссылка);
				ФоновыеЗадания.Выполнить("ллл_МодульКазначействоОбщегоНазначения.ЗаполнитьНаОснованииПредЗаказа",МассивПараметров,строка(Новый УникальныйИдентификатор),
				" создание заказа поставщику к предзаказу с кодом РС " + Объект.ллл_КодРС);
				
				МассивПараметровДляКроссЗаявки=Новый Массив();
				МассивПараметровДляКроссЗаявки.Добавить(Объект.Ссылка);
				ФоновыеЗадания.Выполнить("ллл_МодульКазначействоОбщегоНазначения.ллл_СоздатьКроссЗаявкуАгентуПоДоставкеОтАгентаПоОплате",
				МассивПараметровДляКроссЗаявки,строка(Новый УникальныйИдентификатор),
				" создание кросс-заявки к предзаказу с кодом РС " + Объект.ллл_КодРС);
				
				
			иначе
					  С=Новый СообщениеПользователю();
					  С.Текст="Уже существует заказ с кодом РС " + строка(Объект.ллл_КодРС);
					  С.Сообщить();
		...
		Если  ЭтаФорма.Элементы.ГруппаРеквизитыКазначейства.ТолькоПросмотр=ложь и ЭтаФорма.ОбработаноРуководителемКазначейства тогда
			ЭтаФорма.Элементы.ГруппаРеквизитыКазначейства.ТолькоПросмотр=истина;	
		КонецЕсли;

		Если ОтправлятьОповещениеЛогистам тогда
			ллл_МодульКазначействоОповещения.ОтправитьОповещениеЛогистамОГалочкахЗаводаИзготовителя(Объект.Ссылка,
			ЭтаФорма.ГотовоКОтгрузкеНаЗаводеИзготовителе,ЭтаФОрма.ЗаводИзготовительПолучилДенежныеСредства);
...
Процедура ТоварыНоменклатураПриИзменении(Элемент)
	...
	ТекущаяСтрока.ВесНоменклатурыВСтроке = ЗаполнитьВесНоменклатурыВСтроке(ТекущаяСтрока.Номенклатура) * ТекущаяСтрока.КоличествоУпаковок;
...
Процедура ЗаполнитьДатуПоступленияДляВыделенныхСтрок(Команда)
	...
	Иначе
		ТекстПредупреждения = НСтр("ru = 'В документе не выбраны строки для заполнения. Дата поступления не будет заполнена.';
									|en = 'Cannot fill in the receipt date. Add at least one item.'");
...
Процедура УстановитьУсловноеОформлениеДляСтатейИАналитик()
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = '<для товаров/услуг и работ без назначения>';
																|en = '<для товаров/услуг и работ без назначения>'"));
...
Процедура РассчитатьИтоговыеПоказателиЗаказа(Форма)
	...
	Форма.СуммаЗаказаноСзалоговойТарой = (КоллекцияТовары.Итог("СуммаСНДС") - КоллекцияТовары.Итог("СуммаСНДСОтменено"));	
	...
	Форма.СуммаЗаказано = Форма.СуммаЗаказаноСзалоговойТарой;
	...
	Форма.ПримерныйВесТовара = КоллекцияТовары.Итог("ВесНоменклатурыВСтроке");	
...
Процедура ЗаменитьНаАналоги(АдресВХранилище = Неопределено)
	...
	Результат = АналогиМатериалов.ВыполнитьЗаменуНаАналоги(
		Объект[ИмяТаблицы], АдресВХранилище, ДополнительныеПоля, Истина);
...
Процедура УХ_ЗаполнитьПлановымиПотребностямиНаСервере()
	...
	УправлениеЗакупкамиВстраиваниеПереопределяемыйУХ.ЗаполнитьНоменклатуруЗаказаПоставщикуПоДоговору(
		ДокументОбъект);
...
Процедура УХ_РаспределитьТоварнуюКатегорию(Команда)
	Если Элементы.Товары.ТекущиеДанные <> Неопределено 
		...
		И ТипЗнч(Элементы.Товары.ТекущиеДанные.НоменклатураИсходная) =  Тип("СправочникСсылка.ТоварныеКатегории")
		Тогда
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Распределение доступно только по товарной категории без указанной номенклатуры'"));
	КонецЕсли;
КонецПроцедуры
...
Процедура Подключаемый_ВыполнитьИнтерактивныйКонтроль() Экспорт
	 //КонтрольУХКлиент.ВыполнитьИнтерактивныйКонтроль(ЭтотОбъект);
КонецПроцедуры
...
#Область ллл_Функционал

&НаСервереБезКонтекста
Функция НайтиВерсиюНаСервере(какуюВерсиюИскать,КодРС)
	// какуюВерсиюИскать: 0 - самую старую, 1 - самую свежую
	
	Запрос=Новый Запрос();
	Запрос.Текст="ВЫБРАТЬ
	             |	ллл_ПредварительныйЗаказЗаводуИзготовителю.ллл_КодРС КАК ллл_КодРС,
	             |	МАКСИМУМ(ллл_ПредварительныйЗаказЗаводуИзготовителю.ллл_НомерВерсии) КАК МаксимальныйНомерВерсии,
	             |	МИНИМУМ(ллл_ПредварительныйЗаказЗаводуИзготовителю.ллл_НомерВерсии) КАК МинимальныйНомерВерсии
	             |ПОМЕСТИТЬ ВТ_1
	             |ИЗ
	             ...
	             |////////////////////////////////////////////////////////////////////////////////
	             |ВЫБРАТЬ
	             |	з.ллл_КодРС КАК ллл_КодРС,
	             |	з.МаксимальныйНомерВерсии КАК МаксимальныйНомерВерсии,
	             |	з.МинимальныйНомерВерсии КАК МинимальныйНомерВерсии,
	             |	з.Ссылка КАК СсылкаМаксимальнойВерсии,
	             |	ллл_ПредварительныйЗаказЗаводуИзготовителю.Ссылка КАК СсылкаМинимальнойВерсии
	             |ИЗ
	             |	(ВЫБРАТЬ
	             |		ВТ_1.ллл_КодРС КАК ллл_КодРС,
	             |		ВТ_1.МаксимальныйНомерВерсии КАК МаксимальныйНомерВерсии,
	             |		ВТ_1.МинимальныйНомерВерсии КАК МинимальныйНомерВерсии,
	             |		ллл_ПредварительныйЗаказЗаводуИзготовителю.Ссылка КАК Ссылка
	             |	ИЗ
	             |		ВТ_1 КАК ВТ_1
	             |			ЛЕВОЕ СОЕДИНЕНИЕ Документ.ллл_ПредварительныйЗаказЗаводуИзготовителю КАК ллл_ПредварительныйЗаказЗаводуИзготовителю
	             |			ПО ВТ_1.ллл_КодРС = ллл_ПредварительныйЗаказЗаводуИзготовителю.ллл_КодРС
	             |				И ВТ_1.МаксимальныйНомерВерсии = ллл_ПредварительныйЗаказЗаводуИзготовителю.ллл_НомерВерсии) КАК з
	             |		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ллл_ПредварительныйЗаказЗаводуИзготовителю КАК ллл_ПредварительныйЗаказЗаводуИзготовителю
	             |		ПО з.ллл_КодРС = ллл_ПредварительныйЗаказЗаводуИзготовителю.ллл_КодРС
	             |			И з.МинимальныйНомерВерсии = ллл_ПредварительныйЗаказЗаводуИзготовителю.ллл_НомерВерсии";
	Запрос.УстановитьПараметр("КодРС",КодРС);
	Результат=Запрос.Выполнить();    
	Выборка=Результат.Выбрать();
	если Выборка.Следующий() тогда
		Если КакуюВерсиюИскать=0 и значениеЗаполнено(Выборка.СсылкаМинимальнойВерсии) тогда
			возврат(Выборка.СсылкаМинимальнойВерсии);	
			
		КонецЕсли;	
		
		Если КакуюВерсиюИскать=1 и значениеЗаполнено(Выборка.СсылкаМаксимальнойВерсии) тогда
			возврат(Выборка.СсылкаМаксимальнойВерсии);	
...
&НаКлиенте
Процедура НаПервуюВерсию(Команда)
	ПопытаемсяОткрытьСсылку=НайтиВерсиюНаСервере(0,Объект.ллл_КодРС);
	Если ЗначениеЗаполнено(ПопытаемсяОткрытьСсылку) и ПопытаемсяОткрытьСсылку<>Объект.Ссылка тогда
		ЭтаФорма.Закрыть();
		ОткрытьФорму("Документ.ллл_ПредварительныйЗаказЗаводуИзготовителю.ФормаОбъекта",Новый Структура("Ключ",ПопытаемсяОткрытьСсылку));
		
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура НаПоследнююВерсию(Команда)
		ПопытаемсяОткрытьСсылку=НайтиВерсиюНаСервере(1,Объект.ллл_КодРС);
	Если ЗначениеЗаполнено(ПопытаемсяОткрытьСсылку) и ПопытаемсяОткрытьСсылку<>Объект.Ссылка тогда
		ЭтаФорма.Закрыть();
		ОткрытьФорму("Документ.ллл_ПредварительныйЗаказЗаводуИзготовителю.ФормаОбъекта",Новый Структура("Ключ",ПопытаемсяОткрытьСсылку));
...
&НаСервере
Процедура ллл_ОбработаноРуководителемКазначействаПриИзмененииНаСервере()
	Если ОбработаноРуководителемКазначейства=ложь тогда
		//ЭтаФорма.АгентПоДоставке=Справочники.Контрагенты.ПустаяСсылка();
		//ЭтаФОрма.ДоговорАгентаПоДоставке=Справочники.ДоговорыКонтрагентов.ПустаяСсылка();
		//ЭтаФорма.ПроцентНаценкиАгентаПоДоставке=0;
		//
		//ЭтаФорма.АгентПоОплате=Справочники.Контрагенты.ПустаяСсылка();
		//ЭтаФОрма.ДоговорАгентаПоОплате=Справочники.ДоговорыКонтрагентов.ПустаяСсылка();
		//ЭтаФорма.ПроцентНаценкиАгентаПоОплате=0;
	КонецЕсли;
...
&НаСервере 
Процедура РассчитатьСуммыНаСервере(_ИспользоватьСложныйПроцент,_ПроцентНаценкиАгентаПоДоставке,_ПроцентНаценкиАгентаПоОплате,_ИСпользоватьПроцентСумму,_СуммаАгентуПоДоставке,_СуммаАгентуПоОплате)
	_Объект=РеквизитФормыВЗначение("Объект");
	_Объект.РассчитатьСуммыНаСервере(_ИспользоватьСложныйПроцент,_ПроцентНаценкиАгентаПоДоставке,_ПроцентНаценкиАгентаПоОплате,_ИСпользоватьПроцентСумму,_СуммаАгентуПоДоставке,_СуммаАгентуПоОплате);
	ЗначениеВРеквизитФормы(_Объект,"Объект");
...
Процедура РассчитатьИтоговыеСуммы()
	...
	//_СуммаДоставка=0;
	//_Суммаоплата=0;
	КоллекцияТовары = ЭтаФОрма.Объект.Товары;
	для каждого стр из КоллекцияТовары цикл
	//	_Сумма=_Сумма+стр.Сумма;
	//	_СуммаДоставка=_СуммаДоставка+стр.СуммаАгентуПоДоставке;
	//	_СуммаОплата=_СуммаОплата+стр.СуммаАгентуПоОплате;
		стр.ВесНоменклатурыВСтроке = ЗаполнитьВесНоменклатурыВСтроке(стр.Номенклатура) * стр.КоличествоУпаковок;
	КонецЦикла;
	//
	//ЭтаФОрма.СуммаЗаказано=_Сумма;
	//ЭтаФОрма.СуммаАгентуПоДоставке=_СуммаДоставка;
	//ЭтаФОрма.СуммаАгентуПоОплате=_СуммаОплата;

	ЭтаФОрма.СуммаЗаказаноСзалоговойТарой = (КоллекцияТовары.Итог("СуммаСНДС") - КоллекцияТовары.Итог("СуммаСНДСОтменено"));	
	ЭтаФОрма.СуммаЗаказано = ЭтаФОрма.СуммаЗаказаноСзалоговойТарой;
	
	ЭтаФОрма.СуммаЗаказано = КоллекцияТовары.Итог("Сумма");
	ЭтаФОрма.СуммаАгентуПоДоставке = КоллекцияТовары.Итог("СуммаАгентуПоДоставке");
	ЭтаФОрма.СуммаАгентуПоОплате = КоллекцияТовары.Итог("СуммаАгентуПоОплате");
	
	ЭтаФОрма.ПримерныйВесТовара = КоллекцияТовары.Итог("ВесНоменклатурыВСтроке");	
	
КонецПроцедуры

&НаКлиенте
Процедура ллл_ОбработаноРуководителемКазначействаПриИзменении(Элемент)   

	ПроверкаЗаполненностиКазначейскихреквизитов();	
	
	
	
	
	РассчитатьСуммыНаСервере(ЭтаФорма.ИспользоватьСложныйПроцент,ЭтаФОрма.ПроцентНаценкиАгентаПоДоставке,
	ЭтаФорма.ПроцентНаценкиАгентаПоОплате,ЭтаФорма.ИспользоватьПроцентСумму,ЭтаФОрма.СуммаАгентуПоДоставке,ЭтаФорма.СуммаАгентуПоОплате);
...
&НаКлиенте
Процедура ИспользоватьСложныйПроцентПриИзменении(Элемент)
	РассчитатьСуммыНаСервере(ЭтаФорма.ИспользоватьСложныйПроцент,ЭтаФОрма.ПроцентНаценкиАгентаПоДоставке,
	ЭтаФорма.ПроцентНаценкиАгентаПоОплате,ЭтаФорма.ИспользоватьПроцентСумму,ЭтаФОрма.СуммаАгентуПоДоставке,ЭтаФорма.СуммаАгентуПоОплате);
...
&НаКлиенте
Процедура ПроцентНаценкиАгентаПоОплатеПриИзменении(Элемент) 
	//ПроверкаЗаполненностиКазначейскихреквизитов();
	РассчитатьСуммыНаСервере(ЭтаФорма.ИспользоватьСложныйПроцент,ЭтаФОрма.ПроцентНаценкиАгентаПоДоставке,
	ЭтаФорма.ПроцентНаценкиАгентаПоОплате,ЭтаФорма.ИспользоватьПроцентСумму,ЭтаФОрма.СуммаАгентуПоДоставке,ЭтаФорма.СуммаАгентуПоОплате);
...
&НаКлиенте
Процедура ПроцентНаценкиАгентаПоДоставкеПриИзменении(Элемент) 
//	ПроверкаЗаполненностиКазначейскихреквизитов();
...
&НаСервереБезКонтекста
Функция СуществуетОрганизацияСИННИКПП(ИНН,КПП)
...
&НаКлиенте
Процедура АгентПоОплатеПриИзменении(Элемент)
	ИНН=ВернутьИНН(АгентПоОплате);
	КПП=ВернутьКПП(АгентПоОплате);
	Если не ЗначениеЗаполнено(ИНН) или не значениеЗаполнено(КПП) тогда
		С=Новый СообщениеПользователю();
		С.Поле="АгентПоОплате";
		С.Текст="Для дальнейшей обработки предварительного заказа необходимо, чтобы у агента по оплате был заполнен ИНН и КПП";
		С.Сообщить();
		АгентПоОплате=неопределено;
		возврат;
	КонецЕсли;
	
	Если не СуществуетОрганизацияСИННИКПП(ИНН,КПП) тогда
		С=Новый СообщениеПользователю();
		С.Поле="АгентПоОплате";
		С.Текст="Для дальнейшей обработки предварительного заказа необходимо, чтобы у агента по оплате был эквивалент в справочнике ""Организации"" с такими же значениями 
		| ИНН и КПП";
		С.Сообщить();
...
&НаСервереБезКонтекста
Функция ВернутьИНН(Контрагент)
	
	возврат(Контрагент.ИНН);
КонецФункции

&НаСервереБезКонтекста
Функция ВернутьКПП(Контрагент)
	
	возврат(Контрагент.КПП);
КонецФункции

&НаКлиенте
Процедура АгентПоДоставкеПриИзменении(Элемент)
	ПроверкаЗаполненностиКазначейскихреквизитов();
КонецПроцедуры

&НаКлиенте
Процедура ДоговорАгентаПоДоставкеПриИзменении(Элемент)
	ПроверкаЗаполненностиКазначейскихреквизитов();
КонецПроцедуры

&НаКлиенте
Процедура ДоговорАгентаПоОплатеПриИзменении(Элемент)
	ПроверкаЗаполненностиКазначейскихреквизитов();
КонецПроцедуры

&НаКлиенте
Процедура ПослеЗаписи(ПараметрыЗаписи)
	Если ЭтаФорма.ОбработаноРуководителемКазначейства тогда
		ЭтаФорма.Элементы.ГруппаРеквизитыКазначейства.ТолькоПросмотр=истина;
	КонецЕсли;   
	
	Если ПараметрыЗаписи.ПринудительноЗакрытьФорму тогда
		ЭтаФОрма.Закрыть();
...
Функция ПолучитьОрганизациюПоКонтрагенту(Контрагент)
	...
	возврат(ллл_МодульКазначействоОбщегоНазначения.НайтиОрганизациюПоИННиКПП(ВернутьИНН(Контрагент),ВернутьКПП(Контрагент)));
	
КонецФункции

&НаСервереБезКонтекста
Функция ТипДоговораИмпорт()
	возврат(Перечисления.типыдоговоров.Импорт);
КонецФункции

&НаКлиенте
Процедура КроссДоговорНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	СтандартнаяОбработка=ложь;
	СтруктураОтбора=Новый Структура();
	СтруктураОтбора.Вставить("Организация",ПолучитьОрганизациюПоКонтрагенту(ЭтаФорма.АгентПоОплате));  
	Если ЗначениеЗаполнено(ЭтаФорма.АгентПоДоставке) тогда
	СтруктураОтбора.Вставить("Контрагент",ЭтаФорма.АгентПоДоставке);
	иначе
	СтруктураОтбора.Вставить("Контрагент",Объект.Контрагент);
	КонецЕсли;
	СтруктураОтбора.Вставить("ВалютаВзаиморасчетов",Объект.Валюта);
	СтруктураОтбора.Вставить("ТипДоговора",ТипДоговораИмпорт());
    ...
    ОписаниеОбработкаВыбора = Новый ОписаниеОповещения("ПриЗакрытииФормыВыбора", ЭтаФорма,"Подбор");
	
	
	ОткрытьФорму("Справочник.ДоговорыКонтрагентов.ФормаВыбора",ПараметрыФормы,
	        ЭтаФорма, , , , ОписаниеОбработкаВыбора);
КонецПроцедуры
...
Процедура ПриЗакрытииФормыВыбора(Значение, ДопПараметры) Экспорт
    ...
    ЭтаФорма.КроссДоговор = Значение;
	
	
КонецПроцедуры

&НаКлиенте
Процедура ИспользоватьПроцентСуммуПриИзменении(Элемент)
                                                                                    
	ВидимостьРеквизитовЗависящихОтВыбораПроцентИлиСумма();
	ИспользоватьПроцентСуммуПриИзмененииНаСервере();
 	Элементы.Товары.Обновить();
	РассчитатьИтоговыеСуммы();
КонецПроцедуры

&НаСервере
Процедура ИспользоватьПроцентСуммуПриИзмененииНаСервере()

	Если ЭтаФорма.ИспользоватьПроцентСумму=истина тогда
		ЭтаФорма.СуммаАгентуПоДоставке=0;
		ЭтаФорма.СуммаАгентуПоОплате=0;
	иначе
		ЭтаФорма.ПроцентНаценкиАгентаПоДоставке=0;
		ЭтаФОрма.ПроцентНаценкиАгентаПоОплате=0;
	КонецЕсли;	
...
Процедура ПроверкаЗаполненностиКазначейскихреквизитов()
	КоличествоЗаполненныхКазначейскихРеквизитовДоставка=0;
	КоличествоЗаполненныхКазначейскихреквизитовОплата=0;
	
	Если ЗначениеЗаполнено(ЭтаФорма.ДоговорАгентаПоОплате) тогда
		КоличествоЗаполненныхКазначейскихреквизитовОплата=КоличествоЗаполненныхКазначейскихреквизитовОплата+1;	
	КонецЕсли;	
	...
	Если ЗначениеЗаполнено(ЭтаФОрма.ПроцентНаценкиАгентаПоОплате) или значениеЗаполнено(ЭтаФОрма.СуммаАгентуПоОплате) тогда
		КоличествоЗаполненныхКазначейскихреквизитовОплата=КоличествоЗаполненныхКазначейскихреквизитовОплата+1;	
	КонецЕсли;	
	
	Если ЗначениеЗаполнено(ЭтаФОрма.КроссДоговор) и значениезаполнено(ЭтаФорма.АгентПоДоставке) тогда
		КоличествоЗаполненныхКазначейскихреквизитовОплата=КоличествоЗаполненныхКазначейскихреквизитовОплата+1;	
	КонецЕсли;
	
	
	
	Если (КоличествоЗаполненныхКазначейскихреквизитовОплата=1 или КоличествоЗаполненныхКазначейскихреквизитовОплата=2
		или (КоличествоЗаполненныхКазначейскихреквизитовОплата=3 и Значениезаполнено(ЭтаФорма.АгентПоДоставке))) 
		и ЭтаФорма.ОбработаноРуководителемКазначейства=истина тогда
		 ЭтаФорма.ОбработаноРуководителемКазначейства=ложь;
		 Сообщить("Заполните либо очистите все казначейские реквизиты, связанные с агентом по оплате.");
		
	КонецЕсли;
	
	
	Если ЗначениеЗаполнено(ЭтаФорма.ДоговорАгентаПоДоставке) тогда
		КоличествоЗаполненныхКазначейскихРеквизитовДоставка=КоличествоЗаполненныхКазначейскихРеквизитовДоставка+1;	
	КонецЕсли;	
	...
	Если ЗначениеЗаполнено(ЭтаФОрма.ПроцентНаценкиАгентаПоДоставке) или ЗначениеЗаполнено(ЭтаФОрма.СуммаАгентуПоДоставке) тогда
		КоличествоЗаполненныхКазначейскихРеквизитовДоставка=КоличествоЗаполненныхКазначейскихРеквизитовДоставка+1;	
	КонецЕсли;	                                          
	
	Если (КоличествоЗаполненныхКазначейскихРеквизитовДоставка=1 или КоличествоЗаполненныхКазначейскихРеквизитовДоставка=2) и ЭтаФорма.ОбработаноРуководителемКазначейства=истина тогда
		 //Отказ=истина;   
		 ЭтаФорма.ОбработаноРуководителемКазначейства=ложь;
		 Сообщить("Заполните либо очистите все казначейские реквизиты, связанные с агентом по доставке.");
	 КонецЕсли; 
	 
	 Если //ЗначениеЗаполнено(ЭтаФорма.АгентПоДоставке) или 
		 ЗначениеЗаполнено(ЭтаФорма.АгентПоОплате) тогда
		Элементы.КроссДоговор.Видимость=истина;	 
	 иначе
	 	Элементы.КроссДоговор.Видимость=ложь; 
	 КонецЕсли;	 
...
&НаКлиенте
Процедура ВидимостьРеквизитовЗависящихОтВыбораПроцентИлиСумма()
	
	Если ЭтаФОрма.ИспользоватьПроцентСумму=истина тогда
		//ЭтаФорма.Элементы.ДекорацияПроцентСумма.Заголовок="Задавать процент наценки для агентов:";	
		ЭтаФорма.Элементы.ПроцентНаценкиАгентаПоДоставке.Видимость=истина;
		ЭтаФорма.Элементы.ПроцентНаценкиАгентаПоОплате.Видимость=истина;  
		ЭтаФорма.Элементы.СуммаАгентуПоДоставке1.Видимость=ложь;
		ЭтаФорма.Элементы.СуммаАгентуПоОплате1.Видимость=ложь;
		ЭтаФорма.Элементы.ИспользоватьСложныйПроцент.Видимость=истина;
	иначе
		//ЭтаФорма.Элементы.ДекорацияПроцентСумма.Заголовок="Распределить суммы, перечисляемые агентам:";	
		ЭтаФорма.Элементы.ПроцентНаценкиАгентаПоДоставке.Видимость=ложь;
		ЭтаФорма.Элементы.ПроцентНаценкиАгентаПоОплате.Видимость=ложь;
		ЭтаФорма.Элементы.СуммаАгентуПоДоставке1.Видимость=истина;
		ЭтаФорма.Элементы.СуммаАгентуПоОплате1.Видимость=истина; 
		ЭтаФорма.Элементы.ИспользоватьСложныйПроцент.Видимость=ложь;

	КонецЕсли; 
	
	ЭтаФорма.Элементы.ДекорацияСостояниеРасчетов.Видимость = Ложь;	
	
КонецПроцедуры

 &НаСервереБезКонтекста
Функция ллл_ОтправитьОповещениеАвторуИлиМенеджеруОСменеРеквизитовКазначейства(Заказ,Автор,Менеджер,ТекстОшибки="")
	КомуОтправлять=неопределено;
	Если ЗначениеЗаполнено(Автор) и не Автор.наименование="<Не указан>" тогда
		КомуОтправлять=Автор;			
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Менеджер) и не ЗначениеЗаполнено(КомуОтправлять) и не Менеджер.наименование="<Не указан>" тогда
		КомуОтправлять=Менеджер;			
	КонецЕсли;  
	ТемаСообщения="Изменились казначейские реквизиты в предзаказе "+ Заказ.номер +" от " + 
			Формат(Заказ.Дата,"ДЛФ=Д")+". ";
	ТекстСообщения=ТемаСообщения+ "Создание заявки на оплату на данном этапе не требуется.";
	
	Если ЗначениеЗаполнено(КомуОтправлять) тогда 
		Успех=СобытияоповещенийСерверУх.СоздатьЗаписьСобытия(перечисления.СпособыОповещения.ЭлектроннаяПочта, КомуОтправлять, Справочники.ВидыСобытийОповещений.ПредписаниеИсполнителю, 
		Заказ, , ТекущаяДата(), , , ,,  ТемаСообщения, ,
		, ТекстСообщения, ,Неопределено);
		
		Если Успех тогда
			СобытияоповещенийСерверУх.СоздатьЗаписьСобытия(перечисления.СпособыОповещения.ВСистеме, КомуОтправлять, Справочники.ВидыСобытийОповещений.ПредписаниеИсполнителю, 
		Заказ, , ТекущаяДата(), , , ,,  ТемаСообщения, ,
		, ТекстСообщения, ,Неопределено);	
		КонецЕсли;
		...
		возврат(Успех); 
	иначе
		ТекстОшибки="Смена реквизитов казначейства записана, создание заявки на оплату не требуется, но не определен получатель оповещения об этом событии.";
		возврат(ложь);
...
&НаКлиенте
Процедура СуммаАгентуПоОплате1ПриИзменении(Элемент)
	РассчитатьСуммыНаСервере(ЭтаФорма.ИспользоватьСложныйПроцент,ЭтаФОрма.ПроцентНаценкиАгентаПоДоставке,
	ЭтаФорма.ПроцентНаценкиАгентаПоОплате,ЭтаФорма.ИспользоватьПроцентСумму,ЭтаФОрма.СуммаАгентуПоДоставке,ЭтаФорма.СуммаАгентуПоОплате);
	Элементы.Товары.Обновить();
	РассчитатьИтоговыеСуммы();
...
&НаКлиенте
Процедура СуммаАгентуПоДоставке1ПриИзменении(Элемент)
	РассчитатьСуммыНаСервере(ЭтаФорма.ИспользоватьСложныйПроцент,ЭтаФОрма.ПроцентНаценкиАгентаПоДоставке,
	ЭтаФорма.ПроцентНаценкиАгентаПоОплате,ЭтаФорма.ИспользоватьПроцентСумму,ЭтаФОрма.СуммаАгентуПоДоставке,ЭтаФорма.СуммаАгентуПоОплате);
	Элементы.Товары.Обновить();
	РассчитатьИтоговыеСуммы();
...
&НаКлиенте
Процедура ТекстовоеПолеВыбораРежимаЗаполненияПриИзменении(Элемент)
	ЭтаФорма.ИспользоватьПроцентСумму=не ЭтаФорма.ИспользоватьПроцентСумму;
	ИспользоватьПроцентСуммуПриИзменении(Элемент);
КонецПроцедуры

&НаКлиенте
Процедура ПолучитьФайлыИзСвязанныхДокументов(Команда)
	...
	МассивСсылок=МассивВладельцевПрисоединенныхФайлов(Объект.ллл_КодРС);
	
	Если МассивСсылок.Количество()=0 тогда
		
		Сообщить("Отсутствуют связанные документы, к которым можно было бы прикрепить файлы.");
		
	иначе	
		ПараметрыОткрытия=Новый Структура;
		ПараметрыОткрытия.Вставить("ВладелецФайла",Массивссылок);   
		ПараметрыОткрытия.Вставить("ПростаяФорма",истина);
		ПараметрыОткрытия.Вставить("РежимВыбора",ложь);

	
		ОткрытьФОрму("Обработка.ллл_РаботаСФайлами.Форма.ПрисоединенныеФайлы",ПараметрыОткрытия);
...
&НаСервереБезКонтекста
Функция МассивВладельцевПрисоединенныхФайлов(КодРС)
...
|					з.Ссылка
|				ИЗ
|					ВТ_КустПредзаказов КАК з)
|			ИЛИ ЗаявкаНаРасходованиеДенежныхСредств.ДокументОснование.ДокументОснование В
|				(ВЫБРАТЬ
|					з.Ссылка
|				ИЗ
|					ВТ_КустПредзаказов КАК з))
...
|	И СписаниеБезналичныхДенежныхСредств.ДокументОснование В
|			(ВЫБРАТЬ
|				з3.ссылка
|			ИЗ
|				ВТ_3 КАК з3)
...
| Объединить
| Выбрать различные 
|  ВТ_КустПредзаказов.Ссылка
| из                 ВТ_КустПредзаказов как ВТ_КустПредзаказов
|;
|
|////////////////////////////////////////////////////////////////////////////////
|ВЫБРАТЬ РАЗЛИЧНЫЕ
|	ВТ_5.ссылка.Договор КАК Ссылка
|ИЗ
|	ВТ_5 КАК ВТ_5
|
|ОБЪЕДИНИТЬ ВСЕ
...
&НаСервереБезКонтекста
Функция ЗаполнитьВесНоменклатурыВСтроке(Номенклатура)
	
	ЗначенияРеквизитов  = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Номенклатура, "ВесЧислитель, ВесЗнаменатель");
	ВесЧислитель = ЗначенияРеквизитов.ВесЧислитель;
	ВесЗнаменатель = ?(ЗначенияРеквизитов.ВесЗнаменатель=0, 1, ЗначенияРеквизитов.ВесЗнаменатель);
	
    Возврат (ВесЧислитель/ВесЗнаменатель);
