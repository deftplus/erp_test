...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//Вставка
	//Если типЗнч(Параметры.ВладелецФайла)=тип("Массив") тогда 
	//	Если Параметры.ВладелецФайла.Количество()>0 тогда 
	//	ИмяСправочникаХранилищаФайлов = РаботаСФайламиСлужебный.ИмяСправочникаХраненияФайлов(
	//	Параметры.ВладелецФайла[0], "", ЗаголовокОшибки, ОкончаниеОшибки);
	//	
	...
	НастроитьДинамическийСписок("", ЕстьРеквизитСлужебный);
	...
	ИспользоватьПредпросмотр = истина;
...
Процедура ПриОткрытии(Отказ)
	...
	Элементы.ФормаСохранитьКак1.Доступность=истина;
КонецПроцедуры
...
Процедура СохранитьКак(Команда)
	...
	ДанныеФайла = РаботаСФайламиСлужебныйВызовСервера.ДанныеФайлаДляСохранения(Элементы.Список.ДанныеСтроки(ТекущиеДанные).Ссылка, , УникальныйИдентификатор);
	...
	ПараметрыВыполнения.Вставить("УникальныйИдентификатор", неопределено);
    ПараметрыВыполнения.Вставить("ПутьВыбора", Каталог);
	...
	Счетчик=0;	
	пока МассивИмен.Найти(ИмяСРасширением)<>неопределено Цикл
		Счетчик=Счетчик+1;
		ИмяСРасширением=ДанныеФайла.Наименование+"_"+стрЗаменить(строка(Счетчик),Символы.НПП,"")+"."+Расширение;
	...
	ПолноеИмяФайла=Каталог+"\"+ИмяСРасширением;
	...
	НачатьПолучениеФайловССервера(,ПередаваемыеФайлы);
	
	МассивИмен.Добавить(ИмяСРасширением);
...
Процедура СохраняемНаСервере(ДанныеФайла)
...
|	ЛОЖЬ КАК Служебный
|ИЗ
|	Справочник.ЗаказПоставщикуПрисоединенныеФайлы КАК Файлы
|ГДЕ
|	Файлы.ВладелецФайла = &ВладелецФайлов  и не  Файлы.ПометкаУдаления
|
|ОБЪЕДИНИТЬ ВСЕ
|
|ВЫБРАТЬ
|	ЗаявкаНаРасходованиеДенежныхСредствПрисоединенныеФайлы.Ссылка,
|	ЗаявкаНаРасходованиеДенежныхСредствПрисоединенныеФайлы.ПометкаУдаления,
|	ВЫБОР
|		КОГДА ЗаявкаНаРасходованиеДенежныхСредствПрисоединенныеФайлы.ПометкаУдаления = ИСТИНА
|			ТОГДА ЕСТЬNULL(ЗаявкаНаРасходованиеДенежныхСредствПрисоединенныеФайлы.ИндексКартинки, 2) + 1
|		ИНАЧЕ ЕСТЬNULL(ЗаявкаНаРасходованиеДенежныхСредствПрисоединенныеФайлы.ИндексКартинки, 2)
|	КОНЕЦ,
|	ЗаявкаНаРасходованиеДенежныхСредствПрисоединенныеФайлы.Наименование,
|	ВЫРАЗИТЬ(ЗаявкаНаРасходованиеДенежныхСредствПрисоединенныеФайлы.Описание КАК СТРОКА(500)),
|	ЗаявкаНаРасходованиеДенежныхСредствПрисоединенныеФайлы.Автор,
|	ЗаявкаНаРасходованиеДенежныхСредствПрисоединенныеФайлы.ДатаСоздания,
|	ЗаявкаНаРасходованиеДенежныхСредствПрисоединенныеФайлы.Изменил,
|	ВЫРАЗИТЬ(ЗаявкаНаРасходованиеДенежныхСредствПрисоединенныеФайлы.Размер / 1024 КАК ЧИСЛО(10, 0)),
|	ЗаявкаНаРасходованиеДенежныхСредствПрисоединенныеФайлы.ПодписанЭП,
|	ЗаявкаНаРасходованиеДенежныхСредствПрисоединенныеФайлы.Зашифрован,
|	ЗаявкаНаРасходованиеДенежныхСредствПрисоединенныеФайлы.Расширение,
|	ЗаявкаНаРасходованиеДенежныхСредствПрисоединенныеФайлы.ВладелецФайла,
|	ЗаявкаНаРасходованиеДенежныхСредствПрисоединенныеФайлы.ХранитьВерсии,
|	ЛОЖЬ
|ИЗ
...
|ГДЕ
|	ЗаявкаНаРасходованиеДенежныхСредствПрисоединенныеФайлы.ВладелецФайла = &ВладелецФайлов  и не  ЗаявкаНаРасходованиеДенежныхСредствПрисоединенныеФайлы.ПометкаУдаления
|
|ОБЪЕДИНИТЬ
|
|ВЫБРАТЬ
|	Файлы2.Ссылка,
|	Файлы2.ПометкаУдаления,
|	ВЫБОР
|		КОГДА Файлы2.ПометкаУдаления = ИСТИНА
|			ТОГДА ЕСТЬNULL(Файлы2.ИндексКартинки, 2) + 1
|		ИНАЧЕ ЕСТЬNULL(Файлы2.ИндексКартинки, 2)
|	КОНЕЦ,
|	Файлы2.Наименование,
|	ВЫРАЗИТЬ(Файлы2.Описание КАК СТРОКА(500)),
|	Файлы2.Автор,
|	Файлы2.ДатаСоздания,
|	Файлы2.Изменил,
|	ВЫРАЗИТЬ(Файлы2.Размер / 1024 КАК ЧИСЛО(10, 0)),
|	Файлы2.ПодписанЭП,
|	Файлы2.Зашифрован,
|	Файлы2.Расширение,
|	Файлы2.ВладелецФайла,
|	Файлы2.ХранитьВерсии,
|	ЛОЖЬ
|ИЗ
|	Справочник.СписаниеБезналичныхДенежныхСредствПрисоединенныеФайлы КАК Файлы2
|ГДЕ
|	Файлы2.ВладелецФайла = &ВладелецФайлов  и не Файлы2.ПометкаУдаления
|
|ОБЪЕДИНИТЬ
|
|ВЫБРАТЬ
|	Файлы4.Ссылка,
|	Файлы4.ПометкаУдаления,
|	ВЫБОР
|		КОГДА Файлы4.ПометкаУдаления = ИСТИНА
|			ТОГДА ЕСТЬNULL(Файлы4.ИндексКартинки, 2) + 1
|		ИНАЧЕ ЕСТЬNULL(Файлы4.ИндексКартинки, 2)
|	КОНЕЦ,
|	Файлы4.Наименование,
|	ВЫРАЗИТЬ(Файлы4.Описание КАК СТРОКА(500)),
|	Файлы4.Автор,
|	Файлы4.ДатаСоздания,
|	Файлы4.Изменил,
|	ВЫРАЗИТЬ(Файлы4.Размер / 1024 КАК ЧИСЛО(10, 0)),
|	Файлы4.ПодписанЭП,
|	Файлы4.Зашифрован,
|	Файлы4.Расширение,
|	Файлы4.ВладелецФайла,
|	Файлы4.ХранитьВерсии,
|	ЛОЖЬ
|ИЗ
|	РегистрСведений.ОбъектыСвязанныхФайлов КАК ОбъектыСвязанныхФайлов
|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.Файлы КАК Файлы4
|		ПО (ОбъектыСвязанныхФайлов.СвязанныйФайл = Файлы4.Ссылка)
|ГДЕ
|	ОбъектыСвязанныхФайлов.ИмпортированныйОбъект = &ВладелецФайлов
|
|ОБЪЕДИНИТЬ
|
|ВЫБРАТЬ
|	Файлы5.Ссылка,
|	Файлы5.ПометкаУдаления,
|	ВЫБОР
|		КОГДА Файлы5.ПометкаУдаления = ИСТИНА
|			ТОГДА ЕСТЬNULL(Файлы5.ИндексКартинки, 2) + 1
|		ИНАЧЕ ЕСТЬNULL(Файлы5.ИндексКартинки, 2)
|	КОНЕЦ,
|	Файлы5.Наименование,
|	ВЫРАЗИТЬ(Файлы5.Описание КАК СТРОКА(500)),
|	Файлы5.Автор,
|	Файлы5.ДатаСоздания,
|	Файлы5.Изменил,
|	ВЫРАЗИТЬ(Файлы5.Размер / 1024 КАК ЧИСЛО(10, 0)),
|	Файлы5.ПодписанЭП,
|	Файлы5.Зашифрован,
|	Файлы5.Расширение,
|	Файлы5.ВладелецФайла,
|	Файлы5.ХранитьВерсии,
|	ЛОЖЬ
|ИЗ
|	Справочник.ПартнерыПрисоединенныеФайлы КАК Файлы5
|ГДЕ
|	Файлы5.ВладелецФайла = &ВладелецФайлов и не Файлы5.ПометкаУдаления";;
		...
		ТекстЗапроса=стрЗаменить(ТекстЗапроса,".ВладелецФайла = &ВладелецФайлов",".ВладелецФайла В(&ВладелецФайлов) ");	 
		ТекстЗапроса=стрЗаменить(ТекстЗапроса,".ИмпортированныйОбъект = &ВладелецФайлов",".ИмпортированныйОбъект В(&ВладелецФайлов) ");	 
	КонецЕсли;
	...
	Список.Параметры.Элементы[0].Значение=Параметры.ВладелецФайла;
